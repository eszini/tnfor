...
if [ ! -f ./namescom.f90 ]; then
    # Si no existe, lo copia desde ~/wrk/Midas/ABBICAP/
    cp ~/wrk/Midas/ABBICAP/namescom.f90 .
    echo "Archivo namescom.mon copiado al directorio actual."
fi

./tfor -v -opciones=d5 -exec=5 -inp=namescom.f90 -out=nms.mon -ou2=vcb.txt -ou3=vcb2.txt -ou4=que  -log=vsb.txt  > log
