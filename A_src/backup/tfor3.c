					...
					if (!tiene_amper( (*fnp[i]).l ) )
					{
						...
						if (!tiene_multiple_vars(b1))  
						{
							...
							if (!f_stat)
							{
								...
								strcpy(m1,trim_blanks_beg(b1));
								fprintf (hfou4,"( 9) sl sv    %-30.30s |%s|\n",prog_name,m1);

								if (gp_proceed == 9 )
								{
									flag_alloc_ok = 0;

									printf ("HHH1 proceed (t06) SRC: |%s| \n",prog_name);
									grabar_mapa(1,6,prog_name);

									chg_alloc_t06(n_f,i,&add_lines);
					...
					else
					{
						...
						if (!tiene_multiple_vars(m0))
						{
							...
							if (!f_stat)
							{
								...
								strcpy(m1,trim_blanks_beg(m0));
								fprintf (hfou4,"(13) ml sv    %-30.30s |%s|\n",prog_name,m1);
							...
							else
							{
								...
								strcpy(m1,trim_blanks_beg(m0));
								fprintf (hfou4,"(14) ml sv st %-30.30s |%s|\n",prog_name,m1);
						...
						else
						{
							...
							if (!f_stat)
							{
								...
								strcpy(m1,trim_blanks_beg(m0));
								fprintf (hfou4,"(15) ml mv    %-30.30s |%s|\n",prog_name,m1);
							...
							else
							{
								...
								strcpy(m1,trim_blanks_beg(m0));
								fprintf (hfou4,"(16) ml mv st %-30.30s |%s|\n",prog_name,m1);
							}
...
int	grabar_mapa(n_men,tipo,prog_name)
int	n_men;
char	*tipo;
char	*prog_name;
{
	if (n_men == 1)
		fprintf (hfou5,"%s  src:                           |%s|\n","(T06)",prog_name);
	if (n_men == 2)
		fprintf (hfou5,"%s  src:                           |%s|\n","(T08)",prog_name);
...
int	chg_alloc_t06(num_f,nf_alloc,add_lines)
int	num_f;
int	nf_alloc;
int	*add_lines;
{
	...
	{	
		...
		fprintf (hfou5,"lin use va: %6d (%6d - %6d)  |%s|\n",linea_use,pri_l,ult_l,(*fnp[linea_use]).l );
		...
		*add_lines = 0;
		...
		if (!busco_use_allocate(pri_l,ult_l))
		{
			...
			strcpy( (*fnp[linea_use]).l  ,"      use allocate_vars");
...
int	chg_alloc_g01(num_f,nf_alloc,add_lines)
int	num_f;
int	nf_alloc;
int	*add_lines;
{
	...
	{	
		...
		printf ("HHH4 comienza bloque:  %06d (%d) |%s! \n",
...
printf ("nf_alloc .. (%d) |%s| \n",nf_alloc,(*fnp[nf_alloc]).l );
		...
		sprintf (b5,"(\"%s:%03d\",\"%s\",STV_ER)",src_name,1,var_name);
		strcpy( (*fnp[linea_check]).l  ,"      call check_alloc");
