...
! ALLOCATE ARRAYS
         ...
         IF(.NOT. ALLOCATED(Supply_Production_By_Year)) THEN
            allocate(supply_production_by_year(20,narray,0:30),stat=stv_er)
            call check_alloc("coal_main_prg:0001","supply_production_by_year",stv_er)
            allocate(supply_cost_by_year(20,narray,0:30),stat=stv_er)
            call check_alloc("coal_main_prg:0002","supply_cost_by_year",stv_er)
...
!
         IF(.NOT. Allocated(GenericTransportPtrToTG)) THEN
            ALLOCATE(GenericTransportPtrToTG(0:256,20),stat=stv_er) ! TG,1 = paths TG,>1 pointer
            call check_alloc("coal_main_prg:0003","GenericTransportPtrToTG",stv_er)
...
! ALLOCATE ARRAYS
            ...
            IF(.NOT. ALLOCATED(GenericPlantPtrByTG)) THEN
               ALLOCATE(GenericPlantPtrByTG(0:256),stat=stv_er)
               call check_alloc("coal_main_prg:0004","GenericPlantPtrByTG",stv_er)
