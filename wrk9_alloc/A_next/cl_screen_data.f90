...
! OPERATIONS FILE
         ALLOCATE(S_FUELMX(CL_OPTIONS),stat=stv_er)
         call check_alloc("cl_screen_data:0001","S_FUELMX",stv_er)
     
         ALLOCATE(S_PBTUCT(CL_OPTIONS),stat=stv_er)
         call check_alloc("cl_screen_data:0002","S_PBTUCT",stv_er)
     
         ALLOCATE(S_PFESCR(CL_OPTIONS),stat=stv_er)
         call check_alloc("cl_screen_data:0003","S_PFESCR",stv_er)
     
         ALLOCATE(S_SBTUCT(CL_OPTIONS),stat=stv_er)
         call check_alloc("cl_screen_data:0004","S_SBTUCT",stv_er)
     
         ALLOCATE(S_RPS_PROGRAM_NUMBER(CL_OPTIONS),stat=stv_er)
         call check_alloc("cl_screen_data:0005","S_RPS_PROGRAM_NUMBER",stv_er)
     
         ALLOCATE(S_RPS_PERCENT(CL_OPTIONS),stat=stv_er)
         call check_alloc("cl_screen_data:0006","S_RPS_PERCENT",stv_er)
     
         ALLOCATE(S_SFESCR(CL_OPTIONS),stat=stv_er)
         call check_alloc("cl_screen_data:0007","S_SFESCR",stv_er)
     
         ALLOCATE(S_FUELADJ_IN(CL_OPTIONS),stat=stv_er)
         call check_alloc("cl_screen_data:0008","S_FUELADJ_IN",stv_er)
     
         ALLOCATE(S_MNRATE(CL_OPTIONS,12),stat=stv_er)
         call check_alloc("cl_screen_data:0009","S_MNRATE",stv_er)
     
         ALLOCATE(S_INTER_BLOCKS(CL_OPTIONS,3),stat=stv_er)
         call check_alloc("cl_screen_data:0010","S_INTER_BLOCKS",stv_er)
     
         ALLOCATE(S_FUELADJ(CL_OPTIONS),stat=stv_er)
         call check_alloc("cl_screen_data:0011","S_FUELADJ",stv_er)
     
         ALLOCATE(S_EFOR(CL_OPTIONS),stat=stv_er)
         call check_alloc("cl_screen_data:0012","S_EFOR",stv_er)
     
         ALLOCATE(S_VCPMWH_IN(CL_OPTIONS),stat=stv_er)
         call check_alloc("cl_screen_data:0013","S_VCPMWH_IN",stv_er)
     
         ALLOCATE(S_OMESCR(CL_OPTIONS),stat=stv_er)
         call check_alloc("cl_screen_data:0014","S_OMESCR",stv_er)
     
         ALLOCATE(S_FIXED_COST_IN(CL_OPTIONS),stat=stv_er)
         call check_alloc("cl_screen_data:0015","S_FIXED_COST_IN",stv_er)
     
         ALLOCATE(S_FIXED_COST_ESCALATOR(CL_OPTIONS),stat=stv_er)
         call check_alloc("cl_screen_data:0016","S_FIXED_COST_ESCALATOR",stv_er)
     
         ALLOCATE(S_MONTHLY_CAPACITY_POINTER(CL_OPTIONS),stat=stv_er)
         call check_alloc("cl_screen_data:0017","S_MONTHLY_CAPACITY_POINTER",stv_er)
     
         ALLOCATE(S_HR_FACTOR(CL_OPTIONS),stat=stv_er)
         call check_alloc("cl_screen_data:0018","S_HR_FACTOR",stv_er)
     
         ALLOCATE(S_INPUT_MW(2,CL_OPTIONS),stat=stv_er)
         call check_alloc("cl_screen_data:0019","S_INPUT_MW",stv_er)
     
         ALLOCATE(S_CAP_PLANNING_FAC(CL_OPTIONS),stat=stv_er)
         call check_alloc("cl_screen_data:0020","S_CAP_PLANNING_FAC",stv_er)
     
         allocate(s_coeff(3,cl_options),stat=stv_er)
         call check_alloc("cl_screen_data:0021","s_coeff",stv_er)
         allocate(p_so2(cl_options),stat=stv_er)
         call check_alloc("cl_screen_data:0022","p_so2",stv_er)
         allocate(p_nox(cl_options),stat=stv_er)
         call check_alloc("cl_screen_data:0023","p_nox",stv_er)
         allocate(p_particulates(cl_options),stat=stv_er)
         call check_alloc("cl_screen_data:0024","p_particulates",stv_er)
         allocate(p_emis_oth2(cl_options),stat=stv_er)
         call check_alloc("cl_screen_data:0025","p_emis_oth2",stv_er)
         allocate(p_emis_oth3(cl_options),stat=stv_er)
         call check_alloc("cl_screen_data:0026","p_emis_oth3",stv_er)
     
    
    
    
    
         ALLOCATE(S_ANNUAL_CL_FIXED_COST(CL_OPTIONS),stat=stv_er)
         call check_alloc("cl_screen_data:0027","S_ANNUAL_CL_FIXED_COST",stv_er)
     
         ALLOCATE(S_TRANSACTION_GROUP_ID(CL_OPTIONS),stat=stv_er)
         call check_alloc("cl_screen_data:0028","S_TRANSACTION_GROUP_ID",stv_er)
     
         ALLOCATE(S_EMISSION_MARKET_LINK(CL_OPTIONS),stat=stv_er)
         call check_alloc("cl_screen_data:0029","S_EMISSION_MARKET_LINK",stv_er)
     
         ALLOCATE(S_ANNUAL_CL_FIXED_COST_ESC(CL_OPTIONS),stat=stv_er)
         call check_alloc("cl_screen_data:0030","S_ANNUAL_CL_FIXED_COST_ESC",stv_er)
     
         ALLOCATE(P_FUEL_DELIVERY(CL_OPTIONS),stat=stv_er)
         call check_alloc("cl_screen_data:0031","P_FUEL_DELIVERY",stv_er)
     
         ALLOCATE(START_UP_COSTS(CL_OPTIONS),stat=stv_er)
         call check_alloc("cl_screen_data:0032","START_UP_COSTS",stv_er)
     
         ALLOCATE(START_UP_COSTS_ESCALATION(CL_OPTIONS),stat=stv_er)
         call check_alloc("cl_screen_data:0033","START_UP_COSTS_ESCALATION",stv_er)
     
         ALLOCATE(MIN_DOWN_TIME(CL_OPTIONS),stat=stv_er)
         call check_alloc("cl_screen_data:0034","MIN_DOWN_TIME",stv_er)
     
         ALLOCATE(MIN_UP_TIME(CL_OPTIONS),stat=stv_er)
         call check_alloc("cl_screen_data:0035","MIN_UP_TIME",stv_er)
     
         ALLOCATE(P_FUEL_DELIVERY_2(CL_OPTIONS),stat=stv_er)
         call check_alloc("cl_screen_data:0036","P_FUEL_DELIVERY_2",stv_er)
     
         ALLOCATE(P_FUEL_DELIVERY_3(CL_OPTIONS),stat=stv_er)
         call check_alloc("cl_screen_data:0037","P_FUEL_DELIVERY_3",stv_er)
     
         ALLOCATE(S_FUEL_DELIVERY(CL_OPTIONS),stat=stv_er)
         call check_alloc("cl_screen_data:0038","S_FUEL_DELIVERY",stv_er)
     
         ALLOCATE(S_FUEL_DELIVERY_2(CL_OPTIONS),stat=stv_er)
         call check_alloc("cl_screen_data:0039","S_FUEL_DELIVERY_2",stv_er)
     
         ALLOCATE(s_fuel_delivery_3(CL_OPTIONS),stat=stv_er)
         call check_alloc("cl_screen_data:0040","s_fuel_delivery_3",stv_er)
     
         allocate(apply_nox_season_date(cl_options),stat=stv_er)
         call check_alloc("cl_screen_data:0041","apply_nox_season_date",stv_er)
         allocate(s_primary_mover(cl_options),stat=stv_er)
         call check_alloc("cl_screen_data:0042","s_primary_mover",stv_er)
         allocate(s_unit_gas_region_index(cl_options),stat=stv_er)
         call check_alloc("cl_screen_data:0043","s_unit_gas_region_index",stv_er)
         allocate(state_index(cl_options),stat=stv_er)
         call check_alloc("cl_screen_data:0044","state_index",stv_er)
         allocate(nox_season_date(cl_options),stat=stv_er)
         call check_alloc("cl_screen_data:0045","nox_season_date",stv_er)
         allocate(nox_control_percent(cl_options),stat=stv_er)
         call check_alloc("cl_screen_data:0046","nox_control_percent",stv_er)
         allocate(primary_mover_str(cl_options),stat=stv_er)
         call check_alloc("cl_screen_data:0047","primary_mover_str",stv_er)
         allocate(co2_control_percent(cl_options),stat=stv_er)
         call check_alloc("cl_screen_data:0048","co2_control_percent",stv_er)
         allocate(hg_control_percent(cl_options),stat=stv_er)
         call check_alloc("cl_screen_data:0049","hg_control_percent",stv_er)
         allocate(other3_control_percent(cl_options),stat=stv_er)
         call check_alloc("cl_screen_data:0050","other3_control_percent",stv_er)
         allocate(nox_control_date(cl_options),stat=stv_er)
         call check_alloc("cl_screen_data:0051","nox_control_date",stv_er)
         allocate(co2_control_date(cl_options),stat=stv_er)
         call check_alloc("cl_screen_data:0052","co2_control_date",stv_er)
         allocate(hg_control_date(cl_options),stat=stv_er)
         call check_alloc("cl_screen_data:0053","hg_control_date",stv_er)
         allocate(other3_control_date(cl_options),stat=stv_er)
         call check_alloc("cl_screen_data:0054","other3_control_date",stv_er)
         allocate(monthly_must_run_vector(cl_options),stat=stv_er)
         call check_alloc("cl_screen_data:0055","monthly_must_run_vector",stv_er)
         allocate(sox_control_percent(cl_options),stat=stv_er)
         call check_alloc("cl_screen_data:0056","sox_control_percent",stv_er)
         allocate(sox_control_date(cl_options),stat=stv_er)
         call check_alloc("cl_screen_data:0057","sox_control_date",stv_er)
         allocate(sox_vom(cl_options),stat=stv_er)
         call check_alloc("cl_screen_data:0058","sox_vom",stv_er)
         allocate(sox_fom(cl_options),stat=stv_er)
         call check_alloc("cl_screen_data:0059","sox_fom",stv_er)
         allocate(nox_vom(cl_options),stat=stv_er)
         call check_alloc("cl_screen_data:0060","nox_vom",stv_er)
         allocate(nox_fom(cl_options),stat=stv_er)
         call check_alloc("cl_screen_data:0061","nox_fom",stv_er)
         allocate(co2_vom(cl_options),stat=stv_er)
         call check_alloc("cl_screen_data:0062","co2_vom",stv_er)
         allocate(co2_fom(cl_options),stat=stv_er)
         call check_alloc("cl_screen_data:0063","co2_fom",stv_er)
         allocate(hg_vom(cl_options),stat=stv_er)
         call check_alloc("cl_screen_data:0064","hg_vom",stv_er)
         allocate(hg_fom(cl_options),stat=stv_er)
         call check_alloc("cl_screen_data:0065","hg_fom",stv_er)
         allocate(other3_vom(cl_options),stat=stv_er)
         call check_alloc("cl_screen_data:0066","other3_vom",stv_er)
         allocate(other3_fom(cl_options),stat=stv_er)
         call check_alloc("cl_screen_data:0067","other3_fom",stv_er)
