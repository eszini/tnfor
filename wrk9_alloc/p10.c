...
// Función para extraer variables de una sentencia allocate y formatearlas con el número de línea
void process_allocate_statement(const char *allocate_stmt, char **output, int *count, int line_number) {
    ...
    while (*token) {
        ...
        if ((*token == ',' && paren_count == 0)) { 
            ...
            if (inside_var && strlen(var) > 0) {
                snprintf(output[*count], MAX_LINE, "%04d,allocate(%s, stat=%s)", line_number, var, stat_var);
                (*count)++;
            }
...
int main() {
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        *end = '\0';
        ...
        int count = 0;
        process_allocate_statement(start, alloc_statements, &count, line_number);
