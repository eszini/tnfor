...
void process_allocate_statement(const char *allocate_stmt, char **output, int *count) {
    ...
    *end = '\0'; // Reemplazar con terminador de string
    ...
    // Verificar si hay un `stat=variable`
    ...
    // Procesar variables considerando estructuras con paréntesis
    char *token = start;
    int paren_count = 0;
    char var[MAX_LINE] = {0};
    *count = 0;
    ...
    while (*token) {
        ...
        if (*token == ',' && paren_count == 0) { 
            // Si encontramos una coma en el nivel base, terminamos una variable
            snprintf(output[*count], MAX_LINE, "allocate(%s, stat=%s)", var, stat_var);
            (*count)++;
            var[0] = '\0'; // Reiniciar buffer para la siguiente variable
        } else {
            // Construimos la variable preservando su estructura completa
            strncat(var, token, 1);
        }
        token++;
    }
