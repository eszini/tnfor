...
!
      REAL (KIND=4) :: R_CASH_VALUES(0:12,*),R_INCOME_VALUES(0:12,*),R_BALANCE_SHEET_VALUES(0:12,*), &
            R_MONTHLY_NUC_DECOM_CASH_VALUES(0:12),R_MONTHLY_NUC_DECOM_EARNINGS_ADDEM(0:12), &
            R_MONTHLY_POST_RETIREMENT_CASH_PAYMENTS(0:12),MONTHLY_CASH_TO_POST_RETIREMENT(0:12), &
            MONTHLY_CASH_TO_MEMBER_ACCRUED_REVS_BAL(0:12),R_MONTHLY_RETIREE_MEDICAL_CASH_PAYMENTS(0:12), &
            R_MONTHLY_RETIREMENT_FUND_EARNINGS_ADJ(0:12),R_MONTHLY_BOOK_DEPRECIATION(0:12)
!
...
!
      REAL (KIND=4) :: R_SRP_TARGET_SALES,R_GRE_PRINCIPAL_MANUAL_ADJS,R_GRE_INTEREST_PAYMENTS_MANUAL_ADJS, &
...
!
      ...
      REAL (KIND=4) :: R_NucDecom_Discount_Rate,R_NucDecom_Interest_Accretion,R_ARO_Cash_Payment,R_ARO_Trust_Cash_Payment, &
             R_MO_NucDecom_Discount_Rate(0:12),R_MO_NucDecom_Interest_Accretion(0:12),R_MO_ARO_Cash_Payment(0:12), &
      ...
      REAL (KIND=4) :: R_STD_RATE,R_MONTHLY_STD_RATE(0:12),R_RETURN_ON_ST_INVEST,R_INTEREST_ON_CUSTOMER_DEPOSITS,R_RETURN_ON_LTI, &
             ...
             R_MONTHLY_RETIREMENT_FUND_RATE(0:12),R_MONTHLY_NUC_DECOM_RATE(0:12),OCI_MONTHLY_RETIREMENT_FUND_RATE(0:12), &
             OCI_MONTHLY_NUC_DECOM_RATE(0:12)
      REAL (KIND=4) :: CASH_TO_CASH,CASH_TO_POST_RETIRE_MEDICAL_BAL,CASH_TO_ACCRUED_MEMBER_REVENUES_BAL
!
...
!
      ...
      RETURN
!**********************************************************************
      ENTRY RETURN_MONTHLY_FASB_ADDENDUMS(R_YR,R_CLASS,R_MO_NucDecom_Discount_Rate,R_MO_NucDecom_Interest_Accretion, &
         ...
         IF(R_CLASS <= MAX_ASSET_CLASS_NUM) THEN
            ...
            IF(ASSET_CLASS > 0 .OR. R_CLASS == 0) THEN 
               IF(ALLOCATED(FASB143_ARO_VALUE)) THEN
                  ...
                  R_MO_NucDecom_Interest_Accretion = FASB143_ARO_VALUE(:,YR,ASSET_CLASS,2)
...
!
      ...
      RETURN
!**********************************************************************
      ENTRY RETURN_CASH_TRANSFERS(R_CLASS,R_YR,CASH_TO_CASH,CASH_TO_POST_RETIRE_MEDICAL_BAL,CASH_TO_ACCRUED_MEMBER_REVENUES_BAL)
!**********************************************************************
!
         CASH_TO_CASH = 0.
         ...
         IF(R_CLASS <= MAX_ASSET_CLASS_NUM .AND. ALLOCATED(CASH_MONTHLY_VALUES)) THEN
            ...
            IF(ASSET_CLASS > 0 .OR. R_CLASS == 0) THEN 
               ...
               CASH_TO_ACCRUED_MEMBER_REVENUES_BAL = CASH_MONTHLY_VALUES(0,YR,ASSET_CLASS,3)    ! Cash to Accrued Member Revenue
            ENDIF
         ENDIF
      RETURN
!**********************************************************************
...
!
         IF(ALLOCATED(FUNDS_EARNINGS)) THEN
            IF(R_CLASS <= MAX_ASSET_CLASS_NUM) THEN
               ...
               IF(ASSET_CLASS > 0 .OR. R_CLASS == 0) THEN 
                     ...
                     ELSE
                        CALL RETURN_MONTHLY_DECOMMISIONING_VECTOR_RATES(RATE_FOUND,R_MONTHLY_NUC_DECOM_RATE)
                     ...
                     ELSE
                        CALL RETURN_MONTHLY_RETIREMENT_VECTOR_RATES(RATE_FOUND,R_MONTHLY_RETIREMENT_FUND_RATE)
                     ...
                     ELSE
                        CALL RETURN_MONTHLY_CUSTOMER_VECTOR_RATES(RATE_FOUND,R_MONTHLY_CUST_DEPOSITS_RATE)
      ...
      RETURN
!***********************************************************************
      ENTRY RETURN_MONTHLY_FUNDS_EARNING_RATES(R_YR,R_CLASS,R_INTEREST_ON_CUSTOMER_DEPOSITS,R_RETURN_RETIREMENT_FUND, &
                                      R_NUCL_DECOM_FUND_RETURN,R_MONTHLY_CUST_DEPOSITS_RATE,R_MONTHLY_RETIREMENT_FUND_RATE, &
                                      OCI_MONTHLY_RETIREMENT_FUND_RATE,R_MONTHLY_NUC_DECOM_RATE,OCI_MONTHLY_NUC_DECOM_RATE)
!***********************************************************************
!
         ...
         IF(ALLOCATED(FUNDS_EARNINGS)) THEN
            IF(R_CLASS <= MAX_ASSET_CLASS_NUM) THEN
               ...
               IF(ASSET_CLASS > 0 .OR. R_CLASS == 0) THEN 
                     ...
                     ENDIF
                     OCI_MONTHLY_RETIREMENT_FUND_RATE(:) = FUNDS_EARNINGS(:,YR,ASSET_CLASS,11)
               ...
               DO MO= 0, 12
                  ...
                  R_MONTHLY_RETIREMENT_FUND_RATE(MO) = R_MONTHLY_RETIREMENT_FUND_RATE(MO)/D12 
                  R_MONTHLY_CUST_DEPOSITS_RATE(MO) = R_MONTHLY_CUST_DEPOSITS_RATE(MO)/D12
                  OCI_MONTHLY_RETIREMENT_FUND_RATE(MO) = OCI_MONTHLY_RETIREMENT_FUND_RATE(MO)/D12
...
!
         ...
         IF(ALLOCATED(SALE_REMOVAL)) THEN
            IF(R_CLASS <= MAX_ASSET_CLASS_NUM) THEN
               ...
               ENDIF
               IF(ASSET_CLASS > 0 .OR. R_CLASS == 0) THEN 
                  R_NET_REMOVAL = R_NET_REMOVAL - (SALE_REMOVAL(0,YR,ASSET_CLASS,1) + SALE_REMOVAL(0,YR,ASSET_CLASS,8) - & ! Cost of Removal
...
!
         IF(ALLOCATED(OTHER_VALUE)) THEN
            IF(R_CLASS <= MAX_ASSET_CLASS_NUM) THEN
               ...
               ENDIF
               IF(ASSET_CLASS > 0 .OR. R_CLASS == 0) THEN 
                  IPL_ELECTRIC_PLAN_FUEL_COSTS(:) = IPL_ELECTRIC_PLAN_FUEL_COSTS(:) + OTHER_VALUE(:,YR,ASSET_CLASS,4) ! ElectPlan Fuel)
...
!
         ...
         IF(ALLOCATED(OTHER_VALUE)) THEN
            ...
            DO ASSET_CLASS = 1, NUM_OF_ASSET_CLASSES
               R_SRP_TARGET_SALES = R_SRP_TARGET_SALES + OTHER_VALUE(0,YR,ASSET_CLASS,SRP Target Sales)
            ENDDO
         ENDIF
      RETURN
!***********************************************************************
      ENTRY RETURN_GRE_DSC_ADJUSTMENTS(R_YR,R_GRE_INTEREST_PAYMENTS_MANUAL_ADJS,R_GRE_PRINCIPAL_MANUAL_ADJS)
!***********************************************************************
!
         ...
         IF(ALLOCATED(OTHER_VALUE)) THEN
            YR = R_YR
            DO ASSET_CLASS = 1, NUM_OF_ASSET_CLASSES
               R_GRE_PRINCIPAL_MANUAL_ADJS = OTHER_VALUE(0,YR,ASSET_CLASS,GRE DSC Principal Pay Adj)
               R_GRE_INTEREST_PAYMENTS_MANUAL_ADJS = OTHER_VALUE(0,YR,ASSET_CLASS,GRE DSC Interest Pay Adj)
            ENDDO
         ENDIF
      RETURN
!**********************************************************************
      ENTRY RETURN_MONTHLY_NUC_DECOM_CASH_ADDENDUMS(R_CLASS,R_YR,R_MONTHLY_NUC_DECOM_CASH_VALUES,R_MONTHLY_NUC_DECOM_EARNINGS_ADDEM)
!**********************************************************************
!
         IF(R_CLASS <= MAX_ASSET_CLASS_NUM) THEN
            IF(R_CLASS == 0) THEN
            ...
            IF(ASSET_CLASS > 0 .OR. R_CLASS == 0) THEN
               ...
               IF(ALLOCATED(FUNDS_EARNINGS)) R_MONTHLY_NUC_DECOM_EARNINGS_ADDEM = FUNDS_EARNINGS(:,R_YR,ASSET_CLASS,1)
            ENDIF
         ENDIF
      RETURN
!**********************************************************************
      ENTRY RETURN_MONTHLY_RETIREMENT_CASH_ADDENDUMS(R_CLASS,R_YR,R_MONTHLY_POST_RETIREMENT_CASH_PAYMENTS, &
                               R_MONTHLY_RETIREE_MEDICAL_CASH_PAYMENTS,MONTHLY_CASH_TO_POST_RETIREMENT, &
...
!
         IF(R_CLASS <= MAX_ASSET_CLASS_NUM) THEN
            ...
            IF(ASSET_CLASS > 0 .OR. R_CLASS == 0) THEN
               IF(ALLOCATED(PAYMENTS)) THEN 
                  R_MONTHLY_POST_RETIREMENT_CASH_PAYMENTS = PAYMENTS(:,R_YR,ASSET_CLASS,3)
                  R_MONTHLY_RETIREE_MEDICAL_CASH_PAYMENTS = PAYMENTS(:,R_YR,ASSET_CLASS,4)
               ENDIF
               IF(ALLOCATED(FUNDS_EARNINGS)) R_MONTHLY_RETIREMENT_FUND_EARNINGS_ADJ = FUNDS_EARNINGS(:,R_YR,ASSET_CLASS,2)
            ...
            IF(R_CLASS <= MAX_ASSET_CLASS_NUM) THEN
               ...
               IF(ASSET_CLASS > 0 .OR. R_CLASS == 0) THEN
                  DO MO = 0, 12
                     ...
                     R_CASH_VALUES(MO,wvpa_cash_2_accrd_mbr_revnue) = CASH_MONTHLY_VALUES(MO,YR,ASSET_CLASS,3) ! CASH TO ACCRUED MEMBER BAL
                     R_CASH_VALUES(MO,csh_2_post_retiremt_paymentss) = CASH_MONTHLY_VALUES(MO,YR,ASSET_CLASS,2) ! CASH TO RETIREMENT PAYMENTS
         ...
         IF(R_CLASS <= MAX_ASSET_CLASS_NUM) THEN
            ...
            IF(ASSET_CLASS > 0 .OR. R_CLASS == 0) THEN
               ...
               IF(ALLOCATED(TRANSFER_VALUE)) THEN
                  ...
                  R_TRANSFER_VARIABLES(MO,monthly_coi_retained_earnings_bal) = TRANSFER_VALUE(MO,YR,ASSET_CLASS,58) 
                  ...
                  R_TRANSFER_VARIABLES(MO,monthly_nucl_decommiss_liability) = TRANSFER_VALUE(MO,YR,ASSET_CLASS,44)
                  ...
                  DO MO= 2, 12
                     ...
                     R_TRANSFER_VARIABLES(MO,monthly_coi_retained_earnings_bal) = R_TRANSFER_VARIABLES(MO, &
                                     monthly_coi_retained_earnings_bal) + TRANSFER_VALUE(MO,YR,ASSET_CLASS,58)
                     ...
                     R_TRANSFER_VARIABLES(MO,monthly_nucl_decommiss_liability) = R_TRANSFER_VARIABLES(MO, &
                                     monthly_nucl_decommiss_liability) + TRANSFER_VALUE(MO,YR,ASSET_CLASS,44)
