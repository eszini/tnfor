...
c      INTEGER*2 R_EXPENSE_CLASS_POINTERS(*)
     ...
     +            ACCOUNT_ACTIVE_STATUS
      CHARACTER*3 EXPENSE_COLLECTION
...
c                  IF(FILE_ID == 0) THEN
     ...
     +                     WVPA_COMPANY_ID      ! 224
c     +                     INTRA_ACCT_SUBCLASS ! after budget
c                  ELSE
c                  ENDIF
C
c	            IF(DELETE < 8) THEN
c	               CALL SET_ASSET_CLASSES(ASSET_CLASS_NUM,
c     +                                   NUMBER_OF_BC_EXPENSE_CLASSES,
c     +   	                             MAX_BC_EXPENSE_CLASS_ID_NUM,
c     +                                   TEMP_ASSET_CLASS_POINTER)
c                     IF(INTRA_COMPANY_TRANSACTION == 'Y') THEN
c	                  CALL SET_ASSET_CLASSES(INTRA_ASSET_CLASS_ID,
c     +                                     NUMBER_OF_BC_EXPENSE_CLASSES,
c     +   	                               MAX_BC_EXPENSE_CLASS_ID_NUM,
c     +                                     TEMP_ASSET_CLASS_POINTER)
c                     ENDIF
...
c            ENDFILE(11)
            CLOSE(11)
c            IF(MAX_BC_EXPENSE_CLASS_ID_NUM > 0) THEN
c               ALLOCATE(EXPENSE_BC_ASSET_CLASS_POINTER
c     +                                    (MAX_BC_EXPENSE_CLASS_ID_NUM))
c
cc               CALL CMOVE(TEMP_ASSET_CLASS_POINTER,
cc     +                      EXPENSE_BC_ASSET_CLASS_POINTER,
cc     +                               INT(2*MAX_BC_EXPENSE_CLASS_ID_NUM))
c               DO I = 1, MAX_BC_EXPENSE_CLASS_ID_NUM
c                  EXPENSE_BC_ASSET_CLASS_POINTER(I) =
c     +                                       TEMP_ASSET_CLASS_POINTER(I)
c               ENDDO
c            ENDIF
c            DEALLOCATE(TEMP_ASSET_CLASS_POINTER)
...
c      CALL CINITW(TEMP_ASSET_CLASS_POINTER,INT(1024),INT2(0))
     ...
     +                            CASH_VALUES_MONTHLY, ! 198-221
     +                            CASH_VALUES_TYPE     ! 222-223
c     +                            INTRA_ACCT_SUBCLASS, ! 59
            ENDIF
C
c	      IF(DELETE < 8) THEN
c	         CALL SET_ASSET_CLASSES(ASSET_CLASS_NUM,
c     +                                NUMBER_OF_OL_EXPENSE_CLASSES,
c     +	                             MAX_OL_EXPENSE_CLASS_ID_NUM,
c     +                                TEMP_ASSET_CLASS_POINTER)
c               IF(INTRA_COMPANY_TRANSACTION == 'Y') THEN
c	            CALL SET_ASSET_CLASSES(INTRA_ASSET_CLASS_ID,
c     +                                   NUMBER_OF_OL_EXPENSE_CLASSES,
...
c      IF(ALLOCATED(EXPENSE_OL_ASSET_CLASS_POINTER))
c     +                        DEALLOCATE(EXPENSE_OL_ASSET_CLASS_POINTER)
c      IF(MAX_OL_EXPENSE_CLASS_ID_NUM > 0) THEN
c         ALLOCATE(EXPENSE_OL_ASSET_CLASS_POINTER
c     +                                    (MAX_OL_EXPENSE_CLASS_ID_NUM))
cc         CALL CMOVE(TEMP_ASSET_CLASS_POINTER,
cc     +                   EXPENSE_OL_ASSET_CLASS_POINTER,
cc     +                              INT(2*MAX_OL_EXPENSE_CLASS_ID_NUM))
c         DO I = 1, MAX_OL_EXPENSE_CLASS_ID_NUM
c            EXPENSE_OL_ASSET_CLASS_POINTER(I) =
c     +                                       TEMP_ASSET_CLASS_POINTER(I)
c         ENDDO
c      ENDIF
c      DEALLOCATE(TEMP_ASSET_CLASS_POINTER)
...
C
C***********************************************************************
c      ENTRY OPEN_EX_OUT_FILE(R_UNIT_NUM)
cC***********************************************************************
c         OPEN(R_UNIT_NUM,FILE=TRIM(OUTPUT_DIRECTORY())//EXPENOL//
c     +                             'EX_AST.BIN',ACCESS='DIRECT',RECL=64)
c      RETURN
C
C***********************************************************************
...
C***********************************************************************
      ...
      RETURN
C***********************************************************************
c      ENTRY RETURN_NUM_OF_EXPENSE_CLASSES(R_NUM_OF_EXPENSE_CLASSES,
c     +                                       R_MAX_EXPENSE_CLASS_NUM)
cC***********************************************************************
c         IF(EXPENSE_OVERLAYS_ACTIVE) THEN ! EXPENOL == 'OL') THEN
c            R_NUM_OF_EXPENSE_CLASSES = NUMBER_OF_OL_EXPENSE_CLASSES
c            R_MAX_EXPENSE_CLASS_NUM = MAX_OL_EXPENSE_CLASS_ID_NUM
c         ELSE
c            R_NUM_OF_EXPENSE_CLASSES = NUMBER_OF_BC_EXPENSE_CLASSES
c            R_MAX_EXPENSE_CLASS_NUM = MAX_BC_EXPENSE_CLASS_ID_NUM
c         ENDIF
c      RETURN
cC***********************************************************************
c      ENTRY RETURN_EXPENSE_CLASS_POINTER(R_EXPENSE_CLASS_POINTERS)
cC***********************************************************************
c         IF(EXPENSE_OVERLAYS_ACTIVE) THEN ! EXPENOL == 'OL') THEN
cc            CALL CMOVE(EXPENSE_OL_ASSET_CLASS_POINTER,
cc     +                        R_EXPENSE_CLASS_POINTERS,
cc     +                              INT(2*MAX_OL_EXPENSE_CLASS_ID_NUM))
c            DO I = 1, MAX_OL_EXPENSE_CLASS_ID_NUM
c               R_EXPENSE_CLASS_POINTERS(I) = 
c     +                                 EXPENSE_OL_ASSET_CLASS_POINTER(I)
c     +                                       
c            ENDDO
c         ELSE
cc            CALL CMOVE(EXPENSE_BC_ASSET_CLASS_POINTER,
cc     +                        R_EXPENSE_CLASS_POINTERS,
cc     +                              INT(2*MAX_BC_EXPENSE_CLASS_ID_NUM))
c            DO I = 1, MAX_BC_EXPENSE_CLASS_ID_NUM
c               R_EXPENSE_CLASS_POINTERS(I) = 
...
C
     ...
     +             INTRA_ACCT_SUBCLASS ! after budget
      ...
      CHARACTER*3 EXPENSE_COLLECTION,R_EXPENSE_COLLECTION
...
C
      REAL*4 ALLOC_EXPEN(:),ALLOC_CASH_EXPEN(:),
     +     READ_EXPEN(:),
...
!      
c      SAVE ASSET_CLASS_POINTER,MAX_ASSET_CLASS_NUM
c      SAVE NUM_OF_ASSET_CLASSES,
c     +     FINANCIAL_SIMULATION_YEARS,
c     +     EXPENSES_MONTHLY,REVENUES_MONTHLY,CASH_MONTHLY,
c     +     CASH_REV_EXP_MONTHLY,,
c     +     BUDGET_EXPENSE
c      SAVE SECONDARY_SALES_NOT_IN_RATES,
c     +     OTHER_REVENUES_NOT_IN_RATES
C
      LF95 = LAHEY_LF95()
...
C
C GREG, THE FOLLOWING VARIABLES WERE ADDED.
C
C      INTEGER (KIND=2) :: START_YR
C      REAL (KIND=4) :: AVERAGE_MULTIPLIER
!
               SCENARIO_INDEX =
...
C ALLOCATOTAL COMPANY AND ASSET CLASSES
     ...
     +                                  TAX_TYPE,
     +                                  INTRA_ACCT_SUBCLASS)
C    +                                  INTRA_EXPENSE_COLLECTION) ! NEED THIS VALUE
     ...
     +                                  TAX_TYPE,
     +                                  ACCT_SUBCLASS)
C    +                                  EXPENSE_COLLECTION) ! NEED THIS FOR BTL REVERSALS
                  IF(CASH_TYPE /= 'N') THEN
...
C
     ...
     +                                              R_TAX_TYPE,SubClass)
            ...
            ELSE 
               DO YR = 0, FINANCIAL_SIMULATION_YEARS
                  REVENUES_MONTHLY(0,YR,ASSET_CLASS,Adjustment Clause,                   
     +                                            R_TAX_TYPE,SubClass) =
     +                       R_ANNUAL_EXPENSES(YR)/(1.-OPREV_TAX_RATE)
     +                       + REVENUES_MONTHLY(0,YR,ASSET_CLASS,
     +                                     Adjustment Clause,                   
...
C
C NOTE: ONLY CASH RECEIPTS AND CASH PAYMENTS ARE USED ELSEWHERE IN THE CODE
C IF REVENUE AND EXPENSE CASH IS USED THEN MODIFY CASH_POSITION TO NOT INCLUDE
C NON-CASH ITEMS AND TAXES. MOVE CASH_POSITION HERE AND CHECK INCOME POSITON.
C MSG 8/1/04
     ...
     +               CASH_STATEMENT_POSITION(R_ACCOUNT_CLASSIFICATION)
         ELSE
            IF(INDEX(R_ACCOUNT_CLASSIFICATION,'BTL') /= 0) THEN
               CASH_POS = Cash_BTL_Expenses ! BTL Expenses + LAST_INCOME_LINE ! lumps all BTL cash in one line
            ELSE
               CASH_POS =
     +                 CASH_STATEMENT_POSITION(R_ACCOUNT_CLASSIFICATION)
            ENDIF
...
C
     ...
     +                                                     BTL Expenses, ! = 28,
     +                                                TAX_TYPE,SubClass)
C     +                         + EXPENSES_MONTHLY(PERIOD,YR,ASSET_CLASS,
C     +                                                   BTL Lease Cash, ! = 10,
C     +                                                TAX_TYPE,SubClass)
...
C
               ...
               DO DATA_POS = Cash_Base_Rates, Cash_Government
                  IF(DATA_POS == Cash_Unbilled_Revenues) CYCLE
                  IF(DATA_POS == Cash_Deferred_Revenues) CYCLE
...
C
               YR = R_YR
               INC_MONTH_VARS(:,Monthly_Exp_File_Property_Taxes) =
     +                 INC_MONTH_VARS(:,Monthly_Exp_File_Property_Taxes)
...
C
               INC_MONTH_VARS(:,Monthly_Exp_File_Other_Taxes) =
     +               INC_MONTH_VARS(:,Monthly_Exp_File_Other_Taxes)
...
C
     ...
     +                                                       2,SubClass)
               ENDIF
               MONTH_VARS(:,Monthly_Gas_Wholesale_Revenues) =
     +                   MONTH_VARS(:,Monthly_Gas_Wholesale_Revenues)
     +                   + REVENUES_MONTHLY(:,YR,ASSET_CLASS,
...
C***********************************************************************
C
C        VOID_INT2 = RETURN_MONTHY_CL_CASH_REVENUES(R_CLASS,MONTH_VARS)
C        CALL RETURN_MONTHY_RC_CASH_REVENUES(R_CLASS,MONTH_VARS)
C
         TEMP_CASH_VARS = 0.
     ...
     +                                         R_CLASS,
     +                                         MONTH_VARS)
c         IF(WVPA()) THEN  ! TEMP FIX ON PP SPLIT 5/18/04
c            TEMP_CASH_VARS(:,Cash Purchased Power) =
c     +            TEMP_CASH_VARS(:,Cash Purchased Power)
c     +            + TEMP_CASH_VARS(:,Cash WVPA Non Member Purchases)
c         ENDIF
C
...
C
         CALL MONTHLY_SERVICE_TRANS_CASH(R_CLASS,TEMP_CASH_VARS) !10/18/99 ASSUME NO LAG ON SERVICE TRANSACTIONS.
...
C
               YR = R_YR ! - 1
               DO MO = 0, 12
                  DO DATA_POS = Cash_Base_Rates,Cash_Total_Base_Revenues ! 1, 29
     ...
     +                                      Cash_Lease_Interest_Payments ! 69
...
C
     ...
     +                                 csh_btl_lease_intrst_pymts)
                  MONTH_VARS(MO,Cash_Gas_Wholesale_Revenues) =          ! 177
     +                  MONTH_VARS(MO,Cash_Gas_Wholesale_Revenues)
     +                  + CASH_REV_EXP_MONTHLY(MO,YR,ASSET_CLASS,
...
C
     ...
     +                                              DATA_POS,2,SubClass)
                     ...
                     IF(DATA_POS == Deferred Fuel) THEN
                        MONTH_VARS(MO,Monthly_Deferred_Fuel_Expense-30)= 
     +                           EXPENSES_MONTHLY(MO,YR,ASSET_CLASS,
     +                                              DATA_POS,1,SubClass)
     +                           + EXPENSES_MONTHLY(MO,YR,ASSET_CLASS,
     +                                              DATA_POS,2,SubClass)
     ...
     +                                              DATA_POS,2,SubClass)
                     ...
                     IF(DATA_POS == ATL Book Lease Expense) THEN
                        MONTH_VARS(MO,Monthly_ATL_Lease_Payment-30) = 
     +                           EXPENSES_MONTHLY(MO,YR,ASSET_CLASS,
     +                                              DATA_POS,1,SubClass)
     +                           + EXPENSES_MONTHLY(MO,YR,ASSET_CLASS,
     +                                              DATA_POS,2,SubClass)
...
C
                  DO DATA_POS = 1, LAST_EXPENSE_ITEM
                     IF(DATA_POS == Deferred Fuel) THEN
                        MONTH_VARS(:,Monthly_Deferred_Fuel_Expense-30)= 
     +                           MONTH_VARS(:,
     +                                 Monthly_Deferred_Fuel_Expense-30)
     ...
     +                                              DATA_POS,2,SubClass)
                     ...
                     ENDIF 
                     IF(DATA_POS == ATL Book Lease Expense) THEN
                        MONTH_VARS(:,Monthly_ATL_Lease_Payment-30) = 
     +                       MONTH_VARS(:,Monthly_ATL_Lease_Payment-30)
     +                       + EXPENSES_MONTHLY(:,YR,ASSET_CLASS,
     +                                              DATA_POS,1,SubClass)
     +                       + EXPENSES_MONTHLY(:,YR,ASSET_CLASS,
...
C
               ...
               DO MO = 0, 12
                  DO TAX_TYPE = 1, 2
                     INC_MONTH_VARS(MO,Physical_Revenue_Variable) =
     +                   INC_MONTH_VARS(MO,Physical_Revenue_Variable)
     +                   + REVENUES_MONTHLY(MO,YR,ASSET_CLASS,
     +                                 Derivative Physical Rev Variable,
     +                                                TAX_TYPE,SubClass)
                     INC_MONTH_VARS(MO,Physical_Revenue_Fixed) =
     +                   INC_MONTH_VARS(MO,Physical_Revenue_Fixed)
     +                   + REVENUES_MONTHLY(MO,YR,ASSET_CLASS,
     +                                    Derivative Physical Rev Fixed,
     +                                                TAX_TYPE,SubClass)
                     INC_MONTH_VARS(MO,Financial_Revenue_Variable) =
     +                   INC_MONTH_VARS(MO,Financial_Revenue_Variable)
     +                   + REVENUES_MONTHLY(MO,YR,ASSET_CLASS,
     +                                Derivative Financial Rev Variable,
     +                                                TAX_TYPE,SubClass)
                     INC_MONTH_VARS(MO,Financial_Revenue_Fixed) =
     +                   INC_MONTH_VARS(MO,Financial_Revenue_Fixed)
     +                   + REVENUES_MONTHLY(MO,YR,ASSET_CLASS,
     +                                   Derivative Financial Rev Fixed,
     +                                                TAX_TYPE,SubClass)
C
                     INC_MONTH_VARS(MO,Physical_Expense_Variable) =
     +                  INC_MONTH_VARS(MO,Physical_Expense_Variable)
     +                  + EXPENSES_MONTHLY(MO,YR,ASSET_CLASS,
     +                                 Derivative Physical Exp Variable,
     +                                                TAX_TYPE,SubClass)
C
                     INC_MONTH_VARS(MO,Physical_Expense_Fixed) =
     +                  INC_MONTH_VARS(MO,Physical_Expense_Fixed)
     +                  + EXPENSES_MONTHLY(MO,YR,ASSET_CLASS,
     +                                    Derivative Physical Exp Fixed,
     +                                                TAX_TYPE,SubClass)
C
                     INC_MONTH_VARS(MO,Financial_Expense_Variable) =
     +                  INC_MONTH_VARS(MO,Financial_Expense_Variable)
     +                  + EXPENSES_MONTHLY(MO,YR,ASSET_CLASS,
     +                                Derivative Financial Exp Variable,
     +                                                TAX_TYPE,SubClass)
C
                     INC_MONTH_VARS(MO,Financial_Expense_Fixed) =
     +                  INC_MONTH_VARS(MO,Financial_Expense_Fixed)
     +                  + EXPENSES_MONTHLY(MO,YR,ASSET_CLASS,
     +                                   Derivative Financial Exp Fixed,
     +                                                TAX_TYPE,SubClass)
...
C CASH
C
                  CASH_MONTH_VARS(MO,Cash_Physical_Revenue_Variable) =
     +                CASH_MONTH_VARS(MO,Cash_Physical_Revenue_Variable)
     +                  + CASH_REV_EXP_MONTHLY(MO,YR,ASSET_CLASS,
     +                                 Derivative Physical Rev Variable)
                  CASH_MONTH_VARS(MO,Cash_Physical_Revenue_Fixed) =
     +                   CASH_MONTH_VARS(MO,Cash_Physical_Revenue_Fixed)
     +                  + CASH_REV_EXP_MONTHLY(MO,YR,ASSET_CLASS,
     +                                    Derivative Physical Rev Fixed)
                  CASH_MONTH_VARS(MO,Cash_Physical_Expense_Variable) =
     +                CASH_MONTH_VARS(MO,Cash_Physical_Expense_Variable)
     +                  + CASH_REV_EXP_MONTHLY(MO,YR,ASSET_CLASS,
     +                                  Cash Derative Financial Exp Var)
                  CASH_MONTH_VARS(MO,Cash_Physical_Expense_Fixed) =
     +                   CASH_MONTH_VARS(MO,Cash_Physical_Expense_Fixed)
     +                  + CASH_REV_EXP_MONTHLY(MO,YR,ASSET_CLASS,
     +                                 Cash Derative Physical Exp Fixed)
     ...
     +                  + CASH_REV_EXP_MONTHLY(MO,YR,ASSET_CLASS,
     +                                Derivative Financial Rev Variable)
                  CASH_MONTH_VARS(MO,Cash_Financial_Revenue_Fixed) =
     +                  CASH_MONTH_VARS(MO,Cash_Financial_Revenue_Fixed)
     +                  + CASH_REV_EXP_MONTHLY(MO,YR,ASSET_CLASS,
     +                                   Derivative Financial Rev Fixed)
                  CASH_MONTH_VARS(MO,Cash_Financial_Expense_Variable) =
     +               CASH_MONTH_VARS(MO,Cash_Financial_Expense_Variable)
     +                  + CASH_REV_EXP_MONTHLY(MO,YR,ASSET_CLASS,
     +                                Cash Derative Financial Exp Var)
                  CASH_MONTH_VARS(MO,Cash_Financial_Expense_Fixed) =
     +                  CASH_MONTH_VARS(MO,Cash_Financial_Expense_Fixed)
     +                  + CASH_REV_EXP_MONTHLY(MO,YR,ASSET_CLASS,
     +                                Cash Derative Financial Exp Fixed)
C
         ...
         ENDIF
         DO MO = 0, 12
            INC_MONTH_VARS(MO,Total_Derivative_Revenues) =
     +                   INC_MONTH_VARS(MO,Total_Derivative_Revenues)
     +                   + INC_MONTH_VARS(MO,Physical_Revenue_Variable)
     +                   + INC_MONTH_VARS(MO,Physical_Revenue_Fixed)
     +                   + INC_MONTH_VARS(MO,Financial_Revenue_Variable)
     +                   + INC_MONTH_VARS(MO,Financial_Revenue_Fixed)
            INC_MONTH_VARS(MO,Total_Derivative_Expenses) =
     +                   INC_MONTH_VARS(MO,Total_Derivative_Expenses)
     +                   + INC_MONTH_VARS(MO,Physical_Expense_Variable)
     +                   + INC_MONTH_VARS(MO,Physical_Expense_Fixed)
     +                   + INC_MONTH_VARS(MO,Financial_Expense_Variable)
     +                   + INC_MONTH_VARS(MO,Financial_Expense_Fixed)
            INC_MONTH_VARS(MO,Net_Derivative_Margin) =
     +                    INC_MONTH_VARS(MO,Total_Derivative_Revenues)
     +                    - INC_MONTH_VARS(MO,Total_Derivative_Expenses)
C
            INC_MONTH_VARS(MO,Total_Fuel_Derivative_Revenues) =
     +                 INC_MONTH_VARS(MO,Total_Fuel_Derivative_Revenues)
     +                 + INC_MONTH_VARS(MO,Physical_Fuel_Deriv_Rev_Var)
     +                 + INC_MONTH_VARS(MO,Physical_Fuel_Deriv_Rev_Fix)
     +                 + INC_MONTH_VARS(MO,Financial_Fuel_Deriv_Rev_Var)
     +                 + INC_MONTH_VARS(MO,Financial_Fuel_Deriv_Rev_Fix)
            INC_MONTH_VARS(MO,Total_Fuel_Derivative_Expenses) =
     +                 INC_MONTH_VARS(MO,Total_Fuel_Derivative_Expenses)
     +                 + INC_MONTH_VARS(MO,Physical_Fuel_Deriv_Exp_Var)
     +                 + INC_MONTH_VARS(MO,Physical_Fuel_Deriv_Exp_Fix)
     +                 + INC_MONTH_VARS(MO,Financial_Fuel_Deriv_Exp_Var)
     +                 + INC_MONTH_VARS(MO,Financial_Fuel_Deriv_Exp_Fix)
            INC_MONTH_VARS(MO,Net_Fuel_Derivative_Margin) =
     +               INC_MONTH_VARS(MO,Total_Fuel_Derivative_Revenues)
     +               - INC_MONTH_VARS(MO,Total_Fuel_Derivative_Expenses)
     ...
     +                     + INC_MONTH_VARS(MO,Net_Derivative_Margin)
C
            CASH_MONTH_VARS(MO,Total_Cash_Derivative_Revenues) =
     +             CASH_MONTH_VARS(MO,Total_Cash_Derivative_Revenues)
     +             + CASH_MONTH_VARS(MO,Cash_Physical_Revenue_Variable)
     +             + CASH_MONTH_VARS(MO,Cash_Physical_Revenue_Fixed)
     +             + CASH_MONTH_VARS(MO,cash_financl_rev_variable)
     +             + CASH_MONTH_VARS(MO,Cash_Financial_Revenue_Fixed)
C
            CASH_MONTH_VARS(MO,Total_Cash_Derivative_Expenses) =
     +             CASH_MONTH_VARS(MO,Total_Cash_Derivative_Expenses)
     +             + CASH_MONTH_VARS(MO,Cash_Physical_Expense_Variable)
     +             + CASH_MONTH_VARS(MO,Cash_Physical_Expense_Fixed)
     +             + CASH_MONTH_VARS(MO,Cash_Financial_Expense_Variable)
...
C
     ...
     +           + CASH_MONTH_VARS(MO,Cash_Physical_Fuel_Deriv_Rev_Fix)
     +           + CASH_MONTH_VARS(MO,cash_fin_fuel_deriv_rev_var)
     +           + CASH_MONTH_VARS(MO,Cash_Financial_Fuel_Deriv_Rev_Fix)
C
            CASH_MONTH_VARS(MO,tot_cash_fuel_deriv_expenses) =
     +           CASH_MONTH_VARS(MO,tot_cash_fuel_deriv_expenses)
     +           + CASH_MONTH_VARS(MO,Cash_Physical_Fuel_Deriv_Exp_Var)
     +           + CASH_MONTH_VARS(MO,Cash_Physical_Fuel_Deriv_Exp_Fix)
     +           + CASH_MONTH_VARS(MO,Cash_Financial_Fuel_Deriv_Exp_Var)
     +           + CASH_MONTH_VARS(MO,Cash_Financial_Fuel_Deriv_Exp_Fix)
C
         ENDDO
      RETURN
C***********************************************************************
...
C
     ...
     +                                              'Regulatory Credit')
         ...
         CASE ('Distribution-Maintenance','O&M Transfers to CAPEX')
            INCOME_STATEMENT_POSITION = 32
cGENCO Rental,FSG COGS,ATSI Charges,O&M Transfers to CAPEX,PIP/Uncollectibles,Business Support,SMES
...
C
c         ACCOUNT_SUB_CLASSIFICATION = 6 ! IN THE END WANT 6, OTHER TO BE THE DEFAULT
...
C
     ...
     +                               + ACCUMULATED_VALUES
         ...
         ELSEIF(CASH_TYPE == LAGGED) THEN
            LAG_BY_MONTH = 0.
C            IF(CASH_VALUES_VECTOR == 0.) THEN  ! REMOVED 3/12/06 MSG NOT NEEDED
...
C
     ...
     +            + CASH_LAG_VALUES(0:12,Cash_Adjustment_Clause)
         MONTH_VARS(:,Cash_Secondary_Sales) =
     +            MONTH_VARS(:,Cash_Secondary_Sales)
     +            + CASH_LAG_VALUES(0:12,Cash_Secondary_Sales)
         MONTH_VARS(:,Cash_Capacity_Sales) =
     +            MONTH_VARS(:,Cash_Capacity_Sales)
     +            + CASH_LAG_VALUES(0:12,Cash_Capacity_Sales)
         MONTH_VARS(:,csh_income_rsv_mgn_cap_sales) =
     +           MONTH_VARS(:,csh_income_rsv_mgn_cap_sales)
     +           + CASH_LAG_VALUES(0:12,
     +                        csh_income_rsv_mgn_cap_sales)
         MONTH_VARS(:,Cash_BTL_Revenues) =
     +            MONTH_VARS(:,Cash_BTL_Revenues)
     +            + CASH_LAG_VALUES(0:12,Cash_BTL_Revenues)
         MONTH_VARS(:,Cash_ICAP_Revenues) =
     +            MONTH_VARS(:,Cash_ICAP_Revenues)
     +            + CASH_LAG_VALUES(0:12,Cash_ICAP_Revenues)
C
...
C
     ...
     +       + CASH_LAG_VALUES(0:12,Cash_Other_Revenue)
         MONTH_VARS(:,cash_catawba_revenues:Cash_PGA_Adjustment) =
     +       MONTH_VARS(:,cash_catawba_revenues:Cash_PGA_Adjustment)
     +       + CASH_LAG_VALUES(0:12,
     +                        cash_catawba_revenues:Cash_PGA_Adjustment)
C
C DERIVAIVES REVENUE
C
         MONTH_VARS(:,Cash_Physical_Revenue_Variable) =
     +       MONTH_VARS(:,Cash_Physical_Revenue_Variable)
     +       + CASH_LAG_VALUES(0:12,Cash_Physical_Revenue_Variable)
         MONTH_VARS(:,Cash_Physical_Revenue_Fixed) =
     +          MONTH_VARS(:,Cash_Physical_Revenue_Fixed)
     +          + CASH_LAG_VALUES(0:12,Cash_Physical_Revenue_Fixed)
         MONTH_VARS(:,cash_financl_rev_variable) =
     +      MONTH_VARS(:,cash_financl_rev_variable)
     +      + CASH_LAG_VALUES(0:12,cash_financl_rev_variable)
         MONTH_VARS(:,Cash_Financial_Revenue_Fixed) =
     +         MONTH_VARS(:,Cash_Financial_Revenue_Fixed)
     +         + CASH_LAG_VALUES(0:12,Cash_Financial_Revenue_Fixed)
C
C CHANGE IN ACCOUNTS RECEIVABLE
C
         CASH_REVENUES(:) = SUM(CASH_LAG_VALUES(0:12,
     +                Cash_Adjustment_Clause:Cash_PGA_Adjustment),DIM=2)
     +           + CASH_LAG_VALUES(0:12,Cash_ICAP_Revenues)
     +           + CASH_LAG_VALUES(0:12,Cash_Physical_Revenue_Variable)
     +           + CASH_LAG_VALUES(0:12,Cash_Physical_Revenue_Fixed)
     +           + CASH_LAG_VALUES(0:12,cash_financl_rev_variable)
     ...
     +                Cash_Adjustment_Clause:Cash_PGA_Adjustment),DIM=2)
     +                + BOOKED_VALUES(:,Cash_ICAP_Revenues)
     +                + BOOKED_VALUES(:,Cash_Physical_Revenue_Variable)
     +                + BOOKED_VALUES(:,Cash_Physical_Revenue_Fixed)
     +                + BOOKED_VALUES(:,cash_financl_rev_variable)
     +                + BOOKED_VALUES(:,Cash_Financial_Revenue_Fixed)
     +                + BOOKED_VALUES(:,
     +                        csh_income_rsv_mgn_cap_sales)
...
C
         MONTH_VARS(:,Cash_DOE_Disposal) =
     +            MONTH_VARS(:,Cash_DOE_Disposal) 
     +            + CASH_LAG_VALUES(0:12,Cash_DOE_Disposal)
C
         MONTH_VARS(:,Cash_DOE_Decommissioning) =
     +            MONTH_VARS(:,Cash_DOE_Decommissioning)
     +            + CASH_LAG_VALUES(0:12,Cash_DOE_Decommissioning)
C
...
C
         MONTH_VARS(:,Cash_BTL_Expenses) =
     +            MONTH_VARS(:,Cash_BTL_Expenses)
     +            + CASH_LAG_VALUES(0:12,Cash_BTL_Expenses)
C
...
C
         MONTH_VARS(:,Cash_Purchased_Gas) =
     +            MONTH_VARS(:,Cash_Purchased_Gas) 
     +            + CASH_LAG_VALUES(0:12,Cash_Purchased_Gas)
C
         MONTH_VARS(:,Cash_Other) =
     +            MONTH_VARS(:,Cash_Other)
     +            + CASH_LAG_VALUES(0:12,Cash_Other)
C
         MONTH_VARS(:,Cash_DSM_Expense) =
     +            MONTH_VARS(:,Cash_DSM_Expense) 
     +            + CASH_LAG_VALUES(0:12,Cash_DSM_Expense)
C
         MONTH_VARS(:,Cash_DSM_Rebate) =
     +            MONTH_VARS(:,Cash_DSM_Rebate) 
     +            + CASH_LAG_VALUES(0:12,Cash_DSM_Rebate)
C
         MONTH_VARS(:,Cash_Lease_Expense) =
     +            MONTH_VARS(:,Cash_Lease_Expense) 
     +            + CASH_LAG_VALUES(0:12,Cash_Lease_Expense)
C
         MONTH_VARS(:,Cash_Service_Transactions) =
     +            MONTH_VARS(:,Cash_Service_Transactions) 
     +            + CASH_LAG_VALUES(0:12,Cash_Service_Transactions)
C
         MONTH_VARS(:,Cash_Emission_Credits) =
     +            MONTH_VARS(:,Cash_Emission_Credits) 
     +            + CASH_LAG_VALUES(0:12,Cash_Emission_Credits)
C
...
C DERIVATIVE
C
         MONTH_VARS(:,Cash_Physical_Expense_Variable) =
     +       MONTH_VARS(:,Cash_Physical_Expense_Variable)
     +       + CASH_LAG_VALUES(0:12,Cash_Physical_Expense_Variable)
         MONTH_VARS(:,Cash_Physical_Expense_Fixed) =
     +          MONTH_VARS(:,Cash_Physical_Expense_Fixed)
     +          + CASH_LAG_VALUES(0:12,Cash_Physical_Expense_Fixed)
         MONTH_VARS(:,Cash_Financial_Expense_Variable) =
     +      MONTH_VARS(:,Cash_Financial_Expense_Variable)
     +      + CASH_LAG_VALUES(0:12,Cash_Financial_Expense_Variable)
         MONTH_VARS(:,Cash_Financial_Expense_Fixed) =
     +         MONTH_VARS(:,Cash_Financial_Expense_Fixed)
     +         + CASH_LAG_VALUES(0:12,Cash_Financial_Expense_Fixed)
...
C
     ...
     +                + BOOKED_VALUES(:,Cash_DOE_Disposal)
     +                + BOOKED_VALUES(:,Cash_DOE_Decommissioning)
     ...
     +                + BOOKED_VALUES(:,Cash_BTL_Expenses)
     +                + BOOKED_VALUES(:,Cash_Other_OandM)
     +                + BOOKED_VALUES(:,Cash_Purchased_Gas)
     +                + BOOKED_VALUES(:,Cash_Other)
     +                + BOOKED_VALUES(:,Cash_DSM_Expense)
     +                + BOOKED_VALUES(:,Cash_DSM_Rebate)
     +                + BOOKED_VALUES(:,Cash_Lease_Expense)
     +                + BOOKED_VALUES(:,Cash_Service_Transactions)
     +                + BOOKED_VALUES(:,Cash_Emission_Credits)
     +                + BOOKED_VALUES(:,cash_catawba_expenses)
     +                + BOOKED_VALUES(:,Cash_BTL_Lease_Cash)
     +                + BOOKED_VALUES(:,Cash_Physical_Expense_Variable)
     +                + BOOKED_VALUES(:,Cash_Physical_Expense_Fixed)
     +                + BOOKED_VALUES(:,Cash_Financial_Expense_Variable)
     +                + BOOKED_VALUES(:,Cash_Financial_Expense_Fixed)
     +                + BOOKED_VALUES(:,
     +                    csh_expns_rsv_mgn_cap_purch)
