...
! DECLARATION FOR ALLOCATION VARIABLES
      REAL ::  DEMAND_ALLOCATOR(:),ENERGY_ALLOCATOR(:),
     +     CUSTOMER_ALLOCATOR(:),SYSTEM_LOSSES(:)
      REAL ::  R_DEMAND_ALLOCATOR,R_ENERGY_ALLOCATOR,
     +     R_CUSTOMER_ALLOCATOR,R_SYSTEM_LOSSES
      ALLOCATABLE :: DEMAND_ALLOCATOR,ENERGY_ALLOCATOR,
     +               CUSTOMER_ALLOCATOR,SYSTEM_LOSSES
      LOGICAL (kind=1) ::  LAHEY_LF95
      CHARACTER (len=30) ::  SCREEN_OUTPUT
!      SAVE DEMAND_ALLOCATOR,ENERGY_ALLOCATOR,
!     +     CUSTOMER_ALLOCATOR,SYSTEM_LOSSES
!***********************************************************************
!
...
!         IF(INDEX(COST_ALLOC_OL,'BC') == 0) CALL COST_SERVICE_ALLOCATORS
         COST_ALLOC_OL = 'BC'
      RETURN
!***********************************************************************
!*                                                                           *
...
!
 ...
 1010 FORMAT('&',A)
      END
! COPYRIGHT (C) 1987,93 M.S. GERBER & ASSOCIATES, INC.
! ALL RIGHTS RESERVED
!***********************************************************************
...
!
      ...
      REAL ::  CLASS_PEAK(SYSTEM_CLASS_NUM),ANNUAL_PEAK,
     +     GET_INPUT_SYSTEM_PEAK,
     +     R_CLASS_RATES(*),
     +     TOTAL_PEAK,TOTAL_MWH,
     +     TOTAL_CUSTOMERS,
     +     DEMAND_ALLOCATOR,ENERGY_ALLOCATOR,
     +     CUSTOMER_ALLOCATOR,
     +     CLASS_COSTS_DEMAND(SYSTEM_CLASS_NUM),
     +     CLASS_COSTS_ENERGY(SYSTEM_CLASS_NUM),
     +     CLASS_COSTS_CUSTOMER(SYSTEM_CLASS_NUM),
     +     ALLOC_ENERGY,ALLOC_DEMAND,ALLOC_CUSTOMER,
...
!
     ...
     +          PRIMARY_CLASSES,
     +          PEAK_MONTH
      LOGICAL (kind=1) ::  ALLOCATION_REPORT,SYSTEM_BASED_FORECAST,
     +          POOLING_TRANSACTIONS,PRICE_FEEDBACK_ACTIVE,
     +          CALCULATE_TOTAL_PRICE_ADJUSTER
!
      INTEGER (kind=2) ::    REV_ALLOC_NO,CLASS_REV_ALLOC_HEADER,
     +            REV_FUNC_NO,REV_FUNC_HEADER
...
!
     ...
     +                                         CUSTOMER_ALLOCATOR)
      ...
      ENDIF
!
!     THIS VARIABLE NEEDS TO BE SET IN GETSYS
!
