...
!
      ...
      integer (kind=2) ::  ji
      LOGICAL (kind=1) ::  CALUCLATE_NUC_FUEL_PRICES,TRANS_HYDRO,
     ...
     +               FIRST_HYDRO_LOAD_NO,
     +               HYDRO_AG_IPNTX
      INTEGER (kind=4) ::  PROCOST
      LOGICAL (kind=1) ::  READ_PRODUCTION_PARAMETERS,CoalLPActive
     ...
     +     CO2_RETRO_CAP_MULT,GET_CO2_RETRO_CAP_MULT
      ...
      CHARACTER (len=11) ::  CLK ! INTRINSIC TIME FUNCTION.
     ...
     +          ENRG_LIMIT_FUEL_TYPES='H12')
      ...
      LOGICAL (kind=1) ::  OFFSET_MAINTENANCE_VECTORS,DO_IT,
     ...
     +          POINTER
      INTEGER (kind=2) ::  PSMO,PEMO,PROCMETH,HOURS_IN_PERIOD,I,M,
     ...
     +          GET_THERMAL_ONLINE
      INTEGER (kind=2) ::  SAVE_YEAR
      INTEGER (kind=2) ::  LAST_NUNITS/0/,GET_PROCOST_LAST_NUNITS,
...
!
     ...
     +     ANNUAL_BASE
      LOGICAL (kind=1) ::  DISPATCH_REPORT,ENRG_LIMIT_REPORT,
     ...
     +          USE_EXTERNAL_PRODUCTION_COST
      REAL (kind=8) ::  DEMAND,TOTAL_SALES_ENERGY,TOTAL_SALES_REVENUE,
...
!
      LOGICAL (kind=1) ::    DUKE
      LOGICAL (kind=1) ::    VOID_LOGICAL,UPDATE_CATAWBA_CONTRACT_DATA,
...
! REGIONAL ANALYSIS TRANSACTIONS VARIABLES
      ...
      REAL (kind=4) ::       GET_TOTAL_INCREMENTAL_COST,INC_CAP,
...
!
! SET_EMISSIONS_RATES ALLOCATES ARRAY SIZES BASED ON NUNITS
! WHEN NEXT DECISION PLANNING IS BEING USED NUNITS CAN CHANGE EACH
...
!     KEPCO = .FALSE. ! SET TO FALSE UNTIL KEPCO JOINS THE TC PROJECT
...
!         SEASON_EMISSIONS_RATES = UTILITY_TYPE() == 'T'
!
...
!
         ...
         ELSE ! IF(PRODUCTION_COSTING_PERIODS == 12) THEN
            ...
            PEMO = ISEAS
!         ELSE
!           USER DEFINED SEASONS       REMOVED USE OF PERIODS 10/18/00
!           PSMO = PERIOD(1,ISEAS)
!           PEMO = PERIOD(2,ISEAS)
         ENDIF
...
!           CALL SERVICE_UPDATE_COSTS(ISEAS,YEAR,ESCALATION_RATES)
         ENDIF
!
...
! the following 2 lines replace the commented out line 10/22/01 msg FUEL_ID can be zero
     +                   FUEL_ID > 0 .AND.
...
!
      IF(DUKE()) THEN
!        VOIDR4 = BUYERS_SECTION_82_ENRG_RATE() !REMOVED 6/27/96
...
!
      ...
      YEAR = SAVE_YEAR
!
!      IF(.NOT. PRICE_PLANNING_FIRST_PASS) THEN  ! TEMP TEST. 6/18/96. GAT.
!      IF(.NOT. TRANSACT_ANALYST_ONLY() .OR. INIT_FILE_IS_ACTIVE()) THEN
...
!
! DEALLOCATE ARRAYS NOT USED BY SP_CAPEX
!
...
! ---- CAPEX
!
! DEALLOCATE ARRAYS USED BY PR_COST AND SP_CAPEX
!
! PROCOST ARRAYS
...
!      IF(ALLOCATED(BLENDED_BTU_COST)) THEN
!         DEALLOCATE(BLENDED_BTU_COST,STAT=ERROR) ! 3/2010 msg
!      ENDIF
!      IF(ALLOCATED(ANNUAL_BLENDED_BTU_COST)) THEN
!         DEALLOCATE(ANNUAL_BLENDED_BTU_COST,STAT=ERROR)
!         DEALLOCATE(ANNUAL_FUEL_BTU_COST,STAT=ERROR)
...
!***********************************************************************
      ...
      RETURN
! 1100 FORMAT(1X,4(F12.0,','),F12.3,',',F12.3)
      END
!**********************************************************************
!
...
!        MWBLOK(NBLOK) = MW(1,NUS)
     ...
     +                                                       (MW(2,NUS))
      ...
      IF(MW(2,NUS) > MW(1,NUS) .OR. ZERO_CAPACITY_ACTIVE_UNIT) THEN
         NBLOK = NBLOK + 1
         LSTBLOK = LSTBLOK + 1
!        MWBLOK(NBLOK) = MW(2,NUS) - MW(1,NUS)
         BLKNO(NBLOK) = LSTBLOK
         UNIT(NBLOK) = NUS
...
!**********************************************************************
!
!     COPYRIGHT (C) 1992 M.S. GERBER & ASSOCIATES, INC. ALL RIGHT RESERVED
!
...
!**********************************************************************
      ...
      INTEGER (kind=2) ::  YR,UNIT_NO,NUNITS,I,J,L,R_ISEAS
      INTEGER (kind=2) ::  FUEL
