...
INTEGER (KIND=2) :: UNIT_NO,TG,TG_POSITION,I,GET_TRANS_GROUP_POSITION, &
                    ...
                    RETRO_ID,RETROFIT_ACTIVE_ID,GET_NUNITS, &
                    RETIREMENT_YEAR
...
!
IF(.NOT.(GET_CO2_RETIREMENTS_LOGIC().OR.CO2_RETROFIT_LOGIC_ACTIVE())) THEN
RETURN
ENDIF
IF(.NOT. RPT_FILE_OPEN) THEN
   OPEN(8337,FILE="GRX2EnterprizeRetireRetro-"//TRIM(GET_SCENAME())// &
   ".CSV",STATUS="REPLACE")
...
IF(GET_CO2_RETIREMENTS_LOGIC() .AND. INSTALLED_POINTER > 0) THEN
   ...
   DO I = 1, INSTALLED_POINTER
    ...
    TEMP_R4 = CL_CAPACITY_PLANNING_ADJ(RETIREMENT_YEAR,UNIT_NO,2,.TRUE.)
      ...
      TG = MAX(INT(1,2),TRANSACTION_GROUP(UNIT_NO))
      ...
      IF(GRX_RETIRE_THIS_YEAR) THEN
         DATE_STR = "12/31/"//CONVERT_2_STR(INT(BASE_YEAR+YEAR,4))
      ELSE
         DATE_STR = "12/31/"//CONVERT_2_STR(INT(BASE_YEAR+YEAR-1,4))
      ENDIF
...
! Doug requested interface to Alan program
   ...
   CALL flush_unit(INT(8339,2))
ENDIF
IF(.NOT. (CO2_RETROFIT_LOGIC_ACTIVE() .AND.  UNITS_RETROFITTED > 0)) THEN
RETURN
ENDIF
DO I = 1, UNITS_RETROFITTED
   ...
   H2_STR30 = CONVERT_2_STR(H2_UNIT_ID_NUM(UNIT_NO))
   EV_STR30 = CONVERT_2_STR(HESI_UNIT_ID_NUM(UNIT_NO))   ! EV ID
   DATE_STR = "1/1/"//CONVERT_2_STR(INT(BASE_YEAR+YEAR,4))
   recln = '"'//TRIM(TRANS_GROUP_FULL_NAME(TG_POSITION))//'","'//  &
...
WRITE(9,1009) END_POINT,  &
              ...
              YEAR+BASE_YEAR , &
              "CO2 Market", &
              "CO2 Mkt Purchase-"//CONVERT_2_STR(CO2_CREDIT_PRICE), &
              ! 20 CHARACTERS
              0., &
...
REAL FUNCTION NEWTON_ABAT_PRICE_SEARCH(MIN_POINT,MAX_POINT, &
                                       MIN_CO2,MAX_CO2,CO2_CAP) &
                                                       RESULT(NEW_PRICE)
   REAL (KIND=4), OPTIONAL :: MIN_POINT,MAX_POINT,MIN_CO2,MAX_CO2, &
                              CO2_CAP
