...
!      $IF DEFINED(ReadPlnt)
!      ! AGT will have defined valid values in the command line in his make-file
!      $ELSE
!      ! Greg or Mark:  encode ReadPlnt and ReadLoad below as follows
!      ! both 0     (normal production use; data is passed via arguments)
!      ! ReadLoad=1 (program reads load data from FacetErr.lod)
!      ! ReadPlnt=1 (program reads plnt data from FacetErr.plt)
!      ! both 1     (program reads plnt & load data from files)
!      $DEFINE ReadPlnt=0
!      $DEFINE ReadLoad=0
!      $ENDIF
...
!      $IF DEFINED(FAPrompt)
!      ! AGT will have defined a valid value in the command line in his make-file
!      $ELSE
!      ! Greg or Mark:  encode FAPrompt below as follows
!      ! 0 for no prompting at first entry to Facet Algorithm (production use)
!      ! 1 to force prompting for PrtDetail and (optionally) CallsSkipped
!      $DEFINE FAPrompt=0
!      $ENDIF
      ...
      use end_routine, only: end_program
        include 'FAIntCom.mon' ! merely for ErrorsTrapped,PrtUni,PrtDetail
        integer*2 Prefix,Suffix
        character*(*) Reason
        character*20 ErrFmt
...
!         write(PrtUni,'(1x,a,z4)')'execution error, code #',ErrorsTrapped
        ! PrtDetail=0 ! preclude further attempt at writing to PrtUni by CkAllocSt
        end if
      ...
      subroutine CkAllocSt(Abbrev,iDealloc)
        include 'FAIntCom.mon' ! merely for ErrorsTrapped,AS,PrtUni
        integer*4 iDealloc
        character*(*) Abbrev
        character*3 Prefix(0:1)
        data Prefix/' ',' de'/ ! parameter would pad with trailing blanks

        if(AS/=0) then
          ErrorsTrapped=ior(ErrorsTrapped,1) ! the caller may want to abort
...
!       SOFTWARE TOOLS IN PASCAL, by Kernighan & Plauger, Addison-Wesley 1981
        logical*4 Increasing
        logical*1 Decreasing
        integer*2 iSup,i,j,k,p,q,Gap,iPos(*)
        real*4 a(*)
     ...
     +           (Decreasing.and.(a(p)>=a(q)))) then
      ...
      subroutine wnaa(s) ! write s to StdOut without advancing record
        include 'FAIntCom.mon' ! merely for PrtUni
        character*(*) s
      ...
      subroutine GetOneDigit(i,i0,i1) ! useful only during debugging
        integer*2 i,i0,i1,i2 ! i0,i1 are lower and upper bounds on i, respectively
        character*1 GetCh,AnyChar
     ...
     +  EAVAIL,MAINTENANCE_RATE,ENERGY)
        integer*2 LAST_POINT,NUNITS,NBLOK2,
     ...
     +    UnservedQ,xDynStg
        ...
        integer*2 CallCount,CallsSkipped ! useful only during testing
        data CallCount/0/,CallsSkipped/0/
!       real*4 Time0,Time1,bclk
        character*11 clk

        if(CallCount==0) then
!       PrtDetail below is coded as follows:
!       0 for no debugging output
!       1 for output of FacetAlg objective function's value at each iteration
...
!         if(PrtDetail==5) then
!           read(*,*)CallsSkipped
!           call GetOneDigit(PrtDetail,0,4)
!         end if
!      $ELSE
...
c            write(*,*)' at top ',clk()
            PrtUni=DebugUnit
            open(PrtUni,file='c:FacetAlg.out',carriagecontrol='FORTRAN')
...
!      !if(CallCount>CallsSkipped) call HaltWith(0,CallsSkipped+1,'aborted before FOC #calls=')
     ...
     +        ('bound ordinary variable with the largest-magnitude')
        ...
        call DeallocFAInt ! vars allocated in StorePlantData
        call DeallocAuxes
c        if(PrtDetail>0) write(*,*)' at end ',clk(),' pd. ',CallCount,
c     +    nIter
!       if(CallCount==CallsSkipped) call HaltWith(0,0,'aborted after FOC #calls=CallsSkipped, not')
        return
      end ! subroutine FindOptCostByFacets


!     end of routines known by MinimizeCost's caller




      logical*1 function Nearly0(Arg,VarType)
        include 'FAIntCom.mon'
        real*4 Arg
        integer*4 VarType
...
!       To preclude errors when n==0 and x==0,
!       define x**0=1 for all x, even for x==0
        if(n==0) then
          IntegralPower=1.0
        elseif(x==0.0) then
          IntegralPower=x
...
!       yInterpolate=(y1*(x2-xMid)+y2*(xMid-x1))/(x2-x1) ! 3+1+2+1 ops
...
!       returns the complement of the cumulative standard Normal function as a
!       multiple of its density function; abs(error)<.00001 for non-negative
!       arguments; source is NBS Handbook of Math Functions, paragraph 26.2.16
        real*4 zCCN,t
      ...
      use end_routine, only: end_program, er_message
        ...
        integer*2 i,j,k
!       include 'OCostCom.mon' ! variables common to MinimizeCost routines
        character*8 Fmt1a,Fmt71,Fmt03
        character*58 Unplaceable
        integer*2 LimAbnModes
     ...
     +    LimAbnModes=0) ! 0 to inhibit extraction of abnormal modes from load curve
        real*8 TailUQ
        real*4
     +  ! SplitCrit, ! criterion for a discharge var 'splitting' a key var
     ...
     +    BinCoeff(0:LimCumulants,0:LimCumulants)
        integer*4 I4_4LCmBnM,I4_4nEqPtsp1,I4_4nKappa,I4_nKappa,I4_SupiHP
        integer*2 nEqPoints ! integer*1 suffices for magnitude of those below:
        integer*2 jOrdEnt,jKey,jBind,jPrBk,jBlkPair,
     +    kLvg,kLp1,kInf,kBind,Base2LogSize,nActiveFacets,
     +    LimitCondit,iOVE,nGamma,SupiHP,nStgEqn
        logical*1 Optimal,OrdVarEntering,InitSort,BasisStrucObsolete,
...
!       include 'FAIntAlo.dfn' ! allocatable arrays on the interface
...
!       include 'OCostAlo.dfn' ! allocatable arrays within MinimizeCost
        integer*2
     +    PrevState (:),
     ...
     +    VarCumul(:,:) ! (.,j) is vector for var j; (.,0) is vector for EqLDn0
        ...
        logical*1 BkOwns(:),BindSetOwns(:),TestSetOwns(:),WouldBeZero(:)
...
!       variables used only by ReviseBasisVectors and those it calls:
        integer*2 jLvg,jLp1,jNewlyNKy,FinalState
        logical*1 FacetOwnsBNK
        save jLvg,jLp1,jNewlyNKy,FinalState,FacetOwnsBNK

!       variables used only by entry-points below, in order of use:
        logical*1 EcoReversed,TempLog1
        integer*2 NBLOK2,NUM_ARB_CONSTRAINTS,NUM_CUMULANTS,NUNITS,
     +    UNIT(*),BLKNO(*)
        real*4
     +    LOWER_BOUND_CAP_FACTOR(*),
     +    UPPER_BOUND_CAP_FACTOR(*),MWBLOK(*),
     +    EAVAIL(*),MAINTENANCE_RATE(*)
        integer*2 i1,i2,i3,iMinJ,iUnit,j0,j1,j2,j3,jRet,jDis,
     +    iAux,jAux,iAuxOrg,
...
!    +    yAtxShifted,yExAtxShifted,
     ...
     +    EDIncl(0:LimEqPoints) ! eq-load's duration including outages of Capac
        ...
        parameter(
     +    FmtNoPhi1='(1x,2i3,a,f8.2 ,f12.5,f9.3,f19.2,f9.3,f10.6)',
     ...
     +    ArrayInv(nRows,nRows),ArrayOld(:,:),HeldRow(LimUnknowns)
        ...
        parameter(
     +    NoCompl='var has no aux complement in state unequal to')
        real*4 Alpha
...
!       real*4 LPROB(*),LODDUR(*)
        ...
        nPoints=LAST_POINT
!      $IF ReadLoad==1
!        open(17,file='FacetErr.lod',status='old') ! force-read troublesome data
!        read(17,*)nPoints ! assume nPoints<=LimPoints
!        NonzeroPeak=.false.
!      $ELSE
...
!      $ENDIF
          if(iPoint>1) then ! correct local arrays for non-monotonic input curve
            ...
            if(NoniX.or.IncrY) then ! adjust one or both to conform to a CDF
              if(PrtDetail>0) write(PrtUni,
     +          '(1x,i3,f14.6,f10.7/1x,i3,f14.6,f10.7\)')
     +          iPoint-1,EqLoad(iPoint-1),EqLDn0(iPoint-1),
     +          iPoint  ,EqLoad(iPoint  ),EqLDn0(iPoint  )
              if(IncrY) then
...
!       shift left by 1 those values indexed>jD in arrays indexed by j
c       iOfFacAlgs(jD:nVar-1)=iOfFacAlgs(jD+1:nVar) ! call cmove(iOfFacAlgs(jD+1),iOfFacAlgs(jD),I4_2nRem)
c       UnitOwning(jD:nVar-1)=UnitOwning(jD+1:nVar) ! call cmove(UnitOwning(jD+1),UnitOwning(jD),I4_2nRem)
c       BlockPos  (jD:nVar-1)=BlockPos  (jD+1:nVar) ! call cmove(BlockPos  (jD+1),BlockPos  (jD),I4_2nRem)
c       Capacity  (jD:nVar-1)=Capacity  (jD+1:nVar) ! call cmove(Capacity  (jD+1),Capacity  (jD),I4_4nRem)
c       EquivAvail(jD:nVar-1)=EquivAvail(jD+1:nVar) ! call cmove(EquivAvail(jD+1),EquivAvail(jD),I4_4nRem)
c       CELB      (jD:nVar-1)=CELB      (jD+1:nVar) ! call cmove(CELB      (jD+1),CELB      (jD),I4_4nRem)
c       CEUB      (jD:nVar-1)=CEUB      (jD+1:nVar) ! call cmove(CEUB      (jD+1),CEUB      (jD),I4_4nRem)
c!      InputRate (jD:nVar-1)=InputRate (jD+1:nVar) ! call cmove(InputRate (jD+1),InputRate (jD),I4_4nRem)
c       cOrg      (jD:nVar-1)=cOrg      (jD+1:nVar) ! call cmove(cOrg      (jD+1),cOrg      (jD),I4_4nRem)
c       cMod      (jD:nVar-1)=cMod      (jD+1:nVar) ! call cmove(cMod      (jD+1),cMod      (jD),I4_4nRem)
c       x         (jD:nVar-1)=x         (jD+1:nVar) ! call cmove(x         (jD+1),x         (jD),I4_4nRem)
c       BlockOfSameUnitAs(jD:nVar-1,1)=BlockOfSameUnitAs(jD+1:nVar,1)
c       BlockOfSameUnitAs(jD:nVar-1,2)=BlockOfSameUnitAs(jD+1:nVar,2)
!       call cmove(BlockOfSameUnitAs(jD+1,1),
!    +             BlockOfSameUnitAs(jD  ,1),I4_2nRem)
!       call cmove(BlockOfSameUnitAs(jD+1,2),
!    +             BlockOfSameUnitAs(jD  ,2),I4_2nRem)
!       the above shifts are now performed by the loop:
        do iCE=jD+1,jD+nVar
...
!        do iCE=1,nVar-1
!          iAux=iOfFacAlgs(iCE)
!          if((iAux/=InvalidIndex).and.(iAux<nVar)) then
!            jAux=jOfCallers(iAux)
!            if(jAux/=iCE) call HaltWith(jAux,iAux,'index mismatch in CE')
!          end if
!        end do
...
!       write(PrtUni,'(1x,26i3)')(VarOfDispRank(jSR),jSR=1,nVar)
!       write(PrtUni,'(1x,26i3)')(BlockPos     (jSR),jSR=1,nVar)
!       write(PrtUni,'(1x,26i3)')(BlockOfSameUnitAs(jSR,1),jSR=1,nVar)
!       write(PrtUni,'(1x,26i3)')(BlockOfSameUnitAs(jSR,2),jSR=1,nVar)
        do jSR=1,nVar-1
          iSR=iOfFacAlgs(jSR)
          if(iSR>0) then
...
!         jSR=jOfCallers(iSR)
!         if(jSR>0) then
!           if(iOfFacAlgs(jSR)/=iSR) write(PrtUni,'(1x,3i4,a)')
!    +        iSR,jSR,iOfFacAlgs(jSR),' i array corruption'
!         end if
!       end do
      return
...
!      $IF ReadPlnt==1
!       open(17,file='FacetErr.plt',status='old') ! force-read troublesome data
!       read(17,*)nStgEqn,NBLOK2,nCFLBVar ! assume that # CF-lower-bounded is known
!      $ELSE
        nCFLBVar=0
        call GET_NUM_DYN_STORAGE(nStgEqn) ! from ENRGLIMT. 9/29/95. GAT.
!       nStgEqn=0 !! 1=> crash on call #3
...
!       nKappa=NUM_CUMULANTS
        ...
        nVarUB=min(nVar+nAux+1,LimUnknowns) ! limit the memory allocated
...
!       call cinitw(iOfFacAlgs,I4_nVUB,InvalidIndex)
!       call cinitw(jOfCallers,I4_nVUB,InvalidIndex) ! default => capacity=0
!       call cinitw(AuxRow    ,I4_nVUB,InvalidIndex)
!       call cinitb(AuxCycled ,I4_nVUB,.false.)
!       call cinitd(x,I4_nVUB,0.0) ! default implying var is not an aux slack
!       iOfFacAlgs = InvalidIndex
!       jOfCallers = InvalidIndex ! default => capacity=0
!       AuxRow = InvalidIndex
!       AuxCycled = .false.
!       x = 0.
     ...
     +    'Avlbty MntRat  RawCost BlokSize Capacity SystCapy'
        ...
        SupCost=0.0
!      $IF ReadPlnt==1
!       do i=1,nStgEqn*2 ! read data for return units (odd j) and discharge (even j)
!         read(17,*)j,UnitOwning(j),BlockPos(j),CFLB(j),CFUB(j),EquivAvail(j),
!    +      cOrg(j),cOrg(j),Capacity(j) ! first cOrg skips over maintenance-rate
!         if(SupCost<cOrg(j)) SupCost=cOrg(j)
!         SysCap=SysCap+Capacity(j)
!       end do
!
!       do i=1,NBLOK2 ! read capacity-limited data into caller's arrays
!         read(17,*)j0,UNIT(i),BLKNO(i),
!    +      LOWER_BOUND_CAP_FACTOR(i),
!    +      UPPER_BOUND_CAP_FACTOR(i),
!    +      EAVAIL(UNIT(i)),MAINTENANCE_RATE(UNIT(i)),
!    +      BlockDispCost(UNIT(i),max(BLKNO(i),1)),MWBLOK(i),
!    +      BlockHeatRate(UNIT(i),max(BLKNO(i),1))
!       end do
!      $ELSE
...
!       if(NUM_ARB_CONSTRAINTS>0)
!    +  call GET_BLOCK_HEAT_RATE(UNIT,BLKNO,BlockHeatRate,NBLOK2)
...
c             UnitOwning(j3:j3+I4_BRem)=UnitOwning(j0:j0+I4_BRem) ! call cmove(UnitOwning(j0),UnitOwning(j3),I4_BRem/2)
c             BlockPos  (j3:j3+I4_BRem)=BlockPos  (j0:j0+I4_BRem) ! call cmove(BlockPos  (j0),BlockPos  (j3),I4_BRem)
c!            I4_BRem=I4_BRem*2 ! for real*4 arrays [valid when calling cmove]
c             CFLB      (j3:j3+I4_BRem)=CFLB      (j0:j0+I4_BRem) ! call cmove(CFLB      (j0),CFLB      (j3),I4_BRem)
c             CFUB      (j3:j3+I4_BRem)=CFUB      (j0:j0+I4_BRem) ! call cmove(CFUB      (j0),CFUB      (j3),I4_BRem)
c             EquivAvail(j3:j3+I4_BRem)=EquivAvail(j0:j0+I4_BRem) ! call cmove(EquivAvail(j0),EquivAvail(j3),I4_BRem)
c             Capacity  (j3:j3+I4_BRem)=Capacity  (j0:j0+I4_BRem) ! call cmove(Capacity  (j0),Capacity  (j3),I4_BRem)
c             cOrg      (j3:j3+I4_BRem)=cOrg      (j0:j0+I4_BRem) ! call cmove(cOrg      (j0),cOrg      (j3),I4_BRem)
!             the above shifts are now performed by the loop:
              do i1=0,I4_BRem
...
!         call cinitb(AuxCovers,I4_nAux*I4_nVUB,.false.)
!         AuxCovers = .false.
          call initb1(AuxCovers,int2(nAux*nVarUB),FalseByte)
        end if
        ...
        do i=1,NBLOK2 ! move caller's capacity-limited data into local arrays
          ...
          UnitOwning(j)=iUnit
          BlockPos(j)=BLKNO(i)
          cOrg(j)=BlockDispCost(iUnit,max(BlockPos(j),1)) ! negative is allowed
!         if(NUM_ARB_CONSTRAINTS>0) ! harmless to transfer garbage if unused
!         InputRate(j)=BlockHeatRate(iUnit,max(BlockPos(j),1))
...
!           if(PrtDetail>2) write(PrtUni,'(1x,a,e14.6)')'negligible cap avail: ',Capaci
     ...
     +      (CFLB(j)<CFUB(j)).and.(BlockPos(j)==1)) then
            ...
            cOrg(i)=cOrg(j)
!           InputRate(i)=InputRate(j)
            UnitOwning(i)=UnitOwning(j)
          end if
        end do
        nVar=i

!       call cinitw(BlockOfSameUnitAs,I4_nVUB*2,InvalidIndex)
!       BlockOfSameUnitAs =  InvalidIndex
        call initw1(BlockOfSameUnitAs,int2(nVarUB*2),InvalidIndex)
     ...
     +      cOrg(j)=-cOrg(j+1) ! dispatch storage-return vars in reverse-econ order
          ...
          ! cMod(j)=cMod(j)*1.000001 ! dispatch after block 1 if cOrg's are same
          ! above bias confounds the ability to merge blocks of equal cost in same unit
            i=UnitOwning(j)
            j1=1
...
!             I4_BRem=int4(2*(nVar-i)) ! compress out VarOfDispRank(i)
!             VarOfDispRank(i:nVar-1)=VarOfDispRank(i+1:nVar) ! call cmove(VarOfDispRank(i+1),VarOfDispRank(i),I4_BRem)
...
!           I4_BRem=int4(2*(nVar-i+1))
!           call cmove(VarOfDispRank(i),VarOfDispRank(i-Multiplicity+1),I4_BRem)
!           VarOfDispRank(i-Multiplicity+1:nVar-Multiplicity+1)=
!    +      VarOfDispRank(i:nVar)
...
!         AuxCoeff=0.0 ! call cinitd(AuxCoeff,I4_nVUB,0.0)
          call initd1(AuxCoeff,nVarUB,0.0)
!      $IF ReadPlnt==1
!          if(iAux<=nStgEqn) then ! note implicit assumption that nVarInAux=2
!            do i=1,Multiplicity ! assume AuxCoeff and AuxRHS track Capacity & cOrg
!              read(17,*) nVarInAux,AuxCoeff(2*iAux-1),AuxCoeff(2*iAux),AuxRHS(iAux)
!            end do ! retain the last line's values, while advancing through the file
!          else ! for arbitrary constraints
!            read(17,*) nVarInAux,SlackCoeff,AuxRHS(iAux),ConstraintUnits
!            do j=1,nVarInAux
!              read(17,*) i,BlkCoeff
!              jAux=jOfCallers(i)
!              if(jAux/=InvalidIndex) AuxCoeff(jAux)=BlkCoeff
!            end do
!          end if
!      $ELSE
     ...
     +        AuxCoeff,AuxRHS)
!          else
!            call GET_ARB_CONSTRAINT(iAuxOrg,SlackCoeff,AuxCoeff,AuxRHS,
!      +       jOfCallers,ConstraintUnits)
          end if
!      $ENDIF
          if(iAux<=nStgEqn) then
            AuxRHS(iAux)=real(Multiplicity)*AuxRHS(iAux)
!         else
!       this requires more thought with respect to SlackCoeff:  (AGT on 20000620)
!           if(SlackCoeff/=0.0) then ! create a slack variable
!             nVar=nVar+1
!             call ChkReLim(nVar,LimUnknowns,VarString)
!             AuxCoeff(nVar)=SlackCoeff
!             UnitOwning(nVar)=InvalidIndex
!             BlockPos(nVar)=1
!             x(nVar)=-1.0 ! temporarily mark this as an aux slack (member of Psi0)
!         ! else constraint holds with equality, needs no slack variable
!           end if
!       this requires more thought with respect to BlockHeatRate and InputRate:
!       (AGT on 20000620)
!           if(ConstraintUnits=='H') then ! convert constraint to units of x
!             do j=1,nVarUB
!               if(AuxCoeff(j)>SmallReal)
!      +          AuxCoeff(j)=AuxCoeff(j)/InputRate(j) ! e.g., [BTU]/[BTU/MWH]=[MWH]
!             end do
!         ! else constraint is already in units of x
!           end if
          end if
...
!             approach below would require changing AuxCoeff indices for compression
!             if(j>nVar) then
!               if(AuxCoeff(j)<=0.0) call
!      +          HaltWith(j,j,'var (an aux slack) must have positive Aij')
!               nVar=nVar+1 ! this complicates the array allocations above
!               call ChkReLim(nVar,LimUnknowns,VarString)
!               UnitOwning(j)=InvalidIndex
!               BlockPos(j)=1
!               x(j)=-1.0 ! temporarily mark this as an aux slack (member of Psi0)
!             end if
...
!           SupRHS=0.0
!           do jAux=1,nVar
!             if(AuxCovers(iAux,jAux).and.(jAux/=j).and.
!      +        (AColumn(iAux,jAux)<0.0)) ! AColumn(iAux,j) must be positive
!      +        SupRHS=SupRHS-AColumn(iAux,jAux)*Capacity(jAux)
!           end do
...
!      $ENDIF
     ...
     +        MeanLoad-xAccum,'=UQ'
          ...
          ENERGY(max(BLKNO(iBlk),1),UNIT(iBlk))=CallersQ
        end do
...
!       with nKappa restricted to at most 4, the above reduces to simply:
!         VarCumul(1,jMN)=NCMoment(1)
!         VarCumul(2,jMN)=CtMoment(2)
!         if(nKappa>2) VarCumul(3,jMN)=CtMoment(3)
!         if(nKappa>3) VarCumul(4,jMN)=CtMoment(4)-3.0*CtMoment(2)*CtMoment(2)
        if(PrtDetail>2) then
...
!           the leading factor of which is cancelled by the denominator of Slope:
            do kGMC=1,ModelOrder
              TermsOfOrderK=0.0
              do iGMC=0,kGMC
!               write(PrtUni,'(1x,i2,f10.3,f9.6,2i3,3e9.2,a)')
!    +            iPtGMC,x2,y2,kGMC,iGMC,x1,x2,TermsOfOrderK,' ToOK'
...
!         if((AvgOrdinate>0.0000015).and.(AvgOrdinate>1.01*PriorAvgOrd)) then
...
!           VarCumul(1,j:nVar) = 0.0 IS THIS CORRECT?  AGT replies 'no; see below'
!           VarCumul(:,j)=0.0
...
!         if(PrtDetail>2) write(PrtUni,'(1x,3i4,f9.1,f9.6,f11.4)')
!      +    j,UnitOwning(j),BlockPos(j),Capacity(j),EquivAvail(j),
!      +    VarCumul(1,j)
        end do
...
!     1-Q(z) = N(-1,z)
!      - [N(2,z)*G1/6]
!      + [N(3,z)*G2/24 + N(5,z)*G1*G1/72]
!      - [N(4,z)*G3/120) + N(6,z)*G1*G2/144 + N(8,z)*G1*G1*G1/1296]
!      + [N(5,z)*G4/720) + N(7,z)*(G2*G2/1152 + G1*G3/720)
!      +  N(9,z)*G1*G1*G2/1728 + N(11,z)*G1*G1*G1*G1/31104].
...
!     Q(z) = 1-N(-1,z) + f(z)*(
!     + [R(2,z)*G1/6]
!     - [R(3,z)*G2/24 + R(5,z)*G1*G1/72]
!     + [R(4,z)*G3/120) + R(6,z)*G1*G2/144 + R(8,z)*G1*G1*G1/1296]
!     - [R(5,z)*G4/720) + R(7,z)*(G2*G2/1152 + G1*G3/720)
!     +  R(9,z)*G1*G1*G2/1728 + R(11,z)*G1*G1*G1*G1/31104]).
...
!       implicitly zero and need not be covered in the jFTA-loops below.
        ...
        ! HPcoef=0.0 ! call cinitd(HPcoef,1+I4_SupiHP,0.0)
          call initd0(HPcoef,int2(1+SupiHP),0.0)
!         HPcoef(2)=g1/6.
          HPcoef(2)=c1
!                6=3!
          if(nGamma>=2) then
...
!       do j=1,nVar
!         if((VarState(j)<BasicKey).and.AuxiCovers(j))
!      !    write(PrtUni,FmtNoPhi1)j,FctOwning(j),ChrVS(j),AuxRHS(AuxRow(j)),
!    +      write(PrtUni,FmtNoPhi2)j,FctOwning(j),ChrVS(j),UnitOwning(j),
!    +        BlockPos(j),x(j),cOrg(j),DualVar(j),Capacity(j),AvgDurat(j)
!       end do
!
!       do j=1,nVar
!         if((VarState(j)<BasicKey).and.(.not.AuxiCovers(j)))
!    +      write(PrtUni,FmtNoPhi2)j,FctOwning(j),ChrVS(j),UnitOwning(j),
!    +        BlockPos(j),x(j),cOrg(j),DualVar(j),Capacity(j),AvgDurat(j)
!       end do
...
!       write(PrtUni,'(1x,a,f13.5,a,f15.2,a\)') 'excluding jîPsi0,',
!    +    xAccum,'=äx;',InnerProduct(cOrg,x,nVar),'=cost'
...
!       logical*1 FromHold
        ...
        if(UsingCumul) then
          ...
          ! EqLCumul=HeldCumul ! call cmove(HeldCumul,EqLCumul,I4_4LCmBnM)
            call CopyLinRVec1(HeldCumul,EqLCumul,int2(I4_4LCmBnM/4))
          else              ! capture the current array
          ! HeldCumul=EqLCumul ! call cmove(EqLCumul,HeldCumul,I4_4LCmBnM)
            call CopyLinRVec1(EqLCumul,HeldCumul,int2(I4_4LCmBnM/4))
          end if
        else
          if(FromHold) then
...
!       PriorBlockLoaded=.false. ! during debugging phase only
...
!         if(PrtDetail>2) write(PrtUni,'(1x,a,2f11.4,4e14.6)') 'Kap',
!    +      EqLCumul(1,0),sqrt(EqLCumul(2,0)),
!    +     (EqLCumul(kLA,0),kLA=3,nKappa)
...
!         if((jPrBk==InvalidIndex).or.(BlockPos(jMB)<2)) then ! kInf before 980811
!         if(AuxiCovers(jMB).or.(BlockPos(jMB)==2)) then ! allowed reversal of 1&2
...
!         jDisFits=xAggNoSplit+xKeyDecr+SplitCrit<xDischgUsed ! (ignore noise)
          ...
          ! assign jDis to the facet; x(jDis) 'splits' x(jKey)
            ...
            if(AuxiCovers(jKey).eqv..true.) then
              ...
              do while((iAux<nAux).and.(.not.SameStgEqn))
                iAux=iAux+1
                SameStgEqn=(AuxCovers(iAux,jKey).
     +                  and.AuxCovers(iAux,jDis))
              end do
            end if
            if(PrtDetail>2)
...
!             if(.not.InPriorFct) x(jDis)=xDisPrev ! restore original value
            ...
            ! if(InPriorFct) then ! trouble past, allow further assignments
            !   iLRVInf=1
            !   InPriorFct=.false. ! dubious, as it allows more than one regression
            ! end if
            end if
...
!       call cinitb(WouldBeZero,I4_nVar,.false.) ! enable all vars for basis entry
        ...
        ! EqLDur=EqLDn0 ! call cmove(EqLDn0,EqLDur,int4(4*(1+nPoints))) ! with 0 outages included by convolution
          ...
          I4_4LCmBnM=4*int4(LimCumulants)*(1+nAbnormalModes) ! residual has index 0
...
c                   GrayOrder(HalfSize+iGray-1)= ! since lower half of GrayOrder<HalfSize ...
c    +              GrayOrder(HalfSize-iGray)+HalfSize ! ... there's no carries
                  end do
...
c    +              (mod(InclBitPattern/2**(iMSb-1),2)==0))
