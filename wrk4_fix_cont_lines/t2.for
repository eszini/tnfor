...
!      !if(CallCount>CallsSkipped) call HaltWith(0,CallsSkipped+1,'aborted before FOC #calls=')
        ...
        call DeallocFAInt ! vars allocated in StorePlantData
        call DeallocAuxes
!        if(PrtDetail>0) write(*,*)' at end ',clk(),' pd. ',CallCount,
!     +    nIter
...
!       iOfFacAlgs(jD:nVar-1)=iOfFacAlgs(jD+1:nVar) ! call cmove(iOfFacAlgs(jD+1),iOfFacAlgs(jD),I4_2nRem)
!       UnitOwning(jD:nVar-1)=UnitOwning(jD+1:nVar) ! call cmove(UnitOwning(jD+1),UnitOwning(jD),I4_2nRem)
!       BlockPos  (jD:nVar-1)=BlockPos  (jD+1:nVar) ! call cmove(BlockPos  (jD+1),BlockPos  (jD),I4_2nRem)
!       Capacity  (jD:nVar-1)=Capacity  (jD+1:nVar) ! call cmove(Capacity  (jD+1),Capacity  (jD),I4_4nRem)
!       EquivAvail(jD:nVar-1)=EquivAvail(jD+1:nVar) ! call cmove(EquivAvail(jD+1),EquivAvail(jD),I4_4nRem)
!       CELB      (jD:nVar-1)=CELB      (jD+1:nVar) ! call cmove(CELB      (jD+1),CELB      (jD),I4_4nRem)
!       CEUB      (jD:nVar-1)=CEUB      (jD+1:nVar) ! call cmove(CEUB      (jD+1),CEUB      (jD),I4_4nRem)
!!      InputRate (jD:nVar-1)=InputRate (jD+1:nVar) ! call cmove(InputRate (jD+1),InputRate (jD),I4_4nRem)
!       cOrg      (jD:nVar-1)=cOrg      (jD+1:nVar) ! call cmove(cOrg      (jD+1),cOrg      (jD),I4_4nRem)
!       cMod      (jD:nVar-1)=cMod      (jD+1:nVar) ! call cmove(cMod      (jD+1),cMod      (jD),I4_4nRem)
!       x         (jD:nVar-1)=x         (jD+1:nVar) ! call cmove(x         (jD+1),x         (jD),I4_4nRem)
!       BlockOfSameUnitAs(jD:nVar-1,1)=BlockOfSameUnitAs(jD+1:nVar,1)
!       BlockOfSameUnitAs(jD:nVar-1,2)=BlockOfSameUnitAs(jD+1:nVar,2)
...
!             Capacity  (j3:j3+I4_BRem)=Capacity  (j0:j0+I4_BRem) ! call cmove(Capacity  (j0),Capacity  (j3),I4_BRem)
!             cOrg      (j3:j3+I4_BRem)=cOrg      (j0:j0+I4_BRem) ! call cmove(cOrg      (j0),cOrg      (j3),I4_BRem)
!             the above shifts are now performed by the loop:
              do i1=0,I4_BRem
...
!       write(PrtUni,'(1x,a,f13.5,a,f15.2,a\)') 'excluding jPsi0,',
!    +    xAccum,'=x;',InnerProduct(cOrg,x,nVar),'=cost'
