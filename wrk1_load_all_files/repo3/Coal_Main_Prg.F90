...
!
               ...
               ENDIF
               MAX_SUPPLY_BASIN_ID_BASEFILE = MAX(INT(Basin ID,4),
...
!
     ...
     +                                     ACTIVE_BASINS_IN_BASEFILE + 1
               MAX_BASIN_ID_BASEFILE = MAX(Basin HI ID,
...
!
!
      SUBROUTINE COAL_TRANSPORTATION_LINKS_MAKEBIN(BASE_FILE_NAME,
     +                                             FILE_CODE)
!         
C***********************************************************************
C
...
!
     ...
     +               MAX(MAX_EV_PLANT_ID_BASEFILE(FILE_POS),Plant EV ID)
               MIN_EV_PLANT_ID_BASEFILE(FILE_POS) = 
     +               MIN(MIN_EV_PLANT_ID_BASEFILE(FILE_POS),Plant EV ID)
               WRITE(11,REC=IREC) DELETE,
     +                            Basin to ,
     +                              Plant,
     +                            Active,
...
!
               ...
               IREC = IREC + 1
               IF(Active /= "N" .AND. DELETE < 8)
     +                ACTIVE_COAL_CONTRACTS_IN_BASEFILE =
     +                             ACTIVE_COAL_CONTRACTS_IN_BASEFILE + 1
               WRITE(11,REC=IREC) DELETE,
     +                            Contract Name,
     +                            Contract ID,
     +                            Active,
...
!***********************************************************************
         USE COAL_cnw_FILES_READ_INPUT
         PROCESSING_COAL_CONTRACTS_FILE_OL='BC'
...
!***********************************************************************
      ...
      CHARACTER (LEN=50) :: Basin to=" ",Plant=" "
...
! ALLOCATE ARRAYS
         ...
         CALL AllocateArray(Basin_QtyLBByYr,0,30,1,INT4(nArray))
         CALL AllocateArray(Basin_QtyUBByYr,0,30,1,INT4(nArray))
...
! SO2 & Scrubber INPUT
         ...
         IF(.NOT. COAL_SO2_Scrubber_FILE_EXISTS) THEN
            ...
            nStateSO2Limits = 0
            StateCapVolatility ByYear = 0.
            StateAssignedToGroup = 0
         ELSE
            OPEN(10,FILE=TRIM(OUTPUT_DIRECTORY())//
     +           PROCESSING_COAL_SO2_Scrubber_FILE_OL//
     +                       TRIM(COAL_SO2_Scrubber_BINARY_FILE),
     ...
     +                              T_StateCapVolatility 
               ...
               StateCapVolatility ByYear(0:30,iArray) =
...
!  END READ COAL SUPPLY  INPUT
      ...
      END FUNCTION
!***********************************************************************
      LOGICAL FUNCTION READ_PLANT_CONTRACT_DEMAND_LINK_DATA(BaseYr,
     +                                                      EndYr)
!***********************************************************************
      USE COAL_cnw_FILES_READ_INPUT
      USE COAL_MODEL_INPUT_TO_cnw
     ...
     +                      Transport Vehicles
      ...
      INTEGER (KIND=4) :: FILE_NO,Basin HI ID,Plant EV ID
     ...
     +                             Transport_Quantity_Limit_By_Year(:,:)     
!
         READ_PLANT_CONTRACT_DEMAND_LINK_DATA = .FALSE. 
         IF(.NOT. (COAL_LINK_DATA_FILE_EXISTS .AND.
     +                (PLANT_DEMAND_DATA_FILE_EXISTS .OR.
     +                         COAL_CONTRACTS_DATA_FILE_EXISTS))) RETURN
         READ_PLANT_CONTRACT_DEMAND_LINK_DATA = .TRUE. 
!
!***********************************************************************
...
!     +                         Transport Vehicles
            IF(IOS /= 0) EXIT
            IF(Active == "N" .OR. DELETE > 7) CYCLE
            IF( .NOT. BasinAvailable(Basin HI ID)) CYCLE
            IF(.NOT. (Plant EV ID >= EVPlantIDMin .AND.
     +                               Plant EV ID <= EVPlantIDMax)) CYCLE
            PlantToBasinLinkActive(Plant EV ID) = .TRUE. 
            iArray = iArray + 1
            IF(iArray > nArray) EXIT
         ENDDO
...
! ALLOCATE ARRAYS
            ...
            CALL AllocateArray(
     +                  Plant_Annual_Demand_By_Year,0,30,1,INT4(nArray))
            CALL AllocateArray(
     +                        MaxSO2InCoalBlend,0,30,1,INT4(nArray))
            CALL AllocateArray(
     +                      MaxSO2ForAnyCoalInBlend,0,30,1,INT4(nArray))
            CALL AllocateArray(SOxCapitalCostByYear,0,30,1,INT4(nArray))
            CALL AllocateArray(
     +                      SOxCarryingChargeByYear,0,30,1,INT4(nArray))
            CALL AllocateArray(SOxVarCostByYear,0,30,1,INT4(nArray))
            CALL AllocateArray(SOxFixedCostByYear,0,30,1,INT4(nArray))
     ...
     +                                        EVPlantIDMin,EVPlantIDMax)
            ...
            CALL AllocateArray(
     +                PlantFractionUnscrubbedByYear,0,30,1,INT4(nArray))
...
! ALLOCATE ARRAYS
C         IF(PROCESSING_COAL_LINK_FILE_OL(1) == 'OL') THEN
C            nGenericTransLinks = ACTIVE_COAL_LINK_IN_OVERLAY(1)
C         ELSE
C            nGenericTransLinks = ACTIVE_COAL_LINK_IN_BASEFILE(1)
C         ENDIF
         ...
         CALL AllocateArray(
     +             Transport_Quantity_Limit_By_Year,1,INT4(nArray),0,30)
         CALL AllocateArray(Transport_Price_By_Year,1,INT4(nArray),0,30)
     ...
     +                       PriceEscalation,
     +                       Transport_Quantity_Limit_By_Year(iArray,0),
     ...
     +                       T_Plant_EV_ID(iArray)<=EVPlantIDMax)) CYCLE
            ...
            T_Route_Name(iArray) = TRIM(Basin to)//"-"//TRIM(Plant)
...
! transportation price
     ...
     +                             Transport_Price_By_Year)
         ...
         CALL AllocateArray(
     +                 Route_Quantity_Limit_By_Year,1,INT4(nArray),0,30)
         CALL AllocateArray(Route_Price_By_Year,1,INT4(nArray),0,30)
     ...
     +                   Transport_Quantity_Limit_By_Year(1:nArray,0:30)
...
! READ COAL CONTRACTS  INPUT
         IF(COAL_CONTRACTS_DATA_FILE_EXISTS) THEN
            IF(PROCESSING_COAL_CONTRACTS_FILE_OL == 'OL') THEN
               nCoalContracts = ACTIVE_COAL_CONTRACTS_IN_OVERLAY
            ELSE
               nCoalContracts = ACTIVE_COAL_CONTRACTS_IN_BASEFILE
            ENDIF
...
! ALLOCATE ARRAYS
            ...
            OPEN(10,FILE=TRIM(OUTPUT_DIRECTORY())//
     +           PROCESSING_COAL_CONTRACTS_FILE_OL//
     +                            TRIM(COAL_CONTRACTS_BINARY_FILE_NAME),
     +                            ACCESS="DIRECT",STATUS="OLD",
     +                                        RECL=COAL_CONTRACTS_LRECL)
            IREC = 0
...
! ALLOCATE ARRAYS
     ...
     +                Supply_Cost_By_Year(20,nArray,0:30))
         ...
         CALL AllocateArray(Supply_Minimum_Annual_Production,nArray)
         CALL AllocateArray(Supply_Maximum_Annual_Production,nArray)
         ...
         CALL AllocateArray(Supply_Production_Cost_Escalation,nArray)
         OPEN(10,FILE=TRIM(OUTPUT_DIRECTORY())//
     ...
     +                      Supply_Minimum_Annual_Production(iArray),
     +                      Supply_Maximum_Annual_Production(iArray),
     +                      Supply_Curve_Points(iArray),
     +                      Supply_Production_Escalation(iArray),
     +                      Supply_Production_Cost_Escalation(iArray),
...
! Escalate costs!
         DO iArray = 1, nCoalSupply
            ...
            IF(Supply_Production_Cost_Escalation(iArray) >= 0.) THEN
               DO Yr = 1, 30
                  Supply_Cost_By_Year(1:iCurPnts,iArray,Yr) =
     +              (1.+ Supply_Production_Cost_Escalation(iArray)/100.)
     +                     * Supply_Cost_By_Year(1:iCurPnts,iArray,Yr-1)
               ENDDO
            ELSE  ! A VECTOR IS BEING USED
               VectorNum =ABS(Supply_Production_Cost_Escalation(iArray))
               IF(RETURN_COAL_VECTOR(Vector_Values,VectorNum)) THEN
!                 The vector is a list of pointers to a vector that has
!                 20 values in it
               ENDIF
...
!  END READ COAL INPUTS  INPUT
      ...
      END FUNCTION
!***********************************************************************
      LOGICAL FUNCTION READ_GENERIC_PLANT_DEMAND_LINK_DATA(BaseYr)
!***********************************************************************
      USE COAL_cnw_FILES_READ_INPUT
      USE COAL_MODEL_INPUT_TO_cnw
      USE ArrayAllocationInterface
...
! ALLOCATE ARRAYS
         CALL AllocateArray(GenericLinkUsed,0,int4(nGenericTransLinks))
         GenericLinkUsed = .FALSE. 
         CALL AllocateArray(GenericLinkUsedTemp,0,
     +                                         int4(nGenericTransLinks))
         ...
         CALL AllocateArray(
     +           Generic_Route_Quantity_By_Year,1,INT4(nArray),0,30)
         CALL AllocateArray(
     +              Generic_Route_Price_By_Year,1,INT4(nArray),0,30)
...
!
     ...
     +                             GenericTransportPtrToTG(GenPlantID,1)
            ...
            Generic_Route_Name(iArray)=TRIM(Basin to)//"-"//TRIM(Plant)
            CALL ROUTE_ESCALATIONS(iArray,
     +                             QuantityEscalation,
...
! READ PLANT DEMAND INPUT
         IF(GENERIC_DEMAND_DATA_FILE_EXISTS) THEN
            IF(PROCESSING_PLANT_DEMAND_FILE_OL == 'OL') THEN
               nGenericCoalDemand = ACTIVE_GENERIC_DEMAND_IN_OVERLAY
            ELSE
...
! ALLOCATE ARRAYS
            ...
            CALL AllocateArray(Generic_Plant_Fraction_Unscrubbed,nArray)
            ...
            CALL AllocateArray(
     +            Generic_Plant_ForecastDemand,0,30,1,INT4(nArray))
            CALL AllocateArray(
     +                    Generic_Plant_CoalDemandLinkToCLPlants,nArray)
            ...
            CALL AllocateArray(
     +                    Generic_MaxSO2InCoalBlend,0,30,1,INT4(nArray))
            CALL AllocateArray(
     +              Generic_MaxSO2ForAnyCoalInBlend,0,30,1,INT4(nArray))
     ...
     +                       Generic_Plant_Fraction_Unscrubbed(iArray),
     +                       MaxSO2OfBlend,
     +                       PercentIncreaseSO2,
     +                       PercentDecreaseSO2,
     +                       SO2LowerBound,
...
!     +               "'SDRnOfLk(2,iLink)=RouteID',"//
     ...
     +               "Transport_Vehicles,"
         ...
         ENDIF
!
         CALL AllocateArray(UnitCoalCostByBTU,0,6,1,INT4(nCoalDemand))
         CALL AllocateArray(SO2BlendedRate,INT4(nCoalDemand))
         SO2BlendedRate = 0.
         UnitCoalCostByBTU = 0.
         AddScrubber = .FALSE.  
...
!
     ...
     +                         SulphurEmissionsLimit(Yr,iS) == 0.) CYCLE
            ...
            StateCapVolatility (nS) = StateCapVolatility ByYear(Yr,iS)
...
C***********************************************************************
     ...
     +                       'Plant Name Unit EVID','C',INT2(55),'F','D'
