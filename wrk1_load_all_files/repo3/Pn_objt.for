C     Last change: msg 3/17/2022 11:56:39 AM
...
C
      INCLUDE 'SPINLIB.MON'
      INCLUDE 'SIZECOM.MON'
      CHARACTER*20   R_LOCAL_UNIT_NAME
      INTEGER*2      R_LOCAL_DATA_POINTER,
     +               R_LOCAL_TRANS_ID
      REAL R_LOCAL_UNIT_CAP
!          
      INTEGER*2 IREC,INUNIT,DELETE,LRECL/1628/, ! 120218 ! 081818 ! 022510 ! 052609 1547 ! 011809 1510 ! ???? 1426! 091107 ! 100506 for KCPL ! 080906 ! /961/
     +           BASE_PN_RECORDS,R_BASE_PN_RECORDS
      ...
      INTEGER*2 LAST_UNIT_NUM_OPENED/10/,UNIT_NUM
      CHARACTER*5 BASE_FILE_NAME,OVERLAY_FAMILY_NAME,NWCAP_PROD_FIL
      CHARACTER*20 STATE_PROV_NAME
      CHARACTER*30 DESC,CO2_BASIN_NAME
      CHARACTER* 36 THERMAL_GUID
      CHARACTER*256 FILE_NAME
      CHARACTER*256 BASE_FILE_DIRECTORY,OUTPUT_DIRECTORY
      LOGICAL*4 FILE_EXISTS,NEW_CL_UNIT_OPENED
C DECLARATION FOR DBREAD COMMON BLOCK
!      CHARACTER*1024 RECLN
      CHARACTER*2048 RECLN ! 011809 
C DECLARATION FOR /FOSSIL FILE/
      INTEGER*2 GENGRP,ONLIMO,OFLIMO,PRESCR,SDESCR,VRESCR,
...
!     
     ...
     +     CO2_RETRO_HEAT_MULT,
     +     CO2_RETRO_CAP_MULT
C CHANGED FROM INTEGER*2 TO REAL 5/14/92 ADDED IDS AS INTEGER*1
      INTEGER*2 ANNUAL_CL_FIXED_COST_ESC
      REAL*4 DISPATCH_MULT,MAINT_DAYS,EXCESS_ENERGY_SALES
      REAL MW(2)
      INTEGER*2 RESOURCE_ID,FUEL_SUPPLY_ID
...
C ADDED 12/10/92
     ...
     +          PRIM_FUEL_EMISS_PTR
      ...
      CHARACTER*26 FILE_TYPE/'Expansion-Production Units'/
      CHARACTER*2 NEWPLT_PROD_OL/'BC'/
      CHARACTER*6 PRIM_FUEL_TYPES
      CHARACTER (LEN=2) :: CL_LOAD_TYPE,UNIT_TYPE,UNIT_TYPE_CATEGORY

C
C OPTIONS CHECKING
C
      INTEGER*2 I,R_UNITS_2_CHECK,
     +          R_DATA_POINTER(*)
      REAL R_UNIT_CAP(*)
      CHARACTER*(*) R_UNIT_NAME(*),R_RESOURCE_TYPE(*)
      REAL CAP_MW,GET_CL_MW_FROM_POINTR,POINTR,MONTH_MULT
      INTEGER*2 YEAR_START,PEAK_MONTH,DAY_TYPE_ID
     ...
     +                     LINKED_BETTERMENT_OPTION="False"
      CHARACTER*1 THERMAL_AGGREGATED_UNIT
...
C SPCapEx additions Nov. 2005
     ...
     +                 START_UP_COSTS_ESCALATION
      ...
      INTEGER*2 :: capacity_market_pointer_pno
...
C***********************************************************************
C
C CONVERT THE EXPANSION-PLANT FILE
C***************************************************************
      ENTRY PN_MAKEBIN
C***************************************************************
...
!
     ...
     +               ',,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,'
C
C VARIABLES THAT DO NOT PROPAGATE LEFT TO RIGHT
C
C
               READ(RECLN,*) 
...
!               ENDIF
     ...
     +               INTER_BLOCKS,
c SP CapEx Variables added 11/17/05 MSG
c Recomputed variable numbers 12/21/06 TMS
     +               FIRST_YEAR_DECOMM_AVAIALABLE,     ! 152
     +               DECOMMISSIONING_BASE_YR_COST,            ! 153
     +               DECOMMISSIONING_COST_ESCALATION,
     +               ANNUAL_ENERGY_PLANNING_FACTOR,
...
C***********************************************************************
C
C OVERLAY THE EXPANSION-PLANT FILE
C***************************************************************
...
C***************************************************************
     ...
     +                        R_RESOURCE_TYPE)
C***************************************************************
C 2/23/95. GAT. OUT BECAUSE CLOSED TWICE
C         CALL OPEN_NEWPLT_FILE(10)
C
...
C
     ...
     +                                           PEAK_MONTH(YEAR_START))
               ...
               ENDIF
C           ELSEIF(MONTHLY_CAPACITY_POINTER < 0 .AND.
C    +                                 CL_CAP_AREA_LINKED(UNIT_NO)) THEN
C              LOCAL_NAME = UNITNM(UNIT_NO)
C              LOCAL_POINTER = ABS(MONTHLY_CAPACITY_POINTER)
C              CAP_MW = MIN(GET_CLASS_PEAK_NET_DSM_FOR_CAP(
C    +                      YEAR_START,LOCAL_NAME,LOCAL_POINTER),CAP_MW)
            ENDIF
C
