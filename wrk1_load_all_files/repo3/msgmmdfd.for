...
C
      ...
      INCLUDE 'SIZECOM.MON'
C
      INTEGER*2 DELETE,IREC,INUNIT,LRECL/136/,I
      INTEGER ACCTNO
      INTEGER*4 IOS,IOS_BASE
      INTEGER*2 NUMBER_OF_BC_DEBIT_CLASSES/0/,
     +          MAX_BC_DEBIT_CLASS_ID_NUM/0/
      INTEGER*2 NUMBER_OF_OL_DEBIT_CLASSES/0/,
     +          MAX_OL_DEBIT_CLASS_ID_NUM/0/
      INTEGER*2 UNIT_NUM/10/,ASSET_CLASS_NUM,ASSET_CLASS_VECTOR
      INTEGER*2 R_NUM_OF_DEBIT_CLASSES,R_MAX_DEBIT_CLASS_NUM,
     +          R_DEBIT_CLASS_POINTERS(*)
      INTEGER*2 R_UNIT_NUM,YEAR_DEBIT_BOOKED,MONTH_DEBIT_BOOKED
      CHARACTER*5 BASE_FILE_NAME,OVERLAY_FAMILY_NAME,DEBITFIL
      CHARACTER*256 FILE_NAME
      CHARACTER*256 BASE_FILE_DIRECTORY,OUTPUT_DIRECTORY
      CHARACTER*256 DATA_DRIVE
      CHARACTER*20 WVPA_TRACKING_TYPE
      CHARACTER*35 DESC,TEMP_DESC
      CHARACTER*1 TAX_TIMING_CLASSIFICATION,
...
C DECLARATION FOR /DEBIT FILE/
      CHARACTER*1 INCOME_TREATMENT
      REAL*4 AMORT_DELAY_PERIOD
      REAL*4 DEFERRED_TAX_ALLOCATION,
     +       CURRENT_TAX_DEDUCTION_PERCENT
      REAL*4 DEBAMT,CAMORT,WOYRS
      CHARACTER*23 FILE_TYPE/'Deferred Debit Accounts'/
      CHARACTER*1 METHOD
      CHARACTER*2 DEBITOL/'BC'/,R_DEBITOL
      CHARACTER*8 BALANCE_SHEET_TREATMENT
      INTEGER*2 DEBIT_BC_ASSET_CLASS_POINTER(:),
...
C
      LOGICAL*1 SAVE_BASE_CASE,LGandE
      CHARACTER ABOVE
      PARAMETER(ABOVE = 'A')
      REAL*4 GROSS_DD,AMORT
      REAL*4 LGE_GROSS_DD,LGE_NET_DD
      INTEGER*2 T,IREC,TACCTS,DELETE,YR,MO,
     +          TACCTS_CR,TACCTS_DR,ACTIVE_ACCOUNTS
      INTEGER*4 IOS
C     DECLARATION FOR /FAINPT/
      REAL*4 NET_DD,WOYRS,CASH_ADDITONS
      REAL*4 INT_DEBAMT,INT_CAMORT
     ...
     +       DEFERRED_ADDITONS(AVAIL_DATA_YEARS)
      ...
      CHARACTER*1 METHOD,DUMMY_TYPE,AMORT_DATA_TYPE
C
      INTEGER*2 R_YR,ASSET_CLASS_POINTER(:),CLASS_POINTER,YR_BAL
      INTEGER*2 R_CLASS,YEAR_DEBIT_BOOKED,MONTH_DEBIT_BOOKED
      CHARACTER*1 TAX_TIMING_CLASSIFICATION,DESC*35,INDEX_NAME*34,
     +            ACCOUNT_ACTIVE,DEFERRAL_TYPE
      CHARACTER*8 BALANCE_SHEET_TREATMENT,TEMP_STR
      CHARACTER*20 WVPA_TRACKING_TYPE
      LOGICAL*1 R_CLASS_EXISTS,CPL_IS_ACTIVE
     ...
     +     MONTHLY_AMORT_AMOUNT(:,:)
      REAL*4 R_TDDB,R_TDDRBB,R_TAMRTE,R_TNDB,R_TCAMRT,R_RB_DD_AMRTE
      REAL*4 R_INTEREST_AMORTIZATION,
     +       R_ATL_AMORTIZATION,
     +       R_BTL_AMORTIZATION,
     +       R_ATL_DEF_TAX,
     +       R_BTL_DEF_TAX,
...
c     +               INTEREST_AMORTIZATION_BAL,
     ...
     +               TAX_TIMING_DIFFERENCES,
     +               PURCHASE_POWER_AMORT
      INTEGER*2 START_AMORT_FULL_YR,I
      CHARACTER*1 INCOME_TREATMENT,O_and_M,OTHER_OM,PURCHASE_POWER
      REAL*4 AMORT_DELAY_PERIOD
      REAL*4 DEFERRED_TAX_ALLOCATION,
     +       CURRENT_TAX_DEDUCTION_PERCENT
      LOGICAL*1 IN_RATEBASE
      CHARACTER*1 BTL,INTEREST
      PARAMETER (BTL='B',INTEREST='I',O_and_M='V',OTHER_OM='O',
     +           PURCHASE_POWER='P')
      CHARACTER*1 Temporary,Permanent
...
C
      REAL*4 CLOSING_NET_DD,OPENING_NET_DD,
     ...
     +       BTL_DEFERRED_TAX_BASIS
      LOGICAL*1 REPORTS_ACTIVE,DEBIT_REPORT,
     +          REPORT_ALL_ACCOUNTS,REPORT_HEADER_OPEN/.FALSE./
      INTEGER*2 REPORTING_UNIT,DEBIT_FILE_RPT_HEADER,
     ...
     +       TAX_VARIABLES(0:12,*)
      INTEGER*2 DELAY_MONTHS,DELAY_YEARS,START_AMORT_MONTH
      LOGICAL*1 TAKE_FULL_MONTH
      INTEGER*2 YR2,MO2,WO_MONTHS
      REAL*4 HALF_MONTH_AMORTIZATION,MONTHLY_AMORTIZATION,
     +       CURRENT_MONTH_AMORTIZATION,ANNUAL_AMOUNT
      REAL*4 ATL_TAX_EXPENSE,
     +       BTL_TAX_EXPENSE,
     ...
     +       R_FASB_133_AMORT
      ...
      LOGICAL*1 WVPA,WVPA_TRACKER_DATABASE_FROM_DD
...
C
     ...
     +         MONTHLY_AMORT_AMOUNT(0:12,0:FINANCIAL_SIMULATION_YEARS))
      CALL OPEN_DEFERRED_DEBIT_FILE(INT2(11))
      TACCTS = 0
      TACCTS_CR = 0
      TACCTS_DR = 0
      IREC = 0
...
C
     ...
     +                                                   ALL_VERSIONS,0)
         ...
         ELSE
            BOOKED_YEAR = MAX(0,YEAR_DEBIT_BOOKED-BASE_YEAR)
         ENDIF
...
C
     ...
     +             AMORT_DELAY_PERIOD-BASE_YEAR-BOOKED_YEAR,BOOKED_YEAR)
         ELSE
...
C
         ...
         IF(YEAR_DEBIT_BOOKED < 0) THEN ! GET VECTOR OF CASH ADDITIONS
            CALL GET_MONTHLY_ANNUAL_VALUES(ABS(INT2(YEAR_DEBIT_BOOKED)),
     +                                     DATA_TYPE,
     +                                     VECTOR_TYPE,
     +                                     DEFERRED_ADDITONS,
     +                                     MONTHLY_VALUES(1,1),
...
C
         ANN_AMORT = 0.
         IF(WOYRS < 0.) THEN
            CALL GET_MONTHLY_ANNUAL_VALUES(ABS(INT2(WOYRS)),
     +                                     AMORT_DATA_TYPE,
     +                                     VECTOR_TYPE,
     +                                     VECTOR_VALUES,
     +                                     MONTHLY_VALUES(1,1),
...
C               IF(INCOME_TREATMENT == INTEREST) THEN
c               IF(BAL_SHEET_POS == Unamortized Interest Balance) THEN
c                  DEBIT_OPENING_NET_DD = 0.
c                  DEBIT_CLOSING_NET_DD = 0.
c                  INTEREST_OPENING_NET_DD = OPENING_NET_DD
c                  INTEREST_CLOSING_NET_DD = CLOSING_NET_DD
c               ELSE
...
C
      ...
      IF(SAVE_BASE_CASE) THEN
         CALL OPEN_DD_BASE_CASE_FILE(INT2(10))
...
c                  INCOME_VARIABLES(MO,Monthly Amortization) =
...
c     +                          + ATL_AMORTIZATION(MO,YR,ASSET_CLASS,Dr)
     ...
     +                          INCOME_VARIABLES(MO,
     +                                         Monthly Other BTL Amort)
     +                          + BTL_AMORTIZATION(MO,YR,ASSET_CLASS,Dr)
c                  INCOME_VARIABLES(MO,BTL Monthly Amortization) =
c     +                          INCOME_VARIABLES(MO,
c     +                                         BTL Monthly Amortization)
...
C     +                      + PURCHASE_POWER_AMORT(MO,YR,ASSET_CLASS,Dr)
C     +                      + OTHER_OM_AMORT(MO,YR,ASSET_CLASS,Dr)
C     +                      + VARIABLE_OM_AMORT(MO,YR,ASSET_CLASS,Dr)
C
...
C     +                      - PURCHASE_POWER_AMORT(MO,YR,ASSET_CLASS,Cr)
C     +                      - OTHER_OM_AMORT(MO,YR,ASSET_CLASS,Cr)
C     +                      - VARIABLE_OM_AMORT(MO,YR,ASSET_CLASS,Cr)
...
C
         IF(R_CLASS <= MAX_ASSET_CLASS_NUM) THEN
            ...
            IF(ASSET_CLASS > 0 .OR. R_CLASS == 0) THEN
               YR = R_YR ! - 1
               DO MO = 0, 12
                  BALANCE_VARIABLES(MO,Monthly Net Goodwill) =
     +                               TNDB(MO,YR,ASSET_CLASS,Goodwill,Dr)
                  BALANCE_VARIABLES(MO,Monthly Net Regulatory Assets) =
     +                      TNDB(MO,YR,ASSET_CLASS,Regulatory Assets,Dr)
                  BALANCE_VARIABLES(MO,Monthly Net FASB 109) =
     +                               TNDB(MO,YR,ASSET_CLASS,FASB 109,Dr)
                  BALANCE_VARIABLES(MO,Monthly Net FASB 133) =
...
C DEFERRED CREDITS
     ...
     +                  TNDB(MO,YR,ASSET_CLASS,Regulatory Liabilites,Cr)
                  BALANCE_VARIABLES(MO,Monthly Other Deferred Credits) =
     ...
     +                                     Asset Sales Deferred Gain,Cr)
      ...
      RETURN
C***********************************************************************
      ENTRY MONTHLY_DEFERRED_DEBIT_BS_ELIM_INFO(R_YR,R_CLASS,
     +                                          BALANCE_VARIABLES)
C***********************************************************************
C
C
         IF(R_CLASS <= MAX_ASSET_CLASS_NUM) THEN
            ...
            IF(ASSET_CLASS > 0 .OR. R_CLASS == 0) THEN
               YR = R_YR ! - 1
               BALANCE_VARIABLES(:,Monthly Net Goodwill) =
     +                         BALANCE_VARIABLES(:,Monthly Net Goodwill)
     +                         - TNDB(:,YR,ASSET_CLASS,Goodwill,Dr)
               BALANCE_VARIABLES(:,Monthly Net Regulatory Assets) =
     +                BALANCE_VARIABLES(:,Monthly Net Regulatory Assets)
     +                - TNDB(:,YR,ASSET_CLASS,Regulatory Assets,Dr)
               BALANCE_VARIABLES(:,Monthly Net FASB 109) =
     +                         BALANCE_VARIABLES(:,Monthly Net FASB 109)
     +                         - TNDB(:,YR,ASSET_CLASS,FASB 109,Dr)
               BALANCE_VARIABLES(:,Monthly Net FASB 133) =
     +                         BALANCE_VARIABLES(:,Monthly Net FASB 133)
     +                         - TNDB(:,YR,ASSET_CLASS,FASB 133,Dr)
               BALANCE_VARIABLES(:,Monthly Net Deferred Debits) =
     +                 BALANCE_VARIABLES(:,Monthly Net Deferred Debits)
...
C DEFERRED CREDITS
     ...
     +                - TNDB(:,YR,ASSET_CLASS,Gain on Reaquired Debt,Cr)
               BALANCE_VARIABLES(:,Monthly Regulatory Liabilites Bal) =
     +                 BALANCE_VARIABLES(:,
     +                                Monthly Regulatory Liabilites Bal)
     +                 - TNDB(:,YR,ASSET_CLASS,Regulatory Liabilites,Cr)
               BALANCE_VARIABLES(:,Monthly Other Deferred Credits) =
     +               BALANCE_VARIABLES(:,Monthly Other Deferred Credits)
