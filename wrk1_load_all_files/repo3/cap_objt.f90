...
!
     ...
     +            PLANNING_TRIGGER,
     +            GREEN_MRX_METHOD
      LOGICAL*1   RATIOS_FOUND,READ_CAP_RATIOS_FILE,
     +            READ_ICAP_FILE,
     ...
     +            TEMP_L1,CALC_ANN_ICAP_VALUES
      LOGICAL (KIND=4) :: GRX_WITH_MRX_PLANNING
      INTEGER*2 EXPANSION_UNITS,READ_CAPACITY_OPTIONS_FILE
      INTEGER*2 HARD_WIRED_UNITS_ADDED,ADD_ALL_HARD_WIRED_UNITS,
     +          VOID_INT2,START_STUDY_ENDPOINT,
     +          UPDATE_CL_SCREEN_COSTS,
     +          NUNITS
      CHARACTER*1 PLANNING_DECISIONS,TEMP_CHR
!
...
!
      INCLUDE 'SPINLIB.MON'
      INCLUDE 'GLOBECOM.MON'
      INTEGER*4 PLAN_POINTR/0/,LAST_OPTION_POINTR
      REAL DSM_CAP(:,:)
      ALLOCATABLE :: DSM_CAP
      REAL CURRENT_TARGET_RATIO,TARGET_RATIO
      LOGICAL*1 HARD_WIRED_UNITS_IN,TARGET_MET,CONTINUE_ADDING_UNITS
      INTEGER*2 HARD_WIRED_UNITS_ADDED,ADD_HARD_WIRED_UNIT
      INTEGER*2 CUMULATIVE_UNITS_ADDED
      LOGICAL*1 ADD_OPTIM_UNIT,UNIT_ADDED
      LOGICAL*1 END_OF_STUDY_FLAG,SET_END_OF_STUDY_FLAG_TRUE
...
!
      INTEGER*2 I,ACTIVE_OPTIONS,RETURN_OPTIM_ACTIVE_OPTIONS,
     +          POINTR,CURRENT_OPTION_POINTER,
     +          ANNUAL_UNITS_FOR_OPTION,
     +          YR
      INTEGER*1 STARTING_POINTER,SET_STARTING_POINTER
!
...
!
     ...
     +     OBJECTIVE_FUNCTION_VALUE
      INTEGER*4 NUMBER_OF_SAVED_PLANS/0/,
...
!
      INCLUDE 'SPINLIB.MON'
      INCLUDE 'GLOBECOM.MON'
      LOGICAL*1 ALL_OPTIONS_TESTED/.FALSE./,
     +          SET_END_OF_STUDY_FLAG_TRUE,
     +          R_END_OF_CYCLE
      INTEGER*2 RETURN_OPTIM_ACTIVE_OPTIONS
      INTEGER*2 TOTAL_ACTIVE_OPTIONS
      INTEGER*2 NUMBER_OF_ACTIVE_OPTIONS,I,
     +          OPTION_DEPTH,CURRENT_OPTION_POINTER,
     +          OPTIM_DEPTH,START_STUDY_ENDPOINT
      INTEGER*1 ACTIVE_OPTIONS(:),OPTIONS_POINTER,STARTING_POINTER,
...
!
      INCLUDE 'SIZECOM.MON'
      INCLUDE 'GLOBECOM.MON'
      INTEGER*2 POINTER(3),YR,ICAP,ICAP_POINTER,START_POINTER,
     ...
     +     GET_SCENARIO_ELECTRIC_DEMAND
      ...
      ALLOCATABLE :: DSM_CAP
      REAL*4 TARGET_RATIO,CURRENT_TARGET_RATIO(3)
      CHARACTER*2 CAPACITY_PLANNING_METHOD,PLANNING_METHOD
      CHARACTER*1 CAP_TYPE(3),MOST_NEEDED_CAP
      LOGICAL*1   RESERVE_MARGIN_PLANNING,
     +            RESORT_RATIOS_WITHIN_YEAR,FIRST_RESERVE_ADDITION,
     +            UNIT_WAS_ADDED,EXPANSION_REPORT,
     +            TARGET_NOT_MET,FOUND_NEXT_ADDITION
      LOGICAL*1   HARD_WIRED_UNITS_IN,CONTINUE_ADDING_UNITS,
     +            GET_NEXT_ADDITION,ALWAYS_CYCLE/.FALSE./
!
      CHARACTER*1 R_CAP_TYPE
      LOGICAL*1   THRESHOLD_EXCEEDED,FILL_OPTIONS_ACTIVE
      INTEGER*1   NUMBER_OF_THRESHOLD_UNITS_LEFT,THRESHOLD_UNITS,
     +            UPDATE_THRESHOLD
      INTEGER*2 NUMBER_OF_ACTIVE_FILL_OPTIONS,R_YEAR
      REAL*4      R_CAPACITY,INCREMENTAL_CAPACITY,SO_PEAK(30),R_SO_PEAK
!
      INTEGER*2   FILL_POINTR,FILL_UNITS_NEEDED,GET_FILL_UNITS_NEEDED,
     +            A_POINTER
      REAL*4      RESERVE_CAPACITY_NEEDED,FILLER_CAPACITY_NEEDED(3)
!
      INTEGER*2   HARD_WIRED_UNITS_ADDED,ADD_ALL_HARD_WIRED_UNIT,
     +            MAX_LEAD_TIME/10/,GET_MAX_LEAD_TIME,
     +            NUMBER_OF_CAP_TYPES/3/,CAP,CURRENT_TYPE
      INTEGER*2 ANNUAL_UNITS_LEFT,REDUCE_ANNUAL_UNITS_LEFT
      INTEGER*2 AVAIALABLE_OPTIONS,RESET_ANNUAL_UNITS_LEFT
      INTEGER*2 CUMULATIVE_UNITS_ADDED,PR_ADD_HARD_WIRED_UNIT
!
...
!
     ...
     +     ADJUST_FINANCIAL_INVESTMENT
      INTEGER*2 NUM_OF_UNITS_UNDER_CONSTRUCTION(MAX_PLANNING_YEARS),
     ...
     +          MAX_LEAD_TIME/10/,GET_MAX_LEAD_TIME
      INTEGER*2 ANNUAL_UNITS_LEFT,REDUCE_ANNUAL_UNITS_LEFT
      INTEGER*2 AVAIALABLE_OPTIONS,RESET_ANNUAL_UNITS_LEFT
      LOGICAL*1 EXPANSION_REPORT,EXPANSION_REPORT_ACTIVE
      REAL FIRST_YEAR_CAPACITY,GET_OPTION_CAPACITY
      INTEGER*2 LEAD_TIME,GET_OPTION_LEAD_TIME
      INTEGER*2 TEMP_CUM_UNITS,EXPANSION_PATTERN_NO
      LOGICAL*1 WRITE_2_EXPANSION_PATTERN_REPT,L_TEMP
      REAL CAP_ALREADY_ADDED(0:3,MAX_PLANNING_YEARS),CAPACITY_ADDED
      INTEGER*2 SAVED_POINTERS(3)
      LOGICAL*1 TIME_IS_AVAILABLE_2_ACCEL
      INTEGER*2 TIME_NEEDED_2_ACCEL
      CHARACTER*1 OVER_CAPACITY_DECISION,
     +            UNDER_CAPACITY_DECISION,
     +            WHAT_DO_I_DO,
     +            RETURN_CANCEL_RULE,
     +            VOID_CHR
      INTEGER*2 UNDER_CAPACITY_TABLE_NUM,OVER_CAPACITY_TABLE_NUM
      INTEGER*2 REMAINING_CONSTRUCTION_YRS
      INTEGER*2 PLANNING_FOR_YEAR,PLANNING_YEAR,PLANNING_LEAD_TIME
!
      INTEGER*2 CANCEL_BECAUSE_OF_DELAYS,
     +          CANCEL_4_DELAYS_AND_COST
      REAL COST_LEVEL_TO_COMPLETE
!
      REAL BASE_CAPACITY_ADDED,CYCLE_CAPACITY_ADDED,PEAK_CAPACITY_ADDED
      REAL NEW_ADDITIONS(3),ADD_THIS_UNIT_2_CAPACITY
!
      CHARACTER*1 OPTION_TYPE,GET_OPTION_LOADING_TYPE
!
      INTEGER*2 NEXT_COMMITTED_OPTION,
     +          NEXT_PLANNED_OPTION,
     +          MAX_COMMITTED_OPTIONS,
     +          MAX_PLANNED_OPTIONS,
     +          MUST_BUILD_IT_LEAD_TIME
      LOGICAL*1 HAS_BEEN_ACCELERATED(:),LOG1_FALSE,ABANDON_IT,
     +          MODIFY_RESOURCE_TIMING,TEMP_L1_ARRAY(:)
      REAL PERCENT_TO_COMPLETION
      REAL OPTION_PERCENT_COMPLETED(:),
     +     TEMP_R4_ARRAY(:)
      INTEGER*2 COMMITTED_OPTION_POINTERS(:),
...
!
      ...
      NUM_OF_UNITS_UNDER_CONSTRUCTION = 0.
!
...
!
     ...
     +                       ON_LINE_YEAR <= MAX_PLANNING_YEARS) THEN
               NUM_OF_UNITS_UNDER_CONSTRUCTION(ON_LINE_YEAR) = 1 +
     +                     NUM_OF_UNITS_UNDER_CONSTRUCTION(ON_LINE_YEAR)
            ENDIF
         ENDDO
!
...
!
     ...
     +                                              < TARGET_RATIO(3,YR)
               IF(TARGET_NOT_MET) THEN
                  IF(NUM_OF_UNITS_UNDER_CONSTRUCTION(YR) /= 0) THEN
                     DO I = 1, NEXT_COMMITTED_OPTION
                        IF(COMMITTED_OPTION_TIMES_DELAYED(I)==999) CYCLE
...
! CANCEL LOGIC
     ...
     +                               COMMITTED_OPTION_START_YEAR(I)=2300
                                 NUM_OF_UNITS_UNDER_CONSTRUCTION(YR)=
     +                             NUM_OF_UNITS_UNDER_CONSTRUCTION(YR)-1
                                 CYCLE
                              ENDIF
                              IF(WHAT_DO_I_DO == 'D' .AND.
...
! THE UNIT CAN BE DELAYED
     ...
     +                             NUM_OF_UNITS_UNDER_CONSTRUCTION(YR)-1
                                 NUM_OF_UNITS_UNDER_CONSTRUCTION(YR+1)=
     +                           1+NUM_OF_UNITS_UNDER_CONSTRUCTION(YR+1)
...
! MUST BUILD IT
     ...
     +                                               FIRST_YEAR_CAPACITY
                     ...
                     ENDDO
                     EXIT !CYCLE
                  ELSEIF(NUM_OF_UNITS_UNDER_CONSTRUCTION(YR+1) /= 0)THEN
! ADD ACCELERATION LOGIC
     ...
     +                                               FIRST_YEAR_CAPACITY
                        COMMITTED_OPTION_ON_LINE_YR(I) = -1 +
     +                                    COMMITTED_OPTION_ON_LINE_YR(I)
                        NUM_OF_UNITS_UNDER_CONSTRUCTION(YR+1) = -1 +
     +                             NUM_OF_UNITS_UNDER_CONSTRUCTION(YR+1)
                        NUM_OF_UNITS_UNDER_CONSTRUCTION(YR) = 1 +
     +                               NUM_OF_UNITS_UNDER_CONSTRUCTION(YR)
...
!***********************************************************************
      ...
      use miscmod
!***********************************************************************
      CHARACTER*1 PLANNING_DECISIONS,
     +            OVER_CAPACITY_DECISION,
     +            UNDER_CAPACITY_DECISION,
     +            RETURN_CANCEL_RULE
      CHARACTER*68 FILE_NAME,OVER_UNDER_TABLE_FILE_NAME
      INTEGER*2 I,J,L,COST_STATE,COMPLETION_STATE,R_CONSTRUCTION_YEARS
      REAL R_COMPLETION_PERCENT,CONSTRUCTION_YEARS
      INTEGER*2 OVER_NUM_OF_COST_STATES,OVER_NUM_OF_COMPLETION_STATES
      INTEGER*2 UNDER_NUM_OF_COST_STATES,UNDER_NUM_OF_COMPLETION_STATES
     ...
     +          R_RETURN_CANCEL_RULE
      INTEGER IOS
      CHARACTER*1 OVER_CAPACITY_ACTIONS(:,:,:),
     +            UNDER_CAPACITY_ACTIONS(:,:,:)
      INTEGER*2 OVER_TABLE_CROSS_REF(:),
     +          ROWS_IN_OVER_TABLE(:),
     +          COLUMNS_IN_OVER_TABLE(:)
      INTEGER*2 UNDER_TABLE_CROSS_REF(:),
     +          ROWS_IN_UNDER_TABLE(:),
     +          COLUMNS_IN_UNDER_TABLE(:)
      INTEGER*2 DELETE,TABLE_NUMBER,
     +          TABLE_COLUMNS,TABLE_ROWS,TABLE_POINTER
      CHARACTER*15 ABSOLUTE_CANCEL_RULE,TABLE_TYPE*1,OVER*1,
...
!***********************************************************************
     ...
     +                                 ICAP_POINTER,ICAP)
!***********************************************************************
      REAL*4 CURRENT_TARGET_RATIO(3),PEAK,MIX_RATIO(3),RTEMP,ANN_CAP(3)
      INTEGER*2 I,M,N,YR,ITEMP,CAP_ORDER(3),ICAP_POINTER,ICAP
      LOGICAL*1 CAPACITY_TYPE_EXISTS
!
      DO I = 1, 3
...
!***********************************************************************
     ...
     +      CT_DENOM_LOAD_REDUCTION
      LOGICAL*1 NOT_DENOMINATOR_REDUCTION,R_NOT_DENOMINATOR_REDUCTION,
...
!     ADDL Expansion
     ...
     +               GET_RPS_NAME,
     +               RPS_CROSS_OVER
         INTEGER*2   RETURN_TOTAL_ALL_OPTIONS,
     ...
     +               RPS_NO,RPS_COUNT,PX
         PARAMETER (MAX_RESOURCES_PER_YEAR = MAX_CL_UNITS) ! 6000) 080306. UP FOR NAMER
         INTEGER*2   HIGHEST_OPTION_INDEX(MAX_RESOURCES_PER_YEAR),
     +               SORTED_OPTIONS(MAX_RESOURCES_PER_YEAR),
     +               RPS_RESOURCE_ADDED(MAX_RESOURCES_PER_YEAR),
     +               LAST_SORT_INSTANCE(MAX_RESOURCES_PER_YEAR),
     +               TEMP_INSTANCE(MAX_RESOURCES_PER_YEAR),
     +               HO,EU,
     +               NEXT_CN,
     +               LAST_HO,
     ...
     +               RPS_INDEX_TRANSLATE,
     +               PM
         REAL*4      HIGHEST_NET_MARGIN_ADDED(MAX_RESOURCES_PER_YEAR),
     +               GET_S_MIN_UP_TIME,
     +               GET_S_MIN_DOWN_TIME
         INTEGER SCREENING_REC
         SAVE SCREENING_REC
         REAL*4   ConeLinkCapacity,CONE_AnnualFixedCosts,
...
!     +            HIGHEST_ENERGY, ! CHANGED TO ALLOCATABLE 041209.
     ...
     +            CALC_GRX_FIX_COST_PER_UNIT
         CHARACTER*5 GET_SCENAME,CL_UNIQUE_RPT_STR
         CHARACTER*20   OPTION_NAME(:),
     +                  PA_NAME,
     +                  CX_NAME,
     +                  RPS_NAME*50
         CHARACTER*25   CURVE_NAME,TEMP_CURVE_NAME
         CHARACTER*35   MULTI_AREA_NAME(:),
     +                  GET_GROUP_NAME,
     +                  CM_NAME
         CHARACTER*22   LOCAL_NAME
...
! DETAILED REPORTING
         LOGICAL*1   MX_SUMMARY_REPORT,EXPANSION_REPORT,
     ...
     +               RPS_CURVE_REPORT,YES_RPS_CURVE_REPORT
         INTEGER*2   MX_REPORT_VARIABLES/53/,
     +               MX_ANNUAL_ALT_NO/0/,
     +               MX_ANNUAL_HEADER,
...
!
     ...
     +         LAST_EN,EN,FN
      REAL*4   GET_SCENARIO_COAL_PRICES,
     ...
     +         PA_CUM_MWH(30000)
      ...
      LOGICAL*1   USE_MAXIMUM_RM_LOGIC,
...
!
     ...
     +            ACTIVE_INSTANCE(MAX_RESOURCES_PER_YEAR),
     ...
     +            HIGHEST_ANN_LEVEL_CAP(MAX_RESOURCES_PER_YEAR),
     +            ANNUAL_FUEL_COST(TOTAL_ALL_OPTIONS),
     +            HIGHEST_FUEL_COST(MAX_RESOURCES_PER_YEAR),
!     +            FUEL_COST_PER_MWH(TOTAL_ALL_OPTIONS,13),
     ...
     +            REVENUE_PER_MWH(MAX_RESOURCES_PER_YEAR),
     +            CAPACITY_REVENUE_PER_MWH(MAX_RESOURCES_PER_YEAR),
     +            CAPACITY_REVENUE_MM(MAX_RESOURCES_PER_YEAR),
     +            RPS_REVENUE_MM(MAX_RESOURCES_PER_YEAR),
     +            MRX_RPS_MX_UNIT_CLEARED(MAX_RESOURCES_PER_YEAR),
     +            RPS_REVENUE_PER_MWH(MAX_RESOURCES_PER_YEAR),
     +            ENERGY_REVENUE_MM(MAX_RESOURCES_PER_YEAR),
     +            ENERGY_REVENUE_PER_MWH(TOTAL_ALL_OPTIONS),
     +            HIGHEST_ENERGY_REV_PER_MWH(MAX_RESOURCES_PER_YEAR),
     +            HIGHEST_STRIKE_PRICE(MAX_RESOURCES_PER_YEAR),
     +            HIGHEST_MRX_MARGIN(MAX_RESOURCES_PER_YEAR),
     +            HIGHEST_MRX_CF(MAX_RESOURCES_PER_YEAR),
     +            HIGHEST_MARKET_COST(MAX_RESOURCES_PER_YEAR),
     +            HIGHEST_MRX_ENERGY(MAX_RESOURCES_PER_YEAR),
     +            HIGHEST_RFT(MAX_RESOURCES_PER_YEAR),
     +            HIGHEST_RPM(MAX_RESOURCES_PER_YEAR),
     +            EMISSIONS_PER_MWH(TOTAL_ALL_OPTIONS),
     +            FOM_PER_MWH(TOTAL_ALL_OPTIONS),
     +            LEVELIZED_CAP_PER_MWH(TOTAL_ALL_OPTIONS),
     +            FIXED_COST_PER_MWH(MAX_RESOURCES_PER_YEAR),
     +            MRX_ICAP_INDEX(MAX_RESOURCES_PER_YEAR),
...
!     +            PA_RESERVE_MARGIN(TOTAL_ALL_OPTIONS),
     ...
     +            ICAP_DEMAND_CURVE_MULT(MAX_RESOURCES_PER_YEAR),
     +            HIGHEST_CF(TOTAL_ALL_OPTIONS),
     +            CONE_COST_PER_KWYR(MAX_RESOURCES_PER_YEAR),
     +            CONE_DEMAND_CURVE_ADJ_PER_KWYR(
     +                                          MAX_RESOURCES_PER_YEAR),
     +            EAS_PROFIT_PER_KWYR(MAX_RESOURCES_PER_YEAR),
     +            NET_MARGIN_PER_KWYR(TOTAL_ALL_OPTIONS),
!     +            NET_ENERGY_MARGIN_PER_KWYR(TOTAL_ALL_OPTIONS),
     +            ICAP_REVENUE_PER_KWYR(MAX_RESOURCES_PER_YEAR),
     +            HEAT_RATE(TOTAL_ALL_OPTIONS),
     +            HIGHEST_HEAT_RATE(MAX_RESOURCES_PER_YEAR),
     +            HIGHEST_ANNUAL_UNITS(MAX_RESOURCES_PER_YEAR),
     +            HIGHEST_MARKET_REVENUE(MAX_RESOURCES_PER_YEAR),
     +            HIGHEST_MARGIN_PER_KWYR(MAX_RESOURCES_PER_YEAR),
     +            HIGHEST_FUEL_COST_PER_MWH(MAX_RESOURCES_PER_YEAR),
     +            HIGHEST_VAR_OM_PER_MWH(MAX_RESOURCES_PER_YEAR),
     +            HIGHEST_FUEL_COST_PER_MMBTU(MAX_RESOURCES_PER_YEAR),
     +            HIGHEST_EMISSIONS_PER_MWH(MAX_RESOURCES_PER_YEAR),
     +            HIGHEST_FOM_PER_MWH(MAX_RESOURCES_PER_YEAR),
     +            HIGHEST_LEVEL_CAP_PER_MWH(MAX_RESOURCES_PER_YEAR),
     +            HIGHEST_VOM_COST_MM(MAX_RESOURCES_PER_YEAR),
     +            HIGHEST_EMISSIONS_COST_MM(MAX_RESOURCES_PER_YEAR),
     +            HIGHEST_FOM_COST_MM(MAX_RESOURCES_PER_YEAR),
     ...
     +            VARIABLE_COST_MM(MAX_RESOURCES_PER_YEAR),
     +            FIXED_COST_MM(MAX_RESOURCES_PER_YEAR),
     +            HIGHEST_SCREEN_CAPACITY(MAX_RESOURCES_PER_YEAR),
...
!
         HIGHEST_NET_MARGIN_ADDED = MINIMUM_NET_MARGIN - 0.001 ! 0.
         SORTED_OPTIONS = 0
         HIGHEST_OPTION_INDEX = 0
!
! SYSTEM CO2
!
         ...
         CO2_EMISSIONS = 0.0
         TG = 0
         CO2_EMISSIONS_CAP(TG) = GET_EMISS_CAP_FOR_CLASS(CO2,TG)
         CO2_EMISSIONS(TG) = GET_CL_EMISS_FOR_CLASS(CO2,TG)
...
!
     ...
     +                  (TransferValue(I,J),J=0,UPPER_TRANS_GROUP)
         ...
         MAX_RESOURCES_PER_YEAR_GROUP =
     +                          MAX_RESOURCES_PER_YEAR/UPPER_TRANS_GROUP
...
!         DO  AO = 1, TOTAL_ACTIVE_OPTIONS ! 070915. COMPRESSED LIST
         ...
         DO  AO = 1, TOTAL_ACTIVE_OPTIONS
            ...
            DO J = 1, LOCAL_ANNUAL_UNITS(CN)
               AI = AI + 1
               IF(AI > MAX_RESOURCES_PER_YEAR) EXIT
...
!         TEMP_L = RESET_MRX_RPS_TEST_REQ() ! 022019
         DOWHILE(CX_ITER < MAX_CX_ITER .AND. .NOT. OPTIMAL_ORDER)
            HIGHEST_NET_MARGIN_ADDED = -999999. ! MAY NEED TO CHANGE
...
! 032317
               IF(CX_ITER /= 0) THEN
                  IF(PLANNING_PER_MWH) THEN
                     HIGHEST_NET_MARGIN_ADDED(AI) = NET_MARGIN(CN)
                  ELSE
                     HIGHEST_NET_MARGIN_ADDED(AI) =
     +                                           NET_MARGIN_PER_KWYR(CN)
                  ENDIF
               ENDIF
!
...
!                     IF(J /= AI .AND. K == AO) THEN
                     IF(K == AO) THEN
                        IF(PLANNING_PER_MWH) THEN
                           HIGHEST_NET_MARGIN_ADDED(J) = NET_MARGIN(CN)
                        ELSE
                           HIGHEST_NET_MARGIN_ADDED(J) =
     +                                           NET_MARGIN_PER_KWYR(CN)
                        ENDIF
!                        HIGHEST_NET_MARGIN_ADDED(J) =
!     +                                      HIGHEST_NET_MARGIN_ADDED(AI)
...
!
 1234    CONTINUE
!         HIGHEST_OPTION_INDEX = TEMP_INSTANCE
...
!
     ...
     +                                L,LOCAL_CHRONO_MARKET_PRICES(1,L))
!               TEMP_L =
!     +                CHRONO_MARKET_PRICES(L,LOCAL_CHRONO_MARKET_PRICES)
            ENDIF
...
!
     ...
     +                          MAX_RESOURCES_PER_YEAR_GROUP
            ELSE
               MAX_RESOURCES_PER_GROUP = MAX_RESOURCES_PER_YEAR
            ENDIF
!
! MARKET AREAS LOOP.
!
...
!
                  HO = HO + 1
!
                  IF( HO > MAX_RESOURCES_PER_YEAR) THEN
                     WRITE(4,*) "In the MRX Resource Expansion"
                     WRITE(4,*) "Algorithm, more than ",
     +                           MAX_RESOURCES_PER_YEAR," resources"
                     WRITE(4,*) "were added to meet total system"
                     WRITE(4,*) "market conditions."
                     HO = MAX_RESOURCES_PER_YEAR

                  ENDIF
!
                  HIGHEST_OPTION_INDEX(HO) = HIGHEST_OPTION
                  HIGHEST_NET_MARGIN_ADDED(HO)  = HIGHEST_NET_MARGIN
...
!
         ...
         DO I = HO, 1, -1
            NEXT_CN = SORTED_OPTIONS(I)
            HIGHEST_OPTION = HIGHEST_OPTION_INDEX(NEXT_CN)
!
...
!
     ...
     +                                       RPS_PERCENT(HIGHEST_OPTION)
                     IF(SCREEN_CAPACITY(HIGHEST_OPTION) > 0.01) THEN
                        HIGHEST_NET_MARGIN_ADDED(NEXT_CN) =
     +                       1000.0 *
     +                          (ENERGY_REVENUE_MM(NEXT_CN) +
     +                             RPS_REVENUE_MM(NEXT_CN) +
     +                               CAPACITY_REVENUE_MM(NEXT_CN) -
...
! 042710. RECALCULATE HIGHEST_NET_MARGIN_ADDED BASED ON CURRENT ICAP
...
! 051810.
! NOTE: IOTA IMPACT IS REMOVED BELOW WHEN HIGHEST_NET_MARGIN_ADDED
!       IS RECALCULATED
!            IF( ABS(HIGHEST_NET_MARGIN_ADDED(NEXT_CN)) < 0.01) THEN
            IF( ABS(NET_MARGIN_PER_KWYR(HIGHEST_OPTION)) < 0.01) THEN
               HIGHEST_NET_MARGIN_ADDED(NEXT_CN) = 0.0
               HIGHEST_NET_MARGIN_ADDED(NEXT_CN) =
     +                NET_MARGIN_PER_KWYR(HIGHEST_OPTION) +
     +                                             CM_IOTA(CM)
...
! 041110. ADDED IOTA.
!                  IF(HIGHEST_NET_MARGIN_ADDED(SORTED_OPTIONS(I))+ 0.01<=
!
! !!! 060522. temporary for testing. !!!
!                  IF(HIGHEST_NET_MARGIN_ADDED(NEXT_CN)+ 0.01<=
...
!
!            ELSEIF(HIGHEST_NET_MARGIN_ADDED(SORTED_OPTIONS(I))+0.01 <=
!
! !!! 060522. temporary for testing. !!!
!            ELSEIF(HIGHEST_NET_MARGIN_ADDED(NEXT_CN)+0.01 <=
!     +                                          MINIMUM_NET_MARGIN) THEN
            ELSEIF(HIGHEST_NET_MARGIN_ADDED(NEXT_CN)+0.01 <=
     +                                     MINIMUM_NET_MARGIN .OR.
     +                                      RPS_ONLY_SWITCH_ACTIVE) THEN
...
! **********************************************************************
         ...
         INCLUDE 'SIZECOM.MON'
         INCLUDE 'GLOBECOM.MON'
         CHARACTER*1 NOX_SEASON_REGION,GET_TRANS_GROUP_NOX_SEASON
         LOGICAL*1 NOX_CONTROL_FOR_UNIT,
     +             CALCULATE_NOX,
...
! **********************************************************************
      INCLUDE 'SIZECOM.MON'
      INCLUDE 'GLOBECOM.MON'
      LOGICAL*1 R_RESOURCE_ADDITION
      INTEGER*2 R_HIGHEST_J,R_HIGHEST_INTERVAL,I,J,K,
     +            R_TRANS_GROUP_4_PRICING
      REAL*4   R_MARKET_DURATION(0:R_HIGHEST_INTERVAL),
...
! **********************************************************************
     ...
     +                       ADJUST_GRX_CO2_RETRO_PLAN_CAP
         ...
         INTEGER*2 MAX_SCREENING_INTERVALS,R_YEAR
         PARAMETER(MAX_SCREENING_INTERVALS = 79)
         LOGICAL*1   RESOURCE_AVAILABLE,
     +               DEPENDENT_UNIT_AVAILABLE,
     +               CALC_SCREEN_SLOPE_INTERCEPT,
     ...
     +               GET_SUM_USER_DAY_DATA,RETRO_FITTED_CAP
         INTEGER SCREENING_REC
         SAVE SCREENING_REC
         REAL*4   BEST_SLOPE(MAX_SCREENING_INTERVALS),
     +            BEST_INTERCEPT(MAX_SCREENING_INTERVALS),
     +            SCREEN_SLOPE(:),SCREEN_INTERCEPT(:),
     +            SCREEN_CAPACITY(:),
     +            EQUIVALENT_CAPACITY(:),
...
!     +            HIGHEST_ENERGY, ! CHANGED TO ALLOCATABLE 041209.
     ...
     +         MRX_ICAP_UNIT_LINK
! DETAILED REPORTING
         LOGICAL*1   GRX_SUMMARY_REPORT,EXPANSION_REPORT,
     +               GRX_REPORT_NOT_OPEN/.TRUE./,
     +               YES_INCLUDE_ICAP_REVENUE,INCLUDE_ICAP_REVENUE
         INTEGER*2   GRX_REPORT_VARIABLES,
     +               GRX_ANNUAL_ALT_NO/0/,
     +               GRX_ANNUAL_HEADER
...
!
         DO CN = 1, TOTAL_ALL_OPTIONS ! INDEX FROM THE CAPACITY OPTIONS FILE
!
! COULD CREATE A LIST BY FILE_SOURCE_INDEX TO AVOID CYCLE.
!
!            IF(RESOURCE_TYPE_INDEX(CN) < 2 .OR.
!     +                                RESOURCE_TYPE_INDEX(CN) > 3) CYCLE ! aug 09
...
!
     ...
     +                 (1. - RETURN_CL_SCREEN_EFOR(PROD_POINTER,R_YEAR))
            ...
            ENDIF
!
!            ANNUAL_FIXED_COST(CN) = R_INTERCEPT
            ANNUAL_LEVELIZED_CAPITAL(CN) =
!     +                       RETURN_SCREEN_CAP_COST(I,R_YEAR) *
     +                        RETURN_RISK_ADJ_S_CAP_COST(I,R_YEAR) *
     +                                            SCENARIO_CAP_COST_MULT
!
            FIX_OM_COST(CN) = RETURN_CL_SCREEN_FIXED_COST(PROD_POINTER)
...
!
     ...
     +                        (GRX_ITERATIONS  > 2 .OR.
!     +         (CO2_EMISSIONS(TG) < CO2_EMISSIONS_CAP(TG) .OR.
     +                                     BALANCE_WITH_CO2_PRICE)) THEN
!            CURRENT_CO2_DISPATCH_COST =
!     +                              GRX_CO2_LEAST_SQUARED_PRICE_ADJ(0_2)
     ...
     +                         CO2_EMISSIONS(TG) - CO2_EMISSIONS_CAP(TG)
         IF(GRX_ITERATIONS <= 1) THEN
            GRX_CO2_EMISS_REDUCTION_REQ(TG) =
     +                                  CO2_EMISS_REDUCTION_REQUIRED(TG)
     ...
     +              "ITER",GRX_ITERATIONS,
     +              "CO2_EMISSIONS_CAP",
     +                      GRX_ALL_SECTOR_CO2_EMISSIONS_CAP(THIS_YEAR),
...
!
            ...
            HIGHEST_NET_MARGIN_PER_KWYR = -999999.
!
            HIGHEST_NET_MARGIN_ADDED = -999999.
!
            HIGHEST_OPTION = -1
!
            LOCAL_RESOURCES_AVAILABLE = .FALSE.
...
!
     ...
     +                NET_MARGIN(CN) + .01 >
     +                                 HIGHEST_NET_MARGIN_ADDED(L)) .OR.
     +             (.NOT. PLANNING_PER_MWH .AND.
     +                        NET_MARGIN_PER_KWYR(CN) + .01 >
     +                               HIGHEST_NET_MARGIN_ADDED(L)) ) THEN
!
                  IF(PLANNING_PER_MWH) THEN
                     HIGHEST_NET_MARGIN_ADDED(L) = NET_MARGIN(CN)
                  ELSE
                     HIGHEST_NET_MARGIN_ADDED(L) =
     +                                           NET_MARGIN_PER_KWYR(CN)
                  ENDIF
                  HIGHEST_OPTION = CN
                  HIGHEST_J = J
     ...
     +                                          TEMP_NET_MARGIN_PER_KWYR
                  ...
                  HIGHEST_OPTION_INDEX(L) = HIGHEST_OPTION
...
! MAXIMUM RESERVE MARGIN CONDITION
!
               HIGHEST_OPTION = HIGHEST_OPTION_INDEX(SORTED_OPTIONS(I))
!
               IF(.NOT. DEPENDENT_UNIT_AVAILABLE(HIGHEST_OPTION)) CYCLE
...
!
     ...
     +                                TRANSACTION_GROUP(HIGHEST_OPTION))
               PA = TG_2_PLANNING_AREA(TG)
! PROFITABILITY TEST
               IF(HIGHEST_NET_MARGIN_ADDED(HIGHEST_OPTION) + .01 >
     +                                          MINIMUM_NET_MARGIN) THEN
