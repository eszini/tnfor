...
!       362 CO2 PARAM ! 361 COAL CONTRACTS ! THERMAL RETROFIT = 360 ! RPS => 352 cores and machines => 351 LDG placeholder=> 350 U9 => 349, N5=>344, LNG 336-343 ! 072006 303  ! 050406 !93 OVL FILES gat 1/2/01 164 msg 4/16/01 168 7/16/01 MSG, 09/09/07 GAT
...
!       COAL_MODEL_POINTER_FILE      ! 372  Vector file for coal price model
!       GQB GAS THERMAL 353
!       COAL NODES 354
!       COAL LINKS 355
!       COAL SUPPLY 356
...
!       169 10/10/01 174 11/1/01 175 11/26/01, 185 TMS 04/15/02, 185 GAT
!       4/26/02, 187 tms 5/13/02 188 12/23/02 189 MSG 1/14/03 190-193 MSG MAY '03
!       194-196 06/12/03 197 07/17/03 200 10/23/03
!       201-204 ADDED FOR NEW ENERGY PRODUCTS FILES TO PUT THE NUMBER AT 5
!       205-210 ADDED FOR DB TO EXPAND THE NUMBER OF THERMAL UNIT FILES MSG 7/15/05
...
!       317-320 ADDED BY DR.G FOR ADDITIONAL VARIABLE UNIT PARAMETER FILES
...
! NOTE THAT FILE NUMBER 7 IS USED FOR EXPENSE AND REVENUE NAMES BUT IS NOT "OVERLAYABLE" 11/27/01 MSG
      LOGICAL*1 SYSTEM_BASED_FORECAST,RUN_FUTURE_ASSETS,
...
C
     ...
     +          OVERLAY_BOOK_IS_ACTIVE
      ...
      INTEGER*2 I,END_POINT,RETURN_CODE,ISERROR
...
!
         ...
         CALL COAL_TRANSPORTATION_LINKS_MAKEOVL(
...
!
! FORWARD_OPTION OVERLAY ! 8/17/00. TS, GAT.
C
!         IF(trim(FORWARD_OPTION_FILE) /= 'NONE' .AND.
!     +                                           OVERLAY_BOOK_IS_ACTIVE)
!     +                CALL FORWARD_OPTION_MAKEOVL(FORWARD_OPTION_FILE)
!
...
!
C
! REGIONAL OUTAGES OVERLAY ! 10/10/01. GAT.
! ACCUMULATED WITH PARAMETERS ON 06/12/03.
C
!         IF(REGIONAL_OUTAGES_EXISTS)
!     +              CALL REGIONAL_OUTAGES_MAKEOVL(REGIONAL_OUTAGES_FILE)
C
! REGIONAL PARAMETER OVERLAY ! 05/13/02. TMS.
C
!         IF(REGIONAL_PARAMETER_EXISTS)
!     +          CALL REGIONAL_PARAMETER_MAKEOVL(REGIONAL_PARAMETER_FILE)
C
...
C
     ...
     +                                                .NOT. WVPA()) THEN
!         ProSym_Data_FILE = OVLNAME
!         CALL ProSymInputData(.FALSE.,ProSym_Data_FILE)
      ELSEIF(.NOT. MATCH_FOUND) THEN
         IF(OVERLAY_BOOK_ACTIVE()) THEN
            WRITE(4,*) 'For endpoint ',END_POINT,' no overlay ',
