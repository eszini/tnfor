...
C FILE NAME = ENRGLIMT.FOR
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C                                                                      C
C                  LOAD MODIFICATION PROGRAM                           C
C               FOR ROR AND PEAKING HYDRO UNITS                        C
C                      AND STORAGE UNITS                               C
C               PART OF THE PICAM MODEL                                C
C                                                                      C
C         COPYRIGHT 1983 (C) M.S. GERBER & ASSOCIATES, INC.            C
C                      ALL RIGHTS RESERVED                             C
C                                                                      C
C                   THE MIDAS GOLD MODEL                               C
C          COPYRIGHT (C) 1991-94 M.S. GERBER & ASSOCIATES,INC.         C
C                      ALL RIGHTS RESERVED                             C
C                                                                      C
C                                                                      C
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C                                                                      C
C                                                                      C
C     THE PURPOSE OF THIS PROGRAM IS TO MODIFY HOURLY LOADS TO         C
C     ELIMINATE HYDRO AND STORAGE GENERATION FROM SYSTEM RESOURCES.    C
C                                                                      C
C     FOR ROR HYDRO EACH LOAD IS REDUCED BY THE MINIMUM OUTPUT         C
C        CAPACITY OF THESE UNITS                                       C
C     FOR PEAKING HYDRO THE HIGHEST LOADS ARE REDUCED BY THE           C
C        MAXIMUM OUTPUT MINUS THE MINIMUM OUTPUT OF THE HYDRO          C
C        UNITS.  THIS REDUCTION IS SUBJECT TO THE PEAKEN REDUCTION     C
C        CONSTRAINT.                                                   C
C     FOR STORAGE DEVICES THE PEAKEN AND CAPACITY FROM THE DEVICE      C
C        IS ADDED TO THE PEAKING HYDRO TO REDUCE PEAK LOADS.           C
C        THE PEAKEN AND CAPACITY INPUT TO THE DEVICE IS ADDED TO       C
C        THE MINIMUM HOURLY LOADS.                                     C
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C
...
C
      ...
      LOGICAL*1 ENRG_LIMIT_REPORT,UNUSED_ENRG,
     +          RUN_TRANSACT/.FALSE./,YES_RUN_TRANSACT
      INTEGER*2 YR,ISEAS,ITEMP,I,J,L,LPEAK,NEXT,LPUMP,ORG_LDCPTS,
     +          PERIOD_COUNTER
     ...
     +       DOUBLE_BASE_ENRG
C
C TRANSACT VARIABLES 5/27/98. GAT.
C
     ...
     +               SAVE_NUMBER_OF_TRANS_GROUPS/0/
      REAL*4         TRANS_ROR_CAPACITY(:),R_CAPACITY(0:*),
...
C 5/20/92 ADDED VARIABLES FOR EL_GROUP; 8/16/92 PUMP_GROUP OUT
...
C     TYPE DECLARATION FOR /LOADCR/
      real*8 LODDUR(1000),LPROB(1000) ! REAL before 20030422
      REAL ENRGCT
C     TYPE DECLARATION FOR /WORK/
...
C
     ...
     +          P_OR_G
      real*8 IPEAK_SALE_CAPACITY(MAX_EL_UNITS), ! REAL before 20030422
...
C
     ...
     +                                         IPEAK_SALE_ENERGY(LPSALE)
            ...
            IF(L == 0) L = 3
C
C ADDED 6/15/91 FOR MIDAS GOLD
C
     ...
     +                                   FLOAT(PROD_PERIODS)
C            RTEMP_VARCST = ABS(RTEMP_VARCST)
C            RTEMP_FIXED_COST = ABS(RTEMP_FIXED_COST)
...
C
C
C     Initialize the working arrays in reverse order.
C     MOVED ABOVE PEAK SALE. 6/5/97. GAT.
C
      ORG_LDCPTS = LDCPTS
      ITEMP  = LDCPTS + 1
C     ENRG1  = DEMAND
      ...
      ENDDO
C
C     Reduce the peak load and energy if peaking units are available
C
      IF(NPEAK > 0) THEN
!
         LPEAK = 1
...
!         DO LPEAK = 1, NPEAK
C
C 10/18/93. GAT. ADDITIONAL PEAK SHIFTING LOGIC
C
...
!
      ...
      ENDIF
C
C     Add to off-peak load and energy if these units are available
C
C     Flip the working arrays around again so that peak is now in
C     position 1 and base is in position LDCPTS.
C
!
! 082519.
!
...
C
     ...
     +                     WLPROB(1,CURRNT),LDCPTS,HOURS,
     +                                                 DOUBLE_BASE_ENRG)
C                  ENERGY_DIF = ENRG2 + UNUSED_ENRG - ENRG1
C                  RR_OFPEAK_ENRG = + RR_OFPEAK_ENRG + UNUSED_ENRG
                  IF(ENRG1 < ENRG2) THEN
                     ENERGY_DIF = SNGL(ENRG2 - ENRG1)
                  ELSE
...
!     +                  ( IPUMEN(LPUMP) - (ENRG2 - ENRG1) )
!                  ENDIF
C                  ENRG2 = ENRG2 + RR_OFPEAK_ENRG
               ELSE
                  ENERGY_DIF = 0.
               ENDIF
            ELSE
C               ENRG2 = ENRG1 + IPUMEN(LPUMP)
               ENERGY_DIF = IPUMEN(LPUMP)
...
C
     ...
     +                     ENRG_LIMITED_FIXED_COST(L) - RTEMP_FIXED_COST
               ...
               ENDIF
C
C NEXT PEAK AND VALLEY OFF SYSTEM SALES
C               
            ELSE
               SALES_ENERGY = SALES_ENERGY + DBLE(ENERGY_DIF)
               SALES_ENERGY_NOT_IN_FORECAST = DBLE(ENERGY_DIF) +
...
!      ENDIF
C
C     WARN IF UNITS WERE CONVERTED TO RR
C
     ...
     +                                   IDINT(DEMAND) - IDINT(ENRG1)
      ...
      DEMAND = ENRG1
C
C           CALL Integrate(ENRG1,LODDUR,LPROB,LDCPTS,HOURS,LODDUR(1))
C
      IF(ENRG_LIMITED_CAPACITY < SEASONAL_ENRG_LIMITED_CAPACITY) THEN
         ...
         ENDDO
      ENDIF
C      CALL ANNUAL_EL_REPORT  ! MOVED FOR DYNAMIC HYDRO.
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C                                                                      C
C                       SUBROUTINE HYDRIN                              C
C         COPYRIGHT (C) 1981 M.S. GERBER & ASSOCIATES, INC.            C
C                      ALL RIGHTS RESERVED                             C
C                                                                      C
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C                                                                      C
C     PURPOSE:                                                         C
C        THE PURPOSE OF THIS SUBROUTINE IS TO CALCULATE THE            C
C        INFORMATION NECESSARY FOR THE LOAD-CLIPPING PROGRAM           C
C        FROM A HYDRO FILE.                                            C
C                                                                      C
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
...
C
C
C CLINTON'S BTU TAX ADDED 3/14/93
C
...
C 5/20/92 ADDED FOR EL_GROUP; 8/16/92 PUMP_GROUP OUT
...
C     TYPE DECLARATION FOR /HYDRVL/
      ...
      real*8 PEAKCP(MAX_EL_UNITS),IPUMCP(MAX_EL_UNITS), ! REAL before 20030422
...
C
      ...
      real*8 IPEAK_SALE_CAPACITY(*),IPEAK_SALE_ENERGY(*) ! REAL before 20030422
...
C
C DYNAMIC PUMPED STORAGE VARIABLES
...
!            
     ...
     +                         SEASON_CAPACITY,EL_SO2_EMIS(J))
         ...
         ENDIF
C
C CLINTON'S ENERGY TAX ADDED 3/14/93 MSG
C
...
C
C         GROUP = 4
C         IF(HYDRO_EXPENSE_ASSIGNMENT(J) == 'T') GROUP = 5
C        STORAGE UNITS LIKE PUMPED STORAGE UNITS
C         IF(HYTYPE(J) == 'S' .AND. 
C    +                          HYDRO_EXPENSE_ASSIGNMENT(J) /= 'R') THEN
         IF(HYTYPE(J) == 'S') THEN
            NPEAK = NPEAK + 1
            PEAKCP(NPEAK) = MWB
     ...
     +                                                          PCOWN(J)
            ...
            ENDIF
C        UNCONTROLLED UNITS LIKE ROR UNITS WITH POSITIVE OR NEGATIVE ENERGY
...
C               RTEMP_FIXED_COST = ABS(RTEMP_FIXED_COST)
     ...
     +                                                  RTEMP_FIXED_COST
               ENDIF
            ENDIF
C        CONTROLLED UNITS LIKE PONDAGE HYDRO
...
C FOR DYNAMIC PUMPED STORAGE UNITS
            MWB = ABS(MWB)
            TEMP_ENRG = ABS(HYDRO_MONTHLY_ENRG(J))
     ...
     +                                  HYDRO_EXPENSE_ASSIGNMENT(J)
      ...
      ENDDO 
C
C  Now a sort for the peaking units; from least capacity factor
C  to greatest capacity factor.
C
      DO I = 1, NPEAK - 1
         M = I
         N = I + 1
...
C      
     ...
     +                                     AuxCoeff,AuxRHS)
C
C ASSUMES STORAGE UNITS OCCUPY THE FIRST AUX CONSTRAINTS.
C
...
C NEED TO ADD THIS TO THE RESULTS MODULE AND THE DETAILED REPORT      
C      
...
C***********************************************************************
C
c     SUBROUTINE PKSHIF(PKENRG4,PEAKCP4,WLPROB4,WLOADS4, ! caller's REAL*4 vars
      SUBROUTINE PKSHIF(PKENRG ,PEAKCP ,WLPROB ,WLOADS , ! caller's real*8 vars
     +   CURRNT,HOURS,LDCPTS,R_ISEAS,R_LOCAL_NAME)
C
C***********************************************************************
C
C   This subroutine reduces the peak loads by shifting back along the
C   WLOADS curve and attempting to subtract the PEAKCP.  You get two
C   curves; the original one and one which is identical but lower by
C   PEAKCP.  The original curve uses the I counter, while the lower one
C   uses the J counter.  The area described by the difference of the
C   two curves represents the energy reduction.  This program computes
C   a slope-intercept equation for a segment of both curves, and
C   solves a quadratic equation (as a result of integrating) to define
C   the area.  This process is repeated at each discontinuity of the
C   curve until the cumulative area exceeds exceeds the energy the unit
C   is capable of producing.  Then the left limit is defined so as to
C   yield the correct amount of energy (the right limit being the last
C   limit before the energy was exceeded).  At this point, the program
C   reassigns the WLOAD values to reflect the reduction of energy and
C   the change in shape.
C
c   20030410 AGT changed local REALs to REAL*8, FLOAT to dble,
c     AMIN1 to min, AMAX1 to max; and added copying loops at entry & exit
c   20030422 AGT changed caller's REALs to REAL*8
c
      INTEGER*2 I,ILAST,J,JLAST,ISTART,HOURS,CURRNT,LDCPTS,NEXT,R_ISEAS
      INTEGER*4 ILEFT
c     REAL*4 PKENRG4,PEAKCP4,WLPROB4(1000,2),WLOADS4(1000,2) ! caller's vars
...
c
c     copy values from caller's REAL*4 variables to local REAL*8
c     PKENRG=dble(PKENRG4)
c     PEAKCP=dble(PEAKCP4)
c     do J=1,2
c        do I=1,1000
c           WLPROB(I,J)=dble(WLPROB4(I,J))
c           WLOADS(I,J)=dble(WLOADS4(I,J))
c        end do
c     end do
c
      NEXT = 1 + MOD(CURRNT,2)
      ...
      B2 = 0.
C
C  The following section shifts along the curve and accumulates energy.
C  It tests to see if the full capacity can be used.  It also computes
C  a slope-intercept form for each line between discontinuities on
C  the curve.
C
...
C 3/29/94. GAT. ADDED FOR SRP. MODIFIED 7/12/94.
! 3/10/03. FOR XP.
...
!         IF(ABS(M1-M2) < .0000001) THEN
         ...
         ENDIF
C        WRITE (9,1000) AMCHEK,LXLIM,RXLIM,ENRGBL
C1000    FORMAT(' AMCHECK = ',F10.6,' LXLIM = ',F9.4,' RXLIM = ',
C    +          F9.4,' ENRGBL = ',F10.6)
         IF( .NOT. ((ENRGBL-AMCHEK) > 0.)) GOTO 80
            ...
            IF(temp_real == LXLIM ) THEN
               J = J + 1
            ENDIF
C            IF(ABS(WLOADS(I+1,CURRNT) -LXLIM) < .000001 ) I = I + 1
C            IF(ABS(WLOADS(J+1,CURRNT)-PEAKCP-LXLIM) < .000001) J = J + 1
            GOTO 30
C
C  At this point the energy has been exceeded.  Now the exact limits
C  of integration are found.
C
   80 CONTINUE
C     WRITE (9,1000) AMCHEK,LXLIM,RXLIM,ENRGBL
      IF(M1 == M2) THEN
...
!         IF(ABS(M1-M2) < .0000001) THEN
         ...
         ELSE
            AMCHEK = ((RXLIM+LXLIM)*(M1-M2)/2.+B1-B2)*DELX
         ENDIF
C         IF( ABS(ENRGBL-AMCHEK) > .01) THEN
C            I = 0
C            DELX = (RXLIM - LXLIM)*.5
C            DOWHILE(ABS(ENRGBL - AMCHEK) > .01 .AND. I < 10)
C               IF(ENRGBL-AMCHEK > 0.) THEN
C                  LXLIM = LXLIM - DELX
C               ELSE
C                  LXLIM = LXLIM + DELX
C               ENDIF
C               DELX = DELX * .5
C               I = I + 1
C               AMCHEK = ((RXLIM+LXLIM)*(M1-M2)/2.+B1-B2)*(RXLIM-LXLIM)
C            ENDDO
C         ENDIF
         ENRGBL = ENRGBL - AMCHEK
C        WRITE (9,1000) AMCHEK,LXLIM,RXLIM,ENRGBL
      ENDIF
C
C  This section assigns the new values to WLOADS.  In the first case
C  (110) the full capacity cannot be taken off without violating the
C  energy constraint.
C
      DO I = 1,LDCPTS
  ...
  110    WLOADS(1,NEXT) = LXLIM
         ...
         GOTO 210
C
C  In this case, the full peaking capacity was taken, and the matter
C  is complicated by having to follow the original curve's contour
C  at a lower level.  When the left boundary of energy reduction is
C  reached, decisions must be made about how to assign points.
C
  150    CONTINUE
         J = 1
         ...
         DOWHILE(RTEMP - LXLIM > 0.001)
            ...
            IF(J > LDCPTS) THEN
               CURRNT = NEXT
               RETURN
c              goto 220 ! RETURN after copying revised local vars to REAL*4
            ENDIF
            RTEMP = WLOADS(J,CURRNT)-PEAKCP
         ENDDO
C         GOTO 210
  170    IF (J == LDCPTS) FLAG = .TRUE.
         ILEFT = INT(LXLIM)
C         DIFF = LXLIM - dble(ILEFT) -.001
C         DIFFE = dble(ILEFT + 1) - LXLIM
C         IF (DIFF > DIFFE) DIFF = DIFFE -.001
...
c
c     copy values to caller's REAL*4 variables from local REAL*8
c 220 PKENRG4=sngl(PKENRG)
c     PEAKCP4=sngl(PEAKCP)
c     do J=1,2
c        do I=1,1000
c           WLPROB4(I,J)=sngl(WLPROB(I,J))
c           WLOADS4(I,J)=sngl(WLOADS(I,J))
c        end do
c     end do
      RETURN
      END
...
C
C   This subroutine increases the base loads by shifting forward along
C   the WLOADS curve and attempting to add the IPUMEN.  You get two
C   curves; the original one and one which is identical but higher by
C   IPUMCP.  The original curve uses the I counter, while the higher one
C   uses the J counter.  The area described by the difference of the
C   two curves represents the energy increase.  This program computes
C   a slope-intercept equation for a segment of both curves, and
C   solves a quadratic equation (as a result of integrating) to define
C   the area.  This process is repeated at each discontinuity of the
C   curve until the cumulative area exceeds the energy the unit needs
C   to produce.  Then the right limit is defined so as to
C   yield the correct amount of energy (the left limit being the last
C   limit before the energy was exceeded).  At this point, the program
C   reassigns the WLOAD values to reflect the increase in energy and
C   the change in shape.
C
      INTEGER*2 HOURS,CURRENT,LDCPTS,NEXT,I,J,COPY_START,INEXT,ISHIFT
      real*8 IPUMCP,IPUMEN,WLPROB(1000,2),WLOADS(1000,2), ! REAL before 20030422
     +   ENRG_BAL,ENRG_INC,OPB,A2,SQROOT,DELTA,B2_4AC,DELTA_X,
     +   BASE,XSHIFT,YSHIFT1,YSHIFT2,X1,X2,X3,Y1,Y2,INTPL8
      REAL DX
C      REAL*8 UNUSED_ENRG
...
C
      ...
      ENDIF
C
C  The following section shifts along the curve and cumulates energy
C  until the capacity or energy limit is reached.  It also computes
C  a slope-intercept form for each line between discontinuities on
C  the curve.
C
      I = 1
   ...
   30    CONTINUE
     ...
     +             DELTA/2.
      ...
      ELSE
C
C The energy limit was reached.  Find the intercept value and adjust
C load curve points.
C
         WLOADS(INEXT,NEXT) = WLOADS(I,CURRENT)
         WLPROB(INEXT,NEXT) = YSHIFT1
     ...
     +                                    WLOADS(I+1,CURRENT))
         ...
         IF(I>=LDCPTS-1) THEN
C
C           IF THERE ISN'T SUFFICIENT ENERGY LEFT IN THE TAIL,
C           THEN ASSUME THE TAIL STAYS THE SAME, TRUNCATE THE TAIL, 
C           CALCULATE UNUSED ENERGY. THE PROGRAM IS TERMINATED 
C           DOWN HERE SO THAT WE CAN GET THE INTERPOLATED VALUE
C           FOR THE LAST LDC POINT.
C
C            WLOADS(INEXT,NEXT) = WLOADS(I,CURRENT)
C            WLPROB(INEXT-1,NEXT) = WLOADS(LDCPTS,CURRENT)
            LDCPTS = INEXT-1
C            UNUSED_ENRG = (ENRG_BAL-ENRG_INC)*FLOAT(HOURS)
            UNUSED_ENRG = .TRUE.
            GOTO 50
         ENDIF
         LDCPTS = INEXT
...
C**********************************************************************
C
C     ROUTINE FOR CLASSIFYING POOLING COSTS TO THE MEMBERS
C
...
C
     ...
     +            FISCAL_SEASON/0/
      ...
      SAVE MONTH
      CHARACTER*20 MONTH_NAME,LOCAL_EL_NAME
C **********************************************************************      
      INTEGER*2 LAST_SEASON/0/,CURRENT_YEAR,CURRENT_YEAR_COMPARISON,
...
C
      ...
      INTEGER*2   R_ASSET_CLASS_NUM(*),R_ASSET_CLASS_VECTOR(*),
...
!     
      ...
      INTEGER*2 ASSET_CLASS_POINTER(:),I,J
...
C
C 5/20/92 ADDED GROUP CAPABILITY
C
C 8/16/92 TRAP FOR SALES/PUMP
C
      IF(EL_GRP /= -99) THEN
...
C
     ...
     +                 P_CLASS_ASSIGNED_FIXED_COST(1) - EL_FIXED_COST
      ENDIF
C
C     12/3/93. GAT. ADDED MONTHLY_EL_REPORT.
C
...
!         CAPACITY_FACTOR = 0.
!         IF(CAPACITY /= 0.) THEN
!            CAPACITY_FACTOR = 100.*ENERGY/(CAPACITY*HOURS)
!         ENDIF
!
!         IF(RUN_TRANSACT .AND. CAPACITY_FACTOR > 0.) THEN            
!            REVENUE_FACTOR = CAPACITY_FACTOR
!            CALL CF_2_ANNUAL_REVENUE(REVENUE_FACTOR,
!     +                             AVE_REVENUE,LOCAL_EL_NAME,SAVE_MONTH)
!            MARKET_REVENUE = AVE_REVENUE * ENERGY
!         ELSE
!            MARKET_REVENUE = 0.0
!         ENDIF
!     
!         IF(.NOT. TESTING_PLAN .AND. 
!     +            REPORT_THIS_EL_UNIT(UNIT_NO) .AND.
!     +                               MONTHLY_EL_REPORT_ACTIVE) THEN
!      .OR.
!     +                           TRANS_GROUP_REPORTING_ACTIVE(TG))) THEN
C        IF(MONTHLY_EL_REPORT_ACTIVE) THEN
...
!     +                         CAPACITY*SALES_MULT,
!     +                         ENERGY*SALES_MULT,
!     +                         VAR_COST*SALES_MULT/1000000.,
!     +                         EL_FIXED_COST*SALES_MULT/1000000.,
!     +                         EL_SO2_EMIS*SALES_MULT,
!     +                         CAPACITY_FACTOR,
!     +                         MARKET_REVENUE*SALES_MULT/1000000.
...
C         CALL CMOVE(ASSET_CLASS_NUM,R_ASSET_CLASS_NUM,
C     +                                              INT(2*HYDRO_UNITS))
         R_ASSET_CLASS_VECTOR(1:HYDRO_UNITS) =
     +                                 ASSET_CLASS_VECTOR(1:HYDRO_UNITS)
C         CALL CMOVE(ASSET_CLASS_VECTOR,R_ASSET_CLASS_VECTOR,
C     +                                              INT(2*HYDRO_UNITS))
      RETURN
C**********************************************************************
...
C
C MARKET REVENUE RATES FOR UNION STUDY 8/7/97
C TAKEN OUT. GAT. 9/9/99.                       
!         BASE_LOAD_RATE = EL_BASE_REVENUE_RATE()
!         PEAK_LOAD_RATE = EL_PEAK_REVENUE_RATE()
C
...
C
         ...
         CURRENT_YEAR_COMPARISON = (CURRENT_YEAR-1900)*100
!
C         INTRA_SALES_REVENUE = 0.
C         INTRA_PURCHASE_EXPENSES = 0.
C         INTRA_VARIABLE_EXPENSES = 0.
C         INTRA_FIXED_EXPENSES = 0.
         MAX_INTRA_CLASS = 0
         IF(HYDRO_UNITS <= 0 .OR. 
...
C
C            COLLECTION_GROUP = 
C     +                    INDEX('ABNX',(HYDRO_EXPENSE_COLLECTION(UNIT)))
...
c
     ...
     +                                 EL_MON_MDS_PURCHASES(I,J,R_ISEAS)
c
c ANNUAL TOTALS FOR ALL CLASSES
c
...
C
      ...
      real*8 IPEAK_SALE_ENERGY,IPEAK_SALE_CAPACITY, ! REAL before 20030422
...
C (1) NEED TO ADJUST FOR LF > 50% IE NEED TO TRAP FOR X1 > 1;  DONE. 4/7.
C (2) NEED TO MAKE CHECKS THAT THERE IS ENOUGH LDC LEFT;
C (3) NEED TO TRAP FOR THE PROB VALUE CLOSEST TO X1. DONE. 4/6.
...
C**********************************************************************
C
C     THIS ROUTINE ALTERS THE LOAD DURATION CURVE ACCORDING TO
C     A PRESPECIFIED TYPE OF PURCHASE. DEVELOPED FOR WKP. 8/13/97. GAT.
C
...
!            
     ...
     +                            LODDUR(LOAD_POINT) - INTERVAL_AREA
               ENDDO
            ENDIF
C            PURCHASE_ENERGY_OUT = PURCHASE_AREA
            CALL INTEG8(ENERGY_AFTER,LODDUR,LPROB,PTS,
     +                                       NINT(REAL_HOURS),LODDUR(1))
            PURCHASE_ENERGY_OUT = SNGL(ENERGY_BEFORE - ENERGY_AFTER)/
     +                                                        REAL_HOURS
...
C            INTERVAL_AREA = 0.
     ...
     +                                 PURCHASE_AREA,
     +                                 A,LEFT)
C            LPROB(LOAD_POINT) = LEFT
C            LODDUR(LOAD_POINT) = A
C            PURCHASE_ENERGY_OUT = PURCHASE_AREA
!
            DO LOAD_POINT = 1, PTS
               IF(LOAD_POINT < LAST_WHOLESALE_POINT-1) THEN
     ...
     +                        WHOLESALE_LODDUR(LAST_WHOLESALE_POINT) - A
         ENDIF
!
C         PURCHASE_CAPACITY_OUT = MAX(0.,LODDUR(PTS) -  A + 
C     +                                                INTERVAL_AREA)
...
!         
!            
C            ANN_CLASS_ENERGY = ANN_CLASS_ENERGY * ONE_THOUSAND ! TEMP FOR CHRIS.
!         
...
!         
C            WRITE(ASSET_PRODUCTION_UNIT,REC=ASSET_PRODUCTION_REC) 
C     +                         PRT_ENDPOINT(), 
C     +                         FLOAT(CURRENT_YEAR),
C     +                         CLASS_NAME(CLASS),
C     +                         RESOURCE_TYPE(RESOURCE),
C     +                         ANN_CLASS_CAPACITY,
C     +                         ANN_CLASS_ENERGY
...
C***********************************************************************
C
C     A SUBROUTINE TO INTEGRATE THE Load-Duration CURVE TO FIND THE ENERGY
C
C***********************************************************************
      subroutine Integrate(Energy,LoadVal,LoadPrb,nPts,Duratn,BaseLoad)
c     same as INTEG8 except here the array arguments are real*8
...
C***********************************************************************
C
C     A SUBROUTINE TO CALCULATE THE HOURLY IMPACT OF DIFFERENT HYDRO 
C                  PROGRAMS BY MULTI-AREA TRANSACTION GROUP
C              AFTER THEY HAVE BEEN OPERATED UPON BY HYDRO ROUTINES
C
C***********************************************************************
C
...
!     +       TRANS_LODDUR(TRANS_LOAD_POINTS),
     ...
     +       STORE_PUMP_BY_HOUR,
     +       STORE_GEN_BY_HOUR
c      SAVE TOTAL_DEMAND_AFTER_EL,TOTAL_I4_ENERGY,TOTAL_R8_ENERGY
c      SAVE TRANS_LOAD_AFTER_EL,MONTHLY_TRANS_LOAD_MWH,
c     +     MONTHLY_TRANS_PEAK,
c     +     MONTHLY_TRANS_BASE,
c     +     MONTHLY_TRANS_LOAD_HYDRO_MWH,
c     +     MONTHLY_TRANS_LOAD_HYDRO_MW
!
! END DATA DECLARATIONS
!
!
...
!      
      PEAK_AFTER_EL = LODDUR(R_LDCPTS_AFTER_EL) ! THIS IS A TEST.
!
C      DEMAND_AFTER_EL = GET_DEMAND_AFTER_EL(R_ISEAS,START_MONTH,END_MONTH)
C
!
!
