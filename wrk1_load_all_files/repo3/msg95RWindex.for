...
C
      ...
      CHARACTER*256 STUDY_DIRECTORY,GET_RESULTS_DIRECTORY,COMMAND_LINE,
     +               SOFTWARE_DIRECTORY
...
C         CALL RW_UPDATE_RUNTIME_MESSAGES(SCREEN_MESSAGES)
         RETURN
C***********************************************************************
...
C
      USE SERVICE_ROUTINES
      INTEGER*4 R_DATE,R_TIME,R_SIZE
      LOGICAL*4 EXISTS
      CHARACTER*(*) FILE_NAME
      INTEGER,DIMENSION(13) :: ST
      INTEGER IY
...
C      INCLUDE 'SPINLIB.MON'
      ...
      INTEGER :: IOS
      CHARACTER*(*) DESCRIPTION_FIELD
C
C      CHARACTER*120 COMMAND
C        COMMAND = "DEL "//'"'//trim(DESCRIPTION_FIELD)//'"'
C        CALL SYSTEM(COMMAND,DOSBOX=.TRUE.,SPAWN=.FALSE.)
     ...
     +                          FindNextFileA(carg(FileHandle),FileData)
      ...
      END
C***********************************************************************
      subroutine Sec2Date(ETSecSince1Jan1970,cYear,cMonth,cDay) ! ,cHour,cMinute,cSecond)
C***********************************************************************
C
      integer*4 SecPerDay/86400/ ! 86400=24*60*60
      integer*2 DaysInNLYMo(12)/31,28,31,30,31,30,31,31,30,31,30,31/
!     end of parameter declarations
      integer*4 ETSecSince1Jan1970
      integer*4 cYear,cMonth,cDay,cHour,cMinute,cSecond
!     end of argument declarations
      integer*4 DaysInPriorYear,DaysInPriorYearsSince1970(0:99)/100*0/,
     +  SecInDay
      integer*2 iYr,iMo,WholeDaysSince1Jan1970,JulianDay,
...
!       DaysInPriorYearsSince1970(1)=366 ! for 1971
        do iYr=1,99 ! assign values through calendar-year 2069
          DaysInPriorYear=365
          if(mod(iYr,4)==1) DaysInPriorYear=366 ! note that 2000 is leap-year
...
!     assign defaults valid for extreme cases
     ...
     +    exit
      ...
      JulianDay=WholeDaysSince1Jan1970-DaysInPriorYearsSince1970(iYr)+1 ! on [1,366]
...
C***********************************************************************
C
      INTEGER*2 LINE,COLUMN,LENGHT,WITHD
      RETURN
      END
...
C***********************************************************************
C
C     A SUBROUTINE TO REVERSE VIDEO THE MODULE BEING RUN
C
C***********************************************************************
