     ...
     +               CASH_STATEMENT_POSITION(R_ACCOUNT_CLASSIFICATION)
         ELSE
            IF(INDEX(R_ACCOUNT_CLASSIFICATION,'BTL') /= 0) THEN
               CASH_POS = Cash BTL Expenses ! BTL Expenses + LAST_INCOME_LINE ! lumps all BTL cash in one line
            ELSE
               CASH_POS =
     +                 CASH_STATEMENT_POSITION(R_ACCOUNT_CLASSIFICATION)
            ENDIF
...
C
     ...
     +               + EXPENSES_MONTHLY(:,YR,ASSET_CLASS,63,2,0)
               INC_MONTH_VARS(:,Monthly Exp File Operating Revenue Tax)=
     +                  INC_MONTH_VARS(:,
...
C
     ...
     +                   + CASH_REV_EXP_MONTHLY(:,YR,ASSET_CLASS,91)
               CASH_MONTH_VARS(:,
     +                      WVPA Cash Property Taxes in Cost of Power) =
     +                 CASH_MONTH_VARS(:,
     +                        WVPA Cash Property Taxes in Cost of Power)
     +                 + CASH_REV_EXP_MONTHLY(:,YR,ASSET_CLASS,91)
...
C
               YR = R_YR ! - 1
               DO MO = 0, 12
                  DO DATA_POS = Cash Base Rates,Cash Total Base Revenues ! 1, 29
     ...
     +                  CASH_REV_EXP_MONTHLY(MO,YR,ASSET_CLASS,DATA_POS)
                  IF(.NOT. USE_PRODUCTION_MODULE_EXPENSE) THEN
                     DO DATA_POS = Cash Fossil Fuel, Cash Fixed OandM   ! 41,44
                        MONTH_VARS(MO,DATA_POS)=MONTH_VARS(MO,DATA_POS)+
     +                  CASH_REV_EXP_MONTHLY(MO,YR,ASSET_CLASS,DATA_POS)
     ...
     +                                    + TF_MONTH_PURCHASED_POWER(MO)
                     ...
                     ENDDO
                  ENDIF
                  DO DATA_POS = Cash Other OandM, Cash Other             !45, 47
                     MONTH_VARS(MO,DATA_POS) = MONTH_VARS(MO,DATA_POS) +
     +                  CASH_REV_EXP_MONTHLY(MO,YR,ASSET_CLASS,DATA_POS)
                  ENDDO
                  DO DATA_POS = Cash Leased Nuclear Fuel,                ! 49,
     +                                      Cash Lease Interest Payments ! 69
                     IF(DATA_POS == Cash Post Retirement Payments)CYCLE
                     MONTH_VARS(MO,DATA_POS) = MONTH_VARS(MO,DATA_POS) +
     +                  CASH_REV_EXP_MONTHLY(MO,YR,ASSET_CLASS,DATA_POS)
                  ENDDO
C
                  MONTH_VARS(MO,Cash Unfunded Retirement Payments) =     ! 151
     +                  MONTH_VARS(MO,Cash Unfunded Retirement Payments)
     +                  + CASH_REV_EXP_MONTHLY(MO,YR,ASSET_CLASS,
     +                                    Cash Post Retirement Payments) !Monthly Pension Expense) ! Pension Expense+LAST_INCOME_LINE)
c                  MONTH_VARS(MO,Cash Storm Payments) =                   ! 152
c     +                  MONTH_VARS(MO,Cash Storm Payments)
c     +                  + CASH_REV_EXP_MONTHLY(MO,YR,ASSET_CLASS,
c     +                                              Cash Storm Payments)   !Monthly Storm Expense) ! Storm Expense+LAST_INCOME_LINE)
c                  MONTH_VARS(MO,Cash Vacation Payments) =                ! 153
c     +                  MONTH_VARS(MO,Cash Vacation Payments)
c     +                  + CASH_REV_EXP_MONTHLY(MO,YR,ASSET_CLASS,
c     +                                           Cash Vacation Payments) ! Monthly Vacation Pay) ! Vacation Pay+LAST_INCOME_LINE)
                  MONTH_VARS(MO,Cash Paid on Accounts Payable) =         ! 157
     +                   MONTH_VARS(MO,Cash Paid on Accounts Payable)
     +                   + CASH_MONTHLY(MO,YR,ASSET_CLASS,Cash Payments) ! 4
                  MONTH_VARS(MO,Cash Received Accounts Receivable) =     ! 158
     +                  MONTH_VARS(MO,Cash Received Accounts Receivable)
     +                  + CASH_MONTHLY(MO,YR,ASSET_CLASS,Cash Receipts) ! 3
                  MONTH_VARS(MO,Cash BTL Lease Interest Payments) =     ! 170
     +                  MONTH_VARS(MO,Cash BTL Lease Interest Payments)
     +                  + CASH_REV_EXP_MONTHLY(MO,YR,ASSET_CLASS,
     +                                 Cash BTL Lease Interest Payments)
                  MONTH_VARS(MO,Cash Gas Wholesale Revenues) =          ! 177
     +                  MONTH_VARS(MO,Cash Gas Wholesale Revenues)
     +                  + CASH_REV_EXP_MONTHLY(MO,YR,ASSET_CLASS,
     +                                           Gas Wholesale Revenues) ! 31
                  MONTH_VARS(MO,WVPA Cash Member Cost of Power) =        ! 198
     +                  MONTH_VARS(MO,WVPA Cash Member Cost of Power)
     +                  + CASH_REV_EXP_MONTHLY(MO,YR,ASSET_CLASS,
     +                                       Cash WVPA Member Purchases) ! 87
                  MONTH_VARS(MO,WVPA Cash NonMember Cost of Power) =     ! 199
     +                  MONTH_VARS(MO,WVPA Cash NonMember Cost of Power)
     +                  + CASH_REV_EXP_MONTHLY(MO,YR,ASSET_CLASS,
     +                                   Cash WVPA Non Member Purchases) ! 88
                  MONTH_VARS(MO,WVPA Cash Member Cost of Services) =     ! 205
     +                  MONTH_VARS(MO,WVPA Cash Member Cost of Services)
     +                  + CASH_REV_EXP_MONTHLY(MO,YR,ASSET_CLASS,
     +                                        Cash WVPA Member Services) ! 87
                  MONTH_VARS(MO,WVPA Cash NonMember Cost of Services) =  ! 206
     +               MONTH_VARS(MO,WVPA Cash NonMember Cost of Services)
     +                  + CASH_REV_EXP_MONTHLY(MO,YR,ASSET_CLASS,
...
C
     ...
     +                                      TF_MONTH_PURCHASED_CAPACITY)
        MONTH_VARS(0:12,
     +           Monthly Expense Reserve Margin Capacity Purchase-30)  = 
     +        MONTH_VARS(0:12,
     +              Monthly Expense Reserve Margin Capacity Purchase-30)
     +        + TF_MONTH_PURCHASED_CAPACITY(0:12)
C
c         IF(WVPA()) THEN  ! TEMP FIX ON PP 5/18/04
c               MONTH_VARS(:,Purchased Power) =
c     +                MONTH_VARS(:,Purchased Power)
c     +                + MONTH_VARS(:,WVPA Cost of Power for Non Members)
c         ENDIF
         IF(USE_BUDGET_VARIABLE_OM) THEN
...
C
     ...
     +           CASH_MONTH_VARS(MO,Total Cash Fuel Derivative Revenues)
     +           + CASH_MONTH_VARS(MO,Cash Physical Fuel Deriv Rev Var)
...
C
            CASH_MONTH_VARS(MO,Total Cash Fuel Derivative Expenses) =
     +           CASH_MONTH_VARS(MO,Total Cash Fuel Derivative Expenses)
...
C
     ...
     +                           CASH_REV_EXP_MONTHLY(MO,YR,ASSET_CLASS,
     +                                                Cash STD Interest) ! Monthly Vacation Pay) ! Vacation Pay+LAST_INCOME_LINE)
                  R_LTD_CASH_INTEREST_ADJ(MO) =
     +                           CASH_REV_EXP_MONTHLY(MO,YR,ASSET_CLASS,
     +                                                Cash LTD Interest) ! Monthly Vacation Pay) ! Vacation Pay+LAST_INCOME_LINE)
                  DO TAX_TYPE = 1, 2
...
C            ELSE
C               LAG_BY_MONTH(1) = 1. - MIN(1.,CASH_VALUES_VECTOR)
C               LAG_BY_MONTH(2) = 1. - LAG_BY_MONTH(1)
C            ENDIF
C
...
C
         MONTH_VARS(:,Cash Adjustment Clause) =
     +            MONTH_VARS(:,Cash Adjustment Clause)
     +            + CASH_LAG_VALUES(0:12,Cash Adjustment Clause)
     ...
     +            + CASH_LAG_VALUES(0:12,Cash Capacity Sales)
         MONTH_VARS(:,Cash Income Reserve Margin Capacity Sales) =
     +           MONTH_VARS(:,Cash Income Reserve Margin Capacity Sales)
...
C
     ...
     +                Cash Adjustment Clause:Cash PGA Adjustment),DIM=2)
     +           + CASH_LAG_VALUES(0:12,Cash ICAP Revenues)
     +           + CASH_LAG_VALUES(0:12,Cash Physical Revenue Variable)
     +           + CASH_LAG_VALUES(0:12,Cash Physical Revenue Fixed)
     +           + CASH_LAG_VALUES(0:12,Cash Financial Revenue Variable)
...
C
         MONTH_VARS(:,Cash Expense Reserve Margin Capacity Purchase) =
     +           MONTH_VARS(:,
     +                    Cash Expense Reserve Margin Capacity Purchase)
     +           + CASH_LAG_VALUES(0:12,
     +                    Cash Expense Reserve Margin Capacity Purchase)
...
C WVPA PIECES
C
C         MONTH_VARS(:,WVPA Cash Member Cost of Power) =
C     +            MONTH_VARS(:,WVPA Cash Member Cost of Power) 
C     +            + CASH_LAG_VALUES(0:12,WVPA Cash Member Cost of Power)
C         MONTH_VARS(:,WVPA Cash NonMember Cost of Power) =
C     +         MONTH_VARS(:,WVPA Cash NonMember Cost of Power) 
C     +         + CASH_LAG_VALUES(0:12,WVPA Cash NonMember Cost of Power)
C
...
C     +         + CASH_LAG_VALUES(0:12,WVPA Cash Member Cost of Power)
     ...
     +                + BOOKED_VALUES(:,
     +                    Cash Expense Reserve Margin Capacity Purchase)
C     +           + BOOKED_VALUES(0:12,WVPA Cash NonMember Cost of Power)
C     +           + BOOKED_VALUES(0:12,WVPA Cash Member Cost of Power)
C
...
C            PARENT_CLASS_LOC = PARENT_CLASS_LOC - 1
     ...
     +                    PARENT_CLASS_LOC) = ACTUAL_CASH_VALUES(1:12,3)
            ANNUAL_CASH_ROLL_OVER(1:12,WVPA Cash Member Cost of Power,
     +                    PARENT_CLASS_LOC) = ACTUAL_CASH_VALUES(1:12,4)
            ANNUAL_CASH_ROLL_OVER(1:12,
     +          WVPA Cash From NonMember Power Sales,PARENT_CLASS_LOC) =
