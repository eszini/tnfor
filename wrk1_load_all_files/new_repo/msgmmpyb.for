...
c     INTEGER*2 NUMBER_OF_BC_PAYABLES_CLASSES/0/,
c    +          MAX_BC_PAYABLES_CLASS_ID_NUM/0/
c     INTEGER*2 NUMBER_OF_OL_PAYABLES_CLASSES/0/,
c    +          MAX_OL_PAYABLES_CLASS_ID_NUM/0/
      INTEGER*2 UNIT_NUM/10/,ASSET_CLASS_ID,MO
c     INTEGER*2 R_NUM_OF_PAYABLES_CLASSES,R_MAX_PAYABLES_CLASS_NUM,
c    +          R_PAYABLES_CLASS_POINTERS(*)
      INTEGER*2 R_UNIT_NUM,YR,I
      CHARACTER*30 PAYABLES_CLASSIFICATION
      CHARACTER*5 BASE_FILE_NAME,OVERLAY_FAMILY_NAME,
     +            MONTHLY_PAYABLES_FILE
      CHARACTER*30 COMMENT*50,DESCRIPTION,TEMP_DESCRIPTION
...
C DECLARATION FOR /PAYABLES FILE/
      INTEGER*2 DELETE
      REAL*4 VALUES(12,5),VALUES_24(12,2),VALUES_36(12,3)
      EQUIVALENCE (VALUES(1,1),VALUES_24(1,1)),
     +            (VALUES(1,3),VALUES_36(1,1))
      CHARACTER*16 FILE_TYPE/'Monthly Payables'/
      CHARACTER*2 PAYABLES_OL/'BC'/,R_PAYABLES_OL,CLASS_TYPE
      CHARACTER*1 DATA_TYPE(5),ACCOUNT_STATUS
      LOGICAL*1 LAHEY_LF95
      CHARACTER*30 SCREEN_OUTPUT
      CHARACTER*4 LAST_MONTH(2)
c     INTEGER*2 PAYABLES_BC_ASSET_CLASS_POINTER(:),
c    +          PAYABLES_OL_ASSET_CLASS_POINTER(:),      
c    +          TEMP_ASSET_CLASS_POINTER(:)
c     ALLOCATABLE :: PAYABLES_BC_ASSET_CLASS_POINTER,
c    +               PAYABLES_OL_ASSET_CLASS_POINTER,
c    +               TEMP_ASSET_CLASS_POINTER
c     SAVE PAYABLES_BC_ASSET_CLASS_POINTER,
c    +     PAYABLES_OL_ASSET_CLASS_POINTER
C
C***********************************************************************
C
...
C
     ...
     +                               VALUES_36
C
c         IF(DELETE < 8) CALL SET_ASSET_CLASSES(ASSET_CLASS_ID,
c    +                                NUMBER_OF_BC_PAYABLES_CLASSES,
c    +	                             MAX_BC_PAYABLES_CLASS_ID_NUM,
c    +                                TEMP_ASSET_CLASS_POINTER)
C
               DO YR = 1, 5
...
c         endfile(11)
         CLOSE(11)
c        IF(MAX_BC_PAYABLES_CLASS_ID_NUM > 0) THEN
c           ALLOCATE(PAYABLES_BC_ASSET_CLASS_POINTER
c    +                                   (MAX_BC_PAYABLES_CLASS_ID_NUM))
c           CALL CMOVE(TEMP_ASSET_CLASS_POINTER,
c    +                   PAYABLES_BC_ASSET_CLASS_POINTER,
c    +                             INT(2*MAX_BC_PAYABLES_CLASS_ID_NUM))
c        ENDIF
...
C
     ...
     +                               VALUES_36
            ENDIF
c           IF(DELETE < 8) CALL SET_ASSET_CLASSES(ASSET_CLASS_ID,
c    +                             NUMBER_OF_OL_PAYABLES_CLASSES,
c    +	                          MAX_OL_PAYABLES_CLASS_ID_NUM,
c    +                             TEMP_ASSET_CLASS_POINTER)
...
C***********************************************************************
      ...
      RETURN
C***********************************************************************
c     ENTRY RETURN_NUM_OF_PAYABLES_CLASSES(R_NUM_OF_PAYABLES_CLASSES,
c    +                                       R_MAX_PAYABLES_CLASS_NUM)
C***********************************************************************
c        IF(PAYABLES_OL == 'OL') THEN
c           R_NUM_OF_PAYABLES_CLASSES = NUMBER_OF_OL_PAYABLES_CLASSES
c           R_MAX_PAYABLES_CLASS_NUM = MAX_OL_PAYABLES_CLASS_ID_NUM
c        ELSE
c           R_NUM_OF_PAYABLES_CLASSES = NUMBER_OF_BC_PAYABLES_CLASSES
c           R_MAX_PAYABLES_CLASS_NUM = MAX_BC_PAYABLES_CLASS_ID_NUM
c        ENDIF
c     RETURN
C***********************************************************************
c     ENTRY RETURN_PAYABLES_CLASS_POINTER(R_PAYABLES_CLASS_POINTERS)
C***********************************************************************
c        IF(PAYABLES_OL == 'OL') THEN
c           CALL CMOVE(PAYABLES_OL_ASSET_CLASS_POINTER,
c    +                        R_PAYABLES_CLASS_POINTERS,
c    +                             INT(2*MAX_OL_PAYABLES_CLASS_ID_NUM))
c        ELSE
c           CALL CMOVE(PAYABLES_BC_ASSET_CLASS_POINTER,
c    +                        R_PAYABLES_CLASS_POINTERS,
c    +                             INT(2*MAX_BC_PAYABLES_CLASS_ID_NUM))
c        ENDIF
c     RETURN
...
C
      ...
      CHARACTER*30 PAYABLES_CLASSIFICATION,DESCRIPTION
      INTEGER*2 DELETE,MAX_CLASS_ID,IREC,
     +          R_NUM_ACTIVE_CLASS,
     ...
     +          NUM_ACTIVE_CLASS,ACT_MOS
      ...
      INTEGER*2 ASSET_CLASS_POINTER(0:MAX_CLASS_ID),ASSET_CLASS_LIST(30)
      REAL*4 VALUES(12,5)
      CHARACTER*1 DUMMY_TYPE,CLASS_TYPE*2,ACCOUNT_STATUS
      INTEGER*2 ASSET_CLASS,ASSET_CLASS_ID,CLASS_POINTER,MO,PAYMENT_TYPE
      REAL*4 PAYABLE_MONTHLY_VALUES(0:12,PAYMENT_VARS,-1:*), ! R_NUM_ACTIVE_CLASS)
     +       TEMP_PAYABLE_MONTHLY_VALUES(0:12,PAYMENT_VARS,-1:*)
      REAL*4 FILE_PAYABLE_VALUES(:,:,:,:),ACTUAL_PAYABLE_VALUES(:,:,:,:)
      ...
      INTEGER*2 R_YR,GET_MONTH_NUMBER
      REAL*4 ANNUAL_AMOUNT
      CHARACTER*1 DATA_TYPE(5)
      INTEGER*4 VALUES_2_SET
      LOGICAL*1 INCLUDE_IN_COSOLIDATE
      INTEGER*2 CLASS_POS,VARIABLE
      REAL*4 CASH_VALUES(0:12,1:*)
      INTEGER*2 R_CLASS_POS
      REAL*4 CONSOLIDATED_VALUES(0:12,80:*)
      CHARACTER*4 LAST_MONTH(2),LAST_MONTH_STR
      REAL (KIND=4), DIMENSION(0:12) :: R_OFF_SYSTEM_SALES_RECEIVABLE,
     +                                  R_OFF_SYSTEM_SALES_PAYABLE,
     +                                R_INTERNAL_POWER_COSTS_PAYABLE_PAY
      REAL (KIND=4), DIMENSION(12,6) :: R_ACTUAL_CASH_VALUES
      LOGICAL*1 WVPA_DATA
      INTEGER*2, PARAMETER :: Fuel=1, 
     +                        NonFuel=2,
     +                        Fixed Cost=3,
...
C
         IREC = 0
         CALL OPEN_PAYABLES_FILE(INT2(11))
C
         DO
            IREC = IREC + 1
...
C
            ...
            SELECT CASE(trim(PAYABLES_CLASSIFICATION))
               ...
               CASE ('Federal Income Taxes')
                  PAYMENT_TYPE = payment_federal_income_taxes_paid
               ...
               CASE('Non-Member Revenues Receivable')
                  PAYMENT_TYPE = wvpa_nonmember_revenues_receivable
                  WVPA_DATA = .TRUE. 
     ...
     +                                  YR,VARIABLE,CLASS_POS),1)) CYCLE
            ...
            SELECT CASE(VARIABLE)
               CASE (payment_std_interest)
                  PAYMENT_TYPE = Cash STD Interest
               CASE (payment_operating_revenue_tax)
                  PAYMENT_TYPE = Cash Operating Revenue Tax
               CASE (payment_property_taxes)
                  PAYMENT_TYPE = Cash Property Taxes
               CASE (payment_state_income_taxes_paid)
                  PAYMENT_TYPE = Cash State Income Taxes Paid
               CASE (payment_federal_income_taxes_paid)
                  PAYMENT_TYPE = Cash Federal Income Taxes Paid
               CASE (payment_state_tax_on_capital)
                  PAYMENT_TYPE = Cash State Tax on Capital
               CASE (payment_federal_tax_on_capital)
                  PAYMENT_TYPE = Cash Federal Tax on Capital
               CASE (payment_other_taxes)
                  PAYMENT_TYPE = Cash Other Taxes
               CASE(payment_revenues_receivable)
                  PAYMENT_TYPE = Cash Received Accounts Receivable ! 158
               CASE(payment_expenses_payable)
                  PAYMENT_TYPE = Cash Paid on Accounts Payable     ! 157
               CASE DEFAULT
                  PAYMENT_TYPE = -99
            END SELECT
            IF(PAYMENT_TYPE == -99) CYCLE
...
C
     ...
     +                                                   ASSET_CLASS)
         R_INTERNAL_POWER_COSTS_PAYABLE_PAY(1:12) =
     +         R_INTERNAL_POWER_COSTS_PAYABLE_PAY(1:12)
...
C
         ASSET_CLASS = PARENT_CLASS_ID_NUM + 1
            CASH_VALUES(1:12,WVPA Cash NonMember Cost of Power) =
     +           CASH_VALUES(1:12,WVPA Cash NonMember Cost of Power)
     +           + ACTUAL_PAYABLE_VALUES(1:12,R_YR,
     +                                     wvpa_nonmember_cost_of_power,
     +                                                      ASSET_CLASS)
            CASH_VALUES(1:12,WVPA Cash From NonMember Power Sales) =
     +           CASH_VALUES(1:12,WVPA Cash From NonMember Power Sales)
     +           + ACTUAL_PAYABLE_VALUES(1:12,R_YR,
     +                   wvpa_nonmember_revenues_receivable,
     +                                              ASSET_CLASS)
            CASH_VALUES(1:12,WVPA Cash Member Cost of Power) =
     +             CASH_VALUES(1:12,WVPA Cash Member Cost of Power)
     +             + ACTUAL_PAYABLE_VALUES(1:12,R_YR,
     +                wvpa_member_cost_of_power,
     +                                           ASSET_CLASS)
            ...
            CASH_VALUES(1:,Cash Fossil Fuel) =
     +           CASH_VALUES(1:,Cash Fossil Fuel)
     +           + ACTUAL_PAYABLE_VALUES(1:12,R_YR,wvpa_fuel_expense,
     +                                                      ASSET_CLASS)
            CASH_VALUES(1:,Cash Variable OandM) =
     +           CASH_VALUES(1:,Cash Variable OandM)
     +           + ACTUAL_PAYABLE_VALUES(1:12,R_YR,wvpa_nonfuel_expense, 
     +                                                      ASSET_CLASS)
            CASH_VALUES(1:,Cash Fixed OandM) =
     +           CASH_VALUES(1:,Cash Fixed OandM)
     +           + ACTUAL_PAYABLE_VALUES(1:12,R_YR,wvpa_fixed_expense,
     +                                                      ASSET_CLASS)
C
...
C
         DO VARIABLE = 1, PAYMENT_VARS
            ...
            SELECT CASE(VARIABLE)
               CASE (payment_std_interest)
                  PAYMENT_TYPE = Cash STD Interest    ! 82
c                 CYCLE
               CASE (payment_operating_revenue_tax)
                  PAYMENT_TYPE = Cash Operating Revenue Tax  ! 90
c                  CYCLE
               CASE (payment_property_taxes)
                  PAYMENT_TYPE = Cash Property Taxes ! 92
c                  CYCLE
               CASE (payment_state_income_taxes_paid)
                  PAYMENT_TYPE = Cash State Income Taxes Paid ! 93
               CASE (payment_federal_income_taxes_paid)
                  PAYMENT_TYPE = Cash Federal Income Taxes Paid ! 95
               CASE (payment_state_tax_on_capital)
                  PAYMENT_TYPE = Cash State Tax on Capital ! 94
                  CYCLE
               CASE (payment_federal_tax_on_capital)
                  PAYMENT_TYPE = Cash Federal Tax on Capital ! 96
                  CYCLE
               CASE (payment_other_taxes)
                  PAYMENT_TYPE = Cash Other Taxes ! 91
c                  CYCLE
               CASE(payment_revenues_receivable)
                  PAYMENT_TYPE = Cash Received Accounts Receivable ! 158
               CASE(payment_expenses_payable)
...
C***********************************************************************
C
      INTEGER*2 GET_ACCUMULATION_PERIOD_INFO,TYPE,MO
      LOGICAL*1 ACC_PERIOD(12),DONT_LAG_DECEMBER,EMPIRE,UI,BANGOR,WVPA,
     +          IPALCO,GreatRiver
      INCLUDE 'MTHNMCOM.MON'
C
         ...
         SELECT CASE(TYPE)
            ...
            CASE (payment_property_taxes)
               ...
               ELSE
                  ...
                  ACC_PERIOD(12) = .TRUE.
               ENDIF
c               IF(UI()) GET_ACCUMULATION_PERIOD_INFO = 1
            CASE (payment_federal_income_taxes_paid)
               ACC_PERIOD(3) = .TRUE.
               ACC_PERIOD(5) = .TRUE.
               ACC_PERIOD(8) = .TRUE.
