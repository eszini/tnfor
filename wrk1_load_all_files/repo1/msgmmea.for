...
C
      INCLUDE 'SpinLib.MON'
      INCLUDE 'SIZECOM.MON'
      INTEGER*2 IREC,INUNIT,LRECL/300/,I
     ...
     +          ASSET_CLASS_NUM,ASSET_CLASS_VECTOR
      INTEGER*2 R_NUM_OF_EXISTING_CLASSES,R_MAX_EXISTING_CLASS_NUM,
     +          R_EXISTING_CLASS_POINTERS(*),
     +          BOOK_DEP_VECTOR
      CHARACTER*5 BASE_FILE_NAME,OVERLAY_FAMILY_NAME,EXASTFIL
      CHARACTER*30 DESC,COMMENT,TEMP_DESC
      CHARACTER*256 FILE_NAME
      CHARACTER*256 BASE_FILE_DIRECTORY,OUTPUT_DIRECTORY
      CHARACTER*1 ACCOUNT_ACTIVE,DEP_METHOD
...
C DECLARATION FOR /EXISTING ASSETS FILE/
      INTEGER*2 SERVYR,DELETE
      INTEGER ACCTNO,WVPA_SUB_ACCOUNT_NUMBER,
     +        WVPA_ACCOUNTING_UNIT
      REAL*4 BOKGPV,BOKNPV,BKDPRT,TAXGPV,TXDEP1,TXRATE,DBRATE,
     +     RETIRE(AVAIL_DATA_YEARS),TXNORM,SALVAGE_VALUE,
     +     PROPERTY_TAX_VALUE,
     +     REGULATORY_ALLOCATOR
      CHARACTER*23 FILE_TYPE/'Existing-Asset Accounts'/
      CHARACTER*4 DEPMTH
      CHARACTER*2 EXASTOL/'BC'/,R_EXASTOL
      INTEGER*2 BC_EXISTING_ASSET_CLASS_POINTER(:),
     ...
     +     OL_EXISTING_ASSET_CLASS_POINTER
      ...
      REAL*4 ADR_LIFE,TOTAL_BOOK_DEP
      INTEGER*2 RETIRE_VECTOR,WVPA_COMPANY_ID
...
c      INCLUDE 'KEPCOCOM.MON'
      INCLUDE 'm3active.mon'
      CHARACTER*30 DESC
      INTEGER*2 I,IREC,TACCTS,DELETE,ACCOUNTS_REPORTED,
     +          ASSET_CLASS_NUM,ASSET_CLASS_VECTOR,
     +          RUN_YEARS,EXTENSION_YEARS,FINANCIAL_SIMULATION_YEARS
      REAL*4 RETIRE_30(AVAIL_DATA_YEARS),TXNORM
      REAL*4 EATAXDEP(*),ADD_EA_DEF_TAX(*)
c     REAL ADD_EA_TAX_DEP(*)
      REAL*4 TOTAL_EA_ACE_DEP(*)
      LOGICAL*1 RETPC,TAXES_NORMALIZED,EXISTING_ASSET_REPORT,
     +          EXISTING_ASSETS_REPORT,
     +          TOTAL_ASSET_REPORT,
     +          SAVE_BASE_CASE,
     +          REPORT_ALL_ACCOUNTS
C ADDED 4/13/92 MIDAS GOLD FOR REAL PROPERTY VALUES
      REAL*4 PROPERTY_ESCALATION(*),PROPERTY_TAX_VALUE
...
c     LOGICAL*1 MONTHLY_MIDAS_ACTIVE/.TRUE./
      ...
      INTEGER*2 MONTH_ENDING(LAST_AVAILABLE_MONTHLY_YEAR),YR,MO,
     +          BOOK_DEP_VECTOR
      REAL*4 SALVAGE_VALUE,
     +       ADR_LIFE,
     +       TOTAL_BOOK_DEP,
     +       CRET
      INTEGER*4 VALUES_2_ZERO
      INTEGER*2 WRITE_DRILLING_RPT,VOID_INT2
      CHARACTER*1 DRILLING_REPRT_LEVEL,FINANCIAL_DRILLING,ACCOUNT_ACTIVE
      CHARACTER*30 DRILLING_NAME
      LOGICAL*1 ANNUAL_INFO_ACTIVE,WVPA
      CHARACTER*34 OUTPUT_OPTION_NAME,LEFT_JUSTIFY_I2_IN_STR*15
      REAL*4 TREND_NORM
      REAL*4 CUM_RETIREMENTS
      REAL*4 TEMP_TREND_NORM
      INTEGER*2 TREND_YEAR,
     +          WVPA_COMPANY_ID
      LOGICAL*1 LF95,LAHEY_LF95
      CHARACTER*6 SHORT_MONTH_NAMES
      CHARACTER*5 PRIMARY_ACCOUNT_NAME
      CHARACTER*5 OUTPUT_DEPARTMENT_NAME
...
C
      ...
      IREC = 0
      CALL OPEN_EXISTING_ASSET_FILE(INT2(11))
      IF(LF95) THEN
...
C
     ...
     +              (trim(DEPMTH) == 'SL' .AND. TXNORM == TXRATE))
         IF(TAXES_NORMALIZED) THEN
            IF(TXNORM < 0.) THEN
               CALL GET_ASSET_VAR_TYPE(INT2(TXNORM),DATA_TYPE,
...
C
     ...
     +                                                   ASSET_CLASS_NUM
c               WRITE(EA_REPORTING_UNIT,REC=EA_REPORTING_REC)
c     +                                     PRT_ENDPOINT(),
c     +                                     FLOAT(BASE_YEAR),
c     +                                     OUTPUT_DEPARTMENT_NAME,
c     +                                     OUTPUT_OPTION_NAME,
c     +                                     PRIMARY_ACCOUNT_NAME,
c     +                                     SHORT_MONTH_NAMES(0),
c     +                                     GPV(0,1),
c     +                                     GPV(0,1)-NPV(0,1),
c     +                                     NPV(0,1)
c               EA_REPORTING_REC = EA_REPORTING_REC + 1
               DO I = 1, FINANCIAL_SIMULATION_YEARS
                  DO MO = 0, 12
...
C
      ...
      INCLUDE 'GLOBECOM.MON'
      INCLUDE 'm3active.mon'
      CHARACTER*1 DATA_TYPE,VECTOR_TYPE*20,DATA_TYPE2
      INTEGER*2 FINANCIAL_SIMULATION_YEARS,MO,YR,
     +          BOOK_DEP_VECTOR,BOOK_RATE_VECTOR
      LOGICAL*1 GROUP_BOOK_DEPRECIATION
      REAL*4 BOOK_DEP(0:12,0:FINANCIAL_SIMULATION_YEARS),
     +       GPV(0:12,0:FINANCIAL_SIMULATION_YEARS),
     +       CUM_BOOK_DEP(0:12,0:FINANCIAL_SIMULATION_YEARS),
     +       TOTAL_BOOK_DEP
      REAL*4 BKDPRT,ANNUAL_BOOK_DEP,
     +       NPVFB,DEPSL,SALVAGE_VALUE
      INTEGER*4 VALUES_2_SET
      LOGICAL*1 USE_BOOK_RATE,BOOK_DEP_FINISHED
      INTEGER*2 VECTOR
      ...
      REAL*4 MONTHLY_BOOK_DEP_VECTOR_ADDER(:,:)
      ALLOCATABLE :: MONTHLY_BOOK_DEP_VECTOR_ADDER
      REAL*4 VEC_DOLLAR_ADDER,VEC_RATE_ADDER
      INTEGER*2 TREND_YR
      REAL*4 ANNUAL_VECTOR_DEP
      INTEGER*2 I_YEAR,I_MONTH,REMAINING_MONTHS
C
...
C
      REAL*4 R_TTAXDP,R_TRETIE,R_TBOKDP,
     ...
     +     R_PROPERTY_TAX_NPV,
     +     R_EA_DEF_TAX_BASIS
      INTEGER*2 R_YR,R_CLASS,R_PERIOD,MO
...
C     INTEGER*2 EA_REPORTING_UNIT
C
      INTEGER*2 I,ASSET_CLASS_VECTOR,R_ASSET_CLASS_VECTOR
      INTEGER*2 ASSET_CLASS,CLASS,R_ASSET_CLASS,
     +          ASSET_CLASS_ID,DEACTIVE_YR,MO_DEACT
      REAL*4 BOKNPV,REGULATORY_ALLOCATOR,DEPCUM
      REAL*4 BOKGPV,TOTAL1,REGULATED_TAX_DEP
      REAL*4 RETIRE(0:12,*),BOKDP(0:12,*),GPV(0:12,*),NPV(0:12,*),
     +     CUM_BOOK_DEP(0:12,*)
      REAL*4 TAXDP(*),BOKTAX(*) ! These arrays are 0 based in the main routine
...
C                               having to subtract 1 form the array index 6/17/98
      REAL*4 ASSET_DEFERRED_TAX_BASIS(*),ASSET_DEFERRED_TAXES(*)
c     REAL TAXDP(0:*),BOKTAX(0:*)
c     REAL*4 ASSET_DEFERRED_TAX_BASIS(0:*),ASSET_DEFERRED_TAXES(0:*)
      REAL*4 R_TOTAL_DEF_TAX_BASIS_RATEBASE
      LOGICAL*1 TAXES_NORMALIZED,ASSET_REPORT
C
      INTEGER*2 NUM_OF_ASSET_CLASSES,MAX_ASSET_CLASS_NUM
      REAL*4 PROPERTY_TAX_VALUE,
     +     PROPERTY_ESCALATION(*),
     ...
     +       VECTOR_MONTHLY_DATA(12,LAST_AVAILABLE_MONTHLY_YEAR)
      CHARACTER*1 MONTHLY_DATA_UNITS(LAST_AVAILABLE_MONTHLY_YEAR)
      INTEGER*2 MONTH_ENDING(LAST_AVAILABLE_MONTHLY_YEAR),YR
      CHARACTER*1 DATA_TYPE,VECTOR_TYPE*20
      REAL*4 TREND_NORM
      INTEGER*2 ALLOCATION_VECTOR
      INTEGER*2 ALL_YR
      REAL*4 ANNUAL_ASSET_ALLOCATOR,
     +       ANNUAL_RETIREMENTS,
...
c                  EXIT
               ENDIF
               ALL_YR = MIN(I,AVAIL_DATA_YEARS+1)
...
c               DO MO = 0, 12
               MONTH_VARS(:,Monthly Book Depreciation) =
     +                          MONTH_VARS(:,Monthly Book Depreciation)
...
C MONTHLY ITEMS
c               DO MO = 0, 12
               MONTH_VARS(:,Monthly Gross Plant in Service) =
     +                                            TGPV(:,YR,ASSET_CLASS)
               MONTH_VARS(:,Monthly Retirements) =
     +                                          TRETIE(:,YR,ASSET_CLASS)
               MONTH_VARS(:,Monthly Accumulated Depreciation) =
...
C***********************************************************************
         CALL OPEN_EA_BASE_CASE_FILE(INT2(10))
...
C
      ...
      INCLUDE 'SIZECOM.MON'
C
      INTEGER*2 DEPPCT(15,4,3),J,L,M,BASE_YEAR,SERVYR,YRSINS,SUMYD,
     +          RLIFE,IYR,INYR,LIFE,LIFECK,IDEPCT,IVEC,I
      INTEGER*2 FINANCIAL_SIMULATION_YEARS
      REAL*4 VECTOR_VALUES(AVAIL_DATA_YEARS),VECTOR_VALUE
      REAL*4 NPVFT,TXDEP1,TXLIFE,TAXGPV,DBRATE,
     +     TAXDP(0:FINANCIAL_SIMULATION_YEARS),
     +     DEPSL,DEPDB,DEPBSE,DEPSYD,SL_OFFSET
      CHARACTER*4 DEPMTH,ACRS,ADR,SL,DB,SYD,DUMMY_TYPE*1
