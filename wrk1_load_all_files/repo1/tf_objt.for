...
!			MSG 6/5/2016 11:21:56 AM
!			MSG 12/20/2015 12:20:47 PM
!			MSG 2/21/2015 12:35:42 PM
!			MSG 2/21/2015 12:33:34 PM
C***********************************************************************
C
...
C     +                     YEAR, ! INT2
     ...
     +                     CALCULATION_MODE*1,CALCULATION_MODE_OUT*1, ! CHAR*1
...
! CONVERT GROWTH RATE TO VALUES, SAVE PRIOR YEAR 2/12/11 MSG
     ...
     +                                                    * LAST_YR_PEAK
                     CALCULATION_MODE_OUT = 'C'  ! C is treated the same as M
                  ENDIF
...
!                     TF_YEAR = TF_YEAR + 1 ! OUT. 11/25/98. GAT.
                  ENDIF

C                  IF(TF_YEAR /= YEAR - STUDY_BASE_YEAR) THEN
C                     WRITE(4,*) 'The Base TSYtem Forecast File in'
C                     WRITE(4,*) 'Record',TF_YEAR,'  and Year',YEAR
...
! CONVERT GROWTH RATE TO VALUES, SAVE PRIOR YEAR 2/12/11 MSG
     ...
     +                                                  * LAST_YR_ENERGY
                     MONTHLY_PEAK = (1.+MONTHLY_PEAK/100.)
     +                                                    * LAST_YR_PEAK
                     CALCULATION_MODE = 'C'  ! C is treated the same as M
...
C
C***********************************************************************
      ENTRY TF_GET_PEAKS ! this is not called by any routine MSG 4/18/01
C***********************************************************************
C
      OPEN(10,FILE=trim(OUTPUT_DIRECTORY())//TSYFRC_OL//
     +                           "TSYFC.BIN",ACCESS="DIRECT",RECL=LRECL)
      IREC = 0
      DO
         IREC = IREC + 1
...
C
  ...
  200 CALL MG_LOCATE_WRITE(20,0,trim(RECLN),ALL_VERSIONS,1)
      er_message='stop requested from tf_objt SIID272'
      call end_program(er_message)
C
C***********************************************************************
...
!     +            MAX_TRANS_GROUPS/600/, ! 10/15/04. TEMP.  BIG ARRAYS SHOULD NOT BE DIMENSIONED OFF THIS OR MAX_CUST (2048)
...
!
     ...
     +                     RTO_ISO_GROUP_NAME
      CHARACTER*6          PRIMARY_STATE
      INTEGER*2            CAPACITY_MARKET_POINTER(:),
     +                     CAP_MARKET_MONTH_NO(:),
...
C READ PRICES/COSTS FROM DATA FILE
     ...
     +                     DUMB_INT2, ! TRANSACTION_GROUP(TRANS), ! INT2
     ...
     +                     DUMB_REAL, ! ASSET_CLASS_REV_ALLOC_VECTOR(TRANS), ! REAL4
     +                     DUMB_CHR5, ! REFERENCE_LOAD_NAME(TRANS), ! CHAR*5
     +                     DUMB_INT2, ! REFERENCE_LOAD_NUMBER(TRANS), ! INT2
     +                     DUMB_REAL, ! ANNUAL_ENERGY(TRANS), ! REAL4
     +                     DUMB_REAL, ! ANNUAL_PEAK(TRANS), ! REAL4
     +                     DUMB_REAL, ! ANNUAL_CUSTOMERS(TRANS), ! REAL4
     +                     DUMB_REAL, ! ANNUAL_MULTIPLIER(TRANS), ! REAL4
     +                     (DUMB_REAL,I=1,12), ! (MONTHLY_ENERGY(I,TRANS),I=1,12), ! (12) REAL4
     +                     (DUMB_REAL,I=1,12), ! (MONTHLY_PEAK(I,TRANS),I=1,12), ! (12) REAL4
     +                     (DUMB_REAL,I=1,12), ! (MONTHLY_CUSTOMERS(I,TRANS),I=1,12), ! (12) REAL4
     +                     DUMB_REAL, ! DIST_ENERGY_LOSS_FACTOR(TRANS), ! REAL4
     +                     DUMB_REAL, ! TRANS_ENERGY_LOSS_FACTOR(TRANS), ! REAL4
     +                     DUMB_REAL, ! PEAK_LOSS_FACTOR(TRANS), ! REAL4
     +                     DUMB_REAL, ! PEAK_COIN_FACTOR(TRANS), ! REAL4
...
!
     ...
     +            CAPACITY_MARKET_POINTER(MAX_TRANS_LOAD_TABLES),
...
!     +                                             CALANDER_DAY_OF_WEEK)
     ...
     +                                             CALENDAR_DAY_OF_WEEK)
!         CALL DAYWEEK(INT2(1),INT2(1),LOCAL_DATA_YEAR,
!      +                                           CALENDAR_DAY_OF_WEEK)
!
...
!
     ...
     +                                              LAST_INCOME_LINE,3)) !msg thinks the order is 3=BUSBAR and 2=meter NOT 1=$,2=BUSBAR,3=METER
         ALLOCATE(MONTHLY_AC_CONTRACT_EXPENSE(
     +                      -1:MAX_ASSET_CLASS_GROUPS,0:12,
     +                                               LAST_EXPENSE_ITEM)) !msg thinks the order is 3=BUSBAR and 2=meter NOT 1=$,2=BUSBAR,3=METER
...
!
! move to line 3708 to group all allocates/deallocates 2/12/11 MSG 
!         IF(ALLOCATED(WH_TRANS_ALLOC)) DEALLOCATE (WH_TRANS_ALLOC)
!         ALLOCATE (WH_TRANS_ALLOC(0:MAX_TRANS_LOAD_TABLES,
!     +                                           MAX_TRANS_LOAD_GROUPS))
         WH_TRANS_ALLOC = 0.
...
!            
! 5/20/02. GAT. NEW CONDITION TO AVOID WARNING IN GET_TRANS_GROUP_POSITION           
!
                  IF(I == 0 .OR. 
...
!
! 10/20/03. NEED A SWITCH IN TF FILE BY TABLE
!
!
! 10/20/03. FOR DEVELOPMENT PURPOSES.  NEED TO ADD SWITCH IN TF FILE.
!
            USE_MONTHLY_NONLINEAR_LOADS = 
...
!     +                                 R_ISEAS,
!     +                                 R_YEAR,
!     +                                 TRANSACTION_GROUP(TRANS), ! THIS NEEDS TO BE INDEXED
!     +                                 LOAD_UNIT, ! TO CREATE WEEKLY PATTERN
!     +                                 WEEKLY_HYDRO_ENERGY,
!     +                                 WEEKLY_HYDRO_MINIMUM_MW,
!     +                                 WEEKLY_HYDRO_MAXIMUM_MW,
...
!
            IF(CAPACITY_MARKET_POINTER(TRANS) > 0) THEN
               PRIMARY_FUEL_ID = -1.*
     +              (10000. + FLOAT(CAPACITY_MARKET_POINTER(TRANS)))
               CALL GET_A_CAP_MARKET_PRICE(PRIMARY_FUEL_ID,
     +                         CAP_MARKET_RATE,
     +                         R_ISEAS,
     +                         R_YEAR)
!
...
! PURCHASE POWER - WORKS. NO NEED FOR MODIFICATION.
     ...
     +                                                Purchased Power) +
     +                                                   TEMP_R4 * 1000.
                  MONTHLY_AC_CONTRACT_EXPENSE(AC,0,Purchased Power) =
     +                  MONTHLY_AC_CONTRACT_EXPENSE(AC,0,
     +                                                Purchased Power) +
     +                                                   TEMP_R4 * 1000.
               ELSEIF(CAP_MARKET_TYPE_INDEX(TRANS) == 3) THEN
! CAPACITY SALE - ARBITRARY ASSIGNMENT. NOTE:NEGATIVE REVENUE
c replaced 11/12/07 by DrG
c                  RI = REV_CLASS_INDEX(TRANS,2) ! REVENUE FROM DEMAND PRICING
c                  MONTHLY_AC_CONTRACT_REVENUE(AC,2,R_ISEAS,RI,3) = 
c     +                  MONTHLY_AC_CONTRACT_REVENUE(AC,2,R_ISEAS,RI,3) -
c     +                                                   TEMP_R4 * 1000.
c                  MONTHLY_AC_CONTRACT_REVENUE(AC,2,0,RI,3) = 
c     +                        MONTHLY_AC_CONTRACT_REVENUE(AC,2,0,RI,3) -
c     +                                                   TEMP_R4 * 1000.
                  RI = Capacity Sales
...
!            HOURLY_CUST_MARGIN_VAR = TG_COUNTER(R_TG) + 1 ! INCLUDES WHOLESALE SALES
...
!            
!            X = LOAD_DISPATCH_POSITION(J,R_TG)
!
...
!                  ELSEIF(TEMP_GEN > R_UNSERVED_ENERGY .AND. 
!     +                                              TEMP_LOAD > 0.) THEN
                     LOCAL_AVERAGE_PRICE = 
...
!                     
! FIND THE TOTAL COST OF SUPPLY THE CAPACITY BETWEEN MIN AND MAX SPECIFIED.                     
!     +                     INTERRUPTIBLE_MAX_CAPACITY(TRANS),
!     +                     INTERRUPTIBLE_MIN_CAPACITY(TRANS),
!                     TEMP2_R = HOURLY_CUST_MARGIN_DB(1,K)
...
!     
!                  MONTHLY_INTERRUPTIBLE_REVENUE(,0) = 
!     +                   MONTHLY_INTERRUPTIBLE_REVENUE(K,0) +
!     +                      HOURLY_INTERRUPIBLE_REVENUE(K,HOUR_IN_DAY,2)
!                 
!            
...
!                     
     ...
     +                  (HOURLY_INTERRUPIBLE_REVENUE(K,I,4),I=1,24)
         ...
         ELSE
            ...
            HOURLY_INTERRUPIBLE_REVENUE = 0.  
         ENDIF
C SUM THE MONTHLY TO ANNUAL BY ASSET CLASS
!         MONTHLY_INTERRUPTIBLE_REVENUE(:,0) =
!     +                     SUM(MONTHLY_INTERRUPTIBLE_REVENUE(:,:),dim=1)
...
!               IPL_FAC_DB(13) = IPL_FAC_DB(5) * 
!     +                                      IPL_FAC_DB(11) * R_LAST_HOUR
! 010908.
...
!                  ANN_IPL_FAC_DB(13) = ANN_IPL_FAC_DB(5) * 
!     +                                      ANN_IPL_FAC_DB(11) * 8760.
...
!     +                         LAST_THIS_YR_PEAK(2,GLOBAL_PEAK_MONTH,TG)
               ELSE
...
!               IF(.NOT. IGNORE_NAT_PUR_USE_W) THEN
...
!
!                     TRANS_INDEXED_REVENUE(TRANS) = 
!     +                      TRANS_INDEXED_REVENUE(TRANS) + FINAL_REVENUE
!
! REASSIGNED 1/8/02
!
!                     RI = REV_CLASS_INDEX(TG,1)
     ...
     +                    MONTHLY_AC_CONTRACT_REVENUE(AC,4,0,RI,1) +
     +                                                     FINAL_REVENUE
!                    MONTHLY_INDEXED_ENERGY_REVENUE(AC,R_MONTH,1) = 
!     +                  MONTHLY_INDEXED_ENERGY_REVENUE(AC,R_MONTH,1) + 
!     +                                                     FINAL_REVENUE
!                     MONTHLY_INDEXED_ENERGY_REVENUE(AC,0,1) = 
!     +                  MONTHLY_INDEXED_ENERGY_REVENUE(AC,0,1) + 
!     +                                                     FINAL_REVENUE
!
! OUT PRIOR TO 1/8/02
!                     MONTHLY_INDEXED_ENERGY_REVENUE(AC,R_MONTH,2) = 
!     +                  MONTHLY_INDEXED_ENERGY_REVENUE(AC,R_MONTH,2) + 
!     +                                                         TEMP_LOAD
!                     MONTHLY_INDEXED_ENERGY_REVENUE(AC,0,2) = 
!     +                  MONTHLY_INDEXED_ENERGY_REVENUE(AC,0,2) + 
!     +                                                         TEMP_LOAD
!     
                  ENDDO ! TABLES
...
!     +                                    R_MONTH_COST - R_MONTH_REVENUE
!     
!         MONTHLY_AC_CONTRACT_REVENUE(R_AC,1,R_MONTH) = 
!     +                     MONTHLY_AC_CONTRACT_REVENUE(R_AC,1,R_MONTH) +
!     +                                                   R_MONTH_REVENUE
!     
!         MONTHLY_AC_CONTRACT_REVENUE(R_AC,1,0) = 
!     +                     MONTHLY_AC_CONTRACT_REVENUE(R_AC,1,0) +
!     +                                                   R_MONTH_REVENUE
!
!
...
c     +              + MONTHLY_AC_CONTRACT_REVENUE(ASSET_CLASS,2,0,20,2)
c     +              + MONTHLY_AC_CONTRACT_REVENUE(ASSET_CLASS,3,0,20,2))
...
C
     ...
     +                    MONTHLY_AC_CONTRACT_REVENUE(ASSET_CLASS,RI,MO,
     +                                                       Lighting,1)           
                     MONTH_VARS(MO,Bulk Power) =
     +                  MONTH_VARS(MO,Bulk Power) + .000001 *
     +                    MONTHLY_AC_CONTRACT_REVENUE(ASSET_CLASS,RI,MO,
     +                                                     Bulk Power,1)           
     ...
     +                  MONTH_VARS(MO,Secondary Sales) + .000001 *
     +                    MONTHLY_AC_CONTRACT_REVENUE(ASSET_CLASS,RI,MO,
     +                                                Secondary Sales,1)           
                     MONTH_VARS(MO,Capacity Sales) =
     +                  MONTH_VARS(MO,Capacity Sales) + .000001 *
     +                    MONTHLY_AC_CONTRACT_REVENUE(ASSET_CLASS,RI,MO,
     +                                                 Capacity Sales,1)            
                     MONTH_VARS(MO,Customer Revenues) =
     +                MONTH_VARS(MO,Customer Revenues) + .000001 *
...
C
               DO MO = 0, 12
C
                  MONTH_VARS(MO,Cash Relationship Revenues) =
     +                MONTH_VARS(MO,Cash Relationship Revenues)+.000001*
     +                     MONTHLY_INTERRUPTIBLE_REVENUE(ASSET_CLASS,MO)
                  DO RI = 1, 4 ! across the four revenue sources
     ...
     +                    MONTHLY_AC_CONTRACT_REVENUE(ASSET_CLASS,RI,MO,
     +                                                       Lighting,1)           
                     MONTH_VARS(MO,Cash Bulk Power) =
     +                  MONTH_VARS(MO,Cash Bulk Power) + .000001 *
     +                    MONTHLY_AC_CONTRACT_REVENUE(ASSET_CLASS,RI,MO,
     +                                                     Bulk Power,1)           
                     MONTH_VARS(MO,Cash Government) =
     +                  MONTH_VARS(MO,Cash Government) + .000001 *
     +                    MONTHLY_AC_CONTRACT_REVENUE(ASSET_CLASS,RI,MO,
     +                                                     Government,1)           
                     MONTH_VARS(MO,Cash Secondary Sales) =
     +                  MONTH_VARS(MO,Cash Secondary Sales) + .000001 *
     +                    MONTHLY_AC_CONTRACT_REVENUE(ASSET_CLASS,RI,MO,
     +                                                Secondary Sales,1)           
                     MONTH_VARS(MO,Cash Capacity Sales) =
     +                  MONTH_VARS(MO,Cash Capacity Sales) + .000001 *
     +                    MONTHLY_AC_CONTRACT_REVENUE(ASSET_CLASS,RI,MO,
     +                                                 Capacity Sales,1)            
                     MONTH_VARS(MO,Cash Relationship Revenues) =
     +                MONTH_VARS(MO,Cash Relationship Revenues)+.000001*
...
C***********************************************************************
     ...
     +                    ASSET_CLASS_HOURLY_LOAD(R_HOURS,R_AC,TG)      
      ...
      RETURN
C***********************************************************************
!      ENTRY MONTHLY_TRANSACTION_DURATION()  ! not used by any routine 11/06/06 Dr.G
C***********************************************************************
!
! THESE LOADS WILL BE USED FOR HYDRO OPERATION BY TRANSACTION GROUP
!
!         MONTHLY_TRANSACTION_DURATION = .FALSE.
!         CALL DOES_TF_FILE_EXIST(TF_FILE_EXISTS)
!         IF(.NOT. SAVE_TF_FILE_EXISTS) RETURN
!         MONTHLY_TRANSACTION_DURATION = .TRUE.
!      RETURN 
C***********************************************************************
...
!
!         IF(SCENARIO_NUMBER > 5) THEN ! WHY 5?
!
...
!         CLOSE(R_LOAD_UNIT)
         OPEN(R_LOAD_UNIT,FILE=FILE_NAME,ACCESS="DIRECT",
     +                                            STATUS="OLD",RECL=118)
!         OPEN(2101,FILE=FILE_NAME,ACCESS="DIRECT",STATUS="OLD")
!
!     THIS WOULD BE NICE, BUT IT USES SPECIFIC UNIT NUMBERS.
!     PERHAPS I CAN RESET THIS AT THE END OF A YEAR?
!
...
!            IF(SPINNING_UNITS(TRANS_GROUP) == 'P' .OR.
!     +                          SPINNING_UNITS(TRANS_GROUP) == 'C') THEN
!               SPINNING_RESERVE(TRANS_GROUP) = 
!     +                              SPINNING_RESERVE(TRANS_GROUP) / 100.
!            ENDIF
...
!
!                  PLANNING_AREA_2_TG(NUMBER_OF_PLANNING_GROUPS) =
!     +                                           NUMBER_OF_ACTIVE_GROUPS
...
!     +                   PLANNING_AREA_POSITION(
!     +                        REGIONAL_PLANNING_AREA(TRANS_GROUP))) = PA
!                     PLANNING_AREA_2_TG(PA) = NUMBER_OF_ACTIVE_GROUPS
                     EXIT
                  ENDDO
               ENDIF
!
...
!               TEMP_CAPACITY = GET_VAR(SPINNING_RESERVE(TRANS),
!     +                                  R_MONTH,"Monthly Spinning    ")
               TEMP_CAPACITY = ESCALATED_MONTHLY_VALUE(TEMP_CAPACITY,
...
C              TG = GET_TRANS_GROUP_INDEX(TRANS)
     ...
     +                             TEMP_CAPACITY/100.
            ELSE ! ASSUME UNITS = FALSE
               GET_TRANS_SPIN_FOR_TG = 0.
!               
!               WRITE(4,*) "UNEXPECTED SPINNING UNIT",
!     +                                             SPINNING_UNITS(TRANS)
!               WRITE(4,*) '*** line 5130 TF_OBJT.FOR ***'
...
!            
     ...
     +                             REPORT_THIS_GROUP(TRANS_GROUP) == 'T'
      ...
      RETURN
!C***********************************************************************
!      ENTRY GET_GROUP_NAME(R_TRANS_GROUP)
!C***********************************************************************
!         GET_GROUP_NAME = GROUP_NAME(R_TRANS_GROUP)
!      RETURN
!C***********************************************************************
!      ENTRY GET_TRANSACTION_GROUP_NAME(R_TRANS_GROUP)
!C***********************************************************************
!      CHARACTER*20 GET_TRANSACTION_GROUP_NAME
!         TRANS_GROUP = GET_TRANS_GROUP_POSITION(R_TRANS_GROUP)
!         GET_TRANSACTION_GROUP_NAME = GROUP_NAME(TRANS_GROUP)
!      RETURN
!C***********************************************************************
!      ENTRY GET_HOURLY_PRICE_NAME(R_TRANS_GROUP)
!C***********************************************************************
!         GET_HOURLY_PRICE_NAME = HOURLY_PRICE_NAME(R_TRANS_GROUP)
!      RETURN
C***********************************************************************
!      ENTRY GET_TRANS_GROUP_NOX_SEASON(R_TRANS_GROUP)
C***********************************************************************
!         IF(.NOT. SAVE_TG_FILE_EXISTS) THEN
!            GET_TRANS_GROUP_NOX_SEASON = 'F'
...
!               GET_TRANS_GROUP_NOX_SEASON = 'F'
!            ELSE
!               GET_TRANS_GROUP_NOX_SEASON = NOX_SEASON(TG)
!            ENDIF
!         ELSE
!            GET_TRANS_GROUP_NOX_SEASON = 'F'
!         ENDIF
...
C***********************************************************************
     ...
     +                                          R_GET_HOURLY_PRICE_NAME) ! LOGICAL
C***********************************************************************
         R_GET_HOURLY_PRICE_NAME = HOURLY_PRICE_NAME(R_TRANS_GROUP)
         GET_TF_HOURLY_PRICE_NAME = .TRUE.
      RETURN
C***********************************************************************
      ENTRY GET_TF_GROUP_NAME(R_TRANS_GROUP,R_GET_GROUP_NAME)  ! LOGICAL
C***********************************************************************
...
C***********************************************************************
C         TRANS_GROUP = GET_TRANS_GROUP_POSITION(R_TRANS_GROUP)
         IF(.NOT. SAVE_TG_FILE_EXISTS) THEN
...
C***********************************************************************
     ...
     +                          REPORT_THIS_GROUP(R_TRANS_GROUP) == 'T'
      RETURN
C***********************************************************************
!      ENTRY TRANS_GROUP_SCARCITY_VALUE(
!     +                                 R_TRANS_GROUP,
!     +                                 R_MONTH,
!     +                                 R_YEAR,
!     +                                 R_TRANS_CAP,
...
!         
     ...
     +                                         SCARCITY_CAP_PERCENT(10))
!
c         IF(.NOT. ALLOCATED(FIRST_CAPACITY_VALUE)) RETURN
c         SCARCITY_VALUES(1)  = FIRST_CAPACITY_VALUE(0)
c         SCARCITY_CAP_PERCENT(1) = FIRST_CAPACITY_PERCENT(0)
c         SCARCITY_VALUES(2)  = SECOND_CAPACITY_VALUE(0)
c         SCARCITY_CAP_PERCENT(2) = SECOND_CAPACITY_PERCENT(0)
c         SCARCITY_VALUES(3)  = THIRD_CAPACITY_VALUE(0)
c         SCARCITY_CAP_PERCENT(3) = THIRD_CAPACITY_PERCENT(0)
!         
         TOTAL_SCARCITY_CAPACITY = R_TRANS_MAX_CAP
         SCARCITY_CAP_PERCENT(1) = SCARCITY_CAP_PERCENT(1)/100.
         IF(R_TRANS_CAP <= SCARCITY_CAP_PERCENT(1) *
...
C
     ...
     +               PRICE_ESCALATION_RATE(1)**(R_YEAR-AVAIL_DATA_YEARS)
      END SUBROUTINE
C***********************************************************************
      SUBROUTINE MULTI_AREA_EXTENTION_PERIOD_PRICE_ESCALATION(R_YEAR,
     +                                               TRANS_GROUP,
     +                                               LOCAL_MARKET_PRICE)
         INCLUDE 'SpinLib.MON'
         USE TRANS_GROUP_VARIABLES
...
!
      ...
      INTEGER*2 R_TRANS_GROUP
      CHARACTER*1 SAVE_TF_CHAR_VARIABLES
      CHARACTER*1 GET_TRANS_GROUP_NOX_SEASON,
     +            R_GET_TRANS_GROUP_NOX_SEASON
      CHARACTER*3 GET_HOURLY_PRICE_NAME,R_GET_HOURLY_PRICE_NAME
      CHARACTER*35 GET_GROUP_NAME,R_GET_GROUP_NAME
      CHARACTER*35 GET_TRANSACTION_GROUP_NAME,
...
!
c      SAVE LPROB,DX,PEAK,LODDUR,DEMAND
c      SAVE TRANS_LBH_REC
c      SAVE        TRANS_LBH_UNIT
...
C      WRITE(8800,REC=ISEAS) DEMAND,DX,LODDUR(POINTS_IN_CURVE),BASE,
C     +                     LPROB,LODDUR,POINTS_IN_CURVE,AVE_RUNNING_RATE
C
!
      RETURN
C***********************************************************************
...
!
c      SAVE LPROB,DX,PEAK,LODDUR,DEMAND
c      SAVE HYDRO_LBH_REC
c      SAVE HYDRO_LBH_UNIT
!     END OF DATA DECLARATIONS
!
! NEED TO DO THIS IN CALLING ROUTINE.
!
      MAX_HYDRO_LOAD = 0.
...
!         
         IF(WRITE_HOURLY_REPORT) THEN
            ...
            IF(REMAIN < .001) THEN ! SUMMARIZE THE DAY
               DAY = HR/24
               START_HOUR = HR - 23
!               IF(GET_REPORT_HYDRO_GROUP(R_HYDRO)) THEN
!                  WRITE(HYDRO_LBH_UNIT,REC=HYDRO_LBH_REC) 
!     +                     PRT_ENDPOINT(),
!     +                     FLOAT(CURRENT_YEAR),
!     +                     CL_MONTH_NAME(R_ISEAS),
!     +                     FLOAT(DAY),
...
!     +              TRANS_LOAD_AFTER_EL(HR,R_TRANS_GROUP)*LOAD_ADJUSTER
...
C***********************************************************************
      ...
      RETURN
C***********************************************************************
      ENTRY DOES_SCENARIO_MAKER_FILE_EXIST(R_SCENARIO_MAKER_FILE_EXISTS)
C***********************************************************************
...
!
            NUM_SCEN_VAR = 63 ! 052712 ! 110911 ! 060408 for HG PRICE ! 072006 51 GAS MODEL. ! 12/23/05 48 ! 07/21/03. ! 11/25/02 26 ! 8/20/02. 
!
! 11/25/02.
!
