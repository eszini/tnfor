...
C
      ...
      CHARACTER*(*) R_BASE_FILE_DEFINITION,R_BASE_FILE_FAMILY,R_SCENAME
     ...
     +              COMMAND_LINE,FILE_NAME,GET_RESULTS_DIRECTORY
      CHARACTER*256 SCENAME,BASE_FILE_DEFINITION
...
C
C
        call startup_dsex(TEMP_BASE_FAMILY_NAME, COMMAND_LINE, SCENAME)
C
! SET COMMAND LOGIC SWITCHES
C
         MONTHLY_MIDAS_ACTIVE = (INDEX(COMMAND_LINE,'/ACTMONTHY') /= 0)
         DEBUG_ON_ACTIVE = (INDEX(COMMAND_LINE,'/DF') /= 0)
         MULTI_AREA_TRANC = (INDEX(COMMAND_LINE,'/MULTI_TRANC') /= 0)
         SIMSTATUS = (INDEX(COMMAND_LINE,'/SIMSTATUS') /= 0)
         R_DEBUG_ON = DEBUG_ON_ACTIVE
         NUCLEAR_FUEL_REPORT_ACTIVE = (INDEX(COMMAND_LINE,'/NF') /= 0)
         DONT_SUPPRESS_WARNING_WINDOW =(INDEX(COMMAND_LINE,'/BAT') == 0)
         USE_OVERLAY_BOOK = .TRUE. ! INDEX(COMMAND_LINE,'/OVLBOOK') /= 0
         MONTAUK_ACTIVE_FLAG = INDEX(COMMAND_LINE,'/MONTAUK') /= 0
         CREDITS_IN_INCOME_FLAG = INDEX(COMMAND_LINE,'/CREDIT') /= 0
         CL_ASSET_ANALYST_ACTIVE = INDEX(COMMAND_LINE,'/AACTIVE') /= 0
         QUICKSILVER_ACTIVE = INDEX(COMMAND_LINE,'/QUICKSILVER') /= 0
         NO_AUTO_INDEXING_ACTIVE = INDEX(COMMAND_LINE,'/NOINDX') /= 0
         COAL_MODEL_ONLY_ACTIVE=INDEX(COMMAND_LINE,'/COALMODELONLY') /=0
         IF(QUICKSILVER_ACTIVE .OR. RUN_SP_CAPEX)
     +                            DONT_SUPPRESS_WARNING_WINDOW = .FALSE.
!
         ITERATION_ACTIVE = .FALSE.
         IF(INDEX(COMMAND_LINE,'/ITER') /= 0) THEN
            POS = INDEX(COMMAND_LINE,'/ITER') + 5
            READ(COMMAND_LINE(POS:),*) ITERATION_NUMBER
            ITERATION_ACTIVE = .TRUE.
         ENDIF
C
         ENDING_ENDPOINT = 30000
         STARTING_ENDPOINT = 1
         IF(INDEX(COMMAND_LINE,'/EPS') /= 0) THEN
            POS = INDEX(COMMAND_LINE,'/EPS') + 4
            READ(COMMAND_LINE(POS:),*) STARTING_ENDPOINT
            STARTING_ENDPOINT = MAX(1,STARTING_ENDPOINT)
         ENDIF
         IF(INDEX(COMMAND_LINE,'/EPE') /= 0) THEN
            POS = INDEX(COMMAND_LINE,'/EPE') + 4
            READ(COMMAND_LINE(POS:),*) ENDING_ENDPOINT
            ENDING_ENDPOINT = MAX(1,ENDING_ENDPOINT,STARTING_ENDPOINT)
         ENDIF
         SET_POINTS_IN_LOAD_CURVE = INDEX(COMMAND_LINE,'/PS') /= 0
         IF(SET_POINTS_IN_LOAD_CURVE) THEN
            POS = INDEX(COMMAND_LINE,'/PS') + 3
            READ(COMMAND_LINE(POS:),*) POINTS_IN_LOAD_CURVE
         ELSE
            POINTS_IN_LOAD_CURVE = 79
         ENDIF
         SET_DELTA_X_IN_LOAD_CURVE = INDEX(COMMAND_LINE,'/DX') /= 0
         IF(SET_DELTA_X_IN_LOAD_CURVE) THEN
            POS = INDEX(COMMAND_LINE,'/DX') + 3
            READ(COMMAND_LINE(POS:),*) DELTA_X_FOR_LOAD_CURVE
         ELSE
            DELTA_X_FOR_LOAD_CURVE = -100.
         ENDIF
         VOID_LOGICAL = SET_DELTA_X_VALUE(DELTA_X_FOR_LOAD_CURVE)
C
! STRIP THE COMMAND LINE OF THE PROJECT NAME AND BASE FIMAILY
C
         POS = INDEX(COMMAND_LINE,'/')
         IF(POS-1 >= LENGHT+1) THEN
            COMMAND_LINE = COMMAND_LINE(LENGHT+1:POS-1)
         ELSE
            COMMAND_LINE = COMMAND_LINE(LENGHT+1:)
         ENDIF
         LENGHT = INDEX(COMMAND_LINE,'"')
         IF(LENGHT /= 0) THEN
            LENGHT = INDEX(COMMAND_LINE(LENGHT+1:),'"') + LENGHT
            COMMAND_LINE(LENGHT+1:LENGHT+1) = ','
         ELSE
            LENGHT = INDEX(COMMAND_LINE,' ')
            COMMAND_LINE(LENGHT:LENGHT) = ','
         ENDIF
C
! STRIP PROJECT/DSF FILE NAME
C
!         TEMP_BASE_FILE_DEF = COMMAND_LINE(1:LENGHT)
         COMMAND_LINE = trim(COMMAND_LINE)//",,,"
         TEMP_BASE_FILE_DEF = SCENAME
         TEMP_BASE_FAMILY_NAME = 'NONE'
         READ(COMMAND_LINE,*) TEMP_BASE_FILE_DEF,
     +                        TEMP_BASE_FAMILY_NAME
         BASE_FILE_DEFINITION = SCENAME
         IF(trim(TEMP_BASE_FILE_DEF) /= '')
     +                         BASE_FILE_DEFINITION = TEMP_BASE_FILE_DEF
C
! STRIP BASE FAMILY NAME
C

!         READ(COMMAND_LINE(LENGHT+1:),'(A)') TEMP_BASE_FAMILY_NAME
...
C
C***********************************************************************
      FUNCTION DSEXEC_OBJECT(POINTS_IN_LOAD_CURVE,SCENAME)
...
!      INTEGER*2 BASE_YEAR
      INTEGER*4 IOS
      INTEGER*2 DIREC_LEN,I,ACTIVE_VARIABLES,SET_SHORT_FORM_VARIABLES,
...
!     +          LAST_ELDC_REPORT,MARGINAL_COST_SWITCH
...
! GOLBAL VARIABLES
C
      INTEGER*2 STORED_BASE_YEAR,STORED_CURRENT_YEAR,END_POINT,
     +          STUDY_PERIOD,
...
C
     ...
     +                      CALL DELETE_RESULTS_OF_PREVIOUS_RUN(SCENAME)
C
...
C
     ...
     +          CLASS_BASED_FORECAST
      ...
      LOGICAL*1 SYSTEM_FORECAST_FILE_IN_USE/.TRUE./
      LOGICAL*1 ESC_DISP_ADDER,SHORT_FORM,
     +          USE_EXTERNAL_PRODUCTION_COST
      CHARACTER*1 R_FORECAST_TYPE_CHR,FORECAST_TYPE_CHR,
...
! end of planning switch information
     ...
     +          CHANGE_CPL_STATUS_TO_ACTIVE,TEMPL1
      ...
      LOGICAL*1 STORE_WKP_ACTIVE,WKP_ACTIVE
...
C
      use logging
      INCLUDE 'SPINLIB.MON'
      CHARACTER*(*) BASE_FILE_DIRECTORY,RESULTS_DIRECTORY,SCENAME
...
!     +              'Year                ','N',INT2(4),'V','D'
         WRITE(RPT_UNIT_NUM,REC=NEXT_REC+2)
     +              'Work Order          ','C',
     +                        INT2(Drilling Account Name Width),'V','D',
     +                        INT2(Drilling Account Name Width-4)
         R_NEXT_REC = NEXT_REC + 3
C
