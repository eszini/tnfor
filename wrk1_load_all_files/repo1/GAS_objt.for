C     Last change: msg 9/20/2016 4:05:35 PM
...
c     +      GAS_LOAD_GROUPS_INDEX,
c     +      CUST_CLASS_GROUPS_INDEX,
c     +      GAS_LOAD_2_TRANS_GROUPS,
c     +      GAS_LOAD_GROUP_2_TG,
...
C         CALL OPEN_GAS_HOURLY_LOAD_FILE( TEMP_LOAD_NAME,
C     +                                     TEMP_LOAD_NUMBER,
C     +                                     R_YEAR,LOAD_UNIT)
!
...
!            GAS_DEMAND_PLANNING_PEAK(I) = 0.0
         ENDDO
C
         iPort = 0
         SNamePos = 0
...
!               FIRST_TABLE_TG(GET_GAS_GROUP_POSITION(TG)) = TRANS
...
!
!               IF(.NOT. YES_ASSET_ANALYST_ONLY) THEN
!                  TG_2_PLANNING_AREA(MAX_GAS_LOAD_GROUPS) = 
!     +                      GET_PA_FROM_TG(GET_GAS_GROUP_POSITION(TG))
!
!               ENDIF
...
!            
         ...
         CLOSE(951)
c         DEALLOCATE(ASSET_CLASS_LIST,ASSET_ALLOCATION_LIST)
!
...
!
! 100312. REMOVED. 092912.
!
!                  TARGET_DAILY_STORAGE(LOCAL_MONTH,TG) = 
!     +                  AVE_DAILY_VOLUME(LOCAL_MONTH,TG) *
!     +                        MONTHLY_STORAGE_PATTERN(LOCAL_MONTH)
!                  TARGET_DAILY_STORAGE(LOCAL_MONTH,0) = 
!     +                  AVE_DAILY_VOLUME(LOCAL_MONTH,0) *
!     +                        MONTHLY_STORAGE_PATTERN(LOCAL_MONTH)
!                  TARGET_DAILY_STORAGE(0,TG) = 
!     +                  AVE_DAILY_VOLUME(0,TG) *
!     +                        MONTHLY_STORAGE_PATTERN(LOCAL_MONTH)
!                  TARGET_DAILY_STORAGE(0,0) = 
!     +                  AVE_DAILY_VOLUME(0,0) *
...
!         IF(ALLOCATED(MONTHLY_INDEXED_ENERGY_REVENUE))
!     +                   DEALLOCATE(MONTHLY_INDEXED_ENERGY_REVENUE)
...
!
     ...
     +                                  MAX_GAS_LOAD_GROUPS == 0) THEN
         ...
         ENDIF
!
!         ALLOCATE(REF_HOURLY_LOAD(R_HOURS,MAX_TRANS_LOAD_TABLES))
!
...
!
!         CALL INITIALIZE_TRANS_LOAD_PROB(MAX_GAS_LOAD_GROUPS)
!
...
!                  IF(GAS_LOAD_GROUP_2_TG(J) == 0) CYCLE
!                  I = FIRST_TABLE_TG(J)
                  I = GAS_LOAD_GROUP_2_TG(J)
!                  I = GAS_LOAD_GROUPS_INDEX(I)
...
!     +                                 R_ISEAS,
!     +                                 R_YEAR,
!     +                                 GAS_GROUP_ID(TRANS), ! THIS NEEDS TO BE INDEXED
!     +                                 LOAD_UNIT, ! TO CREATE WEEKLY PATTERN
!     +                                 WEEKLY_HYDRO_ENERGY,
!     +                                 WEEKLY_HYDRO_MINIMUM_MW,
!     +                                 WEEKLY_HYDRO_MAXIMUM_MW,
