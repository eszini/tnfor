...
C
     ...
     +                                     STARTING_MO)
            ...
            ENDIF
! 6/18/01. GAT. SET FUEL_PRICE_YEARS == YEARS_TO_RUN.  
!            IF(FUEL_PRICE_YEARS /= YEARS_TO_RUN) THEN
!               IF(FUEL_PRICE(YEARS_TO_RUN-1) > 0.) 
!     +               FUEL_PRICE(FUEL_PRICE_YEARS) = 
!     +               FUEL_PRICE(YEARS_TO_RUN)/FUEL_PRICE(YEARS_TO_RUN-1)
!            ENDIF
...
!         
      RETURN
C**********************************************************************
      ENTRY GET_A_CAP_MARKET_PRICE(       R_PRIMARY_FUEL_ID,
     +                                    R_TOTAL_DELIVERY_COST,
     +                                    START_MO,R_YR)
C**********************************************************************
         P_COST_1 = R_PRIMARY_FUEL_ID
!
     ...
     +                     FUEL_PRICE_POINTR(ABS(INT2(P_COST_1)))
            IF(THIS_FUEL_POINTER <= 0) THEN
               ...
               WRITE(4,*) 'for Capacity Limited unit '
               WRITE(4,*) '*** line 1011 FUELUSED.FOR ***'
               CALL SEE_WARNING_MESSAGES() ! STOP 'See WARNING MESSAGES'
            ENDIF
!
            P_COST_1 = NEW_FUEL_PRICE(THIS_FUEL_POINTER,START_MO,R_YR)
         ELSE
! JUST USE THE PRICE           
      ...
      RETURN
!
! 8/30/02. DOES NOT APPEAR TO BE AN ACTIVE OR NEEDED ROUTINE
!      
C**********************************************************************
