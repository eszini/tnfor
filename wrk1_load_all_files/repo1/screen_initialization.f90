...
subroutine init_screen(command_line)
CHARACTER (LEN=256):: WINDOW_NAME,COMMAND_LINE
 CHARACTER (LEN=25) :: BUILD_NUM
      ...
      INTEGER, PARAMETER :: idm_msg_file_exit=1, &
                            idm_mg_about = 20
      LOGICAL (KIND=1) :: SET_SP_CAPEX_ACTIVE_WITHOUT_DISPLAY, &
                          VOID_LOGICAL, &
                          SET_SP_CAPEX_ACTIVE_WITH_DISPLAY


      IF(INDEX(COMMAND_LINE,'/SPCAPEX') == 0 .OR. &
                           INDEX(COMMAND_LINE,'/SPCAPEXDISP') /= 0) THEN
         Arial_10_Bold = &
                      select_font(typeface='ARIAL',point=10,bold=.true.)
         IF(INDEX(COMMAND_LINE,'/SPCAPEX') /= 0) THEN
            WINDOW_NAME = "GED CapEx powered by MIDAS Gold"
            VOID_LOGICAL = SET_SP_CAPEX_ACTIVE_WITH_DISPLAY()
         ELSEIF(INDEX(COMMAND_LINE,'/ACTMONTHY') /= 0) THEN
            WINDOW_NAME = &
      ...
      ELSE
         VOID_LOGICAL = SET_SP_CAPEX_ACTIVE_WITHOUT_DISPLAY()
      endif
