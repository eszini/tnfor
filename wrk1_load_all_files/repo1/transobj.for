...
!
      ...
      INTEGER*2   MAX_TRANS_GROUPS,GET_MAX_TRANS_GROUPS,
     ...
     +            STEP_4_ADDER,
     +            R_TRANS
       INTEGER*4 TOTAL_TRANS_PER_MONTH,LOCAL_TIE_FLOW(24),
     +           TIE_FLOW_DATA_MAX(12),TIE_FLOW_DATA_MIN(12),
     +           TIE_FLOW_DATA_SUM(12)
C
C INTER-TIE INFORMATION
       REAL*4  TEMP_NATIVE_COST,
     +         DELIVERED_TIE_COST,
...
!     +         GET_DUKE_RESERVE_CAPACITY,
     ...
     +         GET_HOUR_PATH_MW
!
C LOAD INFORMATION TRANSACTIONS
      REAL*8 PERIOD_TRANSACTION_DEMAND/0. D0/,R_TRANSACTION_DEMAND,
     +         R_PUR_ENRG_FROM_TRANSACT,
     +         SUM_MARKET_PRICES,
     +         SUM_USER_PRICES,
     +         SUM_USER_W_PRICES(:),
...
C SYSTEM INFORMATION
      LOGICAL*1 VOID_LOGICAL,INIT_HOURLY_TIE,
...
!     +          EXISTING_STORAGE_N_HYBRID,
     ...
     +          TRANSACT_C
!
      LOGICAL*1 TEMP_L,READ_TRANSACT_TIE_DATA,
     ...
     +          SET_HOUR_LONG_PATH_PARAMS
      LOGICAL*4 NEW_TRANS_LOGIC/.FALSE./,
...
!
     ...
     +            PRODUCT_FUEL_PRICE(:,:),
     +            PRODUCT_HOURS(:),
     +            PRODUCT_DAILY_RETURN(:,:,:), ! NUM_PRODUCTS X HOURS IN THE MONTH
     +            PRODUCT_MEAN_RETURN(:,:),
     +            PRODUCT_SCARCITY(:,:),
     +            PRODUCT_LAST_PRICE(:,:),
     ...
     +            R_ANNUAL_PRICES(8760)
      LOGICAL*1 PRODUCT_FIRST_PERIOD(NUM_PRODUCTS)
      ALLOCATABLE ::
     +               PRODUCT_PRICE,
     +               PRODUCT_VOLATILITY,
     +               PRODUCT_QUANTITY,
     ...
     +               ANNUAL_PRODUCT_HOURS
!
      CHARACTER*1 TRANSACTION_PERIOD,TRANS_TIME_FRAME,
...
C LOCAL VARIABLES
      REAL LOAD_INCREMENT
      INTEGER*2 I,J,L,M,VALID_FILE_MARK,ISEAS,HR,TRANS_GROUPS_RECORDS,
     +          J_F,K_F,L_F,I_HR,J_HR
C      INTEGER*2 HOURS_IN(12)
C     +                 /744,672,744,720,744,720,744,744,720,744,720,744/
C      INTEGER*2 HOURS_IN_YEAR(12)/1,745,1417,2161,2881,3625,4345,5089,
C     +                                              5833,6553,7297,8017/
      INTEGER*2 HOURS_IN_MONTH,CUM_HOURS,R_HOURS_IN_MONTH,TEMP_HOURS,
     +            HOURS_FOR_TRANSACTIONS,
     ...
     +            ANNUAL_HOURS/8800/
      CHARACTER*256 LOAD_FILE_NAME,MARGINAL_COST_FILE_NAME
      CHARACTER*256 MARKET_RATES_DIR,MIDAS_OUTPUT_DIR,
     +               MIDAS_INPUT_DIR,TRANSACT_OUTPUT_DIR
      CHARACTER*5 LOAD_INPUT_NAME,MARGINAL_COST_INPUT_NAME,
     +            MARKET_PRICE_NAME
      CHARACTER*35 AREA_NAME_35
      CHARACTER*3 GET_HOURLY_PRICE_NAME,TEMP_STR
      LOGICAL*1   ZERO_SELL,ZERO_BUY,
     ...
     +      LAST_TRANS_MC(:)
      ...
      INTEGER*2 SELLER,BUYER,SELLER_DB,BUYER_DB
...
!     +     USER_MARKET_PRICE(24),
     ...
     +     TRANSFER_PROCOST_CHANGE
      REAL*4    TEST_HOURLY_REVENUE(:,:)
      ALLOCATABLE ::  TEST_HOURLY_REVENUE
      CHARACTER*8 EEICODE
      INTEGER*2 TIMZON,TEMPER,DELTMP,DAY_OF_WEEK(:,:),DAY,
     +          CURRENT_HR,DA,DAYS_IN_MONTH,LDE_YEAR,
     +          MARKET_DAY_OF_WEEK,CALENDAR_DAY_OF_WEEK,
     +          LDE_DAY,LDE_MONTH,DAY_WEEK
      INTEGER*2 GET_DAY_OF_WEEK_4,TEMP_I2,TEMP_I
!      
      INTEGER*4 DAILY_LOADS_I4(24),TEMP_I4
      INTEGER*2 DAILY_LOADS_I2(24)
      INTEGER*4 RECORD_LENGHT,FILE_LENGHT
! INT*2 FROM INT*4. 11/26/99. GAT.
      INTEGER*2 IREC,ALINE_LOAD_DATA,
     +            USER_IREC/0/,DAILY_IREC/0/
      LOGICAL*1 READ_I4_LOADS,RUN_MULTIAREA,YES_RUN_MULTIAREA_TRANSACT,
     ...
     +          PRICE_ANCHORED(:)
      ...
      CHARACTER*35 MULTI_AREA_NAME(:)
      INTEGER*2   TVA,CINERGY,START_HOUR,RPT_HR,
     +            END_HOUR,
...
!
      CHARACTER*3 TEMP_TWO_DIGIT_NAME
      CHARACTER*3 R_MULTI_AREA_NAME,TEMP_MULTI_AREA_NAME
      CHARACTER*5 SAVE_MARKET_PRICE_NAME(256),SAVE_TIE_FLOW_NAME(256)
      LOGICAL*1 REPORT_AREA_ACTIVE(:),GET_REPORT_TRANS_GROUP
!
      REAL TIE_FLOW(:,:),
C     +     M_TIE_FLOW(:,:),
     +     MARGINAL_COST_DELTA(:,:),
C     +     M_MARGINAL_COST_DELTA(:,:),
     +     MAX_TIE_FLOW,
     +     MIN_TIE_FLOW,
     +     HOURLY_LOAD_B4_SALES(:,:),
     +     DAILY_MARKET_PRICE(:),
C     +     PRODUCTION_COST_B4_SALES(:),
C     +     PRODUCTION_COST_AFTER_SALES(:),
...
!     
     ...
     +     REMAIN
      INTEGER*2 MAX_TRANS_WITHIN_HOUR,MAX_TRANS_ITERATIONS
!      PARAMETER (MAX_TRANS_WITHIN_HOUR=100) ! 11/2/99.
      INTEGER*2 NUM_ARGS,SPREADS_2_CHECK/0/
      CHARACTER*256  OUTPUT_DIRECTORY,
     +               BASE_FILE_DIRECTORY,
     +               LDE_FILE_DIRECTORY,
     +               PRB_FILE_DIRECTORY
      INTEGER*2 SPREADS,NUM_OF_SPREADS,R_SPREADS/0/
      PARAMETER(NUM_OF_SPREADS=9)
      REAL SPREAD_PRICES(NUM_OF_SPREADS)/0.,1.,2.,2.5,3.,4.,5.,6.,7./
...
C MARKET PRICES
C
      REAL*4 MONTHLY_RR(800)
      REAL*4 MONTHLY_MARKET_PRICES(:),SYSTEM_OUTPUT(:)
      ALLOCATABLE :: MONTHLY_MARKET_PRICES,SYSTEM_OUTPUT
      CHARACTER*4 FILE_SUFFIX
      CHARACTER*256 FILE_NAME
      INTEGER*2 R_YEAR
      INTEGER IOS
      REAL*4 PURCHASE_ENERGY(12),PURCHASE_COSTS(12),
     ...
     +       GET_MARKET_PRICE_SCALAR,MARKET_PRICE_SCALAR,GET_VAR
      REAL*4 MARKET_PRICE, ! GET_PRODUCTION_LEVEL, unused
     +       HOURLY_MARKET_PRICE,
     +       TEMP_MARKET_PRICE,
...
!
     ...
     +            MRX_ACTIVE/.FALSE./
      INTEGER*2   LAST_SEASON/12/,PRODUCTION_PERIODS,
...
!
      INTEGER*4   SEARCHES_WITHIN_TRANSACTIONS,FAILED_SEARCH_SPEEDUP
!
      real*8 ! REAL*4 before 20030429
     ...
     +         TRANS_GROUP_SELLER_CAP,TRANS_GROUP_BUYER_CAP, ! ADDED FOR DISPATCHABLE HYDRO.
     +         TRANS_GROUP_CAP,
     +         I_AVAILABLE_CAPACITY,
     +         J_AVAILABLE_CAPACITY
      REAL*4   AVE_BUY,AVE_SELL,
     +         TEMP_TRANSACTION, ! AGT is considering making this real*8
...
!      CHARACTER*9 CL_MONTH_NAME(13)
     ...
     +            SAVE_CAPACITY_PLANNING_METHOD/'MI'/
      CHARACTER*3 SCEN_FILE_CHAR_EXT
      real*4 SellerTransPower
      integer*2 iTG ! use current value of global HOUR_IN_DAY
      INTEGER*2 MAX_ACTIVE_GROUPS,
     +          GET_MARKET_PRICE_YEAR
      LOGICAL*1 LAHEY_LF95,LAHEY
...
C      MAX_TRANS_GROUPS = MAX(MAX_TRANS_LOADS,GET_MAX_TRANS_GROUPS()) !REMOVE 10/12/01 
!
! 08/19/03. 
!
...
! A TO/FROM REPORT IS NOT AVAILABLE FOR MARKET PRICE METHOD (ONLY TRANSACT AGAINST MARKET)
...
!         
     ...
     +         call SetupConstraints ! for LP solution to inter-nodal flows
         R_TIE_LIMIT = 999999.   
         NUM_ARGS = 8
...
!
      IF(USE_TRANSACT_LOADS .OR. USE_TF_FILE) THEN
         CALL READ_LOAD_DATA(HOURS_IN_MONTH,ISEAS) ! READS LDE FOR EACH TRANS LOAD GROUP.
      ENDIF
!
...
!
! MOVED 10/22/98. GAT.
! READ MONTHLY MARKET PRICE FILE LDE<5 CHAR>.BXY WHERE XY = 01, 02, 03 FOR EACH YEAR.
!
...
C***********************************************************************
C
C CASE 2 WITH BINARY MARKET FILES
C
C
...
!            
     ...
     +                               (LOCAL_MARKET_PRICE(HR),HR=1,24)
               IREC = IREC + 1
C PUT EXTENSION PERIOD MULTIPLIER HERE
               IF(YEAR > AVAIL_DATA_YEARS) THEN
                  CALL SINGLE_AREA_EXTENTION_PERIOD_PRICE_ESCALATION(
...
!
     ...
     +                             MONTH_TOTAL_HOURS,
     +                             ANNUAL_MARKET_PRICES, 
     +                             MONTH_SUM_MARKET_PRICES(LOCAL_MONTH), ! REAL*8
     +                             MONTH_MAX_MARKET_PRICE(LOCAL_MONTH), 
     +                             MONTH_MIN_MARKET_PRICE(LOCAL_MONTH),
     +                             LOCAL_MONTH)
!
...
!
!
!            CALL GET_OUTPUT_MARKET_PRICE_NAME(MARKET_PRICE_NAME)
...
!
         IF(R_DAY == 28 .AND. R_MONTH == 2) THEN
            WRITE(2811,REC=DAILY_IREC)R_MONTH,INT2(29),LOCAL_YEAR,
     +                          EEICODE,R_DAY_WEEK,
     +                          TIMZON,TEMPER,
     +                          DELTMP,
     +                  (R_LOCAL_MARKET_PRICE(MARKET_HR),MARKET_HR=1,24)
...
!
     ...
     +                                       USER_MONTH_STARTING_HOUR,
     +                                                   R_TRANS_GROUP), 
     +                MONTH_SUM_USER_W_PRICES(USER_MONTH,R_TRANS_GROUP), ! REAL*8
     +                 MONTH_MAX_USER_W_PRICE(USER_MONTH,R_TRANS_GROUP), 
     +                 MONTH_MIN_USER_W_PRICE(USER_MONTH,R_TRANS_GROUP),
     +                  USER_MONTH,
...
!
     ...
     +                                   USER_ANNUAL_STARTING_HOUR,
     +                                               R_TRANS_GROUP), 
     +                             SUM_USER_W_PRICES(R_TRANS_GROUP), ! REAL*8
     +                             MAX_USER_W_PRICE(R_TRANS_GROUP), 
     +                             MIN_USER_W_PRICE(R_TRANS_GROUP),
     +                             ANNUAL_USER_MONTH,
     +                             MULTI_AREA_NAME(R_TRANS_GROUP))
...
!
            IF(R_DAY == 28 .AND. R_MONTH == 2) THEN
!               LOCAL_TIE_FLOW(MARKET_HR) =            
!     +                                 TIE_FLOW(R_TRANS_GROUP,MARKET_HR)
               WRITE(LOCAL_UNIT_NO,REC=USER_IREC)
     +                          R_MONTH,INT2(29),USER_YEAR,
     +                          EEICODE,R_DAY_WEEK,
     +                          TIMZON,TEMPER,
...
!            
     ...
     +                         TEMP_DAILY_PRICE ! 24 ELEMENT ARRAY
         USER_IREC = USER_IREC + 1
         IF(YEAR > AVAIL_DATA_YEARS) THEN
            CALL MULTI_AREA_EXTENTION_PERIOD_PRICE_ESCALATION(YEAR,
...
!
     ...
     +                  GET_HRLY_TG_SCEN_ELECT_MULT(J_HR,I_HR,R_TRANS)
      ...
      entry allocate_2_for_market_price_trans_routine
...
!     +                                              DAY_OF_WEEK(DA,I)) ! ,I) argument list doesn't match
                     SYSTEM_HOURLY_LOADS(CURRENT_HR,0) =
     +                               SYSTEM_HOURLY_LOADS(CURRENT_HR,0) +
     +                                 SYSTEM_HOURLY_LOADS(CURRENT_HR,I)
                     PERIOD_TRANSACTION_DEMAND = 
...
!
               CLOSE(10)
!
C              MAY WANT TO RESORT HIGHEST TO LOWEST IN THE FUTURE
!
            ENDDO ! TRANS LOAD GROUPS
         ENDIF ! JUST MARKET
      RETURN
      
      ENTRY allocate_1_for_market_price_trans_routine
      ...
      entry market_price_transactions_init
        call allocate_1_for_market_price_trans_routine
!
...
!
     ...
     +                                                 MULTI_AREA_PRICE)
!     
            call allocate_2_for_market_price_trans_routine
      ...
      entry allocate_for_fiscal_purchase_energy
...
!
         ...
         ENDIF
         call allocate_3_for_market_price_trans_routine
...
!
      ...
      entry allocate_3_for_market_price_trans_routine
...
!
         ...
         WRITE_DAY_OF_WEEK = 0.
      entry reinit_for_first_monthly_transact
...
! FIRST PASS AND FISCAL_SEASON_RESET (FROM BELOW)        
        call allocate_for_fiscal_purchase_energy
!
...
!
         IF(R_MONTH == FIRST_MONTHLY_TRANSACT) THEN
            call reinit_for_first_monthly_transact
         ENDIF
!         
!         
...
!     +                             +HOURLY_FORWARDS_4_MONTH(I,HR)
     ...
     +                  GET_MARGINAL_COST_AT_MARKET(
     +                      dble(HOURLY_LOAD_B4_SALES(I,HOUR_IN_DAY)),
...
!
     ...
     +                    GET_MARGINAL_COST_AT_MARKET(
     +                       dble(TRANSFER_LOAD_B4(0)),INT2(1),
...
!
                        SELLERS_COST = SELLERS_COST + ! THIS RESETS THE POINT IN THE COST CURVE
     +                     GET_MARGINAL_COST_AT_MARKET(
     +                       dble(TEMP_R1),
     +                       DATA_BASE,SCARCITY_COST(I,HOUR_IN_DAY),L_M,
     +                        SCARCITY_MULT(I)) 
...
!
C               IF(CENTRAL_DISPATCH_TRANSACT) THEN
C                  IF(I == 0) THEN
C                     HOURLY_MARKET_PRICE = 999999.
C                  ELSE  ! USE THE CENTRAL DISPATCH MARGINAL COST
C                     HOURLY_MARKET_PRICE = 
C     +                             HOURLY_MARGINAL_COST(0,HOUR_IN_DAY) *
C     +                                               MARKET_PRICE_SCALAR
C                  ENDIF 
C               ELSE
C                  HOURLY_MARKET_PRICE = MONTHLY_MARKET_PRICES(HR) *
C     +                                               MARKET_PRICE_SCALAR
C               ENDIF
!               
!
...
!
               TG = GET_TRANS_GROUP_INDEX(I)
               DATA_BASE = GET_DATA_BASE_FOR_TRANS(TG,DAY_TYPE)
               IF(.NOT. TRANS_GROUP_GEN_ACTIVE(DATA_BASE)) CYCLE ! DATA BASE NOT DEFINED IN THIS HOUR
...
!
!               
! GET THE STATE OF THE SYSTEM BEFORE TRANSACTIONS I.E. SERVING NATIVE LOAD
!
...
! 6/27/02. 
     ...
     +                     I,
     +                     ALL_MONTH_PRICES(HR,I),   ! (24,31) hourly detail ! Alan: I need a single dimension string with *
     +                     SYSTEM_STORAGE(0,I), ! (24,31) hourly detail ! Alan: I need a single dimension 801 values
     +                     R_MONTH,
     +                     DAYS_IN_MONTH,
...
! 04/12/04.     
     ...
     +                     I,
     +                     ALL_MONTH_PRICES(HR,I),   ! (24,31) hourly detail ! Alan: I need a single dimension string with *
     +                     SYSTEM_DERIVATIVES(0,I), ! (24,31) hourly detail ! Alan: I need a single dimension 801 values
     +                     R_MONTH,
     +                     DAYS_IN_MONTH,
...
!
! NOTE REASSIGNMENT OF HOURLY_LOAD_B4_SALES AND READ ACROSS 24 HOURS ! 080101
!
                  IF(HOUR_IN_DAY == 1) THEN
                     DO J = 1, 24
                        HOURLY_LOAD_B4_SALES(I,J) = 
     +                           GET_TRANS_LOAD_AFTER_EL(INT2(HR+J-1),I)
...
!
     ...
     +               MAX(0.d0,dble(HOURLY_LOAD_B4_SALES(I,HOUR_IN_DAY) +
     +                              HOURLY_TRANSFER_MWH(I,HOUR_IN_DAY) +
     +                           HOURLY_FORWARD_SALE -
     +                                TRANS_ROR_CAPACITY(I)))
...
!         
     ...
     +                                                 TEMP_SELL_SPREAD)
               ENDIF
!
! COMMITMENT IMPACT.  5/13/01. MOVED TEST UP AND HOURLY MARGINAL COST INSIDE
!
...
!
     ...
     +                  HOURLY_MARKET_PRICE -
     +                               HOURLY_MARGINAL_COST(I,HOUR_IN_DAY)
               IF(MARGINAL_COST_DELTA(I,HOUR_IN_DAY) < 0.) THEN ! NET BUYER
...
!
               ...
               ELSEIF(dble(TIE_POWER) > LAST_HOUR_SELL(I) ) THEN ! RAMP UP

                  MAX_HOURLY_RAMP_UP =
     +                     MIN(sngl(dble(TRANS_RAMP_UP(I)) +
     +                                     LAST_HOUR_SELL(I)),TIE_POWER)
               ELSE
                  MAX_HOURLY_RAMP_UP =
     +                     MAX(sngl(LAST_HOUR_SELL(I) -
     +                              dble(TRANS_RAMP_DOWN(I))),TIE_POWER)
...
! END 6/22/98.
            ...
            ENDIF ! COMMITMENT
!
C               WROTE_REPORT =  WRITE_CONTRIBUTION_REPORT(HR,I)
!               
...
!
     ...
     +                  GET_MARGINAL_COST_AT_MARKET(
     +                     dble(HOURLY_LOADS(I,HOUR_IN_DAY) +
     +                              HOURLY_OPTION_CAPACITY -
     +                                       RES_N_SUP_CAPACITY_USED),
     +                     DATA_BASE,SCARCITY_COST(I,HOUR_IN_DAY),L_M,
     +                     SCARCITY_MULT(I))
...
!     
                     R_MONTH = R_MONTH ! NATIVE LOOSING MONEY ON THE TRANSACTION
                  ENDIF
...
!     
     ...
     +                                         HOURLY_MARKET_PRICE) THEN
!     
                     R_MONTH = R_MONTH ! MARKET LOOSING MONEY ON THE TRANSACTION
                  ENDIF
               ELSE
...
!            
     ...
     +                         HOURLY_LOAD_B4_SALES(INT2(0),HOUR_IN_DAY)
...
!
     ...
     +                     CL_MONTH_NAME(R_MONTH),
     +                     FLOAT(DAY),
     +                     AREA_NAME_35, ! MULTI_AREA_NAME(I)(1:7)//' Central Disp',
...
!         
         IREC = ALINE_LOAD_DATA(INT2(1),R_MONTH)
!         
         DEALLOCATE(PRODUCT_PRICE)
         DEALLOCATE( 
!    +      PRODUCT_LAST_PRICE,  ! REMOVED 6/18/98 MSG BECAUSE IT WASN'T
!                                  ALLOCATED ON SECOND MONTH
     +      PRODUCT_VOLATILITY,
     +      PRODUCT_QUANTITY,
     +      PRODUCT_HEATRATE,
     +      PRODUCT_MARGINAL_FUEL,
...
!
     ...
     +              SYSTEM_STORAGE,
     +              SYSTEM_DERIVATIVES)
C MISSING DELALOCTED ARRAYS 8/8/01 MSG
...
C            R_MONTHLY_ECONOMY_REVENUE = SALES_REVENUE(R_MONTH)
C     +                                         R_MONTHLY_ECONOMY_REVENUE
         ELSE ! 'T'
            R_MONTHLY_ECONOMY_BOUGHT = PURCHASE_ENERGY(R_MONTH)
C            R_MONTHLY_ECONOMY_COST = PURCHASE_COSTS(R_MONTH)
            R_MONTHLY_ECONOMY_SOLD = SALES_ENERGY(R_MONTH)
C            R_MONTHLY_ECONOMY_REVENUE = SALES_REVENUE(R_MONTH)
         ENDIF
...
C         R_ANNUAL_ECONOMY_REVENUE = R_ANNUAL_ECONOMY_REVENUE +
C     +                                         R_MONTHLY_ECONOMY_REVENUE
!
      RETURN
C***********************************************************************
...
C***********************************************************************
     ...
     +                                             SALES_ENERGY(R_MONTH)
      ...
      RETURN
C***********************************************************************
      entry GetTransPowerSold(iTG,SellerTransPower) ! positive for the seller
C***********************************************************************
...
!     
C         UPPER_TRANS_GROUP = MAX(MAX_TRANS_LOADS,
C     +                                          GET_MAX_TRANS_ID_USED())
!
c         CALL LOCATE(15,9)
c         WRITE(4,"('&Multi-Party Transaction ')") 
      IF(.NOT. LAHEY_LF95()) 
...
!
     ...
     +                                       TRANS_MAX_EXPORT)
         IF(R_MONTH == FIRST_MONTHLY_TRANSACT) THEN
            IF(ALLOCATED(PRODUCT_LAST_PRICE)) ! ADD 6/18/98 MSG. MOVED. 6/22/98. GAT.
     +                            DEALLOCATE(PRODUCT_LAST_PRICE)
            IF(ALLOCATED(ANNUAL_UNSERVED_ENERGY_COST)) 
     +                           DEALLOCATE(ANNUAL_UNSERVED_ENERGY_COST,
     +                                      ANNUAL_COST_ABOVE_RESOURCES,
...
! THERE ARE NO MARGINS GREATER THAN .1 $/MWH OR NO ADDITIONAL RESOURCES ARE
! AVAILABLE.
!
! A RESOURCE WITH A NATIVE LOAD IS ASSUMED TO ALWAYS MEET THAT LOAD.
! IF IT CANNOT MEET THAT LOAD WITH ITS OWN RESOURCES IT MUST PAY AN EXTERNAL
! TRANSACTION GROUP FOR ADDITIONAL RESOURCE. IF AN EXTERNAL SOURCE CAN SUPPLY
! THAT RESOURCE MORE CHEAPLY, THEN IT WILL BUY FROM THAT ENTITY.
!
...
!
! WHAT IS THE MARKET PRICE? IT IS THE POINT WHERE MARGINAL REVENUE EQUALS
! MARGINAL COST.  IF THE ALGORITHM CORRECTLY FINDS THE GREATEST TO SMALLEST
! NET MARGINS, THE MARKET PRICE SHOULD BE THE PRICE OF THE LAST TRANSACTION 
! ON THE SYSTEM IN A PARTICULAR HOUR.
!
...
!
         LAHEY = LAHEY_LF95()
c         CALL LOCATE(16,9)
c         IF(.NOT. LAHEY) WRITE(4,"('&Hour in Month           ')")
         IF(.NOT. LAHEY)
...
! MOVED UP. 7/22/98. GAT.
            ...
            ELSE
               DAY = HR/24 + 1
            ENDIF
c            CALL GET_DAY_OF_WEEK_IN_MONTH(
c     +                                 DAY,R_MONTH,CALENDAR_DAY_OF_WEEK)
            CALENDAR_DAY_OF_WEEK = GET_DAY_OF_WEEK_4(R_MONTH,DAY)
!
            DAY_TYPE = 
     +              GET_DAY_TYPE(HOUR_IN_DAY,
...
!     
     ...
     +                                  CALENDAR_DAY_OF_WEEK,
     +                        LOCAL_PRODUCT_TYPE(HOUR_IS_ON_PEAK))) THEN
!     +                           LOCAL_PRODUCT_TYPE(HOUR_IS_5X16))) THEN
               PEAK_HOUR = 1
            ELSE
               PEAK_HOUR = 2
            ENDIF
...
!            
     ...
     +                               SELLER == 0  .OR. BUYER == 0) CYCLE
               ...
               ENDDO
!
!               DO I = 1, MIN(50,MAX_TRANS_WITHIN_HOUR)
! BIG TRANSACTIONS
!                  IF(MW_FOR_TRANSACTION(I) > 100.) THEN
!                     LAST_BUY_FOR_TRANSACTION(K) =
!     +                                            BUY_FOR_TRANSACTION(I)
!                     LAST_SELL_FOR_TRANSACTION(K) =
!     +                                           SELL_FOR_TRANSACTION(I)
!                     LAST_MW_FOR_TRANSACTION(K) =
!     +                                          MW_FOR_TRANSACTION(I)
!                  ENDIF
!               ENDDO
...
! OTHER CRITERIA
               LAST_TWH = K - 1
c               DO I = K, MAX_TRANS_WITHIN_HOUR
c                  LAST_BUY_FOR_TRANSACTION(K) = 0
c                  LAST_SELL_FOR_TRANSACTION(K) = 0
c                  LAST_MW_FOR_TRANSACTION(K) = 0.d0
c               ENDDO
...
!            HOURLY_EUE = 0.
            ...
            LAST_SELLER = 0.
            VOID_LOGICAL = INIT_HOURLY_TIE()
!            VOID_LOGICAL = INIT_HOUR_PATH_LIMIT()
! 1/8/03. ALTERED HOUR_PATH_LIMIT TO REFLECT MULTIPLIERS
...
!
!
C            M_HOURLY_LOAD_B4_SALES(0,HOUR_IN_DAY) = r8TEMP
!
!            HOURLY_MARKET_PRICE = GET_MARGINAL_COST_AT_MARKET(
!     +                                r8TEMP,
!     +                                DATA_BASE)
C            HOURLY_MARKET_PRICE = GET_MARGINAL_COST(
C     +                                SYSTEM_HOURLY_LOADS(HR,0),
C     +                                DATA_BASE,999999.)
            TEMP_CAP = 0.
            DO I = 1, UPPER_TRANS_GROUP
               TG = GET_TRANS_GROUP_INDEX(I)
...
!                  CAPPED_PRICE(I) = GET_PRICE_CAP(I)
!               ELSE
!                  CAPPED_PRICE(I) = 999999.
!               ENDIF
!               TEMP_CAP = TEMP_CAP +
!     +                 GET_PRODUCTION_LEVEL_AT_MARKET(
!     +                                 HOURLY_MARKET_PRICE,DATA_BASE)
C     +               GET_PRODUCTION_LEVEL(  HOURLY_MARKET_PRICE,
C     +                                      DATA_BASE,
C     +                                      999999.)
               
            ENDDO
!
C            IF(USE_MARKET_PRICING) THEN
C               HOURLY_MARKET_PRICE =  MONTHLY_MARKET_PRICES(HR)
C            ELSE
...
!     +                           HOUR_IN_DAY,
!     +                           CALENDAR_DAY_OF_WEEK,DAY,I,
!     +                           PRODUCT_LAST_PRICE(I,3),R_MONTH) ! ??? NOT SURE IF I IS RIGHT ???
!     +                           DAILY_PRICE(HOUR_IN_DAY),R_MONTH) ! ??? NOT SURE IF I IS RIGHT ???
!
! 061808. FOR BURESH AND THE UZD REPORT
!
!               IF(STEP_4) THEN
!                  DO J_F = 1, 3
!                    L_F = HOUR_IN_DAY + J_F
...
!
                     IF(HOUR_IN_DAY == 1) THEN
                        DO J = 1, 24
                           TEMP_I2 = HR+J-1
                           M_HOURLY_LOAD_B4_SALES(I,J) = dble(
     +                               GET_TRANS_LOAD_AFTER_EL(TEMP_I2,I))
                           DAILY_PEAK(I) = MAX(DAILY_PEAK(I),
...
!
!                     r8LoadB4 = MAX(0.,GET_TRANS_LOAD_AFTER_EL(HR,I) -
                     r8LoadB4 = MAX(0.d0,
     +                       M_HOURLY_LOAD_B4_SALES(I,HOUR_IN_DAY) -
     +                                            TRANS_ROR_CAPACITY(I))
!                     r8TEMP = MAX(0.,
!     +                  GET_TRANS_LOAD_AFTER_EL(HR,I) -
!     +                               TRANS_ROR_CAPACITY(I) +
!     +                               TRANS_SPINNING_CAPACITY(I)) ! 10/14/98. GAT.
                     r8TEMP = M_HOURLY_LOAD_B4_SALES(I,HOUR_IN_DAY) -
     +                               TRANS_ROR_CAPACITY(I) +
     +                                       HOURLY_SPINNING_CAPACITY(I)  ! 4/16/99. GAT.
C                     r8TEMP = MAX(0.d0,HOURLY_TRANSACTION_LOAD(HR,I))
                  ELSE
...
!
                  ...
                  M_HOURLY_LOAD_B4_SALES(INT2(0),HOUR_IN_DAY) =
     +                     M_HOURLY_LOAD_B4_SALES(INT2(0),HOUR_IN_DAY) +
     +                                                          r8LoadB4
                  M_HOURLY_LOADS(I,HOUR_IN_DAY) = r8TEMP
!
C                  HOURLY_TRANS_GROUP_GEN_ACTIVE(I) =
C     +                        TRANS_GROUP_GEN_ACTIVE(DATA_BASE) .AND.
C     +                  TRANS_GROUP_CAP(DATA_BASE) >
C     +                                         M_HOURLY_LOAD_B4_SALES(I)
!                  HOURLY_TRANS_GROUP_GEN_ACTIVE(I) =
!     +                                 TRANS_GROUP_GEN_ACTIVE(DATA_BASE)
               ELSE
!
!                  M_HOURLY_LOAD_B4_SALES(I,HOUR_IN_DAY) = 0.d0
                  M_HOURLY_LOAD_B4_SALES(I,HOUR_IN_DAY) =
     +                                              HOURLY_FORWARD_SALE
...
! 4/25/01. GAT.
     ...
     +               HOURLY_MARGINAL_COST(I,HOUR_IN_DAY) = ! ALSO USED TO SET POS()
     +                     GET_MARGINAL_COST_AT_MARKET(
...
! GLOBAL DATABASE.
!
            DATA_BASE = GET_DATA_BASE_FOR_TRANS(INT2(0),DAY_TYPE)
!            
!            IF(USE_TF_FILE) THEN
!               r8TEMP = GET_TRANS_LOAD_AFTER_EL(HR,0)
!            ELSE
!               r8TEMP = SYSTEM_HOURLY_LOADS(HR,0)
!            ENDIF
!
...
!
     ...
     +                              SCARCITY_COST(MARKET_TG,HOUR_IN_DAY)
            ...
            ENDIF
!
! FIND DELIVERED COSTS BETWEEN ALL TRANSACTION GROUPS FOR THE NEXT TRANSACTION
!
!
            FAILED_DIAGNOSTICS = .FALSE.
!
            DO ! DO UNTIL TRANSACTIONS ARE NO LONGER ECONOMIC OR PHYICALLY POSSIBLE
!
!
...
!
     ...
     +                        MAX_LOCAL_IMPORT,
     +                        MAX_LOCAL_EXPORT,
     +                        dble(PATH_CAPACITY)*.99d0) ! 01/23/03.
...
!
                  DATA_BASE = GET_DATA_BASE_FOR_TRANS(TG,DAY_TYPE)
!
!                  IF(M_HOURLY_LOADS(I,HOUR_IN_DAY) <= 0.0d0) THEN
!                     HOURLY_TRANS_GROUP_LOAD_ACTIVE(I) = .FALSE.
!                  ELSE
                     HOURLY_TRANS_GROUP_LOAD_ACTIVE(I) = .TRUE.
!                  ENDIF
!
...
!
         ...
         ELSE
!
               IF(FAILED_DIAGNOSTICS) THEN ! MAX TRANSACTIONS AND OTHER DIAGNOSTICS
C                  CALL LOCATE (12,5)
C                  WRITE(4,*) "More than 100 transactions within hour"
C                  WRITE(4,*) "HOUR = ",HR,"  MONTH = ",R_MONTH
C                  WRITE(4,*) "  "
!
                  EXIT
!                 STOP
...
!
!
            IF(CURRENT_YEAR == 2036 .AND. HR == 481 .AND. 
     +                                                  TWH == 173) THEN
               CURRENT_YEAR = CURRENT_YEAR
            ENDIF
!
!               IF(YEAR == 19 .AND. R_MONTH == 2 .AND. HR == 28 .AND.
!     +                                                  TWH == 139) THEN
!                  TWH = TWH
!               ENDIF
!
...
!
!
! CALCULATE DELIVERED COST OF CURRENT TRANSACTION GROUP, I, TO OTHER GROUPS J
!
                  IF(HOURLY_TRANS_GROUP_GEN_ACTIVE(I)) THEN
                     SELLER_DB = 
     +                     GET_DATA_BASE_FOR_TRANS(
...
!                     IF(I_AVAILABLE_CAPACITY < -1.d0) THEN
     ...
     +                                           TRANSACT_ABUNDANCE_COST
                  ...
                  ENDIF
!     
                  DO J = I-1, 1, -1 ! DON'T CHECK DIAGONAL. 4/21/98. GAT.
!                     
...
!                     
     ...
     +                     HOURLY_TRANS_GROUP_LOAD_ACTIVE(J)) THEN ! SELLER = I, BUYER = J.
...
!                        
                        IF( TEMP_MARGIN > NET_MARGIN(J) .AND.! NEGATIVE MARGIN MEANS I AM BETTER OFF SELLING TO MYSELF
     +                     TEMP_MARGIN >= 
...
!
                              MAX_HOURLY_RAMP_UP =
     +                              MAX(0.d0,dble(TRANS_RAMP_UP(I)) -
     +                                      HOURLY_TRANSACTION(I) +
     +                                           LAST_HOUR_SELL(I))
                              MAX_HOURLY_RAMP_DOWN =
     +                            MAX(0.d0,dble(TRANS_RAMP_DOWN(J)) -
     +                                      HOURLY_TRANSACTION(J) -
     +                                           LAST_HOUR_SELL(J))
!
                              PATH_CAPACITY = HOUR_TIE_LIMIT(I,J)
...
! 081307. TEMP !!!
     ...
     +                        HOURLY_TRANS_GROUP_LOAD_ACTIVE(I)) THEN ! SELLER = J, BUYER = I, ! DOES THE OTHER SIDE OF THE DIAGONAL     
!
...
!
     ...
     +                                         TOTAL_DELIVERED_COST(J,I)
                        ...
                        IF(TEMP_MARGIN > NET_MARGIN(I) .AND. ! NEGATIVE MARGIN MEANS I AM BETTER OFF SELLING TO MYSELFI
     +                     TEMP_MARGIN >= 
     +                             MAX(PATH_SPREAD_RATE(J,I,
     +                                     PEAK_HOUR,R_MONTH,YEAR)+.001,
     +                            PATH_SPREAD_RATE(I,J,
...
!
                              MAX_HOURLY_RAMP_UP =
     +                              MAX(0.d0,dble(TRANS_RAMP_UP(J)) -
     +                                      HOURLY_TRANSACTION(J) +
     +                                           LAST_HOUR_SELL(J))
                              MAX_HOURLY_RAMP_DOWN =
     +                            MAX(0.d0,dble(TRANS_RAMP_DOWN(I)) -
     +                                      HOURLY_TRANSACTION(I) -
     +                                           LAST_HOUR_SELL(I))
!
...
!
     ...
     +                         BEST_MARGIN > TRANSACTION_TOLERANCE) THEN ! DID THE MODEL FIND A VALID TRANSACTION?
!
C                  HOURLY_MARKET_PRICE = 
C     +                           HOURLY_MARGINAL_COST(BUYER,HOUR_IN_DAY)
!
...
!
     ...
     +                                         MAX_LOCAL_EXPORT,
     +                                         dble(PATH_CAPACITY),
     +                                   dble(MAXIMUM_TRANSACTION_SIZE))
!
!                  BUYERS_LOAD = MAX(M_HOURLY_LOADS(BUYER,HOUR_IN_DAY),
!     +                                  GET_MUST_RUN_CAPACITY(BUYER_DB))
!
                  BUYERS_LOAD = M_HOURLY_LOADS(BUYER,HOUR_IN_DAY)
!
! 06/03/04.
...
! BE LESS THAN OR EQUAL TO THE AMOUNT THAT THE BUYER IS ABLE PRODUCE AT ITS OWN COST. 
! IE SELLERS_COST + TIE_COST... < BUYERS_COST AND BUYERS_COST'S ARE NON-DECREASING.
!
                  TEMP_BUYER_TRANSACTION =
...
!
     ...
     +                           SCARCITY_MULT(BUYER))
                  ...
                  ENDIF
!                  
C                  HOURLY_MARKET_PRICE = HOURLY_MARGINAL_COST(BUYER,HOUR_IN_DAY)
                  MINIMUM_MARGIN = 
     ...
     +                              SECOND_BEST_MARGIN)
               ...
               ELSE
!
                  EXIT ! NO TRANSACTIONS POSSIBLE AFTER CALCULATING MARGINS. GO ON TO THE NEXT HOUR.
!
               ENDIF
!
               HOURLY_DUMP_USED = 0.
...
! 06/03/04.
     ...
     +                         sngl(TEMP_INTERP_TRANSACTION) > 0.1) THEN
                  TEMP_TRANSACTION = MIN(TEMP_INTERP_TRANSACTION,
     +                                       BUYERS_UNSERVED_ENERGY+.01)
                  R8TEMP = SELLERS_LOAD+dble(TEMP_TRANSACTION)
                  SELLERS_COST =
     +               GET_MARGINAL_COST_AT_MARKET(
     +               R8TEMP,
     ...
     +                                       TRANS_GROUP_BUYER_CAP) THEN
                     R8TEMP = BUYERS_LOAD-dble(TEMP_TRANSACTION)
                     BUYERS_COST =
     +                     GET_MARGINAL_COST_AT_MARKET(
     +                           R8TEMP,BUYER_DB,
     +                           SCARCITY_COST(BUYER,HOUR_IN_DAY),L_M,
...
!
               ...
               ELSEIF(TEMP_INTERP_TRANSACTION > 0.5d0) THEN ! MINIMUM SIZE TRANSACTION
C               IF(TEMP_INTERP_TRANSACTION > 0.d0 .AND.
C     +                              TEMP_SELLER_TRANSACTION < 0.d0) THEN ! LINEAR INTERSECTION
!
...
!
                  IF(.NOT. HOURLY_PRICE_RECONCILE) THEN
                     MIN_PRICE =
     +                   dble(HOURLY_MARGINAL_COST(SELLER,HOUR_IN_DAY) +
     +                                          TIE_COST_OF_BEST_MARGIN)
     ...
     +                                         TIE_COST_OF_BEST_MARGIN))
                     MAX_PRICE =
     +                  MAX(MIN_PRICE,
     +                    dble(HOURLY_MARGINAL_COST(BUYER,HOUR_IN_DAY) -
     +                                         TIE_COST_OF_BEST_MARGIN))
                  ENDIF
                  SEARCHES_WITHIN_TRANSACTIONS = 0
                  TEMP_TRANSACTION = 0.
...
!
!
! ASSUME FUNCTIONS ARE APPROXIMATELY LINEAR AND SOLVE FOR THE INTERSECTION
! OF THE TWO LINES. THE TWO LINES HAVE COMMON MINIMUM AND MAXIMUM PRICES
! (I THINK).
! FURTHER ASSUME THAT THE BUYER IS A TRADITIONAL DOWNWARD SLOPING DEMAND CURVE,
! AND THE SELLER IS THE TRADITIONAL UPWARD SLOPING DEMAND CURVE.  
! ALIGN THE TWO LINES SO THAT THE MAX PRICE IS SET TO THE ORIGIN.
! THIS MEANS THAT THE INTERSECTION OCCURS AT A NEGATIVE TRANSACTION VALUE.  
! THE ASSUMPTION OF LINEARITY MAY BE TESTED BY SEEING HOW CLOSE THE 
! RECALCULATED PRICES ARE TO THE PRICES ESTIMATED BY THE INTERSECTED LINES.
!
!
!
...
!                     
     ...
     +                               HOURLY_DUMP_USED)
                     ...
                     DOWHILE(.NOT. TRANSACTION_COMPLETED)
                        IF(TEMP_SELLER_TRANSACTION == MIN_TRANSACTION)
     +                                                              THEN
                           TEMP_TRANSACTION = MIN_TRANSACTION
                           TRANSACTION_COMPLETED = .TRUE.
!                        ELSEIF( ABS(TEMP_BUYER_TRANSACTION -
!     +                                     MIN_TRANSACTION)<.001d0) THEN
     ...
     +                                  "IRREGULAR COST CURVES DETECTED"
                       er_message='See WARNING MESSAGES -transobj.for-9'
                              call end_program(er_message)
                           ENDIF
                           SELLER_SLOPE = (MAX_PRICE - MIN_PRICE) /DENOM ! unused
!unused                    SELLER_INTERCEPT = MAX_PRICE
     ...
     +                                  "IRREGULAR COST CURVES DETECTED"
                     er_message='See WARNING MESSAGES -transobj.for-10'
                              call end_program(er_message)
                           ENDIF
                           BUYER_SLOPE = (MAX_PRICE - MIN_PRICE) / DENOM ! unused
...
!
!                          TRANSACTION WAS NOT BIG ENOUGH. FLAT PART OF THE COST CURVES.
!
...
!
!
!                       TRANSACTION WAS TOO BIG.
!
!                        ELSEIF(TOTAL_DELIVERED_COST(SELLER,BUYER) -
!     +                                           BUYERS_COST > 0.2) THEN
!                           TEMP_SELLER_TRANSACTION = TEMP_TRANSACTION
!                           MIN_TRANSACTION = 0.0d0
!                           MAX_PRICE = BUYERS_COST
!                           MIN_PRICE = SELLERS_COST
!
! !!!! TESTING. 10/25/02.
!
                        ELSE
...
!                     
                     ...
                     CONVERGENCE_PATH = 26.
                  ENDIF
!               ELSEIF(TEMP_INTERP_TRANSACTION > 0.d0) THEN ! ADDED FOR SRP. 8/24/99. GAT.
               ELSEIF(TWH <= 2*UPPER_TRANS_GROUP  .AND. 
     +                              TEMP_INTERP_TRANSACTION > 0.d0) THEN ! ADDED FOR SRP. 8/24/99. GAT.
                  TEMP_TRANSACTION = TEMP_INTERP_TRANSACTION
                  TRANSACTION_COMPLETED = .TRUE.
                  CONVERGENCE_PATH = 3.
! DEFAULT CASE. 071109 (CONVERGENT CASE = -99)                  
               ELSEIF(TEMP_TRANSACTION > 0.) THEN 
C               ELSEIF(TEMP_TRANSACTION > 0. .AND.
C     +                              TEMP_SELLER_TRANSACTION < 0.d0) THEN
                  MIN_TRANSACTION = 0.0d0
...
! HOURLY_DUMP LOGIC
!
!               TEMP_TRANSACTION = TEMP_TRANSACTION + HOURLY_DUMP_USED
!
               HOURLY_DUMP_CAPACITY(SELLER) = 
     +                   HOURLY_DUMP_CAPACITY(SELLER) - HOURLY_DUMP_USED
               IF(HOURLY_DUMP_CAPACITY(SELLER) < .01) ! 070108.
     +                               HOURLY_DUMP_CAPACITY(SELLER) = 0.0
!               IF(HOURLY_DUMP_USED > 0.) THEN
!                  r8TEMP = MAX(0., HOURLY_DUMP_CAPACITY(SELLER) -
!     +                                                 TEMP_TRANSACTION)
!                  TEMP_TRANSACTION = MAX(0.,TEMP_TRANSACTION -
!     +                                     HOURLY_DUMP_CAPACITY(SELLER))
!                  HOURLY_DUMP_CAPACITY(SELLER) = r8TEMP
!               ENDIF
!
!
...
!                     TOTAL_ZERO_MW_TRANS_PER_MONTH = 
!     +                            TOTAL_ZERO_MW_TRANS_PER_MONTH + 1
!                  ENDIF
!                  
...
!
! TEST ALLOWING THE LONG PATH
! 12/26/02. MAJOR RE-WRITE OF THIS SECTION.
!
!
!                           LONG_PATH_TRANSACTION_PAIR(
!     +                              SELL_FOR_TRANSACTION(TWH),
...
!                              IF(LONG_PATH_TRANSACTION_PAIR(
!     +                              SELL_FOR_TRANSACTION(TWH),
!     +                             BUY_FOR_TRANSACTION(TWH-1)) < 1) THEN
...
!     +                                          PEAK_HOUR,R_MONTH,YEAR))
     ...
     +                                     L_P)
!                              ELSE
! THIS APPEARS TO GET RID OF LONG PATH                                 
!                                 TEMP_R1 = 0.
!                                 TEMP_R2 = 
!     +                                  SET_HOUR_LONG_PATH_PARAMS(
!     +                                     TEMP_R1,
...
!               ELSE
!                  HOURLY_TRANS_GROUP_LOAD_ACTIVE(BUYER) = .TRUE.
               ENDIF
!
! 7/30/99. GAT.
!
...
! 01/31/02. FOR LGE.
!               HOURLY_LAST_PRICE(BUYER,HOUR_IN_DAY) =
!     +               MIN(HOURLY_LAST_PRICE(BUYER,HOUR_IN_DAY),
!     +                          HOURLY_MARGINAL_COST(BUYER,HOUR_IN_DAY))
!
...
!     
               ELSE
!
! 052310 REMOVED BECAUSE OF AN ARRAY DIMENSION VIOLATION IN THE ROUTINE MSG
C                  VOID_LOGICAL = 
C     +               RECORD_TRANSACTION_TO_TIE(SELLER,BUYER,
C     +                                     TEMP_TRANSACTION,HOUR_IN_DAY)
!
...
!
     ...
     +                                                  TEMP_TRANSACTION
               ENDIF
!
!               TEST_HOUR_TIE_LIMIT(SELLER,BUYER) =
!     +              TEST_HOUR_TIE_LIMIT(SELLER,BUYER) - TEMP_TRANSACTION
!               IF(ABS(HOUR_TIE_LIMIT(SELLER,BUYER,
!     +                                 PEAK_HOUR,R_MONTH,YEAR) -
!     +                            TEST_HOUR_TIE_LIMIT(SELLER,BUYER)) >
!     +                                                          1.) THEN
!                  TEMP_R1 = HOUR_TIE_LIMIT(SELLER,BUYER,
!     +                                 PEAK_HOUR,R_MONTH,YEAR) -
!     +                            TEST_HOUR_TIE_LIMIT(SELLER,BUYER)
!
!               ENDIF
!
!               VOID_LOGICAL = ! AGT notes inconsistent order of arguments:
!     +            REDUCE_PATH_CAPACITY(HR,R_MONTH,
!     +                                    SELLER,BUYER,TEMP_TRANSACTION)
!
               TEMP_TRANSACTION = 0.
               IF(DC_LOAD_FLOW) THEN               
                  call LPsolver(NewStructure) ! optimize the inter-nodal flows within limits
...
!
!
            ENDDO ! DO UNTIL TRANSACTIONS ARE NO LONGER ECONOMIC OR PHYICALLY POSSIBLE
            CALL RW_PROCESS_MESSAGES()
!
! 1/3/3.
!
...
!
            IF(PRICE_LAST_MW) THEN
               DO I = 1, UPPER_TRANS_GROUP
                  IF(LAST_BUYER(I) == 0) THEN ! LAST TRANSACTION WAS A BUY
                     SELLER = LAST_SELLER(I)
                     BUYER = I
                     IF(MAX_HOURLY_IMPORT(BUYER) + ! HOURLY_TRANSACTION IS NEGATIVE FOR BUYER
     +                            HOURLY_TRANSACTION(BUYER) < .1d0) THEN
                        BUYER_DB = GET_DATA_BASE_FOR_TRANS(
     +                            GET_TRANS_GROUP_INDEX(BUYER),DAY_TYPE)
!     +                           GET_DATA_BASE_FOR_TRANS(BUYER,DAY_TYPE)
...
!
                     HOURLY_LAST_PRICE(SELLER,HOUR_IN_DAY) =
!     +                    MAX(0.,HOURLY_LAST_PRICE(BUYER,HOUR_IN_DAY)  - ! ALTERED 5/22/02.
...
!
     ...
     +                                    SCARCITY_MULT(I))
! 041008. FOR BURESH.
!               IF(GET_CREATE_HOURLY_PRICE(I)) THEN
...
!     +                              HOURLY_DUMP_BEFORE(I) -
     +                                           HOURLY_DUMP_CAPACITY(I)
               ENDIF
! MOVED FROM INSIDE THE ABOVE IF STATEMENT TO SEE WHETHER IT FIXES THE PROBLEM.
               IF(SELLER_DB > 0) THEN
! 091712. TO GET ENERGY COST
                  ENERGY_COST(I,HOUR_IN_DAY) = ! THIS RESETS THE POINT IN THE COST CURVE
     +                  GET_MARGINAL_COST_AT_MARKET(
     +                         M_HOURLY_LOADS(I,HOUR_IN_DAY),
     +                         SELLER_DB,
...
!
               IF(.NOT. HOURLY_PRICE_RECONCILE) THEN
                  IF(LAST_BUYER(I) /= 0) THEN ! LAST TRANSACTION WAS A SELL
                     HOURLY_LAST_PRICE(I,HOUR_IN_DAY) =
     +                     HOURLY_MARGINAL_COST(I,HOUR_IN_DAY) +
     +                     SCARCITY_COST(MARKET_TG,HOUR_IN_DAY)
! 12/27/01.
...
!
!
!                  HOURLY_UNSERVED_ENERGY = HOURLY_UNSERVED_ENERGY -
!     +                                              I_AVAILABLE_CAPACITY
...
!               IF(trim(MULTI_AREA_NAME(I)) == 'MEMO') 
!     +                  USER_MARKET_PRICE(HOUR_IN_DAY) = 
!     +                                  HOURLY_LAST_PRICE(I,HOUR_IN_DAY)
...
!
 ...
 !                 ENDIF
            ...
            ENDIF
            IF(REMAIN < .001) THEN ! SUMMARIZE THE DAY
               START_HOUR = Max(1,HR - 23) ! 9/25/09 fix becasue of -2 value for start_hour msg)
!               DAY = HR/24
!
               WRITE_DAY_OF_WEEK(DAY) = CALENDAR_DAY_OF_WEEK
!
...
!
! 05/09/01.
! TESTING FOR TVA W/O THIS CONDITION AND MOVED FROM YES_TRANSACT_HOURLY_REPORTS:
!                  IF(UPPER_TRANS_GROUP > 1) 
!
...
C     
     ...
     +                  FLOAT(DAY),
     +                  MULTI_AREA_NAME(0),
     +                  (M_HOURLY_MC_B4_SALES(INT2(0),RPT_HR),
     +                                             RPT_HR=1,DAILY_HOURS)
                  TRANS_MCB_REC = TRANS_MCB_REC + 1
               ENDIF
               IF(YES_HOURLY_AFTER_HYDRO_REPORT .OR.
...
!            
     ...
     +                                   M_MONTHLY_LOAD_B4_SALES(I))
            ...
            ENDIF
!
!            M_MONTHLY_LOAD_B4_SALES(I) = M_MONTHLY_LOAD_B4_SALES(I) -
!     +                                    GET_WH_MONTH_ENERGY(R_MONTH,I)
!
            IF(USE_TF_FILE) THEN
!
               WH_MONTH_ENERGY = GET_WH_MONTH_ENERGY(R_MONTH,I)
...
! HOURLY CALC. 8/1/01.
!
!            TEMP_SPINNING_MWH = TRANS_SPINNING_CAPACITY(I) * 
!     +                                                  R_HOURS_IN_MONTH
            M_SPINNING_MWH(I) = M_SPINNING_MWH(I) + 
     +                                       HOURLY_SPINNING_CAPACITY(I)
!
...
!
     ...
     +                           ANNUAL_PRODUCT_FUEL_PRICE(I,J) + 
     +                                           PRODUCT_FUEL_PRICE(I,J)
!                     ANNUAL_PRODUCT_HOURS(J) = ANNUAL_PRODUCT_HOURS(J) +
!     +                                                  PRODUCT_HOURS(J)
!
                     IF(PRODUCT_HOURS (J) > 1.) THEN
                        PRODUCT_PRICE(I,J) = 
...
C     +               PRODUCTION_COST_AFTER_SALES)
C     +               HOURLY_LAMDA,
C     +               TRANSACTIONS_WITHIN_HOUR,
C     +               M_MONTHLY_PRO_COST_AFTER_SALES,
C     +               M_MONTHLY_PRO_COST_B4_SALES,
C     +               M_MONTHLY_LOAD_AFTER_SALES,
C     +               M_MONTHLY_LOAD_B4_SALES,
C     +               M_HOURLY_LOAD_B4_SALES,
C     +               HOURLY_TRANS_GROUP_LOAD_ACTIVE,
C     +               HOURLY_TRANS_GROUP_GEN_ACTIVE)
!
C ARRAYS THAT WERE NOT BEING DEALLOCATED AS OF 8/8/01 MSG         
