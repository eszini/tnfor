...
!     CALL CINITW(NUC_RECORD_POSITION,INT(200*15),0)
C
...
! SP CapEx Variables added 11/17/05 MSG
     ...
     +                      ACCESS="DIRECT",STATUS="UNKNOWN",RECL=LRECL)
               DO I = 1, NUC_UNITS_IN_FILE(FILE_ID)
                  IREC = NUC_RECORD_POSITION(I,FILE_ID) !debugstop
...
C***********************************************************************
C
!          SUBROUTINE TO READ FOSSIL DIRECT ACESS BINARY FILES
!          COPYRIGHT (C) 1983, 84, 85  M.S. GERBER & ASSOCIATES, INC.
C
C***********************************************************************
C
...
C
!      RECURSIVE FUNCTION CL_UNITS_READ()
...
! Primary_mover_index_db function moved to prim_mover_idx module.c-----
...
!               TG=TRANSACTION_GROUP(R_UNIT_NO) ! TRANSACTION_GROUP FUNCTION REPLACED WITH THE ARRAY 11/02/06
