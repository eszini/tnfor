...
! MARKETSYM TRANSFER VARIABLE
     ...
     +                    UNITS_BEFORE_ADDITIONS
      LOGICAL (KIND=1), SAVE :: MarketSymFileOpen=.FALSE.
      INTEGER (KIND=4) :: MARKETSYM_REC
C
      LOGICAL*1 KEPCO,CONTRACTS_IN_PERIOD,WABASH_VALLEY,
     +          REALLY_KEPCO,
     ...
     +          GET_DERIV_ZONE_ID
      ...
      CHARACTER*6 MARKET_ID,EIA_PLANT_CODE,
     ...
     +             CM_NAME*35 ! 092011. GAT.
      ...
      CHARACTER*2 CAPACITY_PLANNING_METHOD,LOAD_FILE_CHAR_EXT
      CHARACTER*6 STATE_PROVINCE ! CHANGED 060206.
      CHARACTER*20 TEMP_NAME
      LOGICAL*1 UNIT_OUTPUT_REPORT,MONTHLY_BLOCK_REPORT
      LOGICAL*1MONTHLY_SUMMARY_REPORT_ACTIVE,MONTHLY_SUMMARY_REPORT
      INTEGER*2 ISEAS,UNITNO,YR,I,BLKNUM,
...
!     INTEGER*4 FMT1,FMT2
     ...
     +     AVERAGE_FUEL_COST_PER_MWH,DAYS_IN_MONTH
      REAL*8 DEMAND,PENRG,PMMBTUS,PFUELCST,PVARCOST,
     +  P_PUR_POWER_COST,
...
!
     ...
     +         MONTHLY_P_HEAT,MONTHLY_S_HEAT,MONTHLY_E_HEAT
      REAL*8   MONTHLY_TRANS_HEAT,MONTHLY_TRANS_FUEL_COST,
     +         FISCAL_CL_UNIT_MMBTUS(MAX_CL_UNITS)
! NDIM DETAILED REPORTS. 12/2/93.
      LOGICAL*1   MON_CL_REPORT_NOT_OPEN/.TRUE./,
...
!
     ...
     +               FISCAL_UNIT_START_COSTS
      ...
      INTEGER*2 MON_CL_UNIT_NO,MON_CL_UNIT_HEADER,
...
!
     ...
     +                          'November ','December ',
     +                          'Annual   ','Fiscal Yr'/
      CHARACTER*3 THREE_CHAR_MONTH_NAMES,PA_CHAR_NUM,
     +            CM_CHAR_NUM
      SAVE     MON_CL_UNIT_NO,
...
! 112920. MFT1=14 FOR DG (?).
	  ...
	  real, allocatable :: ANNUAL_FUEL_COST_BY_TG_BY_FUEL(:,:) ! ACCUMULATE BY TRANSACTION GROUP AND FUEL TYPE
...
C
C
      LOGICAL*1   MONTHLY_TRANS_REPORT,
     +            ANNUAL_TRANS_REPORT,
     ...
     +       AVERAGE_VAR_OM,
     +       TOTAL_NOX_HEAT
      REAL*4 MONTHLY_NOX_SEASON_THERM
!
      CHARACTER*5 CL_UNIQUE_RPT_STR !  THAT GETS A UNIQUE ID FOR THE UNIT
      CHARACTER*10 SAC_ONLINE_DATE,SAC_DATE
      CHARACTER*16 SAC_UNIT_NAME,GET_RUN_DATE_TIME,SAC_STATION_GROUP
!
! DETAILED REPORTING FOR ICAP.
!
         LOGICAL*1   MX_ICAP_SUMMARY_REPORT/.FALSE./,
     +               MX_ICAP_REPORT_NOT_OPEN/.TRUE./,
     +               MARGINAL_ICAP

         INTEGER*2   MX_ICAP_REPORT_VARIABLES,
...
!      CHARACTER*35      GET_GROUP_NAME
     ...
     +                  TransEffectiveCapHours,
     +                  TransAvailHoursMonth
      REAL*4 TOTAL_EMISSION_COSTS,PER_UNIT_EMISSION_COST
! 031005
      REAL*4 ::  GET_WH_MONTH_ENERGY,
     +         TEMP_TL_MWH,
     +         ROR_CAPACITY,
     ...
     +                               'PWB'//TRIM(GET_SCENAME())//'.XML'
         ELSE
            FILE_NAME = 'PWB'//TRIM(GET_SCENAME())//'.CSV'
         ENDIF
...
C
     ...
     +     CL_UNIT_ANNUAL_VAR_COST
      LOGICAL*1 KEPCO,ANNUAL_OUTPUT_REPORT,ANNUAL_UNIT_OUTPUT_REPORT,
     +          WABASH_VALLEY,REALLY_KEPCO
      REAL CNTR_SALES_REVENUE,CNTR_SALES_ENERGY
      CHARACTER*1 UTILITY_TYPE,COUNTRY,SCENAME*5,UNIT_NAME*20,
     +            KEPCO_REPORTS,CSV_REPORT
      LOGICAL*1 CANADA
      INTEGER*2 I,YR,EM,J,
     +          NUMBER_OF_CONTRACTS,RECORDS_PER_UNIT/0/,
     +          RECORDS_PRINTED_PER_UNIT/0/
      INTEGER*2 LAST_YEAR,RUN_YEARS,SAVE_YEAR
      REAL CAP_FACTOR
      INTEGER*2 CNTR_ON_LI(MAX_CONTRACTS),CNTR_OFF_LI(MAX_CONTRACTS)
      CHARACTER*20  CNTRNM(MAX_CONTRACTS)
      INTEGER*4 OFFSET/0/,IREC,NEXT_OFFSET/0/
!     INTEGER*4 FMT1
      LOGICAL*1 TVA
     ...
     +              NEW_LINE*2,FILE_NAME*64
      CHARACTER*80 PRODUCTION_METHOD_TITLE
      LOGICAL*4 FILE_EXIST
      LOGICAL*1 POOLING_TRANSACTIONS,
     +          FIRST_TIME_BY_UNIT/.TRUE./,
     +          FIRST_TIME_BY_YEAR/.TRUE./,
     +          SPREAD_SHEET,BY_UNIT,BY_YEAR
      REAL*4 ANNUAL_TOTAL_EMISSIONS(NUMBER_OF_EMISSION_TYPES)
      REAL*4 ANNUAL_AI_INVESTMENT,TOTAL_COST,AVERAGE_COST
      REAL   ENERGY_EXCHANGE_ADJUSTMENT
      REAL*8 CAP_LIMITED_DEMAND

      LOGICAL*1 ANN_CL_REPORT_NOT_OPEN/.TRUE./
      INTEGER*2 ANN_CL_UNIT_NO,ANN_CL_UNIT_HEADER,
...
C
      INCLUDE 'POOLCOM.MON'
      LOGICAL*1 LOTUS_SPREAD_SHEET/.FALSE./
C
...
!
      ...
      IF(FIRST_TIME_BY_UNIT .AND. CSV_BY_UNIT(0)) THEN
         ...
         FILE_NAME = 'BYU'//trim(SCENAME())//'.CSV'
     ...
     +                                       FORM='FORMATTED',RECL=200)
      ...
      IF(FIRST_TIME_BY_YEAR .AND. BY_YEAR) THEN
         ...
         FILE_NAME = 'BYY'//trim(SCENAME())//'.CSV'
...
C
      INCLUDE 'SpinLib.MON'
      INCLUDE 'SIZECOM.MON'
      INTEGER*2 YR,PSMO,PEMO,I
      REAL*8 DEMAND_AFTER_DSM_BEFORE_EL
...
!     ****************************************************************
      ...
      LOGICAL*1 LDC_REPORT_NOT_OPEN/.TRUE./,BEFORE
      INTEGER*2 NOPT,I,ISEAS,END_POINT,YEAR,LDC_REPORT_NO,
     +            LDC_REPORT_HEADER,SEASON
      REAL LODVAL(1000),PBIL(1000)
      CHARACTER*9 WRITE_MONTH_NAME(12),MONTH_NAME*20,
     +            BEFORE_AFTER*6
...
C************************************************************************
      ...
      CHARACTER*20 UNITNM,RETURN_UNITNM,FUEL_NAME,UNIT_NAME
      CHARACTER*9 FUEL_MONTH_NAME,MONTH_NAME*20
      INTEGER*2 RETURN_SHADOW_UNIT_NUMBER
      REAL PRIM_HEAT,SEC_HEAT,EMIS_HEAT,CUM_GAS
      REAL*8 MMBTU_FUEL_BALANCE,TEMP_HEAT
      INTEGER*2   SHADOW_UNIT_NUMBER,I,FUEL_INVEN_NO,
     +            FUEL_INVENTORY_HEADER,FUEL_USED_BY_UNIT,
     +            R_ISEAS,UNIT_BLOCK,SEGMENT_NO
      LOGICAL*1 FUEL_INVENTORY_NOT_OPEN/.TRUE./
...
!
        ...
        IF(YES_REFERENCE_CASE_REPORT()) THEN
            IF(.NOT. MarketSymFileOpen) THEN
               FILE_NAME = "MARKETSYM-"//TRIM(GET_SCENAME())//".CSV"
