...
C
      ...
      INCLUDE 'SpinLib.MON'
c      SAVE !removed 11/8/01 to increase LF95 speed
      INCLUDE 'SIZECOM.MON'
...
C
     ...
     +                                            0:FUTURE_ASSETS_FILES)
      ...
      INTEGER*2 R_NUM_OF_CLASSES,R_MAX_CLASS_NUM
C     +          R_CLASS_POINTERS(*)
      INTEGER*2 NUM_OF_OL_ASSET_CLASSES/0/,
     +          MAX_OL_CLASS_ID_NUM/0/
      INTEGER*2 NUM_OF_BC_ASSET_CLASSES/0/,
     +          MAX_BC_CLASS_ID_NUM/0/
      INTEGER*2 BC_FA_CLASS_POINTER(:),
     +          OL_FA_CLASS_POINTER(:),
     ...
     +               TEMP_ASSET_CLASS_POINTER
      ...
      INTEGER*2 R_ASSET_CLASS,
     +          R_ASSET_CLASS_VECTOR
C
      INTEGER*2 IREC,AFDC_CAP_VECTOR,UNIT_NO,
     ...
     +          SAVE_FN_RECORDS_IN_BASE_FILE/0/,G_RECORDS_IN_BASE_FILE
      ...
      INTEGER*2 R_CURRENT_YEAR,R_OPTION_TIMES_DELAYED
      INTEGER*2 DELETE,INUNIT,LRECL/1024/,REC_LEN,R_REC
      INTEGER ACCTNO
      LOGICAL*1 R_LOGICAL_1,THERE_ARE_NEW_ASSESTS,R_ABANDON_IT,
     +          FROM_ADD_NEXT_UNIT_FINANCIAL,
     +          FLEX_PLANNING_RPT_NOT_OPEN/.TRUE./
      CHARACTER (LEN=1) :: FUASTFIL,VOID_CHR
      CHARACTER*5 OVERLAY_FAMILY_NAME
      CHARACTER*30 DESC,OPTION_NAME*38,DELAYED_STR*12,TEMP_DESC
      CHARACTER*30 COMMENT
      CHARACTER*15 LEFT_JUSTIFY_I2_IN_STR
      INTEGER*2 TOTAL_UNITS_ADDED/0/
     ...
     +     TAX_BASIS_ADJUSTOR
      CHARACTER*40 LAG_STR,CASH_STR
      REAL R_PERCENT_COMPLETED
      INTEGER*2 CONSTRUCTION_YEARS_COMPLETED,R_REC_POSITION
      INTEGER*2 R_ON_LINE_YR
      LOGICAL*4 FILE_EXISTS,FILE_OPENED
      LOGICAL*1 FIRST_FINANCIAL_ADDITION/.TRUE./
      LOGICAL*1 FUTURE_ASSET_FILE_OPEN/.FALSE./
      LOGICAL*1 MODEL_ADDED_ASSET_FILE_OPEN/.FALSE./
C DECLARATION FOR DBREAD COMMON BLOCK
      CHARACTER*1024 RECLN
C DECLARATION FOR /FUTURE ASSETS FILE/
      INTEGER*2 AFDCSW,ABYEAR,ABMETH,FIRSTYR,
     +          CONSTRUCTION_ESCALATION_VECTOR,
     +          AFDCPERIODS,SERVICEMO
      REAL (KIND=4) :: CONSTRUCTION_ESCALATION_VECTOR_RATE
C ADD_FINANCIAL_RECORD VARIABLES
      INTEGER*2 OPERATION_LIFE,LEAD_TIME,
     +          INVESTMENT_DATA_POINTER,ESCALATION_VECTOR,
     +          ON_LINE_MO,R_ON_LINE_MO
      REAL CONSTRUCTION_COSTS,BOKLFX,R_CONSTRUCTION_COSTS,
     +     RR_CONSTRUCTION_COSTS
      INTEGER*2 AI_REMAINING_LIFE,SET_AI_CL_REMAINING_LIFE,
     +          SET_AI_EL_REMAINING_LIFE
C     ADDED MIDAS GOLD 7/24/91
      INTEGER*2 CASH_VECT,PLANT_VECT,PROJECTS_IN_PROCESS
      REAL ADRLIFE,CONSTRUCTION_PERIOD,PV_PLANT_COST,
     +     REGULATORY_ALLOCATOR
      REAL DBRATE,CWIP1,CWIPRB,AFUDC1,AFDCB1,BOKLF,TAXLF,
     +      ABTAX,WOYRS,OHRATE,TAXEXP,PLANT(15),CASH(15)
      CHARACTER*21 FILE_TYPE/'Future Asset Accounts'/
      CHARACTER*4 DEPMET
      CHARACTER*1 ACCOUNT_ACTIVE,ABACCT,DATA_TYPE,
     +            TAKE_BONUS_TAX_DEPRECIATION
      LOGICAL (KIND=1) :: R_FUASTOL
C
      INTEGER*2 ON_LINE_YR,IOFFSET,IEND,I,DELAY_PERIODS,YR
     ...
     +     PROJECT_ITC_RATE          !82
      ...
      ALLOCATABLE :: CEP(:),CE(:),ESCALATION_RATES(:),
     +               TEMP_HOLDING_ARRAY(:)
      LOGICAL*1 VOID_LOG,RETURN_A_VECTOR_FOR_ALL_YEARS
      INTEGER*2 PROJECTS_ACCEPTED
C
      LOGICAL*1 LAHEY_LF95
      CHARACTER*30 SCREEN_OUTPUT
      INTEGER*2 OPTIONS_UNIT_NUM
      LOGICAL*1 OPTIONS_FINANCIAL_FILE_EXISTS
      CHARACTER*1 BOOK_DEP_METHOD
      SAVE BC_FA_CLASS_POINTER,
     +     OL_FA_CLASS_POINTER
      SAVE RECORDS_IN_BASE_FILE,IREC,PROJECTS_IN_PROCESS,
     +     LAST_UNIT_NO_OPENED,RECORDS_IN_ORG_BASE_FILE,
     +     THERE_ARE_NEW_ASSESTS,FLEX_RPT_UNIT,PROJECTS_ACCEPTED,
     +     FLEX_RPT_REC
      SAVE OPTIONS_FINANCIAL_FILE_EXISTS
      CHARACTER*20 TAB_NAME
C
C WVPA DATA ITEMS
C
      INTEGER*2 PROJECT_COMPLETION_MO,PROJECT_COMPLETION_YR,
     +          WVPA_COMPANY_ID
      INTEGER WVPA_SUB_ACCOUNT_NUMBER,
     +        WVPA_BUDGET_ITEM_NUM
      CHARACTER*15 WVPA_DEPARTMENT_HEAD
      REAL OM_CAPITIALIZED_CASH,
     +     OM_CAPITIALIZED_NON_CASH
      CHARACTER*10 WVPA_WORK_ORDER_NUMBER
      CHARACTER (LEN=40) :: OVERNIGHT_CAPITAL_COST_STOCASTIC
...
C***********************************************************************
C
C CONVERT THE FUTURE-ASSET-COMMITTED FILE
      ENTRY FA_MAKEBIN
C
...
C
     ...
     +                                 TRIM(FA_FILE_BASE_NAMES(FILE_ID))
     ...
     +           ACCESS="DIRECT",STATUS="UNKNOWN",RECL=LRECL)
         ...
         DO
            ...
            CONSTRUCTION_ESCALATION_VECTOR_RATE = 0.
            ...
            WOYRS = 99
            WVPA_COMPANY_ID = 1
            OVERNIGHT_CAPITAL_COST_STOCASTIC = "Not Active"
...
c     +            ABACCT,
     ...
     +            PV_PLANT_COST,CONSTRUCTION_ESCALATION_VECTOR_RATE,
     +            PLANT,
     +            CASH,DESC,COMMENT,AFDC_CAP_VECTOR,PLANT_VECT,
     ...
     +            WVPA_DEPARTMENT_HEAD,  ! 72
     +            PROJECT_COMPLETION_MO,
     +            PROJECT_COMPLETION_YR, ! 74
     +            WVPA_WORK_ORDER_NUMBER,
     +            WVPA_BUDGET_ITEM_NUM,
     +            OM_CAPITIALIZED_CASH,   ! 77
     +            OM_CAPITIALIZED_NON_CASH, ! 78
     +            WVPA_COMPANY_ID,
     +            OVERNIGHT_CAPITAL_COST_STOCASTIC,  ! 80
     +            PERCENT_CASH_EARNS_ITC,   !81
     +            PROJECT_ITC_RATE          !82
C
     ...
     +                                   TEMP_ASSET_CLASS_POINTER)
               ...
               IREC = IREC + 1
C
C RIPPLE DOWN PLANT AND CASH VALUES
C
...
C
     ...
     +            PV_PLANT_COST,CONSTRUCTION_ESCALATION_VECTOR_RATE,
     +            PLANT,
     +            CASH,DESC,AFDC_CAP_VECTOR,PLANT_VECT,
     ...
     +            OM_CAPITIALIZED_NON_CASH,
     +            WVPA_COMPANY_ID,
     +            OVERNIGHT_CAPITAL_COST_STOCASTIC,
     +            PERCENT_CASH_EARNS_ITC,   !81
     +            PROJECT_ITC_RATE          !82
...
c      ELSE IF(INDEX(FA_FILE_BASE_NAMES(0),'NONE') == 0) THEN
c         CALL STOP_NOFILE(FILE_TYPE,FILE_NAME)
...
C***********************************************************************
C
C OVERLAY THE FUTURE-ASSET-COMMITTED FILE
C***********************************************************************
...
C***********************************************************************
     ...
     +         CONSTRUCTION_ESCALATION_VECTOR_RATE,PLANT,CASH,
     +         DESC,AFDC_CAP_VECTOR,PLANT_VECT,
     +         TXNORM,SALVAGE_VALUE,
     +         ASSET_CLASS_NUM,ASSET_CLASS_VECTOR,
     +         BOOK_DEP_METHOD,TAX_BASIS_ADJUSTOR,
     +         LAG_STR,CASH_STR,
...
C***********************************************************************
         ...
         IF(.NOT. FILE_OPENED) OPEN(UNIT_NO,FILE=FILE_NAME,
...
c            ENDFILE(LAST_UNIT_NO_OPENED,IOSTAT=IOS)
            CLOSE(LAST_UNIT_NO_OPENED)
            FUTURE_ASSET_FILE_OPEN = .FALSE.
         ENDIF
C        IREC = RECORDS_IN_BASE_FILE
C        FIRST_FINANCIAL_ADDITION = .TRUE.
      RETURN
C***********************************************************************
...
C***********************************************************************
     ...
     +                                'BC_FUAST.BIN',FORM='UNFORMATTED')
      RETURN
CC***********************************************************************
C      ENTRY RETURN_NUM_OF_FA_CLASSES(R_NUM_OF_CLASSES,
C     +                                         R_MAX_CLASS_NUM)
CC***********************************************************************
C         DO FILE_ID = 0, FUTURE_ASSETS_FILES
C         IF(FUASTOL == 'OL') THEN
C            R_NUM_OF_CLASSES = NUM_OF_OL_ASSET_CLASSES
C            R_MAX_CLASS_NUM = MAX_OL_CLASS_ID_NUM
C         ELSE
C            R_NUM_OF_CLASSES = NUM_OF_BC_ASSET_CLASSES
C            R_MAX_CLASS_NUM = MAX_BC_CLASS_ID_NUM
C         ENDIF
C         ENDDO
C         R_MAX_CLASS_NUM = MAX(MAX_BC_CLASS_ID_NUM,MAX_OL_CLASS_ID_NUM)
C      RETURN
CC***********************************************************************
C      ENTRY RETURN_FA_POINTER(R_CLASS_POINTERS)
CC***********************************************************************
C         IF(FUASTOL == 'OL') THEN
C            R_CLASS_POINTERS(1:MAX_OL_CLASS_ID_NUM) =
C     +                   OL_FA_CLASS_POINTER(1:MAX_OL_CLASS_ID_NUM)
C         ELSE
C            R_CLASS_POINTERS(1:MAX_BC_CLASS_ID_NUM) =
C     +                        BC_FA_CLASS_POINTER(1:MAX_BC_CLASS_ID_NUM)
C         ENDIF
C      RETURN
C***********************************************************************
   ...
   10    IF(FIRST_FINANCIAL_ADDITION) THEN
     ...
     +                                           G_RECORDS_IN_BASE_FILE)
            ...
            IF(OPTIONS_FINANCIAL_FILE_EXISTS) THEN
               CALL OPEN_MODEL_ADDED_ASSET_FILE(INT2(11))
...
C
C NEED TO PASS THE A&I STUFF TO THE CL OR EL OBJECT
C
         BOKLFX = BOKLF
     ...
     +                          SET_AI_EL_REMAINING_LIFE(OPERATION_LIFE)
            ELSE
               AI_REMAINING_LIFE = SET_AI_EL_REMAINING_LIFE(INT2(BOKLF))
            ENDIF
         ELSE
            IF(BOKLF < 0.) THEN
               AI_REMAINING_LIFE = 
     +                          SET_AI_CL_REMAINING_LIFE(OPERATION_LIFE)
            ELSE
               AI_REMAINING_LIFE = SET_AI_CL_REMAINING_LIFE(INT2(BOKLF))
            ENDIF
         ENDIF
         IF(BOKLF < -10000.) THEN
         ...
         ENDIF
C
C SET-UP CASH FLOW AND PLANT ENTERING SERVICE VECTORS
C
         ...
         CE = 0.
C
         FIRSTYR = MAX(R_ON_LINE_YR - LEAD_TIME,0)
     ...
     +              MAX(FIRSTYR - BASE_YEAR,1))
         IEND = MAX(0,MIN(15,MAX_FINANCIAL_SIMULATION_YEARS-IOFFSET))
C
C NEED TO HAVE A MEANS TO HAVE A PLANT UNDER CONSTRUCTION
C
         ...
         IF(INDEX(R_CAP_HYPERCUBE,"Not Active") == 0) THEN
            ...
            IF(SCENARIO_INDEX > 0) THEN
               HYPER_YR =  MAX(FIRSTYR - BASE_YEAR,1)
               R_CONSTRUCTION_COSTS = RR_CONSTRUCTION_COSTS
     ...
     +                                                     CURRENT_VALUE
            ENDIF
C
C           TEMP_CWIP  = TEMP_CWIP + CE(I)
C           CEP(I)   = MIN(CEP(I),1.) * TEMP_CWIP
C           TEMP_CWIP  = TEMP_CWIP - CEP(I)
         ENDDO
C
C FOR A REASON I DON'T REMEMBER THE AFUDC SWITCH ON NEW UNITS WAS SET TO 
C 2.  THIS IS WRONG ON 11/15/95 I CHANGED IT TO SET IT TO 2 IF THE SWITCH IS
C NOT EQUAL TO 0 MSG)
C
...
C
     ...
     +                       FLOAT(R_ON_LINE_YR - YEAR - BASE_YEAR)            
     ...
     +                                      CONSTRUCTION_YEARS_COMPLETED
                  ...
                  ENDIF
               ENDDO
               DO YR = R_ON_LINE_YR-BASE_YEAR+1, 
...
C
C ACCELERATE THE IN-SERVICE VECTOR
C
...
C
 ...
 1010 FORMAT('&',A)
      END
C  FILE FUTASSET.FOR 6/19/85
C***********************************************************************
C                                                                      *
C          SUBROUTINE TO CALCULATE FUTURE ASSET VALUE                  *
C          COPYRIGHT (C) 1983 M.S. GERBER & ASSOCIATES, INC.           *
C                                                                      *
C***********************************************************************
C
      SUBROUTINE FUASST(SAVE_BASE_CASE,PROCESSING_NEW_ADDITIONS)
C
C NOTE: 6/30/94 AFDCRT,AFDCBR,INTEREST_CAP_RATE ARE ONLY PASSED TO
C AFUDC
C CURRENT_INTEREST_CAP_RATE IS USED IN THIS ROUTINE
C
      USE IREC_ENDPOINT_CONTROL
...
C
      LOGICAL*4 SAVE_BASE_CASE,PROCESSING_NEW_ADDITIONS
      CHARACTER*15 LEFT_JUSTIFY_I2_IN_STR
      INTEGER*2 FINANCIAL_SIMULATION_YEARS,VOID_INT2,
     +          WRITE_DRILLING_RPT
      LOGICAL*1 MONTHLY_CASH_VALUES_EXIST,
...
C
     ...
     +     ABANDONMENT_FIXED_CHARGE,
     +     ABANDONMENT_CHARGE_PERCENT
      INTEGER*2 FA_RPT_UNIT/0/,FUTURE_ASSET_REPT_HEADER,MO_START
      INTEGER FA_RPT_REC
      SAVE FA_RPT_REC
      LOGICAL*1 FUTURE_ASSET_RPT_NOT_OPEN/.TRUE./,
...
C
      LOGICAL*1 MIDAS_ADDED_INVESTMENT
      INTEGER*2 START,END,LAST_REPORTING_YEAR,LAST_FA_RECORD,ON_LINE_YR,
     +          CONSTRUCTION_YEARS_COMPLETED
      CHARACTER*1 UTILITY_TYPE,DUMMY_TYPE,BOOK_DEP_METHOD,
     +            TAKE_BONUS_TAX_DEPRECIATION,TAX_BOOK_DEP_METHOD
      CHARACTER*30 DESC,RPT_TITLE(15),RECORD_TYPE*6
      CHARACTER*34 WORKORDER_NAME,TEMP_WORKORDER_NAME,INDEXING_STR*4
C      CHARACTER*40 OUTPUT_OPTION_NAME
      CHARACTER*10 PERIOD_NAME,TEMP_STR
      CHARACTER*38 OPTION_NAME
      LOGICAL*1 CAP_PLANNING_METHOD_ANNDECOMP
      INTEGER*2 I,J,IREC,IEND,DELETE,TACCTS,IOFFSET,
     ...
     +     TAX_NORMALIZATION_PEROID,SALVAGE_VALUE,TAX_BASIS_ADJUSTOR
      CHARACTER*40 LAG_STR,CASH_STR,READ_STR*50
      REAL*4 CASH_LAG_PATTERN(12),CASH_PAYMENTS(12)
     ...
     +     VECTOR_DATA_PLNT(:),
     +     VECTOR_DATA(:)
      ALLOCATABLE :: VECTOR_DATA_CASH,VECTOR_DATA_PLNT,
     +               VECTOR_DATA
      LOGICAL*1 FUTURE_ASSET_REPORT,FUTURE_ASSETS_REPORT,
     +          REPORT_ALL_ACCOUNTS,HALF_YEAR_BOOK_DEP
C ADDED 4/13/92 MIDAS GOLD FOR REAL PROPERTY VALUES
      LOGICAL*1 REAL_PROPERTY_TAX_VALUES,CAL_REAL_PROPERTY_TAX_VALUES
C
C     TYPE DECLARATION FOR /LCARRY/
      REAL WODFTX,AFDC1B,AFDC2B,AFDCDF
C TYPE DECLARATION FOR /WKARRY/
C     TAX ITEMS
...
C TYPE DECLARATION FOR /FAINPT/
      CHARACTER*1 DPBAIS,ABACCT,ACCOUNT_ACTIVE
      CHARACTER*4 DEPMET
      INTEGER*2 AFDCSW,ABYEAR,ABMETH,FIRSTYR,
     +          ASSET_CLASS_NUM,ASSET_CLASS_VECTOR
      INTEGER ACCTNO
...
C
C
      CHARACTER*2 CAPACITY_PLANNING_METHOD,TEMP_CHR
      LOGICAL*1 NEXT_DECISION_CAPACITY_PLANNING
      INTEGER*2 REPORTING_YEARS
      SAVE ASSETS_IN_BASE_FILE
      LOGICAL*1 REPORT_HEADER_OPEN/.FALSE./
      INTEGER*2 FA_REPORTING_UNIT,FUTURE_ASSET_RPT_HEADER
      INTEGER FA_REPORTING_REC
      SAVE FA_REPORTING_UNIT,FA_REPORTING_REC
C
C
C STANDARD VARIABLES FOR GETTING MONTHLY VECTORS
C
      CHARACTER*1 CASH_MONTHLY_DATA_UNITS(LAST_AVAILABLE_MONTHLY_YEAR)
      CHARACTER*1 PLANT_MONTHLY_DATA_UNITS(LAST_AVAILABLE_MONTHLY_YEAR)
      REAL*4 MONTHLY_VECTOR_DATA(12,LAST_AVAILABLE_MONTHLY_YEAR)
C     CHARACTER*4 MIDAS_LAST_MONTH(LAST_AVAILABLE_MONTHLY_YEAR)
      INTEGER*2 CASH_MONTH_ENDING(LAST_AVAILABLE_MONTHLY_YEAR),YR
      INTEGER*2 PLANT_MONTH_ENDING(LAST_AVAILABLE_MONTHLY_YEAR)
      CHARACTER*1 DATA_TYPE,VECTOR_TYPE*20,CASH_VECTOR_DATA_TYPE,
     +            PLANT_VECTOR_DATA_TYPE
      CHARACTER*1 DRILLING_REPRT_LEVEL,FINANCIAL_DRILLING,
     +            ASSET_REPORTING_LEVEL
      SAVE ASSET_REPORTING_LEVEL
      CHARACTER*40 ACCOUNT_NAME
      CHARACTER*30 DRILLING_NAME
      LOGICAL*1 ANNUAL_INFO_ACTIVE,FIRST_PASS_NEW_ADDITIONS
      SAVE FIRST_PASS_NEW_ADDITIONS
C     REAL*4 TREND_NORM
C
      REAL*4 CASH_EXPENDITURES(:,:)
      ALLOCATABLE :: CASH_EXPENDITURES
      REAL*4 BOOK_EXPEN(:,:),PLANT_2_SERVICE(:,:),
     ...
     +               NPV,
     +               TEMP_PLANT_2_SERVICE
      CHARACTER*1 RIPPLE_ZERO
      PARAMETER(RIPPLE_ZERO='Z')
      REAL*4 TEMP_SUM,PLANT_CAP
      LOGICAL*1 VOID_LOGICAL,SET_DRILLING_DATA_CASH_TRUE
      INTEGER*4 VALUES_2_ZERO
      REAL*4 PERIOD,PERIOD_AMOUNT
      LOGICAL*1 CASH_NOT_ZERO,BOOK_DEP_NOT_ZERO
      LOGICAL*1 LF95,LAHEY_LF95
      CHARACTER*6 SHORT_MONTH_NAMES
      INTEGER*2 LAST_REPORTING_MO
      CHARACTER*1 MONTHLY_DETAIL/'M'/,
     +            ANNUAL_DETAIL/'A'/
      REAL*4 CWIP_OPENING_BAL
      INTEGER*2 PROJECT_COMPLETION_MO,
     +          PROJECT_COMPLETION_YR,
     +          WVPA_COMPANY_ID
      INTEGER*4 WVPA_SUB_ACCOUNT_NUMBER,
     +          WVPA_BUDGET_ITEM_NUM
      CHARACTER*10 WVPA_WORK_ORDER_NUMBER
      REAL OM_CAPITIALIZED_CASH,
     +     OM_CAPITIALIZED_NON_CASH
      CHARACTER*15 WVPA_DEPARTMENT_HEAD
      INTEGER WVPA_WORKORDER_CONTRL_REC,
     +        WVPA_WORKORDER_EXPENSES_REC
      INTEGER*2 WVPA_WORKORDER_CONTRL_UNIT,
     +          WVPA_WORKORDER_EXPENSES_UNIT
      LOGICAL*1 WVPA_CONTROL_FILE_OPEN/.FALSE./,WVPA,
     +          WVPA_WO_EXPENSES_FILE_OPEN/.FALSE./,
     +          WVPA_WO_SUMMARY_FILE_OPEN/.FALSE./
      INTEGER*2 WVPA_WORKORDER_CONTROL_RPT,
...
C
     ...
     +                 COMPLETION_CHECK_MARK
      INTEGER (KIND=4) :: WVPA_WORKORDER_SUMMARY_REPORT_REC
      INTEGER (KIND=2) :: WVPA_WORKORDER_SUMMARY_REPORT_UNIT
...
C     FINANCIAL_SIMULATION_YEARS=MIN(MAX_FINANCIAL_SIMULATION_YEARS,
C    +                                MAX(STUDY_PERIOD+EXTENSION_PERIOD,
C    +                                              AVAIL_DATA_YEARS)+1)
      LF95 = LAHEY_LF95()
      FIRSTYR_VEC = 0
      STO_PTR = 0
      IF(LF95) THEN
...
C
C      ITC_RATE = 0.
C      IF(UTILITY_TYPE() == 'I') ITC_RATE = .10
C      PCITC = 0.
      ACCOUNTS_REPORTED = 0
C
...
C
      ...
      ALLOCATE(PERCENT_PLANT_2_SERVICE(0:12,
     +                                    0:FINANCIAL_SIMULATION_YEARS))
      ALLOCATE(MONTHLY_AFUDC_ON_CASH(0:12,0:FINANCIAL_SIMULATION_YEARS))
      ALLOCATE(MONTHLY_AFUDC_ON_PLANT(0:12,
     +                                    0:FINANCIAL_SIMULATION_YEARS))
      ALLOCATE(MONTHLY_CWIP(0:12,0:FINANCIAL_SIMULATION_YEARS))
      ALLOCATE(MONTHLY_AFUDC_IN_CWIP(0:12,0:FINANCIAL_SIMULATION_YEARS))
      ALLOCATE(MONTHLY_CWIP_IN_RATEBASE(0:12,
     +                                    0:FINANCIAL_SIMULATION_YEARS))
      ALLOCATE(MONTHLY_CAPITALIZED_INTEREST(0:12,
     +                                    0:FINANCIAL_SIMULATION_YEARS))
      ALLOCATE(MONTHLY_CURRENT_INTEREST(0:12,
     +                                    0:FINANCIAL_SIMULATION_YEARS))
      ALLOCATE(CASH_EXPENDITURES(0:12,0:FINANCIAL_SIMULATION_YEARS))
      ALLOCATE(MONTHLY_INTEREST_TO_TAX_VALUE(0:12,
     +                                    0:FINANCIAL_SIMULATION_YEARS))
      ALLOCATE(MONTHLY_TAX_VALUE_OF_ASSET(0:12,
     +                                    0:FINANCIAL_SIMULATION_YEARS))
      ALLOCATE(BONUS_DEP_MONTHLY_2001(0:12,
     +                                    0:FINANCIAL_SIMULATION_YEARS))
      ALLOCATE(MONTHLY_TAX_DEP_PREFERENCE(0:12,
     +                                    0:FINANCIAL_SIMULATION_YEARS))
      ALLOCATE(MONTHLY_TAX_DEPRECIATION_ALT(0:12,
     +                                    0:FINANCIAL_SIMULATION_YEARS))
      ...
      ALLOCATE(MONTHLY_TAX_DEPRECIATION(0:12,
     +                                    0:FINANCIAL_SIMULATION_YEARS))
      ALLOCATE(MONTHLY_DEFERRED_TAX_BASIS(0:12,
     +                                    0:FINANCIAL_SIMULATION_YEARS))
      ALLOCATE(MONTHLY_TAX_EXPENSE(0:12,0:FINANCIAL_SIMULATION_YEARS))
      ALLOCATE(MONTHLY_CURRENT_INTEREST_CAP(0:12,
     +                                    0:FINANCIAL_SIMULATION_YEARS))
...
C     +                                               FUTURE_ASSET_REPORT
     ...
     +                            FUTURE_ASSET_REPORT .AND. WVPA()) THEN
         WVPA_WORKORDER_SUMMARY_REPORT_UNIT =
     +     WVPA_WORKORDER_SUMMARY_RPT(WVPA_WORKORDER_SUMMARY_REPORT_REC)
...
C
      ...
      ELSE
         ...
         CALL INIT_FA_ARRAYS
C         CALL OPEN_FUTURE_ASSET_FILE(INT2(11))
         LAST_FA_RECORD = 32000
         AVAIL_FILES = 1
c         CALL LAST_FUTURE_ASSET_RECORD(LAST_FA_RECORD)
         TACCTS = 0
      ...
      ENDIF
C
c      IF(LAST_FA_RECORD > 0) CALL CLS(18,70,74)
      DO FILE_ID = 0, AVAIL_FILES
         IF(.NOT. PROCESSING_NEW_ADDITIONS) THEN
            CALL OPEN_FUTURE_ASSET_FILE(INT2(11),FILE_ID,FILE_EXISTS)
            IF(.NOT. FILE_EXISTS) CYCLE
         ENDIF
       IREC = 0
...
C
     ...
     +            REGULATORY_ALLOCATOR,PV_PLANT_COST,
     +            CONSTRUCTION_ESCALATION_VECTOR_RATE,PLANT,CASH,DESC,
     ...
     +            ITC_RATE
            IF(IOS /= 0) THEN
               CALL IOSTAT_MSG(IOS,ERR_MSG)
               EXIT !RECORD LOOP
            ENDIF
            IF(DELETE > 7 .OR. ACCOUNT_ACTIVE == 'N') CYCLE
C            IF(FIRSTYR_VEC > LAST_STUDY_YEAR) CYCLE ! Changed to below to pull investment in the study period
...
C
            ...
            FIRSTYR = FIRSTYR_VEC
            IOFFSET = MIN(MAX_FINANCIAL_SIMULATION_YEARS,
     +                    MAX(FIRSTYR - BASEYEAR,1))
            IF(IOFFSET >= MAX_FINANCIAL_SIMULATION_YEARS) CYCLE
            IEND = MAX(0,MIN(15,MAX_FINANCIAL_SIMULATION_YEARS-IOFFSET))
            IEND = MAX(0,MIN(30,MAX_FINANCIAL_SIMULATION_YEARS-IOFFSET))
            BOOK_EXPEN = 0.
            IF(CASH_VECT /= 0) THEN
               IVEC = ABS(CASH_VECT)
C              CALL GET_ASSET_VAR(IVEC,DUMMY_TYPE,VECTOR_DATA_CASH)
C
...
!
!               IF(FILE_ID /= 1) THEN
                  CONSTRUCTION_ESCALATION_VECTOR_RATE =
     +                              -CONSTRUCTION_ESCALATION_VECTOR_RATE
!               ENDIF
C
C MODIFY QUANTIES USING LATIN HYPERCUBE
C
               IF(INDEX(OVERNIGHT_CAPITAL_COST_STOCASTIC,
     +                                          "Not Active") == 0) THEN
                  SCENARIO_INDEX =
     +              GET_SCENARIO_INDEX(OVERNIGHT_CAPITAL_COST_STOCASTIC)
...
C END LHC MODIFICATIONS
     ...
     +                              MAX_FINANCIAL_SIMULATION_YEARS)
            ENDIF
C
C NOW HAVE ANNUAL CASH AMOUNT IS MILLIONS OF DOLLARS
C NEED TO DISTRIBUTE AMOUNT MONTHS
C
C            IF(CASH_VECT /= 0 .AND. MONTHLY_MIDAS_ACTIVE .AND. ! CHANGED FOR WVPA TO GET MONTHLY INFOR IF THERE
C     +                                   MONTHLY_CASH_VALUES_EXIST) THEN
...
C
     ...
     +                       (1:Drilling Account Name Width-4)//TEMP_STR
                     ENDIF
...
C
     ...
     +
                     WVPA_WORKORDER_EXPENSES_REC =
     +                                   WVPA_WORKORDER_EXPENSES_REC + 1
                  ENDIF
                  BOOK_EXPEN(0,YR) = TEMP_SUM
               ENDDO
 
C
C WVPA WORKORDER CONTROL REPORT
C
                  IF(WVPA() .AND. FUTURE_ASSET_REPORT) THEN
                     PROJECT_SUM = SUM(BOOK_EXPEN(0,IOFFSET+5:))
     +                             + PROJECT_SUM
                     WRITE(INDEXING_STR,'(I4)') PROJECT_COMPLETION_YR
C
C                     WRITE(TEMP_STR,'(I6)') ACCTNO
C                     DRILLING_ACCOUNT_NAME =
C     +                                     TEMP_STR//' '//WORKORDER_NAME
                     DRILLING_ACCOUNT_NAME =
...
c     +                                 (BOOK_EXPEN(MO,YR),MO=1,12)
                     WVPA_WORKORDER_CONTRL_REC =
     +                                     WVPA_WORKORDER_CONTRL_REC + 1
...
C
            ENDIF
C
C CAPITIALIZED O&M ADDERS
C
            CASH_OM_ADDER = OM_CAPITIALIZED_CASH * BOOK_EXPEN
            NON_CASH_OM_ADDER = OM_CAPITIALIZED_NON_CASH * BOOK_EXPEN
C
C SET-UP PLANT VALUES
C
            PLANT_2_SERVICE = 0.
            IF(PLANT_VECT /= 0) THEN
               IVEC = ABS(PLANT_VECT)
C              CALL GET_ASSET_VAR(IVEC,DUMMY_TYPE,VECTOR_DATA_PLNT)
C
C RULE IF DATA_TYPE IS NOT D THEN MONTHLY PES PATTERNS ARE NOT ALLOWED
C
               CALL GET_MONTHLY_ANNUAL_VALUES(IVEC,
     +                                        PLANT_VECTOR_DATA_TYPE,
     +                                        VECTOR_TYPE,
     ...
     +                                        PLANT_MONTH_ENDING)
               CALL IS_THERE_MONTHLY_INFORMATION(
     +                                   MONTHLY_PLANT_IN_SERVICE_EXIST)
C               IF(MONTHLY_MIDAS_ACTIVE .AND.  ! WVPA CHANGE
C     +                              MONTHLY_PLANT_IN_SERVICE_EXIST) THEN ! .AND. DATA_TYPE == 'D') THEN
               IF(MONTHLY_MIDAS_ACTIVE .OR.
     +                              MONTHLY_PLANT_IN_SERVICE_EXIST) THEN ! .AND. DATA_TYPE == 'D') THEN
     ...
     +                                       SERVICEMO)
C
C                 CALL MONTHLY_BOOK_VALUES_IN_DOLLARS(VECTOR_DATA,
C    +                                              MONTHLY_VECTOR_DATA,
C    +                                              MONTHLY_DATA_UNITS,
C    +                                              MONTH_ENDING)
               ELSE
     ...
     +                                   IOFFSET,
     +                                   FINANCIAL_SIMULATION_YEARS)
C                 CALL TREND_ANNUAL_INPUT(PLANT_2_SERVICE,
C    +                                    VECTOR_DATA,
C    +                                    IOFFSET,
C    +                                    FINANCIAL_SIMULATION_YEARS)
               ENDIF
            ELSEIF(WVPA()) THEN
               DATA_TYPE = 'R'
               MO_START = SERVICEMO
...
C
     ...
     +                              MIN(PLANT_2_SERVICE(0,YR),TEMP_CWIP)
     ...
     +                                  MAX_FINANCIAL_SIMULATION_YEARS)
            ...
            ELSEIF(.NOT. WVPA()) THEN 
               DO YR = 1, FINANCIAL_SIMULATION_YEARS 
                  PLANT_2_SERVICE(MIN(12,SERVICEMO),YR) = 
C     +                                          TEMP_PLANT_2_SERVICE(YR)
C                  PLANT_2_SERVICE(0,YR) = TEMP_PLANT_2_SERVICE(YR)
     +                                             PLANT_2_SERVICE(0,YR)
               ENDDO
            ENDIF
C
C MOVE CASH TO MONTHLY VIEW
C
C            IF(CASH_VECT /= 0 .AND. MONTHLY_MIDAS_ACTIVE .AND.   ! CHANGED FOR WVPA TO CONTINUE TO USE MONTHLY DATA
C     +                                   MONTHLY_CASH_VALUES_EXIST) THEN
            IF(CASH_VECT /= 0 .AND. (MONTHLY_MIDAS_ACTIVE .OR.
     +                                  MONTHLY_CASH_VALUES_EXIST)) THEN
               IF(WVPA()) THEN
...
c                             BOOK_EXPEN(1,YR) = PLANT_2_SERVICE(0,YR) -
c    +                                                         TEMP_CWIP
                              PERIOD = 12.
     ...
     +                                          TEMP_PLANT_2_SERVICE(YR)
                  ...
                  ENDDO
               ELSE
c                 WRITE(6,*) 'Plant Capitalization Vector without Cash'
c                 WRITE(6,*) 'Vector is not supported in Monthly MIDAS'
c                 WRITE(6,*) 'Correct account ',OPTION_NAME
c                 STOP 'Future Assets'
               ENDIF
            ENDIF
C
c            IF(PLANT_VECT /= 0 .AND. MONTHLY_MIDAS_ACTIVE .AND. 
c     +                              MONTHLY_PLANT_IN_SERVICE_EXIST) THEN ! .AND. DATA_TYPE == 'D') THEN
c               CALL TREND_MONTHLY_INPUT(PLANT_2_SERVICE,
c     +                                  PLANT_MONTHLY_DATA_UNITS,
c     +                                  PLANT_MONTH_ENDING,
c     +                                  IOFFSET,
c     +                                  MAX_FINANCIAL_SIMULATION_YEARS)
c            ELSE
c               DO YR = 1, FINANCIAL_SIMULATION_YEARS 
c                  PLANT_2_SERVICE(MIN(12,SERVICEMO),YR) = 
c     +                                             PLANT_2_SERVICE(0,YR)
c               ENDDO
c            ENDIF
C
C MOVED FROM ABOVE 9/30/99
C
...
C                        PLANT_2_SERVICE(0,YR) = PLANT_2_SERVICE(0,YR)
C     +                                          + PLANT_2_SERVICE(MO,YR)
                     ENDDO
     ...
     +                                                MAX(TEMP_CWIP,0.))
                     ENDIF
                     IF(ABS(PLANT_2_SERVICE(0,YR)) < .00001)
     ...
     +                                             PLANT_2_SERVICE(0,YR)
            ...
            ENDIF
C
C PASS MONTHLY BACK TO OLD ANNUAL VARIABLES
C
...
C
     ...
     +                         PERCENT_PLANT_2_SERVICE)
C
C CALCULATE BOOK DEPRECIATION FOR AFDC
C
            IF(MONTHLY_MIDAS_ACTIVE) THEN
     ...
     +                        HALF_YEAR_BOOK_DEP)
C
C RECOMPUTE BOOK DEPRECIATION THAT INCLUDES SALVAGE VALUE
C
...
C
     ...
     +                       CEP,DEPMET,FINANCIAL_SIMULATION_YEARS)
            ...
            ELSE
C
C CALCULATE TAX VALUE AND ITCs
C
     ...
     +               MONTHLY_CURRENT_INTEREST_CAP)
C
C CALCULATE TAX DEPRECIATION ON PLANT
C
     ...
     +                     BONUS_DEP_MONTHLY_2001)
            ENDIF
C
C CALCULATE TAX ITEMS FOR 1986 TAX REFORM ACT
C
            IF(DEPMET /= 'ACRS') THEN
C
C CALCULATE TAX DEPRECIATION PREFERANCE ON PLANT
C
     ...
     +                     BONUS_DEP_MONTHLY_2001)
            ...
            ENDIF
C
C CALCULATE ACE_BOOK_DEP FOR THE AMT TAX CALCULATION
C
     ...
     +                   MONTHLY_ACE_BOOK_DEP)
C
C CALCULATE BOOK DEPRECIATION FOR DEFFERED TAXES
C
     ...
     +                           HALF_YEAR_BOOK_DEP)
               ...
               ENDDO
C
C CALCULATE THE BASIS FOR DEFFERED TAXES
C
...
C     +                               - MONTHLY_CURRENT_INTEREST(MO,YR)
     +                          - MONTHLY_CAPITALIZED_INTEREST(MO,YR)
     +                            * (1.-CURRENT_INTEREST_CAP_RATE(YR+1))
                    
                     ENDDO
                  ENDDO
c                  DO J = 1, FINANCIAL_SIMULATION_YEARS
c                     BKDPTX(J) = MONTHLY_TAX_BOOK_DEP(0,J-1)
c                  ENDDO
               ENDIF
            ENDIF
C
C CALCULATE REAL PROPERTY TAX VALUE IF BEING USED
C
c         IF(CAL_REAL_PROPERTY_TAX_VALUES) THEN !removed 2/16/01 by MSG because the real property tax
c switch isn't available at this point. It's choosen in asset_class_analysis.
            CALL REAL_PROPERTY_VALUES(SERVICEMO,PROPERTY_ESCALATION,
     +                                       FINANCIAL_SIMULATION_YEARS)
c         ENDIF
C
C CALCULATE THE CHANGES FROM AN ABANDONMENT
C
            IF(ABMETH > 0) THEN
     ...
     +                                ON_LINE_YR == YEAR+BASE_YEAR) THEN
                  ...
                  RECORD_TYPE = 'Commit'
                  IF(ABYEAR == YEAR+BASE_YEAR .OR. 
     +              ON_LINE_YR == YEAR+BASE_YEAR) RECORD_TYPE = 'Finish'
                  END = 31
C                 END = STUDY_PERIOD+1
...
C
C ADD DRILLING INFORMATION
C
c     +                 BOOK_EXPEN,
c     +                 PLANT_2_SERVICE,
c     +                 MONTHLY_AFUDC_ON_PLANT,
c     +                 MONTHLY_CWIP_IN_RATEBASE)
     ...
     +                                    MONTHLY_CWIP(I,YR)
                     ...
                     BOOK_EXPEN(I,YR) = REGULATORY_ALLOCATOR *
     +                                  BOOK_EXPEN(I,YR)
                  ENDDO
...
C
                           WRITE(WVPA_WORKORDER_SUMMARY_REPORT_UNIT,
     +                         REC = WVPA_WORKORDER_SUMMARY_REPORT_REC)
     +                                  PRT_ENDPOINT(),
     +                                  FLOAT(BASE_YEAR+YR),
     +                                  WVPA_DEPARTMENT_HEAD,
     +                                  DRILLING_ACCOUNT_NAME, ! WORKORDER_NAME//INDEXING_STR,
...
C
                           WVPA_WORKORDER_SUMMARY_REPORT_REC = 1 +
     +                                 WVPA_WORKORDER_SUMMARY_REPORT_REC
c                           EXPENDITURES_ProjToDate = BOOK_EXPEN(MO,YR)
c     +                                         + EXPENDITURES_ProjToDate
c                           YearToDate = YearToDate
c     +                                  + BOOK_EXPEN(MO,YR)
                         ENDDO
                      ENDDO
                  ENDIF
     ...
     +                                         CASH_EXPENDITURES,
     +                                         MONTHLY_CWIP)
                  FIRSTYR = MAX(0,FIRSTYR-BASE_YEAR)
                  PROJECT_COMPLETION_YR = MAX(0,
     +                                  PROJECT_COMPLETION_YR-BASE_YEAR)
...
C                 VOID_LOGICAL = SET_DRILLING_DATA_CASH_TRUE()
...
C
     ...
     +                               FLOAT(YR+BASE_YEAR),
     +                               'AnRslt/EOY',
     +                               REGULATORY_ALLOCATOR * CE(I),
     +                               REGULATORY_ALLOCATOR * AFDC1(I),
     +                               REGULATORY_ALLOCATOR * CWIP(I),
     ...
     +                                   + AFDC2(I)),                    
     +                               REGULATORY_ALLOCATOR * 
     +                                        CEP(I)*SALVAGE_VALUE/100., 
     +                               GPV(12,YR),                         ! 18
     +                               CUM_BOOK_DEP(12,YR),
     +                               GPV(12,YR) - CUM_BOOK_DEP(12,YR),  ! 20 NPV(MO,YR)
     +                               CASH_EXPENDITURES(0,YR),                  
     +                               MONTHLY_CWIP(12,YR),
     +                               REGULATORY_ALLOCATOR*
     +                                     BONUS_DEP_MONTHLY_2001(0,YR) ! 23
...
C
     ...
     +                                      + MONTHLY_CWIP(MO,YR),
     +                        REGULATORY_ALLOCATOR
     +                              * MONTHLY_TAX_VALUE_OF_ASSET(MO,YR), 
     +                        PLANT_2_SERVICE(MO,YR) 
     +                                         * (1.-SALVAGE_VALUE/100.)
...
C
     ...
     +                                   + AFDC2(I)),                      
     +                               REGULATORY_ALLOCATOR * 
     +                                        CEP(I)*SALVAGE_VALUE/100., ! SALVAGE VALUE
...
C
C     WRITE THE CUMULATED RESULTS TO THE ASSET FILE
C
     ...
     +                                 TACCTS,'-Future Asset Accounts'
         ENDIF
         CALL MG_LOCATE_WRITE(0,0,TRIM(SCREEN_MESSAGES),
     +                                                   ALL_VERSIONS,1)
      ENDIF
      IF(PROCESSING_NEW_ADDITIONS) THEN
         FUTURE_ASSET_REPORT = FUTURE_ASSET_REPORT .AND.
     +                                            MIDAS_ADDED_INVESTMENT
         IF(MIDAS_ADDED_INVESTMENT) THEN
...
C
      ...
      INCLUDE 'GLOBECOM.MON'
C
      INTEGER*2 R_YR,R_CLASS,MO
      LOGICAL*1 R_CLASS_EXISTS,LGandE
      REAL*4 ALLOCATION_VALUE(0:AVAIL_DATA_YEARS)
...
C
     ...
     +               CLASS_AFUDC_IN_CWIP
C
      LOGICAL*1 FUTURE_ASSET_REPORT
      CHARACTER*40 OUT_OPTION_NAME
      INTEGER*2 FA_REPORTING_UNIT,DEACTIVE_YR,ASSET_CLASS_ID,MO_DEACT
      INTEGER FA_REPORTING_REC
      REAL*4 CURRENT_INTEREST_CAP_RATE(*)
      LOGICAL*1 VOID_LOGICAL,RETURN_ASSET_CLASS_LISTS
      INTEGER*2 ALLOCATION_VECTOR,J
      CHARACTER*6 SHORT_MONTH_NAMES 
      LOGICAL*1 ANNUAL_INFO_ACTIVE
c      SAVE MAX_ASSET_CLASS_NUM
c      SAVE ASSET_CLASS_POINTER
c      SAVE FINANCIAL_SIMULATION_YEARS
c      SAVE TCE,TCEP,TTAXDP,TBOKDP,TAFCDP,
c     +     TAFDC1,TAFDC2,TCWIP,TRBCWP,TITCDP,
c     +     TITCDF,TTXEXP,TTXDEF,TDDB,TRBDDB,
c     +     TAMRTE,TEXEXP,TAFDPA,TCEPTX,TAFCAJ,
c     +     TBKDPT,TITCPY,TAFEXP,TBKDPA,
c     +     TPCAPINRST,TTXPREFDEP,TOTAL_ACE_BOOK_DEP,
c     +     TAFDCB,TWODFT,TAFDCF,RB_NPV,
c     +     RB_TAXDP,RB_BOKDP,RB_PCAPINRST,
c     +     RBCWIP_AFDC_METH2,RB_TXEXP,RB_TXDEF,
c     +     RB_TXPREFDEP,RB_AMRTE,RB_ITCPY,RB_ITC,
c     +     RB_AFDC1,PROPERTY_TAX_GPV,PROPERTY_TAX_NPV,
c     +     CLASS_GPV,CLASS_CUMULATIVE_DEPRECIATION,
c     +     CLASS_AFUDC_IN_CWIP
C      
...
C         IF(ALLOCATED(ASSET_CLASS_POINTER))
C     +                                   DEALLOCATE(ASSET_CLASS_POINTER)
C         IF(MAX_ASSET_CLASS_NUM > 0) THEN
C            ALLOCATE(ASSET_CLASS_POINTER(MAX_ASSET_CLASS_NUM))
C            CALL RETURN_FA_POINTER(ASSET_CLASS_POINTER)
C         ENDIF
C
...
C
     ...
     +                                             0:NUM_OF_FA_CLASSES))
         ALLOCATE(TCWIP(FINANCIAL_SIMULATION_YEARS,
     +                                             0:NUM_OF_FA_CLASSES))
         ALLOCATE(TRBCWP(FINANCIAL_SIMULATION_YEARS,
     +                                             0:NUM_OF_FA_CLASSES))
         ALLOCATE(TITCDP(FINANCIAL_SIMULATION_YEARS,
     +                                             0:NUM_OF_FA_CLASSES))
         ALLOCATE(TITCDF(FINANCIAL_SIMULATION_YEARS,
     +                                             0:NUM_OF_FA_CLASSES))
         ALLOCATE(TTXEXP(FINANCIAL_SIMULATION_YEARS,
     +                                             0:NUM_OF_FA_CLASSES))
         ALLOCATE(TTXDEF(FINANCIAL_SIMULATION_YEARS,
     +                                             0:NUM_OF_FA_CLASSES))
         ALLOCATE(TDDB(FINANCIAL_SIMULATION_YEARS,
     +                                             0:NUM_OF_FA_CLASSES))
         ALLOCATE(TRBDDB(FINANCIAL_SIMULATION_YEARS,
     +                                             0:NUM_OF_FA_CLASSES))
         ALLOCATE(TAMRTE(FINANCIAL_SIMULATION_YEARS,
     +                                             0:NUM_OF_FA_CLASSES))
         ALLOCATE(TEXEXP(FINANCIAL_SIMULATION_YEARS,
     +                                             0:NUM_OF_FA_CLASSES))
         ALLOCATE(TAFDPA(FINANCIAL_SIMULATION_YEARS,
     +                                             0:NUM_OF_FA_CLASSES))
         ALLOCATE(TCEPTX(FINANCIAL_SIMULATION_YEARS,
     +                                             0:NUM_OF_FA_CLASSES))
         ALLOCATE(TAFCAJ(FINANCIAL_SIMULATION_YEARS,
     +                                             0:NUM_OF_FA_CLASSES))
         ALLOCATE(TBKDPT(FINANCIAL_SIMULATION_YEARS,
     +                                             0:NUM_OF_FA_CLASSES))
         ALLOCATE(TITCPY(FINANCIAL_SIMULATION_YEARS,
     +                                             0:NUM_OF_FA_CLASSES))
         ALLOCATE(TAFEXP(FINANCIAL_SIMULATION_YEARS,
     +                                             0:NUM_OF_FA_CLASSES))
         ALLOCATE(TBKDPA(FINANCIAL_SIMULATION_YEARS,
     +                                             0:NUM_OF_FA_CLASSES))
         ALLOCATE(TPCAPINRST(FINANCIAL_SIMULATION_YEARS,
     +                                             0:NUM_OF_FA_CLASSES))
         ALLOCATE(TTXPREFDEP(FINANCIAL_SIMULATION_YEARS,
     +                                             0:NUM_OF_FA_CLASSES))
         ALLOCATE(TOTAL_ACE_BOOK_DEP(FINANCIAL_SIMULATION_YEARS,
     +                                             0:NUM_OF_FA_CLASSES))
         ALLOCATE(TAFDCB(FINANCIAL_SIMULATION_YEARS,
     +                                             0:NUM_OF_FA_CLASSES))
         ALLOCATE(TWODFT(FINANCIAL_SIMULATION_YEARS,
     +                                             0:NUM_OF_FA_CLASSES))
         ALLOCATE(TAFDCF(FINANCIAL_SIMULATION_YEARS,
     +                                             0:NUM_OF_FA_CLASSES))
...
C
            IF(REGULATORY_ALLOCATOR /= 1.) THEN
               DO I = 0, AVAIL_DATA_YEARS
                  ALLOCATION_VALUE(I) = REGULATORY_ALLOCATOR *
     +                                               ALLOCATION_VALUE(I)
               ENDDO
            ENDIF
            IF(ABMETH > 0) THEN
               I = MAX(2,ABYEAR - BASE_YEAR + 1)
               J = MIN(I-1,AVAIL_DATA_YEARS)
               TTXEXP(I,CLASS) = TTXEXP(I,CLASS) + TAXWO *
     +                                          ALLOCATION_VALUE(J)/100.
            ENDIF
C           CLASS_GPV(1,CLASS) = 0.
            GPV_ADDITIONS = 0.
            DEP_ADDITIONS = 0.
            CUM_DD_AMORT = 0.
...
c     +                         -CLASS_CUMULATIVE_DEPRECIATION(I-1,CLASS)
                  CLASS_AFUDC_IN_CWIP(I,CLASS) = 0.
c     +                                   -CLASS_AFUDC_IN_CWIP(I-1,CLASS)
                  TCWIP(I,CLASS)  = 0. ! -TCWIP(I-1,CLASS)
                  TRBCWP(I,CLASS) = 0. ! -TRBCWP(I-1,CLASS)
                  EXIT
               ENDIF
               IF(I > 1) THEN
                  IF(LGandE()) THEN
     ...
     +                               + (CEP(I) + AFDC2(I))
                     CLASS_GPV(I,CLASS) = CLASS_GPV(I,CLASS)
     +                                    + ALLOCATOR * GPV_ADDITIONS
C     +                              (CLASS_GPV(I,CLASS) + GPV_ADDITIONS)
                     DEP_ADDITIONS = DEP_ADDITIONS 
     +                               + BOKDP(I) + AFCDP(I)
     ...
     +                                ALLOCATOR=ALLOCATION_VALUE(I)/100.
            ENDDO
...
C              
     ...
     +               (CWIP(I) + AFUDC_IN_CWIP(I)) * REGULATORY_ALLOCATOR
               RB_TAXDP(I) = RB_TAXDP(I) + TAXDP(I)  *
     +                                              REGULATORY_ALLOCATOR
               RB_AFDC1(I) = RB_AFDC1(I) + AFDC1(I)  *
     +                                              REGULATORY_ALLOCATOR
     ...
     +                                              REGULATORY_ALLOCATOR
               IF(ABMETH > 0) THEN
                  RB_AMRTE(I) = RB_AMRTE(I) + AMORTE(I) *
     +                                              REGULATORY_ALLOCATOR
                  RB_ITCPY(I) = RB_ITCPY(I) + ITCPAY(I) *
     +                                              REGULATORY_ALLOCATOR
               ENDIF
            ENDDO
         ENDIF
...
C 
     ...
     +      PROPERTY_TAX_GPV(I,0),PROPERTY_TAX_NPV(I,0)
      ...
      IF(SAVE_BASE_CASE) THEN
         CALL OPEN_FA_BASE_CASE_FILE(INT2(10))
...
C***********************************************************************
     ...
     +                        FA_CASH,FA_CAPITIALIZED,FA_TAX_DEP,
     +                        FA_BOOK_DEP,FA_AFDC_CASH,
...
C***********************************************************************
C*                                                                     *
C*                      REAL_PROPERTY_VALUES                           *
C*                                                                     *
C*          COPYRIGHT (C) 1992 M.S. GERBER & ASSOCIATES, INC.          *
C*                       ALL RIGHTS RESERVED                           *
C*                                                                     *
C***********************************************************************
C*                                                                     *
C*    PURPOSE:                                                         *
C*       CALCULATES THE ESCALATING GROSS PLANT AND NPV OF NEW PLANT    *
C*                                                                     *
C***********************************************************************
C
...
C
     ...
     +     NPV,SL_DEP,DEP,GPV,ESCALATOR
      INTEGER*2 SERVICEMO,FINANCIAL_SIMULATION_YEARS
      INTEGER*2 J,LAST_YEAR
      INTEGER*2 I,ABMETH,ABYEAR
      CHARACTER*1 ABACCT
      REAL*4 AFUDC_IN_CWIP(*)
      REAL NRTXWO,BOKBL,BOKWO,BOKDAL,AFCBL,AFCWO,AFCDAL
      REAL PCAPINRST(*),TXPREFDEP(*),ACE_BOOK_DEP(*),ABTAX
      REAL CCE,CCEP,CCEPTX,CTAXDP,CTXEXP,CITCDP,CITCDF,CBKDPT,CBOKDP,
     +     CAFDC1,CAFDC2,CAFDCF,CAFCDP,CRBCWP,TEMP1,TEMP2,TEMP3
      REAL TAXWO
C     TYPE DECLARATION FOR /WKARRY/
C        TAX ITEMS
           REAL TXDEF,TAXDP,CEPTX,ITCDF,ITCDP,TAXEXP,TXDEFC,ITCPAY
...
C
C
C     THEN ABANDONMENT ITEMS ARE COMPUTED
C
      ...
      ENDDO
C
C     ADJUSTMENTS ARE MADE TO THE PLANT PARAMETERS
C     TO REFLECT THE WRITE-OFF AMOUNTS
C
      CE(LAST_YEAR)     = TEMP1
      CEP(LAST_YEAR)    = -CCEP
...
C
C    WRITE OFF EFFECTS ARE COMPUTED IN NEXT SEVEN LINES:
C
C     TOTAL TAX DEDUCTION OF THE WO
      ...
      ENDIF
C     AMOUNT OF TAX DEDUCTION THAT GOES TO RATE PAYERS
C     NRTXWO = TAXWO * ABTAX
      NRTXWO = TAXWO * ABTAX
C     TOTAL BOOK WO OF BRICKS AND MORTAR
      BOKBL  = (CCE + CE(LAST_YEAR) + CWIP(1) - CBOKDP)
C     BOOK WO BELOW THE LINE
      BOKDAL = BOKBL * (1.-ABTAX)
C     BOOK WO ABOVE THE LINE
      BOKWO  = BOKBL - BOKDAL
C     TOTAL AFUDC WO
      AFCBL  = (CAFDC1 + AFDC1(LAST_YEAR) - CAFCDP)
C     AFUDC WO BELOW THE LINE
      AFCDAL = AFCBL * (1.-ABTAX)
C     AFUDC WO ABOVE THE LINE
      AFCWO  = AFCBL - AFCDAL
      RETURN
      END
C***********************************************************************
...
C*                        T A X V A L U E                              *
C*                                                                     *
C*          COPYRIGHT (C) 1986 M.S. GERBER & ASSOCIATES, INC.          *
C*                       ALL RIGHTS RESERVED                           *
C*                                                                     *
C***********************************************************************
C*                                                                     *
C*    PURPOSE:                                                         *
C*       TAXVALUE CALCULATES THE TAX VALUE OF THE ASSET.               *
C*                                                                     *
C**********************************************************************
C
...
C     INTERNAL VARIABLES
      INTEGER*2 ENDYEAR,STARTYR,FINANCIAL_SIMULATION_YEARS
      INTEGER*2 YR,MO
...
C     DECLARATION FOR WORLD VARIABLES FOR FUTURE ASSETS
      INTEGER*2 BASEYEAR,BASE_YEAR
C      
      REAL*4 BOOK_EXPEN(0:12,0:FINANCIAL_SIMULATION_YEARS),
     +       PLANT_2_SERVICE(0:12,0:FINANCIAL_SIMULATION_YEARS),
     +       MONTHLY_TAX_VALUE_OF_ASSET(0:12,
     +                                    0:FINANCIAL_SIMULATION_YEARS),
     +       MONTHLY_TAX_EXPENSE(0:12,0:FINANCIAL_SIMULATION_YEARS),
     +       MONTHLY_CWIP(0:12,0:FINANCIAL_SIMULATION_YEARS),
     +       MONTHLY_INTEREST_TO_TAX_VALUE(0:12,
     +                                    0:FINANCIAL_SIMULATION_YEARS),
     +       MONTHLY_CURRENT_INTEREST_CAP(0:12,
     +                                    0:FINANCIAL_SIMULATION_YEARS)
C
      BASEYEAR = BASE_YEAR()
      TAXBAL = TAXEXP(1)
...
C               MONTHLY_ITC_EARNED(MO,YR-1) = RTITC *
C     +                              (BOOK_EXPEN(MO,YR-1)
C     +                                   - MONTHLY_TAX_EXPENSE(MO,YR-1))
            ENDDO
...
C
     ...
     +                       CEPTX(YR)/CEP(YR)* PLANT_2_SERVICE(MO,YR-1)
      ...
      DO YR = STARTYR, FINANCIAL_SIMULATION_YEARS
         ...
         IF(CWIP(YR-1) + CE(YR) /= 0. .AND. CEP(YR) /= 0.) THEN
            TAXAMT = CEP(YR)/(CWIP(YR-1) + CE(YR)) * TAXBAL
            CEPTX(YR)  = TAX_BASIS_ADJUSTOR * (CEP(YR) -  TAXAMT)
     +                   + CAPINRST(YR)
            CEPTXBOK(YR) = TAX_BASIS_ADJUSTOR * CEP(YR)
     +                     + CURRENT_INTEREST_CAP(YR)
C
            TAXBAL    = TAXBAL - TAXAMT
            DO MO = 0, 12 
               IF(PLANT_2_SERVICE(MO,YR-1) /= 0.) THEN
                  MONTHLY_TAX_VALUE_OF_ASSET(MO,YR-1)=CEPTX(YR)/CEP(YR)
     +                                        * PLANT_2_SERVICE(MO,YR-1)
               ENDIF
            ENDDO
         ENDIF
      ENDDO
...
C     INTERNAL VARIABLES
      INCLUDE 'SpinLib.MON'
      LOGICAL*1 NO_SWITCH,DUMP_AT_TAX_LIFE
      INTEGER*2 YREND,SUMYD,J,L,LIFECK,LIFE,IYEAR,IYR,M
      INTEGER*2 FINANCIAL_SIMULATION_YEARS
...
C     EXTERNAL VARIABLES
     ...
     +     TAX_VALUE_BEGINNING_90(FINANCIAL_SIMULATION_YEARS)
      REAL DEP_AMOUNT
      INTEGER*2 BASEYEAR,BASE_YEAR,IN_SERVICE_MONTH,MO
      CHARACTER*1 TAKE_BONUS_TAX_DEPRECIATION
      REAL*4 R_MONTHLY_TAX_VALUE_OF_ASSET(0:12,
     +                                    0:FINANCIAL_SIMULATION_YEARS),
     +       MONTHLY_TAX_DEPRECIATION(0:12,
     +                                     0:FINANCIAL_SIMULATION_YEARS)
      REAL*4 MONTHLY_TAX_VALUE_OF_ASSET(0:12,0:101),BONUS_DEP_2001
      REAL*4 BONUS_DEP_MONTHLY_2001(0:12,0:FINANCIAL_SIMULATION_YEARS)
C
      DATA DEPPCT/.25,.38,.37,17*0.,
     +            .15,.22,3*.21,15*0.,
     +            .08,.14,.12,3*.10,4*.09,10*0.,
     +            .05,.10,.09,.08,2*.07,9*.06,5*0.,
     +            20*0.,
     +            20*0./
C
      BASEYEAR = BASE_YEAR()
...
C
            ...
            IF(IYEAR > 2012) EXIT
            IF(IYEAR < 2001) CYCLE
C            IF(IYEAR == 2001 .AND. IN_SERVICE_MONTH < 9) CYCLE
...
C
     ...
     +                                 INDEX(DEPMET,'MACR') /= 0) THEN
         ...
         IF(INDEX(DEPMET,'MACR') /= 0) THEN
            ...
            DBRATE = 1.5
         ENDIF
         NO_SWITCH = INDEX(DEPMET,'DBNS') /= 0 .OR. 
     +                                         INDEX(DEPMET,'DBND') /= 0
...
C 120 IF(DEPMET == SYD) THEN
     ...
     +                                       MONTHLY_TAX_DEPRECIATION)
      ...
      ENDIF
C
C DISTRIBUTE THE ANNUAL TAX DEP ACROSS MONTHLYS
C      
      IF(TAKE_BONUS_TAX_DEPRECIATION == 'Y') THEN
         DO J = 2, FINANCIAL_SIMULATION_YEARS
C
...
C*                                                                     *
C*    PURPOSE:                                                         *
C*       DEFTAX CALCULATES THE DEFFERRED TAX ITEMS                     *
C*                                                                     *
C***********************************************************************
C
...
C
      INCLUDE 'SpinLib.MON'
      INCLUDE 'SIZECOM.MON'
      INTEGER*2 I,FINANCIAL_SIMULATION_YEARS
C     EXTERNAL VARIABLES
      REAL TAXDP(FINANCIAL_SIMULATION_YEARS),
...
C*                       ALL RIGHTS RESERVED                           *
C*  TAKEN FROM FIN ON JUNE 5, 1991 BY M. GERBER FOR USE IN MIDAS GOLD  *
C***********************************************************************
C                                                                      *
C     PURPOSE:                                                         *
C        CALCULATES THE ACE DEPRECIATION FOR FUTURE ASSETS             *
C                                                                      *
C***********************************************************************
C
...
C
      ...
      INCLUDE 'SIZECOM.MON'
C
      INTEGER*2 J,L,FINANCIAL_SIMULATION_YEARS
...
C*                                                                     *
C*    PURPOSE:                                                         *
C*       CALCULATES THE ANNAUL TAX VALUE OF THE ASSET AND CANADIAN     *
C*       CAPITAL COST ALLOCATION, CCA                                  *
C*                                                                     *
C***********************************************************************
C
...
C     INTERNAL VARIABLES
      CHARACTER*4 DEPMET
      INTEGER*2 FIRSTYR,FINANCIAL_SIMULATION_YEARS
      INTEGER*2 I
...
C
      ...
      DOWHILE ((CEP(I) == 0.) .AND. (I <= FINANCIAL_SIMULATION_YEARS))
     ...
     +                                PCAPINRST(I) - TOTAL_ROLLING_START
         I = I + 1
         DOWHILE (I <= FINANCIAL_SIMULATION_YEARS)
...
C
     ...
     +                                 INDEX(DEPMET,'CCAP') /= 0) THEN
         ...
         RS_TEST = 0.
         DOWHILE (CEP(I) == 0. .AND. I <= FINANCIAL_SIMULATION_YEARS)
            IF(BASE_YEAR + I - 1 - FIRSTYR >= 2) RS_TEST = RS_TEST +
     +             CE_TAX(I) + PCAPINRST(I) - ROLLING_START(I)
            IF(BASE_YEAR + I - 1 - FIRSTYR > 3) THEN
               LONG_TERM_PROJECT(I) = MAX(0., MIN(RS_TEST,
     +                                LTP_TEST-LONG_TERM_PROJECT(I-1)))
            ELSE
               LONG_TERM_PROJECT(I) = ROLLING_START(I)
            ENDIF
            TOTAL_LONG_TERM_PROJECT = TOTAL_LONG_TERM_PROJECT +
     +                                              LONG_TERM_PROJECT(I)
            IF(BASE_YEAR + I - 1 >= FIRSTYR) LTP_TEST = LTP_TEST +
...
C*                                                                     *
C*    PURPOSE:                                                         *
C*       CALCULATES THE TAC VALUE AND THE TAX DEP FOR CANADIAN         *
C*       FAST TAX RATE 34                                              *
C*                                                                     *
C***********************************************************************
C
...
C
      INCLUDE 'SpinLib.MON'
C      INCLUDE 'SIZECOM.MON'
C     INTERNAL VARIABLES
      INTEGER*2 J,L,IYR,FINANCIAL_SIMULATION_YEARS
...
C
      ...
      DO J = 2, FINANCIAL_SIMULATION_YEARS
         ...
         IF(CWIP(J-1) + CE(J) /= 0. .AND. CEP(J) /= 0.) THEN
            ...
            TAXBAL    = TAXBAL - TAXAMT
            NPVFT = CEPTX(J)
            DO L = J, MIN(J+2,FINANCIAL_SIMULATION_YEARS)
               IYR = L - J + 1
...
C*                       ALL RIGHTS RESERVED                           *
C*                                                                     *
C*                       MIDAS GOLD ADDITIONS                          *
C*          COPYRIGHT (C) 1991   M.S. GERBER & ASSOCIATES, INC         *
C*                       ALL RIGHTS RESERVED                           *
C***********************************************************************
C                                                                      *
C     PURPOSE:                                                         *
C        DEPBOK CALCULATES THE BOOK DEPRECIATION FOR FUTURE            *
...
C
      ...
      INCLUDE 'GLOBECOM.MON'
C
      LOGICAL*1 HALF_YEAR_BOOK_DEP
      INTEGER*2 FINANCIAL_SIMULATION_YEARS
      CHARACTER*1 DATA_TYPE
      INTEGER*2 I,J,L,M,IVEC,SERVICEMO
      INTEGER*2 FIRSTYR,IOFFSET,MAX_DEP_YEARS
      INTEGER*2 RL_END_YEAR_INDEX,RL_END_MONTH
...
C     EXTERNAL VARIABLES
     ...
     +     DEPVEC(MAX_FINANCIAL_SIMULATION_YEARS-1),FIRST_YEAR_DEP
      INTEGER*2 BASE_YEAR_REF
      LOGICAL*1 REMAINING_LIFE_DEP
C
      REAL*4 MONTHLY_BOOK_DEP(0:12,0:FINANCIAL_SIMULATION_YEARS),
     +       PLANT_2_SERVICE(0:12,0:*),
     +       MONTHLY_AFUDC_ON_PLANT(0:12,0:*)
      INTEGER*2 DEP_MONTHS,MO_START,MO2,YR2,MO,YR
      CHARACTER*1 BOOK_DEP_METHOD,VECTOR_TYPE*20
      REAL*4 NPV,MONTHLY_DEP_AMOUNT,BOOK_LIFE,ANNUAL_BOOK_DEP
      LOGICAL*1 GROUP_DEPRECIATION,ACCOUNT_COMPLETE
      LOGICAL*1 BOOK_DEP_FINISHED
      INTEGER*2 START_YR
C
C MONTHLY DEP VECTOR VARIABLES
C
      INTEGER*2 VECTOR
      REAL*4 MONTHLY_BOOK_DEP_VECTOR(12,LAST_AVAILABLE_MONTHLY_YEAR)
      REAL*4 ANNUAL_BOOK_DEP_30(AVAIL_DATA_YEARS),TREND_NORM
      CHARACTER*1 MONTHLY_DATA_UNITS(LAST_AVAILABLE_MONTHLY_YEAR)
      INTEGER*2 MONTH_ENDING(LAST_AVAILABLE_MONTHLY_YEAR)
      INTEGER*2 YR1
C
C     IF THE BOOK LIFE IS LESS THAN ONE YEAR THE ASSUMPTION IS THAT
C     THE TOTAL AMOUNT OF CEP AND AFDC2 ARE TO BE EXPENSED IN ONE
C     YEAR
C
...
C
     ...
     +                                          (I <= AVAIL_DATA_YEARS))                
      ...
      ENDIF
C
C     IF THE BOOK LIFE IS GREATER THAN 98 YEARS DEPRECIATION IS NOT
C     TAKEN
C
      IF((BOKLF > 1.0  .AND. BOKLF <= 98.0) .OR. 
     +              (BOKLF < 0. .AND. INDEX('YPR',DATA_TYPE) /= 0)) THEN
C
C     CALCULATE BOOK DEPRECIATION ON BRICKS AND MORTAR
C
     ...
     +                                   + FLOAT(13-SERVICEMO)
               ...
               ENDDO
            ENDIF
c            IF(REMAINING_LIFE_DEP) THEN
c               BOKLF = MAX(.5,BOKLF-1)
c            ENDIF
         ENDDO
      ELSEIF(BOKLF < 0.) THEN
         NPVFB = 0.0
C        THEN USE THE DOLLAR AMOUNT VECTORS
         DO I = 2, FINANCIAL_SIMULATION_YEARS
...
C
         ...
         IF(BOOK_DEP_METHOD == 'R') THEN
            IF(BOOK_LIFE > BASE_YEAR_REF) THEN
               ...
               RL_END_MONTH = 
     +                   NINT((BOOK_LIFE-FLOAT(RL_END_YEAR_INDEX))*100.)
               IF(RL_END_MONTH == 0) RL_END_MONTH = 12
...
C
     ...
     +                                     MONTHLY_AFUDC_ON_PLANT(MO,YR)
                  ...
                  IF(HALF_YEAR_BOOK_DEP) THEN
C                     FIRST_YEAR_DEP = MIN(6.*MONTHLY_DEP_AMOUNT,NPV)
     ...
     +                                         MONTHLY_BOOK_DEP(MO2,YR2)
                     ...
                     ENDDO
                     MO_START = 1
                     IF(.NOT. GROUP_DEPRECIATION
     +                                      .AND. ACCOUNT_COMPLETE) EXIT
                  ENDDO
               ENDDO
            ENDDO
C
...
C
            ...
            IF(DATA_TYPE == 'D') THEN
c              DO YR1 = 1, FINANCIAL_SIMULATION_YEARS
c                 IF(PLANT_2_SERVICE(0,YR1) == 0.) CYCLE
                  yr1 = 1
     ...
     +                                              TREND_NORM
                        ...
                        ELSEIF(YR > AVAIL_DATA_YEARS) THEN
                           IF(TREND_NORM == 0.) THEN
                              DEPSL =
     +                          ANNUAL_BOOK_DEP_30(AVAIL_DATA_YEARS)/12.
                           ELSE
                              DEPSL =
...
C
     ...
     +     AFUDC_IN_CWIP(MAX_FINANCIAL_SIMULATION_YEARS)
      ...
      LOGICAL*1 R_CLASS_EXISTS,FIRST_PASS_IS_ACTIVE
      REAL R_CLASS_GPV,R_CLASS_CUMULATIVE_DEPRECIATION
      REAL FA_CASH,FA_CAPITIALIZED,FA_TAX_DEP,
     +     FA_BOOK_DEP,FA_AFDC_CASH,
     +     FA_AFDC_CAPITIALIZED,FA_CWIP,
...
C
     ...
     +     CLASS_AFUDC_IN_CWIP
      LOGICAL*1 FUTURE_ASSET_REPORT
      CHARACTER*40 OUT_OPTION_NAME
      INTEGER*2 FA_REPORTING_UNIT
      INTEGER FA_REPORTING_REC
      REAL*4 CURRENT_INTEREST_CAP_RATE(*)
      REAL*4 ALLOCATION_VALUE(0:AVAIL_DATA_YEARS)
C
      LOGICAL*1 VOID_LOGICAL,RETURN_ASSET_CLASS_LISTS,LGandE
      INTEGER*2 ALLOCATION_VECTOR
C
         FINANCIAL_SIMULATION_YEARS = STUDY_PERIOD + EXTENSION_PERIOD +1
         CALL RETURN_NUM_OF_OPTIONS_CLASSES(NUM_OF_FA_CLASSES,
     +                                              MAX_ASSET_CLASS_NUM)
         IF(ALLOCATED(ASSET_CLASS_POINTER))
     +                                   DEALLOCATE(ASSET_CLASS_POINTER)
         IF(MAX_ASSET_CLASS_NUM > 0) THEN
...
C***********************************************************************
C
c         VALUES_TO_ZERO = INT(FINANCIAL_SIMULATION_YEARS) * 
c     +                                      INT(NUM_OF_FA_CLASSES+1)
C
         CLASS_GPV = 0.
         CLASS_CUMULATIVE_DEPRECIATION = 0.
         CLASS_AFUDC_IN_CWIP = 0.
...
C
            IF(ABMETH > 0) THEN
               I = ABYEAR - BASE_YEAR + 1
               ALLOCATOR = 
     +                  ALLOCATION_VALUE(MIN(AVAIL_DATA_YEARS,I-1))/100.
...
C     +                               (CLASS_GPV(I,CLASS)+ GPV_ADDITIONS)
     ...
     +                                   NPV_PROPERTY_TAX(I) * ALLOCATOR
               IF(ABMETH > 0) THEN
                  ...
                  TDDB(I,CLASS)   = TDDB(I,CLASS) +
     +                               (DDB(I) - CUM_DD_AMORT) * ALLOCATOR
C                 TDDB(I,CLASS)   = TDDB(I,CLASS) + DDB(I) * ALLOCATOR
                  TRBDDB(I,CLASS) = TRBDDB(I,CLASS) + RBDDB(I)*ALLOCATOR
                  TAMRTE(I,CLASS) = TAMRTE(I,CLASS)+AMORTE(I)*ALLOCATOR
                  TEXEXP(I,CLASS) = TEXEXP(I,CLASS) + EXEXP(I)*ALLOCATOR
                  TAFEXP(I,CLASS) = TAFEXP(I,CLASS)+AFCEXP(I)*ALLOCATOR
...
C
     ...
     +                               TCWIP(I,0)+CLASS_AFUDC_IN_CWIP(I,0)
      ...
      ENDIF
C
C     IF(.NOT. ALLOCATED(CUMULATIVE_CASH_ADJUSTMENT))
C    +  ALLOCATE(CUMULATIVE_CASH_ADJUSTMENT(FINANCIAL_SIMULATION_YEARS))
      CUMULATIVE_CASH_ADJUSTMENT = 0.
     ...
     +                                      FIRST_PASS_IS_ACTIVE()) THEN
         ...
         ENDDO
      ENDIF
      CALL OPEN_NEW_ASSETS_OUT_FILE(INT2(10))
      DO I = 1, FINANCIAL_SIMULATION_YEARS
         WRITE(10,REC=I) TCE(I,0),TCEP(I,0),TTAXDP(I,0),TBOKDP(I,0),
     +      TAFCDP(I,0),TAFDC1(I,0),TAFDC2(I,0),TCWIP(I,0),TRBCWP(I,0),
...
C
     ...
     +                                   FA_WO_ADJUSTMENT_2_CUM_AFDC_DEP
               FA_AFDC_BORROWED = FA_AFDC_BORROWED +
     +                                          TAFDCB(R_YR,ASSET_CLASS)
               FA_WO_ADJUSTMENT_2_CUM_AFDC = TAFCAJ(R_YR,ASSET_CLASS) +
...
C
      ...
      INCLUDE 'SIZECOM.MON'
      INCLUDE 'GLOBECOM.MON'
C     INCLUDE 'NAMESCOM.MON'
      INTEGER*2 OFFSET,CAL_YEARS
      REAL*4 ANNUAL_DATA(*)
      REAL*4 MONTHLY_DATA(12,*),ACTIVE_TREND(0:12)
      REAL*4 OUTPUT_MONTHLY(0:12,0:*),PLANT_2_SERVICE(0:12,0:*)
      REAL*4 CWIP_BAL_BY,QUICK_CASH,CWIP_BAL
      CHARACTER*1 MONTHLY_DATA_UNITS(LAST_AVAILABLE_MONTHLY_YEAR)
      INTEGER*2 MONTH_ENDING(LAST_AVAILABLE_MONTHLY_YEAR)
      INTEGER*2 YR,MO,J,DUMMY_MONTH_ENDING,INSERVICE_MO
      CHARACTER*1 TREND
      PARAMETER(TREND='T') 
      CHARACTER*1 RIPPLE_TYPE,AVERAGE,ZERO,NORMALIZE
      PARAMETER(AVERAGE='A',ZERO='Z',NORMALIZE='N')
      LOGICAL*1 ACTIVE_TREND_FOUND
C
         J = 1
...
C
     ...
     +                                            DUMMY_MONTH_ENDING)
               ELSE
                  ...
                  OUTPUT_MONTHLY(0,YR) = OUTPUT_MONTHLY(0,YR) 
     +                                   - QUICK_CASH 
     ...
     +                                   + QUICK_CASH 
                  DO MO = 1, INSERVICE_MO               
                     OUTPUT_MONTHLY(MO,YR) = OUTPUT_MONTHLY(MO,YR) 
     +                                  + QUICK_CASH/FLOAT(INSERVICE_MO) 
                  ENDDO
               ENDIF
            ENDIF               
            CWIP_BAL = CWIP_BAL
...
C
      ...
      INCLUDE 'SIZECOM.MON'
      REAL*4 TOTAL_MONTHLY_FA_VALUES(:,:,:,:)
      ...
      INTEGER*4 VALUES_2_SET
      INTEGER*2 MAX_ASSET_CLASS_NUM,NUM_OF_CLASSES
c     REAL*4 CASH_EXPENDITURES(:,:)
c     ALLOCATABLE :: CASH_EXPENDITURES
c     SAVE CASH_EXPENDITURES
      LOGICAL*1 MONTHLY_ACTIVE
C
      INTEGER*2 R_ASSET_CLASS_VECTOR,R_ASSET_CLASS,ASSET_CLASS_ID,
     +          DEACTIVE_YR,MO_DEACT
      REAL*4 BY_CWIP_BALANCE,REGULATORY_ALLOCATOR
      REAL*4 R_BOOK_EXPEN(0:12,0:*),
     ...
     +       AFUDC_BORROWED_RATES(*)
      REAL*4 R_CASH_LAG_PATTERN(12),
     +       R_CASH_PAYMENTS(12)
      REAL*4 ASSET_CLASS_LIST(30)
      REAL*4 ASSET_ALLOCATION_LIST(30),ALLOCATOR
      INTEGER*2 CLASS,CLASS_POINTER,MO,YR ! ,AVAIL_DATA_YEARS/30/
      CHARACTER*1 DUMMY_TYPE
      REAL*4 ALLOCATION_VALUE(0:AVAIL_DATA_YEARS)
      INTEGER*2 ALLOCATION_VECTOR
      REAL*4 MONTHLY_CWIP(0:12),MONTHLY_GPV(0:12)
      REAL*4 MONTH_VARS(0:12,1:*)
      INTEGER*2 R_YR
      LOGICAL*1 VOID_LOGICAL,RETURN_ASSET_CLASS_LISTS
c      SAVE TOTAL_MONTHLY_FA_VALUES,ASSET_CLASS_POINTER,
c     +     FINANCIAL_SIMULATION_YEARS
c      SAVE MAX_ASSET_CLASS_NUM
c      SAVE MONTHLY_ACTIVE
C
         MONTHLY_ACTIVE = .FALSE.
         IF(ALLOCATED(TOTAL_MONTHLY_FA_VALUES))
     +                               DEALLOCATE(TOTAL_MONTHLY_FA_VALUES,
     +                                          ASSET_CLASS_POINTER)
c    +                                          CASH_EXPENDITURES)
         FINANCIAL_SIMULATION_YEARS= RUN_YEARS() + EXTENSION_YEARS()
         CALL RETURN_INITIALIZATION_CLASSES(NUM_OF_CLASSES,
     +                                              MAX_ASSET_CLASS_NUM)
         IF(MAX_ASSET_CLASS_NUM > 0) THEN
            ALLOCATE(ASSET_CLASS_POINTER(MAX_ASSET_CLASS_NUM))
     ...
     +                                     0:NUM_OF_CLASSES,
     +                                     MAX_FA_VARIALBES))
c    +             CASH_EXPENDITURES(0:12,0:FINANCIAL_SIMULATION_YEARS))
...
c         CALL CINITD(R_CASH_EXPENDITURES,
c     +                       INT(13*(FINANCIAL_SIMULATION_YEARS+1)),0.)
...
C                     
     ...
     +                                              ALLOCATION_VALUE(YR)
            ...
            ENDIF
C
C           ALLOCATOR = ASSET_ALLOCATION_LIST(CLASS_POINTER)/100.
C
     ...
     +                                             DEACTIVE_YR,MO_DEACT)
            DO YR = 1, FINANCIAL_SIMULATION_YEARS
               ...
               IF(YR <= AVAIL_DATA_YEARS)
     +                             ALLOCATOR = ALLOCATION_VALUE(YR)/100.
               MONTHLY_CWIP(0) = MONTHLY_CWIP(12)
               MONTHLY_GPV(0) = MONTHLY_GPV(12)
               DO MO = 0, 12
                  IF(YR == DEACTIVE_YR + 1 .AND. MO >= MO_DEACT)
     +                                                    ALLOCATOR = 0.
     ...
     +                                - R_MONTHLY_AFUDC_ON_PLANT(MO,YR))
                     MONTHLY_GPV(MO) = MONTHLY_GPV(MO-1)
     +                 + ALLOCATOR * (R_PLANT_2_SERVICE(MO,YR)
     +                                + R_MONTHLY_AFUDC_ON_PLANT(MO,YR))
                  ENDIF
...
C
         IF(R_ASSET_CLASS <= MAX_ASSET_CLASS_NUM) THEN
            ...
            IF(CLASS > 0 .OR. R_ASSET_CLASS == 0) THEN
               DO MO = 0, 12
                  MONTH_VARS(MO,Cash Plant Construction) =
     +                          MONTH_VARS(MO,Cash Plant Construction) +
...
C
         IF(R_ASSET_CLASS <= MAX_ASSET_CLASS_NUM) THEN
            ...
            IF(CLASS > 0 .OR. R_ASSET_CLASS == 0) THEN
               DO MO = 0, 12
                  MONTH_VARS(MO,Monthly AFUDC Equity) =
     +                             MONTH_VARS(MO,Monthly AFUDC Equity) +
     +                          TOTAL_MONTHLY_FA_VALUES(MO,R_YR,CLASS,
     +                                                  afudc_on_cash) -
     +                          TOTAL_MONTHLY_FA_VALUES(MO,R_YR,CLASS,
     +                                           afudc_borrowed_on_cash)
                  MONTH_VARS(MO,Monthly AFUDC Borrowed) =
     +                           MONTH_VARS(MO,Monthly AFUDC Borrowed) +
     +                          TOTAL_MONTHLY_FA_VALUES(MO,R_YR,CLASS,
     +                                           afudc_borrowed_on_cash)
                  MONTH_VARS(MO,Monthly AFUDC Total) =
     +                              MONTH_VARS(MO,Monthly AFUDC Total) +
     +                          TOTAL_MONTHLY_FA_VALUES(MO,R_YR,CLASS,
     +                                                    afudc_on_cash)
                  MONTH_VARS(MO,Monthly Book Depreciation) =
     +                        MONTH_VARS(MO,Monthly Book Depreciation) +
     +                        TOTAL_MONTHLY_FA_VALUES(MO,R_YR,CLASS,
     +                                          book_dep_on_plant_afudc)
               ENDDO
...
C
         IF(R_ASSET_CLASS <= MAX_ASSET_CLASS_NUM) THEN
            ...
            IF(CLASS > 0 .OR. R_ASSET_CLASS == 0) THEN
               DO MO = 0, 12
                  MONTH_VARS(MO,Monthly CWIP) =
     +                        TOTAL_MONTHLY_FA_VALUES(MO,R_YR,CLASS,
     +                                                  fa_cwip_balance)
                  MONTH_VARS(MO,Monthly Gross Plant in Service) =
     +                     MONTH_VARS(MO,Monthly Gross Plant in Service)
     +                     + TOTAL_MONTHLY_FA_VALUES(MO,R_YR,CLASS,
     +                                                   fa_gpv_balance)
                  MONTH_VARS(MO,Monthly TOTAL UTILITY PLANT) =
     +                     MONTH_VARS(MO,Monthly Gross Plant in Service)
     +                     + MONTH_VARS(MO,Monthly CWIP)
                  MONTH_VARS(MO,Monthly AFUDC Capitialized) =
     +               TOTAL_MONTHLY_FA_VALUES(MO,R_YR,CLASS,
     +                                                   afudc_on_plant)
                  MONTH_VARS(MO,Monthly Plant Capitialized) =
     +               TOTAL_MONTHLY_FA_VALUES(MO,R_YR,CLASS,
     +                                                  plant_2_service)
               ENDDO
            ENDIF
...
C***********************************************************************
C
      REAL*4 CASH_LAG_PATTERN(12),PATTERN_TOTAL
      INTEGER*2 MONTH,MO
         PATTERN_TOTAL = 0.
         DO MONTH = 12, 1, -1
...
C***********************************************************************
C
      REAL*4 PV_PLANT_COST,BOOK_CASH(0:12,0:*)
      INTEGER*2 ESCALATION_VECTOR,SIMULATION_PERIOD
      INTEGER*2 AVAIL_DATA_YEARS
      PARAMETER(AVAIL_DATA_YEARS=30)
      REAL*4 FV_PLANT_COST,CURRENT_VALUE
...
C
     ...
     +                                            ESCALATION_VECTOR)
         ...
         ENDDO
C
C PLANT PERCENT OF CWIP CHECKING FOR > 100%
C
C    REAL*4 PLANT_CAPITIALZED(0:12,0:*),CWIP
C        CWIP = PLANT_CAPITIALZED(0,0)
C        PLANT_CAPITIALZED(0,0) = 0.
C        DO YR = 1, SIMULATION_PERIOD
C           PLANT_CAPITIALZED(0,YR)  = PLANT_CAPITIALZED(0,YR)/100.
C           CWIP = CWIP + BOOK_CASH(0,YR)
C           PLANT_CAPITIALZED(0,YR)=MIN(PLANT_CAPITIALZED(0,YR),1.)*CWIP
C           CWIP = CWIP - PLANT_CAPITIALZED(0,YR)
C        ENDDO
      RETURN
      END
C***********************************************************************
...
C
      INCLUDE 'GLOBECOM.MON'
      INTEGER*2 YR
      REAL*4 MONTHLY_VALUES(0:12),AMOUNT
      CHARACTER*1 MONTHLY_DATA_UNITS
      INTEGER*2 MO,MONTH_END
      INTEGER*2 MONTH_ENDING
         ...
         ELSE
            ...
            IF(MONTHLY_DATA_UNITS == PERCENT) THEN
               ...
               DO MO = 1, 12
                  ...
                  MONTHLY_VALUES(YR) = MONTHLY_VALUES(YR) +
     +                                                MONTHLY_VALUES(MO)
               ENDDO
            ENDIF
         ...
         ENDIF
C
C SAVE CURRENT PATTERN FOR TREND ANALYSIS
C
c         DO MO = 0, 12
...
C
      REAL*4 TAX_DEP
      REAL*4 MONTHLY_TAX_VALUE_OF_ASSET(0:12,0:*),
     +       MONTHLY_TAX_DEPRECIATION(0:12,0:*),
     +       TAX_ALLOCATOR,TAX_DEP_2_ALLOCATE
      INTEGER*2 IN_SERVICE_YR,
     +          DISTRIBUTION_YR,
     +          YR,MO,MOP1
C
     ...
     +                      MONTHLY_TAX_VALUE_OF_ASSET(0,YR) /= 0.) THEN
            ...
            DO MO = 1, 12 
               IF(MONTHLY_TAX_VALUE_OF_ASSET(MO,YR) /= 0.) THEN
                  TAX_DEP_2_ALLOCATE = TAX_ALLOCATOR / FLOAT(13-MO)
     +                               * MONTHLY_TAX_VALUE_OF_ASSET(MO,YR)
