...
! 1=COAL; 2=GAS ; 3=OIL; 4=NUCLEAR; 5=WATER; 6=OTHER 
    ...
    integer*2, parameter :: max_prod_types_clr=24 !clreport
    integer, parameter :: MAX_PROD_TYPES_mo=19 ! margnobl
    integer*2, parameter :: max_prod_types_tfo=24 ! tf_objt
    integer*2, parameter :: max_prod_types_pd=18 ! Picadisp
    integer*2, parameter :: MAX_FUEL_TYPES_pd=6 ! Picadisp
    ...
    real*4, allocatable :: DEV_TG_CAP(:,:,:)
    real*4, allocatable ::   DEV_NEW_TG_CAP(:,:,:)
    real*4, allocatable ::  SAVED_DEV_NEW_TG_CAP(:,:,:)
    real*4, allocatable :: RPS_HYDRO_DB(:,:,:,:)
    real*4, allocatable ::  TG_CapacitySupplyObligation(:,:)
    real (kind=4), allocatable :: GRX_QUAL_GEN_DB(:,:,:), &
         SAVE_GRX_QUAL_GEN_DB(:,:,:)

    real*4 :: TG_ResourceActualPreference
    real*4, allocatable :: TG_EffectiveCapacity(:,:)
    integer*2 :: TRANS_PROD_NO=0
    ...
    real*4 :: WH_MONTH_ENERGY
    integer*2 :: T_I,T_J
    logical*1 :: TRANSACT_PROD_REPORT_ACTIVE=.false.
