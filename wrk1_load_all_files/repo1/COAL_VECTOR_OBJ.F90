...
! COAL_ESCALATIONS_READ_DATA
         INTEGER (KIND=2), PARAMETER :: COAL_ESCALATIONS_LRECL=512 ! OUTPUT FILE RECORD LENGTH
         CHARACTER (LEN=40), PARAMETER ::
     +         COAL_ESCALATIONS_BINARY_FILE_NAME ="COAL_ESCALATIONS.BIN"
         LOGICAL (KIND=4), SAVE ::  COAL_ESCALATIONS_FILE_EXISTS=.FALSE.
         CHARACTER (LEN=2), SAVE ::
     +                          PROCESSING_COAL_ESCALATIONS_FILE_OL='BC'
...
!
!
      SUBROUTINE COAL_ESCALATIONS_MAKEBIN(BASE_FILE_NAME)  ! ENTERY NAME HERE
!         
C***********************************************************************
C
!
...
!
     ...
     +                          TRIM(COAL_ESCALATIONS_BINARY_FILE_NAME),
...
! **********************************************************************
      LOGICAL FUNCTION READ_COAL_ESCALATION_FILE(BASE_YEAR)
         ...
         IF(COAL_ESCALATIONS_FILE_EXISTS) THEN
            IF(PROCESSING_COAL_ESCALATIONS_FILE_OL == 'OL') THEN
...
!
     ...
     +        PROCESSING_COAL_ESCALATIONS_FILE_OL//
     +                          TRIM(COAL_ESCALATIONS_BINARY_FILE_NAME),
     +                            ACCESS="DIRECT",STATUS="UNKNOWN",
     +                              RECL=COAL_ESCALATIONS_LRECL)
            IREC = 0
...
C THE VALUES FOR A VECTOR ARE RETURN FOR THE AVAILABLE DATA YEARS
C
...
!
     ...
     +                                  TRIM(OVERLAY_FILE_NAME)//".DAT"
      ...
      IF(DATA_FILE_EXISTS .AND. COAL_ESCALATIONS_FILE_EXISTS) THEN
         ...
         ESCALATIONS_VECTOR_OVERLAY = 0
         INUNIT = 12
         IF(PROCESSING_COAL_ESCALATIONS_FILE_OL == 'BC') THEN
            OPEN(11,FILE=TRIM(OUTPUT_DIRECTORY())//"BC"//
     +                          TRIM(COAL_ESCALATIONS_BINARY_FILE_NAME),
     +                               ACCESS="DIRECT",STATUS="UNKNOWN",
     +                                      RECL=COAL_ESCALATIONS_LRECL)
...
!         
     ...
     +                         START_MONTH,END_MONTH
         ...
         CLOSE(10)
         CLOSE(12)
         IF(PROCESSING_COAL_ESCALATIONS_FILE_OL == 'BC') CLOSE(11)
         PROCESSING_COAL_ESCALATIONS_FILE_OL = 'OL'
      ENDIF
      RETURN
      END
!
