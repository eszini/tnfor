...
C
      INCLUDE 'SpinLib.MON'
      INCLUDE 'SIZECOM.MON'
      INTEGER*2 NUMBER_OF_BC_CLASSES/0/,
     +          MAX_BC_CLASS_ID_NUM/0/
      INTEGER*2 NUMBER_OF_OL_CLASSES/0/,
     +          MAX_OL_CLASS_ID_NUM/0/
      INTEGER*2 INUNIT,IREC,DELETE,LRECL/1042/,
     +          ASSET_CLASS_NUM
      INTEGER*4 IOS,IOS_BASE
      CHARACTER*5 OVERLAY_FAMILY_NAME
      CHARACTER*10 STATE_OR_PROVINCE
      CHARACTER*30 COMMENT
      CHARACTER*256 FILE_NAME
      CHARACTER*256 BASE_FILE_DIRECTORY
      CHARACTER*256 OUTPUT_DIRECTORY,DATA_DRIVE
      LOGICAL*4 FILE_EXISTS,FILE_OPEN
      CHARACTER*5 BASE_FILE_NAME,CLASS_SALES_BASE_FILE
      REAL*4 FILE_VALUES(41) ! READS VARIABLES 4 THROUGH 44
      CHARACTER*1 INTRA_COMPANY_TRANSACTION,USE_TRANSACT_FILE
      REAL ENERGY_ADJ_RATE(12)    ! 114-125
! DECLARATION FOR DBREAD COMMON BLOCK
      CHARACTER*1024 RECLN
! DECLARATION FOR CLASS FORECASTS
      INTEGER*2 YEAR,DATA_RECORDS_IN_TABLE
      CHARACTER*32 CLASS_NAME,ENERGY_UNITS*3,TEMP_CLASS_NAME

	  ! leading space needed
      CHARACTER*16 FILE_TYPE/'Revenue Forecast'/
      INTEGER*2 SAVE_CLASS_SALES_UNIT,
     +          CLASS_SALES_UNIT
      INTEGER*2 BC_TABLE_NUMBER,OL_TABLE_NUMBER
      INTEGER*2 R_NUMBER_OF_TABLES
      SAVE BC_TABLE_NUMBER,OL_TABLE_NUMBER
      CHARACTER*2 CLASS_SALES_OL/'BC'/
      REAL*4 MONTHLY_PRICE_INFORMATION(36)
      CHARACTER*1 CASH_TREATMENT
      CHARACTER*30 LAG_PERIOD,
     +             CASH_RECEIVABLE_PAYABLE
      REAL*4 ENERGY_LOSS_FACTOR,
     +       DEMAND_LOSS_FACTOR,
     +       STATE_RPS_ALLOCATION,
     +       STATE_RPS_EXEMPTION
!
      CHARACTER*1 DEMAND_PRICING_SWITCH
      CHARACTER*1 OUC_CLASSIFICATIONS
      CHARACTER*30 OUC_TAX_TREATMENT
      REAL*4 OUC_BASE_EL_STAB_ADDER,
     +       OUC_FUEL_STAB_ADDER,
...
C
      ...
      CHARACTER*40 INTRA_EXPENSE_CLASSIFICATION,
     ...
     +             FE_COMPETITIVE_CLASSIFICATION ! 112
      CHARACTER*20 COMMODITY_LHC_DISTRIBUTION,
     +             DEMAND_LHC_DISTRIBUTION, ! 105
     +             CUSTOMER_LHC_DISTRIBUTION ! 106
      CHARACTER*1 ACCOUNT_TYPE ! 107
      INTEGER*2 TRANSACT_FORECAST_GROUP, ! 110
     +          TRANSACT_CUSTOMER_GROUP ! 111
      INTEGER*2 BC_ASSET_CLASS_POINTER(:),
     ...
     +               TEMP_ASSET_CLASS_POINTER
      ...
      INTEGER*2 R_CLASS_POINTERS(R_MAX_CLASS_NUM)
      INTEGER RECORDS_IN_OVERLAY_TABLE
      LOGICAL*1 READ_OVERLAY_INPUT
C
C***********************************************************************
C
...
C
     ...
     +                                ',,,,,,,,,,,,,,,,,,,,,,,,,,,,,,'//
     +                                ',,,,,,,,,,,,,,,,,,,,,,,,,,,,,,'
               READ(RECLN,*,ERR=200) DELETE,YEAR,CLASS_NAME,COMMENT,
     +                            FILE_VALUES,INTRA_COMPANY_TRANSACTION,
     ...
     +                            LAG_PERIOD,
     +                            CASH_RECEIVABLE_PAYABLE,
     +                            CPL_JURISDICTION, ! 91 FE competitive classes
     +                            OUC_CLASSIFICATIONS,
     +                            OUC_TAX_TREATMENT,
     ...
     +                            ACCOUNT_ACTIVE,
     +                            COMMODITY_LHC_DISTRIBUTION, ! 104
     +                            DEMAND_LHC_DISTRIBUTION, ! 105
     +                            CUSTOMER_LHC_DISTRIBUTION, ! 106
     +                            ACCOUNT_TYPE, ! 107
     +                            EXPENSE_CLASSIFICATION, ! 108
     ...
     +                            STATE_RPS_EXEMPTION  ! 128
               ...
               ASSET_CLASS_NUM = INT2(FILE_VALUES(4))
C
...
C
               ...
               WRITE(12,REC=IREC) DELETE,YEAR,CLASS_NAME,
     +                            FILE_VALUES,
     +                            INTRA_COMPANY_TRANSACTION,
     +                            INTRA_ASSET_CLASS_ID,
     +                            INTRA_ASSET_CLASS_ALLOCATION,
     ...
     +                                DEALLOCATE(OL_ASSET_CLASS_POINTER)
         IF(MAX_OL_CLASS_ID_NUM > 0) THEN
            ...
            OL_ASSET_CLASS_POINTER(:) = TEMP_ASSET_CLASS_POINTER(
     +                                            1:MAX_OL_CLASS_ID_NUM)
         ENDIF
         DEALLOCATE(TEMP_ASSET_CLASS_POINTER)
      RETURN
...
C
      INTEGER*2 ASSET_CLASS_POINTER(:),MAX_ASSET_CLASS_NUM,
     +          NUM_OF_ASSET_CLASSES,R_MONTH
      ALLOCATABLE :: ASSET_CLASS_POINTER
      INTEGER*2 INTRA_CLASS,LAG_MO,MO1
      PARAMETER (INTRA_CLASS=-1)
      INTEGER*4 VALUES_TO_ZERO
      INTEGER*2 MO,I,IREC,R_YEAR,
     +          R_ST_TG
      INTEGER*4 IOS
      LOGICAL*1 FirstEnergyActive,FirstEnergy,
     +          CPL_IS_ACTIVE,CPL_ACTIVE,WVPA,
     +          YES_RPS_CALC
      CHARACTER*32 RATE_CLASS_NAME(:),INTRA_COMPANY_TRANSACTION*1
      REAL*4 INTRA_COMPANY_REVENUE
      REAL*4 R_TOTAL_BASE_REVENUE,TOTAL_BASE_REVENUE
      REAL*4 R_CLASS_REVENUES(0:*),ALLOCATED_REVENUES,
     +       TWO_YR_CASH_DISTRIBUTION(24)
!      REAL*4 REVENUES(:),
      REAL*4 CLASS_ALLOCATION_ID(:),
...
!     ALLOCATABLE :: REVENUES,
     ...
     +     TEMP_TRANS_CUSTOMERS(0:12)
      REAL*4 MONTHLY_ENERGY_DOLLARS(0:12),
     +       MONTHLY_CUSTOMER_DOLLARS(0:12),
     +       MONTHLY_DEMAND_DOLLARS(0:12),
     +       BILLING_DEMAND(0:12),
     +       R_MONTHLY_VALUES(0:12)
      LOGICAL*1 GET_TG_CG_DATA,DATA_FOUND,TEMP_DATA_FOUND,
     +          TEMP_L,GET_STATE_RPS_REQS
      REAL*4 ANNUAL_PEAK_DEMAND,ANNUAL_ENERGY
      INTEGER*2 FILE_TABLES
      INTEGER*2 ASSET_CLASS,CLASS_POINTER,SALES_UNIT/3458/,DELETE,
     ...
     +          ST,STATE_ID_LOOKUP,
     +          RPS,ST_TG
      CHARACTER*3 ENERGY_UNITS
      REAL*4 MONTHLY_PRICE_INFORMATION(12,3)
      CHARACTER*1 CASH_TREATMENT,
     +            R_CASH_TREATMENT
!
      CHARACTER*1 OUC_CLASSIFICATIONS
     ...
     +       OUC_PLACEHOLDER_FUND_A,
     +       OUC_PLACEHOLDER_FUND_B
      REAL*4 REVENUES_MONTHLY(:,:,:),EXPENSES_MONTHLY(:,:,:),
     ...
     +               ENERGY_MONTHLY
      ...
      INTEGER*2 RUN_YEAR
      INTEGER*2 EXTYPE,REV_TYPE
      REAL*4 OPREV_TAX_RATE
      INTEGER*2 TEMP_YR
      REAL*4 LAG_BY_MONTH(12),ANNUAL_CASH_TOTAL,
...
C
      INTEGER*2 R_CLASS
      CHARACTER*(*) R_REVENUE_CLASSIFICATION,R_EXPENSE_CLASSIFICATION,
     ...
     +     R_ATL_LEASE_EXP,R_BTL_LEASE_EXP,R_SERVICE_TRANSACTIONS
      REAL*4 EXPENSE_ELIM,REVENUE_ELIM
      LOGICAL*1 CLASS_LINKED_2_PARENT
      INTEGER*2 SC_FUEL_FACTOR_ADJ_MONTH/4/,
     +          SC_FUEL_FACTOR_CAL_MONTH/3/,
     +          NC_FUEL_FACTOR_ADJ_MONTH/10/,
     +          NC_FUEL_FACTOR_CAL_MONTH/3/
      REAL*4 SC_PURCHASE_POWER_ALLOCATION(10),
     ...
     +       NC_EMF_FACTOR_INPUT(12,7)
      CHARACTER*1 OFF_SYSTEM_RATES_ACTIVE(7)
C
! SC AND NC FUEL ACCOUNTING
C
      INTEGER*2 REPORTING_UNIT,CPL_DEFERRED_FUEL_RPT_HEADER
      LOGICAL*1 REPORT_HEADER_OPEN/.FALSE./
      REAL*4 SC_CUMULATIVE_FUEL_COST,
     +       SC_CUMULATIVE_ENERGY_SALES,
     +       SC_CUMULATIVE_JURIS_SALES,
     +       SC_BASE_FUEL_FACTOR,
     +       SC_DEFERRED_FUEL_BALANCE
      REAL*4 NC_CUMULATIVE_FUEL_COST,
     ...
     +       NC_EMF_FUEL_BALANCE
      INTEGER*2 CPL_DEFERRED_FUEL_RPT_CLASS_ID,
     +          NC_DEFERRED_FUEL_RPT_CLASS_ID,
     +          SC_DEFERRED_FUEL_RPT_CLASS_ID
      CHARACTER*1 USE_BUDGET_FORECAST_IN(7)
! ADDITIONAL SECTION
      REAL*4 ENERGY_LOSS_FACTOR,
     +       DEMAND_LOSS_FACTOR,
     ...
     +       SPLIT_FACTOR (4)
      ...
      CHARACTER*1 DEMAND_PRICING_SWITCH,
     +            ACCOUNT_ACTIVE
      INTEGER*2 SALES_CLASSIFICATION,CPL_SALES_CLASS,
     +          NC_SALES_CLASS/1/,SC_SALES_CLASS/2/,
     +          WHOLE_SALES_CLASS/3/,NCEMPA_SALES_CLASS/5/,
     +          NCEMC_SALES_CLASS/6/,
     +          PEAK_MONTH,A
      INTEGER*2 INCOME_STATEMENT_POSITION
      REAL*4 SC_FUEL_REV_CASH_JAN,
     +       NC_FUEL_REV_CASH_JAN,
     +       BU_FUEL_REV_CASH_JAN
      INTEGER*2 SC_CALCULATION_START_YR,
     +          NC_CALCULATION_START_YR
      CHARACTER*1 DEF_CLASS_TYPE
      REAL*4 NC_CALC_BASE_FUEL_FACTOR,NC_CALC_EMF_FACTOR,
     +       SC_CALC_BASE_FUEL_FACTOR
      LOGICAL*1 CPL_DEFERRED_FUEL_NOT_ACTIVE
C
      INTEGER*2 ALLOCATION_VECTOR
      REAL*4 ALLOCATION_VALUE(AVAIL_DATA_YEARS)
      LOGICAL*1 VOID_LOGICAL,RETURN_ASSET_CLASS_LISTS
      REAL*4      PA_TOTAL_RETAINED,
     +            PA_SEPA_CAPACITY,
     +            PA_FIRM_PURCHASE_CAPACITY,
     +            EMC_SEPA_CAPACITY
C
      INTEGER*2 R_RUN_YEAR
      CHARACTER*30 R_LAG_PERIOD,R_CASH_RECEIVABLE_PAYABLE
      CHARACTER*40 LAGGED_PATTERN
      REAL*4 R_MONTHLY_REVENUES(0:12),R_ASSET_ALLOCATOR,
     +       R_MONTHLY_ENERGY(0:12),R_CLASS_CUSTOMERS(0:12)
      REAL*4 MONTHLY_ALLOCATED_REVENUES(0:12),
     +       MONTHLY_ALLOCATED_ENERGY
      REAL*4 CURRENT_AMOUNT,LAGGED_AMOUNT
      REAL*4 R_REVENUE_ELIM
      REAL*4 R_MONTHLY_EXPENSES(0:12),
     +       MONTHLY_ALLOCATED_EXPENSES(0:12)
      LOGICAL*1 EXPENSE_TYPE_IS_CASH
      REAL*4 R_EXPENSES_ELIM
      INTEGER*2 PERIOD
      REAL*4 R_RESIDENTIAL_REVENUES,
     ...
     +       R_ADJ_EXP
C
      REAL*4 R_MON_CHANGE_ACCTS_RECEIVABLE(0:12),
     +       R_MON_CHANGE_ACCTS_PAYABLE(0:12)
      INTEGER*2 DATA_POS,J
C
      REAL*4 R_CHANGE_IN_ACCOUNTS_RECEIVABLE,
     +       R_CHANGE_IN_ACCOUNTS_PAYABLE
C
      CHARACTER*5 CPL_DEFERRED_FUEL_FILE
      CHARACTER*256 FILE_NAME
      CHARACTER*256 BASE_FILE_DIRECTORY
      CHARACTER*5000 RECLN,DUMMY_CHR*20
      INTEGER*2 SKIP,YR,CLASS_POS
      CHARACTER*1 R_CLASS_TYPE(0:*)
      INTEGER*2 R_MASTER_CLASS_LIST(0:*),RC_REVENUE_FORECAST_HEADER
C
      REAL*4 CPL_GEN_COST(0:12),
     +       CPL_NUC_COST(0:12),
     +       CPL_PA_ENTITLE(0:12),
     +       CPL_PUR_POWER(0:12),
     ...
     +       NC_NEW_EMF_FACTOR
      REAL*4 CPL_NCEMPA_MWH(0:12),CPL_NCEMC_MWH(0:12)
      INTEGER*2 OTHER2_FORECAST
      REAL*4 ZERO_POS
      PARAMETER(ZERO_POS=0.,OTHER2_FORECAST=5)
      CHARACTER*6 SHORT_MONTH_NAMES
      INTEGER*2 CURRENT_YEAR
      REAL*4 SC_PUR_POWER_ALLOCATION,
     +       NC_PUR_POWER_ALLOCATION,
     +       TOTAL_OFF_SYSTEM_ENRG,
     +       TOTAL_OFF_SYSTEM_COST,
     +       OFF_SYSTEM_ENRG_RATE
      REAL*4 BUDGET_GEN_COST(0:12),BUDGET_PUR_POWER(0:12)
      REAL*4 ANNUAL_CPL_GEN_COST,ANNUAL_CPL_PUR_POWER
      REAL*4 ENRG_RATIO,P_POWER_RATIO
C
      REAL*4 R_CPL_FUEL_REVENUES,
     +       R_DEFERRED_FUEL_EXPENSE
C
      REAL*4 R_CPL_MONTHLY_FUEL_REVENUES(0:12),
     ...
     +       R_FUEL_REVENUE_RECEIVABLE
      ...
      CHARACTER*40 EXT_RATE_CLASS_NAME
      CHARACTER*8 ACCOUNT_TYPE_NAME
      CHARACTER*20 VARIABLE_TYPE_NAME(0:10)
      REAL*4 VARIABLE_VALUE(0:13,0:10)
      LOGICAL*1 RC_RPT_HEADER_NOT_OPEN/.TRUE./
      INTEGER*4 NEXT_REC/0/
      INTEGER*2 RC_UNIT_NO/0/
      REAL*4 THE_RATIO_OF_A_TO_B
...
C
     ...
     +                                                LAST_INCOME_LINE),
     +               CASH_CARRY_OVER_PAYABLE(1:12,
     +                                          -1:NUM_OF_ASSET_CLASSES,
     +                                               LAST_EXPENSE_ITEM),
     +               ENERGY_MONTHLY(0:12,-1:NUM_OF_ASSET_CLASSES,
     +                                                LAST_INCOME_LINE))
            IF(ALLOCATED(ASSET_CLASS_CUSTOMERS))
     +                                 DEALLOCATE(ASSET_CLASS_CUSTOMERS)
            ALLOCATE(ASSET_CLASS_CUSTOMERS(0:12,
     +                                         -1:NUM_OF_ASSET_CLASSES))

            IF(ALLOCATED(ASSET_CLASS_ENERGY))
...
c
         IF(RC_RPT_HEADER_NOT_OPEN) THEN
            ...
            RC_UNIT_NO = RC_REVENUE_FORECAST_HEADER(INT2(20),NEXT_REC)
         ENDIF
C
! ZERO ARRAYS
C
...
!            IF(CASH_TREATMENT == 'N') LAG_PERIOD = -1.
!            IF(CASH_TREATMENT == 'B') LAG_PERIOD = 0.
            DO MO = 1, 12
               ENERGY_PRICE(MO) =  MONTHLY_PRICE_INFORMATION(MO,1)
     +                             + MARKET_ENERGY_PRICE
     +                             + ENERGY_ADJ_RATE(MO)    ! 114-125
     ...
     +                                             MARKET_CUSTOMER_PRICE
            ...
            DATA_FOUND = .FALSE.
            IF(ACCOUNT_TYPE == 'C') REVENUE_CLASSIFICATION =
     ...
     +                                         TEMP_TRANS_CUSTOMERS)
                     ...
                     IF(TEMP_DATA_FOUND) THEN
                        DO MO = 1, 12
                           TRANS_ENERGY(MO) = TRANS_ENERGY(MO)
     +                                        + TEMP_TRANS_ENERGY(MO)
                           TRANS_PEAK(MO) = TRANS_PEAK(MO)
     +                                      + TEMP_TRANS_PEAK(MO)
                           TRANS_CUSTOMERS(MO) = TRANS_CUSTOMERS(MO)
     +                                        + TEMP_TRANS_CUSTOMERS(MO)
                        ENDDO
                     ENDIF
...
C
     ...
     +               * GET_SCENARIO_BY_INDEX(RUN_YEAR,MO,SCENARIO_INDEX)
            ...
            IF(INDEX(DEMAND_LHC_DISTRIBUTION,"Not Active") == 0) THEN
               ANNUAL_PEAK_DEMAND = 0.
               SCENARIO_INDEX =
     +                       GET_SCENARIO_INDEX(DEMAND_LHC_DISTRIBUTION)
               DO MO = 1, 12
                  CLASS_DEMAND(MO,I) = CLASS_DEMAND(MO,I)
     +               * GET_SCENARIO_BY_INDEX(RUN_YEAR,MO,SCENARIO_INDEX)
                  ANNUAL_PEAK_DEMAND = MAX(ANNUAL_PEAK_DEMAND,
     +                                               CLASS_DEMAND(MO,I))
            ...
            IF(INDEX(CUSTOMER_LHC_DISTRIBUTION,"Not Active") == 0) THEN
               SCENARIO_INDEX =
     +                     GET_SCENARIO_INDEX(CUSTOMER_LHC_DISTRIBUTION)
               DO MO = 1, 12
                  CLASS_CUSTOMERS(MO,I) = CLASS_CUSTOMERS(MO,I)
     +               * GET_SCENARIO_BY_INDEX(RUN_YEAR,MO,SCENARIO_INDEX)
               END DO
...
! END LHC MODIFICATIONS
            CPL_JURISDICTIONAL_INFOR(0,SALES_CLASSIFICATION,CPL MWhrs) =
...
!
     ...
     +                 CUSTOMER_PRICE(MO)*CLASS_CUSTOMERS(MO,I)/1000000.
     ...
     +                       + DEMAND_PRICE(MO)*ANNUAL_PEAK_DEMAND/1000.
               ...
               ENDIF
               MONTHLY_DOLLARS(MO) = MONTHLY_ENERGY_DOLLARS(MO)
     +                               + MONTHLY_DEMAND_DOLLARS(MO)
     +                               + MONTHLY_CUSTOMER_DOLLARS(MO)
               MONTHLY_DOLLARS(0) = MONTHLY_DOLLARS(0) +
     +                                               MONTHLY_DOLLARS(MO)
               MONTHLY_ENERGY_DOLLARS(0) = MONTHLY_ENERGY_DOLLARS(0)
     +                                     + MONTHLY_ENERGY_DOLLARS(MO)
               MONTHLY_DEMAND_DOLLARS(0) = MONTHLY_DEMAND_DOLLARS(0)
     +                                     + MONTHLY_DEMAND_DOLLARS(MO)
               MONTHLY_CUSTOMER_DOLLARS(0) = MONTHLY_CUSTOMER_DOLLARS(0)
     +                                    + MONTHLY_CUSTOMER_DOLLARS(MO)
               BILLING_DEMAND(0) = BILLING_DEMAND(0)
     +                             + BILLING_DEMAND(MO)
               CLASS_DEMAND(0,I) = CLASS_DEMAND(0,I)
     +                             + CLASS_DEMAND(MO,I)
               CLASS_CUSTOMERS(0,I) = CLASS_CUSTOMERS(0,I)
     +                                   + CLASS_CUSTOMERS(MO,I)
!
            ENDDO
...
C
     ...
     +                                     CASH_RECEIVABLE_PAYABLE)
C
!                     CALL CLASS_LINKED_TO_PARENT(ASSET_CLASS,
!     +                                           CLASS_LINKED_2_PARENT)
!                     IF(.NOT. CLASS_LINKED_2_PARENT) EXPENSE_ELIM =
!     +                             EXPENSE_ELIM + ASSET_ALLOCATOR *
!     +                                                MONTHLY_DOLLARS(0)
...
!                 CALL CL_SALES_EXP_ELIMINATION(INTRA_CLASS,
!    +                                     EXPENSE_ELIM,
!    +                                     INTRA_EXPENSE_CLASSIFICATION,
!    +                                     INTRA_EXPENSE_COLLECTION)
                  ASSET_ALLOCATOR = 1.0
...
C
     ...
     +                     ASSET_CLASS_LIST(CLASS_POINTER) == -99.) EXIT
            ENDDO
            IF(INTRA_COMPANY_TRANSACTION == 'Y') THEN
               INTRA_COMPANY_REVENUE = INTRA_COMPANY_REVENUE +
     +                                                      REVENUE_ELIM
!              CALL CL_SALES_REV_ELIMINATION(INTRA_CLASS,
!    +                                       REVENUE_ELIM,
!    +                                       REVENUE_CLASSIFICATION)
               ASSET_ALLOCATOR = 1.
...
C
     ...
     +                CLASS_CUSTOMERS(MO,I)
            ...
            ! CA 3.8.98
            ! TEMP_PEAK = MAX(TEMP_PEAK,CLASS_DEMAND(MO,I))
     ...
     +                                            ANNUAL_ENERGY
            ...
            REV_TYPE =
     +               INCOME_STATEMENT_POSITION(REVENUE_CLASSIFICATION)
            IF(REV_TYPE == 11) THEN
               A = 1
            ...
            IF (SALES_CLASSIFICATION == 1 .AND. A/=0) THEN
               CPL_NC_PEAK = CPL_NC_PEAK +
     +                  TEMP_PEAK *
     +                        NCP_MULTIPLIER (A)
!     +                        SPLIT_FACTOR (A)
            ELSEIF (SALES_CLASSIFICATION == 2 .AND. A/=0) THEN
               CPL_SC_PEAK = CPL_SC_PEAK +
     +                  TEMP_PEAK *
     +                        NCP_MULTIPLIER (A)
!     +                        (1 - SPLIT_FACTOR(A))
...
! TRY TO FIGURE OUT HOW TO GET THE VALUE FROM DEMAND PEAK
      ...
      DO MO = 4, 9
         ...
         DO SALES_CLASSIFICATION = 1,7
            ...
            TEMP_PEAK = TEMP_PEAK +
     +                  CPL_JURISDICTIONAL_INFOR
     +                              (MO,SALES_CLASSIFICATION,CPL Demand)
      ...
      SYSTEM_PEAK_SUMMER_NC =
     +               CPL_JURISDICTIONAL_INFOR
     +                        (PEAK_MONTH,NC_SALES_CLASS,CPL Demand)
      SYSTEM_PEAK_SUMMER_SC =
     +               CPL_JURISDICTIONAL_INFOR
     +                        (PEAK_MONTH,SC_SALES_CLASS,CPL Demand)

!     CA ADDED 2/25/99

      SYSTEM_PEAK_SUMMER_WHOLE =
     +               CPL_JURISDICTIONAL_INFOR
     +                        (PEAK_MONTH,WHOLE_SALES_CLASS,CPL Demand)

      SYSTEM_PEAK_SUMMER_NCEMPA =
     +               CPL_JURISDICTIONAL_INFOR
     +                        (PEAK_MONTH,NCEMPA_SALES_CLASS,CPL Demand)

!

      SYSTEM_PEAK_SUMMER_NCEMC =
     +               CPL_JURISDICTIONAL_INFOR
     +                        (PEAK_MONTH,NCEMC_SALES_CLASS,CPL Demand)


!     2/28/99. GAT.

      CALL GET_TOTAL_RETAINED(PA_TOTAL_RETAINED)
      CALL GET_PA_FIRM_N_SEPA(PA_FIRM_PURCHASE_CAPACITY,
     +                           PA_SEPA_CAPACITY)
     ...
     +                                 PA_SEPA_CAPACITY -
     +                                 PA_FIRM_PURCHASE_CAPACITY
      SYSTEM_PEAK_WINTER_NCEMC = SYSTEM_PEAK_WINTER_NCEMC -
     +                                 EMC_SEPA_CAPACITY
      SYSTEM_PEAK_WINTER =
     +         SYSTEM_PEAK_WINTER_NC +
     +         SYSTEM_PEAK_WINTER_SC +
...
!
     ...
     +       1000.*CPL_JURISDICTIONAL_INFOR(MO,NC_SALES_CLASS,CPL MWhrs)
      SYSTEM_ENERGY_SALES_SC = SYSTEM_ENERGY_SALES_SC +
     +       1000.*CPL_JURISDICTIONAL_INFOR(MO,SC_SALES_CLASS,CPL MWhrs)
      SYSTEM_ENERGY_SALES_WHOLE = SYSTEM_ENERGY_SALES_WHOLE +
     +       1000.*
     +          CPL_JURISDICTIONAL_INFOR(MO,WHOLE_SALES_CLASS,CPL MWhrs)
       SYSTEM_ENERGY_SALES_NCEMPA = SYSTEM_ENERGY_SALES_NCEMPA+
     +       1000.*
     +         CPL_JURISDICTIONAL_INFOR(MO,NCEMPA_SALES_CLASS,CPL MWhrs)
       SYSTEM_ENERGY_SALES_NCEMC = SYSTEM_ENERGY_SALES_NCEMC +
     +       1000.*
     +          CPL_JURISDICTIONAL_INFOR(MO,NCEMC_SALES_CLASS,CPL MWhrs)

!      DO SALES_CLASSIFICATION = 1, 6 ! WHY NOT 7?
!         SYSTEM_ENERGY_SALES = SYSTEM_ENERGY_SALES +
!     +            1000.*CPL_JURISDICTIONAL_INFOR(
!     +                                MO,SALES_CLASSIFICATION,CPL MWhrs)
!      ENDDO
...
!  CPL_JURISDICTIONAL_INFOR(MO,SALES_CLASSIFICATION,CPL MWhrs)
      SYSTEM_PEAK_12SUM = 0.
...
!
     ...
     +         CPL_JURISDICTIONAL_INFOR(MO,WHOLE_SALES_CLASS,CPL Demand)
!
...
C***********************************************************************
     ...
     +                                     MAX_ASSET_CLASS_NUM > 0) THEN
            ...
            IF(ASSET_CLASS > 0 .OR. R_CLASS <= 0) THEN
               DO MO = 0,12
                  R_CLASS_CUSTOMERS(MO) =
     +                                  SUM(ASSET_CLASS_CUSTOMERS(MO,:))
...
C
         ...
         IF(EXTYPE < 9 .OR. EXTYPE > LAST_EXPENSE_ITEM) RETURN
C
c$ifdefined(not_use_this_code)
!         IF(R_EXPENSE_COLLECTION(1:1)=='A') THEN
!            REVENUES_MONTHLY(0,R_CLASS,Adjustment Clause) =
!     +                     R_EXPENSES_ELIM/(1.-OPREV_TAX_RATE) +
!     +                     REVENUES_MONTHLY(0,R_CLASS,Adjustment Clause)
!         ENDIF
c$endif
C
...
C
     ...
     +                            SC_CUMULATIVE_ENERGY_SALES,
     +                            DUMMY_CHR, ! 21
     +                            (SC_PURCHASE_POWER_ALLOCATION(YR),
     +                                                         YR=1,10),
     +                            (NC_PURCHASE_POWER_ALLOCATION(YR),
     +                                                         YR=1,10),
...
C
     ...
     +                                R_YEAR,DEF_CLASS_TYPE)
         CPL_JURISDICTIONAL_INFOR(0,SC Fuel Clause,
     +                                    CPL Cumulative Fuel Cost) =
     +                                           SC_CUMULATIVE_FUEL_COST
         CPL_JURISDICTIONAL_INFOR(0,SC Fuel Clause,
     +                                 CPL Cumulative Energy Sales) =
     +                                        SC_CUMULATIVE_ENERGY_SALES
         CPL_JURISDICTIONAL_INFOR(0,SC Fuel Clause,
     +                          CPL Cumulative Jursidication Sales) =
     +                                         SC_CUMULATIVE_JURIS_SALES
C
         SC_PUR_POWER_ALLOCATION =
     +                 SC_PURCHASE_POWER_ALLOCATION(MIN(R_YEAR,10))/100.
         NC_PUR_POWER_ALLOCATION =
     +                 NC_PURCHASE_POWER_ALLOCATION(MIN(R_YEAR,10))/100.
     ...
     +                         CPL_OFF_SYS_ENRG_RATE(MO))
            IF(OFF_SYSTEM_RATES_ACTIVE(MIN(R_YEAR,7)) == 'Y') THEN
               OFF_SYSTEM_ENRG_RATE =
     +                       CPL_OFF_SYSTEM_FUEL_RATES(MO,MIN(R_YEAR,7))
            ELSE
...
C
         IF(USE_BUDGET_FORECAST_IN(MIN(7,R_YEAR)) == 'Y') THEN
...
C
            CPL_JURISDICTIONAL_INFOR(MO,SC Fuel Clause,CPL MWhrs) =
     +           CPL_JURISDICTIONAL_INFOR(MO,SC Retail,CPL MWhrs)
     +           + CPL_JURISDICTIONAL_INFOR(MO,NC Retail,CPL MWhrs)
     +           + CPL_JURISDICTIONAL_INFOR(MO,Wholesale,CPL MWhrs)
     +           + CPL_NCEMC_MWH(MO)/1000.
            CPL_JURISDICTIONAL_INFOR(MO,SC Fuel Clause,CPL Fuel Cost) =
     +                                  (CPL_GEN_COST(MO)
     +                                   + CPL_NUC_COST(MO)
!    +                                   - CPL_PA_ENTITLE(MO)  ! removed per Kathy 3/23/99
     +                                   + CPL_PUR_POWER(MO) *
     +                                           SC_PUR_POWER_ALLOCATION
     +                                   - CPL_OFF_SYS_INC_COST(MO)
     ...
     +               + CPL_JURISDICTIONAL_INFOR(MO,SC Fuel Clause,
     +                                                    CPL Fuel Cost)
               SC_CUMULATIVE_ENERGY_SALES = SC_CUMULATIVE_ENERGY_SALES +
     +             CPL_JURISDICTIONAL_INFOR(MO,SC Fuel Clause,CPL MWhrs)
               SC_CUMULATIVE_JURIS_SALES = SC_CUMULATIVE_JURIS_SALES +
     +            CPL_JURISDICTIONAL_INFOR(MO,SC Retail,CPL MWhrs)/1000.
            ENDIF
            CPL_JURISDICTIONAL_INFOR(MO,SC Fuel Clause,
     +                                CPL Average Fuel Cost) = 1000.*
     +        CPL_JURISDICTIONAL_INFOR(MO,SC Fuel Clause,CPL Fuel Cost)/
     +        CPL_JURISDICTIONAL_INFOR(MO,SC Fuel Clause,CPL MWhrs)
            CPL_JURISDICTIONAL_INFOR(MO,SC Fuel Clause,
     +                                   CPL Jurisdictional Fuel Cost) =
     +       CPL_JURISDICTIONAL_INFOR(MO,SC Fuel Clause,
     +                                          CPL Average Fuel Cost) *
     +            CPL_JURISDICTIONAL_INFOR(MO,SC Retail,CPL MWhrs)/1000.
C
! NC FUEL CALCULATIONS
C
            CPL_JURISDICTIONAL_INFOR(MO,NC Fuel Clause,CPL MWhrs) =
     +                CPL_JURISDICTIONAL_INFOR(MO,SC Retail,CPL MWhrs) +
     +                CPL_JURISDICTIONAL_INFOR(MO,NC Retail,CPL MWhrs) +
     +                CPL_JURISDICTIONAL_INFOR(MO,Wholesale,CPL MWhrs) +
     +                (CPL_NCEMPA_MWH(MO)+CPL_NCEMC_MWH(MO))/1000.
            CPL_JURISDICTIONAL_INFOR(MO,NC Fuel Clause,CPL Fuel Cost) =
     +                             (CPL_GEN_COST(MO)
     +                              + CPL_NUC_COST(MO)
     +                              + CPL_PUR_POWER(MO) *
     +                                           NC_PUR_POWER_ALLOCATION
     +                              - CPL_OFF_SYS_INC_COST(MO))/1000000.
            CPL_JURISDICTIONAL_INFOR(MO,NC Fuel Clause,
     +                                CPL Average Fuel Cost) = 1000.*
     +        CPL_JURISDICTIONAL_INFOR(MO,NC Fuel Clause,CPL Fuel Cost)/
     +        CPL_JURISDICTIONAL_INFOR(MO,NC Fuel Clause,CPL MWhrs)
     ...
     +            CPL_JURISDICTIONAL_INFOR(MO,NC Fuel Clause,
     +                                          CPL Average Fuel Cost) *
     +            CPL_JURISDICTIONAL_INFOR(MO,NC Retail,CPL MWhrs)/1000.
         ENDDO
C
...
C
     ...
     +                              MO == NC_FUEL_FACTOR_ADJ_MONTH) THEN
            ...
            ENDIF
            CPL_JURISDICTIONAL_INFOR(MO,NC Fuel Clause,
     +                       CPL Base Fuel Factor) = NC_BASE_FUEL_FACTOR
            CPL_JURISDICTIONAL_INFOR(MO,NC Fuel Clause,
     +                CPL Jurisdictional Revenue) = NC_BASE_FUEL_FACTOR*
     +            CPL_JURISDICTIONAL_INFOR(MO,NC Retail,CPL MWhrs)/1000.
            CPL_JURISDICTIONAL_INFOR(MO,NC Fuel Clause,
     +                                   CPL EMF Factor) = NC_EMF_FACTOR
            CPL_JURISDICTIONAL_INFOR(MO,NC Fuel Clause,
     +                                CPL EMF Revenues) = NC_EMF_FACTOR*
     +            CPL_JURISDICTIONAL_INFOR(MO,NC Retail,CPL MWhrs)/1000.
            CPL_JURISDICTIONAL_INFOR(MO,NC Fuel Clause,
     +                                   CPL Jurisdictional Fuel Cost) =
     +           CPL_JURISDICTIONAL_INFOR(MO,NC Retail,CPL MWhrs)*
...
C
            CPL_JURISDICTIONAL_INFOR(MO,NC Fuel Clause,
     +                                          CPL Deferral Recovery) =
     +            CPL_JURISDICTIONAL_INFOR(MO,NC Fuel Clause,
     +                                     CPL Jurisdictional Revenue) -
     +            CPL_JURISDICTIONAL_INFOR(MO,NC Fuel Clause,
     +                                     CPL Jurisdictional Fuel Cost)
C
...
C
            CPL_JURISDICTIONAL_INFOR(MO,NC Fuel Clause,
     +                                      CPL EMF Beginning Balance) =
     +                                               NC_EMF_FUEL_BALANCE
            NC_EMF_FUEL_BALANCE = NC_EMF_FUEL_BALANCE +
     +            CPL_JURISDICTIONAL_INFOR(MO,NC Fuel Clause,
     +                                            CPL Deferral Recovery)
            CPL_JURISDICTIONAL_INFOR(MO,NC Fuel Clause,
     +                                         CPL EMF Ending Balance) =
     +                                              NC_EMF_FUEL_BALANCE
C
! DEFERRED FUEL SUMMARY
C
            CPL_JURISDICTIONAL_INFOR(MO,NC Fuel Clause,
     +                                          CPL Beginning Balance) =
     +                                          NC_DEFERRED_FUEL_BALANCE
...
C
            IF(CURRENT_YEAR >= NC_CALCULATION_START_YR .AND.
     +                              MO == NC_FUEL_FACTOR_CAL_MONTH) THEN
...
C
!        CPL_JURISDICTIONAL_INFOR(0,SC Fuel Clause,
!    +                                    CPL Cumulative Fuel Cost) = 0.
!        CPL_JURISDICTIONAL_INFOR(0,SC Fuel Clause,
!    +                                 CPL Cumulative Energy Sales) = 0.
!        CPL_JURISDICTIONAL_INFOR(0,SC Fuel Clause,
!    +                          CPL Cumulative Jursidication Sales) = 0.
         DO MO = 1, 12
     ...
     +                SC_DEFERRED_FUEL_BALANCE/SC_CUMULATIVE_JURIS_SALES
            ...
            ENDIF
            CPL_JURISDICTIONAL_INFOR(MO,SC Fuel Clause,
     +                       CPL Base Fuel Factor) = SC_BASE_FUEL_FACTOR
            CPL_JURISDICTIONAL_INFOR(MO,SC Fuel Clause,
     +                CPL Jurisdictional Revenue) = SC_BASE_FUEL_FACTOR*
     +            CPL_JURISDICTIONAL_INFOR(MO,SC Retail,CPL MWhrs)/1000.
            CPL_JURISDICTIONAL_INFOR(MO,SC Fuel Clause,
     +                                   CPL Jurisdictional Fuel Cost) =
     +           CPL_JURISDICTIONAL_INFOR(MO,SC Retail,CPL MWhrs)*
     ...
     +                                          SC_DEFERRED_FUEL_BALANCE
            CPL_JURISDICTIONAL_INFOR(MO,SC Fuel Clause,
     +                                       CPL Cumulative Fuel Cost) =
     +         CPL_JURISDICTIONAL_INFOR(MO-1,SC Fuel Clause,
     +                                       CPL Cumulative Fuel Cost) +
     +         CPL_JURISDICTIONAL_INFOR(MO,SC Fuel Clause,CPL Fuel Cost)
            CPL_JURISDICTIONAL_INFOR(MO,SC Fuel Clause,
     +                                    CPL Cumulative Energy Sales) =
...
! WRITE REPORT
C
         CPL_JURISDICTIONAL_INFOR(0,NC Fuel Clause,
     +                                  CPL Jurisdictional Revenue) = 0.
         CPL_JURISDICTIONAL_INFOR(0,NC Fuel Clause,CPL EMF Revenues)=0.
         CPL_JURISDICTIONAL_INFOR(0,NC Fuel Clause,
     +                                       CPL Deferral Recovery) = 0.
         CPL_JURISDICTIONAL_INFOR(0,SC Fuel Clause,
     +                                  CPL Jurisdictional Revenue) = 0.
         CPL_JURISDICTIONAL_INFOR(0,SC Fuel Clause,
     +                                       CPL Deferral Recovery) = 0.
...
C
     ...
     +                 SHORT_MONTH_NAMES(MO),
     +                 ZERO_POS,
     +                (CPL_JURISDICTIONAL_INFOR(MO,SC Fuel Clause,I),
     +                                               I=1,CPL_FUEL_VARS),
     +                (CPL_JURISDICTIONAL_INFOR(MO,NC Fuel Clause,I),
     +                                               I=1,CPL_FUEL_VARS),
...
C
     ...
     +                                       CPL Jurisdictional Revenue)
C
            CPL_JURISDICTIONAL_INFOR(0,NC Fuel Clause,CPL EMF Revenues)=
     +            CPL_JURISDICTIONAL_INFOR(0,NC Fuel Clause,
     +                                                 CPL EMF Revenues)
     +            + CPL_JURISDICTIONAL_INFOR(MO,NC Fuel Clause,
     +                                                 CPL EMF Revenues)
...
C
         CASE ('NC Retail')
               CPL_SALES_CLASS = NC Retail
         CASE ('SC Retail')
               CPL_SALES_CLASS = SC Retail
         CASE ('Wholesale')
               CPL_SALES_CLASS = Wholesale
         CASE ('Other')
               CPL_SALES_CLASS = CPL Other
         CASE ('NCEMPA')
...
C**
C**
C**            DATE : DECEMBER 16, 1998
C**
C**            SOURCE FILE FOR NORTH CAROLINA POWER LINE
C**
C**            (ALLOCATION SECTION)
C**
C**--------------------------------------------------------------------
C**
C**
...
C**-------------------------------------------------------------------
      ...
      USE IREC_ENDPOINT_CONTROL
      INCLUDE 'GLOBECOM.MON'
      INTEGER*2 R_YEAR,VARIABLE_NUMBER
      LOGICAL*4 REPORT_ALLOWED
      REAL*4
...
! DETAILED REPORT OVERHEAD
      LOGICAL*1 JURIS_REPORT_NOT_OPEN/.TRUE./,WVPA,
     +          DYNAMIC_JURIS_ALLOC_ACTIVE,DYNAMIC_JURIS_ALLOC
      INTEGER*2 DYNAMIC_JURIS_RPT_HEADER,DYNAMIC_JURIS_UNIT/0/
      INTEGER DYNAMIC_JURIS_REC
      SAVE DYNAMIC_JURIS_REC
C
      INTEGER*2 YR,R_VECTOR
      REAL*4 R_DATA(30)
...
! Single Coincident Method for SC
     ...
     +            JURIS_PEAK_SUMMER_NC,      !0
     +            SYSTEM_PEAK_SUMMER,
     +            JURIS_PEAK_WINTER_NC,
     +            SYSTEM_PEAK_WINTER,
     +            JURIS_ENERGY_SALES_NC/1000.,
     +            SYSTEM_ENERGY_SALES/1000.,   ! 5
     +            JURIS_NCP_NC,
     +            SYSTEM_NCP_NC_SC,
     +            JURIS_PEAK_SUMMER_SC,
     +            JURIS_ENERGY_SALES_SC/1000.,
     +            JURIS_NCP_SC,                ! 10
     +            JURIS_PEAK_WHOLE,
     +            SYSTEM_PEAK_WHOLE,
     +            JURIS_ENERGY_SALES_WHOLE/1000.,
     +            SEPA_MW_NET_WHOLE,
     +            DEMAND_FACTOR_NC*100.,       ! 15
     ...
     +            JURIS_PEAK_SUMMER_WHOLE,
     +            JURIS_PEAK_SUMMER_NCEMPA, ! 37
     +            JURIS_PEAK_SUMMER_NCEMC,
     +            JURIS_PEAK_WINTER_SC,
     +            JURIS_PEAK_WINTER_WHOLE,
     +            JURIS_PEAK_WINTER_NCEMPA,     ! 41
     +            JURIS_PEAK_WINTER_NCEMC,
     +            JURIS_ENERGY_SALES_NCEMPA/1000.,
     +            JURIS_ENERGY_SALES_NCEMC/1000.,
     +            LOAD_FACTOR*100.,           !45
     +            DEMAND_FACTOR_SC_A*100.,
     +            ENERGY_FACTOR_SC_A*100.
            DYNAMIC_JURIS_REC = DYNAMIC_JURIS_REC + 1


      RETURN

      ENTRY GET_CPL_JURIS_N_SALES (
     +               R_YEAR,
     +               R_PROD_DEMAND_ALLOC_NC,
     +               R_ENERGY_ALLOC_NC,
     +               R_TRANS_DEMAND_ALLOC_NC,
...
!
     ...
     +                      LOAD_FACTOR_NC
         ...
         ENERGY_ALLOC_NC = JURIS_ENERGY_SALES_NC /
     +   SYSTEM_ENERGY_SALES_NC
...
!
         ...
         IF(SYSTEM_PEAK_SUMMER_SC /= 0.)
     +                   PROD_DEMAND_ALLOC_SC = JURIS_PEAK_SUMMER_SC/
     +                                             SYSTEM_PEAK_SUMMER_SC

         IF(SYSTEM_ENERGY_SALES_SC /= 0.)
     +                       ENERGY_ALLOC_SC = JURIS_ENERGY_SALES_SC/
     +                                            SYSTEM_ENERGY_SALES_SC

         IF(SYSTEM_PEAK_SUMMER_SC /= 0.)
     +                     TRANS_DEMAND_ALLOC_SC = JURIS_PEAK_SUMMER_SC/
     +                                             SYSTEM_PEAK_SUMMER_SC


         IF(SYSTEM_NCP_NC_SC /= 0.)
...
!
         ...
         ENERGY_ALLOC_WHOLE = JURIS_ENERGY_SALES_WHOLE /
     +                        SYSTEM_ENERGY_SALES_WHOLE
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      ...
      INCLUDE 'SIZECOM.MON'
      INCLUDE 'GLOBECOM.MON'
         INTEGER*2 LOCAL_YEAR,END_YEAR
         LOGICAL*1 CPL_IS_ACTIVE,CPL_ACTIVE
         LOGICAL*4 REPORT_ALLOWED
!
! END DATA DECLARATIONS
!
...
!
      TEMP_LEAP_YEAR = MOD (CURRENT_YEAR,  100 )
      LEAP_YEAR = MOD (TEMP_LEAP_YEAR,  4 )
...
C
      ...
      CHARACTER*256 OUTPUT_DIRECTORY,DATA_DRIVE
      LOGICAL*4 FILE_EXISTS,FILE_OPEN
      CHARACTER*5 BASE_FILE_NAME,RPS_STATE_DEMAND_FILE
! DECLARATION FOR DBREAD COMMON BLOCK
      CHARACTER*1024 RECLN
! DECLARATION FOR CLASS FORECASTS
      INTEGER*2 YEAR,DATA_RECORDS_IN_TABLE
      CHARACTER*32 CLASS_NAME,ENERGY_UNITS*3,TEMP_CLASS_NAME
      CHARACTER*16 FILE_TYPE/'RPS State Demand'/ ! leading space needed
      INTEGER*2 SAVE_CLASS_SALES_UNIT,
     +          CLASS_SALES_UNIT
      INTEGER*2 BC_TABLE_NUMBER,OL_TABLE_NUMBER
      INTEGER*2 R_NUMBER_OF_TABLES
...
C
      ...
      INTEGER*2 MO,I,IREC,R_YEAR,
     +          R_ST_TG,DELETE,
     +          SALES_UNIT,LOCAL_YEAR
      INTEGER*4 IOS
      CHARACTER*2 GREEN_MRX_METHOD
      CHARACTER*32 RATE_CLASS_NAME
      INTEGER*2   STATE_RPS_VAR_NUM/13/, ! 052817
...
C
      ...
      INTEGER*2 RC_UNIT_NO/0/
      REAL*4 THE_RATIO_OF_A_TO_B
      CHARACTER*9 CL_MONTH_NAME(0:12)
     +                         /'Annual   ',
     +                          'January  ','February ',
     +                          'March    ','April    ',
     +                          'May      ','June     ',
     ...
     +                          'September','October  ',
     +                          'November ','December '/
      LOGICAL*1 STATE_RPS_REPORT_ACTIVE,
     ...
     +          YES_OLD_RPS_PROGRAMS_REPORT
      INTEGER*2 STATE_RPS_NO,STATE_RPS_RPT_HEADER,
     +          NUM_RESOURCE_RPS_VARS/16/ ! 052817
      INTEGER*4 STATE_RPS_REC
      REAL*4 RESOURCE_RPS_VARS(16), ! 052817
     +       TOTAL_RES_REQ_RPS_VARS(16) ! 052817
      CHARACTER*20 TEMP_RPS_NAME,STATE_NAME_LOOKUP
!
! END DATA DECLARATIONS
!
...
!
            ...
            RPS_USER_DEFINED_REGION(ST) = INT2(RPS_REQS(10))
...
! RPS
     ...
     +                                       STATE_RPS_ALLOCATION * 0.01
                  STATE_RPS_DB(1,ST,MO) = STATE_RPS_DB(1,ST,MO) +
     +                                                            TEMP_R
                  STATE_RPS_DB(1,ST,0) = STATE_RPS_DB(1,ST,0) +
     +                                                            TEMP_R
!
...
!
               IF(TEMP_L .AND. ST_TG > 0) THEN
                  TEMP_R = TRANS_ENERGY(MO) * 0.001 *
     +                                       STATE_RPS_ALLOCATION * 0.01
                  STATE_RPS_DB(1,ST_TG,MO) = STATE_RPS_DB(1,ST_TG,MO) +
     +                                                            TEMP_R
                  STATE_RPS_DB(1,ST_TG,0) = STATE_RPS_DB(1,ST_TG,0) +
     +                                                            TEMP_R
!
! LOSS PERCENT (3) CALCULATED AT REPORT TIME
!
...
! RETAIL DEMAND
                  TEMP_R = TEMP_R  * (1.-ENERGY_LOSS_FACTOR/100.)
                  STATE_RPS_DB(4,ST_TG,MO) =
     +                                 STATE_RPS_DB(4,ST_TG,MO) + TEMP_R
                  STATE_RPS_DB(4,ST_TG,0) =
     +                                  STATE_RPS_DB(4,ST_TG,0) + TEMP_R
...
!                  STATE_RPS_EXEMPTION = (1.-STATE_RPS_EXEMPTION*0.01)
     ...
     +                                               STATE_RPS_EXEMPTION
                  END DO
! ALLOWED REC MWH (IMPLICITELY INDEX 13)
!                  STATE_RPS_DB(RPS,ST_TG,MO) =
!     +               STATE_RPS_DB(RPS,ST_TG,MO) +
!     +                   TEMP_R * RPS_REQS(1) * RPS_REQS(8)* 0.0001 *
!     +                                               STATE_RPS_EXEMPTION
!                  STATE_RPS_DB(RPS,ST_TG,0) =
!     +               STATE_RPS_DB(RPS,ST_TG,0) +
!     +                   TEMP_R * RPS_REQS(1) * RPS_REQS(8)* 0.0001 *
...
! 090616. FOR GRX OR MRX.
         IF (ALLOCATED(STATE_RPS_DB)) THEN
            ...
            IF(GREEN_MRX_METHOD() == 'GX') THEN
               IF(ALLOCATED(ANN_RPS_INTRA_REGION_SURPLUS))
     +                          DEALLOCATE(ANN_RPS_INTRA_REGION_SURPLUS)
               ALLOCATE(ANN_RPS_INTRA_REGION_SURPLUS(0:6,
     +                                              0:RPS_REGION_COUNT))
               ...
               DO ST = 1, 61
                  ...
                  DO MO = 1, 12
                     ...
                     IF(GRX_ITERATIONS == 0) THEN
                        TEMP_L1 = GET_TRANS_RPS_SUM(MO,ST,
     +                                                RESOURCE_RPS_VARS)
                     ELSE
                        TEMP_L1 = GET_TRANS_GRX_RPS_SUM(MO,ST,
     +                                                RESOURCE_RPS_VARS)
                     ENDIF
     ...
     +                                      STATE_RPS_DB(J+6,ST,MO)
                        IF(RPS_SURPLUS(J,ST) > 0.0) THEN ! SURPLUS
                           ANNUAL_SURPLUS_OR_DEFICIT(1,ST,J) =
     +                                                 RPS_SURPLUS(J,ST)
                        ELSE
...
!                  IF(.FALSE. .AND.
!     +                        RPS_REC_PERCENT_ALLOWED(ST) <= 0.999) THEN
...
C***********************************************************************
         IF (ALLOCATED(STATE_RPS_DB)) THEN
            RPS_SURPLUS = 0.0
            ANNUAL_SURPLUS_OR_DEFICIT = 0.0
!            STATE_RPS_REPORT_ACTIVE = TRANSACT_FUEL_REPORT()
     ...
     +                   RPS_CATEGORIES + 1
               STATE_RPS_NO = STATE_RPS_RPT_HEADER(TEMP_I2,
     +                                             STATE_RPS_REC)
               STATE_RPS_REPORT_NOT_OPEN = .FALSE.
            ENDIF
            IF(STATE_RPS_REPORT_ACTIVE .OR.
     ...
     +                                  STATE_RPS_DB(1,ST,R_MONTH))
                  ENDIF
                  TEMP_L1 = GET_TRANS_RPS_SUM(R_MONTH,ST,
     +                                                RESOURCE_RPS_VARS)
...
C
      INCLUDE 'SpinLib.MON'
      INCLUDE 'SIZECOM.MON'
      INTEGER*2 INUNIT,IREC,DELETE,LRECL/160/
      INTEGER*4 IOS,IOS_BASE
      CHARACTER*5 OVERLAY_FAMILY_NAME
      CHARACTER*10 STATE_OR_PROVINCE
      CHARACTER*256 FILE_NAME
      ...
      CHARACTER*256 OUTPUT_DIRECTORY,DATA_DRIVE
      LOGICAL*4 FILE_EXISTS,FILE_OPEN
      CHARACTER*5 BASE_FILE_NAME,THERMAL_RETROFIT_FILE
      CHARACTER*6 UNIT_ID
! DECLARATION FOR DBREAD COMMON BLOCK
      CHARACTER*1024 RECLN
! DECLARATION FOR CLASS FORECASTS
      INTEGER*2 YEAR,DATA_RECORDS_IN_TABLE
      CHARACTER*32 CLASS_NAME,ENERGY_UNITS*3,TEMP_CLASS_NAME
      CHARACTER*16 FILE_TYPE/'Thermal Retrofit     '/ ! leading space needed
      INTEGER*2 SAVE_CLASS_SALES_UNIT/0/,
     +          CLASS_SALES_UNIT
      INTEGER*2 BC_RECORD_NUMBER/0/,OL_RECORD_NUMBER/0/
      INTEGER*2 R_NUMBER_OF_RECORDS,
     +          R_MAX_PROJECT_ID,
     +          MAX_PROJECT_ID/0/
      INTEGER*4 EV_ID
...
! DATA FOR THERMAL VARIABLES
      CHARACTER*30 Project_Name ! C*30
      CHARACTER*1 Project_Active ! C*1
      CHARACTER*12 Project_Type ! C*12
      INTEGER*2    Project_ID, ! I*2
     +             First_Year_Available, ! I*2
     +             Last_Year_Available, ! I*2
     +             Capital_Cost Esc, ! I*2
     +             Var_OM_Esc, ! I*2
     +             Fixed_OM_Esc ! I*2
      REAL*4
     +             Learning_Curve, ! R*4
...
C***********************************************************************
         ...
         OPEN(CLASS_SALES_UNIT,FILE=trim(OUTPUT_DIRECTORY())//
     +          CLASS_SALES_OL//
     +               "RETROFIT.BIN",ACCESS="DIRECT",RECL=LRECL)
      RETURN
C
C***********************************************************************
...
C
      INTEGER*4 NEXT_REC/0/
      INTEGER*2 RC_UNIT_NO/0/
      INTEGER*2 RECORD_NUM,
     +          SAVE_RECORD_NUM/0/,
     +          RETROFIT_POSITION(:),
     +          R_MONTH
! DATA FOR THERMAL RETROFIT VARIABLES
      CHARACTER*30 Project_Name(:) ! C*30
      CHARACTER*1 Project_Active_CHR ! C*1
      CHARACTER*12 Project_Type(:) ! C*12
      INTEGER*2    Project_ID(:), ! I*2
     +             First_Year_Available(:), ! I*2
     +             Last_Year_Available(:), ! I*2
...
!         RETROFIT_INFO_ACTIVE = RECORD_NUM > 0
!         IF(.NOT. RETROFIT_INFO_ACTIVE) TEMP_L4=SET_RETROFIT_LOGIC_OFF()
     ...
     +                           Min_Capacity_Change, ! R*4
     +                           Max_Capacity_Change,
     +                           RETROFIT_POSITION) ! R*4
            ALLOCATE(
     +                           Project_Name(RECORD_NUM), ! C*30
     +                           Project_Active(RECORD_NUM), ! C*1
     +                           Project_Type(RECORD_NUM), ! C*12
     +                           Project_ID(RECORD_NUM), ! I*2
     +                           First_Year_Available(RECORD_NUM), ! I*2
     +                           Last_Year_Available(RECORD_NUM), ! I*2
     +                           Learning_Curve(RECORD_NUM), ! R*4
     +                           Capital_Cost_CCS(RECORD_NUM), ! R*4
     +                           Capital_Cost_Trans(RECORD_NUM), ! R*4
     +                           Capital_Cost Esc(RECORD_NUM), ! I*2
     +                           CRR_On_Project(RECORD_NUM), ! R*4
     +                           Var_OM_Adder(RECORD_NUM), ! R*4
     +                           Var_Storage_OM_Adder(RECORD_NUM), ! R*4
     +                           Var_OM_Esc(RECORD_NUM), ! I*2
     +                           Fixed_OM_Capture_Adder(RECORD_NUM), ! R*4
     +                           Fixed_OM_Trans(RECORD_NUM), ! R*4
     +                           Fixed_OM_Esc(RECORD_NUM), ! I*2
     +                           SO2_Control_Percent(RECORD_NUM), ! R*4
     +                           NOx_Control_Percent(RECORD_NUM), ! R*4
     +                           CO2_Control_Percent(RECORD_NUM), ! R*4
...
!         DO IREC = 1, min(325,SAVE_RECORD_NUM)
!            IREC = IREC + 1
!            IF(IREC > RECORD_NUM) THEN
!               WRITE(4,*) "MISCOUNT IN RPS REGIONS"
!               EXIT
!            ENDIF
            READ(SALES_UNIT,REC=IREC,IOSTAT=IOS)
     +                           DELETE,
     +                           Project_Name(IREC), ! C*30
     +                           Project_Active_CHR, ! C*1
     +                           Project_Type(IREC), ! C*12
     +                           Project_ID(IREC), ! I*2
     +                           First_Year_Available(IREC), ! I*2
     +                           Last_Year_Available(IREC), ! I*2
     +                           Learning_Curve(IREC), ! R*4
     +                           Capital_Cost_CCS(IREC), ! R*4
     +                           Capital_Cost_Trans(IREC), ! R*4
     +                           Capital_Cost Esc(IREC), ! I*2
     +                           CRR_On_Project(IREC), ! R*4
     +                           Var_OM_Adder(IREC), ! R*4
     +                           Var_Storage_OM_Adder(IREC), ! R*4
     +                           Var_OM_Esc(IREC), ! I*2
     +                           Fixed_OM_Capture_Adder(IREC), ! R*4
     +                           Fixed_OM_Trans(IREC), ! R*4
     +                           Fixed_OM_Esc(IREC), ! I*2
     +                           SO2_Control_Percent(IREC), ! R*4
     +                           NOx_Control_Percent(IREC), ! R*4
     +                           CO2_Control_Percent(IREC), ! R*4
...
! END RETROFIT
      RETURN
C***********************************************************************
      ENTRY GET_RETRO_AVAIL_PERIOD(R_Retro_Project_First_Year_Avail,
     +                             R_Retro_Project_Last_Year_Avail)
         R_Retro_Project_First_Year_Avail=Retro_Project_First_Year_Avail
...
C***********************************************************************
     ...
     +                             R_RETROFIT_ID <= MAX_PROJECT_ID) THEN
            ...
            IF(Var_OM_Esc(R_RETROFIT_ID) > 0) THEN
               VOM_ESC =
     +              ESCALATED_VECTOR_OR_RETURN_VALUE(VECTOR_TYPE_ESCAL,
...
!      INTEGER IOS
!
      INTEGER*2   SAVE_NUMBER_OF_TRAN_EXPS/0/,
     ...
     +            TRAN_EXP_OWNER
      ...
      CHARACTER*1  TRAN_EXP_ACTIVE
      LOGICAL*4    FILE_EXISTS/.FALSE./,TRAN_EXP_FILE_EXISTS/.FALSE./,
     +             R_TRAN_EXP_FILE_EXISTS
! DECLARATION FOR DBREAD COMMON BLOCK
!      CHARACTER*1024 RECLN
! DECLARATION FOR TRANSACT TRAN_EXP DETERMINANTS
      CHARACTER*16 FILE_TYPE/'TRANSMISSION EXPAND  '/
      CHARACTER*2  TRAN_EXP_OL/'BC'/
      LOGICAL*1 LAHEY_LF95
      CHARACTER*30 SCREEN_OUTPUT
...
C
      ...
      INCLUDE 'SIZECOM.MON'
      SAVE
      LOGICAL*1   SAVE_TRAN_EXP_FILE_EXISTS/.FALSE./,
     ...
     +            R_MONTH,R_YEAR
      ...
      INTEGER*2   GET_HOUR_TRAN_EXPS_INDEX,
...
!     +            SET_HOUR_LONG_TRAN_EXP_PARAMS,
     ...
     +            R_CONGESTION_QUANT
      ...
      LOGICAL*1   MANAGE_TRAN_EXP_FORECASTS,TRANS_GROUP_ACTIVE_SWITCH,
     ...
     +            ACTIVE_SUPPLY_NODE(:),
     +            TEST_TRAN_EXP_CF
      LOGICAL*4   TRAN_EXP_FILE_EXISTS
      CHARACTER*1 TRAN_EXP_ACTIVE
!
!
      LOGICAL*1 SET_HOUR_LONG_TRAN_EXP_WI_TRAN_EXPS,
     +          SET_HOUR_LONG_CAP_TWH

      INTEGER*2 TRAN_EXPS_LONG_TRAN_EXPS/0/,L_BUYER,L_SELLER
      REAL*4    L_CAPACITY
!
...
!
      ...
      integer*2 iLine,iNode,jNode,TRAN_EXPID
...
!
     ...
     +                                MAX_OUT_TRAN_EXPS), ! MAX COUNT FOR J,K BY MAX TOTAL LINKS INCL. RESERVE
...
C
      ...
      INCLUDE 'SIZECOM.MON'
      SAVE
      LOGICAL*1 MANAGE_RPS_SUPPLY_CURVES,
     ...
     +          R_RPS_RESOURCE_COUNT
      REAL*4 RPS_SUPPLY_CURVES(:,:,:), ! ST, TECH, MAX RESOURCES
     +       SYSTEM_RPS_SUPPLY_CURVES(:),
     +       R_RESOURCE_MWHS(*),
     +       R_RESOURCE_REV_PER_MWH(*),
     +       RPS_NET_REV(:),
     +       RPS_MWHS(:)
      CHARACTER*20 R_OPTION_NAME(*),RPS_OPTION_NAME(:)
...
C***********************************************************************
         ...
         NUM_RPS_RESOURCES = 0
         IF(ALLOCATED(RESOURCES_PER_TYPE))
     +                              DEALLOCATE(RESOURCES_PER_TYPE,
     +                                       RPS_RESOURCES_INDEX)
...
!
      ...
      INCLUDE 'SIZECOM.MON'
      INTEGER*2   UNIT_NUM/10/
      INTEGER*2 INUNIT,IREC,LRECL/300/
      INTEGER*2   SAVE_NUMBER_OF_CO2_PARAM/0/,
     +            R_NUMBER_OF_CO2_PARAM
      ...
      CHARACTER*256 BASE_FILE_DIRECTORY
      CHARACTER*256 DATA_DRIVE,OUTPUT_DIRECTORY
      CHARACTER*1  CO2_PARAM_ACTIVE
      LOGICAL*4    FILE_EXISTS/.FALSE./,CO2_PARAM_FILE_EXISTS/.FALSE./,
     +             R_CO2_PARAM_FILE_EXISTS
! DECLARATION FOR TRANSACT CO2_PARAM DETERMINANTS
      CHARACTER*21 FILE_TYPE/'CO2 Offset Markets   '/
      CHARACTER*2  CO2_PARAM_OL/'BC'/
      LOGICAL*1 LAHEY_LF95
      CHARACTER*30 SCREEN_OUTPUT
!
     ...
     +                    CO2_GRX_Iterations_After_Convergence
...
C
     ...
     +                            .AND. GREEN_MRX_METHOD() == 'GX') THEN
            CALL MG_LOCATE_WRITE(20,0,'For GRX planning a CO2 '//
...
C
      ...
      INCLUDE 'SIZECOM.MON'
      SAVE
      LOGICAL*1   SAVE_CO2_PARAM_FILE_EXISTS/.FALSE./,
...
!
     ...
     +               Dom_Min_Off_Sets_To_Buy(YR1_DATA:YR2_DATA),
     ...
     +         CO2_GRX_Iterations_After_Convergence(YR1_DATA:YR2_DATA),
     ...
     +                               YR,
     +                               CO2_Emission_Cap(YR),
     +                               Residential_Emissions(YR),
     +                               Commercial_Emission(YR),
     +                               Industrial_Emissions(YR),
     ...
     +                               CO2_Banking(YR),
     +                               Dom_Price_Equation_1_Type(YR),
     +                               Dom_1_A_Coefficient(YR),
     +                               Dom_1_B_Coefficient(YR),
     +                               Dom_Price_Break(YR),
     +                               Dom_Price_Equation_2_Type(YR),
     +                               Dom_2_A_Coefficient(YR),
     +                               Dom_2_B_Coefficient(YR),
     +                               Int_Price_Equation_1_Type(YR),
     +                               Int_1_A_Coefficient(YR),
     +                               Int_1_B_Coefficient(YR),
     +                               Int_Price_Break(YR),
     +                               Int_Price_Equation_2_Type(YR),
     +                               Int_2_A_Coefficient(YR),
     +                               Int_2_B_Coefficient(YR),
     +                               Non_El_Price_Equation_1_Type(YR),
     +                               Non_El_1_A_Coefficient(YR),
     +                               Non_El_1_B_Coefficient(YR),
     +                               Non_El_Price_Break(YR),
     +                               Non_El_Price_Equation_2_Type(YR),
     +                               Non_El_2_A_Coefficient(YR),
     +                               Non_El_2_B_Coefficient(YR),
     +                               COMMENT,
     +                               CO2_Price_Forecast(YR),
     +                               CO2_Price_Evaluation_Switch,
     +                               CO2_Max_Price_Cap(YR),
     +                               CO2_Min_Price(YR),
     +                               CO2_Min_GRX_Iterations(YR),
     +                               CO2_Max_GRX_Iterations(YR),
     +                         CO2_GRX_Iterations_After_Convergence(YR),
     +                               CO2_Market_Available_Switch
     ...
     +                             CO2_Market_Available_Switch(1:1)=='A'
            POKE_CURRENT_CO2_DISPATCH_COST(YR) =
     +                  INDEX(CO2_Price_Evaluation_Switch,'REGION') == 0
            IF(.not. (CO2_Min_GRX_Iterations(YR) == 0 .AND.
     +                            CO2_Max_GRX_Iterations(YR) == 0)) THEN
               CO2_Min_GRX_Iterations(YR) =
     +                                 MAX(1,CO2_Min_GRX_Iterations(YR))
               CO2_Max_GRX_Iterations(YR)=
     +                           MIN(20,MAX(CO2_Max_GRX_Iterations(YR),
     +                                      CO2_Min_GRX_Iterations(YR)))
            ENDIF
            I = I + 1
         ENDDO
...
!
      INCLUDE 'SPINLIB.MON'
      INCLUDE 'SIZECOM.MON'
	  character*256 :: alt_filename
      INTEGER*2   UNIT_NUM/10/
      INTEGER*2 INUNIT,IREC,LRECL/245/
!
      INTEGER*2   SAVE_NUMBER_OF_RPS_PROGRAMS/0/,
     +            R_NUMBER_OF_RPS_PROGRAMS
      ...
      CHARACTER*256 BASE_FILE_DIRECTORY
      CHARACTER*256 DATA_DRIVE,OUTPUT_DIRECTORY
      CHARACTER*1  RPS_PROGRAM_ACTIVE
      LOGICAL*4 ::  FILE_EXISTS=.FALSE.,
     +             RPS_PROGRAM_FILE_EXISTS=.FALSE.
      logical*4 ::  R_RPS_PROGRAM_FILE_EXISTS
! DECLARATION FOR TRANSACT RPS_PROGRAM DETERMINANTS
      CHARACTER*16 FILE_TYPE/'RPS PROGRAM          '/
      CHARACTER*2  RPS_PROGRAM_OL/'BC'/
      LOGICAL*1 LAHEY_LF95
      CHARACTER*30 SCREEN_OUTPUT
...
C***********************************************************************
	  ...
	  alt_filename=trim(BASE_FILE_DIRECTORY())//
	 ...
	 if(trim(file_name)/=trim(alt_filename)) THEN
      ...
      IF(FILE_EXISTS) THEN
         ...
         OPEN(10,FILE=FILE_NAME)
         OPEN(11,FILE=trim(DATA_DRIVE)//
     +                      "BCRPS_PROGRAM.BIN",ACCESS="DIRECT",
     +                                      STATUS="UNKNOWN",RECL=LRECL)
         IREC = 1
!
!        Initialize values
...
C
     ...
     +                                                       RECL=LRECL)
      ...
      ENDIF
      OPEN(12,FILE=trim(DATA_DRIVE)//
     +                              "OLRPS_PROGRAM.BIN",ACCESS="DIRECT",
     +                                      STATUS="UNKNOWN",RECL=LRECL)
      IREC = 0
!
      READ(10,1000,IOSTAT=IOS) RECLN
...
!
      ...
      INCLUDE 'SIZECOM.MON'
      SAVE
      LOGICAL*1   SAVE_RPS_PROGRAM_FILE_ACTIVE/.FALSE./,
     +            MANAGE_RPS_PROGRAM_FORECASTS,
     +            RPS_PROGRAM_FILE_EXISTS,
     +            PUT_RPS_ENRG_CAP,
...
!            PROGRAM_INDEX(ACTIVE_RPS) = Program_Number(ACTIVE_RPS)
            MAX_PROGRAM_NUMBER = MAX(MAX_PROGRAM_NUMBER,
     +                                       Program_Number(ACTIVE_RPS))
...
!   arrays to capacity_arrays module
            ...
            DO CURRENT_RECORD = 1, ACTIVE_RPS
               PROGRAM_POSITION(Program_Number(CURRENT_RECORD)) =
     +                                                    CURRENT_RECORD
            ENDDO
...
! 091820. DEPENDS ON COUNT.
            ...
            COUNT_RPS = INT2(GET_ANNUAL_VECTOR_VALUE(R_RPS_NO,I))
! 062721
...
C***********************************************************************
         IF(R_RPS_NO /= 0 .AND. R_PM /= 13 .AND.
     +                                SAVE_RPS_PROGRAM_FILE_ACTIVE) THEN
            GET_QUALIFYING_GEN_DB =
     +                          QUALIFYING_GEN_DB(R_RPS_NO,R_PM,R_INDEX)
         ELSE
...
C***********************************************************************
     ...
     +                                SAVE_RPS_PROGRAM_FILE_ACTIVE) THEN
            ...
            DO
               ...
               ELSE
                  COUNT_RPS =
     +                    INT2(GET_ANNUAL_VECTOR_VALUE(R_RPS_NO,I))
!     +               INT2(ESCALATED_MONTHLY_VALUE(FLOAT(COUNT_RPS),
!     +                             ABS(R_RPS_NO),I,INT2(1),INT2(1)))
               ENDIF
               IF(COUNT_RPS > 0 .AND.
     +                             COUNT_RPS <= MAX_PROGRAM_NUMBER) THEN
! DOUBLE INDEX
                  COUNT_RPS = PROGRAM_POSITION(COUNT_RPS)
                  IF(COUNT_RPS > 0 .AND.
     +                                     COUNT_RPS <= ACTIVE_RPS) THEN
!     +                             COUNT_RPS <= MAX_PROGRAM_NUMBER) THEN
                     QUALIFYING_GEN_DB(COUNT_RPS,R_PM,1) =
     +                     QUALIFYING_GEN_DB(COUNT_RPS,R_PM,1) + R_ENRG
                     QUALIFYING_GEN_DB(COUNT_RPS,R_PM,2) =
     +                     QUALIFYING_GEN_DB(COUNT_RPS,R_PM,2) + R_CAP
!
                     QUALIFYING_GEN_DB(COUNT_RPS,0,1) =
     +                     QUALIFYING_GEN_DB(COUNT_RPS,0,1) + R_ENRG
                     QUALIFYING_GEN_DB(COUNT_RPS,0,2) =
     +                     QUALIFYING_GEN_DB(COUNT_RPS,0,2) + R_CAP
                     IF(I < 30) THEN
                        I = I + 1
                     ELSE
                        STOP_RPS = .TRUE.
...
! DOUBLE INDEX
     ...
     +                              SAVE_GRX_QUAL_GEN_DB(COUNT_RPS,0,1))
                     TEMP_R4 =
     +                   MAX(0.0,MIN(TEMP_R4,R_ENRG))
                     IF(TEMP_R4 > 0.0001) THEN
                        GET_RPS_REV_BY_OPTION = GET_RPS_REV_BY_OPTION +
     +                            TEMP_R4 * LOCAL_ALT_PRICE(COUNT_RPS)
                        MRX_TEST_REQUIREMENT(COUNT_RPS) =
     +                         MRX_TEST_REQUIREMENT(COUNT_RPS) + TEMP_R4
                     ENDIF
                     IF(I < 30) THEN
                        I = I + 1
                     ELSE
...
C***********************************************************************
         ...
         IF(R_RPS_NO /= 0 .AND. SAVE_RPS_PROGRAM_FILE_ACTIVE) THEN
            MRX_GEN_WEIGHTED_REC_PRICE(R_RPS_NO) =
     +                                          R_GEN_WEIGHTED_REC_PRICE
...
! NEED TO ACCOUNT FOR MULTIPLE MARKETS CONTRIBUTING TO THE REC PRICE
     ...
     +                     GRX_QUAL_GEN_DB(COUNT_RPS,0,1) + R_ENRG
                     GRX_QUAL_GEN_DB(COUNT_RPS,R_PM,1) =
     +                     GRX_QUAL_GEN_DB(COUNT_RPS,R_PM,1) + R_ENRG
                     GRX_QUAL_GEN_DB(COUNT_RPS,R_PM,2) =
     +                     GRX_QUAL_GEN_DB(COUNT_RPS,R_PM,2) + R_CAP
                     GRX_QUAL_GEN_DB(COUNT_RPS,0,2) =
     +                     GRX_QUAL_GEN_DB(COUNT_RPS,0,2) + R_CAP
                     TEMP_R4 = QUALIFYING_GEN_DB(COUNT_RPS,0,1) +
     +                           GRX_QUAL_GEN_DB(COUNT_RPS,0,1) -
     +                              SAVE_GRX_QUAL_GEN_DB(COUNT_RPS,0,1)
                     MRX_GEN_WEIGHTED_REC_PRICE(COUNT_RPS) =
     +                                          R_GEN_WEIGHTED_REC_PRICE
! 082120
...
!                        MRX_REC_PRICE(COUNT_RPS) = R_ALT_PRICE
!                     ELSEIF( TEMP_R4 >
!     +                        Annual_Requirement(COUNT_RPS,R_YEAR) .AND.
!     +                    TEMP_R4 - R_ENRG <
!     +                        Annual_Requirement(COUNT_RPS,R_YEAR)) THEN
!                        MRX_REC_PRICE(COUNT_RPS) = R_ALT_PRICE
!                     ELSEIF(TEMP_R4 >
!     +                        Annual_Requirement(COUNT_RPS,R_YEAR)) THEN
!                        MRX_REC_PRICE(COUNT_RPS) = 0.0
!                     ELSE
!                        MRX_REC_PRICE(COUNT_RPS) =
!     +                                       LOCAL_ALT_PRICE(COUNT_RPS)
!                     ENDIF
...
C***********************************************************************
         ...
         IF(.NOT. SAVE_RPS_PROGRAM_FILE_ACTIVE .OR.
     +                                            ACTIVE_RPS < 1) RETURN
...
!         GET_LOCAL_ENDING_BALANCE = NEXT_ITER_REQUIREMENT(R_RPS_NO)
...
C***********************************************************************
     ...
     +                         ABS(INT2(Alt_Compliance_Price(I))),
...
!
         IF(YES_RPS_PROGRAMS_REPORT() .AND.
     +                 RPS_PROGRAMS_REPORT_NOT_OPEN) THEN
...
! NEED CARRY-FORWARD BALANCE
     ...
     +                                     Annual_Requirement(I,R_YEAR))
            ...
            LOCAL_REC_REVENUE = LOCAL_REC_PRICE(I) *
     +                                          QUALIFYING_GEN_DB(I,0,1)
