	  ...
	  use startup_tasks
	  IMPLICIT NONE
	  CHARACTER (LEN=256):: COMMAND_LINE, cl

		cl=trim(command_line)
...
C
     ...
     +                      idm_mg_about = 20
      ...
      INTEGER*2 INVERSE_VIDEO
      LOGICAL*1 USER_TERMINATED
       character*25 :: build_num
...
C***********************************************************************
     ...
     +              SIM_MONTH_DAY
      ...
      CHARACTER*25 BUILD_NUM
      REAL, DIMENSION(7) :: Status_Portion
     +                                    /.07,.20,.47,.045,.05,.12,.07/
