...
C              
     ...
     +                 CASH_EXPENSES_PAID
      ...
      PARAMETER(VARIABLE_NUMBER=950) !next unused 734 RANGE FROM 0 TO VARIABLE_NUMBER-1
      REAL*4 NOT_AVAIL
...
C
     ...
     +          VOID_LOGICAL,
     +          WVPA_MONTHLY_MEMBER_ACCRUED_REVS
...
C
     ...
     +       LTD_BOOKED_ISSUED_INTEREST(0:12)
      REAL*4 ATL_TAXABLE_INCOME(0:12), ! PROXIE FOR CALCULATING A PATTERN
...
C***********************************************************************
     ...
     +       OCI_MONTHLY_RETIREMENT_FUND_RATE(0:12),
...
C
     ...
     +                   + INCOME_LINKED_LEVEL(:,
     +                          Monthly Model Calculated Property Taxes,
     +                                                      CLASS_LEVEL)
...
C
     ...
     +                       INCOME_VARIABLES(MO,Monthly Total Revenues)
     ...
     +                        TEMP_PAYABLE_MONTHLY_VALUES(:,:,CLASS_POS)
         CASH_VARIABLES(:,Cash Received Accounts Receivable) =
     +             PAYABLE_MONTHLY_VALUES(:,payment_revenues_receivable,
     +                                                        CLASS_POS)
         PAYABLE_MONTHLY_VALUES(:,payment_revenues_receivable,
     +                                                   CLASS_POS) = 0.
         CASH_VARIABLES(:,Cash Paid on Accounts Payable) =
...
c
            INCOME_VARIABLES(:,Monthly Leased Nuclear Fuel Expense) =
     +                  INCOME_VARIABLES(:,Monthly Nuclear Fuel Expense)
            INCOME_VARIABLES(:,Monthly Nuclear Fuel Expense) = 
     +         INCOME_VARIABLES(:,Monthly Owned Nuclear Fuel Expense)
     +         + INCOME_VARIABLES(:,Monthly Leased Nuclear Fuel Expense)
            INCOME_VARIABLES(:,Monthly Tax Expense Nuclear Fuel) = 
     +           INCOME_VARIABLES(:,Monthly Leased Nuclear Fuel Expense)
     +           + INCOME_VARIABLES(:,Monthly DOE Disposal)
     ...
     +                              PURCHASE_POWER_IN_DEFERRED_FUEL = 1.
            IF(USE_DEFERRED_FUEL_ACCOUNTING) THEN
               IF(CPL_ACTIVE()) THEN
                  DO MO = 0, 12
                     INCOME_VARIABLES(MO,Adjustment Clause) =
     +                            INCOME_VARIABLES(MO,Adjustment Clause)
     +                            + CPL_MONTHLY_FUEL_REVENUES(MO)
                     INCOME_VARIABLES(MO,Monthly Deferred Fuel Expense)=
     +                     INCOME_VARIABLES(MO,
     +                                    Monthly Deferred Fuel Expense)
     +                     + MONTHLY_DEFERRED_FUEL_EXPENSE(MO)
                     CASH_VARIABLES(MO,Cash Adjustment Clause) =
     +                         CASH_VARIABLES(MO,Cash Adjustment Clause)
     +                         + CPL_CASH_FUEL_REVENUES(MO)
               ...
               ELSE
                  DO MO = 0, 12
                     INCOME_VARIABLES(MO,Monthly Deferred Fuel Expense)=
     +                 INCOME_VARIABLES(MO,Adjustment Clause)
     +                 - INCOME_VARIABLES(MO,Monthly Fossil Fuel)
     +                 - INCOME_VARIABLES(MO,
...
C
               ...
               DO MO = 0, 12
                  INCOME_VARIABLES(MO,Monthly Catawba Expenses) =
     +                     INCOME_VARIABLES(MO,Monthly Catawba Expenses)
     +                     + CATAWBA_EXPENSES(MO)
                  INCOME_VARIABLES(MO,Monthly Catawba Revenues) =
     +                     INCOME_VARIABLES(MO,Monthly Catawba Revenues)
     +                     + CATAWBA_REVENUES(MO)
                  IF(MO > 0) THEN
                     IF(MO < 12) THEN
...
C               DO MO = 0, 12
     ...
     +                 INCOME_VARIABLES(:,Monthly Other ATL Amort)
     ...
     +                 + INCOME_VARIABLES(:,Monthly Other BTL Amort)
     +                 + INCOME_VARIABLES(:,
     +                          Monthly Debit File Purchase Power Amort)
     +                 + INCOME_VARIABLES(:,
     +                                Monthly Debit File Interest Amort)
     +                 + INCOME_VARIABLES(:,
     +                                 Monthly Debt File Interest Amort)
C
C LOWER LEVEL DEBIT AMORTZATION WAS NOT BEING PASSED UP.
C
     +                 + INCOME_LINKED_LEVEL(:,
     +                              Monthly Other ATL Amort,CLASS_LEVEL)
     ...
     +                                                      CLASS_LEVEL)
     +                 + INCOME_LINKED_LEVEL(:,
     +                              Monthly Other BTL Amort,CLASS_LEVEL)
     +                 + INCOME_LINKED_LEVEL(:,
     +                       Monthly Debit File Purchase Power Amort,
     +                                                      CLASS_LEVEL)
     +                 + INCOME_LINKED_LEVEL(:,
     +                    Monthly Debit File Interest Amort,CLASS_LEVEL)
C               ENDDO
            ENDIF
...
C            DO MO = 0, 12
               INCOME_VARIABLES(:,Monthly LTD Amort Interest) =
     +                        INCOME_VARIABLES(:,
     +                                 Monthly Debt File Interest Amort)
     +                        + INCOME_VARIABLES(:,
     +                                Monthly Debit File Interest Amort)
     ...
     +                                                      CLASS_LEVEL)
     +                        - INCOME_VARIABLES(:,
     +                             Monthly Debit File Interest Amort Cr)
     +                        - INCOME_LINKED_LEVEL(:,
     +                             Monthly Debit File Interest Amort Cr,
     +                                                      CLASS_LEVEL)
               INCOME_VARIABLES(0,Monthly LTD Amort Interest) =
     +              SUM(INCOME_VARIABLES(1:,Monthly LTD Amort Interest))
               UNMODELED_SUB_INCOME(:) = 
     +                     INCOME_VARIABLES(:,Monthly Subsidiary Income)
               UNMODELED_SUB_DIVIDENDS(:) =
     +                        CASH_VARIABLES(:,Cash Subsidary Dividends)
     
C            ENDDO
            MIDAS_ISSUED_COMMON = ANNUAL_VARS(101)
     +                     - CASH_VARIABLES(0,Cash Common Stock Issued)
     +                     + CASH_VARIABLES(0,Cash Common Stock Buyback)
            CASH_VARIABLES(0,Cash Common Stock Issued) = 
     +                        CASH_VARIABLES(0,Cash Common Stock Issued)
     +                        + MIDAS_ISSUED_COMMON
            CASH_VARIABLES(7,Cash Common Stock Issued) =
     +                        CASH_VARIABLES(7,Cash Common Stock Issued)
     +                        + MIDAS_ISSUED_COMMON
            MIDAS_COMMON_SHARES_ISSUED = ANNUAL_VARS(299)
     +                          - ANNUAL_VARS(409)
     +                          - CASH_VARIABLES(0,Common Shares Issued)
            CASH_VARIABLES(0,Common Shares Issued) =
     +                            CASH_VARIABLES(0,Common Shares Issued)
     +                            + MIDAS_COMMON_SHARES_ISSUED
            CASH_VARIABLES(7,Common Shares Issued) =
     +                            CASH_VARIABLES(7,Common Shares Issued)
     +                            + MIDAS_COMMON_SHARES_ISSUED
     ...
     +                 + CASH_VARIABLES(MO,Common Shares Issued)
            ENDDO
C
            CASH_VARIABLES(7,Cash PS Issued) =
     +                                  CASH_VARIABLES(7,Cash PS Issued)
     +                                  + PREFERRED_STOCK_ISSUED
            CASH_VARIABLES(0,Cash PS Issued) =
     +                                  CASH_VARIABLES(0,Cash PS Issued)
     +                                  + PREFERRED_STOCK_ISSUED
            CASH_VARIABLES(7,Cash LTD Issued) =
     +                                 CASH_VARIABLES(7,Cash LTD Issued)
     +                                 + LONG_TERM_DEBT_ISSUED
            CASH_VARIABLES(0,Cash LTD Issued) =
     +                                 CASH_VARIABLES(0,Cash LTD Issued)
     +                                 + LONG_TERM_DEBT_ISSUED
         ENDIF ! CLASS TYPE
C
...
C
         ...
         ENDIF
         TEMP_EMISSION_DATA(:) =
     +                 INCOME_VARIABLES(:,Monthly Emission Credits)
     +                 + INCOME_LINKED_LEVEL(:,Monthly Emission Credits,
     +                                                      CLASS_LEVEL)
         IF(ABS(ANNUAL_VARS(15) - TEMP_EMISSION_DATA(0)) > .001) THEN
            UNALLOCATED_AMOUNT(Monthly Emission Credits) =
     ...
     +                                             TEMP_EMISSION_DATA(:)
         ENDIF
         CASH_VARIABLES(:,Cash Emission Credits) =
     +                  CASH_VARIABLES(:,Cash Emission Credits)
     +                  + INCOME_VARIABLES(:,Monthly Emission Credits)

C ADD PREVIOUS LEVELS
C
         CASH_VARIABLES(:,Cash ARO Payments) =
     +                              FASB143_MONTHLY_ARO_CASH_PAYMENTS(:)
         CASH_VARIABLES(:,Cash ARO Trust Payments) =
     +                        FASB143_MONTHLY_ARO_TRUST_CASH_PAYMENTS(:)
         INCOME_VARIABLES(:,Monthly FASB 143 Amort) =
     +                           FASB143_MONTHLY_ARO_INTEREST_ACCREATION
         IF(ADD_2_INCOME_STATEMENT == 'T') THEN
            INCOME_VARIABLES(:,
     +                   Monthly Income Reserve Margin Capacity Sales) =
     +          INCOME_VARIABLES(:,
     +                     Monthly Income Reserve Margin Capacity Sales)
     +          + MONTHLY_CAPACITY_SALES_TO_LEVEL_RM 
            INCOME_VARIABLES(:,
...
c
         ...
         IF(WVPA() .AND. CLASS_TYPE == PARENT) THEN
            VOID_LOGICAL = WVPA_MONTHLY_MEMBER_ACCRUED_REVS(
     +                         INCOME_VARIABLES(0,
     +                            Monthly WVPA Accrued Member Revenues))
...
C            IF(VARIABLE == Monthly Emission Credits) CYCLE
            IF(VARIABLE == Monthly Emission Credits
     +                                         .AND. .NOT. WVPA()) CYCLE
            IF(VARIABLE == Monthly LTD Amort Interest) CYCLE 
            IF(VARIABLE == Monthly Other Taxes) CYCLE
            IF(VARIABLE == Monthly Operating Revenue Tax) CYCLE
            IF(VARIABLE == Monthly NF Income Tax Deferrals Cr) CYCLE
C
            IF(VARIABLE == Monthly Property Taxes) CYCLE
            IF(VARIABLE==Monthly Exp File Property Taxes) CYCLE
            IF(VARIABLE==Monthly Exp File Other Taxes) CYCLE
            IF(VARIABLE==Monthly Exp File Operating Revenue Tax) CYCLE
            IF(VARIABLE == Monthly Model Calculated Property Taxes) THEN
c               INCOME_VARIABLES(:,VARIABLE) = PROPERTY_TAX_ADJUSTMENT(:)
...
c            IF(VARIABLE == Cash Emission Credits) CYCLE
            IF(VARIABLE == Cash Emission Credits
     +                                         .AND. .NOT. WVPA()) CYCLE
            IF(VARIABLE == Common Shares Issued) CYCLE
...
C STD AND CUSTOMER DEPOSIT INTEREST EXPENSE
         INCOME_VARIABLES(0,Monthly STD Interest) = 0. ! ANNUAL_VARS(40)
C
C INTEREST ON CUSTOMER DEPOSITS
C
         CASH_VARIABLES(0,Cash Customer Deposits) = 0.
         BALANCE_SHEET_VARIABLES(0,monthly_customer_deposits) =
     +                                        CUSTOMER_DEPOSITS_BAL(BOY)
         INCOME_VARIABLES(0,Monthly Amortization) =
     ...
     +           + INCOME_VARIABLES(0,monthly_common_stock_issue_amort)
     +           + INCOME_VARIABLES(0,Monthly Other ATL Amort)
     +           - INCOME_VARIABLES(0,Monthly ATL Gain Amort)
     +           - INCOME_VARIABLES(0,Monthly CIAC Amort)




         INCOME_VARIABLES(0,BTL Monthly Amortization) =
     +               INCOME_VARIABLES(0,Monthly Lease BTL Amort Expense)
     +               + INCOME_VARIABLES(0,Monthly Other BTL Amort)
     +               - INCOME_VARIABLES(0,Monthly BTL Gain Amort)
         DO MO = 1, 12
C
...
C
     ...
     +         (CASH_VARIABLES(MO,Cash Customer Deposits)/2. +
     +          BALANCE_SHEET_VARIABLES(MO-1,monthly_customer_deposits))
            BALANCE_SHEET_VARIABLES(MO,monthly_customer_deposits) =
     +           BALANCE_SHEET_VARIABLES(MO-1,monthly_customer_deposits)
     +                       + CASH_VARIABLES(MO,Cash Customer Deposits)
            INCOME_VARIABLES(MO,Monthly Amortization) =
     ...
     +           + INCOME_VARIABLES(MO,monthly_common_stock_issue_amort)
     +           + INCOME_VARIABLES(MO,Monthly Other ATL Amort)
     +           - INCOME_VARIABLES(MO,Monthly ATL Gain Amort)
     +           - INCOME_VARIABLES(MO,Monthly CIAC Amort)
            INCOME_VARIABLES(MO,BTL Monthly Amortization) =
     +            INCOME_VARIABLES(MO,Monthly Other BTL Amort)
     +            - INCOME_VARIABLES(MO,Monthly BTL Gain Amort)
     +            + INCOME_VARIABLES(MO,Monthly Lease BTL Amort Expense)
C
C STD INTEREST
C            
            INCOME_VARIABLES(MO,Monthly STD Interest) =
     ...
     +                                * STD_MONTHLY_INTEREST_RATE(MO)/2.
             IF(MO == 1 .AND. STI_CASH_LAG /= 0) THEN
               CASH_VARIABLES(MO,Cash STD Interest) =
     +                     PAYABLE_MONTHLY_VALUES(MO,
     ...
     +                         INCOME_VARIABLES(MO,Monthly STD Interest)
            ENDIF
         ENDDO
         CASH_VARIABLES(0,Cash Customer Deposits) =
     +                    SUM(CASH_VARIABLES(1:,Cash Customer Deposits))
C
C ADD STD INTEREST EXPENSE THAT HAS BEEN LAGGED BY THE EXPENSE FILE
C
         INCOME_VARIABLES(1:,Monthly STD Interest) =
     +                         INCOME_VARIABLES(1:,Monthly STD Interest) 
     +                         + STD_INTEREST_ADJ(1:) ! Interest from Expense File
         CASH_VARIABLES(1:,Cash STD Interest) =
     +                              CASH_VARIABLES(1:,Cash STD Interest) 
     +                              + STD_CASH_INTEREST_ADJ(1:) ! lagged cash interest from expense file
         CASH_VARIABLES(0,Cash STD Interest) =
     +                         SUM(CASH_VARIABLES(1:,Cash STD Interest))
         INCOME_VARIABLES(0,Monthly STD Interest) =
     +                    SUM(INCOME_VARIABLES(1:,Monthly STD Interest))
C
C ALLOCATE EXPENSE ITEMS
C
         UNALLOCATED_AMOUNT = 0.
         UNALLOCATED_AMOUNT(Monthly Book Depreciation) =
     +          ALLOCATE_ACCOUNT_BALANCE(ANNUAL_VARS(17),
     +                    INCOME_VARIABLES(0,Monthly Book Depreciation))
         UNALLOCATED_AMOUNT(Monthly Amortization) =
...
C
         UNALLOCATED_AMOUNT(Monthly AFUDC Equity) =
     +          ALLOCATE_ACCOUNT_BALANCE(ANNUAL_VARS(37),
     +                     INCOME_VARIABLES(0,Monthly AFUDC Equity))
         UNALLOCATED_AMOUNT(Monthly AFUDC Borrowed) =
     +          ALLOCATE_ACCOUNT_BALANCE(ANNUAL_VARS(41),
     +                     INCOME_VARIABLES(0,Monthly AFUDC Borrowed))
...
C
         ...
         INCOME_VARIABLES(:,BTL Monthly STInvestmet Income) =
     +                                                   STI_EARNINGS(:)
         INCOME_VARIABLES(:,Monthly Model LTInvestment Income) =
     +                                                   LTI_EARNINGS(:)
         INCOME_VARIABLES(:,BTL Monthly Interest Income) =
     +              INCOME_VARIABLES(:,BTL Monthly STInvestmet Income)
     +              + INCOME_VARIABLES(:,
     +                                Monthly Model LTInvestment Income)
     +              + INCOME_VARIABLES(:,
     +                             Monthly Debt File LInvestment Income)
     +              + INCOME_VARIABLES(:,
     +                                  monthly_notes_receivable_income) 
         INCOME_VARIABLES(:,Monthly Total LTInvestment Income) =
     +              INCOME_VARIABLES(:,
     +                                Monthly Model LTInvestment Income)
     +              + INCOME_VARIABLES(:,
     +                             Monthly Debt File LInvestment Income)
         ...
         INCOME_VARIABLES(:,
     +       Monthly Net of Tax Nuclear Decom Fund Earnings) =
     +                       MONTHLY_NUC_DECOM_FUND_NET_OF_TAX_RETURN(:)
         INCOME_VARIABLES(:,Monthly Retirement Medical Fund Earnings) =
     +                                 RETIREMENT_MEDICAL_FUND_RETURN(:)
         INCOME_VARIABLES(:,Monthly Investment Earnings) =
     +            RETIREMENT_MEDICAL_FUND_RETURN(:)
     +            + INCOME_VARIABLES(:,Monthly Dividend 70 Earnings)
     +            + INCOME_VARIABLES(:,Monthly Regular Dividend Income)
         IF(SALT_RIVER_PROJECT()) THEN
            INCOME_VARIABLES(:,Monthly Investment Earnings) =
     +                   INCOME_VARIABLES(:,Monthly Investment Earnings)
     +                   + MONTHLY_NUC_DECOM_FUND_RETURN(:)
            INCOME_VARIABLES(:,
     +                  Monthly Nuclear Decommissioning Fund Earnings) =
     +                                  MONTHLY_NUC_DECOM_FUND_RETURN(:)

            INCOME_VARIABLES(:,Monthly Investment Earnings Receivable) =
     +            + INCOME_VARIABLES(:,Monthly Dividend 70 Earnings)
     +            + INCOME_VARIABLES(:,Monthly Regular Dividend Income)
     ...
     +            + INCOME_VARIABLES(:,Monthly Regular Dividend Income)
         ...
         CASH_VARIABLES(:,Cash Investment Interest Earnings) =
     +               CASH_VARIABLES(:,Cash Investment Interest Earnings)
     +               + LTI_EARNINGS(:)! ADDED 5/13/04
C
C TAL TAXES ITES 81-95; TOTAL EXPENSE 150 OP INCOME 151 INCOME_VARIABLES(0:12,Monthly Federal ATL Income Tax Paid)
C
         UNALLOCATED_AMOUNT(Monthly Model Calculated Property Taxes) =
     +          ALLOCATE_ACCOUNT_BALANCE(ANNUAL_VARS(435), ! NOW PROPERTY TAXES BASES ON VALUE217),
     +                       INCOME_VARIABLES(0,
     +                         Monthly Model Calculated Property Taxes))
         INCOME_VARIABLES(:,Monthly Model Calculated Property Taxes) =
     +       INCOME_VARIABLES(:,Monthly Model Calculated Property Taxes)
     +       + PROPERTY_TAX_ADJUSTMENT(:)
C
         INCOME_VARIABLES(0,Monthly ITC Amortization) = ANNUAL_VARS(50)
         INCOME_VARIABLES(0,Monthly State ATL Income Tax Paid) =
     +                                                   ANNUAL_VARS(24)
         INCOME_VARIABLES(0,Monthly State Tax on Capital) =
     +                                                   ANNUAL_VARS(25)
         INCOME_VARIABLES(0,Monthly Federal ATL Income Tax Paid) =
     +                                                   ANNUAL_VARS(26)
...
C
     ...
     +                                                  ANNUAL_VARS(294)
         ...
         INCOME_VARIABLES(0,BTL Monthly Amortization) =
     +                                                  ANNUAL_VARS(350)
         INCOME_VARIABLES(0,Monthly Subsidiary Income) =
     +                                                   ANNUAL_VARS(43)
...
C DEFERRED TAXES CR
C
         INCOME_VARIABLES(0,Monthly LTD PS Income Tax Deferrals Cr) =
     +                                                  ANNUAL_VARS(681)
         INCOME_VARIABLES(1:,Monthly LTD PS Income Tax Deferrals Cr) = 
     +            INCOME_VARIABLES(0,
     +                       Monthly LTD PS Income Tax Deferrals Cr)/12.
         INCOME_VARIABLES(0,Monthly Other Income Tax Deferrals Cr) =
     +                                                  ANNUAL_VARS(682)
         INCOME_VARIABLES(1:,Monthly Other Income Tax Deferrals Cr) =
     +            INCOME_VARIABLES(0,
     +                        Monthly Other Income Tax Deferrals Cr)/12.
         INCOME_VARIABLES(0,Monthly NF Income Tax Deferrals Cr) =
     +       RETURN_NF_DEFERRED_TAXES(INCOME_VARIABLES(0,
     +                              Monthly NF Income Tax Deferrals Cr))
         INCOME_VARIABLES(:,Monthly NF Income Tax Deferrals Cr) =
     +            INCOME_VARIABLES(:,Monthly NF Income Tax Deferrals Cr) 
     +            + INCOME_LINKED_LEVEL(:,
     +                               Monthly NF Income Tax Deferrals Cr,
     +                                                      CLASS_LEVEL)
         INCOME_VARIABLES(:,Monthly Income Tax Deferrals Cr) =
     +      INCOME_VARIABLES(:,Monthly Other Income Tax Deferrals Cr)
     +      + INCOME_VARIABLES(:,Monthly LTD PS Income Tax Deferrals Cr)
     +      + INCOME_VARIABLES(:,Monthly NF Income Tax Deferrals Cr)
         INCOME_VARIABLES(0,WVPA Non Member Cost of Services) =
     +        SUM(INCOME_VARIABLES(1:,WVPA Non Member Cost of Services))
         INCOME_VARIABLES(0,WVPA Member Cost of Services) =
     +            SUM(INCOME_VARIABLES(1:,WVPA Member Cost of Services))
C
C COMMON DIVIDENDS BY QUARTER
C
         QUARTER = 1
         SKIP_MONTH = 0
         CASH_VARIABLES(0,Cash Common Dividends) = 0.
         IF(SALT_RIVER_PROJECT()) THEN 
            INCOME_VARIABLES(:,Monthly Common Dividends) =
     +                                       MONTHLY_COMMON_DIVIDENDS(:)
            CASH_VARIABLES(:,Cash Common Dividends) =
     +                                       MONTHLY_COMMON_DIVIDENDS(:)
               ...
               IF(MPS()) THEN
                  IF(CLASS_TYPE == PARENT) THEN
                     IF(CS_PAY_MONTH(MO)) THEN
                        CASH_VARIABLES(MO,Cash Common Dividends) =
     +                                            TEMP_CS_DIV_CARRY_OVER
                        CASH_VARIABLES(0,Cash Common Dividends) =
     +                           CASH_VARIABLES(0,Cash Common Dividends)
     +                           + TEMP_CS_DIV_CARRY_OVER
     ...
     +                        + CASH_VARIABLES(MO,Cash Common Dividends)
                     ELSEIF(MO /= SKIP_MONTH) THEN
                        CASH_VARIABLES(MO,Cash Common Dividends) = 0.
                     ENDIF
     ...
     +                     INCOME_VARIABLES(MO,Monthly Common Dividends)
            ...
            ENDDO
         ELSE ! DIVIDEND DPR
            INCOME_VARIABLES(0,Monthly Common Dividends) = ! ANNUAL_VARS(48)
     +                                       MONTHLY_COMMON_DIVIDENDS(0)
            IF(CLASS_TYPE == PARENT) THEN
     ...
     +                                   + QUARTERY_CS_DIVIDENDS(4)))/2.
               ...
               ENDIF
            ELSE
               DIVIDEND_ALLOCATION = 2*ANNUAL_VARS(77)-ANNUAL_VARS(101) ! 2*COMMON EQUITY - COMMON ISSUED
...
C
         CASH_VARIABLES(0,Cash Common Dividends) =
     +                     SUM(CASH_VARIABLES(1:,Cash Common Dividends))
         INCOME_VARIABLES(0,Monthly Common Dividends) =
...
C
     ...
     +                                     MONTHLY_PREVIOUS_OPT_REVS(:)
         ...
         INCOME_VARIABLES(0,Total Derivative Revenues) =
     +               SUM(INCOME_VARIABLES(1:,Total Derivative Revenues))
         INCOME_VARIABLES(0,Total Derivative Expenses) =
     +               SUM(INCOME_VARIABLES(1:,Total Derivative Expenses))
         INCOME_VARIABLES(0,Total Fuel Derivative Revenues) =
     +          SUM(INCOME_VARIABLES(1:,Total Fuel Derivative Revenues))
         INCOME_VARIABLES(0,Total Fuel Derivative Expenses) =
     +          SUM(INCOME_VARIABLES(1:,Total Fuel Derivative Expenses))
...
C
     ...
     +          INCOME_VARIABLES(0,Monthly Total Revenues) -
     +          INCOME_VARIABLES(0,Monthly Total Taxes Expense)
         MO = 0
...
C
     ...
     +                    INCOME_VARIABLES(0,Monthly LTD Total Interest)
     +                    + LTD_BOOKED_ISSUED_INTEREST(0)
            INCOME_VARIABLES(0,Monthly LTD Booked Interest) =
     +                    INCOME_VARIABLES(0,Monthly LTD Total Interest)
     +                    + LTD_BOOKED_ISSUED_INTEREST(0)
            INCOME_VARIABLES(0,Monthly LTD Total Interest) =
     +                 INCOME_VARIABLES(0,Monthly LTD Total Interest)
     ...
     +                 + ANNUAL_VARS(630) ! was 384 but only debit file is included. LTD INTEREST AMORTIZATION
            DO MO = 1, 12
               INCOME_VARIABLES(MO,Monthly LTD Total Interest) =
     +                   INCOME_VARIABLES(MO,Monthly LTD Total Interest)
     +                   + LTD_BOOKED_ISSUED_INTEREST(MO)
               INCOME_VARIABLES(MO,Monthly LTD Booked Interest) =
     +                   INCOME_VARIABLES(MO,Monthly LTD Total Interest)
     ...
     +                                 + LONG_TERM_DEBT_ISSUED)
                  ...
                  INCOME_VARIABLES(1:6,Monthly LTD Total Interest) =
     +                                                      HALF_YEAR/6.

               HALF_YEAR = LTD_ISSUE_YR_INTEREST_PAYMENT - HALF_YEAR 

                  INCOME_VARIABLES(7:12,Monthly LTD Total Interest) =
     +                                                      HALF_YEAR/6.
     ...
     +                 + ANNUAL_VARS(384) ! LTD INTEREST AMORTIZATION
              if(.false.) then
               INCOME_VARIABLES(9,Monthly LTD Total Interest) =
     +                 INCOME_VARIABLES(9,Monthly LTD Total Interest)
     +                 + LTD_ISSUE_YR_INTEREST_BOOKED/2.
               INCOME_VARIABLES(12,Monthly LTD Total Interest) =
     +                 INCOME_VARIABLES(12,Monthly LTD Total Interest)
     +                 + LTD_ISSUE_YR_INTEREST_BOOKED/2.
              else
               INCOME_VARIABLES(7:12,Monthly LTD Total Interest) =
     ...
     +                   INCOME_VARIABLES(1:,Monthly LTD Total Interest)
               INCOME_VARIABLES(1:,Monthly LTD Booked Interest) =
     +                   INCOME_VARIABLES(1:,Monthly LTD Total Interest)
     ...
     +                                       PS_ISSUE_YR_DIVIDEND_BOOKED
            INCOME_VARIABLES(0,Monthly Total PS Dividends) =
     +                                       PS_ISSUE_YR_DIVIDEND_BOOKED
     +                                       + ANNUAL_VARS(360) ! PS AMORTIZATIONS
            INCOME_VARIABLES(9,Monthly Total PS Dividends) =
     +                                    PS_ISSUE_YR_DIVIDEND_BOOKED/2.
            INCOME_VARIABLES(12,Monthly Total PS Dividends) =
     +                                    PS_ISSUE_YR_DIVIDEND_BOOKED/2.
         ELSE
            INCOME_VARIABLES(0,Monthly Booked PS Dividends) =
     +                    INCOME_VARIABLES(0,Monthly Total PS Dividends)
...
C
          ...
          INCOME_VARIABLES(1:,Monthly State ATL Income Tax Paid) =
     +         INCOME_VARIABLES(0,Monthly State ATL Income Tax Paid)/12.
          INCOME_VARIABLES(1:,Monthly Federal ATL Income Tax Paid) =
     +       INCOME_VARIABLES(0,Monthly Federal ATL Income Tax Paid)/12.
          INCOME_VARIABLES(1:,Monthly Income Tax Deferrals Dr) =
...
C
     ...
     +               - FED_DEF_TAX_DR_FROM_NOLS_AMT(0)
            ENDIF
            DO MO = 0, 12
               INCOME_VARIABLES(MO,Monthly State ATL Income Tax Paid) =
     +                                      RPT_STATE_ATL_INCOME_TAX(MO)
               INCOME_VARIABLES(MO,Monthly Federal ATL Income Tax Paid)=
     +                                    RPT_FEDERAL_ATL_INCOME_TAX(MO)
               INCOME_VARIABLES(MO,Monthly Total Fed Income Tax Paid) =
     +                                        RPT_FEDERAL_INCOME_TAX(MO)
               INCOME_VARIABLES(MO,Monthly Total State Income Tax Paid)=
     +                                          RPT_STATE_INCOME_TAX(MO)
               INCOME_VARIABLES(MO,Monthly Fed Def Tax Dr NOLs AMT) =
     +                                    STATE_DEF_TAX_DR_FROM_NOLS(MO)
               INCOME_VARIABLES(MO,Monthly State Def Tax Dr NOLs) =
...
C
     ...
     +          INCOME_VARIABLES(0,Monthly Total Fed Income Tax Paid),
     +          INCOME_VARIABLES(0,Monthly Total State Income Tax Paid),
     +          INCOME_VARIABLES(0,Monthly Income Tax Deferrals Dr))
         DO MO = 0, 12
            INCOME_VARIABLES(MO,Monthly Federal ATL Income Tax Paid)=
     +            INCOME_VARIABLES(MO,Monthly Total Fed Income Tax Paid)
...
C
         INCOME_VARIABLES(0,Monthly Other Taxes) = 0.
         DO MO = 1, 12
...
C
            ...
            INCOME_VARIABLES(MO,Monthly ITC Amortization) =
     +                  INCOME_VARIABLES(0,Monthly ITC Amortization)/12.
...
C REVENUE TAXES
C
               INCOME_VARIABLES(MO,Monthly Operating Revenue Tax) =
...
C OTHER TAXES
C
               INCOME_VARIABLES(MO,Monthly Other Taxes) = 
     +                          MONTHLY_OTHER_TAXES(MO,CLASS_LEVEL,
     +                                       INCOME_VARIABLES,
     +                                       OTHER_TAX_ADJUSTMENT,
     +                                       MONTHLY_OTHER_REV_TAX_RATE,
     +                                       MONTHLY_OTHER_EXP_TAX_RATE)
               IF(MO==12) INCOME_VARIABLES(MO,Monthly Other Taxes) =
     +                          INCOME_VARIABLES(MO,Monthly Other Taxes)
...
C
     ...
     +                               test_total_exp_taxes
         ...
         ENDDO
C
         INCOME_VARIABLES(0,Monthly Operating Revenue Tax) =
     +           SUM(INCOME_VARIABLES(1:,Monthly Operating Revenue Tax))
         INCOME_VARIABLES(0,Monthly Other Taxes) =
     +                     SUM(INCOME_VARIABLES(1:,Monthly Other Taxes))
         INCOME_VARIABLES(0,Monthly Total Taxes Expense) =
     +         MONTHLY_TOTAL_EXPENSES_B4_TAXES(INT2(0),INCOME_VARIABLES)
     +         + TOTAL_TAXES_EXPENSE(INT2(0),INCOME_VARIABLES) 
         INCOME_VARIABLES(0,Monthly Op Income) = 
     ...
     +                 - INCOME_VARIABLES(MO,Monthly Total PS Dividends)
         INCOME_VARIABLES(0,Monthly RETAINED EARNINGS) = 
     +                    INCOME_VARIABLES(0,Monthly Earnings 2 Common)
     +                    - INCOME_VARIABLES(0,Monthly Common Dividends)
     ...
     +                                  + RPT_FEDERAL_BTL_INCOME_TAX(MO)
            ENDDO
         ELSE
            INCOME_VARIABLES(:,BTL Monthly Income Taxes) = 0.
...
C
     ...
     +                   INCOME_VARIABLES(MO,Monthly Total PS Dividends)
            INCOME_VARIABLES(MO,Monthly Total PS Dividends) =
     +                 INCOME_VARIABLES(MO,Monthly Total PS Dividends)
     +                 + INCOME_VARIABLES(MO,Monthly PS Amort Dividends)
...
!  Deleted commented out code below this comment, which is probably no longer relevant.
     ...
     +                CASH_VARIABLES(0,Cash Operating Revenue Tax),
     +                INCOME_VARIABLES(0,Monthly Operating Revenue Tax),
     ...
     +                CLASS_ID)
         CALL LAG_CASH_VALUES(
     +                CASH_VARIABLES(0,Cash Other Taxes),
     +                INCOME_VARIABLES(0,Monthly Other Taxes),
     +                PAYABLE_MONTHLY_VALUES(0,
     ...
     +                CASH_VARIABLES(0,
     +                            Cash Model Calculated Property Taxes),
     +                INCOME_VARIABLES(0,
     +                         Monthly Model Calculated Property Taxes),
     +                PAYABLE_MONTHLY_VALUES(0,
...
C Monthly State ATL Income Tax Paid
         AVERAGE_VALUES = 0.
         AVERAGE_VALUES(0) =
     +           INCOME_VARIABLES(0,Monthly Total State Income Tax Paid) !  ANNUAL_VARS(212)
         IF(KCPL()) THEN
            AVERAGE_VALUES(3) = SUM(INCOME_VARIABLES(1:3,
     +                             Monthly Total State Income Tax Paid)) !  ANNUAL_VARS(212)
     ...
     +                             Monthly Total State Income Tax Paid)) !  ANNUAL_VARS(212)
         ...
         IF(CLASS_TYPE == PARENT .OR. CLASS_TYPE == SUBSIDIARY) THEN
            CALL LAG_CASH_VALUES(
     +              CASH_VARIABLES(0,Cash State Income Taxes Paid),
     ...
     +              CLASS_ID)
         ELSE
            CASH_VARIABLES(0,Cash State Income Taxes Paid) =
     +                                                 AVERAGE_VALUES(0)
            CASH_VARIABLES(4,Cash State Income Taxes Paid) =
     +                                                 AVERAGE_VALUES(3)
            CASH_VARIABLES(6,Cash State Income Taxes Paid) =
     +                                                 AVERAGE_VALUES(5)
            CASH_VARIABLES(9,Cash State Income Taxes Paid) =
     +                                                 AVERAGE_VALUES(8)
            CASH_VARIABLES(12,Cash State Income Taxes Paid) =
     +                                                AVERAGE_VALUES(12)
         ENDIF
C Federal Income Taxes Paid
         AVERAGE_VALUES = 0.
         AVERAGE_VALUES(0) =
     +             INCOME_VARIABLES(0,Monthly Total Fed Income Tax Paid) !  ANNUAL_VARS(211)
         IF(EMPIRE()) THEN
         ...
         ELSEIF(IPALCO().AND. CLASS_TYPE == PARENT) THEN
            AVERAGE_VALUES(3) = SUM(INCOME_VARIABLES(1:3,
     +                               Monthly Total Fed Income Tax Paid)) !  ANNUAL_VARS(211)
     ...
     +                           Monthly Total Fed Income Tax Paid)))/9. !  ANNUAL_VARS(211)
     +                      - AVERAGE_VALUES(3)
     +                      - AVERAGE_VALUES(5)
     +                      - AVERAGE_VALUES(8)
         ELSEIF(KCPL()) THEN
            AVERAGE_VALUES(3) = SUM(INCOME_VARIABLES(1:3,
     +                               Monthly Total Fed Income Tax Paid))
            AVERAGE_VALUES(5) = SUM(INCOME_VARIABLES(4:6,
     +                               Monthly Total Fed Income Tax Paid))
            AVERAGE_VALUES(8) = SUM(INCOME_VARIABLES(7:9,
     +                               Monthly Total Fed Income Tax Paid))
     ...
     +                               Monthly Total Fed Income Tax Paid))
         ...
         IF(IPALCO().AND. CLASS_TYPE == PARENT) THEN
            CASH_VARIABLES(4,Cash Federal Income Taxes Paid) =
     +                                                 AVERAGE_VALUES(3)
            CASH_VARIABLES(6,Cash Federal Income Taxes Paid) =
     +                                                 AVERAGE_VALUES(5)
            CASH_VARIABLES(9,Cash Federal Income Taxes Paid) =
     +                                                 AVERAGE_VALUES(8)
            CASH_VARIABLES(12,Cash Federal Income Taxes Paid) =
     +                                                AVERAGE_VALUES(12)
     ...
     +                                             + AVERAGE_VALUES(8)
     +                                             + AVERAGE_VALUES(12))
            CASH_VARIABLES(1:,Cash Federal Income Taxes Paid) =
     +                 CASH_VARIABLES(1:,Cash Federal Income Taxes Paid)
     +                  + PAYABLE_MONTHLY_VALUES(1:,
     +                      payment_federal_income_taxes_paid,CLASS_POS)
            IF(FEDERAL_INC_TAX_CARRY_OVER(CLASS_POS) >= 0) THEN
               CASH_VARIABLES(3,Cash Federal Income Taxes Paid) =
     +                  CASH_VARIABLES(3,Cash Federal Income Taxes Paid)
     ...
     +                  + FEDERAL_INC_TAX_CARRY_OVER(CLASS_POS)
            ENDIF 
            CASH_VARIABLES(0,Cash Federal Income Taxes Paid) =
     +            SUM(CASH_VARIABLES(1:,Cash Federal Income Taxes Paid))
     ...
     +                PAYABLE_MONTHLY_VALUES(0,
     +                     payment_federal_income_taxes_paid,CLASS_POS),
     +                     payment_federal_income_taxes_paid,
     +                     CLASS_ID)
         ELSE
            CASH_VARIABLES(4,Cash Federal Income Taxes Paid) =
     +                                                 AVERAGE_VALUES(3)
...
C Monthly State Tax on Capital
         CALL LAG_CASH_VALUES(
     +                CASH_VARIABLES(0,Cash State Tax on Capital),
     +                INCOME_VARIABLES(0,Monthly State Tax on Capital),
     +                PAYABLE_MONTHLY_VALUES(0,
...
C Federal Tax on Capital
         CALL LAG_CASH_VALUES(
     +               CASH_VARIABLES(0,Cash Federal Tax on Capital),
     +               INCOME_VARIABLES(0,Monthly Federal Tax on Capital),
     +               PAYABLE_MONTHLY_VALUES(0,
...
C
         CASH_VARIABLES(0,Cash Working Capital) = ANNUAL_VARS(118)
C
C TEMP INSTALLATION OF NUCLEAR FUEL
C
         CASH_VARIABLES(0,Cash Nuclear Fuel Fabrication) =
     +                                                  ANNUAL_VARS(111)
C
         DO MO = 1, 12

            CASH_VARIABLES(MO,Cash Working Capital) =
     +                            (ASSETS_NEC(MO)- LIABS_NEC(MO))
     +                             -(ASSETS_NEC(MO-1) - LIABS_NEC(MO-1))
            CASH_VARIABLES(MO,Cash Nuclear Fuel Fabrication) =
     +                                              ANNUAL_VARS(111)/12.
         ENDDO
         IF(CPL_ACTIVE() .OR. MORTGAGE_DEBT) THEN
            CASH_VARIABLES(1:,Cash LTD Interest) =
     +                              CASH_VARIABLES(1:,Cash LTD Interest)
     +                              + LTD_PAID_ISSUED_INTEREST(1:)
            CASH_VARIABLES(1:,Cash LTD Retirements) =
     +                           CASH_VARIABLES(1:,Cash LTD Retirements)
     +                           + MGT_DEBIT_RETIREMENT(1:) ! Mortgage Principal Payments
         ...
         ELSE
            ...
            IF(CLASS_TYPE == REGULATED_GROUP) THEN
               CASH_VARIABLES(7,Cash LTD Retirements) = LTD_RETIREMENTS
               CASH_VARIABLES(1:,Cash LTD Interest) =
     +                   INCOME_VARIABLES(1:,Monthly LTD Total Interest)
            ELSE
               CASH_VARIABLES(10,Cash LTD Interest) =
     +                              CASH_VARIABLES(10,Cash LTD Interest)
     +                              + LTD_ISSUE_YR_INTEREST_PAYMENT
            ENDIF
         ENDIF
         CASH_VARIABLES(0,Cash LTD Retirements) =
     +                      SUM(CASH_VARIABLES(1:,Cash LTD Retirements))
C
         CASH_VARIABLES(0,Cash LTD Interest) =
     +                         SUM(CASH_VARIABLES(1:,Cash LTD Interest))
         IF(EMPIRE()) THEN
            CASH_VARIABLES(0,Cash PS Dividends) =
     +                               CASH_VARIABLES(0,Cash PS Dividends)
     +                               + PS_ISSUE_YR_DIVIDEND_PAYMENT
            CASH_VARIABLES(9,Cash PS Dividends) =
     +                              CASH_VARIABLES(9,Cash PS Dividends)
     +                              + PS_ISSUE_YR_DIVIDEND_PAYMENT/2.
            CASH_VARIABLES(12,Cash PS Dividends) =
     +                              CASH_VARIABLES(12,Cash PS Dividends)
     +                              + PS_ISSUE_YR_DIVIDEND_PAYMENT/2.
         ELSEIF(CLASS_TYPE == REGULATED_GROUP) THEN
            CASH_VARIABLES(7,Cash PS Retirements) = PS_RETIREMENTS
            CASH_VARIABLES(7,Cash PS Issued) = PREFERRED_STOCK_ISSUED
            CASH_VARIABLES(0,Cash PS Dividends) =
     +                                      PS_ISSUE_YR_DIVIDEND_PAYMENT
            CASH_VARIABLES(9,Cash PS Dividends) =
     +                                   PS_ISSUE_YR_DIVIDEND_PAYMENT/2.
     ...
     +                               CASH_VARIABLES(0,Cash PS Dividends)
     +                               + PS_ISSUE_YR_DIVIDEND_PAYMENT
            CASH_VARIABLES(10,Cash PS Dividends) =
     +                              CASH_VARIABLES(10,Cash PS Dividends)
     +                              + PS_ISSUE_YR_DIVIDEND_PAYMENT
         ENDIF
C
     ...
     +                  + CASH_VARIABLES(0,Cash Change LTInvestments)
     +                  + INVESTMENTS_BOY
     +                  + CASH_VARIABLES(0,Cash Change Debt Investments)
         IF(EOY_LONG_TERM_INVESTMENTS < MIN_LONG_TERM_INVESTMENT) THEN
            CASH_VARIABLES(0,Cash Change LTInvestments) =
     +                       CASH_VARIABLES(0,Cash Change LTInvestments)
     +                       + MIN_LONG_TERM_INVESTMENT
     +                       - EOY_LONG_TERM_INVESTMENTS
            CASH_VARIABLES(1,Cash Change LTInvestments) =
     +                       CASH_VARIABLES(1,Cash Change LTInvestments)
     +                       + MIN_LONG_TERM_INVESTMENT
     +                       - EOY_LONG_TERM_INVESTMENTS
...
C
         CASH_VARIABLES(1:,Cash Acquisition Cost) =
     +                                         NEW_ACQUISITIONS_COST(1:)
         CASH_VARIABLES(0,Cash Acquisition Cost) =
     +                     SUM(CASH_VARIABLES(1:,Cash Acquisition Cost))
C
...
C
     ...
     +                       CASH_VARIABLES(0,Non Cash Pension in CapX)) 
         CASH_VARIABLES(:,Cash Net Plant CapX) = 
     +                      CASH_VARIABLES(:,Cash Plant Construction)
     +                      - CASH_VARIABLES(:,Non Cash Pension in CapX)
         IF(CLASS_TYPE == PARENT) THEN
...
C PROPERTY TAXES
C
         INCOME_VARIABLES(:,Monthly Property Taxes) =
     +              INCOME_VARIABLES(:,Monthly Exp File Property Taxes)
     +              + INCOME_VARIABLES(:,
     +                          Monthly Model Calculated Property Taxes)
         CASH_VARIABLES(:,Cash Property Taxes) =
     +            CASH_VARIABLES(:,Cash Model Calculated Property Taxes)  
     +            + CASH_VARIABLES(:,Cash Exp File Property Taxes)  
C
C OTHER TAXES
C
         INCOME_VARIABLES(:,Monthly Other Taxes) =
     +                INCOME_VARIABLES(:,Monthly Other Taxes)
     +                + INCOME_VARIABLES(:,Monthly Exp File Other Taxes)
         CASH_VARIABLES(:,Cash Other Taxes) =
     +                     CASH_VARIABLES(:,Cash Other Taxes)
     +                     + CASH_VARIABLES(:,Cash Exp File Other Taxes)
C
C REVENUE TAXES
C
         INCOME_VARIABLES(:,Monthly Operating Revenue Tax) =
     +                INCOME_VARIABLES(:,Monthly Operating Revenue Tax)
     +                + INCOME_VARIABLES(:,
     +                           Monthly Exp File Operating Revenue Tax)
         CASH_VARIABLES(:,Cash Operating Revenue Tax) =
     +                     CASH_VARIABLES(:,Cash Operating Revenue Tax)
     +                     + CASH_VARIABLES(:,
     +                              Cash Exp File Operating Revenue Tax)
C
         DO MO = 0, 12
            CASH_VARIABLES(MO,Cash Operating Method Adj) =
     +                  INCOME_VARIABLES(MO,Operating Method Adjustment)
            CASH_VARIABLES(MO,Cash Prior Years Method Adj) =
     +                  INCOME_VARIABLES(MO,
     +                                    Prior Years Method Adjustment)
            CASH_VARIABLES(MO,Cash STInvestment Interest Income) =
     +                                      CASH_BTL_INTEREST_INCOME(MO)

            CASH_VARIABLES(MO,Cash TOTAL OPERATING RECEIPTS) =
     +                  CASH_VARIABLES(MO,Cash TOTAL OPERATING RECEIPTS) 
     +                  + CASH_VARIABLES(MO,
     +                                Cash STInvestment Interest Income)
     +                  + CASH_VARIABLES(MO,Cash Operating Method Adj)
     +                  + CASH_VARIABLES(MO,Cash Prior level Method Adj)
     +                  + CASH_VARIABLES(MO,
     +                                Cash Investment Dividend Earnings)
     +                  + CASH_VARIABLES(MO,
     +                                Cash Investment Interest Earnings)
     +                  + CASH_VARIABLES(MO,Cash Prior Years Method Adj)

            CASH_VARIABLES(MO,NET CASH RECEIPTS OR PAYMENTS) =
     +                CASH_VARIABLES(MO,Cash TOTAL OPERATING RECEIPTS) -
     +                CASH_VARIABLES(MO,Cash TOTAL EXPENSE PAYMENTS)
C
C CAPITAL SERVICE
C
            CASH_VARIABLES(MO,CASH CAPITAL SERVICE PAYMENTS) =
     +               CASH_VARIABLES(MO,Cash PS Dividends)
     +               + CASH_VARIABLES(MO,Cash LTD Interest)
     +               + CASH_VARIABLES(MO,Cash STD Interest)
     +               + CASH_VARIABLES(MO,Cash Common Dividends)
     +               + CASH_VARIABLES(MO,Cash Interest on Notes Payable)
C
C TAXES 
C
            CASH_VARIABLES(MO,CASH TOTAL TAX PAYMENTS) = 
     +               CASH_VARIABLES(MO,Cash Operating Revenue Tax)
     +               + CASH_VARIABLES(MO,Cash Other Taxes)
     +               + CASH_VARIABLES(MO,Cash Property Taxes)
     +               + CASH_VARIABLES(MO,Cash State Income Taxes Paid)
     +               + CASH_VARIABLES(MO,Cash State Tax on Capital)
     +               + CASH_VARIABLES(MO,Cash Federal Income Taxes Paid)
     +               + CASH_VARIABLES(MO,Cash Federal Tax on Capital)
C
C
C
            IF(CLASS_TYPE == PARENT) THEN
               CASH_VARIABLES(MO,Cash Parent Taxes Paid) =
     +                CASH_VARIABLES(MO,Cash Federal Income Taxes Paid)

               CASH_VARIABLES(MO,Cash Subsidiary Tax Payments) =
     +                             SUB_FED_TAX_PAYMENTS_2_PARENT(MO)
     +                             + SUB_STATE_TAX_PAYMENTS_2_PARENT(MO)  ! 12/11/03
               CASH_VARIABLES(MO,Cash Consolidate Taxes Paid) =
     +                             CONSOLIDATED_VARIABLES(MO,
     +                                   Cash Federal Income Taxes Paid)


            ENDIF
            CASH_VARIABLES(MO,TOTAL TAX AND OTHER) =
     +                 + CASH_VARIABLES(MO,CASH TOTAL TAX PAYMENTS)
     +                 + CASH_VARIABLES(MO,
     +                       Cash Subsidiary State Taxes Paid by Parent)
     +                 - CASH_VARIABLES(MO,Cash Parent Taxes Paid)
     +                 - CASH_VARIABLES(MO,Cash Subsidiary Tax Payments)
     +                 + CASH_VARIABLES(MO,Cash Consolidate Taxes Paid)
C
...
C
            CASH_VARIABLES(MO,TOTAL FROM OTHER SOURCES) =
     +                     + CASH_VARIABLES(MO,Cash Subsidary Dividends)
     +                     + CASH_VARIABLES(MO,Cash Customer Deposits)
     +                     + CASH_VARIABLES(MO,Cash CIAC)
     +                     + CASH_VARIABLES(MO,Cash From Sale of Assets)
     +                     + CASH_VARIABLES(MO,
     +                              Cash From Repayment of Notes Issued)
     +                     + CASH_VARIABLES(MO,
     +                                 Cash From Notes Issued by Others)
C
C CONSTRUCTION
C
            CASH_VARIABLES(MO,Cash Total Construction) = 
     +                 SUM_CONSTRUCTION_CASH_PAYMENTS(MO,CASH_VARIABLES)
            CASH_VARIABLES(MO,TOTAL CAPITAL REQUIREMENTS) =
     +            SUM_CASH_TOTAL_CAPITAL_REQUIREMENTS(MO,CASH_VARIABLES)
            CASH_VARIABLES(MO,FUNDS FROM OPERATIONS) =
     +                  CASH_VARIABLES(MO,NET CASH RECEIPTS OR PAYMENTS)
     +                - CASH_VARIABLES(MO,CASH CAPITAL SERVICE PAYMENTS)
C    +                - CASH_VARIABLES(MO,CASH TOTAL TAX PAYMENTS)
     +                - CASH_VARIABLES(MO,TOTAL TAX AND OTHER)
     +                + CASH_VARIABLES(MO,TOTAL FROM OTHER SOURCES)
            CASH_VARIABLES(MO,CHANGE IN FUNDS BEFORE FINANCING) =
     +                   CASH_VARIABLES(MO,FUNDS FROM OPERATIONS)
     +                   - CASH_VARIABLES(MO,TOTAL CAPITAL REQUIREMENTS)
C
         ENDDO
...
C
     ...
     +                                               OPENING_STD_BALANCE
            BALANCE_SHEET_VARIABLES(0,Monthly Short Term Investments) =
     +                                                FUNDS_BALANCE(BOY)
            BALANCE_SHEET_VARIABLES(0,
     +                             Monthly Cash Long Term Investments) =
     +                                         BOY_LONG_TERM_INVESTMENTS
            CASH_VARIABLES(0,Cash Period Change) = 0.
            CASH_VARIABLES(0,EXTERNAL FINANCING PERFORMED) = 0.
            CASH_VARIABLES(1,Cash Opening Balance) = FUNDS_BALANCE(BOY)
            CASH_VARIABLES(0,Cash Opening Balance) = FUNDS_BALANCE(BOY)
            STD_BALANCE = OPENING_STD_BALANCE
            INCOME_VARIABLES(0,BTL Monthly Interest Income) = 0.
            CASH_VARIABLES(0,Cash STInvestment Interest Income) = 0.
c           CASH_VARIABLES(0,Cash Change LTInvestments) = 0.
            IF(CPL_ACTIVE()) THEN
               CASH_VARIABLES(7,Cash LTD Issued) =
     +                                 CASH_VARIABLES(7,Cash LTD Issued)
     +                                 - LONG_TERM_DEBT_ISSUED
            ...
            CASH_VARIABLES(0,Cash LTD Issued) = 0.
C
C
            DO MO = 1, 12
               LTD_ISSUED_DISTRIBUTION(MO) = 0.
               CASH_VARIABLES(MO,Cash Period Change) = 0
               CHANGE_IN_FUNDS =
     +               CASH_VARIABLES(MO,CHANGE IN FUNDS BEFORE FINANCING)
     +               + CASH_VARIABLES(MO,Cash Lease Receipts)
     +               + CASH_VARIABLES(MO,Cash to Cash)
               FUNDS_4_ST_INVESTMENTS = 0.
               IF(MO >= 7 .AND.
     +                CASH_VARIABLES(MO,Cash Opening Balance) <
     ...
     +                              - FUNDS_4_ST_INVESTMENTS
               ENDIF
               CAPITAL_ISSUES = CASH_VARIABLES(MO,Cash PS Issued)
     +                     + CASH_VARIABLES(MO,Cash LTD Issued)
     +                     + CASH_VARIABLES(MO,Cash Common Stock Issued)
               IF(CPL_ACTIVE()) THEN
     ...
     +                           STD_BALANCE > STD_MINIMUM_BALANCE) THEN
                        CASH_VARIABLES(MO,Cash STD Issued) =
     +                          -(MIN(CHANGE_IN_FUNDS,
     +                               STD_BALANCE - STD_MINIMUM_BALANCE))
                        AVAILABLE_FUNDS = AVAILABLE_FUNDS
     +                              + CASH_VARIABLES(MO,Cash STD Issued)
                     ENDIF
                     CASH_VARIABLES(MO,Cash Period Change) = 
     +                                                   AVAILABLE_FUNDS
     ...
     +                               CASH_VARIABLES(MO,Cash STD Issued))
                     ...
                     IF(CASH_NEEDED > 0.) THEN
                        IF(CASH_VARIABLES(MO,Cash Opening Balance) > 
     +                                        MINIMUM_CASH_BALANCE) THEN
                           CASH_VARIABLES(MO,Cash Period Change) =
     +                         -MIN(CASH_NEEDED,
     +                           CASH_VARIABLES(MO,Cash Opening Balance)
...
! it refers to the code below anymore.
     ...
     +                                                   AVAILABLE_FUNDS
                        ELSE
                           CASH_VARIABLES(MO,Cash STD Issued) = 0.
                           CASH_VARIABLES(MO,Cash Period Change) = 
     +                                                   AVAILABLE_FUNDS
                        ENDIF
                     ELSE ! CAPITAL ISSUES WERE REFUNDS
                        IF(CHANGE_IN_FUNDS + CAPITAL_ISSUES >= 0.) THEN ! ADD TO CASH
                           CASH_VARIABLES(MO,Cash STD Issued) = 0.
                           CASH_VARIABLES(MO,Cash Period Change) = 
     ...
     +                               Monthly Cash Long Term Investments)
     +                + CASH_VARIABLES(MO,Cash Change LTInvestments)/2.)
               IF(SALT_RIVER_PROJECT()) THEN  ! REPLACE WITH SWITCH
                  CASH_VARIABLES(MO,Cash Change LTInvestments) =
     +                      CASH_VARIABLES(MO,Cash Change LTInvestments)
     ...
     +                    + CASH_VARIABLES(MO,Cash Change LTInvestments)
               CASH_VARIABLES(MO,Cash Period Change) =
     +               CASH_VARIABLES(MO,CHANGE IN FUNDS BEFORE FINANCING)
     +               + CASH_VARIABLES(MO,Cash PS Issued)
     +               + CASH_VARIABLES(MO,Cash LTD Issued)
     +               + CASH_VARIABLES(MO,Cash STD Issued)
     +               + CASH_VARIABLES(MO,Cash Common Stock Issued)
     +               + CASH_VARIABLES(MO,Cash Lease Receipts)
               BALANCE_SHEET_VARIABLES(MO,monthly_short_term_debt_bal) =
     +                                                       STD_BALANCE
               CASH_VARIABLES(MO,Cash Closing Balance) = 
     +                         CASH_VARIABLES(MO,Cash Opening Balance)
     +                         + CASH_VARIABLES(MO,Cash Period Change)
               BALANCE_SHEET_VARIABLES(MO,
     +                                 Monthly Short Term Investments) =
     +                           CASH_VARIABLES(MO,Cash Closing Balance)
               BALANCE_SHEET_VARIABLES(MO,
     +                             Monthly Cash Long Term Investments) =
     +                    BALANCE_SHEET_VARIABLES(MO-1,
     ...
     +                    + CASH_VARIABLES(MO,Cash Change LTInvestments)
               IF(MO < 12) THEN
                  CASH_VARIABLES(MO+1,Cash Opening Balance) = 
     +                           CASH_VARIABLES(MO,Cash Closing Balance)
               ENDIF
               CASH_VARIABLES(MO,EXTERNAL FINANCING PERFORMED) =
     +                     CASH_VARIABLES(MO,Cash PS Issued)
     +                     + CASH_VARIABLES(MO,Cash LTD Issued)
     +                     + CASH_VARIABLES(MO,Cash STD Issued)
     +                     + CASH_VARIABLES(MO,Cash Common Stock Issued)
     +                     + CASH_VARIABLES(MO,Cash Lease Receipts)
C
               STI_EARNINGS(MO)=(CASH_VARIABLES(MO,Cash Closing Balance)
     +                        + CASH_VARIABLES(MO,Cash Opening Balance))
     +                         * MONTHLY_RETURN_ON_ST_INVEST(MO)/2.
            ...
            CASH_VARIABLES(:,Cash STInvestment Interest Income) = 
     ...
     +             CASH_VARIABLES(:,Cash STInvestment Interest Income)
            ...
            CASH_VARIABLES(0,Cash Period Change) =
     +                        SUM(CASH_VARIABLES(1:,Cash Period Change))
            CASH_VARIABLES(0,Cash STD Issued) =
     +                           SUM(CASH_VARIABLES(1:,Cash STD Issued))
            CASH_VARIABLES(0,Cash LTD Issued) =
     +                           SUM(CASH_VARIABLES(1:,Cash LTD Issued))
            CASH_VARIABLES(0,EXTERNAL FINANCING PERFORMED) =
     +              SUM(CASH_VARIABLES(1:,EXTERNAL FINANCING PERFORMED))
            CASH_VARIABLES(0,Cash Closing Balance) = 
     +                           CASH_VARIABLES(12,Cash Closing Balance)
C
C RE CALC 
C
            INCOME_VARIABLES(:,BTL Monthly STInvestmet Income) =
...
C
         CASH_2_LT_INVESTMENTS =
     +                       CASH_VARIABLES(0,Cash Change LTInvestments)
         FUNDS_BALANCE(EOY) = CASH_VARIABLES(12,Cash Closing Balance)
         STI_CHANGE_IN_CASH = FUNDS_BALANCE(EOY) - FUNDS_BALANCE(BOY)
         STD_ISSUED = 
     +          BALANCE_SHEET_VARIABLES(12,monthly_short_term_debt_bal)
     +          - BALANCE_SHEET_VARIABLES(0,monthly_short_term_debt_bal)

         R_STD_CASH_INTEREST = CASH_VARIABLES(0,Cash STD Interest)
     +                + CASH_VARIABLES(0,Cash Interest on Notes Payable)
         R_STD_INCOME_INTEREST=INCOME_VARIABLES(0,Monthly STD Interest)
         R_NON_INCOME_TAXES_ACCRUAL_ADJ =
     +              INCOME_VARIABLES(0,Monthly Other Taxes)
     +              + INCOME_VARIABLES(0,Monthly Property Taxes)
     +              + INCOME_VARIABLES(0,Monthly Operating Revenue Tax)
     +              + INCOME_VARIABLES(0,Monthly State Tax on Capital)
     +              + INCOME_VARIABLES(0,Monthly Federal Tax on Capital)
     +              + INCOME_VARIABLES(0,
     +                                WVPA Property Taxes in Power Cost)
     +              - CASH_VARIABLES(0,Cash Other Taxes)
     +              - CASH_VARIABLES(0,Cash Property Taxes)  
     +              - CASH_VARIABLES(0,Cash Operating Revenue Tax)  
     +              - CASH_VARIABLES(0,Cash State Tax on Capital)
     +              - CASH_VARIABLES(0,Cash Federal Tax on Capital)
         R_STATE_TAXES_ACCRUAL_ADJ =
     ...
     +                                SUB_STATE_TAX_PAYMENTS_2_PARENT(0)
         ...
         ENDIF
         R_OTHER_TAXES =  INCOME_VARIABLES(0,Monthly Other Taxes) 
     +                    - ANNUAL_VARS(216) ! environmental tax
         R_OP_REV_TAX=INCOME_VARIABLES(0,Monthly Operating Revenue Tax)
C
...
C
         FUEL_EXPENSE = INCOME_VARIABLES(0,Monthly Fossil Fuel)
         VARIABLE_EXPENSE = INCOME_VARIABLES(0,Monthly Variable OandM)
         FIXED_EXPENSE = INCOME_VARIABLES(0,Monthly Fixed OandM)
         CASH_REVENUES_RECEIVED =
     +               CASH_VARIABLES(0,Cash Received Accounts Receivable)
         CASH_EXPENSES_PAID =
     +                   CASH_VARIABLES(0,Cash Paid on Accounts Payable)
         CHANGE_IN_INVESTMENT_PAYABLE(:) =
     +          INCOME_VARIABLES(:,
     +                           Monthly Investment Earnings Receivable)
     +          + INCOME_VARIABLES(:,Monthly Total LTInvestment Income)
     +          + INCOME_VARIABLES(:,BTL Monthly STInvestmet Income)
     +          - CASH_VARIABLES(:,Cash Investment Dividend Earnings)
     +          - CASH_VARIABLES(:,Cash Investment Interest Earnings)
     +          - CASH_VARIABLES(:,Cash STInvestment Interest Income)  ! 112
         R_BTL_CASH_EARNINGS =
     +             CASH_VARIABLES(0,Cash STInvestment Interest Income)
     +             + CASH_VARIABLES(0,Cash Investment Dividend Earnings)
     +             + CASH_VARIABLES(0,Cash Investment Interest Earnings)
      RETURN
C***********************************************************************
...
C
     ...
     +                     - AVERAGE_VALUES(8)
            ...
            CONSOLIDATED_VARIABLES(4,Cash Federal Income Taxes Paid) =
     +                                                 AVERAGE_VALUES(3)
            CONSOLIDATED_VARIABLES(6,Cash Federal Income Taxes Paid) =
     +                                                 AVERAGE_VALUES(5)
            CONSOLIDATED_VARIABLES(9,Cash Federal Income Taxes Paid) =
     +                                                 AVERAGE_VALUES(8)
            CONSOLIDATED_VARIABLES(12,Cash Federal Income Taxes Paid) =
     +                                                AVERAGE_VALUES(12)
     ...
     +                                             + AVERAGE_VALUES(8)
     +                                             + AVERAGE_VALUES(12))
            CONSOLIDATED_VARIABLES(1:,Cash Federal Income Taxes Paid) =
     +                 CONSOLIDATED_VARIABLES(1:,
     +                                   Cash Federal Income Taxes Paid)
     +                  + PAYABLE_MONTHLY_VALUES(1:,
     +                             payment_federal_income_taxes_paid,-1)
            IF(FEDERAL_INC_TAX_CARRY_OVER(-1) >= 0) THEN
               CONSOLIDATED_VARIABLES(3,Cash Federal Income Taxes Paid)=
     ...
     +                  + FEDERAL_INC_TAX_CARRY_OVER(-1)
            ENDIF 
            CONSOLIDATED_VARIABLES(0,Cash Federal Income Taxes Paid) =
...
C
         ...
         IF(KCPL()) THEN
            AVERAGE_VALUES(3) = SUM(INCOME_VARIABLES(1:3,
     +                             Monthly Total State Income Tax Paid))
            AVERAGE_VALUES(5) = SUM(INCOME_VARIABLES(4:6,
     +                             Monthly Total State Income Tax Paid))
            AVERAGE_VALUES(8) = SUM(INCOME_VARIABLES(7:9,
     +                             Monthly Total State Income Tax Paid))
...
C
     ...
     +          CONSOLIDATED_VARIABLES(0,Cash Federal Income Taxes Paid)
         CONSOLIDATED_STATE_TAXES_PAID = 
     +            CONSOLIDATED_VARIABLES(0,Cash State Income Taxes Paid)
C
...
C
     ...
     +                     + TRANSFER_VARIABLES(MO,Monthly Net Goodwill)
         ENDDO
         CLASS_GOODWILL_EOY(CLASS_POS) = CumulatedBalances(12)
         BALANCE_SHEET_VARIABLES(0,Monthly Net Goodwill) = GOODWILL_BOY
         BALANCE_SHEET_VARIABLES(1:12,Monthly Net Goodwill) =
     +                BALANCE_SHEET_VARIABLES(1:12,Monthly Net Goodwill)
     +                + CumulatedBalances(1:12)
         ...
         DO VARIABLE = 1, BAL_SHEET_VARS
            DO MO = 0, 12
                  ...
                  IF(VARIABLE == Monthly ARO Net Asset Value) THEN
     ...
     +                   + BALANCE_LINKED_LEVEL(MO,VARIABLE,CLASS_LEVEL)
                  ELSEIF(VARIABLE == Monthly Net Goodwill) THEN
                     CYCLE
                  ELSE
                     BALANCE_SHEET_VARIABLES(MO,VARIABLE) =
     +                   BALANCE_SHEET_VARIABLES(MO,VARIABLE)
...
C
     ...
     +                                  MONTHLY_CHANGE_IN_RECEIVABLES,
     +                                  MONTHLY_CHANGE_IN_PAYABLES)
         CALL MONTHLY_PRODUCTION_RECEIVABLES_PAYABLES(
     +                                  MONTHLY_CHANGE_IN_RECEIVABLES,
     +                                  MONTHLY_CHANGE_IN_PAYABLES)
...
C
     ...
     +                                        CURRENT_PORTION_OF_LTD(MO)
               ...
               BALANCE_SHEET_VARIABLES(MO,
     +                               Monthly Unamortized Interest Bal) =
     +                                 BOY_DEBIT_UNAMORT_INTEREST

               BALANCE_SHEET_VARIABLES(MO,
     +                              monthly_unamortized_issue_exp_bal) = ! 77
     +                                                 BOY_ISSUE_EXP_BAL
               BALANCE_SHEET_VARIABLES(MO,
     +                                  Monthly Subsidiary Investment) =
     +                                    BOY_INVESTMENT_IN_SUBSIDIARIES
               BALANCE_SHEET_VARIABLES(MO,Monthly Capitialized Leases) =
     +                                           BOY_CAPITIALZIED_LEASES
               BALANCE_SHEET_VARIABLES(MO,
     +                                Monthly Nuc Decommiss Trust Bal) =
     +                                            BOY_NUC_DECOM_FUND_BAL
               BALANCE_SHEET_VARIABLES(MO,Monthly Net Nuclear Fuel) =
     +                                            BOY_CLASS_NET_NF_VALUE
               BALANCE_SHEET_VARIABLES(MO,Monthly CWIP) = BOY_CWIP
               BALANCE_SHEET_VARIABLES(0,
     +                               Monthly Accumulated Depreciation) =
     +                                           ACCUMLATED_BOOK_DEP_BOY
               BALANCE_SHEET_VARIABLES(0,Monthly Deferred Revenues) =
     +                                              CUM_DEF_REVENUES_BOY
               BALANCE_SHEET_VARIABLES(0,Monthly Net Deferred Debits) =
     +                                               DEFERRED_DEBITS_BOY
               BALANCE_SHEET_VARIABLES(0,
     +                               Monthly Deferred Income Taxes Dr) =
     +                                             DEFERRED_TAXES_DR_BOY
               BALANCE_SHEET_VARIABLES(0,Monthly Current Assets) =
     +                                                    ASSETS_NEC_BOY
               BALANCE_SHEET_VARIABLES(0,
     +                                monthly_accounts_receivable_bal) =
     +                                           ACCOUNTS_RECEIVABLE_BOY
               BALANCE_SHEET_VARIABLES(0,Monthly Deferred Fuel Bal) =
     +               ANNUAL_VARS(392)
     +               + INCOME_VARIABLES(0,Monthly Deferred Fuel Expense)
...
C
     ...
     +                                                    BOY_PS_BALANCE
C
               BALANCE_SHEET_VARIABLES(0,Monthly Fuel Inventory Bal) =
     +                                                FUEL_INVENTORY_BOY

               BALANCE_SHEET_VARIABLES(0,Monthly Gas in Storage Bal) =
     +                                                   GAS_STORAGE_BOY
               BALANCE_SHEET_VARIABLES(0,
     +                                 Monthly Materials Supplies Bal) =
     +                                            MATERIALS_SUPPLIES_BOY
     ...
     +                                                 NOTES_PAYABLE_BOY
               BALANCE_SHEET_VARIABLES(0,
     +                             monthly_deferred_purchase_powr_bal) =
     +                                       DEFERRED_PURCHASE_POWER_BOY
               BALANCE_SHEET_VARIABLES(0,Monthly Accrued Pension) =
     +                                             PENSION_LIABILITY_BOY
     ...
     +                                          IncentiveCompBalance_BOY
               BALANCE_SHEET_VARIABLES(0,Monthly Accrued Vacation Pay) =
     +                                          VACATION_PAY_BALANCE_BOY
               BALANCE_SHEET_VARIABLES(0,Monthly Other Investments) =
     +                                     OTHER_INVESTMENTS_BALANCE_BOY
               BALANCE_SHEET_VARIABLES(0,Monthly Deferred Income Taxes)=
     +                                             DEFERRED_TAXES_CR_BOY
               BALANCE_SHEET_VARIABLES(0,Monthly Deferred ITC Credit) =
     +                                                 DEFERRED_ITCS_BOY
               BALANCE_SHEET_VARIABLES(0,
     ...
     +                                              ACCOUNTS_PAYABLE_BOY
               BALANCE_SHEET_VARIABLES(0,Monthly CICA) = CIAC_BALANCE(0)

               BALANCE_SHEET_VARIABLES(0,Monthly Liabilities NEC) =
     +                                    LIABS_NEC_BOY
      BALANCE_SHEET_VARIABLES(0,monthly_nucl_decommiss_liability) =  ! 37
     +                                  BOY_NUC_DECOM_FUND_LIABILITY
               BALANCE_SHEET_VARIABLES(0,
     +                             Monthly Post Retre Medical Payable) =  ! 43
     +                                       BOY_POST_RETIRE_MED_PAYABLE
               BALANCE_SHEET_VARIABLES(0,
     +                               Monthly Post Retire Medical Fund) =  ! 11
     +                                      BOY_POST_RETIRE_MEDICAL_FUND
               BALANCE_SHEET_VARIABLES(MO,
     +                             Monthly Debt Long Term Investments) =  ! 24
     +                                                   INVESTMENTS_BOY
               BALANCE_SHEET_VARIABLES(MO,
     +                                  Monthly Long Term Investments) =  ! 15
     +                    BALANCE_SHEET_VARIABLES(MO,
     +                               Monthly Cash Long Term Investments)  ! 23
     +                    + BALANCE_SHEET_VARIABLES(MO,
     +                               Monthly Debt Long Term Investments)  ! 24
            ELSE
C
C 9/5/00 NEED TO ADD ACQUISTION OF NEW SUB.
     ...
     +               + INCOME_VARIABLES(MO,
     +                  Monthly Change in Debit File Interest Exp Amort)
                  BALANCE_SHEET_VARIABLES(MO,
     +                              monthly_unamortized_issue_exp_bal) = ! 77
     +               BALANCE_SHEET_VARIABLES(MO-1,
     +                                monthly_unamortized_issue_exp_bal) ! 77
     +               + CASH_VARIABLES(MO,Cash Common Issue Expense)
     +  - INCOME_VARIABLES(MO,monthly_common_stock_issue_amort)
     +               + INCOME_VARIABLES(MO,
     +     Monthly Change in Debit File Issue Exp Amort)
                  BALANCE_SHEET_VARIABLES(MO,
     +                                  Monthly Subsidiary Investment) =
     +               BALANCE_SHEET_VARIABLES(MO-1,
     +                                    Monthly Subsidiary Investment)
     +               + INCOME_VARIABLES(MO,Monthly Subsidiary Income)
     +               - CASH_VARIABLES(MO,Cash Subsidary Dividends)
     +               + CASH_VARIABLES(MO,Cash Equity to Subsidiaries)
                 BALANCE_SHEET_VARIABLES(MO,Monthly Net Nuclear Fuel) =
     +            BALANCE_SHEET_VARIABLES(MO-1,Monthly Net Nuclear Fuel)
     +            + CASH_VARIABLES(MO,Cash Nuclear Fuel Fabrication)
     +            - INCOME_VARIABLES(MO,Monthly Nuclear Fuel Expense)
     +            + ANNUAL_VARS(298) /12.
C
                  BALANCE_SHEET_VARIABLES(MO,
     +                                    Monthly Capitialized Leases) =
     +                  BALANCE_SHEET_VARIABLES(MO-1,
     +                                      Monthly Capitialized Leases)
     +                  + CASH_VARIABLES(MO,Cash Lease Receipts)
     +                  - INCOME_VARIABLES(MO,
     +                               Monthly Lease Amortization Expense)
     +                  - INCOME_VARIABLES(MO,
     ...
     +                BALANCE_SHEET_VARIABLES(MO-1,
     +                                  Monthly Nuc Decommiss Trust Bal)
     +                + CASH_VARIABLES(MO,Cash Nuc Decommissioning Fund) 
     ...
     +                       BALANCE_SHEET_VARIABLES(MO-1,
     +                                        Monthly Deferred Revenues)
     +                       + INCOME_VARIABLES(MO,Deferred Revenues)
     +                       - INCOME_VARIABLES(MO,
     +                            Monthly Deferred Revenue Amortization)
     +                       + INCOME_VARIABLES(MO,
     +                             Monthly WVPA Accrued Member Revenues)
     +                       + CASH_VARIABLES(MO,
     +                              WVPA Cash to Accrued Member Revenue)
                  IF(SALT_RIVER_PROJECT()) THEN
      BALANCE_SHEET_VARIABLES(MO,monthly_nucl_decommiss_liability) =  ! 37
     + BALANCE_SHEET_VARIABLES(MO-1,monthly_nucl_decommiss_liability)
     +                    + MONTHLY_OCI_NUC_DECOM_FUND_RETURN(MO)
                  ELSE
                     BALANCE_SHEET_VARIABLES(MO,
     + monthly_nucl_decommiss_liability) =  ! 37
     +                BALANCE_SHEET_VARIABLES(MO-1,
     +   monthly_nucl_decommiss_liability)
     +    + INCOME_VARIABLES(MO,Monthly DOE Decommissioning)
     +     + MONTHLY_NUC_DECOM_FUND_NET_OF_TAX_RETURN(MO)
     ...
     +                             Monthly Post Retre Medical Payable) =  ! 43
     +                BALANCE_SHEET_VARIABLES(MO-1,
     +                               Monthly Post Retre Medical Payable)
     +                + CASH_VARIABLES(MO,Cash Post Retirement Payments)
     +                - CASH_VARIABLES(MO,Cash Retiree Medical Payments)
                  BALANCE_SHEET_VARIABLES(MO,
     +                               Monthly Post Retire Medical Fund) =  ! 11
     +                BALANCE_SHEET_VARIABLES(MO-1,
     +                                 Monthly Post Retire Medical Fund)
     +             + CASH_VARIABLES(MO,Cash Post Retirement Payments)
     +             + CASH_VARIABLES(MO,Cash to Post Retirement Payments)
     +             - CASH_VARIABLES(MO,Cash Retiree Medical Payments)
     ...
     +                             Monthly Debt Long Term Investments) =  ! 24
     +                 BALANCE_SHEET_VARIABLES(MO-1,
     +                             Monthly Debt Long Term Investments)
     +                 + CASH_VARIABLES(MO,Cash Change Debt Investments)
               BALANCE_SHEET_VARIABLES(MO,
     +                                  Monthly Long Term Investments) =  ! 15
     +                    BALANCE_SHEET_VARIABLES(MO,
     +                               Monthly Cash Long Term Investments)  ! 23
     ...
     +                  + INCOME_VARIABLES(MO,BTLMonthlyDeferralTaxCr)
                  ENDIF
                  BALANCE_SHEET_VARIABLES(MO,Monthly CWIP) =
     +                        BALANCE_SHEET_VARIABLES(MO-1,Monthly CWIP)
     +                      + INCOME_VARIABLES(MO,Monthly AFUDC Total)
     +                      + CASH_VARIABLES(MO,Cash Plant Construction)
     +                      - BALANCE_SHEET_VARIABLES(MO,
     +                                       Monthly AFUDC Capitialized)
     +                      - BALANCE_SHEET_VARIABLES(MO,
     +                                       Monthly Plant Capitialized)
                  BALANCE_SHEET_VARIABLES(MO,
     +                                    Monthly Net Deferred Debits) =
     +               BALANCE_SHEET_VARIABLES(MO-1,
     +                                      Monthly Net Deferred Debits)
     +               + CASH_VARIABLES(MO,Cash Deferred Expenses)
     +               - CHANGE_IN_DEFERRED_DEBIT_BAL(MO)
     +               - INCOME_VARIABLES(MO,Monthly Issue Expense Amorts)
     +               + CASH_VARIABLES(MO,Cash LTD PS Issue Expense)
     +               + CASH_VARIABLES(MO,Cash Common Issue Expense)
     +               + GOODWILL_OF_NEW_SUB_INVESTMENT(MO)               !Goodwill
     +               + TRANSFER_VARIABLES(MO,Monthly Net Goodwill)      !Goodwill
                  IF(CLASS_TYPE == PARENT) THEN
                     BALANCE_SHEET_VARIABLES(MO,
     +                            monthly_other_long_term_liabs) =
     +                        BALANCE_SHEET_VARIABLES(MO-1,
     +                              monthly_other_long_term_liabs)
     +                     - CASH_VARIABLES(MO,Cash Leased Nuclear Fuel)! - NUCLEAR_FUEL_LEASE_PAYMENTS
     +                     + CASH_VARIABLES(MO,Cash Lease Receipts)     ! + CAPITIALIZED_LEASE_ADDITIONS
     +                     - CASH_VARIABLES(MO,Cash Lease Expense)      ! - ATL_LEASE_PAYMENTS
     +                     - CASH_VARIABLES(MO,Cash BTL Lease Cash)     ! - BTL_LEASE_PAYMENTS
...
C NOTES PAYABLE INTEREST
     +               + INCOME_VARIABLES(MO,
     +                                Monthly Interest on Notes Payable)
     +               - CASH_VARIABLES(MO,Cash Interest on Notes Payable)
C NON_INCOME_TAXES_ACCRUAL_ADJ
     +               + INCOME_VARIABLES(MO,Monthly Other Taxes)
     +               + INCOME_VARIABLES(MO,Monthly Property Taxes)
     +               + INCOME_VARIABLES(MO,
...
C R_FEDERAL_TAXES_ACCRUAL_ADJ
     ...
     +                                Monthly Total Fed Income Tax Paid)
     +               + CASH_VARIABLES(MO,
     +                               Cash Subsidiary Federal Taxes Paid)
     +               - CONSOLIDATED_VARIABLES(MO,
     +                                   Cash Federal Income Taxes Paid)
C WVPA ADDITIONS
...
C
     ...
     +                + INCOME_VARIABLES(MO,Unbilled Revenues)
     +                + INCOME_VARIABLES(MO,
     +                           Monthly Investment Earnings Receivable)
     +                + INCOME_VARIABLES(MO,
     ...
     +                                Cash Investment Interest Earnings)
     +                - CASH_VARIABLES(MO,
     +                                Cash STInvestment Interest Income)  ! 112
C WVPA RECEIVABLES
     +                + WVPA_OFF_SYSTEM_SALES_RECEIVABLE(MO)
C
               BALANCE_SHEET_VARIABLES(MO,Monthly CICA)=CIAC_BALANCE(MO)
               BALANCE_SHEET_VARIABLES(MO,Monthly Liabilities NEC) =
     +                                                     LIABS_NEC(MO)
               BALANCE_SHEET_VARIABLES(MO,Monthly Current Assets) =
     +                                                    ASSETS_NEC(MO)
               BALANCE_SHEET_VARIABLES(MO,Monthly Deferred ITC Credit) =
     +                   BALANCE_SHEET_VARIABLES(MO-1,
     +                                      Monthly Deferred ITC Credit)
     +                   + INCOME_VARIABLES(MO,Monthly ITC Amortization)
               BALANCE_SHEET_VARIABLES(MO,Monthly Other Investments) =
     +           BALANCE_SHEET_VARIABLES(MO-1,Monthly Other Investments)
     +                 + INCOME_VARIABLES(MO,MonthlyMarkToMarket)
     +                 + INCOME_VARIABLES(MO,MonthlyEarningNonCompany)
     +                 + CASH_VARIABLES(MO,Cash Net Investments)
               BALANCE_SHEET_VARIABLES(MO,Monthly Deferred Fuel Bal) =
     +           BALANCE_SHEET_VARIABLES(MO-1,Monthly Deferred Fuel Bal)
     +           - INCOME_VARIABLES(MO,Monthly Deferred Fuel Expense)
C
               BALANCE_SHEET_VARIABLES(MO,
     +                               Monthly Accumulated Depreciation) =
     +            + BALANCE_SHEET_VARIABLES(MO-1,
     +                                 Monthly Accumulated Depreciation)
     +            - BALANCE_SHEET_VARIABLES(MO,Monthly Retirements)
     +            + INCOME_VARIABLES(MO,Monthly Book Depreciation)
     +            + BALANCE_SHEET_VARIABLES(MO,
     +                                      Monthly Acc Dep Adjustments)
     +            - CASH_VARIABLES(MO,Cash Net Salvage)
     +            + BALANCE_SHEET_DEACTIVE_VARS(MO,
     +                                 Monthly Accumulated Depreciation,
     +                                                      CLASS_LEVEL)
C
               BALANCE_SHEET_VARIABLES(MO,Monthly Fuel Inventory Bal) =
     +               BALANCE_SHEET_VARIABLES(MO-1,
     +                                       Monthly Fuel Inventory Bal)
     +               + CASH_VARIABLES(MO,Cash Fuel Inventory Change)
     +               - INCOME_VARIABLES(MO,Monthly Fuel Inventory Amort)
C
               BALANCE_SHEET_VARIABLES(MO,Monthly Gas in Storage Bal) =
     +                  BALANCE_SHEET_VARIABLES(MO-1,
     +                                       Monthly Gas in Storage Bal)
     +                  + CASH_VARIABLES(MO,
     +                              Cash Gas Storage Inventory Addition)
     +                  - INCOME_VARIABLES(MO,Monthly Gas Storage Amort) 
C
               BALANCE_SHEET_VARIABLES(MO,
     +                                 Monthly Materials Supplies Bal) =
     +                   BALANCE_SHEET_VARIABLES(MO-1,
     +                                   Monthly Materials Supplies Bal)
     +                   + CASH_VARIABLES(MO,
     +                                Cash Materials Inventory Addition)
     +                   - INCOME_VARIABLES(MO,
     +                                Monthly Material n Supplies Amort)
C
               BALANCE_SHEET_VARIABLES(MO,
...
C
     ...
     +                + CASH_VARIABLES(MO,Cash Common Stock Issued)
     +                - CASH_VARIABLES(MO,Cash Common Stock Buyback)
C
               BALANCE_SHEET_VARIABLES(MO,monthly_preferred_stock) =
     +             BALANCE_SHEET_VARIABLES(MO-1,monthly_preferred_stock)
     +             + CASH_VARIABLES(MO,Cash PS Issued)
     +             - CASH_VARIABLES(MO,Cash PS Retirements)
     +             - INCOME_VARIABLES(MO,Monthly PS Premium Amort)
C
               BALANCE_SHEET_VARIABLES(MO,monthly_long_term_debt) =
     +              BALANCE_SHEET_VARIABLES(MO-1,monthly_long_term_debt)
     +              + CASH_VARIABLES(MO,Cash LTD Issued)
     +              - CASH_VARIABLES(MO,Cash LTD Retirements)
     +              - INCOME_VARIABLES(MO,Monthly LTD Premium Amort)
C
     ...
     +               - CASH_VARIABLES(MO,Cash For Redeming Notes Owed)
               BALANCE_SHEET_VARIABLES(MO,Monthly Accrued Pension) =
     +            BALANCE_SHEET_VARIABLES(MO-1,Monthly Accrued Pension)
     +            + INCOME_VARIABLES(MO,Monthly Pension Expense)
     +            - CASH_VARIABLES(MO,Cash Unfunded Retirement Payments)
     +            + FASB87_PENSION_LIAB_ADJ(MO)
     +            + CASH_VARIABLES(MO,Non Cash Pension in CapX)
               BALANCE_SHEET_VARIABLES(MO,Monthly Strom Reserve) =
     +               BALANCE_SHEET_VARIABLES(MO-1,Monthly Strom Reserve)
     +               + INCOME_VARIABLES(MO,Monthly Storm Expense)
     +               - CASH_VARIABLES(MO,Cash Storm Payments)
     ...
     +                - CASH_VARIABLES(MO,CashIncentiveCompReserve)
               BALANCE_SHEET_VARIABLES(MO,Monthly Accrued Vacation Pay)=
     +              BALANCE_SHEET_VARIABLES(MO-1,
     +                                     Monthly Accrued Vacation Pay)
     +              + INCOME_VARIABLES(MO,Monthly Vacation Pay)
     +              - CASH_VARIABLES(MO,Cash Vacation Payments)
C
...
C
         BALANCE_SHEET_VARIABLES(:,Monthly Unamortized Interest Bal) =
     +               BALANCE_SHEET_VARIABLES(:,
     +                                 Monthly Unamortized Interest Bal)
     +               + UNAMORTIZED_INTEREST_BALANCE(:)
C
...
C
         DO MO = 0, 12
               ...
               BALANCE_SHEET_VARIABLES(MO,Monthly CWIP) =
     +                          BALANCE_SHEET_VARIABLES(MO,Monthly CWIP)
     +                          + TRANSFER_VARIABLES(MO,Monthly CWIP)
               BALANCE_SHEET_VARIABLES(MO,
     +                            monthly_other_long_term_liabs) =
     +                BALANCE_SHEET_VARIABLES(MO,
     +                              monthly_other_long_term_liabs)
     ...
     +                              monthly_other_long_term_liabs)
               BALANCE_SHEET_VARIABLES(MO,Monthly Net Nuclear Fuel) =
     +            BALANCE_SHEET_VARIABLES(MO,Monthly Net Nuclear Fuel)
     +            + TRANSFER_VARIABLES(MO,Monthly Net Nuclear Fuel)
               BALANCE_SHEET_VARIABLES(MO,Monthly Capitialized Leases) =
     +           BALANCE_SHEET_VARIABLES(MO,Monthly Capitialized Leases)
     +           + TRANSFER_VARIABLES(MO,Monthly Capitialized Leases)
               BALANCE_SHEET_VARIABLES(MO,monthly_common_stock) =
     +                  BALANCE_SHEET_VARIABLES(MO,monthly_common_stock)
     +                  + TRANSFER_VARIABLES(MO,monthly_common_stock)
               BALANCE_SHEET_VARIABLES(MO,
     +                                  Monthly Subsidiary Investment) = ! 7
     +                     BALANCE_SHEET_VARIABLES(MO,
     +                                    Monthly Subsidiary Investment) ! 7
     +                     + NEW_SUB_INVESTMENT(MO)
c
...
C Deleted a large chunk of code below this comment.
               ...
               BALANCE_SHEET_VARIABLES(MO,Monthly Net Other Debits) =
     +                  BALANCE_SHEET_VARIABLES(MO,
     +                                      Monthly Net Deferred Debits)
     +                  - BALANCE_SHEET_VARIABLES(MO,
     +                                             Monthly Net Goodwill) ! 72
     +                  - BALANCE_SHEET_VARIABLES(MO,
     +                                    Monthly Net Regulatory Assets) ! 73
     +                  - BALANCE_SHEET_VARIABLES(MO,
     +                                             Monthly Net FASB 109) ! 74
     +                  - BALANCE_SHEET_VARIABLES(MO,
     +                                             Monthly Net FASB 133) ! 75
     +                  - BALANCE_SHEET_VARIABLES(MO,
     +                                 Monthly Unamortized Interest Bal) ! 76
     +                  - BALANCE_SHEET_VARIABLES(MO,
     +                                monthly_unamortized_issue_exp_bal) ! 77
...
C
     ...
     +                               CLASS_TYPE == REGULATED_GROUP) THEN
                  IF(VARIABLE==Monthly Cash Long Term Investments) CYCLE
                  IF(VARIABLE==Monthly Debt Long Term Investments) CYCLE
                  IF(VARIABLE == Monthly Short Term Investments) CYCLE
...
C
               DO MO = 0, 12
                  IF(.NOT. CLASS_IS_ACTIVE(MO)) THEN
                     ...
                     IF(VARIABLE==Monthly Accumulated Depreciation .AND.
...
C
     ...
     +                                          THIS_IS_REPORTING_CLASS)
            DO VARIABLE = 1, INCOME_VARS
               IF(VARIABLE == Monthly Issue Expense Amorts) CYCLE
               IF(VARIABLE == Monthly PS Amort Dividends) CYCLE
               IF(CLASS_TYPE == SBU .OR.
     +                               CLASS_TYPE == REGULATED_GROUP) THEN
                  IF(VARIABLE == Monthly Investment Earnings) CYCLE
                  IF(VARIABLE == Monthly Investment Earnings Receivable)
     +                                                             CYCLE
                  IF(VARIABLE == Monthly LTD Total Interest) CYCLE
                  IF(VARIABLE == Monthly STD Interest) CYCLE
                  IF(VARIABLE == Monthly Interest on Notes Payable)CYCLE
                  IF(VARIABLE == Monthly Total PS Dividends) CYCLE
                  IF(VARIABLE == Monthly Common Dividends) CYCLE
                  IF(VARIABLE == Monthly Total LTInvestment Income)CYCLE
                  IF(VARIABLE == Monthly Model LTInvestment Income)CYCLE
                  IF(VARIABLE ==
     +                        Monthly Debt File LInvestment Income)CYCLE
                  IF(VARIABLE == monthly_notes_receivable_income)CYCLE
                  IF(VARIABLE == Monthly Dividend 70 Earnings)CYCLE
                  IF(VARIABLE == Monthly Regular Dividend Income)CYCLE

                  IF(VARIABLE == BTL Monthly STInvestmet Income) CYCLE
                  IF(VARIABLE == BTL Monthly Interest Income) CYCLE
                  IF(VARIABLE == BTL Monthly Income Taxes) CYCLE
                  IF(VARIABLE == Monthly Shares Outstanding) CYCLE
                  IF(VARIABLE == Monthly Federal ATL Income Tax Paid)
     +                                                             CYCLE
                  IF(VARIABLE == Monthly Federal BTL Income Tax Paid)
     +                                                             CYCLE
                  IF(VARIABLE == Monthly Total Fed Income Tax Paid)
     +                                                             CYCLE
                  IF(VARIABLE == Monthly LTD Premium Amort) CYCLE
                  IF(VARIABLE == Monthly PS Premium Amort) CYCLE

                  IF(.NOT. EXCLUDE_SBU_PROPERTY_TAX) THEN
                     IF(VARIABLE == Monthly Property Taxes) CYCLE
                     IF(VARIABLE==Monthly Exp File Property Taxes) CYCLE
                     IF(VARIABLE == 
     +                    Monthly Model Calculated Property Taxes) CYCLE
                  ENDIF
                  IF(.NOT. EXCLUDE_SBU_REVENUE_TAX .AND. 
     +                  VARIABLE == Monthly Operating Revenue Tax) CYCLE

                  IF(VARIABLE == Monthly Other Taxes) CYCLE
                  IF(.NOT. EXCLUDE_SBU_STATE_INCOME_TAX) THEN
                     IF(VARIABLE == Monthly State ATL Income Tax Paid)
     +                                                             CYCLE
                     IF(VARIABLE == Monthly State BTL Income Tax Paid)
     +                                                             CYCLE
                     IF(VARIABLE == Monthly Total State Income Tax Paid)
     +                                                             CYCLE
                  ENDIF
     ...
     +                VARIABLE == Monthly Federal Tax on Capital)) CYCLE
               ELSEIF(CLASS_TYPE == SUBSIDIARY) THEN
                  ...
                  IF(VARIABLE == Monthly Subsidiary Income) CYCLE
                  IF(VARIABLE == Monthly Shares Outstanding) CYCLE
     ...
     +                                               SUB_LINK_2_CLASS) =
     +                     INCOME_LINKED_LEVEL(MO,
     +                       Monthly Subsidiary Income,SUB_LINK_2_CLASS)
     +                     + INCOME_VARIABLES(MO,
     +                                        Monthly Earnings 2 Common)
                        ENDIF
                     ENDDO
                     INCOME_LINKED_LEVEL(0,Monthly Subsidiary Income,
     +                                               SUB_LINK_2_CLASS) =
     ...
     +                            SUM(SUB_TAXABLE_INCOME_B4_DEDUCTS(1:))
         ...
         IF(CLASS_LINK_2 >= -1 .AND. .NOT. DONT_TALLY_THIS_CLASS) THEN
            DO VARIABLE = 1, CASH_VARS
               IF(CLASS_TYPE == SBU .OR.
     +                               CLASS_TYPE == REGULATED_GROUP) THEN
                  IF(VARIABLE==Cash From Repayment of Notes Issued)CYCLE
                  IF(VARIABLE == Cash For Redeming Notes Owed) CYCLE
                  IF(VARIABLE == Cash For New Notes Issued) CYCLE
                  IF(VARIABLE == Cash From Notes Issued by Others) CYCLE
                  IF(VARIABLE == Cash Interest on Notes Payable) CYCLE
                  IF(VARIABLE == Cash Investment Dividend Earnings)CYCLE
                  IF(VARIABLE == Cash Investment Interest Earnings)CYCLE
                  IF(VARIABLE == Cash LTD Interest) CYCLE
                  IF(VARIABLE == Cash STD Interest) CYCLE
                  IF(VARIABLE == Cash PS Dividends) CYCLE
                  IF(VARIABLE == Cash Common Dividends) CYCLE
                  IF(VARIABLE == Cash State Income Taxes Paid) CYCLE
                  IF(VARIABLE == Cash Federal Income Taxes Paid) CYCLE
                  IF(VARIABLE == Cash State Tax on Capital) CYCLE
                  IF(VARIABLE == Cash Federal Tax on Capital) CYCLE
                  IF(VARIABLE == Cash Parent Taxes Paid) CYCLE
                  IF(VARIABLE == Cash Subsidiary Tax Payments) CYCLE
                  IF(VARIABLE==Cash Subsidiary Federal Taxes Paid) CYCLE
                  IF(VARIABLE==Cash Subsidiary State Taxes Paid) CYCLE
                  IF(VARIABLE == Cash Consolidate Taxes Paid) CYCLE

                  IF(VARIABLE == Cash PS Retirements) CYCLE
                  IF(VARIABLE == Cash LTD Retirements) CYCLE
                  IF(VARIABLE == Cash Common Stock Buyback) CYCLE
                  IF(VARIABLE == Cash PS Issued) CYCLE
                  IF(VARIABLE == Cash LTD Issued) CYCLE
                  IF(VARIABLE == Cash STD Issued) CYCLE
                  IF(VARIABLE == Cash Common Stock Issued) CYCLE
                  IF(VARIABLE == Cash Closing Balance) CYCLE
                  IF(VARIABLE == Cash Opening Balance) CYCLE
                  IF(VARIABLE == Cash Period Change) CYCLE
                  IF(VARIABLE == Common Shares Issued) CYCLE
                  IF(VARIABLE == Cash Common Issue Expense) CYCLE
                  IF(VARIABLE == Cash LTD PS Issue Expense) CYCLE
                  IF(VARIABLE == Cash Change LTInvestments) CYCLE
                  IF(VARIABLE == Cash Change Debt Investments) CYCLE
c
c to cover the excluded items
c
                  IF(VARIABLE ==
     +                       Cash Model Calculated Property Taxes) CYCLE  ! 9/9/99 as fix for double paying
                  IF(VARIABLE == Cash Operating Revenue Tax) CYCLE  ! 9/9/99 as fix for double paying
                  IF(VARIABLE == Cash Other Taxes) CYCLE  ! 9/9/99 as fix for double paying

                  IF(.NOT. EXCLUDE_SBU_PROPERTY_TAX) THEN
                     IF(VARIABLE == Cash Property Taxes) CYCLE
                     IF(VARIABLE == Cash Exp File Property Taxes) CYCLE
                     IF(VARIABLE == 
     +                       Cash Model Calculated Property Taxes) CYCLE
                  ENDIF
                  IF(.NOT. EXCLUDE_SBU_REVENUE_TAX .AND. 
     +                     VARIABLE == Cash Operating Revenue Tax) CYCLE

                  IF(VARIABLE == Cash Other Taxes) CYCLE
                  IF(.NOT. EXCLUDE_SBU_STATE_INCOME_TAX .AND. 
     +                   VARIABLE == Cash State Income Taxes Paid) CYCLE
                  IF(.NOT. EXCLUDE_SBU_CAPITAL_TAX .AND. 
     +                  (VARIABLE == Cash State Tax on Capital .OR.
     +                   VARIABLE == Cash Federal Tax on Capital)) CYCLE
               ELSEIF(CLASS_TYPE == SUBSIDIARY) THEN

                  IF(VARIABLE == Cash Subsidary Dividends) CYCLE
                  IF(VARIABLE == Cash Equity to Subsidiaries) CYCLE
                  IF(VARIABLE == Common Shares Issued) CYCLE
C
C MOVE TO PARENT AND TO CONSOLIDATED
C
                  IF(VARIABLE == Cash Common Dividends) THEN
                     DO MO = 1, 12
                        IF(CLASS_IS_ACTIVE(MO)) THEN

                           CASH_LINKED_LEVEL(MO,
     +                          Cash Subsidary Dividends,CLASS_LINK_2) =
     +                        CASH_LINKED_LEVEL(MO,
     +                            Cash Subsidary Dividends,CLASS_LINK_2)
     +                        + CASH_VARIABLES(MO,Cash Common Dividends)
                           CASH_LINKED_LEVEL(MO,
     +                       Cash Subsidary Dividends,SUB_LINK_2_CLASS)=
     +                        CASH_LINKED_LEVEL(MO,
     +                        Cash Subsidary Dividends,SUB_LINK_2_CLASS)
     +                        + CASH_VARIABLES(MO,Cash Common Dividends)
                        ENDIF
     ...
     +                       Cash Subsidary Dividends,SUB_LINK_2_CLASS))
                     CYCLE
                  ENDIF
                  IF(VARIABLE == Cash Common Stock Issued) THEN
                     DO MO = 1, 12
                        IF(CLASS_IS_ACTIVE(MO)) THEN

                           CASH_LINKED_LEVEL(MO,
     +                       Cash Equity to Subsidiaries,CLASS_LINK_2) =
     +                     CASH_LINKED_LEVEL(MO,
     +                         Cash Equity to Subsidiaries,CLASS_LINK_2)
     +                     + CASH_VARIABLES(MO,Cash Common Stock Issued)
                           CASH_LINKED_LEVEL(MO,
     +                    Cash Equity to Subsidiaries,SUB_LINK_2_CLASS)=
     +                   CASH_LINKED_LEVEL(MO,
     +                     Cash Equity to Subsidiaries,SUB_LINK_2_CLASS)
     +                   + CASH_VARIABLES(MO,Cash Common Stock Issued)
                        ENDIF
     ...
     +                    Cash Equity to Subsidiaries,SUB_LINK_2_CLASS))
                     CYCLE
                  ENDIF
                  IF(VARIABLE == Cash Subsidiary Federal Taxes Paid)THEN
                     DO MO = 1, 12
                        IF(CLASS_IS_ACTIVE(MO)) THEN
                           CASH_LINKED_LEVEL(MO,
     +                               Cash Subsidiary Federal Taxes Paid,
     ...
     +                                                SUB_LINK_2_CLASS))
                     CYCLE
                  ENDIF
                  IF(VARIABLE == Cash Subsidiary State Taxes Paid) THEN
                     DO MO = 1, 12
                        IF(CLASS_IS_ACTIVE(MO)) THEN
                           CASH_LINKED_LEVEL(MO,
     +                                 Cash Subsidiary State Taxes Paid,
     +                                               SUB_LINK_2_CLASS) =
     +                           CASH_LINKED_LEVEL(MO,
     +                                 Cash Subsidiary State Taxes Paid,
     +                                                 SUB_LINK_2_CLASS)
     ...
     +                                                SUB_LINK_2_CLASS))
                  ...
                  ENDIF
                  IF(VARIABLE ==
     +                  Cash Subsidiary State Taxes Paid by Parent) THEN
                     DO MO = 1, 12
                        IF(CLASS_IS_ACTIVE(MO)) THEN
                           CASH_LINKED_LEVEL(MO,
     +                       Cash Subsidiary State Taxes Paid by Parent,
     ...
     +                       + INCOME_VARIABLES(MO,
     +                                    Prior Years Method Adjustment)
                     ELSEIF(VARIABLE == Cash Operating Method Adj .AND.
     +                                .NOT. (CLASS_TYPE == PARENT)) THEN
...
C
         ...
         IF(CLASS_TYPE  == PARENT) THEN
            PARENT_CS_DIV_CARRY_OVER = TEMP_CS_DIV_CARRY_OVER
               ...
               CASH_VARIABLES(:,Cash Parent Taxes Paid) =
     +                CASH_VARIABLES(:,Cash Federal Income Taxes Paid)

               CASH_VARIABLES(:,Cash Subsidiary Tax Payments) =
     ...
     +                        CASH_VARIABLES(:,Cash Common Stock Issued)
...
C
     ...
     +                                    PARENT_DEFERRED_TAXES_DR)
               ...
               CASH_VARIABLES(:,Cash Consolidate Taxes Paid) =
     +                             CONSOLIDATED_VARIABLES(:,
     +                                   Cash Federal Income Taxes Paid)
...
C
            DO VARIABLE = 1, INCOME_VARS 
               IF(VARIABLE == Monthly Subsidiary Income) THEN
                  INCOME_VARIABLES(:,VARIABLE) = UNMODELED_SUB_INCOME(:)
                  CYCLE
               ENDIF
               IF(VARIABLE == Monthly Other Taxes) THEN
...
C ELIMINATE NA3 ODEC INCOME STATEMENT TRANSACTIONS
     ...
     +            + CASH_VARIABLES(:,Cash Exp File Property Taxes)  
            DO VARIABLE = 1, CASH_VARS 
               ...
               IF(VARIABLE == Cash Subsidiary Federal Taxes Paid ) CYCLE
               IF(VARIABLE == Cash Subsidiary State Taxes Paid ) CYCLE
               IF(VARIABLE==Cash Subsidiary State Taxes Paid by Parent)
     +                                                             CYCLE
               IF(VARIABLE == Cash Consolidate Taxes Paid) CYCLE
               IF(VARIABLE == Cash Subsidary Dividends) THEN
                  CASH_VARIABLES(:,Cash Subsidary Dividends) =
     +                                        UNMODELED_SUB_DIVIDENDS(:)
                  CYCLE
               ENDIF
               IF(VARIABLE == Cash Other Taxes) THEN
                  CASH_VARIABLES(:,VARIABLE) = SUB_CASH_OTHER_TAXES(:)
                  CYCLE
               ENDIF
               IF(VARIABLE == Cash Equity to Subsidiaries) CYCLE
               IF(VARIABLE == Common Shares Issued) THEN
                  CASH_VARIABLES(:,VARIABLE) =
     +                        CASH_LINKED_LEVEL(:,VARIABLE,CLASS_LEVEL)
     +                        - CASH_VARIABLES(:,VARIABLE)
                  CYCLE
     ...
     +                         - CASH_VARIABLES(:,VARIABLE)
            ENDDO
            CASH_VARIABLES(:,Cash State Income Taxes Paid) =

     +                          + CONSOLIDATED_CASH_STATE_TAXES(:)
            CASH_VARIABLES(:,Cash Federal Income Taxes Paid) =
     +                           CONSOLIDATED_VARIABLES(:,
     +                                   Cash Federal Income Taxes Paid)
            CONSOLIDATED_VARIABLES(:,Cash State Income Taxes Paid) = 0.
            CONSOLIDATED_VARIABLES(:,Cash Federal Income Taxes Paid)=0.
C
...
C 
     ...
     +            + CASH_VARIABLES(MO,Cash Prior level Method Adj)
     +            + CASH_VARIABLES(MO,Cash Prior Years Method Adj)
     +            + CASH_VARIABLES(MO,Cash Investment Dividend Earnings)
     +            + CASH_VARIABLES(MO,Cash Investment Interest Earnings)
C
               CASH_VARIABLES(MO,Cash TOTAL EXPENSE PAYMENTS) =
     +                              SUM_CASH_PAYMENTS(MO,CASH_VARIABLES)
C
...
C
     ...
     +                 SUM_CONSTRUCTION_CASH_PAYMENTS(MO,CASH_VARIABLES)
               CASH_VARIABLES(MO,TOTAL CAPITAL REQUIREMENTS) =
     +            SUM_CASH_TOTAL_CAPITAL_REQUIREMENTS(MO,CASH_VARIABLES)
               CASH_VARIABLES(MO,Cash Additions to Inventories) =
     +                     CASH_VARIABLES(MO,Cash Fuel Inventory Change)
     +                     + CASH_VARIABLES(MO,
     +                              Cash Gas Storage Inventory Addition)
     +                     + CASH_VARIABLES(MO,
...
C
     ...
     +               + CASH_VARIABLES(MO,EXTERNAL FINANCING PERFORMED)
            ENDDO
            CASH_VARIABLES(0,Cash Opening Balance) =
     +                                    CONSOLIDATED_BOY_FUNDS_BALANCE
            CASH_VARIABLES(1,Cash Opening Balance) =
     +                                    CONSOLIDATED_BOY_FUNDS_BALANCE
            DO MO = 1, 12
              CASH_VARIABLES(MO,Cash Closing Balance) =
     +                         CASH_VARIABLES(MO,Cash Opening Balance)
...
C
     ...
     +                   INCOME_LINKED_LEVEL(MO,
     +                       Monthly State ATL Income Tax Paid,INT2(-1))

                  INCOME_VARIABLES(MO,
     +                              Monthly State BTL Income Tax Paid) =
     +                   INCOME_LINKED_LEVEL(MO,
     +                       Monthly State BTL Income Tax Paid,INT2(-1))

                  INCOME_VARIABLES(MO,
     +                            Monthly Total State Income Tax Paid) =
     +                 INCOME_LINKED_LEVEL(MO,
     +                     Monthly Total State Income Tax Paid,INT2(-1))

                  INCOME_VARIABLES(MO,
     +                            Monthly Federal ATL Income Tax Paid) =
     +                                    RPT_FEDERAL_ATL_INCOME_TAX(MO)
                  INCOME_VARIABLES(MO,
     +                              Monthly Total Fed Income Tax Paid) =
     +                                        RPT_FEDERAL_INCOME_TAX(MO)
     ...
     +                            Monthly Federal ATL Income Tax Paid) =
     +                  INCOME_VARIABLES(0,
     +                          Monthly Federal ATL Income Tax Paid)/12.
...
C
     ...
     +                                          OTHER_LT_LIABILITIES_BOY
            DO VARIABLE = 1, BAL_SHEET_VARS
               ...
               IF(VARIABLE == monthly_gain_on_reaquired_debt_bal) CYCLE
               IF(VARIABLE == Monthly Regulatory Liabilites Bal) CYCLE
     ...
     +                  - INCOME_VARIABLES(MO,Monthly LTD Premium Amort)
                  ...
                  CYCLE
               ENDIF
               IF(VARIABLE == monthly_total_capital_wo_current_ltd) THEN
     ...
     +                  + INCOME_VARIABLES(MO,Monthly RETAINED EARNINGS)
                  ...
                  CYCLE
               ENDIF
               IF(VARIABLE == Monthly Subsidiary Investment) THEN
                  BALANCE_SHEET_VARIABLES(0,
     +                                  Monthly Subsidiary Investment) =
     +                                    BOY_INVESTMENT_IN_SUBSIDIARIES
                  DO MO = 1, 12
     ...
     +                  - CASH_VARIABLES(MO,Cash Subsidary Dividends)
                  ...
                  CYCLE
               ENDIF
               IF(VARIABLE == Monthly Deferred Income Taxes Dr) THEN
                  IF(BANGOR()) THEN
                     DO MO = 1, 12
                        BALANCE_SHEET_VARIABLES(MO,
     +                               Monthly Deferred Income Taxes Dr) =
     ...
     +                     + SUB_FASB87_DEF_TAX_DR_ADJ(MO)
                  ...
                  CYCLE
               ENDIF
               IF(VARIABLE == Monthly Other Investments) THEN
                  BALANCE_SHEET_VARIABLES(0,Monthly Other Investments) =
     +                                     OTHER_INVESTMENTS_BALANCE_BOY
                  DO MO = 1, 12
                     BALANCE_SHEET_VARIABLES(MO,
     +                                      Monthly Other Investments) =
     +                 BALANCE_SHEET_VARIABLES(MO-1,
     +                                        Monthly Other Investments)
     +                 + CASH_VARIABLES(MO,Cash Net Investments)
     +                 + INCOME_VARIABLES(MO,MonthlyMarkToMarket)
     +                 + INCOME_VARIABLES(MO,MonthlyEarningNonCompany)
                  ...
                  CYCLE
               ENDIF
               IF(VARIABLE == Monthly Deferred Income Taxes) THEN
                  IF(BANGOR()) THEN
                     DO MO = 1, 12
                        BALANCE_SHEET_VARIABLES(MO,
     +                                  Monthly Deferred Income Taxes) =
...
C NON_INCOME_TAXES_ACCRUAL_ADJ
     +               + INCOME_VARIABLES(MO,Monthly Other Taxes)
     +               + INCOME_VARIABLES(MO,Monthly Property Taxes) ! REMOVE 8.3.04
     +               + INCOME_VARIABLES(MO,
     +                                    Monthly Operating Revenue Tax)
     +               + INCOME_VARIABLES(MO,Monthly State Tax on Capital)
     +               + INCOME_VARIABLES(MO,
...
C 
            BALANCE_SHEET_VARIABLES(0,Monthly CWIP) = BOY_CWIP
            DO MO = 1, 12 
               BALANCE_SHEET_VARIABLES(MO,Monthly CWIP) =
     +                BALANCE_SHEET_VARIABLES(MO-1,Monthly CWIP)
     +                + INCOME_VARIABLES(MO,Monthly AFUDC Total)
     ...
     +                                 Monthly Plant Capitialized)
            ...
            BALANCE_SHEET_VARIABLES(0,Monthly Short Term Investments) =
     +                            CASH_VARIABLES(0,Cash Opening Balance)
            BALANCE_SHEET_VARIABLES(1:,Monthly Short Term Investments) =
     +                           CASH_VARIABLES(1:,Cash Closing Balance)
C  Monthly Current Assets
            ASSETS_NEC =
     +           BALANCE_SHEET_VARIABLES(:,Monthly Current Assets)
            BALANCE_SHEET_VARIABLES(0,Monthly Current Assets) =
     +                                              ASSETS_NEC_BOY
            DO MO = 1, 12 
               BALANCE_SHEET_VARIABLES(MO,
     ...
     +                   +  (ASSETS_NEC(MO) - ASSETS_NEC(MO-1))
            ENDDO
C Monthly Net Deferred Debits
            CALL MONTHLY_DEFERRED_DEBIT_BS_ELIM_INFO(R_YR,
     +                                         ELIMINAITON_CLASS_ID_NUM,
     +                                          BALANCE_SHEET_VARIABLES)
            BALANCE_SHEET_VARIABLES(0,Monthly Net Deferred Debits) =
     +                                               DEFERRED_DEBITS_BOY
               BALANCE_SHEET_VARIABLES(:,Monthly Net Other Debits) =
     +            BALANCE_SHEET_VARIABLES(:,Monthly Net Deferred Debits)
     +            - BALANCE_SHEET_VARIABLES(:,Monthly Net Goodwill)
     +            - BALANCE_SHEET_VARIABLES(:,
     +                                    Monthly Net Regulatory Assets)
     +            - BALANCE_SHEET_VARIABLES(:,Monthly Net FASB 109)
     +            - BALANCE_SHEET_VARIABLES(:,Monthly Net FASB 133)
     +            - BALANCE_SHEET_VARIABLES(:,
     +                               Monthly Unamortized Interest Bal) ! 76
     +            - BALANCE_SHEET_VARIABLES(:,
     +                                monthly_unamortized_issue_exp_bal) ! 77
...
C
            INCOME_VARIABLES(0,Monthly Shares Outstanding) =
     +           SUM(INCOME_VARIABLES(1:,Monthly Shares Outstanding))/12.

            CASH_VARIABLES(:,Cash Total Investment Interest Earnings) =
     +             CASH_VARIABLES(:,Cash Investment Interest Earnings)
     +             + CASH_VARIABLES(:,Cash STInvestment Interest Income)
            CASH_VARIABLES(:,Cash Total ARO Payments) =
     +                       CASH_VARIABLES(:,Cash ARO Payments)
     +                       + CASH_VARIABLES(:,Cash ARO Trust Payments)
            PREVIOUS_BAL_SHEET_INBALANCE = 0.
            QUARTERLY_EARNINGS_PER_SHARE = 0.
            CASH_VARIABLES(:,Cash Post Retirement Payments) =
     +              CASH_VARIABLES(:,Cash Post Retirement Payments)
     +              + CASH_VARIABLES(:,Cash to Post Retirement Payments)
...
C
            DO MO = 0, 12
               IF(CASH_VARIABLES(MO,Common Shares Issued) /= 0) THEN
...
C
     ...
     +                                Monthly Total Fed Income Tax Paid)
     +                  - CASH_VARIABLES(MO,
     +                                   Cash Federal Income Taxes Paid)   ! 108
               ENDIF
C
     ...
     +                         Quarterly Earnings per Share) = NOT_AVAIL
               ...
               ENDIF
C
               INCOME_VARIABLES(MO,BTL Monthly Interest Income) =
...
C
               IF(CALCULATE_BTL_INCOME_TAXES) THEN
                  RPT_BTL_TAXABLE_INTEREST_INCOME =
     +                  INCOME_VARIABLES(MO,BTL Monthly Interest Income)   ! 33
                  RPT_BTL_TAXABLE_INVEST_EARNINGS =
     +                  INCOME_VARIABLES(MO,Monthly Investment Earnings)   
     +                  - .7*INCOME_VARIABLES(MO,
     +                                     Monthly Dividend 70 Earnings) ! 34
     +                  - INCOME_VARIABLES(MO,
     +                         Monthly Retirement Medical Fund Earnings) ! 9/28/04 changes
                  RPT_BTL_TAXABLE_LTI_INCOME =
     +                     INCOME_VARIABLES(MO,
     +                                Monthly Total LTInvestment Income) ! 35
                  RPT_BTL_TAXABLE_STI_INCOME =
     +                     INCOME_VARIABLES(MO,
...
C
               TOTAL_INCOME_TAXES = 
     +                  INCOME_VARIABLES(MO,
     +                              Monthly Total State Income Tax Paid) ! 134
     +                  + INCOME_VARIABLES(MO,
     +                                Monthly Total Fed Income Tax Paid) ! 136
     +                  + INCOME_VARIABLES(MO,
     ...
     +                       + INCOME_VARIABLES(MO,Monthly AFUDC Equity)         ! 108
     +                       + INCOME_VARIABLES(MO,
...
C STD INTEREST
     +                       INCOME_VARIABLES(MO,Monthly STD Interest) 
     +                       - CASH_VARIABLES(MO,Cash STD Interest),    !101
C LTD INTEREST
     +                  INCOME_VARIABLES(MO,Monthly LTD Booked Interest)
     +                  - CASH_VARIABLES(MO,Cash LTD Interest),         !102
C COMMON DIVIDENDS
     +                  INCOME_VARIABLES(MO,Monthly Common Dividends)
     +                  - CASH_VARIABLES(MO,Cash Common Dividends),     !103
C PS DIVIDENDS
     +                  INCOME_VARIABLES(MO,Monthly Booked PS Dividends)
     +                  - CASH_VARIABLES(MO,Cash PS Dividends),         !104
C NOTES PAYABLE INTEREST
     +            INCOME_VARIABLES(MO,Monthly Interest on Notes Payable)
     +            - CASH_VARIABLES(MO,Cash Interest on Notes Payable),  !105
C NON_INCOME_TAXES_ACCRUAL_ADJ
     +             INCOME_VARIABLES(MO,Monthly Other Taxes)
     +             + INCOME_VARIABLES(MO,Monthly Property Taxes)
     +             + INCOME_VARIABLES(MO,Monthly Operating Revenue Tax)
     ...
     +             - CASH_VARIABLES(MO,Cash Property Taxes)  
     +             - CASH_VARIABLES(MO,Cash State Tax on Capital)  
     +             - CASH_VARIABLES(MO,Cash Federal Tax on Capital),    ! 106
C R_STATE_TAXES_ACCRUAL_ADJ
     +          INCOME_VARIABLES(MO,Monthly Total State Income Tax Paid)
     +          - CASH_VARIABLES(MO,Cash State Income Taxes Paid),      ! 107
C R_FEDERAL_TAXES_ACCRUAL_ADJ
...
C BALANCE SHEET BALANCED?
     ...
     +          INCOME_VARIABLES(MO,monthly_common_stock_issue_amort),    ! 111
     +          INCOME_VARIABLES(MO,
     +                           Monthly Investment Earnings Receivable)
     +          + INCOME_VARIABLES(MO,Monthly Total LTInvestment Income)
     +          + INCOME_VARIABLES(MO,BTL Monthly STInvestmet Income)
     +          - CASH_VARIABLES(MO,Cash Investment Dividend Earnings)
     +          - CASH_VARIABLES(MO,Cash Investment Interest Earnings)
     +          - CASH_VARIABLES(MO,Cash STInvestment Interest Income),  ! 112
     +          INCOME_VARIABLES(MO,Unbilled Revenues),    !113
     +          INCOME_VARIABLES(MO,Monthly ATL Gain Amort)
     +          + INCOME_VARIABLES(MO,Monthly BTL Gain Amort), ! 114
...
C
     ...
     +               INCOME_VARIABLES(MO,Operating Method Adjustment),
     +               INCOME_VARIABLES(MO,Base Rates),  ! 5
     ...
     +               INCOME_VARIABLES(MO,Total Base Revenues),
     +               INCOME_VARIABLES(MO,Adjustment Clause),
     +               INCOME_VARIABLES(MO,Secondary Sales),
     +               INCOME_VARIABLES(MO,Capacity Sales),
     +               INCOME_VARIABLES(MO,Bulk Power), ! 15
     +               INCOME_VARIABLES(MO,Other Revenue),
     +               INCOME_VARIABLES(MO,Gas Revenues),
     +               INCOME_VARIABLES(MO,Relationship Revenues),
     +               INCOME_VARIABLES(MO,Monthly Catawba Revenues),
     ...
     +               INCOME_VARIABLES(MO,Total Derivative Revenues),     ! 23
     +               INCOME_VARIABLES(MO,
     +                                  Total Fuel Derivative Revenues), ! 24
     +               INCOME_VARIABLES(MO,Monthly Net Income),            ! 25
     +               INCOME_VARIABLES(MO,Monthly AFUDC Equity),          ! 26
     +               INCOME_VARIABLES(MO,
...
C Total Investment Income
     ...
     +               INCOME_VARIABLES(MO,PGA Adjustment),               ! 38
     +               (ZERO_POS, I=39, 40),                              ! 39-40
     +               CASH_VARIABLES(MO,Cash Net Salvage),               ! 41
...
C EXPENSE ITEMS ARE OFFEST BY  EXP_OFFSET_LINE WHICH IS 30
     +               INCOME_VARIABLES(MO,Monthly Fossil Fuel),
     +               INCOME_VARIABLES(MO,
     +                                Monthly Tax Expense Nuclear Fuel),  ! 52
     +               INCOME_VARIABLES(MO,Monthly DOE Decommissioning),
     +               INCOME_VARIABLES(MO,Monthly DOE Disposal),
     +               INCOME_VARIABLES(MO,monthly_purchased_power),
     +               INCOME_VARIABLES(MO,Monthly Variable OandM),
     +               INCOME_VARIABLES(MO,Monthly Fixed OandM),
     +               INCOME_VARIABLES(MO,Monthly Service Transactions),
     +               INCOME_VARIABLES(MO,Monthly Other OandM),
     +               INCOME_VARIABLES(MO,Monthly Purchased Gas),         ! 60
     ...
     +               INCOME_VARIABLES(MO,Monthly Customer Accounts),
     +               INCOME_VARIABLES(MO,Monthly Customer Services),
     +               INCOME_VARIABLES(MO,Monthly Sales Expense),
     +               INCOME_VARIABLES(MO,Monthly AG Operations),
     +               INCOME_VARIABLES(MO,Monthly AG Maintenance),
     +              INCOME_VARIABLES(MO,Monthly Lease Interest Expense), ! 71
     +               INCOME_VARIABLES(MO,Monthly DSM Expense),
     +               INCOME_VARIABLES(MO,Monthly DSM Rebate),
     +               INCOME_VARIABLES(MO,Monthly Emission Credits),
     +               INCOME_VARIABLES(MO,Monthly Catawba Expenses),      ! 75
     +               TAX_BOOK_RETIREMENT_PAYMENTS,                       ! 76
     +               TAX_BOOK_STORM_PAYMENTS,                            ! 77
     +               TAX_BOOK_VACATION_PAYMENTS,                         ! 78
     +               INCOME_VARIABLES(MO,Total Derivative Expenses),      ! 79
     +               INCOME_VARIABLES(MO,Monthly Retirement Expense),    ! 80
C BTL ITEMS ARE NOT OFFSET
     +               INCOME_VARIABLES(MO,Monthly STD Interest),          ! 81
     +               INCOME_VARIABLES(MO,
     +                               Monthly Interest on Notes Payable), ! 82
     +               TAX_VARIABLES(MO,monthly_ltd_tax_deduction),        ! 83
     +               INCOME_VARIABLES(MO,Monthly LTD Total Interest)     
     +               - INCOME_VARIABLES(MO,Monthly LTD Amort Interest),  ! 84
     +               INCOME_VARIABLES(MO,Monthly Other Taxes),           ! 85
     +               INCOME_VARIABLES(MO,Monthly Property Taxes),        ! 86
     +               INCOME_VARIABLES(MO,Monthly Operating Revenue Tax), ! 87
     +               INCOME_VARIABLES(MO,Monthly State Tax on Capital),  ! 88
     +               INCOME_VARIABLES(MO,Monthly LTD Amort Interest),    ! 89
     +              INCOME_VARIABLES(MO,Total Fuel Derivative Expenses), ! 90
C TAKE DEDUCTIONS
...
C
     ...
     +               CASH_VARIABLES(MO,Cash LTD PS Issue Expense),       ! 100
     +               CASH_VARIABLES(MO,Cash Common Issue Expense),       ! 101
     ...
     +               INCOME_VARIABLES(MO,
     +                               Monthly State ATL Income Tax Paid), ! 127
     +               RPT_FEDERAL_BTL_TAXABLE_INCOME(MO),                 ! 128
     +               RPT_FEDERAL_BTL_INCOME_TAX(MO),                     ! 129
     +               INCOME_VARIABLES(MO,
     +                             Monthly Federal ATL Income Tax Paid), ! 130
     +               INCOME_VARIABLES(MO,
     +                              Monthly Total State Income Tax Paid)
     +               + INCOME_VARIABLES(MO,
     +                               Monthly Total Fed Income Tax Paid), ! 131
     +               RPT_STATE_BTL_INCOME_TAX(MO)
...
C
     ...
     +                INCOME_VARIABLES(MO,Monthly Expense 
     +                                Reserve Margin Capacity Purchase), ! 215
     +                INCOME_VARIABLES(MO,
     +                    Monthly Income Reserve Margin Capacity Sales), ! 216
...
C TAXABLE DEDUCTIONS
C EXPENSE ITEMS ARE OFFEST BY  EXP_OFFSET_LINE WHICH IS 30
     +               INCOME_VARIABLES(MO,Fossil Fuel+ EXP_OFFSET_LINE),  ! 51
     +               INCOME_VARIABLES(MO,
     +                                Monthly Tax Expense Nuclear Fuel), ! 52
     +               INCOME_VARIABLES(MO,DOE Decommissioning
     +                                               + EXP_OFFSET_LINE), ! 53
     +               INCOME_VARIABLES(MO,DOE Disposal+ EXP_OFFSET_LINE), ! 54
     +               INCOME_VARIABLES(MO,Purchased Power
     +                                               + EXP_OFFSET_LINE), ! 55
     +               INCOME_VARIABLES(MO,Variable OandM
     +                                               + EXP_OFFSET_LINE), ! 56
     +               INCOME_VARIABLES(MO,Fixed OandM+ EXP_OFFSET_LINE),  ! 57
     +               INCOME_VARIABLES(MO,Service Transactions
     +                                              + EXP_OFFSET_LINE),  ! 58
     +               INCOME_VARIABLES(MO,Other OandM+ EXP_OFFSET_LINE),  ! 59
     +               INCOME_VARIABLES(MO,Purchased Gas
     +                                               + EXP_OFFSET_LINE), ! 60
     +               INCOME_VARIABLES(MO,Other+ EXP_OFFSET_LINE),        ! 61
     +               INCOME_VARIABLES(MO,Transmission Operation
     +                                               + EXP_OFFSET_LINE),
     +               INCOME_VARIABLES(MO,Transmission Maintenance
     +                                               + EXP_OFFSET_LINE),
     +               INCOME_VARIABLES(MO,Distribution Operation
     +                                               + EXP_OFFSET_LINE),
     +               INCOME_VARIABLES(MO,Distribution Maintenance
     +                                               + EXP_OFFSET_LINE), ! 65
     +               INCOME_VARIABLES(MO,Customer Accounts
     +                                               + EXP_OFFSET_LINE), ! 66
     +               INCOME_VARIABLES(MO,Customer Services
     +                                               + EXP_OFFSET_LINE),
     +               INCOME_VARIABLES(MO,Sales Expense
     +                                               + EXP_OFFSET_LINE),
     +               INCOME_VARIABLES(MO,AG Operations
     +                                               + EXP_OFFSET_LINE), 
     +               INCOME_VARIABLES(MO,AG Maintenance
     +                                               + EXP_OFFSET_LINE), ! 70
     +              INCOME_VARIABLES(MO,Monthly Lease Interest Expense), ! 71
     +               INCOME_VARIABLES(MO,DSM Expense+ EXP_OFFSET_LINE),
     +               INCOME_VARIABLES(MO,DSM Rebate+ EXP_OFFSET_LINE),
     +               INCOME_VARIABLES(MO,Monthly Emission Credits),
     +               INCOME_VARIABLES(MO,Catawba Expenses
     +                                               + EXP_OFFSET_LINE), ! 75
     +               TAX_BOOK_RETIREMENT_PAYMENTS,                       ! 76
     +               TAX_BOOK_STORM_PAYMENTS,                            ! 77
     +               TAX_BOOK_VACATION_PAYMENTS,                         ! 78
...
C
     ...
     +                - INCOME_VARIABLES(MO,
     +                         Monthly Retirement Medical Fund Earnings) ! 9/28/04
     +                + INCOME_VARIABLES(MO,
     +                                Monthly Total LTInvestment Income)
     +                + INCOME_VARIABLES(MO,
     +                                   BTL Monthly STInvestmet Income)
...
C
     ...
     +                       + BALANCE_LINKED_LEVEL(:,Monthly CICA,
     +                                                      CLASS_LEVEL) 
...
C
         IF(R_CLASS_LEVEL >= 0) THEN
            ...
            DO VARIABLE = 1, TAX_VARS 
               IF(VARIABLE == monthly_atl_federal_income_taxes) CYCLE
               IF(VARIABLE == monthly_btl_federal_income_taxes) CYCLE
...
C***********************************************************************
C
         CALL RETURN_MONTHLY_FUNDS_EARNING_RATES(R_YR,CLASS_ID,
     +                                 INTEREST_ON_CUSTOMER_DEPOSITS,
     +                                 RETURN_RETIREMENT_MEDICAL_FUND,
     +                                 NUCL_DECOM_FUND_RETURN,
     +                                 CUSTOMER_DEPOSIT_INTEREST_RATE,
     +                                 MONTHLY_RETIREMENT_FUND_RATE,
     +                                 OCI_MONTHLY_RETIREMENT_FUND_RATE,
     +                                 MONTHLY_NUC_DECOM_RATE,
     +                                 OCI_MONTHLY_NUC_DECOM_RATE)
C
C DECOMMISION FUND EARNINGS
C
            CALL RETURN_MONTHLY_NUC_DECOM_CASH_ADDENDUMS(CLASS_ID,R_YR,
...
C
            CALL RETURN_MONTHLY_RETIREMENT_CASH_ADDENDUMS(CLASS_ID,R_YR,
...
C
     ...
     +            + MONTHLY_RETIREMENT_FUND_EARNINGS_ADJ(MO)
               OCI_RETIREMENT_MEDICAL_FUND_RETURN(MO) =
     +            OCI_MONTHLY_RETIREMENT_FUND_RATE(MO) *
     +              (POST_RETIREMENT_MEDICAL_FUND_BAL
     +               + (MONTHLY_POST_RETIREMENT_CASH_PAYMENTS(MO)
     +                  + MONTHLY_CASH_TO_POST_RETIREMENT(MO)
     +                  - MONTHLY_RETIREE_MEDICAL_CASH_PAYMENTS(MO))/2.)
...
C
            IF(MO == 0) THEN
               CPL_TAX__VARIABLES(MO,Tax Tax Depreciation,ATL) =
     +                                                 -ANNUAL_VARS(130) ! Tax Depreciation
               CPL_TAX__VARIABLES(MO,Tax Construction Expenses,ATL) =
     +                                                 -ANNUAL_VARS(228) ! Construction Expenses
               CPL_TAX__VARIABLES(MO,Tax Foreign Tax Credit,ATL) = 0.
               CPL_TAX__VARIABLES(MO,Tax Possessions Tax Credit,ATL)=0.
               CPL_TAX__VARIABLES(MO,Tax Section 29 Credits Used,BTL) =
     +                                                  ANNUAL_VARS(373) ! Sec. 29 Federal Tax Credits Used ($M)  
               CPL_TAX__VARIABLES(MO,Tax Investment Tax Credit,ATL) = 0.
               CPL_TAX__VARIABLES(MO,Tax Section 43 Credits Used,ATL) =
     +                                                  ANNUAL_VARS(341) ! Federal Tax Credits Used ($M)
               CPL_TAX__VARIABLES(MO,Tax AMT Credits Used,ATL) = 0.
               CPL_TAX__VARIABLES(MO,Tax Adjustments to Taxes,ATL) =
     +                                                  ANNUAL_VARS(138) ! Federal Tax Adjustments
               CPL_TAX__VARIABLES(MO,Tax M1 Current Deductions,ATL) =
     +                                                  ANNUAL_VARS(137) ! Federal Tax Deductions 
               CPL_TAX__VARIABLES(MO,Tax Federal AMT Tax,TOTAL) =
     +                                                  ANNUAL_VARS(355) ! Federal Tax Deductions 
               CPL_TAX__VARIABLES(MO,Tax Capitialized Interest,ATL) =
     +                                                  ANNUAL_VARS(129) ! CAPITIALIZED INTEREST
               CPL_TAX__VARIABLES(MO,Tax Net Salvage,ATL) =
     ...
     +                                             -ANNUAL_VARS(356)/12. ! NET SALVAGE
            ENDIF
            CPL_TAX__VARIABLES(MO,Tax Tax Depreciation,BTL) = 0.
            CPL_TAX__VARIABLES(MO,Tax Construction Expenses,BTL) = 0.
            CPL_TAX__VARIABLES(MO,Tax Capitialized Interest,BTL) = 0.
            CPL_TAX__VARIABLES(MO,Tax Net Salvage,BTL) = 0.
            CPL_TAX__VARIABLES(MO,Tax Book Nuclear Fuel,BTL) = 0.
C
C
C
            CPL_TAX__VARIABLES(MO,Income Before Income Taxes,ATL) =
     +          INCOME_VARIABLES(MO,Monthly Total Revenues) -
     +          INCOME_VARIABLES(MO,Monthly Expenses B4 Taxes) -
     +          INCOME_VARIABLES(MO,Monthly State Tax on Capital) -
     +          INCOME_VARIABLES(MO,Monthly Property Taxes) -
     +          INCOME_VARIABLES(MO,Monthly Federal Tax on Capital) -
     +          INCOME_VARIABLES(MO,Monthly Other Taxes) -
     +          INCOME_VARIABLES(MO,Monthly Operating Revenue Tax)
            CPL_TAX__VARIABLES(MO,Tax AFUDC Equity,ATL) = 
     +                         INCOME_VARIABLES(MO,Monthly AFUDC Equity)
            CPL_TAX__VARIABLES(MO,Tax AFUDC Borrowed,ATL) =
     +                       INCOME_VARIABLES(MO,Monthly AFUDC Borrowed)
            CPL_TAX__VARIABLES(MO,Tax Total AFUDC,ATL) = 
     +                  -(CPL_TAX__VARIABLES(MO,Tax AFUDC Equity,ATL) +
     +                    CPL_TAX__VARIABLES(MO,Tax AFUDC Borrowed,ATL))

            CPL_TAX__VARIABLES(MO,Tax Interest Expenses,ATL) =
     +       -(INCOME_VARIABLES(MO,Monthly LTD Total Interest) ! NEED THIS VALUE WITHOUT AMORT
     +         + INCOME_VARIABLES(MO,Monthly STD Interest)
     +         + INCOME_VARIABLES(MO,Monthly Interest on Notes Payable))

            CPL_TAX__VARIABLES(MO,Tax Interest Adjustment,ATL) = 0.
            CPL_TAX__VARIABLES(MO,
     +                         Tax Interest to Associated Comp,ATL) = 0.
            CPL_TAX__VARIABLES(MO,Tax Book Depreciation,ATL) =
     +                    INCOME_VARIABLES(MO,Monthly Book Depreciation)
            CPL_TAX__VARIABLES(MO,Tax Amortization,ATL) =
     +                    INCOME_VARIABLES(MO,Monthly Amortization)
            CPL_TAX__VARIABLES(MO,Tax Interest Amortization,ATL) =
     +                   INCOME_VARIABLES(MO,Monthly LTD Amort Interest)
            CPL_TAX__VARIABLES(MO,Tax Book Nuclear Fuel,ATL) =
     +                 INCOME_VARIABLES(MO,Monthly Nuclear Fuel Expense)
            CPL_TAX__VARIABLES(MO,Tax State Income Tax,ATL) =
     +                  TAX_VARIABLES(MO,monthly_atl_state_income_taxes)
C
C TAX TIMING
C
            CPL_TAX__VARIABLES(MO,Temporary Tax Differences,ATL) =
     +                   TAX_VARIABLES(MO,monthly_temporary_atl_tax_dif)
            CPL_TAX__VARIABLES(MO,Permanent Tax Differences,ATL) =
...
C BTL ITEMS
C
            CPL_TAX__VARIABLES(MO,Income Before Income Taxes,BTL) =
     +          INCOME_VARIABLES(MO,BTL Monthly Other Income)
     +          + INCOME_VARIABLES(MO,Monthly Total LTInvestment Income)
     +          + INCOME_VARIABLES(MO,BTL Monthly STInvestmet Income)
     +          - INCOME_VARIABLES(MO,BTL Monthly Expenses)
     +          - INCOME_VARIABLES(MO,BTL Monthly Amortization)
            CPL_TAX__VARIABLES(MO,Tax AFUDC Equity,BTL) = 0.
            CPL_TAX__VARIABLES(MO,Tax AFUDC Borrowed,BTL) = 0.
            CPL_TAX__VARIABLES(MO,Tax Total AFUDC,BTL) = 0.
            CPL_TAX__VARIABLES(MO,Tax Interest Expenses,BTL) = 0.
            CPL_TAX__VARIABLES(MO,Tax Interest Adjustment,BTL) = 0.
            CPL_TAX__VARIABLES(MO,
     +                         Tax Interest to Associated Comp,BTL) = 0.
            CPL_TAX__VARIABLES(MO,Tax Book Depreciation,BTL) = 0.
            CPL_TAX__VARIABLES(MO,Tax Amortization,BTL) =
     +                    INCOME_VARIABLES(MO,BTL Monthly Amortization)
            CPL_TAX__VARIABLES(MO,Tax Interest Amortization,BTL) = 0.
C
            CPL_TAX__VARIABLES(MO,Temporary Tax Differences,BTL) =
     +                   TAX_VARIABLES(MO,monthly_temporary_btl_tax_dif)
            CPL_TAX__VARIABLES(MO,Permanent Tax Differences,BTL) =
     +                   TAX_VARIABLES(MO,monthly_permanent_btl_tax_dif)
            CPL_TAX__VARIABLES(MO,Tax State Income Tax,BTL) =
     +                  TAX_VARIABLES(MO,monthly_btl_state_income_taxes)
C 
C
            DO VAR_TYPE = Income Before Income Taxes, 
     +                                  Tax Interest to Associated Comp
               CPL_TAX__VARIABLES(MO,Tax Total Income B4 Taxes,ATL) = 
     +            CPL_TAX__VARIABLES(MO,Tax Total Income B4 Taxes,ATL) +
     +                               CPL_TAX__VARIABLES(MO,VAR_TYPE,ATL)
               CPL_TAX__VARIABLES(MO,Tax Total Income B4 Taxes,BTL) = 
     +            CPL_TAX__VARIABLES(MO,Tax Total Income B4 Taxes,BTL) +
...
C
C
            DO VAR_TYPE = Tax Book Depreciation,
     +                                        Tax Taken For Book Total-1
               CPL_TAX__VARIABLES(MO,Tax Taken For Book Total,ATL) = 
     +            CPL_TAX__VARIABLES(MO,Tax Taken For Book Total,ATL) +
     +                               CPL_TAX__VARIABLES(MO,VAR_TYPE,ATL)
               CPL_TAX__VARIABLES(MO,Tax Taken For Book Total,BTL) = 
     +            CPL_TAX__VARIABLES(MO,Tax Taken For Book Total,BTL) +
     +                               CPL_TAX__VARIABLES(MO,VAR_TYPE,BTL)
            ENDDO
C
C
C
            DO VAR_TYPE = Tax Tax Depreciation,Tax Taken For Tax Total-1
               CPL_TAX__VARIABLES(MO,Tax Taken For Tax Total,ATL) = 
     +            CPL_TAX__VARIABLES(MO,Tax Taken For Tax Total,ATL) +
     +                               CPL_TAX__VARIABLES(MO,VAR_TYPE,ATL)
               CPL_TAX__VARIABLES(MO,Tax Taken For Tax Total,BTL) = 
     +            CPL_TAX__VARIABLES(MO,Tax Taken For Tax Total,BTL) +
     +                               CPL_TAX__VARIABLES(MO,VAR_TYPE,BTL)
            ENDDO
C
...
C
            DO VAR = ATL, BTL
               ...
               CPL_TAX__VARIABLES(MO,Tax Federal Tax Rate,VAR) = 
     +                                                  ANNUAL_VARS(213) ! Federal Tax Rate
               CPL_TAX__VARIABLES(MO,Tax Capital Gains Tax Rate,VAR)=20.
            
C
C REPORT SUB-TOTALS
C
               CPL_TAX__VARIABLES(MO,Taxable Income B4 State Taxes,VAR)= 
     +            CPL_TAX__VARIABLES(MO,Tax Total Income B4 Taxes,VAR) +
     +            CPL_TAX__VARIABLES(MO,Tax Taken For Book Total,VAR) + 
     +            CPL_TAX__VARIABLES(MO,Tax Taken For Tax Total,VAR) +
     +            CPL_TAX__VARIABLES(MO,Permanent Tax Differences,VAR) +
     +            CPL_TAX__VARIABLES(MO,Temporary Tax Differences,VAR)
C
              CPL_TAX__VARIABLES(MO,Current Federal Taxable Income,VAR)=
     +         CPL_TAX__VARIABLES(MO,Taxable Income B4 State Taxes,VAR)- 
     +         CPL_TAX__VARIABLES(MO,Tax State Income Tax,VAR) -
     +         CPL_TAX__VARIABLES(MO,Tax Capital Gains Income,VAR) -    
     +         CPL_TAX__VARIABLES(MO,Tax NOLs Used,VAR) -
     +         CPL_TAX__VARIABLES(MO,Tax M1 Current Deductions,VAR)
C
               CPL_TAX__VARIABLES(MO,Tax Federal Tax B4 Credits,VAR) =
     +        CPL_TAX__VARIABLES(MO,Current Federal Taxable Income,VAR)*
     +          CPL_TAX__VARIABLES(MO,Tax Federal Tax Rate,VAR)/100.+
     +        CPL_TAX__VARIABLES(MO,Tax Capital Gains Income,VAR) *     
     +        CPL_TAX__VARIABLES(MO,Tax Capital Gains Tax Rate,VAR)/100.
C
               CPL_TAX__VARIABLES(MO,Tax Total Credits Used,VAR) =
     +          CPL_TAX__VARIABLES(MO,Tax Foreign Tax Credit,VAR) +
     +          CPL_TAX__VARIABLES(MO,Tax Possessions Tax Credit,VAR) +
     +          CPL_TAX__VARIABLES(MO,Tax Section 29 Credits Used,VAR) +
     +          CPL_TAX__VARIABLES(MO,Tax Investment Tax Credit,VAR) +
     +          CPL_TAX__VARIABLES(MO,Tax Section 43 Credits Used,VAR) +
     +          CPL_TAX__VARIABLES(MO,Tax AMT Credits Used,VAR)
C
               CPL_TAX__VARIABLES(MO,Tax Federal Tax After Credits,VAR)=
     +            CPL_TAX__VARIABLES(MO,Tax Federal Tax B4 Credits,VAR)-
     +            CPL_TAX__VARIABLES(MO,Tax Total Credits Used,VAR) -
     +            CPL_TAX__VARIABLES(MO,Tax Adjustments to Taxes,VAR)
               
               CPL_TAX__VARIABLES(MO,Tax Current Fed Income Tax,VAR) =
     +          CPL_TAX__VARIABLES(MO,Tax Federal Tax After Credits,VAR)
            ENDDO
c    +       CPL_TAX__VARIABLES(MO,,ATL) -
c    +       CPL_TAX__VARIABLES(MO,,ATL)  
...
C
            DO VAR_TYPE = 1, CPL_TAX_VARS
               IF(VAR_TYPE == Tax Federal AMT Tax) CYCLE
               CPL_TAX__VARIABLES(MO,VAR_TYPE,TOTAL) = 
     +                             CPL_TAX__VARIABLES(MO,VAR_TYPE,ATL) +
     +                             CPL_TAX__VARIABLES(MO,VAR_TYPE,BTL)
               IF(VAR_TYPE == Tax Federal Tax Rate) 
     +               CPL_TAX__VARIABLES(MO,Tax Federal Tax Rate,TOTAL) = 
     +                                                  ANNUAL_VARS(213) ! Federal Tax Rate
               IF(VAR_TYPE == Tax Capital Gains Tax Rate)     
     +               CPL_TAX__VARIABLES(MO,
     +                               Tax Capital Gains Tax Rate,VAR)=20.
            ENDDO
         ENDDO
         DO MO = 0, 12
            IF(MO == 0) THEN
...
C***********************************************************************
     ...
     +                          + INCOME_VARIABLES(MO,Adjustment Clause) 
     +                          + INCOME_VARIABLES(MO,Deferred Revenues)
     +                          + INCOME_VARIABLES(MO,PGA Adjustment)
...
C
     ...
     +                        CLASS_REVENUE_TAX_REVENUES(MO)
            ...
            ELSE
               ...
               MONTHLY_REVENUE_TAX =
     +             INCOME_VARIABLES(0,Monthly Operating Revenue Tax)/12.
...
C
     ...
     +              INCOME_VARIABLES(MO,Cash Fixed OandM)
     +              + INCOME_VARIABLES(MO,Cash Other OandM)
     +              + INCOME_VARIABLES(MO,Cash Purchased Gas)
     +              + INCOME_VARIABLES(MO,Cash Other)
     +              + INCOME_VARIABLES(MO,Cash Transmission Operation)
     +              + INCOME_VARIABLES(MO,Cash Transmission Maintenance)
     +              + INCOME_VARIABLES(MO,Cash Distribution Operation)
     +              + INCOME_VARIABLES(MO,Cash Distribution Maintenance)
     +              + INCOME_VARIABLES(MO,Cash Customer Accounts)
     +              + INCOME_VARIABLES(MO,Cash Customer Services)
     +              + INCOME_VARIABLES(MO,Cash Sales Expense)
     +              + INCOME_VARIABLES(MO,Cash AG Operations)
     +              + INCOME_VARIABLES(MO,Cash AG Maintenance)
     +              + INCOME_VARIABLES(MO,Cash DSM Expense)
     +              + INCOME_VARIABLES(MO,Cash DSM Rebate)
C                   
...
C***********************************************************************
     ...
     +                   + INCOME_VARIABLES(MONTH,Adjustment Clause) 
     +                   + INCOME_VARIABLES(MONTH,Deferred Revenues)
     +                   + INCOME_VARIABLES(MONTH,PGA Adjustment)
...
C 
     ...
     +                                                 INCOME_VARIABLES)
     +          + INCOME_VARIABLES(MO,Monthly Tax Expense Nuclear Fuel)
     +          + TAX_BOOK_RETIREMENT_PAYMENTS
     +          + TAX_BOOK_STORM_PAYMENTS
     +          + TAX_BOOK_VACATION_PAYMENTS
C
...
C
     ...
     +                    ANNUAL_VARS(129)/12. ! CAPITIALIZED_INTEREST/12.
     +                    - CASH_VARIABLES(MO,Cash Net Salvage)
     +                    - TAX_CASH_RETIREMENT_PAYMENTS
     +                    - TAX_CASH_STORM_PAYMENTS
     +                    - TAX_CASH_VACATION_PAYMENTS
...
C TAKE DEDUCTIONS
     ...
     +                                       + ANNUAL_VARS(228)/12. ! CONSTRUCTION_TAX_EXPENSE/12.
...
C              R_FED_INCOME_TAXES_B4_CREDITS(MO)=R_FEDERAL_TAX_RATE(MO)*
C    +                                      R_FEDERAL_TAXABLE_INCOME(MO)     
C              R_FEDERAL_INCOME_TAXES(MO) = 
C    +                                 R_FED_INCOME_TAXES_B4_CREDITS(MO)
...
C$ifdefined(use_this_tax_code) ! 7/11/00 removed for testing KCP&L sub use of 29 credits
...
C
     ...
     +       MONTHLY_ATL_INCOME_STATEMENT_TAX_DEDUCTIONS
      ...
      LOGICAL*1 WVPA,IMPA
C      
         INCOME_VARIABLES(MO,BTL Monthly Interest Income) =       ! 101
     +              INCOME_VARIABLES(MO,BTL Monthly STInvestmet Income)
     +              + INCOME_VARIABLES(MO,
     +                                Monthly Model LTInvestment Income)
     +              + INCOME_VARIABLES(MO,
     ...
     +          INCOME_VARIABLES(MO,Monthly Op Income)
     +          + INCOME_VARIABLES(MO,BTL Monthly Deferred Revenues)
     +          + INCOME_VARIABLES(MO,BTL Monthly Other Income)
     +          + INCOME_VARIABLES(MO,MonthlyMarkToMarket)
     +          + INCOME_VARIABLES(MO,MonthlyEarningNonCompany)
     +          + INCOME_VARIABLES(MO,BTL Monthly NoTax Other Income)
     +          + INCOME_VARIABLES(MO,Monthly Investment Earnings)       ! 157
     +          + INCOME_VARIABLES(MO,BTL Monthly Interest Income)       ! 101
...
C***********************************************************************
     ...
     +          + INCOME_VARIABLES(MO,Monthly Exp File Property Taxes)
     +          + INCOME_VARIABLES(MO,
     +                          Monthly Model Calculated Property Taxes)
     +          + INCOME_VARIABLES(MO,Monthly Operating Revenue Tax)
     +          + INCOME_VARIABLES(MO,Monthly State ATL Income Tax Paid)
...
c
     ...
     +           + INCOME_VARIABLES(MO,Monthly Deferred Fuel Expense)
     +           + INCOME_VARIABLES(MO,Monthly Nuclear Fuel Expense)
c    +           + INCOME_VARIABLES(MO,Owned Nuclear Fuel)  ! included in above
c    +           + INCOME_VARIABLES(MO,Leased Nuclear Fuel) ! included in above
     +           + INCOME_VARIABLES(MO,Monthly DOE Decommissioning)
     +           + INCOME_VARIABLES(MO,Monthly DOE Disposal)
     +           + INCOME_VARIABLES(MO,Monthly Amortization)
     +           + INCOME_VARIABLES(MO,
...
C
     ...
     +            + INCOME_VARIABLES(MO,Base Rates)
     +            + INCOME_VARIABLES(MO,Unbilled Revenues)
     +            + INCOME_VARIABLES(MO,Residential)
     +            + INCOME_VARIABLES(MO,Commercial)
     +            + INCOME_VARIABLES(MO,Industrial)
...
C
     ...
     +                + INCOME_VARIABLES(MO,Secondary Sales)
     +                + INCOME_VARIABLES(MO,Capacity Sales)
     +                + INCOME_VARIABLES(MO,Bulk Power)
     +                + INCOME_VARIABLES(MO,Other Revenue)
     +                + INCOME_VARIABLES(MO,Gas Revenues)
     +                + INCOME_VARIABLES(MO,Relationship Revenues)
     +                + INCOME_VARIABLES(MO,Monthly Catawba Revenues)
     +                + INCOME_VARIABLES(MO,ICAP Revenues)
     +                + INCOME_VARIABLES(MO,Total Derivative Revenues)
     +                + INCOME_VARIABLES(MO,
     ...
     +                 SUM_NONREGULATED_REVENUES
     +                 + INCOME_VARIABLES(MO,WVPA NonMember Power Sales)
      RETURN
      END
C***********************************************************************
...
C      
     ...
     +           + INCOME_VARIABLES(MO,Monthly Service Transactions)
     +           + INCOME_VARIABLES(MO,Monthly Other OandM)
     +           + INCOME_VARIABLES(MO,Monthly Purchased Gas)
     +           + INCOME_VARIABLES(MO,Monthly Other)
     +           + INCOME_VARIABLES(MO,Monthly Transmission Operation)
     +           + INCOME_VARIABLES(MO,Monthly Transmission Maintenance)
     +           + INCOME_VARIABLES(MO,Monthly Distribution Operation)
     +           + INCOME_VARIABLES(MO,Monthly Distribution Maintenance)
     +           + INCOME_VARIABLES(MO,Monthly Customer Accounts)
     +           + INCOME_VARIABLES(MO,Monthly Customer Services)
     +           + INCOME_VARIABLES(MO,Monthly Sales Expense) 
     +           + INCOME_VARIABLES(MO,Monthly AG Operations) 
     ...
     +           + INCOME_VARIABLES(MO,Monthly Emission Credits)
     +           + INCOME_VARIABLES(MO,Monthly Catawba Expenses)
     +           + INCOME_VARIABLES(MO,Total Derivative Expenses)
     +           + INCOME_VARIABLES(MO,Total Fuel Derivative Expenses)
     +           + INCOME_VARIABLES(MO,Monthly Retirement Expense)
     +           + INCOME_VARIABLES(MO,
     +                 Monthly Expense Reserve Margin Capacity Purchase)
     +           + INCOME_VARIABLES(MO,
     +                                WVPA Property Taxes in Power Cost)
...
C
         DO MO = 0, 12
            BALANCE_SHEET_VARIABLES(MO,Monthly TOTAL UTILITY PLANT) =
     +                        BALANCE_SHEET_VARIABLES(MO,
     +                                   Monthly Gross Plant in Service)
     +                        + BALANCE_SHEET_VARIABLES(MO,Monthly CWIP)
            BALANCE_SHEET_VARIABLES(MO,Monthly NET UTILITY PLANT) =
     +           BALANCE_SHEET_VARIABLES(MO,Monthly TOTAL UTILITY PLANT)
     +           - BALANCE_SHEET_VARIABLES(MO,
     +                                 Monthly Accumulated Depreciation)
     +           + BALANCE_SHEET_VARIABLES(MO,Monthly Net Nuclear Fuel)
     ...
     +                                            monthly_total_assets-3 ! 27
               IF(I == Monthly Retirements) CYCLE ! NOT PART OF TOTAL
               IF(I == Monthly Acc Dep Adjustments) CYCLE ! NOT PART OF TOTAL
               IF(I == Monthly Long Term Investments) CYCLE ! NOT PART OF TOTAL
...
C
         BALANCE_SHEET_VARIABLES(:,
     +                          Monthly LTD Balance WO Current Amount) =
     +           BALANCE_SHEET_VARIABLES(:,monthly_long_term_debt)
     +           - BALANCE_SHEET_VARIABLES(:,monthly_ltd_current_amount)
         BALANCE_SHEET_VARIABLES(:,
     +                           monthly_total_capital_wo_current_ltd) =
     +           BALANCE_SHEET_VARIABLES(:,monthly_total_capital)
     +           - BALANCE_SHEET_VARIABLES(:,monthly_ltd_current_amount)
C
         BALANCE_SHEET_VARIABLES(:,Monthly Total Deferred Credits) =
     +          BALANCE_SHEET_VARIABLES(:,monthly_deferred_gains)
     +          + BALANCE_SHEET_VARIABLES(:,Monthly CICA)
     +          + BALANCE_SHEET_VARIABLES(:,
     +                                    Monthly Deferred Income Taxes)
     +          + BALANCE_SHEET_VARIABLES(:,Monthly Deferred ITC Credit)
     +          + BALANCE_SHEET_VARIABLES(:,
     +                               monthly_gain_on_reaquired_debt_bal)
     +          + BALANCE_SHEET_VARIABLES(:,
     +                                Monthly Regulatory Liabilites Bal)
     +          + BALANCE_SHEET_VARIABLES(:,
     +                                   Monthly Other Deferred Credits)
...
C
         SUM_CASH_RECEIPTS =
     +        CASH_VARIABLES(R_MO,Cash Base Rates)
     +        + CASH_VARIABLES(R_MO,Cash Adjustment Clause)
     +        + CASH_VARIABLES(R_MO,Cash Secondary Sales)
     +        + CASH_VARIABLES(R_MO,Cash Other Revenue)
     +        + CASH_VARIABLES(R_MO,cash_catawba_revenues)
     +        + CASH_VARIABLES(R_MO,Cash Gas Revenues)
     +        + CASH_VARIABLES(R_MO,Cash Relationship Revenues)
     ...
     +        + CASH_VARIABLES(R_MO,Cash Bulk Power)
     +        + CASH_VARIABLES(R_MO,Cash Capacity Sales)
     +        + CASH_VARIABLES(R_MO,Cash Government)
     +        + CASH_VARIABLES(R_MO,Cash BTL Revenues)
     +        + CASH_VARIABLES(R_MO,Cash Net of Tax BTL Revenues)
     +        + CASH_VARIABLES(R_MO,Cash Received Accounts Receivable)
     +        + CASH_VARIABLES(R_MO,Cash ICAP Revenues)
     +        + CASH_VARIABLES(R_MO,Total Cash Derivative Revenues)
     +        + CASH_VARIABLES(R_MO,Total Cash Fuel Derivative Revenues)
     +        + CASH_VARIABLES(R_MO,Cash Gas Wholesale Revenues)
     +        + CASH_VARIABLES(R_MO,
     +                        Cash Income Reserve Margin Capacity Sales)
     +        + CASH_VARIABLES(R_MO,
     +                             WVPA Cash From NonMember Power Sales)
     +        + CASH_VARIABLES(R_MO,Cash ATL Other Rev)
...
C
         SUM_CASH_PAYMENTS =  
     +        CASH_VARIABLES(R_MO,Cash BTL Lease Interest Payments)
     +        + CASH_VARIABLES(R_MO,Cash Fossil Fuel)
     +        + CASH_VARIABLES(R_MO,Cash Purchased Power)
     +        + CASH_VARIABLES(R_MO,Cash Service Transactions)
     +        + CASH_VARIABLES(R_MO,Cash Variable OandM)
     +        + CASH_VARIABLES(R_MO,Cash Fixed OandM)
     +        + CASH_VARIABLES(R_MO,Cash Other OandM)
     +        + CASH_VARIABLES(R_MO,Cash Purchased Gas)
     +        + CASH_VARIABLES(R_MO,Total Cash Derivative Expenses)
     +        + CASH_VARIABLES(R_MO,Total Cash Fuel Derivative Expenses)
     +        + CASH_VARIABLES(R_MO,Cash Other)
     +        + CASH_VARIABLES(R_MO,Cash Leased Nuclear Fuel)
     +        + CASH_VARIABLES(R_MO,Cash DSM Expense)
     +        + CASH_VARIABLES(R_MO,Cash DSM Rebate)
     +        + CASH_VARIABLES(R_MO,Cash Lease Interest Payments)
     +        + CASH_VARIABLES(R_MO,Cash Emission Credits)
c    +          + CASH_VARIABLES(R_MO,Cash DOE Decommissioning) ! 9/7/00 removed to match annual
     +        + CASH_VARIABLES(R_MO,Cash DOE Disposal)
     +        + CASH_VARIABLES(R_MO,cash_catawba_expenses)
     +        + CASH_VARIABLES(R_MO,Cash BTL Expenses)
     +        + CASH_VARIABLES(R_MO,Cash Transmission Operation)
     +        + CASH_VARIABLES(R_MO,Cash Transmission Maintenance)
     +        + CASH_VARIABLES(R_MO,Cash Distribution Operation)
     +        + CASH_VARIABLES(R_MO,Cash Distribution Maintenance)
     +        + CASH_VARIABLES(R_MO,Cash Customer Accounts)
     +        + CASH_VARIABLES(R_MO,Cash Customer Services)
     +        + CASH_VARIABLES(R_MO,Cash Sales Expense)
     +        + CASH_VARIABLES(R_MO,Cash AG Operations)
     +        + CASH_VARIABLES(R_MO,Cash AG Maintenance)
     +        + CASH_VARIABLES(R_MO,Cash Unfunded Retirement Payments)
     +        + CASH_VARIABLES(R_MO,Cash Storm Payments)
     +        + CASH_VARIABLES(R_MO,Cash Vacation Payments)
     +        + CASH_VARIABLES(R_MO,Cash Post Retirement Payments)
     +        + CASH_VARIABLES(R_MO,Cash Paid on Accounts Payable)
     +        + CASH_VARIABLES(R_MO,Cash to Post Retirement Payments)
     +        + CASH_VARIABLES(R_MO,
     +                    Cash Expense Reserve Margin Capacity Purchase)
     ...
     +        + CASH_VARIABLES(R_MO,CashEarningsNonCorporate)
         IF(WVPA()) THEN
            SUM_CASH_PAYMENTS =
     +          SUM_CASH_PAYMENTS
     +          + CASH_VARIABLES(R_MO,WVPA Cash Member Cost of Power) ! these are zero 5/3/04
     +          + CASH_VARIABLES(R_MO,WVPA Cash NonMember Cost of Power) ! these are zero 5/3/04
     +          + CASH_VARIABLES(R_MO,WVPA Cash Member Cost of Services) ! 205
     +          + CASH_VARIABLES(R_MO,
     +                             WVPA Cash NonMember Cost of Services) ! 206
     +          + CASH_VARIABLES(R_MO,
     +                              WVPA Cash to Accrued Member Revenue) ! 209
...
C***********************************************************************
C
         CASH_VARIABLES(R_MO,Cash Net Investment Salvage Other) =
     +               CASH_VARIABLES(R_MO,Cash Net Investments)
     +               + CASH_VARIABLES(R_MO,Cash Net Salvage)
     +               + CASH_VARIABLES(R_MO,Cash Net Other)
         SUM_CONSTRUCTION_CASH_PAYMENTS =  
     +          CASH_VARIABLES(R_MO,Cash Net Plant CapX)
     +          + CASH_VARIABLES(R_MO,Cash AI Investment)
     +          + CASH_VARIABLES(R_MO,Cash Nuclear Fuel Fabrication)
     +          + CASH_VARIABLES(R_MO,Cash Net Investment Salvage Other)
     +          + CASH_VARIABLES(R_MO,Cash Equity to Subsidiaries)
     +          + CASH_VARIABLES(R_MO,Cash Capital Leases)
     +          + CASH_VARIABLES(R_MO,Cash Acquisition Cost)
      RETURN
C***********************************************************************
      ENTRY SUM_CASH_TOTAL_CAPITAL_REQUIREMENTS(R_MO,CASH_VARIABLES)
C***********************************************************************
C
         CASH_VARIABLES(R_MO,Cash Additions to Inventories) =
     +        CASH_VARIABLES(R_MO,Cash Fuel Inventory Change)
     +        + CASH_VARIABLES(R_MO,Cash Gas Storage Inventory Addition)
     +        + CASH_VARIABLES(R_MO,Cash Materials Inventory Addition)
C
         SUM_CASH_TOTAL_CAPITAL_REQUIREMENTS =  
     +              CASH_VARIABLES(R_MO,Cash Total Construction)
     +              + CASH_VARIABLES(R_MO,Cash Nuc Decommissioning Fund)
     +              + CASH_VARIABLES(R_MO,Cash Working Capital)
     +              + CASH_VARIABLES(R_MO,Cash Fuel Inventory Change)
     +              + CASH_VARIABLES(R_MO,Cash LTD Retirements)
     +              + CASH_VARIABLES(R_MO,Cash PS Retirements)
     +              + CASH_VARIABLES(R_MO,Cash LTD PS Issue Expense)
     +              + CASH_VARIABLES(R_MO,Cash Common Issue Expense)
     +              + CASH_VARIABLES(R_MO,Cash Common Stock Buyback)
     +              + CASH_VARIABLES(R_MO,Cash Deferred Expenses)
     +              + CASH_VARIABLES(R_MO,Cash Change LTInvestments)
     +              + CASH_VARIABLES(R_MO,Cash Change Debt Investments)
     +              + CASH_VARIABLES(R_MO,Cash For Redeming Notes Owed)
     +              + CASH_VARIABLES(R_MO,Cash For New Notes Issued)
     +              + CASH_VARIABLES(R_MO,Cash Lease Expense)
     +              + CASH_VARIABLES(R_MO,Cash BTL Lease Cash)
     +              + CASH_VARIABLES(R_MO,
     +                              Cash Gas Storage Inventory Addition)
     +              + CASH_VARIABLES(R_MO,
     +                                Cash Materials Inventory Addition)
     +              + CASH_VARIABLES(R_MO,Cash ARO Payments)
     +              + CASH_VARIABLES(R_MO,Cash ARO Trust Payments)
...
C
     ...
     +                                            MONTHLY_OCI_BALANCE(:)
         R_BALANCE_SHEET_VARIABLES(:,Monthly Net FASB 87) =
...
C
     ...
     +                               Monthly Owned Nuclear Fuel Expense)
         MONTHLY_LEASED_NF(:) = INCOME_VARIABLES(:,
     +                              Monthly Leased Nuclear Fuel Expense)
...
c
     ...
     +         INCOME_VARIABLES(:,Monthly INCOME BEFORE INTEREST)
     +         + INCOME_VARIABLES(:,Monthly ITC Amortization)
     +         + INCOME_VARIABLES(:,Monthly State ATL Income Tax Paid)
     +         + INCOME_VARIABLES(:,Monthly State Tax on Capital)
     +         + INCOME_VARIABLES(:,Monthly Federal ATL Income Tax Paid)
     +         + INCOME_VARIABLES(:,Monthly Federal Tax on Capital)
     +         + INCOME_VARIABLES(:,Monthly Income Tax Deferrals Dr)
     +         + INCOME_VARIABLES(:,Monthly Income Tax Deferrals Cr)
     +         + INCOME_VARIABLES(:,Monthly AFUDC Borrowed)
     +         + INCOME_VARIABLES(:,BTL Monthly Income Taxes)
     +         + INCOME_VARIABLES(:,BTL Monthly Income Tax Deferrals)
         INCOME_VARIABLES(:,Monthly EBITAD) =
     +                   INCOME_VARIABLES(:,Monthly EBIT)
     +                   + INCOME_VARIABLES(:,Monthly Book Depreciation)
     +                   + INCOME_VARIABLES(:,Monthly Amortization)
     +                   + INCOME_VARIABLES(:,BTL Monthly Amortization)
      END SUBROUTINE
C***********************************************************************
...
c
         INCOME_VARIABLES(MO,Monthly IMPA Net Income Coverage) = NA
...
c         ENDIF
C         DO  MO = 0, 12
            SUM_VALUES = CASH_VARIABLES(MO,Cash LTD Retirements) 
     +                 + INCOME_VARIABLES(MO,Monthly Other ATL Amort)
     +                 + INCOME_VARIABLES(MO,Monthly LTD Total Interest)
     +                 - INCOME_VARIABLES(MO,Monthly LTD Amort Interest)
            IF(SUM_VALUES /= 0.) THEN
               INCOME_VARIABLES(MO,Monthly IMPA Net Income Coverage) =
