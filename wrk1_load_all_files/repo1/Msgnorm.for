C     Last change: msg 9/11/2021 10:07:35 AM
!			MSG 10/30/2015 3:57:06 PM
!			MSG 9/15/2015 2:04:05 PM
!			MSG 9/15/2015 1:51:30 PM
!			MSG 9/15/2015 1:02:52 PM
!			MSG 9/15/2015 12:31:29 PM
!			MSG 9/15/2015 12:11:16 PM
!			MSG 5/19/2015 10:23:46 AM
!			MSG 5/18/2015 4:59:38 PM
!			MSG 5/18/2015 4:28:09 PM
!			MSG 5/18/2015 4:26:23 PM
!			MSG 5/18/2015 4:10:54 PM
!			MSG 5/18/2015 4:08:31 PM
!			MSG 5/18/2015 4:04:01 PM
C***********************************************************************
      FUNCTION MIDAS_NORMAL_SIMULATION(SCENAME)
C***********************************************************************
C
C
C VARIABLES TO SIZE THE MODEL
C
      INCLUDE 'SpinLib.MON'
...
C
     ...
     +                    READ_PLANT_CONTRACT_DEMAND_LINK_DATA,
     +                    READ_GENERIC_PLANT_DEMAND_LINK_DATA,
...
C     VARIABLES FOR COST OF SERVICE
     ...
     +          ERROR_FILE_EXISTS
      SAVE FIRST_PASS
C     LOGICAL*1 HAVE_READ_LM_INPUT
C
C TYPE DECLARATION FOR THE CHECKING THE HISTORICAL LOAD DATA
C
C 2/14/93. GAT. ADDED DAY_* FOR KCPL. 3/3/93. GAT. ALTERED FOR HOURLY_IN
C
C     TYPE DECLARATION FOR /SCREEN/
C
      LOGICAL*4 ESCAPE
      LOGICAL*1 LM_FIN_FILE_EXISTS
C     LOGICAL*4 REPORT_FILE_EXISTS
      LOGICAL*1 RUN_LOAD,RUN_PROCOST,
     +          RUN_FINANCE,
     +          RUN_FINASSET
C 3/1/93. GAT. HOURLY REFERENCE LOAD SHAPES
C 5/5/93. GAT. SCREENING INFORMATION.
      INTEGER*4 END_POINT_STARTING_RECORD,
     +          SAVE_END_POINT_STARTING_RECORD,
     +          SET_OUT_REC_TO_ENDPNT_START_REC
C     LOGICAL*1 CONVERT_ALL
      LOGICAL*1 FORECAST_REPORT
C FILE NAMES
      CHARACTER (LEN=256) :: FILE_NAME,STATUS_FILE_NAME,
     +                       CURRENT_DIRECTORY,BIP_FILE_NAME
C DECLARATION SECTION FOR LAM PROGRAMS
C
C COMMON BLOCKS
C
      INCLUDE 'LAMCOM.MON'
...
C LAM FILE NAMES
C     CHARACER*5 RESFRC,INDFRC,COMFRC,OTH1FRC,OTH2FRC,OTH3FRC,
C    +            CLSHIST,CLSHAPE,LMGLS,ELASTICITY_FILE,LDMGT,
C    +            CAP_RATIOS_FILE,ESCALFL,ALLOCATOR_FILE,UNIT_PARMS
C     INCLUDE 'OVLYCOM.MON'
C
C MONTHLY FUEL PRICE ITEMS ADDED 9/22/92
C
      LOGICAL*1 FUEL_PRICE_DATA_AVAILABLE
      INCLUDE 'DSMFRCOM.MON'
C     INCLUDE 'DSMCOM.MON'
C
      INTEGER*4 ERROR,DEALLOCATE_FUEL_INVENTORY_ID
C
C PARAMETER SECTION
C  COMMON BLOCK FOR ERROR MESSAGES
...
C  BEGINNING OF THE COMMON BLOCK SECTION FOR THE PRODUCTION MODULE
      INCLUDE 'PRODCOM.MON'
      INCLUDE 'PROD2COM.MON'
      INCLUDE 'CNTRCOM.MON'
C
C VARIABLES FOR THE DETAILED REPORTS
C
      INCLUDE 'DREPTCOM.MON'
C
      INCLUDE 'ANNLCOM.MON'
C
C  END COMMON BLOCK SECTION FOR THE PRODUCTION MODULES
C
C NEW VARIABLES ADDED BY M.S.G. JULY 29, 1993 - 
C      
     ...
     +          WVPA_INITIALIZE_TRACKER_DATABASE,
     +          READ_ESCALATION_FILE
      LOGICAL*1 SYSTEM_BASED_FORECAST,SYSTEM_FORECAST,
...
C COAL MODEL 
     ...
     +                    RUN_COAL_MODEL_ONLY
C      LOGICAL (KIND=4) :: READ_BASIN_SUPPLY_SO2_DATA,
C     +                    READ_PLANT_CONTRACT_DEMAND_LINK_DATA
      INTEGER (KIND=2) :: CURRENT_YEAR
      LOGICAL (KIND=1) :: TRUE=.TRUE.,FALSE=.FALSE.
C    
C********** END OF THE DATA DECLARATIONS *********
C
C
...
! 091416.
      ...
      CAPACITY_AREA_NAME = " "
C
C IF CAPACITY PLANNING IS ACTIVE READ THE OBJECTIVE FUNCTION 
C
...
! 042811. MOVED FOR TVA STOCHASTICS.
     ...
     +                 RUN_FUTURE_ASSETS)
C
C FOR A ONE BRANCH TREE NEED TO UNSET THE END_OF_STUDY_FLAG 
C  WHEN AUTO_OPTIM IS BEING RUN
C
...
c            WRITE(6,'("&",I4)') END_POINT
     ...
     +                   ENDPOINTS_RUN,*30,RUN_FUTURE_ASSETS)
      ...
      ENDIF
c
c code to run coal model as a stand alone
c
      TestFactor = .5
     ...
     +                                                   ALL_VERSIONS,0)
         ...
         CoalLPActive = READ_PLANT_CONTRACT_DEMAND_LINK_DATA(BASE_YEAR,
     +                                           BASE_YEAR+STUDY_PERIOD)
c         CoalLPActive = READ_GENERIC_PLANT_DEMAND_LINK_DATA(BASE_YEAR)
     ...
     +                            ACCESS="TRANSPARENT",STATUS="UNKNOWN")
      ...
      ENDIF   
c the following is for CP&L dynamic allocations
c removed 2/2/99 because of divid by zero problem
      IF(WVPA()) VOID_LOGICAL = WVPA_INITIALIZE_TRACKER_DATABASE()
      ...
      ENDIF
C     IF(RUN_FUTURE_ASSETS) THEN
C        CALL OPEN_ASSET_VECTOR_FILE(82)
C         CALL FINASSET(.FALSE.) ! MOVED BELOW
C        CALL CLOSE_ASSET_VECTOR_FILE
C     ENDIF
C
C THIS IS THE RECYCLE POINT FOR AUTO_OPTIM OPERATION
C
C
C SET PROPER ESCALATION VALUES
C
C CALL TO ESCALATION OBJECT ADDED 5/20/93 WILL REMOVE THE OTHER READS
C LATER
C
...
C
C SET-UP MONTHLY FUEL PRICE FILE FOR THIS END POINT
C
...
C
C     VOID_LOGICAL = RUN_SPECS_OBJECT()
...
C
C      
c         PRODP_FILE_EXISTS = PRODUCTION_PARAMETERS_OBJECT(INT2(472)) MSG
c         VOID_LOGICAL = UPDATE_PRODUCTION_PARAMETERS(YEAR)  MSG
!
...
C
C
C 8/9/94. GAT. MOVED FROM INSIDE THE YEAR=1 LOOP           
C
...
C
     ...
     +                                                  FIRST_PASS) THEN
               TESTING_PLAN = .TRUE.
            ENDIF
c        CALL RINIT ! 8/12/94. GAT. MOVED FROM INSIDE FINANBSE. removed msg 3/2/98
...
c
c the following call order must be kept.
c
               CoalLPActive = READ_COAL_ESCALATION_FILE(BASE_YEAR)
               CoalLPActive = READ_BASIN_SUPPLY_SO2_DATA()
               CoalGenericPlant =
     +                    READ_GENERIC_PLANT_DEMAND_LINK_DATA(BASE_YEAR)
               CoalExistingPlant = 
     +                   READ_PLANT_CONTRACT_DEMAND_LINK_DATA(BASE_YEAR,
     +                                           BASE_YEAR+STUDY_PERIOD)
     ...
     +                                                CoalLPActive) THEN 
               ...
               ENDIF   
             ENDIF   
C            IF(CAPACITY_PLANNING_METHOD() == 'MX'  .AND. 
C     +                         GREEN_MRX_METHOD() == 'GX') END_POINT = 1
C            CALL STORE_END_POINT(END_POINT)
...
!            
            ...
            ENDIF
C            WRITE(FORECAST_YEAR,100)  'Forecast Year: ',BASE_YEAR+YEAR
C
...
C
     ...
     +                                  GREEN_MRX_METHOD() == 'GX') THEN
            ...
            ENDIF
C
C TESTING THE GRX PLANNING LOOP FOR HOLDING THE CURRENT VALUE OF VARIABLES
C
          GRX_CONVERGED =.FALSE. 
          OneMoreIter = .false.
     ...
     +                                                   ALL_VERSIONS,1)
               ENDIF
C               VOID_LOGICAL = UPDATE_PRODUCTION_PARAMETERS(YEAR)
...
C
C NEED PEAKS FOR CAPACITY PLANNING
C
                        CALL DISPLAY_TIME
                        CALL CLS(16,9,32)
!
...
c            VOID_LOGICAL = EL_RESET_PRICES()
...
!            VOID_REAL = INIT_ANNUAL_GROSS_MARGIN()
               GRX_ITERATIONS = GRX_ITERATIONS + 1
               GRX_PRT_ENDPOINT = GRX_PRT_ENDPOINT + 1 
C               END_POINT = END_POINT + 1
C               CALL STORE_END_POINT(END_POINT)
C               ENDPOINTS_RUN = MAX(ENDPOINTS_RUN,END_POINT)
...
c           IF(.NOT. ASSET_ANALYST_ONLY) THEN
            IF(ACTIVE_CAPACITY_PLANNING_METHOD == 'PR' .AND. 
     +                         CAPACITY_PLANNING_IS_ACTIVE .AND.
     +                                                  FIRST_PASS) THEN
...
C
C RECOVERY MEMORY AT END OF ENDPOINT 7/24/00 MSG
C
...
C
     ...
     +                                                  FIRST_PASS) THEN
            ...
            CAPACITY_PLANNING_IS_ACTIVE = .FALSE.
c
c temp code to look at run differences 2/19/95
c
C            ENDPOINTS_RUN = ENDPOINTS_RUN + 1
C            END_POINT = END_POINT + 1
C            CALL STORE_END_POINT(END_POINT)
         ELSE
            EXIT
...
!      ERROR = DEALLOCATE_FUEL_INVENTORY_ID()
      IF(YEAR_LOOP > LAST_LOOP_YEAR) THEN
         ENDPOINTS_RUN = ENDPOINTS_RUN + 1
c         INQUIRE(UNIT=38,RECL=REC_LENGHT)
c         CLOSE(38)
c         OPEN(38,FILE=BIP_FILE_NAME,ACCESS="TRANSPARENT",STATUS="OLD",
c     +                                                       IOSTAT=IOS)
c         WRITE(38,REC=4)ENDPOINTS_RUN,BASE_YEAR,BASE_YEAR+LAST_LOOP_YEAR
c         CLOSE(38)
c         OPEN(38,FILE=BIP_FILE_NAME,ACCESS="DIRECT",STATUS="OLD",
c     +                                                  RECL=REC_LENGHT)
      ELSE
C
C NEED TO RESET THE OUTPUT POINTER BECAUSE THE PLAN DIDN'T
C
         END_POINT_STARTING_RECORD = SET_OUT_REC_TO_ENDPNT_START_REC()
C
      ENDIF
C     IF(ACTIVE_CAPACITY_PLANNING_METHOD=='SC') 
C    +                                  CALL CLOSE_SCREENING_INFORMATION
C
C CLEAN UP NEEDED ? IF AUTO_OPTIM NOT ACTIVE (NORMAL CLEAN UP)  THIS 
C  NEEDS TO REVIEW BECAUSE IT IS 640k AND FILES = DEPENDENT
C
      CALL CLOSE_FINANCIAL_PARAMETER_FILE
      CALL CLOSE_CAPITAL_RATES_VECTOR_FILE
C
C CLOSE THE MULTI-YEAR FINANCIAL RUN SPECIFICATIONS FILE
C
...
!
C      CALL CLOSE_SERVICE_TRANSACTIONS
c      CALL FREE_INIT_ASSET_CLASS_INFO_ARRAYS()
...
! THE FOLLOWING CLOSES ALL OPEN BINARY FILES.
      ...
      CALL CLOSE_FILES(SCENAME,ENDPOINTS_RUN,BASE_YEAR+LAST_LOOP_YEAR)
      MESSAGE = .FALSE.
