...
!      LOGICAL*1 TEST_DEPTH_2_ADVANCE
      REAL ::  VOID_REAL
...
!
      INTEGER (kind=2) ::  ACTIVE_OPTIONS,POINTR,
...
!
     ...
     +     MINIMUM_TESTING_RM,MIN_CAPACITY_TESTING_MARGIN
      ...
      INTEGER (kind=2) ::  BEST_VALUE_LOCATION,NEXT_OPTION_TO_CHECK/1/,
     +          OPTION_DEPTH
...
!
     ...
     +          LAST_LEVEL_PLANS
      LOGICAL (kind=1) ::  MATCH_NOT_FOUND,POSITION_MATCHED(:),
     +          VOID_LOGICAL
...
!
      ...
      INTEGER (kind=2) ::    MAX_BAD_PLANS,NUM_OF_BAD_PLANS,
...
!
      ...
      INTEGER (kind=2) ::   NUMBER_OF_ACTIVE_FILL_OPTIONS,POS,
...
!
!      SAVE ODD_PLANS,BAD_EVEN_PLANS,BAD_ODD_PLANS,
!     +     NUM_OF_BAD_PLANS,NUM_OF_BAD_PLANS_LAST_DEPTH,
!     +     MAX_BAD_PLANS
!      SAVE FILL_OPTIONS_ACTIVE
!      SAVE DEPTH_BEST_PLAN_FOUND,LOCATION_OF_BEST_PLAN
!      SAVE DEPTH_BEST_PLANS,
!     +     DEPTH_PLAN_OBJECTIVE_VALUE,
!     +     PLANS_2_TEST_AT_NEXT_DEPTH,
...
!     +     PLAN_HAS_MUT_EXC_UNITS,
!     +     PLAN_HAS_MUT_EXC_UNIT,
!     +     ANN_CAP,PEAK,OPTION_POINTR,
!     +     OPTION_ANN_UNITS,OPTION_AVAIL_UNITS,
!     +     AVAIL_CUM_UNITS,
!     +     AVAIL_ANNUAL_UNITS,
!     +     OPTION_CAP,OPTION_LOADING_TYPE,
!     +     OPTION_IS_ACTIVE,
...
!     +     MAX_OPTION_DEPTH,MIN_OPTION_DEPTH,
...
!           TARGET_MET = LOLP_CHECK(LOLP)
         ELSEIF(PLANNING_TRIGGER_METHOD == 'U') THEN
!           TARGET_MET = UNSERVED_ENRG_CHECK(UNSERVEC_ENERGY)
         ELSE
...
!                    TARGET_MET = UNSERVED_ENRG_CHECK()
                  ELSE
                     ...
                     ENDIF
!                     CURRENT_CAPACITY_RATIO = TEST_ANN_CAP/TEST_PEAK
!                     TARGET_MET =
!     +                  CURRENT_CAPACITY_RATIO >= CURRENT_TARGET_RATIO
...
!                          TARGET_MET = UNSERVED_ENRG_CHECK()
                        ELSE
                           ...
                           ENDIF
!                          CURRENT_CAPACITY_RATIO=TEST_ANN_CAP/TEST_PEAK
!                          TARGET_MET = CURRENT_CAPACITY_RATIO >=
!    +                                              CURRENT_TARGET_RATIO
!                          TARGET_MET =
!    +                        RETURN_RESERVE_MARGIN_RATIO(
!    +                                 TEST_PEAK,TEST_ANN_CAP,R_YEAR)
...
!         CALL LOCATE(4,66)
!         WRITE(6,"('&',F9.1/)")  OBJECTIVE_FUNCTION_VALUE
...
!         IF(OPTION_DEPTH > 1)
!     +        VOID_REAL=STORE_DEPTH_BEST_PLANS(OBJECTIVE_FUNCTION_VALUE)
         IF(OPTION_DEPTH > 1)
...
!            CALL CLS(3,41,48)
!            CALL LOCATE(3,41)
!            WRITE(6,"('&',F7.1/)") BEST_OBJECT_FUNCTION_VALUE
...
!
      ...
      RETURN
!***********************************************************************
!      ENTRY TEST_DEPTH_2_ADVANCE
!C***********************************************************************
!         TEST_DEPTH_2_ADVANCE = .TRUE.
!         IF(MINIMIZED_OBJECT_FUNCTION) THEN
!            IF(CURRENT_DEPTH_BEST_VALUE >= LAST_DEPTH_WOREST_VALUE)
!     +                                    TEST_DEPTH_2_ADVANCE = .FALSE.
!         ELSE
!            IF(CURRENT_DEPTH_BEST_VALUE <= LAST_DEPTH_WOREST_VALUE)
!     +                                    TEST_DEPTH_2_ADVANCE = .FALSE.
!         ENDIF
!         IF(TEST_DEPTH_2_ADVANCE) THEN
!            LAST_DEPTH_WOREST_VALUE = SET_DEPTH_TEST_VALUES()
!         ELSE
!            VOID_REAL = RESET_LAST_DEPTH_WOREST_VALUE()
!            VOID_REAL = SET_DEPTH_TEST_VALUES()
!         ENDIF
...
!        CALL PRINTV(1000,OPTION_STRING(1:14))
      RETURN
