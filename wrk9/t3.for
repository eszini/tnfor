...
!
     ...
     +       S_RESERVE_CAPACITY_REV/0./,
     +       S_WHEELING_REV/0./,
     +       S_OEI_REV/0./,
     +       S_PKR_REV/0./,
     +       S_SUPPLEMENTAL_ENERGY_REV/0./,
     +       S_EXCHANGE_CAPACITY_EXP/0./,
     +       S_PURCHASED_ENERGY_EXP/0./,
     +       S_SURPLUS_ENERGY_EXP/0./,
     +       S_EXCHANGE_ENERGY_EXP/0./,
     +       S_PURCHASED_CAP_PAYMENTS/0./,
     +       S_LEVEL_PURCHASED_CAP_PAYMENTS/0./,
     +       S_OTHER_NET_REV_RETURN/0./,
     +       S_PURCHASED_CAP_DEFERRED_DEBITS/0./,
     +       S_DEFERRED_TAXES_PURCHASED_CAP/0./,
     +       S_IS_DEFERRED_TAXES/0./
...
!
     ...
     +       CATAWBA_TOTAL_LEVEL_CAP_PAYMTS,CATAWBA_TOTAL_CAP_PAYMENTS,
     +       CATAWBA_TOTAL_EXPENSES,CATAWBA_TOTAL_REVENUES
!
...
!
      ...
      INTEGER (kind=2) ::    R_YEAR,I,CLASS_POINTER,DELETE,YR,
     +            ASSET_CLASS,ASSET_CLASS_ID,
     +            ASSET_ALLOCATION_VECTOR,
     +            I2TEMP,SAVE_CPL_READ_VARIABLES
      INTEGER ::  IOS
!
      REAL (kind=4) ::       ASSET_ALLOCATOR
      REAL (kind=4) ::
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     +            R_EMC_ENERGY_PRICE_SWITCH,
     +            R_PA_ENERGY_PRICE_SWITCH
      INTEGER (kind=2) ::    EMC_PA_CLASS,ANNUAL_EMC_PA_CLASS/0/,
     +            CLASS_FROM_READ_CPL,SAVE_CPL_READ_VARIABLES
      REAL (kind=4) ::    EMC_ANN_AVE_PROD,PA_ANN_AVE_PROD,
     +         R_EMC_ANN_AVE_PROD,R_PA_ANN_AVE_PROD,
     +         EMC_ANN_AVE_PROD_SAVE/0./,PA_ANN_AVE_PROD_SAVE/0./
!
!
!
...
!     INCLUDE 'LAMCOM.MON'
!
      INTEGER (kind=2) ::  CURRENT_YEAR,CURRENT_MONTH,
     +   EMC_CLASS,R_MONTH,R_CURRENT_YEAR
      PARAMETER (EMC_CLASS=5) ! (OTHER 2) IN THE DATA BASE
      REAL (kind=4) ::    EMC_ENERGY_REVENUE
      REAL (kind=4) ::    ANN_EMC_ENERGY_REVENUE/0./,R_EMC_RATE,
     +         ANN_EMC_ENERGY_SALES/0./,EMC_SALES,NON_COIN_PEAK,
     +         ANN_NON_COIN_PEAK/0./,R_ANN_EMC_ENERGY_SALES,
     +         R_ANN_NON_COIN_PEAK,R_ANN_EMC_ENERGY_REVENUE,
     +         R_ANN_EMC_CAP_REVENUE,
     +         R_ANN_EMC_TRAN_REVENUE,ANN_SUM_NON_COIN_PEAK/0./,
     +         EMC_TRANS_RATE,R_ANN_SUM_TRAN_PEAK
      REAL (kind=4) ::
...
!
      ...
      FUNCTION CPL_ACTIVE()
      use logging
      LOGICAL (kind=1) ::    SAVE_CPL_STATUS/.FALSE./,CPL_ACTIVE,
     +            CHANGE_CPL_STATUS_TO_ACTIVE
!
...
!
      ...
      INTEGER (kind=2) ::    I,J,HOURS_INCREMENT,UNITNO,LAST_POINT,
     +            PA_UNIT_INDEX(:),NBLOK2,BLKNUM,
     +            CURRENT,PAUNITNO,ISEAS,ISTART,NO_PA_RESOURCES,
     +            NO_PA_CONTRACTS/5/,CURRENT_CPL_CONTRACT,
     +            NO_PA_ACCTS/7/,
     +            OFF_SYSTEM_SALES_CLASS,R_ISEAS,
     +            Reserve Capacity,
     +            Unused Supplemental,
     +            Supplemental Cap,
     +            PA Resource,
     ...
     +         LPROB(CONVOLUTION_POINTS,2),
!     +         CPL_LPROB(CONVOLUTION_POINTS,2),
     +         SYSTEM_LODDUR(CONVOLUTION_POINTS),
     +         PA_DX,PA_LODDUR(CONVOLUTION_POINTS),
!     +         CPL_LODDUR(CONVOLUTION_POINTS),
     +         LEFT(:),RIGHT(:),
     +         EA,A,B,ENRG,PA_BASE,PA_PEAK,SYSTEM_PEAK,
     +         LIMR,PA_CAPSYS,
     +         SEAS_HOURS/0/,REMAINING_ENERGY,CAPON,CAPBLK,
     +         PA_ENERGY(:,:),EFFECTIVE_CAPACITY(:,:),CUMCAP,BLK_MW,
     +         MAINTENANCE_RATE(MAX_CL_UNITS),UNIT_ENRG,
     +         TEMPEA(2,MAX_CL_UNITS),RESERVE_CAPACITY,
     +         RESERVE_PERCENT,
     +         UNUSED_SUPP_CAPACITY,SUPPLEMENTAL_CAPACITY,
     +         PA_EXP_CAPSYS,PA_CONTRACT_CAPACITY,
     +         PA_RESOURCE_ENERGY_COST,SEPA_ENERGY(12),
     +         SEPA_ENERGY_PER_ECAP(12),
     +         OFF_SYSTEM_SALES_ENERGY(0:12),
     +         R_CPL_GEN_COST,R_CPL_PA_ENTITLE,
     +         R_CPL_PUR_POWER,R_CPL_OFF_SYS_INC_COST,
     +         R_CPL_PA_MWH_COST,
     +         CPL_FUEL_GENERATION(0:12),
     +         CPL_NUC_GENERATION(0:12),
     +         CPL_PUR_POWER_ENERGY_COST(0:12),
     +         CPL_NCEMPA_ENTITLEMENT(0:12),
     +         CPL_NCEMPA_ENTITLEMENT_MWH(0:12),
     +         CPL_NCEMPA_MWH_SUPPLIED_BY_CPL(0:12),
     +         CPL_NCEMPA_COST_MWH_SUP_BY_CPL(0:12),
     +         CPL_OFF_SYSTEM_INCR_COST(0:12)
...
!
     ...
     +         SYS_EFFECTIVE_CAPACITY(2,MAX_CL_UNITS),
     +         SURPLUS_ENERGY,REPLACEMENT_ENERGY,WRITE_ENERGY,
     +         BUYBACK_ENERGY,MAYO1_BUYBACK_PERCENT/.0334/,
     +         HARRIS1_BUYBACK_PERCENT/.3333/,
     +         MAYO1_BUYBACK,HARRIS_BUYBACK,RETAINED_PERCENT,
     +         MAYO1_REPLACEMENT,ROX4_REPLACEMENT,
     +         TOTAL_ENT_ENERGY/0./,TOTAL_ENT_CAPACITY/0./,
     +         RESOURCE_PERCENT
!
! REVENUE CALCULATIONS
!
      REAL (kind=4) ::          WRITE_FUEL_COST,AVERAGE_FUEL_COST,
     +               AVERAGE_PRODUCTION_COSTS,SYS_FUEL_COST,SYS_ENERGY,
     +               BLOCK_FUEL_COST(2,MAX_CL_UNITS),
     +               WRITE_PRODUCTION_COST,TOTAL_ENT_FUEL_COST/0./,
...
!
     ...
     +         MON_PA_COST(:,:),
     +         MON_PA_CAP(:,:),
     +         ANN_PA_CAP(:),ANN_SYS_CAP(:),ANN_PA_COST(:),
     +         ANN_SYS_COST(:),
     +         PA_HOURLY_LOADS(:),
     +         CPL_ENERGY_REVENUE_FROM_PA(0:12),CPL_ENERGY_TO_PA(0:12),
     +         CPL_EMC_REVENUE(0:12),R_CPL_EMC_REVENUE,
     +         CPL_ENERGY_EXPENSE_TO_PA(0:12),CPL_ENERGY_FROM_PA(0:12),
     +         CPL_ANN_EXPENSE_TO_PA/0./,CPL_ANN_REVENUE_FROM_PA/0./,
     +         R_CPL_ENERGY_REVENUE_FROM_PA,R_CPL_ENERGY_TO_PA,
     +         R_CPL_ENERGY_EXPENSE_TO_PA,R_CPL_ENERGY_FROM_PA,
     +         PA_RESERVE_RATE,PA_SUPPLEMENTAL_RATE,
     +         PA_TRANSMISSION_RATE,
     +         PA_MAYO_BUYBACK,PA_HARRIS_BUYBACK,R_BUYBACK,
     +         S_CATAWBA_REVENUES,S_CATAWBA_EXPENSES,
     +         S_CPL_ENERGY_TO_PA,S_CPL_ENERGY_FROM_PA
...
!
     ...
     +            CPL_PAEN_RPT_HEADER,CPL_PAEN_UNIT,CURRENT_YEAR,
     +            CURRENT_MONTH,ANNUAL_COUNTER/0/,
     +            REMAINING_SYSTEM_CLASS/4/
      INTEGER ::  CPL_PAEN_REC
      REAL (kind=4) ::  WRITE_CAPACITY
      REAL (kind=4) ::  CF,TOTAL_WRITE_ENERGY,TOTAL_WRITE_CAPACITY,
     +       RS_CLASS_LOSSES
...
!
      LOGICAL (kind=1) ::    ECAP_REPORT_NOT_OPEN/.TRUE./
      LOGICAL (kind=1) ::    ECAP_REPORT_ACTIVE,
     +            HOURLY_ECAP_REPORT,HOURLY_ECAP_ACTIVE,PA_SWITCH
...
!     INCLUDE 'LAMCOM.MON'
      ...
      REAL (kind=4) ::    LPROB2(*)
      REAL (kind=4) ::    SYSTEM_LODDUR(CONVOLUTION_POINTS),SYSTEM_PEAK,
     +         PA_LODDUR(CONVOLUTION_POINTS),PA_PEAK,
     +         LODDUR_SLOPE,LODDUR_INTERCEPT,SEAS_HOURS,PA_DX,
     +         BUYBACK_ENERGY,R_PA_HOURLY_LOADS(*)
      REAL (kind=8) ::    SYSTEM_DEMAND,PA_DEMAND,PA_DEMAND_TEST,
     +         SYSTEM_AVE_ENERGY,PA_AVE_ENERGY
      INTEGER (kind=2) ::    I
      INTEGER (kind=2) ::    PA_CLASS/6/
      INTEGER (kind=2) ::    ISEAS,IMAX,IREC,ALINE_LOAD_DATA,
     +            DAYS_IN_MONTH,CURRENT_HR,LOCAL_HOURS,
     +            DA,LDE_DAY,LDE_MONTH,LDE_YEAR,DAY_WEEK,HR
      CHARACTER (len=8) ::  EEICODE
      INTEGER (kind=2) ::  TIMZON,TEMPER,DELTMP,LOCAL_LOAD_POINTS
      PARAMETER (LOCAL_LOAD_POINTS=79)
      REAL (kind=4) ::  MONTHLY_PA_LOADS(:),PA_HIST_BASE,PA_HIST_PEAK,
     +         LOCAL_LPROB(LOCAL_LOAD_POINTS),
     +         LOCAL_LODDUR(LOCAL_LOAD_POINTS)
...
!
      ...
      INTEGER (kind=2) ::  I,HR,IMAX,HOURS_INCREMENT,IPEAK,COUNT,
     +            INTERVALS,COUNTER,POINTS_IN_CURVE,
     +            R_LOAD_POINTS,R_MAX_TRANS_LOAD_GROUPS,
...
!
      ...
      REAL (kind=4) ::    R_RESERVE_CAPACITY,R_UNUSED_SUPP_CAPACITY,
     +         R_SUPPLEMENTAL_CAPACITY,
     +         LAST_YEAR_CPL_N_PA_CAP/0./,SEPA_CAPACITY,
     +         RESERVE_PERCENT,R_RESERVE_PERCENT,
     +         THIS_YEAR_PEAK,
     +         THIS_YEAR_CPL_N_PA_CAP/0./,
     +         SAVE_UNUSED_SUPP_CAPACITY/0./,
     +         PA_RETAINED_CAPACITY/0./,
     +         LAST_YEAR_PEAK,SAVE_RESERVE_CAPACITY/0./,
     +         R_ANN_RESERVE_CAPACITY,
     +         R_PA_EXP_CAPSYS,R_PA_CAPSYS,
     +         R_PA_PEAK,PA_FIRM_PURCHASE,
     +         R_MAINTENANCE_RATE(*),R_EAVAIL(*),
     +         PA_EXP_RETAINED_CAPACITY,UPDATE_NET_PLANNING_PEAK,
...
!
      ...
      INTEGER (kind=2) ::    ROUTINE_BASE_YEAR/1995/,YEAR_INDEX,
     +            R_PA_UNIT_INDEX(*),RESOURCE_NO
      REAL (kind=4) ::    R_PA_RETAINED_CAPACITY
      REAL (kind=4) ::    R_PA_EXP_RETAINED_CAPACITY,
     +         BRUNS1_RETAINED/144.8/,BRUNS2_RETAINED/144.8/,
     +         HARRIS1_RETAINED,ROX4_RETAINED/90.6/,
     +         MAYO1_RETAINED,R_MAINTENANCE_RATE(*),R_EAVAIL(*),
     +         RESOURCE_PERCENT
...
!
         ...
         INTEGER (kind=2) ::    CPL_MON_SALE_GEN_HEADER,R_YEAR,
     +               EMC_CLASS/5/,VOID_I2,GET_CL_ENERGY_BY_TYPE
         REAL (kind=4) ::
