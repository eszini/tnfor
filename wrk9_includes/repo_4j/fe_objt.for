...
!
     ...
     +                               (RESERVED,I=1,5),
     +                               (EMISSIONS_RATES(I),I=1,3),
     +                               RESERVED,
     +                               (EMISSIONS_RATES(I),I=4,6),
     +                               DESC,COMMENT,
     +                               (EMISSIONS_RATES(I),I=7,16)
               IF(IOS /= 0) THEN
     ...
     +                                                   ALL_VERSIONS,1)
               ...
               ENDIF
               IREC = IREC + 1
               WRITE(11,REC=IREC) DELETE,FUEL_ID,
     +                            EMISSIONS_RATES,DESC
            ENDDO
            IF(IOS /= 0) EXIT
         ENDDO
         CLOSE(10)
...
!***********************************************************************
      ...
      ENDIF
      DATA_DRIVE = OUTPUT_DIRECTORY()
       file_name=get_makeovl_filename(data_drive, "FEO", 
     + OVERLAY_FAMILY_NAME)
      OPEN(10,FILE=FILE_NAME)
      READ(10,*) DELETE
      INUNIT = 12
      ...
      IF(FUEL_EMISSIONS_FILE_OL == 'BC') THEN
         OPEN(11,FILE=trim(DATA_DRIVE)//"BCFUELEM.BIN",RECL=LRECL,
     +                                 ACCESS="DIRECT",STATUS="UNKNOWN")
         INUNIT = 11
      ENDIF
      OPEN(12,FILE=trim(DATA_DRIVE)//"OLFUELEM.BIN",ACCESS="DIRECT",
     +                                      STATUS="UNKNOWN",RECL=LRECL)
      ...
      DO
         DO
            ...
            IF(RECLN(1:1) == '7') EXIT
            IREC = IREC + 1
            READ(INUNIT,REC=IREC,IOSTAT=IOS_BASE) DELETE,FUEL_ID,
     +                                            EMISSIONS_RATES,DESC
            IF(IOS_BASE /= 0) EXIT
            IF(IOS == 0) THEN
               RECLN = trim(RECLN)//',,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,'
     ...
     +                                                   ALL_VERSIONS,1)
               ...
               ENDIF
            ENDIF
            WRITE(12,REC=IREC) DELETE,FUEL_ID,
     +                         EMISSIONS_RATES,DESC
         ENDDO
         IF(IOS_BASE /= 0) EXIT
      ENDDO
...
!
!***********************************************************************
      ENTRY OPEN_FUEL_EMISSIONS_FILE(R_UNIT_NUM,
     +                               R_EMISSIONS_RATES_FILE_ACTIVE)
!***********************************************************************
         UNIT_NUM = R_UNIT_NUM
         R_EMISSIONS_RATES_FILE_ACTIVE = FILE_EXISTS
         IF(FILE_EXISTS) OPEN(UNIT_NUM,FILE=trim(OUTPUT_DIRECTORY())//
     +             FUEL_EMISSIONS_FILE_OL//"FUELEM.BIN",ACCESS="DIRECT",
     +                                      STATUS="UNKNOWN",RECL=LRECL)
      RETURN
!
!***********************************************************************
...
!      
     ...
     +      EMISS_FUEL_EMIS_REPORT(:,:),
     +      ANNUAL_EMIS_REPORT_GROUPS(MAX_EMISSION_REPORT_GROUPS),
     +      R_ANNUAL_EMIS_REPORT_GROUPS(*),RTEMP,GET_VAR,
     +      TONS_CONVERSION,GET_TONS_CONVERSION,EMISSION_RATES(:,:)
...
!
      ...
      ALLOCATE( EMISS_FUEL_EMIS_REPORT(MAX_EMISSION_REPORT_GROUPS,
     +                                                          NUNITS))
!
...
!
            IF(IREC == 0) THEN 
               WRITE(4,*) "The emissions rates pointer", 
...
!
         ...
         DO I = 1, R_RECORDS ! NOTE: IREC <= R_RECORDS
            READ(10,REC=IREC,IOSTAT=IOS) DELETE,FUEL_ID,
     +                (EMISSION_RATES(IREC,J),J=1,TOTAL_EMISSION_GROUPS) ! SAVE EMISSION_RATES
            IF(IOS .NE. 0) EXIT
            ...
            DO J = 1, NUNITS
               ...
               DO I = 1, MAX_EMISSION_REPORT_GROUPS
                  ...
                  IF(P_EMIS_REPORT(I,J) /= 0.) THEN
                     ...
                     ELSE
                        RTEMP = 
     +                        GET_VAR(P_EMIS_REPORT(I,J),YEAR,FUEL_TYPE)
                     ENDIF
                  ELSEIF(S_EMIS_REPORT(I,J) /= 0.) THEN
                     ...
                     ELSE
                        RTEMP = 
     +                        GET_VAR(S_EMIS_REPORT(I,J),YEAR,FUEL_TYPE)
                     ENDIF
                  ELSEIF(EMISS_FUEL_EMIS_REPORT(I,J) /= 0.) THEN
                     ...
                     ELSE
                        RTEMP = 
     +                     GET_VAR(
     +                       EMISS_FUEL_EMIS_REPORT(I,J),YEAR,FUEL_TYPE)
                     ENDIF
                  ELSE
                     RTEMP = 0.
                  ENDIF
                  ANNUAL_EMIS_REPORT_GROUPS(I) = 
     +                              ANNUAL_EMIS_REPORT_GROUPS(I) + RTEMP
               ENDDO
            ENDDO
         ENDIF
      RETURN
...
!         
         DO I = 1, MAX_EMISSION_REPORT_GROUPS
            EMIS_UNITS(I) = 'BTU'
         ENDDO
         IF(UTILITY_TYPE() == 'T' .AND. 
     +                              MAX_EMISSION_REPORT_GROUPS > 9) THEN
...
!      
!***********************************************************************
      ENTRY CALC_EMISSION_REPORT_GROUPS(  R_ISEAS,R_UNIT_NO,
     +                                    R_PRIM_UNIT_HEAT,
     +                                    R_SEC_UNIT_HEAT,
     +                                    R_EMIS_UNIT_HEAT,R_UNIT_ENRG)
!***********************************************************************
         IF(EMIS_REPORT_GROUPS_NOT_USED) RETURN
         IF(USING_MWH_CONVERSIONS) 
     +      TOTAL_HEAT = MAX(1., R_PRIM_UNIT_HEAT + 
     +                               R_SEC_UNIT_HEAT + R_EMIS_UNIT_HEAT)
         IF(R_PRIM_UNIT_HEAT > 0.) THEN
            DO I = 1 , MAX_EMISSION_REPORT_GROUPS
               ...
               ELSE
                  RTEMP = 
     +                GET_VAR(P_EMIS_REPORT(I,R_UNIT_NO),YEAR,FUEL_TYPE)
               ...
               ELSEIF(EMIS_UNITS(I) == 'MWH') THEN
                  CONTRIBUTION = RTEMP*R_PRIM_UNIT_HEAT*
     +                                            R_UNIT_ENRG/TOTAL_HEAT
               ELSE
                  CONTRIBUTION = 0.
               ENDIF
               ANNUAL_EMIS_REPORT_GROUPS(I) = 
     +                       ANNUAL_EMIS_REPORT_GROUPS(I) + CONTRIBUTION
            ENDDO
         ENDIF
         IF(R_SEC_UNIT_HEAT > 0.) THEN
            DO I = 1 , MAX_EMISSION_REPORT_GROUPS
               ...
               ELSE
                  RTEMP = 
     +                GET_VAR(S_EMIS_REPORT(I,R_UNIT_NO),YEAR,FUEL_TYPE)
                  IF(RTEMP < 0.) THEN ! 11/6/97. GAT.
                     RTEMP = GET_VAR(RTEMP,R_ISEAS,FUEL_TYPE)
                  ENDIF
               ENDIF
     ...
     +                       ANNUAL_EMIS_REPORT_GROUPS(I) + CONTRIBUTION
         ...
         IF(R_EMIS_UNIT_HEAT > 0.) THEN
            DO I = 1 , MAX_EMISSION_REPORT_GROUPS
               ...
               ELSE
                  RTEMP = GET_VAR(
     +               EMISS_FUEL_EMIS_REPORT(I,R_UNIT_NO),YEAR,FUEL_TYPE)
                  IF(RTEMP < 0.) THEN ! 11/6/97. GAT.
                     RTEMP = GET_VAR(RTEMP,R_ISEAS,FUEL_TYPE)
                  ENDIF
               ENDIF
...
!***********************************************************************
         DO I = 1 , MAX_EMISSION_REPORT_GROUPS
            IF(EMIS_UNITS(I) == 'BTU')  THEN
               R_ANNUAL_EMIS_REPORT_GROUPS(I) = 
     +                    ANNUAL_EMIS_REPORT_GROUPS(I)/TONS_CONVERSION
            ELSE
               R_ANNUAL_EMIS_REPORT_GROUPS(I) = 
     +                                      ANNUAL_EMIS_REPORT_GROUPS(I)
            ENDIF
