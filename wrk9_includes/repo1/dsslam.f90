...
!
      ...
      LOGICAL (kind=1) ::  CLASS_EXISTS(MAX_LOAD_CLASSES),LDMGT_ACTIVE,R_LDMGT_ACTIVE
      INTEGER (kind=2) ::  PSMO,PEMO,ISEAS,HOURS,HOURS_INCREMENT,PRODUCTION_PERIODS,I
...
!
      INTEGER (kind=2) ::  HOURS_IN_PERIOD,SYSTEM_POINTER,CLASS,SEASON,DAY,HOUR
      REAL ::  PEAK_LOADS(2,12),NUMBER_OF_DAYS
      REAL ::  FRCLOD(:)
      ALLOCATABLE :: FRCLOD
      REAL ::  MONTHLY_ENERGY(12)
      REAL ::  PEAK_BEFORE_DSM(12),BASE_BEFORE_DSM(12),PEAK_AFTER_DSM(12),BASE_AFTER_DSM(12)
      ...
      LOGICAL (kind=1) ::  CLASS_BASED_FORECAST,POOLING_TRANSACTIONS,CONTROL_AREA_FORECAST,SYSTEM_BASED_FORECAST
...
!
         DO SEASON = 1, 12
            DO DAY = 1, 2
               PEAK_LOADS(DAY,SEASON) = SYSTEM_HISTORICAL_PEAKS(HISTORICAL_PEAK_HOUR(DAY,SEASON),DAY,SEASON)
            ENDDO
         ENDDO
!
...
!
      IF(SYSTEM_FORECAST_DATA) THEN
         IF(SYSTEM_BASED_FORECAST()) THEN
            CALL RETURN_ENERGY_PEAK_SYSTEM(SYSTEM_FORECAST_ENERGY,SYSTEM_FORECAST_PEAK)
         ELSE
            CALL RETURN_ENERGY_PEAK_CLASS(SYSTEM_FORECAST_ENERGY,SYSTEM_FORECAST_PEAK)
         ENDIF
         ...
         DO SEASON = 1 , 12
            DO DAY = 1 , 3
               ...
               ELSE
                  ...
                  IF(DAY == 1) NUMBER_OF_DAYS = HIST_DAY_COUNT - 4 + WEEKEND_PEAK_DAYS(SEASON)
                  IF(DAY == 2) NUMBER_OF_DAYS = HIST_DAY_COUNT - WEEKEND_PEAK_DAYS(SEASON)
               ENDIF
               SUM = 0.
               DO HOUR = 1, 24
...
!
      ...
      INTEGER (kind=2) ::  HR,CLASS,SEASON,DAY_TYPE,BASE_YEAR,HDYCNT(2,12),END_POINT,YEAR,LOCAL_BASE_YEAR
      REAL ::  HIST_ENERGY_DAILY,FUTURE_ENERGY_DAILY,HIST_ENERGY_MONTHLY,FUTURE_ENERGY_MONTHLY,HIST_DAYS(4,12)
      REAL ::  FORECAST_LOADS(24,3,12,*), &
               SYSLOD(24,3,12), &
               CLSLOD(24,3,12,*)
      CHARACTER (len=20) ::  LOADS_MONTH_NAME*9,WRITE_MONTH_NAME(12)*9,DAY_TITLE(4)*8
      CHARACTER (len=15) ::  CLASS_NAME(7)
!
      INTEGER (kind=2) ::  FORECASTED_DAYS_REPORT_HEADER,FORE_DAYS_NO,FORE_MONTH_NO,FORE_MONTH_REPORT_HEADER
      INTEGER ::  FORE_DAYS_REC,FORE_MONTH_REC
      SAVE FORE_DAYS_REC,FORE_MONTH_REC
      LOGICAL (kind=1) ::  FORECASTED_DAYS_NOT_OPEN=.TRUE.,FIRST_CALL_FOR_THIS_END_POINT
      ...
      IF(FIRST_CALL_FOR_THIS_END_POINT) THEN
         DO SEASON = 1 , 12
            ...
            DO DAY_TYPE = 1 , 3
               ...
               DO HR = 1 , 24
                  HIST_ENERGY_DAILY = HIST_ENERGY_DAILY + SYSLOD(HR,DAY_TYPE,SEASON)
               ENDDO
!
...
!
               WRITE(FORE_MONTH_NO,REC=FORE_MONTH_REC) PRT_ENDPOINT(), &
                     ...
                     HIST_ENERGY_DAILY,HIST_ENERGY_DAILY*HIST_DAYS(DAY_TYPE,SEASON), &
                     FLOAT(PKPEAK(DAY_TYPE,SEASON))
               FORE_MONTH_REC = FORE_MONTH_REC + 1
               HIST_ENERGY_MONTHLY = HIST_ENERGY_MONTHLY + HIST_DAYS(DAY_TYPE,SEASON)*HIST_ENERGY_DAILY
!
            ENDDO
!
            WRITE(FORE_DAYS_NO,REC=FORE_DAYS_REC) PRT_ENDPOINT(), &
...
!
            ...
            DO DAY_TYPE = 1 , 3
               ...
               DO HR = 1 , 24
!
                  FUTURE_ENERGY_DAILY = FUTURE_ENERGY_DAILY + FORECAST_LOADS(HR,DAY_TYPE,SEASON,CLASS)
!
               ENDDO
!
               WRITE(FORE_DAYS_NO,REC=FORE_DAYS_REC) PRT_ENDPOINT(), &
...
!
               WRITE(FORE_MONTH_NO,REC=FORE_MONTH_REC) PRT_ENDPOINT(), &
                     ...
                     WRITE_MONTH_NAME(SEASON),DAY_TITLE(DAY_TYPE), &
                     HIST_DAYS(DAY_TYPE,SEASON), &
                     FUTURE_ENERGY_DAILY,FUTURE_ENERGY_DAILY*HIST_DAYS(DAY_TYPE,SEASON), &
                     FUT_PEAK(DAY_TYPE,SEASON)
               FORE_MONTH_REC = FORE_MONTH_REC + 1
!
               FUTURE_ENERGY_MONTHLY = FUTURE_ENERGY_MONTHLY + FUTURE_ENERGY_DAILY*HIST_DAYS(DAY_TYPE,SEASON)
!
            ENDDO
!
            WRITE(FORE_DAYS_NO,REC=FORE_DAYS_REC) PRT_ENDPOINT(), &
