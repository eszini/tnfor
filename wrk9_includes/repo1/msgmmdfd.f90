...
!
      ...
      INTEGER(kind=2) :: NUMBER_OF_BC_DEBIT_CLASSES=0,MAX_BC_DEBIT_CLASS_ID_NUM=0
      INTEGER(kind=2) :: NUMBER_OF_OL_DEBIT_CLASSES=0,MAX_OL_DEBIT_CLASS_ID_NUM=0
      INTEGER(kind=2) :: UNIT_NUM=10,ASSET_CLASS_NUM,ASSET_CLASS_VECTOR
      INTEGER(kind=2) :: R_NUM_OF_DEBIT_CLASSES,R_MAX_DEBIT_CLASS_NUM,R_DEBIT_CLASS_POINTERS(*)
      ...
      CHARACTER(len=1) :: TAX_TIMING_CLASSIFICATION,ACCOUNT_ACTIVE,DEFERRAL_TYPE
...
! DECLARATION FOR /DEBIT FILE/
      ...
      REAL(kind=4) :: DEFERRED_TAX_ALLOCATION,CURRENT_TAX_DEDUCTION_PERCENT
...
!***********************************************************************
      ...
      FILE_NAME = trim(BASE_FILE_DIRECTORY())//"DDB"//trim(BASE_FILE_NAME)//".DAT"
      ...
      IF(FILE_EXISTS) THEN
         ...
         OPEN(11,FILE=trim(DATA_DRIVE)//"BCDEBIT.BIN",ACCESS="DIRECT",STATUS="UNKNOWN",RECL=LRECL)
         IREC = 0
         READ(10,*) DELETE
         DO
            ASSET_CLASS_NUM = 0
...
!***********************************************************************
      ...
      IF(DEBITOL == 'BC') THEN
         ...
         INUNIT = 11
      ENDIF
      OPEN(12,FILE=trim(DATA_DRIVE)//"OLDEBIT.BIN",ACCESS="DIRECT",STATUS="UNKNOWN",RECL=LRECL)
      ALLOCATE(TEMP_ASSET_CLASS_POINTER(1024))
      TEMP_ASSET_CLASS_POINTER = 0
      ...
      DEBITOL = 'OL'
      IF(ALLOCATED(DEBIT_OL_ASSET_CLASS_POINTER))DEALLOCATE(DEBIT_OL_ASSET_CLASS_POINTER)
...
!***********************************************************************
         OPEN(R_UNIT_NUM,FILE=trim(OUTPUT_DIRECTORY())//'BC_DEBIT.BIN',FORM='UNFORMATTED')
      RETURN
!***********************************************************************
      ENTRY OPEN_DD_OUT_FILE(R_UNIT_NUM)
!***********************************************************************
         OPEN(R_UNIT_NUM,FILE=trim(OUTPUT_DIRECTORY())//DEBITOL//'DD_AST.BIN',ACCESS='DIRECT',RECL=32)
      RETURN
!
!***********************************************************************
...
!
      ...
      INTEGER(kind=2) :: T,IREC,TACCTS,DELETE,YR,MO,TACCTS_CR,TACCTS_DR,ACTIVE_ACCOUNTS
      INTEGER(kind=4) :: IOS
!     DECLARATION FOR /FAINPT/
      REAL(kind=4) :: NET_DD,WOYRS,CASH_ADDITONS
      REAL(kind=4) :: INT_DEBAMT,INT_CAMORT
      REAL(kind=4) :: VECTOR_VALUES(AVAIL_DATA_YEARS),DEFERRED_ADDITONS(AVAIL_DATA_YEARS)
...
!
      ...
      CHARACTER(len=1)::TAX_TIMING_CLASSIFICATION,DESC*35,INDEX_NAME*34,ACCOUNT_ACTIVE,DEFERRAL_TYPE
      CHARACTER(len=8) :: BALANCE_SHEET_TREATMENT,TEMP_STR
      CHARACTER(len=20) :: WVPA_TRACKING_TYPE
      LOGICAL(kind=1) :: R_CLASS_EXISTS,CPL_IS_ACTIVE
      INTEGER(kind=2) :: RUN_YEARS,EXTENSION_YEARS,ASSET_CLASS_VECTOR,ASSET_CLASS,BOOKED_YEAR
...
!
      ...
      LOGICAL(kind=1) :: REPORTS_ACTIVE,DEBIT_REPORT,REPORT_ALL_ACCOUNTS,REPORT_HEADER_OPEN=.FALSE.
      INTEGER(kind=2) :: REPORTING_UNIT,DEBIT_FILE_RPT_HEADER,ASSET_CLASS_ID,DEACTIVE_YR
      ...
      REAL(kind=4) :: HALF_MONTH_AMORTIZATION,MONTHLY_AMORTIZATION_LOC,CURRENT_MONTH_AMORTIZATION,ANNUAL_AMOUNT
...
!
      LF95 = LAHEY_LF95()
      REPORTS_ACTIVE = DEBIT_REPORT(REPORT_ALL_ACCOUNTS) .AND. .NOT. SAVE_BASE_CASE
...
!
         IF(MONTH_DEBIT_BOOKED == 0 .OR. MONTH_DEBIT_BOOKED > 12) MONTH_DEBIT_BOOKED = 7
!
         IF(AMORT_DELAY_PERIOD >= 1900) THEN
            AMORT_DELAY_PERIOD = MAX(0.,AMORT_DELAY_PERIOD-BASE_YEAR-BOOKED_YEAR,REAL(BOOKED_YEAR))
         ELSE
            ...
            ELSE
               DO YR = 1, FINANCIAL_SIMULATION_YEARS
                  IF(YR > AVAIL_DATA_YEARS) THEN
                     ANN_CASH_ADDITIONS(0,YR) = DEFERRED_ADDITONS(AVAIL_DATA_YEARS)
                  ELSE
                     ANN_CASH_ADDITIONS(0,YR) = DEFERRED_ADDITONS(YR)
                  ENDIF
                  ANN_CASH_ADDITIONS(MONTH_DEBIT_BOOKED,YR) = ANN_CASH_ADDITIONS(0,YR)
               ENDDO
            ENDIF
...
!
         ...
         ANN_AMORT(0,0) = INT_CAMORT
!
         IF(AMORT_DATA_TYPE == 'Y' .AND. AMORT_DELAY_PERIOD  <= FINANCIAL_SIMULATION_YEARS) THEN
          ...
          START_AMORT_FULL_YR=MAX(INT(1,2),YR+INT(AMORT_DELAY_PERIOD,2))
                  ...
                  ENDIF
                     MONTHLY_AMORTIZATION_LOC = ANN_CASH_ADDITIONS(MO,YR)/WO_MONTHS
                  NET_DD = ANN_CASH_ADDITIONS(MO,YR)
                  DO YR2 = START_AMORT_FULL_YR,FINANCIAL_SIMULATION_YEARS
                     ...
                     DO MO2 = START_AMORT_MONTH, 12
                        ...
                        ELSE
                           ...
                           TAKE_FULL_MONTH = .TRUE.
                        ENDIF
                        ANNUAL_AMOUNT = ANNUAL_AMOUNT + CURRENT_MONTH_AMORTIZATION
                        NET_DD = NET_DD - CURRENT_MONTH_AMORTIZATION
                        IF(ABS(NET_DD) < .00001) EXIT
                     ENDDO
                     START_AMORT_MONTH = 1
                     MONTHLY_AMORT_AMOUNT(0,YR2) = ANNUAL_AMOUNT + MONTHLY_AMORT_AMOUNT(0,YR2)
                     IF(ABS(NET_DD) < .00001) EXIT
                  ENDDO
               ENDDO
            ENDDO
...
!
         IF(INCOME_TREATMENT == INTEREST .AND. trim(BALANCE_SHEET_TREATMENT) == ' ') THEN
...
!
                  DO YR = 1, AVAIL_DATA_YEARS
                     ...
                     DO MO = 0, 12
                        IF(MO == 0) THEN
                           ALLOCATION_VALUE(YR,MO) = ANNUAL_VECTOR_VALUES(YR)/100.
                        ELSEIF(YR > LAST_AVAILABLE_MONTHLY_YEAR) THEN
                           ALLOCATION_VALUE(YR,MO) = ANNUAL_VECTOR_VALUES(YR)/100.
                        ELSE
                           ALLOCATION_VALUE(YR,MO) = VECTOR_MONTHLY_DATA(MO,YR)/100.
                        ENDIF
                     ENDDO
                  ENDDO
               ELSE
...
!
            ...
            LGE_NET_DD = INT_DEBAMT - INT_CAMORT
               ...
               TDDB(12,0,ASSET_CLASS,BAL_SHEET_POS,ACCT_TYPE) = &
                         TDDB(12,0,ASSET_CLASS,BAL_SHEET_POS,ACCT_TYPE) &
                         + GROSS_DD
               TDDB(12,0,ASSET_CLASS,Total_Deferred_Debits,ACCT_TYPE) = &
                 TDDB(12,0,ASSET_CLASS,Total_Deferred_Debits,ACCT_TYPE) &
                 + GROSS_DD
               TNDB(12,0,ASSET_CLASS,BAL_SHEET_POS,ACCT_TYPE) = &
                          TNDB(12,0,ASSET_CLASS,BAL_SHEET_POS,ACCT_TYPE) &
                          + NET_DD
               TNDB(12,0,ASSET_CLASS,Total_Deferred_Debits,ACCT_TYPE) = &
                  TNDB(12,0,ASSET_CLASS,Total_Deferred_Debits,ACCT_TYPE) &
                  + NET_DD
               TNDB(0,1,ASSET_CLASS,BAL_SHEET_POS,ACCT_TYPE) = &
                          TNDB(0,1,ASSET_CLASS,BAL_SHEET_POS,ACCT_TYPE) &
                          + NET_DD
               TNDB(0,1,ASSET_CLASS,Total_Deferred_Debits,ACCT_TYPE) = &
                  TNDB(0,1,ASSET_CLASS,Total_Deferred_Debits,ACCT_TYPE) &
                  + NET_DD
!            ENDIF
            IF(IN_RATEBASE) THEN
               TDDRBB(12,0,ASSET_CLASS,ACCT_TYPE) = TDDRBB(12,0,ASSET_CLASS,ACCT_TYPE) + NET_DD
               RB_DD_AMRTE(12,0,ASSET_CLASS,ACCT_TYPE) = AMORT + RB_DD_AMRTE(12,0,ASSET_CLASS,ACCT_TYPE)
            ENDIF
!
            START_MO = 12
...
!    
               ...
               IF(IN_RATEBASE) THEN
                  TDDRBB(0,YR,ASSET_CLASS,ACCT_TYPE) = TDDRBB(12,YR-1,ASSET_CLASS,ACCT_TYPE)
               ENDIF
               DO MO = 1, 12
...
!
                  TDDB(MO,YR,ASSET_CLASS,BAL_SHEET_POS,ACCT_TYPE) = &
                                   TDDB(MO,YR,ASSET_CLASS,BAL_SHEET_POS, &
                                                              ACCT_TYPE) &
                                   + GROSS_DD
                  TDDB(MO,YR,ASSET_CLASS,Total_Deferred_Debits,ACCT_TYPE) = &
                           TDDB(MO,YR,ASSET_CLASS,Total_Deferred_Debits, &
                                                              ACCT_TYPE) &
                           + GROSS_DD
                  IF(INCOME_TREATMENT == BTL) THEN
                  ...
                  ELSE
                     ...
                     IF(INCOME_TREATMENT == INTEREST) THEN
                        ...
                        INTEREST_AMORTIZATION(MO,YR,ASSET_CLASS,ACCT_TYPE) + AMORT
!
                     ELSEIF(INCOME_TREATMENT == O_and_M) THEN
                     ...
                     ELSEIF(INCOME_TREATMENT == PURCHASE_POWER) THEN
                        ...
                        PURCHASE_POWER_AMORT(MO,YR,ASSET_CLASS,ACCT_TYPE) + AMORT
                     ELSE   
                        ATL_AMORTIZATION(MO,YR,ASSET_CLASS,ACCT_TYPE) = &
                        ATL_AMORTIZATION(MO,YR,ASSET_CLASS,ACCT_TYPE) + AMORT
                        ATL_AMORT_SUB_ITEMS(MO,YR,ASSET_CLASS,BAL_SHEET_POS,ACCT_TYPE) = &
                             + ATL_AMORT_SUB_ITEMS(MO,YR,ASSET_CLASS,BAL_SHEET_POS,ACCT_TYPE) &
                             + AMORT
                     ENDIF
                  ENDIF
                  IF(TAX_TIMING_CLASSIFICATION == Temporary) THEN
                     IF(INCOME_TREATMENT == BTL) THEN
                        TAX_TIMING_DIFFERENCES(YR,ASSET_CLASS,2,2,ACCT_TYPE) = &
                        TAX_TIMING_DIFFERENCES(YR,ASSET_CLASS,2,2,ACCT_TYPE) + AMORT
                     ELSE
                        TAX_TIMING_DIFFERENCES(YR,ASSET_CLASS,2,1,ACCT_TYPE) = &
                        TAX_TIMING_DIFFERENCES(YR,ASSET_CLASS,2,1,ACCT_TYPE) + AMORT
                     ENDIF
                  ELSEIF(TAX_TIMING_CLASSIFICATION == Permanent) THEN
                     IF(INCOME_TREATMENT == BTL) THEN
                        TAX_TIMING_DIFFERENCES(YR,ASSET_CLASS,1,2,ACCT_TYPE) = &
                        TAX_TIMING_DIFFERENCES(YR,ASSET_CLASS,1,2,ACCT_TYPE) + AMORT
                     ELSE
                        TAX_TIMING_DIFFERENCES(YR,ASSET_CLASS,1,1,ACCT_TYPE) = &
                        TAX_TIMING_DIFFERENCES(YR,ASSET_CLASS,1,1,ACCT_TYPE) + AMORT
                     ENDIF
                  ENDIF
                  ...
                  TNDB(MO,YR,ASSET_CLASS,Total_Deferred_Debits,ACCT_TYPE) = &
                          TNDB(MO,YR,ASSET_CLASS,Total_Deferred_Debits,ACCT_TYPE) &
                          + NET_DD
                  IF(IN_RATEBASE) THEN
...
!
       ...
       DO ASSET_CLASS = 0, NUM_OF_ASSET_CLASSES
         TCAMRT(MO,1,ASSET_CLASS,ACCT_TYPE) = TAMRTE(MO,1,ASSET_CLASS,ACCT_TYPE)
         DO YR = 1, FINANCIAL_SIMULATION_YEARS
...
!
        ...
        DO ACCT_TYPE = 1, 2
         IF(ACCT_TYPE == 1 .AND. TACCTS_DR == 0) CYCLE
         IF(ACCT_TYPE == 2 .AND. TACCTS_CR == 0) CYCLE
         DEBIT_OPENING_NET_DD = TNDB(12,0,0,Total_Deferred_Debits,ACCT_TYPE)        
         INTEREST_OPENING_NET_DD = 0. ! TNDB(12,0,0,
         DO YR = 1, FINANCIAL_SIMULATION_YEARS
!
            DEBIT_CLOSING_NET_DD = TNDB(12,YR,0,Total_Deferred_Debits,ACCT_TYPE)        
...
!
            WRITE(REPORTING_UNIT,REC=REPORTING_REC) PRT_ENDPOINT(), &
                           ...
                           BTL_AMORTIZATION(MO,YR,0,ACCT_TYPE), &
                           DEBIT_CLOSING_NET_DD, &
                          (TDDRBB(12,YR-1,0,ACCT_TYPE)+TDDRBB(12,YR,0,ACCT_TYPE))/2.,ATL_DEF_TAX(MO,YR,0,ACCT_TYPE), &
                           BTL_DEF_TAX(MO,YR,0,ACCT_TYPE), &
                           ATL_CURRENT_TAX_EXPENSE(MO,YR,0,ACCT_TYPE), &
                           BTL_CURRENT_TAX_EXPENSE(MO,YR,0,ACCT_TYPE), &
                           VARIABLE_OM_AMORT(MO,YR,0,ACCT_TYPE), &
                           ...
                           OTHER_OM_AMORT(MO,YR,0,ACCT_TYPE), &
                           PURCHASE_POWER_AMORT(MO,YR,0,ACCT_TYPE), &
                          (ATL_AMORT_SUB_ITEMS(MO,YR,0,B_POS,ACCT_TYPE),B_POS = 1, BAL_SHEET_OPTIONS)
            REPORTING_REC = REPORTING_REC + 1
!
...
!***********************************************************************
         MO = 0
         R_TDDB = TDDB(MO,R_YR,0,Total_Deferred_Debits,DEBIT) 
         R_TDDRBB = TDDRBB(MO,R_YR,0,DEBIT)
         R_TAMRTE = TAMRTE(MO,R_YR,0,DEBIT)
         R_TNDB = TNDB(MO,R_YR,0,Total_Deferred_Debits,DEBIT)
...
!***********************************************************************
         ...
         IF(R_CLASS <= MAX_ASSET_CLASS_NUM) THEN
            ...
            IF(ASSET_CLASS > 0 .OR. R_CLASS == 0) THEN 
               R_TNDB=TNDB(MO,0,ASSET_CLASS,Total_Deferred_Debits,DEBIT)
               R_OTHER_DD_BALANCE = TNDB(MO,0,ASSET_CLASS,Other_Deferred_Debits,DEBIT)
               R_GOODWILL = TNDB(MO,0,ASSET_CLASS,Goodwill,DEBIT)
               R_CLASS_REGULATORY_ASSETS = TNDB(MO,0,ASSET_CLASS,RegulatoryAssets,DEBIT)
               R_FASB_109 = TNDB(MO,0,ASSET_CLASS,FASB109,DEBIT)
               R_FASB_133 = TNDB(MO,0,ASSET_CLASS,FASB133,DEBIT)
               R_TDDRBB = TDDRBB(MO,0,ASSET_CLASS,DEBIT)
               R_UNAMORTIZED_INTEREST_BALANCE = TNDB(MO,0,ASSET_CLASS,Unamortized_InterestBalance,DEBIT)
               UNAMORTIZED_ISSUE_EXP_BALANCE = TNDB(MO,0,ASSET_CLASS,Unamortized_Issue_Expen_Balance,DEBIT)
!
            ENDIF
         ENDIF
      RETURN
...
!
         ...
         IF(R_CLASS <= MAX_ASSET_CLASS_NUM) THEN
            ...
            ENDIF
            IF(ASSET_CLASS > 0 .OR. R_CLASS == 0) THEN 
               R_OTHER_DEF_CREDITS = TNDB(MO,0,ASSET_CLASS,Other_Deferred_Credit,CREDIT)
               R_RegulatoryLiabilites = TNDB(MO,0,ASSET_CLASS,Regulatory_Liabilites,CREDIT)
               R_GainOnReaquiredDebt = TNDB(MO,0,ASSET_CLASS,Gain_on_Reaquired_Debt,CREDIT)
               R_AssetSalesDeferredGain = &
                           R_AssetSalesDeferredGain &
...
!***********************************************************************
      ENTRY DEBIT_INFO(R_YR,R_CLASS,R_CLASS_EXISTS, &
                       R_TDDB,R_TDDRBB,R_TAMRTE,R_TNDB,R_TCAMRT,R_RB_DD_AMRTE, &
                       R_INTEREST_AMORTIZATION,R_ATL_AMORTIZATION,R_BTL_AMORTIZATION, &
                       R_ATL_DEF_TAX,R_BTL_DEF_TAX,R_UNAMORTIZED_INTEREST_BALANCE, &
                       R_DEFERRED_CASH_ADDITIONS,R_VARIABLE_OM_AMORT,R_ATL_CURRENT_TAX_EXPENSE, &
                       R_BTL_CURRENT_TAX_EXPENSE,R_OTHER_OM_AMORT,R_PURCHASE_POWER_AMORT, &
                       R_OTHER_DD_BALANCE,R_GOODWILL,R_CLASS_REGULATORY_ASSETS,R_FASB_109,R_FASB_133, &
                       R_OTHER_DD_AMORT, &
                       R_GOODWILL_AMORT, &
                       R_CLASS_REGULATORY_ASSETS_AMORT, &
                       R_FASB_109_AMORT, &
...
!
               R_TDDB = TDDB(MO_BAL,YR_BAL,ASSET_CLASS,Total_Deferred_Debits,DEBIT)
               R_TDDRBB = R_TDDRBB + TDDRBB(MO_BAL,YR_BAL,ASSET_CLASS,DEBIT)
               R_TNDB = R_TNDB + TNDB(MO_BAL,YR_BAL,ASSET_CLASS,Total_Deferred_Debits,DEBIT)
               R_OTHER_DD_BALANCE = R_OTHER_DD_BALANCE + TNDB(MO_BAL,YR_BAL,ASSET_CLASS,Other_Deferred_Debits,DEBIT)
               R_GOODWILL=TNDB(MO_BAL,YR_BAL,ASSET_CLASS,Goodwill,DEBIT)
               R_CLASS_REGULATORY_ASSETS = TNDB(MO_BAL,YR_BAL,ASSET_CLASS,RegulatoryAssets,DEBIT)
               R_FASB_109=TNDB(MO_BAL,YR_BAL,ASSET_CLASS,FASB109,DEBIT)
               R_FASB_133=TNDB(MO_BAL,YR_BAL,ASSET_CLASS,FASB133,DEBIT)
!
! CREDIT BALANCES
!
               R_OTHER_DEF_CREDITS = TNDB(MO,YR_BAL,ASSET_CLASS,Other_Deferred_Credit,CREDIT)
               R_RegulatoryLiabilites = TNDB(MO,YR_BAL,ASSET_CLASS,Regulatory_Liabilites,CREDIT)
               R_GainOnReaquiredDebt = TNDB(MO,YR_BAL,ASSET_CLASS,Gain_on_Reaquired_Debt,CREDIT)
               R_AssetSalesDeferredGain = &
                           R_AssetSalesDeferredGain &
...
!
               R_OTHER_DD_AMORT = ATL_AMORT_SUB_ITEMS(MO,YR,ASSET_CLASS,Other_Deferred_Debits,DEBIT)
               R_GOODWILL_AMORT = ATL_AMORT_SUB_ITEMS(MO,YR,ASSET_CLASS,Goodwill,DEBIT)
               R_CLASS_REGULATORY_ASSETS_AMORT = ATL_AMORT_SUB_ITEMS(MO,YR,ASSET_CLASS,RegulatoryAssets,DEBIT)
               R_FASB_109_AMORT = ATL_AMORT_SUB_ITEMS(MO,YR,ASSET_CLASS,FASB109,DEBIT)
               R_FASB_133_AMORT = ATL_AMORT_SUB_ITEMS(MO,YR,ASSET_CLASS,FASB133,DEBIT)
               R_TCAMRT = TCAMRT(MO_BAL,YR_BAL,ASSET_CLASS,DEBIT)
               R_UNAMORTIZED_INTEREST_BALANCE = TNDB(MO_BAL,YR_BAL,ASSET_CLASS,Unamortized_InterestBalance,DEBIT)
               UNAMORTIZED_ISSUE_EXP_BALANCE = TNDB(MO_BAL,YR_BAL,ASSET_CLASS,Unamortized_Issue_Expen_Balance,DEBIT)
!
!
...
!
               ...
               R_INTEREST_AMORTIZATION = INTEREST_AMORTIZATION(MO,YR,ASSET_CLASS,DEBIT)
               R_ATL_AMORTIZATION = ATL_AMORTIZATION(MO,YR,ASSET_CLASS,DEBIT)
               R_BTL_AMORTIZATION = BTL_AMORTIZATION(MO,YR,ASSET_CLASS,DEBIT)
               R_ATL_DEF_TAX = ATL_DEF_TAX(MO,YR,ASSET_CLASS,DEBIT)
               R_BTL_DEF_TAX = BTL_DEF_TAX(MO,YR,ASSET_CLASS,DEBIT)
               R_DEFERRED_CASH_ADDITIONS = DEFERRED_CASH_ADDITIONS(MO,YR,ASSET_CLASS,DEBIT)
               R_VARIABLE_OM_AMORT = R_VARIABLE_OM_AMORT + VARIABLE_OM_AMORT(MO,YR,ASSET_CLASS,DEBIT)
               R_OTHER_OM_AMORT = R_OTHER_OM_AMORT + OTHER_OM_AMORT(MO,YR,ASSET_CLASS,DEBIT)
               R_PURCHASE_POWER_AMORT = R_PURCHASE_POWER_AMORT + PURCHASE_POWER_AMORT(MO,YR,ASSET_CLASS,DEBIT)
               R_ATL_CURRENT_TAX_EXPENSE = ATL_CURRENT_TAX_EXPENSE(MO,YR,ASSET_CLASS,DEBIT)
               R_BTL_CURRENT_TAX_EXPENSE = BTL_CURRENT_TAX_EXPENSE(MO,YR,ASSET_CLASS,DEBIT)
!
! ANNUAL TOTAL CREDIT VALUES
!
               R_RB_DD_AMRTE = R_RB_DD_AMRTE - RB_DD_AMRTE(MO,YR,ASSET_CLASS,CREDIT)
               R_TAMRTE = R_TAMRTE - TAMRTE(MO,YR,ASSET_CLASS,CREDIT)
               R_INTEREST_AMORTIZATION = R_INTEREST_AMORTIZATION - INTEREST_AMORTIZATION(MO,YR,ASSET_CLASS,CREDIT)
               R_ATL_AMORTIZATION = R_ATL_AMORTIZATION - ATL_AMORTIZATION(MO,YR,ASSET_CLASS,CREDIT)
               R_BTL_AMORTIZATION = R_BTL_AMORTIZATION - BTL_AMORTIZATION(MO,YR,ASSET_CLASS,CREDIT)
               R_ATL_DEF_TAX = R_ATL_DEF_TAX - ATL_DEF_TAX(MO,YR,ASSET_CLASS,CREDIT)
               R_BTL_DEF_TAX = R_BTL_DEF_TAX - BTL_DEF_TAX(MO,YR,ASSET_CLASS,CREDIT)
               R_DEFERRED_CASH_ADDITIONS = R_DEFERRED_CASH_ADDITIONS - DEFERRED_CASH_ADDITIONS(MO,YR,ASSET_CLASS,CREDIT)
               R_VARIABLE_OM_AMORT = R_VARIABLE_OM_AMORT - VARIABLE_OM_AMORT(MO,YR,ASSET_CLASS,CREDIT)
               R_OTHER_OM_AMORT = R_OTHER_OM_AMORT - OTHER_OM_AMORT(MO,YR,ASSET_CLASS,CREDIT)
               R_PURCHASE_POWER_AMORT = R_PURCHASE_POWER_AMORT - PURCHASE_POWER_AMORT(MO,YR,ASSET_CLASS,CREDIT)
               R_ATL_CURRENT_TAX_EXPENSE = R_ATL_CURRENT_TAX_EXPENSE - ATL_CURRENT_TAX_EXPENSE(MO,YR,ASSET_CLASS,CREDIT)
               R_BTL_CURRENT_TAX_EXPENSE = R_BTL_CURRENT_TAX_EXPENSE -  BTL_CURRENT_TAX_EXPENSE(MO,YR,ASSET_CLASS,CREDIT)
            ENDIF
         ENDIF
      RETURN
!***********************************************************************
...
!
         IF(R_CLASS <= MAX_ASSET_CLASS_NUM) THEN
            ...
            IF(ASSET_CLASS > 0 .OR. R_CLASS == 0) THEN 
               YR = R_YR - 1
               DO MO = 0, 12
                  INCOME_VARIABLES(MO,mty_dbt_fl_intrst_amrt) = &
                             INTEREST_AMORTIZATION(MO,YR,ASSET_CLASS,Dr)
                  INCOME_VARIABLES(MO,mty_dbt_fl_intrst_amrt_cr) = &
                             INTEREST_AMORTIZATION(MO,YR,ASSET_CLASS,Cr)
                  INCOME_VARIABLES(MO,mt_dbt_fl_chg_intrst_exp_amrt) = &
                           NET_CHANGE_IN_DB(MO,YR,ASSET_CLASS,Unamortized_InterestBalance,Dr)
                  INCOME_VARIABLES(MO,mt_chg_dbt_fl_issu_exp_amrt) = &
                           NET_CHANGE_IN_DB(MO,YR,ASSET_CLASS,Unamortized_Issue_Expen_Balance,Dr)
!
...
!
                  INCOME_VARIABLES(MO,Monthly_Goodwill_Amort) = &
                                INCOME_VARIABLES(MO,Monthly_Goodwill_Amort) &
                                + ATL_AMORT_SUB_ITEMS(MO,YR,ASSET_CLASS,Goodwill,Dr)
                  INCOME_VARIABLES(MO,Monthly_Regulatory_Assets_Amort) = &
                                INCOME_VARIABLES(MO,Monthly_Regulatory_Assets_Amort) &
                                + ATL_AMORT_SUB_ITEMS(MO,YR,ASSET_CLASS,RegulatoryAssets,Dr)
                  INCOME_VARIABLES(MO,Monthly_FASB_109_Amort) = &
                                INCOME_VARIABLES(MO,Monthly_FASB_109_Amort) &
                               + ATL_AMORT_SUB_ITEMS(MO,YR,ASSET_CLASS,FASB109,Dr)
                  INCOME_VARIABLES(MO,Monthly_FASB_133_Amort) = &
                                INCOME_VARIABLES(MO,Monthly_FASB_133_Amort) &
                                + ATL_AMORT_SUB_ITEMS(MO,YR,ASSET_CLASS,FASB133,Dr)
                  INCOME_VARIABLES(MO,monthly_other_atl_amort) = &
                            INCOME_VARIABLES(MO,monthly_other_atl_amort) &
                            + ATL_AMORT_SUB_ITEMS(MO,YR,ASSET_CLASS,Other_Deferred_Debits,Dr)

! CREDIT AMORTIZATIONS
!
                  INCOME_VARIABLES(MO,monthly_other_atl_amort) = &
                            INCOME_VARIABLES(MO,monthly_other_atl_amort) &
                            - ATL_AMORT_SUB_ITEMS(MO,YR,ASSET_CLASS,Other_Deferred_Debits,Cr)
                  ...
                  INCOME_VARIABLES(MO,Monthly_Regulatory_Assets_Amort) = &
                                INCOME_VARIABLES(MO,Monthly_Regulatory_Assets_Amort) &
                                - ATL_AMORT_SUB_ITEMS(MO,YR,ASSET_CLASS,RegulatoryAssets,Cr)
                  INCOME_VARIABLES(MO,monthly_variable_oandm) = &
                             INCOME_VARIABLES(MO,monthly_variable_oandm) &
...
!
         IF(R_CLASS <= MAX_ASSET_CLASS_NUM) THEN
            ...
            IF(ASSET_CLASS > 0 .OR. R_CLASS == 0) THEN
               YR = R_YR ! - 1
               DO MO = 0, 12
                  BALANCE_VARIABLES(MO,monthly_net_goodwill) = TNDB(MO,YR,ASSET_CLASS,Goodwill,Dr)
                  BALANCE_VARIABLES(MO,monthly_net_regulatory_assets) = TNDB(MO,YR,ASSET_CLASS,RegulatoryAssets,Dr)
                  BALANCE_VARIABLES(MO,mty_net_fasb_109) = TNDB(MO,YR,ASSET_CLASS,FASB109,Dr)
                  BALANCE_VARIABLES(MO,mty_net_fasb_133) = TNDB(MO,YR,ASSET_CLASS,FASB133,Dr)
! DEFERRED CREDITS
                  BALANCE_VARIABLES(MO,monthly_gain_on_reaqd_debt_bal) = &
                       TNDB(MO,YR,ASSET_CLASS,Gain_on_Reaquired_Debt,Cr)
                  BALANCE_VARIABLES(MO,mty_regty_liablties_bal) = &
                        TNDB(MO,YR,ASSET_CLASS,Regulatory_Liabilites,Cr)
                  BALANCE_VARIABLES(MO,monthly_other_deferred_credits) = &
                        TNDB(MO,YR,ASSET_CLASS,Other_Deferred_Credit,Cr)
                  BALANCE_VARIABLES(MO,monthly_deferred_gains) = &
                        TNDB(MO,YR,ASSET_CLASS,Asset_Sales_Deferred_Gain,Cr)
               ENDDO
            ENDIF
         ENDIF
      RETURN
         ...
         IF(R_CLASS <= MAX_ASSET_CLASS_NUM) THEN
            ...
            IF(ASSET_CLASS > 0 .OR. R_CLASS == 0) THEN
               ...
               BALANCE_VARIABLES(:,monthly_net_regulatory_assets) = &
                      BALANCE_VARIABLES(:,monthly_net_regulatory_assets) &
                      - TNDB(:,YR,ASSET_CLASS,RegulatoryAssets,Dr) 
               BALANCE_VARIABLES(:,mty_net_fasb_109) = &
                               BALANCE_VARIABLES(:,mty_net_fasb_109) &
                               - TNDB(:,YR,ASSET_CLASS,FASB109,Dr)
               BALANCE_VARIABLES(:,mty_net_fasb_133) = &
                               BALANCE_VARIABLES(:,mty_net_fasb_133) &
                               - TNDB(:,YR,ASSET_CLASS,FASB133,Dr) 
               BALANCE_VARIABLES(:,monthly_net_dfrd_debits) = &
                       BALANCE_VARIABLES(:,monthly_net_dfrd_debits) &
                       - TNDB(:,YR,ASSET_CLASS,Total_Deferred_Debits,Dr)

! DEFERRED CREDITS
               BALANCE_VARIABLES(:,monthly_gain_on_reaqd_debt_bal) = &
                     BALANCE_VARIABLES(:,monthly_gain_on_reaqd_debt_bal) &
                      - TNDB(:,YR,ASSET_CLASS,Gain_on_Reaquired_Debt,Cr)
               BALANCE_VARIABLES(:,mty_regty_liablties_bal) = &
                       BALANCE_VARIABLES(:,mty_regty_liablties_bal) &
                       - TNDB(:,YR,ASSET_CLASS,Regulatory_Liabilites,Cr)
               BALANCE_VARIABLES(:,monthly_other_deferred_credits) = &
                     BALANCE_VARIABLES(:,monthly_other_deferred_credits) &
                     - TNDB(:,YR,ASSET_CLASS,Other_Deferred_Credit,Cr)
               BALANCE_VARIABLES(:,monthly_deferred_gains) = &
                     BALANCE_VARIABLES(:,monthly_deferred_gains) &
                  - TNDB(:,YR,ASSET_CLASS,Asset_Sales_Deferred_Gain,Cr)
            ENDIF
         ENDIF
      RETURN
!***********************************************************************
