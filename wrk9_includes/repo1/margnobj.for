...
!
      ...
      LOGICAL*1   CAL_MARGINAL_COST,GET_THOSE_MARGINAL_COSTS,
     ...
     +            YES_MULTI_AREA_PRICE
      REAL*4      R_SYSTEM_MARGINAL_COST,GET_PRODUCTION_LEVEL,
...
!
      LOGICAL*1 YES_TECHNOLOGY_SCARCITY,
     ...
     +          REPORT_THIS_CL_UNIT
      INTEGER*4 MARGINAL_UNIT_REC/0/,UU_DAILY_REC/0/,UV_HOURLY_REC/0/,
     +          UK_DAILY_REC/0/
      INTEGER*2 TECH_POINTS/5/,
     ...
     +          UK_DAILY_HEADER,
     +          UV_HOURLY_HEADER
      REAL*4    TECH_WEIGHTED_PERCENT(5),TECH_BREAK_TEST,
...
!
      REAL*8   CAL_MAR_MONTHLY_ENERGY_routine,
     +         TRANS_GROUP_CAP,
     +         GET_PRODUCTION_LEVEL_AT_MARKET
!
!
!
      LOGICAL*1   POINT_NOT_FOUND
      INTEGER*2   NEW_POINT,NPC,R_I,R_NEW_POINT,
     +            SAVE_SEASON,
...
!
      LOGICAL*1   MARGINAL_REPORT_NOT_OPEN/.TRUE./,
     +            TECH_SCAR_REPORT_NOT_OPEN/.TRUE./,
     +            TECH_SCAR_REPORT,YES_TECH_SCAR_REPORT
      INTEGER*2   MARGINAL_NO,MARGINAL_COST_HEADER,
     +            TECH_SCAR_NO,TECH_SCAR_HEADER,
!     +            LAST_SEASON,PRODUCTION_PERIODS,
     +            NBLOK2_SAVE,SYS_TRANS_GROUP
      INTEGER MARGINAL_REC,TECH_SCAR_REC
      INTEGER*4   DATA_BASIS
      CHARACTER*1 LOGIC_TYPE
      CHARACTER*1 TRANSACTION_PERIOD,TRANS_TIME_FRAME
      LOGICAL*1 GET_START_UP_LOGIC
      CHARACTER*6 BEFORE_OR_AFTER_SALES
      CHARACTER*9
     +      CL_MONTH_NAME(13)/'January',
     +      'February','March','April','May','June',
     +      'July','August','September','October','November',
     +      'December','Annual'/
! ECITY
      CHARACTER*5 CL_UNIQUE_RPT_STR
      CHARACTER*20 SPECIAL_ID_NAME
      CHARACTER*25 CL_TRANS_NAME
      CHARACTER*14 PRIME_MOVER_NAME(12)/
     +               'Combined Cycle','Fuel Cell','Geothermal',
     +               'GT Gas Turbine','Hydro','IC Gas Turbine','Unkown',
     +               'Nuclear','Solar','Steam','Wind','NUG'/
      CHARACTER*35 LOCAL_GROUP_NAME,
     +             GET_GROUP_NAME,R_TRANS_NAME
      INTEGER*2 RESERVE_CAP_NO(2)/0,0/,SUPPLEMENTAL_CAP_NO(2)/0,0/,
!     +          RESERVE_BLOCK,
     +          GET_DUKE_RESERVE_CAP_NO
      REAL*4 RESERVE_CAPACITY(2)/0.,0./,GET_DUKE_RESERVE_CAPACITY,
...
C
      LOGICAL*1 LOOKING_AHEAD
!      INTEGER*2   AFTER,U_AFTER,B_AFTER,
!     +            TEMP_U,TEMP_B
      INTEGER*2 TEMP_BLOCK
     ...
     +      GET_TOP_CAP_PRODUCTION_LEVEL,TOP_CAP_MARGIN_COST
      LOGICAL*1   WRITE_TO_MC_DATA_BASE,
     +            MARGINAL_DATA_BASE_NOT_OPEN/.TRUE./,
     +            TRANS_GROUP_GEN_ACTIVE
      INTEGER*2 MARGINAL_DATA_NO/0/,MARGINAL_DATA_BASE_HEADER,
     +            COST_POINT,
...
!
      LOGICAL*1 TRANS_CONTRIB_NOT_OPEN/.TRUE./,
     +          YES_MARGINAL_UNIT_REPORT/.FALSE./,MARGINAL_UNIT_REPORT,
     +          YES_MARGINAL_COST_MODULE,
     +          MARGINAL_COST_SWITCH
      INTEGER*2 TRANS_CONTRIBUTION_RPT_HEADER,TRANS_CON_UNIT,R_HOUR,
     +            R_MONTH,LOCAL_HOUR/0/
...
!
      INTEGER*2 DAY_TYPE_COUNTER

      REAL*4 WRITE_CONTRIBUTION_REPORT,
...
!
      INTEGER*2 SAVE_UPPER_TRANS_GROUP, ! /0/,
...
!
     ...
     +            NUM_CREATE_HOURLY_PRICE
      REAL*4    DEPTH_MARKET_INTERVAL_SIZE/0/
!
      LOGICAL*1 WRITE_DEPTH_OF_MARKET_REPORT,
     +          SAVE_DEPTH_OF_MARKET_DATA,
     +          SAVE_TRANS_DEPTH_DATA,
     +          GET_CREATE_HOURLY_PRICE,
     +          TEMP_L,
     +          GET_TF_GROUP_NAME
      REAL*4   MINIMUM_INTERVAL,LOCAL_INTERVAL,DEPTH_AFTER_LOAD,
     +         D_SCARCITY_COST
      CHARACTER*6 LOCAL_INTERVAL_CHR
      INTEGER*2 D_HOUR,D_DATA_BASES,D_L_M,R_DAY,HR
      INTEGER ALLOCATED_DATA_BASES
      INTEGER*2 MAX_BLOCK_GUESS/4000/
      LOGICAL*1 LAHEY_LF95,TRANSACT_ANALYST_ONLY,INIT_FILE_IS_ACTIVE,
     +          FIRST_PASS_MAX_BLOCK_GUESS/.FALSE./
...
! 11/19/04
     ...
     +                             MAX(BLOK_ARRAY_ALLOCATOR,NBLOK2,4000)
...
!
     ...
     +                                       DEPTH_TARGET_GROUP)
         IF(YES_DEPTH_OF_MARKET) THEN
            DEPTH_MARKET_INTERVALS = MAX(3,DEPTH_MARKET_INTERVALS)
...
!
                  TEMP_TL_PEAK = MONTH_PEAK_BY_CG(INT2(1))
...
!
     ...
     +                         GET_ANNUAL_CL_CAPACITY(INT2(1),U,ISEAS)
            ENDIF
         ENDDO
      ENDIF
!
...
!
      ...
      DO
         ...
         IF(BLKNO(I) < 1) CYCLE
         OLD_FIRST_ECONOMY_COST =
     +                           UNIT_DISP_COST(UNIT(I),MAX(1,BLKNO(I)))
         EXIT
      ENDDO
!
      LAST_BLOCK_COST =
     +                UNIT_DISP_COST(UNIT(NBLOK2),MAX(1,BLKNO(NBLOK2)))
!
!
...
! DRATE
     ...
     +                                MW_AFTER_OUTAGES(BLOCK,DATA_BASES)
            ...
            INTERVALS = MIN(INTERVALS,999)
!
            NEXT(DATA_BASES) = MOD(CURRENT(DATA_BASES),2) + 1
!
            CAPACITY_ASSIGNED = 0.
!
!            NEW_POINT = 0
...
!
               IF(DRATE_TRANSACT) THEN
                  ...
                  ELSEIF(I <= 400) THEN
                     DIVISOR = 4
                     REMAIN = MOD((I-200),DIVISOR)

                     NEW_POINT = ((I-200)/DIVISOR) + 25
                  ELSEIF(I <= 800) THEN
                     DIVISOR = 2
                     REMAIN = MOD((I-400),DIVISOR)
...
!
               ...
               ELSE
                  COST_POINT = MIN(NEW_POINT + 1,TRANS_GROUP_POINTS)
               ENDIF
...
!
     ...
     +                  MAX(dble(BLOCK_DISP_COST(CURRENT_BLOCK_NUMBER)),
...
!
     ...
     +               CUM_MUST_RUN_POSITION(I) == 0 .AND. dble(
     +               CUM_MUST_RUN_MW(I)) <
...
!     +                              TECH_BREAK_MW_PERCENT_CHANGE *
     ...
     +                  TRANSITION_MARGINAL_COST(NEW_POINT,I,4) +
     +                   MAX(0.,TRANSITION_MARGINAL_COST(NEW_POINT,I,1)*
     +                                     TECH_BREAK_MW_PERCENT_CHANGE)
                  ENDDO
               ENDDO
...
!
         IF(TECH_SCAR_REPORT) THEN
            IF(TECH_SCAR_REPORT_NOT_OPEN) THEN
               TECH_SCAR_NO = TECH_SCAR_HEADER(INT2(PRODUCTS+1),
     +                                                    TECH_SCAR_REC)
...
!
         IF(R_SELLER_TRANS /= R_TRANS_GROUP) THEN ! BUYER
            MPOS = MAX(1,POS(R_SELLER_DATA_BASES))
            NPC =
...
! 051608.
         ...
         FT = MAX(0,MIN(FT,MAX_FUEL_TYPES))
         IF(FT == 2) THEN
            R_FT = 1.0
         ELSE
            R_FT = 0.0
         ENDIF
!
         B = MAX(1,BLKNO_SAVE(BLOCK_POSITION))
!
...
!
            TOP_CAP_MARGIN_COST =
     +                SYSTEM_MARGINAL_COST(HALF_POINT+INT2(1),
     +                                                 R_TRANS_GROUP,
     +                                                 R_MARKET_PRICE)
...
C***********************************************************************
C
         start_val=INT( MIN(31999,R_MARKET_PRICE*10.))
         SPEED_INDEX = .FALSE.
...
!
     ...
     +                         LAST_DB_POINT(R_DATA_BASES),R_DATA_BASES)
         ...
         IF(HALF_POINT >= 1) THEN
            IF(GET_PRODUCTION_LEVEL_AT_MARKET ==
     +                         dble(CUM_MUST_RUN_MW(R_DATA_BASES))) THEN
...
C
     ...
     +                                                    R_MARGIN_COST)
            IF(TOP_CAP_PERCENT > 0.) THEN
               GET_TOP_CAP_MARGINAL_COST =
     +            SYSTEM_MARGINAL_COST(POS(R_TRANS_GROUP)+INT2(1),
...
!
     ...
     +                                     (DEPTH_MARKET_INTERVALS-1)/2.
            ...
            DO I = 1, DEPTH_MARKET_INTERVALS
               ...
               DEPTH_AFTER_LOAD =
     +            MIN(dble(DEPTH_AFTER_LOAD),
     +                  CAPACITY_GIVEN_MARKET(
     +                        LAST_DB_POINT(D_DATA_BASES),D_DATA_BASES))
               DEPTH_PRICE(I,D_HOUR) =
     +               GET_MARGINAL_COST_AT_MARKET(dble(DEPTH_AFTER_LOAD),
     +                                              D_DATA_BASES,
     +                                              D_SCARCITY_COST,
     +                                              D_L_M,
     +                                              D_SCARCITY_MULT)
...
!
         DOWHILE(dble(R_BUYER_LOAD) <
     +                 CAPACITY_GIVEN_MARKET(BUYER_POSITION,R_BUYER_DB))
            BUYER_POSITION = BUYER_POSITION + 1
         ENDDO
         DOWHILE(dble(R_SELLER_LOAD) <
     +               CAPACITY_GIVEN_MARKET(SELLER_POSITION,R_SELLER_DB))
...
!         IF(R_LOAD > CUM_CAP(R_DATA_BASES) ) THEN
            IF(dble(R_LOAD) > CAPACITY_GIVEN_MARKET(
     +                   LAST_DB_POINT(R_DATA_BASES),R_DATA_BASES)) THEN
...
! ENERGY AND HEAT CALC
               ...
               K = MAX(1,TRANSACTION_GROUP(I))
               RTG = FLOAT(K)
               RFT = MAX(1,MIN(GET_PRIMARY_MOVER(I),MAX_FUEL_TYPES))
...
!               DO I = 1, 12
                  DO J = 1, SAVE_UPPER_TRANS_GROUP
! ENERGY AND HEAT CALC
                     K = MAX(1,GET_TRANS_GROUP_INDEX(J))
...
!
     ...
     +      DEALLOCATE( ! NEED TO BE DEALLOCATED AT THE END OF EACH ENDPOINT
     +         ACTIVE_DATA_BASE,
...
!
     ...
     +                                       LAST_BLOCK,SYS_TRANS_GROUP)
            ...
            U = UNIT_SAVE(BLOCK_POSITION)
            B = MAX(1,BLKNO_SAVE(BLOCK_POSITION))
            DOWHILE(UNIT_DISP_COST(U,B)/10. > SYS_MARKET_PRICE .AND.
     +                                                   LAST_BLOCK > 0)
     ...
     +                                       LAST_BLOCK,SYS_TRANS_GROUP)
            ...
            ENDDO
!
            DOWHILE(LAST_BLOCK > 0) ! THIS CAN BE REMOVED IF I SAVE WEIGHTED MARGINAL COSTS AT CALC TIME.
...
!      USE SIZECOM
C
      LOGICAL*1 GET_THOSE_MARGINAL_COSTS, HAVE_MARGINAL_COSTS
      INTEGER*2 MONTH,DAY,MC_YEAR,TEMPER/0/,DELTMP/0/
      INTEGER*4 LDE_RECORD_COUNT/0/
      INTEGER*2 IHRL(24)/24*0/,K
      INTEGER*4 LOADS(24)/24*0/,REC_LENGHT
      INTEGER*2 DAYWEK,TIMZON,I,J,DAYS_PER_MONTH(12),DAY_COUNT,
     +          MONTH_COUNT
      CHARACTER*8 EEICODE
      LOGICAL*1 LEAP_YEAR,NEW_LDE_FORMAT/.FALSE./
      REAL REMAIN
      DATA DAYS_PER_MONTH/31,29,31,30,31,30,31,31,30,31,30,31/
C
!     MARGINAL COSTING VARIABLES
C
      INTEGER*2 START_MONTH,END_MONTH,LOWER_MC
      REAL  DX,MARGINAL_COST(0:1000),HOURLY_MARGINAL_COST(24)
C
      INTEGER*2 HOURLY_LOAD_OUT,FILE_EXT,START_DAY,END_DAY/0/,MO
      CHARACTER*5 BSYRLOAD
      CHARACTER*2 HOURLY_CHAR
      CHARACTER*64 OUTPUT_DIRECTORY,FILE_NAME
C
...
C
     ...
     +                      trim(BSYRLOAD())//".B0"//CHAR(FILE_EXT+48)
         ELSE
            ...
            HOURLY_CHAR(2:2) = CHAR(48 + MOD(FILE_EXT,10))
            FILE_NAME = trim(OUTPUT_DIRECTORY())//"MCE"//
...
C
     ...
     +            DEALLO_CAL_HOURLY_MC_AFTER_EL
      INTEGER*2   I,ISEAS,MONTH,DAY,MC_YEAR,DAYWEK,TIMZON,
     +            DAYS_PER_MONTH(12),WRITE_YEAR,MAX_DATA_BASES,R_HOUR,
     +            R_TRANS_GROUP
      PARAMETER   (MAX_DATA_BASES=6)
      INTEGER*4   DAILY_MW(24)
      REAL*4      LPROB(*),LODDUR(*),
     +            GET_TRANS_GROUP_HOURLY_LOAD
      INTEGER*2   CUM_HOURS_BY_MONTH(0:12),CUM_HOURS
      CHARACTER*2 HOURLY_CHAR
      CHARACTER*8 EEICODE
      REAL*8 DEMAND_AFTER_EL,TOTAL_DEMAND_AFTER_EL,GET_DEMAND_AFTER_EL
      INTEGER*4 I4_ENERGY,TOTAL_I4_ENERGY
      REAL*4 R4_ENERGY,TOTAL_R4_ENERGY,LOAD_ADJUSTER
...
!
         ...
         ENDIF
         HOURLY_CHAR(2:2) = CHAR(48 + MOD(MC_YEAR,10))
C
...
!
               AEL_PEAK(MONTH,1) = MAX(AEL_PEAK(MONTH,1),DAILY_MW(I))
!
               IF(DAYWEK < 6) THEN
                  AEL_PEAK(MONTH,2) = MAX(AEL_PEAK(MONTH,2),DAILY_MW(I))
               ENDIF
               AEL_BASE(MONTH) = MIN(AEL_BASE(MONTH),DAILY_MW(I))
...
!      end
      ...
      SAVE
      include 'mcBTNCom.mon'
      integer*2 iz
      real*4 y,z,Density,DGauss,CCNFMD
      ...
      real*4 function CCNormOrd(xCCNOmMean)
      SAVE
      include 'mcBTNCom.mon'
      real*4 xCCNOmMean
      integer*2 iz
      ...
      include 'mcBTNCom.mon'
      real*4 a(*),b(*),bAccum
      integer*2 nUnique,i,j,k
      integer*4 nB
      real*4 a_temp_array(:),b_temp_array(:)
...
!          call cmove(b(i),b(j),nB)
     ...
     +      nUnique,i,j,bAccum,i-j,' deleted'
      ...
      INCLUDE 'mcBTNCom.mon'
      integer*2 kFNV,i,j
      logical*1 DupsLikely
      real*4 BranchCOut,BranchProb
...
!      call cmove(PrevCOut,NodeCOut,VectSizeByte)
      ...
      real*4 function NodeProbWeightedLDC(xNPWD,k,jPt)
      SAVE
      real*4 xNPWD
      integer*2 k,jPt ! passed only for use if PrtDetail>0
      include 'mcBTNCom.mon'
