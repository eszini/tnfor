...
C
     ...
     +       ENERGY_ADJUSTMENTS(12)
      CHARACTER*1 ENERGY_ADJUSTMENT_METHOD,DEMAND_ADJUSTMENT_METHOD  
      CHARACTER*1 DEMAND_PRICING_SWITCH,
     +            ACCOUNT_ACTIVE(:)
      INTEGER*4 RATE_CODE(:) 
      CHARACTER*10 RATE_TRACKER(:),
     +             RATE_OPTION(:)
      LOGICAL*1 RETURN_WVPA_RATES
C
C
     ...
     +       R_MONTHLY_VALUES(0:12)
      LOGICAL*1 GET_TG_CG2_DATA,DATA_FOUND,TEMP_DATA_FOUND
     ...
     +          ASSET_ALLOCATION_VECTOR(:)
      ...
      CHARACTER*40 REVENUE_CLASSIFICATION(:),
     +             R_REVENUE_CLASSIFICATION
      CHARACTER*1 USE_TRANSACT_FILE ! 113
      INTEGER*2, ALLOCATABLE :: TRANSACT_FORECAST_GROUP(:), ! 110
...
!
     ...
     +               ENERGY_MONTHLY
      INTEGER*2 RUN_YEAR
      INTEGER*2 REV_TYPE
      REAL*4 LAG_BY_MONTH(24),ANNUAL_CASH_TOTAL,
...
C
      ...
      REAL*4 R_MONTHLY_REVENUES(0:12),
     +       R_ASSET_ALLOCATOR,
...
C
      INTEGER*2 YR
      INTEGER*2 WVPA_COOP_REVENUE_HEADER
C
      CHARACTER*6 SHORT_MONTH_NAMES
C
      CHARACTER*40 EXT_RATE_CLASS_NAME
      CHARACTER*20 VARIABLE_TYPE_NAME(0:18)
      REAL*4 VARIABLE_VALUE(0:23)
      ...
      INTEGER*2 USE_ENERGY_DATA_MONTH,
...
C
         ...
         DO MO = 1, 12
            IF(MOD(MO,3) == 1) THEN  ! UPDATE ADJ FACTORS
...
C
      ...
      INTEGER*2 NUM_OF_ASSET_CLASSES,R_NUM_OF_ASSET_CLASSES
      INTEGER*2 MAX_ASSET_CLASS_NUM,R_MAX_ASSET_CLASS_NUM
      INTEGER*2 R_ASSET_CLASS_POINTER(R_MAX_ASSET_CLASS_NUM)
      INTEGER*2 ASSET_CLASS_POINTER(:),R_MONTH,I,ASSET_CLASS,REV_TYPE,MO
      ALLOCATABLE ASSET_CLASS_POINTER  
     ...
     +     ASSET_CLASS_POINTER
C
      REAL*4 R_REVENUES_MONTHLY(0:12,-1:R_NUM_OF_ASSET_CLASSES,
     +                                                LAST_INCOME_LINE),
...
C
               R_ADJUSTMENT_CLAUSE_REVENUES =
     +                 R_ADJUSTMENT_CLAUSE_REVENUES +
     +                 REVENUES_MONTHLY(0,ASSET_CLASS,Adjustment Clause)
               R_GAS_ADJUSTMENT_CLAUSE_REVENUE =
     +                  R_GAS_ADJUSTMENT_CLAUSE_REVENUE
     +                  + REVENUES_MONTHLY(0,ASSET_CLASS,PGA Adjustment)
               R_BASE_RATES_REVENUES = R_BASE_RATES_REVENUES +
     +                        REVENUES_MONTHLY(0,ASSET_CLASS,Base Rates)
               R_SECONDARY_SALES_REVENUES = R_SECONDARY_SALES_REVENUES +
     +                   REVENUES_MONTHLY(0,ASSET_CLASS,Secondary Sales)
               R_OTHER_REVENUES = R_OTHER_REVENUES +
     +                     REVENUES_MONTHLY(0,ASSET_CLASS,Other Revenue)
               R_BTL_REVENUES = R_BTL_REVENUES +
     +                      REVENUES_MONTHLY(0,ASSET_CLASS,BTL Revenues)
               R_GAS_REVENUES = R_GAS_REVENUES +
     +                      REVENUES_MONTHLY(0,ASSET_CLASS,Gas Revenues)
               R_CAT_REVENUES = R_CAT_REVENUES +
     +                  REVENUES_MONTHLY(0,ASSET_CLASS,Catawba Revenues)
     ...
     +                        REVENUES_MONTHLY(0,ASSET_CLASS,Bulk Power)
               R_CAPACITY_SALES = R_CAPACITY_SALES +
     +                    REVENUES_MONTHLY(0,ASSET_CLASS,Capacity Sales)
...
C
     ...
     +                                     MAX_ASSET_CLASS_NUM > 0) THEN
            ...
            IF(ASSET_CLASS > 0 .OR. R_CLASS <= 0) THEN
               DO MO = 0, 12
                  DO DATA_POS = Cash BTL Lease Cash, Cash Other
                     IF(DATA_POS == Cash Purchased Power) THEN
                        R_MONTHLY_VALUES(MO) =
...
c        R_CHANGE_IN_ACCOUNTS_RECEIVABLE = 0.
     ...
     +                                     MAX_ASSET_CLASS_NUM > 0) THEN
            ...
            ENDIF
            IF(ASSET_CLASS > 0 .OR. R_CLASS <= 0) THEN
               DO DATA_POS = Cash Base Rates, Cash Gas Revenues
                  R_CHANGE_IN_ACCOUNTS_RECEIVABLE =
     +                             R_CHANGE_IN_ACCOUNTS_RECEIVABLE +
     +                   REVENUES_MONTHLY(0,ASSET_CLASS,DATA_POS) -
     +                   CASH_AMOUNTS_RECEIVABLE(0,ASSET_CLASS,DATA_POS)
               ENDDO
               DO DATA_POS = Cash Relationship Revenues,
     +                                         Cash Operating Method Adj
                  R_CHANGE_IN_ACCOUNTS_RECEIVABLE =
     +                             R_CHANGE_IN_ACCOUNTS_RECEIVABLE +
     +                   REVENUES_MONTHLY(0,ASSET_CLASS,DATA_POS) -
     +                   CASH_AMOUNTS_RECEIVABLE(0,ASSET_CLASS,DATA_POS)
               ENDDO
               DO DATA_POS = BTL Lease Cash, Other
                  R_CHANGE_IN_ACCOUNTS_PAYABLE =
     +                             R_CHANGE_IN_ACCOUNTS_PAYABLE +
     +                   EXPENSES_MONTHLY(0,ASSET_CLASS,DATA_POS) -
     +                   CASH_AMOUNTS_PAYABLE(0,ASSET_CLASS,DATA_POS)
               ENDDO
               DO DATA_POS = Total Nuclear Fuel, AG Maintenance
                  R_CHANGE_IN_ACCOUNTS_PAYABLE =
     +                             R_CHANGE_IN_ACCOUNTS_PAYABLE +
     +                   EXPENSES_MONTHLY(0,ASSET_CLASS,DATA_POS) -
     +                   CASH_AMOUNTS_PAYABLE(0,ASSET_CLASS,DATA_POS)
