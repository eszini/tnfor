...
!
      REAL (kind=4) :: 
!     +     DSM_PEAK_RESERVE_ALLOCATION(3,7,12),
...
!
     ...
     +     FREE_DRIVERS(:),
     +     PROGRAM_LOSS_FACTOR(:),
!     +     DSM_PEAK_RESERVE_ALLOCATION(3,SYSTEM_CLASS_NUM,12),
!     +     GREG_ENERGY_RESERVE_ALLOCATION(3,SYSTEM_CLASS_NUM,12),
!     +     DSM_PEAK_RESERVE_ALLOCATION(:,:,:),
!     +     GREG_ENERGY_RESERVE_ALLOCATION(:,:,:),
...
!     +     R_DSM_PEAK_RESERVE_ALLOCATION(3,*),
!     +     R_DSM_ENER_RESERVE_ALLOCATION(3,*),
!     +     R_DSM_ENER_ALLOCATION(*)
...
!                  FREE_RIDERS(APP_NUM) = FREE_RIDERS(APP_NUM)/100.
!                  FREE_DRIVERS(APP_NUM) = FREE_DRIVERS(APP_NUM)/100.
                  PROGRAM_LOSS_FACTOR(APP_NUM) =
...
!     *************************************************************
!     ENTRY MDSLDCAP(CAPACITY_PEAKS,LDMGT_ACTIVE,
!    +               DSM_CAP,
!    +               INPUT_SYSTEM_PEAK)
!     *************************************************************
!
! SEARCHES FOR THE AMOUNT OF DSM FORECASTED DURING THE PEAK HOUR
! FOR EACH DAY TYPE FOR EACH MONTH
!
! NOTE: TO REDUCE ARRAY SIZES, ALLOWANCE FOR ONLY 3 CUSTMOMER CLASSES
! WITH LOAD MANAGEMENT IS MADE, AND THE NUMBERS MUST CORRESPOND TO
! THE ORDER OF THE CLASSES AS IMPLICITLY DEFINED BY THE NAMES IN
! ARRAY CLNAM, FILLED IN SUBROUTINE GETCLS.
!
!     *************************************************************
!     READ IN THE LOAD MANAGEMENT FILE
!     *************************************************************
!
!      ENTRY DSM_UPDATE_ANNUAL_CAPACITIES(R_YEAR)
!
...
!               R_DSM_PEAK_RESERVE_ALLOCATION(1,CLASS) =
!     +                   DSM_PEAK_RESERVE_ALLOCATION(1,CLASS,MONTH)
!               R_DSM_PEAK_RESERVE_ALLOCATION(2,CLASS) =
!     +                   DSM_PEAK_RESERVE_ALLOCATION(2,CLASS,MONTH)
!               R_DSM_PEAK_RESERVE_ALLOCATION(3,CLASS) =
!     +                   DSM_PEAK_RESERVE_ALLOCATION(3,CLASS,MONTH)
            ENDDO
!!        ELSE
!!           DO CLASS = 1, MAX_LOAD_CLASSES
!!              R_DSM_PEAK_RESERVE_ALLOCATION(1,CLASS) = 0.
!!              R_DSM_PEAK_RESERVE_ALLOCATION(2,CLASS) = 0.
!!              R_DSM_PEAK_RESERVE_ALLOCATION(3,CLASS) = 0.
!!           ENDDO
!!        ENDIF
      RETURN
!     ******************************************************************
...
!         ELSE
!            DO CLASS = 1, MAX_LOAD_CLASSES
!               R_DSM_ENER_RESERVE_ALLOCATION(1,CLASS) = 0.
!               R_DSM_ENER_RESERVE_ALLOCATION(2,CLASS) = 0.
!               R_DSM_ENER_RESERVE_ALLOCATION(3,CLASS) = 0.
!            ENDDO
!         ENDIF
      RETURN
!     ******************************************************************
...
!     ******************************************************************
         ...
         MONTH = R_MONTH
!         IF(ALLOCATED(DSM_PEAK_RESERVE_ALLOCATION)) THEN
!            R_DSM_PEAK = DSM_PEAK_RESERVE_ALLOCATION(1,CLASS,MONTH)
...
!
      ...
      RETURN
!   10 CALL LOCATE(20,2)
!      WRITE(6,"('&',A,I4,A)") 'Error reading variable asset '//
