...
!
         IF(LOAD_POINT <= LODDUR(1)) THEN
            LOLP_VALUE = 1.
         ELSE
            DOWHILE (.NOT. (LODDUR(BOX) <= LOAD_POINT .AND.
...
!
      ENDIF
!
      DOWHILE(CONTRACT .LT. NUMBER_OF_CONTRACTS .AND.
...
!
      ...
      IF(B .GT. LODDUR(1)) THEN
         ISTOP = MAX(ISTART,1)
         DOWHILE(.NOT.(LODDUR(ISTOP).LE.B .AND. B.LE.LODDUR(ISTOP+1)))
...
!
!
      DOWHILE(RIGHT .GT. 0. .AND. CONTRACTS_ACTIVE)
...
!
            LD_PTS = ISTOP
            DOWHILE( CONTRACT_MINIMUM_PROB .LT. LPROB(LD_PTS))
               LD_PTS = LD_PTS + 1
            ENDDO
            IF(LD_PTS .EQ. 1 ) THEN
!
...
! 12/23/94. GAT. MOVED FROM ABOVE THE IF
     ...
     +                                                       TIER_2_TEMP
               ENDIF
               CONTRACT = 1
               DOWHILE(CONTRACT <= NUMBER_OF_CONTRACTS .AND.
     +                                             TIER_2_ENERGY > 0.0 )
                  IF(AVAILABLE_CONTRACT(CONTRACT) .AND.
...
!
            ...
            CUR_POS = SURPLUS_CAP_ORDER(CONTRACT)
!
            DOWHILE( CONTRACT <= END_SURPLUS .AND.
...
!***********************************************************************
     ...
     +                          CONTRACT_CAPACITY(LOWEST_COST_CONTRACT))
         ...
         DOWHILE(CUR_SUR <= MAX_SUR .AND. CAPACITY > 0.)
