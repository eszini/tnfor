...
!
      ...
      REAL :: R_VALUE,R_CURRENT_YEAR_VALUES(*),R_VECTOR_FOR_ALL_YEARS(*),ESCALATED_MONTHLY_VALUE,ESCALATED_MONTHLY_VAL, &
              ...
              RET_VAL_FROM_ESCALATION_VECTOR,RETURN_SINGLE_VALUE_FROM_ESCALATION_VECTOR
...
!***********************************************************************
      ...
      RETURN
!***********************************************************************
      ENTRY ESCALATED_VECTOR_OR_RETURN_VALUE(R_VECTOR_TYPE,R_VECTOR_NO,R_YR,R_SEASON,R_UPDATE_SEASON)
         ...
         IF(.NOT. ALLOCATED(VECTOR_PERIOD_IS_ANNUAL)) THEN ! NO ESCALATION RATES FILE.
            ESCALATED_VECTOR_OR_RETURN_VALUE = 1.
         ...
         IF(VECTOR_PERIOD_IS_ANNUAL(I4)) THEN         
            ...
            ENDIF
            ESCALATED_VECTOR_OR_RETURN_VALUE = ESCALATED_MONTHLY_VAL
         ENDIF         
      RETURN
!***********************************************************************
...
! 092707. REPLACE VALUE IF VECTOR IS NOT VALUE OR IN EXTENSION PERIOD
      ...
      RETURN
!***********************************************************************
      ENTRY RETURN_SINGLE_VALUE_FROM_ESCALATION_VECTOR(R_VECTOR_NO,R_YEAR,R_SEASON)
!***********************************************************************
! THIS ASSUMES THAT R_VALUE HAS BEEN ESCALATED TO THE PREVIOUS YEAR
! THE NEW VALUE IS RETURNED
!
         ...
         ELSE
            I = ABS(R_VECTOR_NO)
         ENDIF
         RETURN_SINGLE_VALUE_FROM_ESCALATION_VECTOR = 0.
         YR = MIN(R_YEAR,AVAIL_DATA_YEARS)
         IF(ALLOCATED(VECTOR_PERIOD_IS_ANNUAL)) THEN ! NO ESCALATION RATES FILE.
            I4 = I4_MONTHLY_VECTOR_MAP(I,R_SEASON)
            IF(I4 == -2) I4 = I4_VECTOR_MAP(I) 
            RETURN_SINGLE_VALUE_FROM_ESCALATION_VECTOR = VECTOR_VALUES(YR,I4)
         ENDIF
      RETURN
!***********************************************************************
