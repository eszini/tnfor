     ...
     GRX_CO2_ABATEMENTS_AVAILABLE = 10**6*GRX_CO2_ABATEMENTS_AVAILABLE
      ...
      FUNCTION CHECK_FOR_GRX_ABATEMENT_CONVERGENCE(TG)
        ...
        REAL (KIND=4) :: CO2_ELECTRIC_EMISSION, &
          ...
          GRX_CONVERGED = .FALSE.
          CO2_EMISSIONS_CAP = GRX_ALL_SECTOR_CO2_EMISSIONS_CAP &
            (BASE_YEAR+YEAR)
          CO2_ELECTRIC_EMISSION = GET_CO2_EMISS_FOR_SYSTEM()
          CO2_NON_ELECTRIC_EMISSION = &
          GRX_NON_ELECTRIC_SECTOR_CO2_EMISSIONS(BASE_YEAR+YEAR)
...
! min credit market limits on bank purchases
          CO2_BANK_CREDITS_USED = &
          ABS(MIN(0,GRX_CO2_BANK_DELTA(BASE_YEAR+YEAR))) 
          ! (  negative term when bank is used for offset. )
          
          CO2_BANK_CREDITS_PURCHASED = &
          MAX(0,GRX_CO2_BANK_DELTA(BASE_YEAR+YEAR))
          ! (  positive term when bank purchases offsets )
          CO2_BANK_CAP_ADJUSTMENT = &
          GRX_CO2_BANK_DELTA(BASE_YEAR+YEAR) 
          ! ( positive term when bank purchases offsets)
          ...
          IF(CO2_Market_Available(BASE_YEAR+YEAR)) THEN
               CURRENT_CO2_EMISS =  CURRENT_CO2_EMISS &
                        ...
                        BASE_YEAR+YEAR, CURRENT_CO2_DISPATCH_COST)
               IF(CURRENT_CO2_EMISS > CO2_EMISSIONS_CAP) THEN
                  ABOVE_PRICE = CURRENT_CO2_DISPATCH_COST
                  ABOVE_EMISS = CO2_ELECTRIC_EMISSION
        ...
        GRX_ITERATIONS >= CO2_Min_GRX_Iterations(BASE_YEAR+YEAR)) THEN
               ...
               RETURN
          ENDIF
        END FUNCTION CHECK_FOR_GRX_ABATEMENT_CONVERGENCE
...
! CO2 PRICE TOO HIGH ADJUST TO FIRST MARKET PRICE
    ...
    REAL (KIND=4) :: CO2_ELECTRIC_EMISSION,CO2_NON_ELECTRIC_EMISSION, &
        ...
        WRITE(1338,REC=6) 'Transaction Group   ','C',INT2(35),'V','D'
         ...
         NEXT_REC = NEXT_REC + 1
         CO2_EMISS_CAP = &
         GRX_ALL_SECTOR_CO2_EMISSIONS_CAP(BASE_YEAR+YEAR) /1000000.
         CO2_BANK_CREDITS_USED = &
         ABS(MIN(0,GRX_CO2_BANK_DELTA(BASE_YEAR+YEAR)))/10.**6 
         ! ( negative term when bank is used for offset. )
         
         CO2_BANK_CREDITS_PURCHASED = &
         MAX(0,GRX_CO2_BANK_DELTA(BASE_YEAR+YEAR))/10.**6
         ! ( positive term when bank purchases offsets )
         
         CO2_ELECTRIC_EMISSION = GET_CO2_EMISS_FOR_SYSTEM()/1000000.
         CO2_NON_ELECTRIC_EMISSION = &
         GRX_NON_ELECTRIC_SECTOR_CO2_EMISSIONS(BASE_YEAR+YEAR)/1000000.
         CO2_EMISSIONS = CO2_ELECTRIC_EMISSION &
                         + CO2_NON_ELECTRIC_EMISSION
         CO2_ABATEMENTS_AVAIL = &
              grx_co2_intrl_abatements_av(BASE_YEAR+YEAR, &
               ...
               IF(CO2_PRICE <= Non_El_Price_Break(YEAR)) THEN
                  ...
                  ELSE
                     IF(INDEX(Non_El_Price_Equation_1_Type(YEAR),"Expon") /= 0) THEN
                        grx_co2_intrl_abatements_av=MAX(0.,(LOG(CO2_PRICE)-LOG(Non_El_1_A_Coefficient(YEAR)))/ &
                                                                        Non_El_1_B_Coefficient(YEAR))
                     ELSE
                        grx_co2_intrl_abatements_av = (CO2_PRICE-Non_El_1_A_Coefficient(YEAR))/ &
                                                                 Non_El_1_B_Coefficient(YEAR)
                     ENDIF
                  ENDIF
               ...
               ELSE ! Equation 2
                  ...
                  ELSE
                     IF(INDEX(Non_El_Price_Equation_2_Type(YEAR),"Expon") /= 0) THEN
                        grx_co2_intrl_abatements_av=MAX(0.,(LOG(CO2_PRICE)-LOG(Non_El_2_A_Coefficient(YEAR)))/ &
                                                                        Non_El_2_B_Coefficient(YEAR))
                     ELSE
                        grx_co2_intrl_abatements_av = (CO2_PRICE-Non_El_2_A_Coefficient(YEAR))/  &
                                                                    Non_El_2_B_Coefficient(YEAR)
                     ENDIF
                  ENDIF
...
!               CO2_ABATEMENTS_PURCHASED                                                       +
         ...
         IF(PRESENT(CO2_CREDITS)) THEN
            ...
            IF(YEAR <= YR2_DATA .AND. YEAR >= YR1_DATA .AND. CO2_PARAM_FILE_EXISTS) THEN
               IF(INDEX(Non_El_Price_Equation_1_Type(YEAR),"Expon") /= 0) THEN
                  GRX_CO2_PRICE = Non_El_1_A_Coefficient(YEAR)*EXP(CO2_CREDITS*Non_El_1_B_Coefficient(YEAR))
                  IF(GRX_CO2_PRICE > Non_El_Price_Break(YEAR)) THEN
                     IF(INDEX(Non_El_Price_Equation_2_Type(YEAR),"Expon") /= 0) THEN
                        GRX_CO2_PRICE = Non_El_2_A_Coefficient(YEAR)*EXP(CO2_CREDITS*Non_El_2_B_Coefficient(YEAR))
                     ELSE
                        GRX_CO2_PRICE = Non_El_2_A_Coefficient(YEAR) + CO2_CREDITS*Non_El_2_B_Coefficient(YEAR)
                     ENDIF
                  ENDIF
               ELSE
                  GRX_CO2_PRICE = Non_El_1_A_Coefficient(YEAR) + CO2_CREDITS*Non_El_1_B_Coefficient(YEAR)
                  IF(GRX_CO2_PRICE > Non_El_Price_Break(YEAR)) THEN
                     IF(INDEX(Non_El_Price_Equation_2_Type(YEAR),"Expon") /= 0) THEN
                        GRX_CO2_PRICE = Non_El_2_A_Coefficient(YEAR)*EXP(CO2_CREDITS*Non_El_2_B_Coefficient(YEAR))
                     ELSE
...
!                 GRX_CO2_DOMISIC_ABATEMENTS_PRICE =  COEFF1(YEAR) + COEFF2(YEAR) * CO2_ABATEMENTS_PURCHASED                                                       +
               ENDIF
