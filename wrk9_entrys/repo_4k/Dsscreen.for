C     Last change: MSG 1/29/2007 2:01:29 PM
C***********************************************************************

C       INITIALIZE THE SCREEN CONTROL VARIABLES
...
C***********************************************************************
      ...
      implicit none
      character (len=512) :: screen_messages_ord
      CHARACTER*1 UTILITY_TYPE,OBJ_FUNCTION_DESC*50
      INTEGER*2 PROCMETH,COVERAGE_RATIO
      LOGICAL*1 SHORT_FORM,
     +          POOLING_TRANSACTIONS,CONTROL_AREA_FORECAST,
     +          CAP_PLANNING_METHOD_ANNDECOMP,FACET_IS_ACTIVE,
     +          ASSET_ANALYST_ONLY,TRANSACT_ANALYST_ONLY,
     +          INIT_FILE_IS_ACTIVE
      CHARACTER*5 CHR_POINTS
      CHARACTER*1 OPMETH
      INTEGER*4 SCREEN1(1024)
      INTEGER*4 SCREEN2(1024)
      INTEGER*2 NORMAL,INVERSE
      INTEGER*2 NORMAL_VIDEO,INVERSE_VIDEO
      INTEGER*2 END_POINTS,I,GET_NUM_END_POINTS_IN_OVL_FILE
      CHARACTER*4 END_POINT_STR
      CHARACTER*5 SCENAME
      CHARACTER*80 BANNER
      CHARACTER*4 TOTAL_NUM_OF_END_POINTS_STR/"1"/
      CHARACTER*50 STUDY_PERIOD_STR
      SAVE BANNER,NORMAL,INVERSE,SCREEN1,
     +     CAP_PLANNING_METHOD_ANNDECOMP
      CHARACTER*256 SIM_FILE_NAME,LC_SIM_FILE_NAME
      CHARACTER*5 MONTH_DAY,SIM_MONTH_DAY
      CHARACTER*2 SIM_MONTH
      LOGICAL*1 LAHEY_LF95,VERSION_IS_RW
      CHARACTER*5 VERSION
...
C FIND TOTAL NUMBER OF ENDPOINTS
     ...
     +                trim(LC_SIM_FILE_NAME)//" ",INVERSE)
         ...
         CALL FILLB(2," ",NORMAL)
C DO NOT INDENT THE FOLLOWING LINES
     ...
     +                                                   ALL_VERSIONS,0)
         ...
         WRITE(screen_messages_ord,"(I4,A1,I4)") BASE_YEAR+1,'-',ENDYR
         CALL MG_LOCATE_WRITE(6,62,trim(screen_messages_ord),
     +                                                   ALL_VERSIONS,0)
         IF(LAST_EXTENSION_YEAR > ENDYR) THEN
            WRITE(screen_messages_ord,"(I4)") LAST_EXTENSION_YEAR
...
C        CALL BLDB(5,23,0,STATUS_LINE,INVERSE)
         CALL DISPSCN(SCREEN2)
         CALL LOCATEW(7,0,54)
         CALL PRINTW(7,'End Point     /'//TOTAL_NUM_OF_END_POINTS_STR)

C SET UP VIRTUAL SCREEN FOR OPTIONS SELCECTED AND OPTIONS TESTED
...
C***********************************************************************
      ...
      CHARACTER*256 SIM_FILE_NAME,lc_SIM_FILE_NAME
      character (len=6) :: screen_messages_ord


      INTEGER (KIND=4) :: SET_VERSION,LENGHT

      LOGICAL*1 THIS_VERSION_IS_WINDOWS/.FALSE./,
...
C ADD THE VERSION LOGIC HERE
C            THIS_VERSION_IS_RealWIN = .TRUE.
C      RETURN
...
C***********************************************************************
     ...
     +      (THIS_VERSION_IS_RealWIN .OR. THIS_VERSION_IS_WINDOWS)) THEN
         ...
         ELSEIF(VERSION==ALL_VERSIONS .OR. VERSION==LF77_VERSION) THEN
            !LF77 VERSION
            LENGHT = LEN(screen_messages_ord)
            CALL LOCATE(ROW1,COL1)
            CALL WRITE_LF77_SCREEN(screen_messages_ord)
         ENDIF
      RETURN
...
C***********************************************************************
     ...
     +      (THIS_VERSION_IS_RealWIN .OR. THIS_VERSION_IS_WINDOWS)) THEN
      ...
      RETURN
C***********************************************************************
      ENTRY UPDATE_TIME_ON_STATUS_LINE(screen_messages_ord)
C***********************************************************************
         IF(THIS_VERSION_IS_RealWIN) THEN
            CALL RW_UPDATE_STATUS_TIME(screen_messages_ord)
         ENDIF
      RETURN
      END
