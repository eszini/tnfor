...
C
     ...
     +                   + INCOME_LINKED_LEVEL(:,
     +                          Monthly Model Calc Property Taxes,
     +                                                      CLASS_LEVEL)
...
C
     ...
     + + ns_class_parameters%NPV_IN_ASSETS_NEC * NET_UTILITY_PLANT_EOY
...
c
            INCOME_VARIABLES(:,mthly_lsd_nclr_fl_xpns) =
...
C
     ...
     +                           FASB143_MONTHLY_ARO_INTEREST_ACCREATION
         IF(ADD_2_INCOME_STATEMENT == 'T') THEN
            INCOME_VARIABLES(:,
     +                   monthly_income_rsv_mgn_cap_sls) =
     +          INCOME_VARIABLES(:,
     +                     monthly_income_rsv_mgn_cap_sls)
     +          + MONTHLY_CAPACITY_SALES_TO_LEVEL_RM
            INCOME_VARIABLES(:,
     +               monthly_expense_reserve_mgn_cap_purch) =
     +            INCOME_VARIABLES(:,
     +                 monthly_expense_reserve_mgn_cap_purch)
     +            + MONTHLY_CAPACITY_PURCHASES_TO_LEVEL_RM
            CASH_VARIABLES(:,
...
C
            ...
            IF(VARIABLE==monthly_exp_file_operating_rev_tax) CYCLE
            IF(VARIABLE == Monthly Model Calc Property Taxes) THEN
...
C
     ...
     +                             Monthly Debt File LInvestment Income)
         ...
         INCOME_VARIABLES(:,
     +       mnthly_net_tax_nclr_decom_fund_erngs) =
     +                       MONTHLY_NUC_DECOM_FUND_NET_OF_TAX_RETURN(:)
         INCOME_VARIABLES(:,mnthly_rtrmt_med_fund_earnings) =
     ...
     +                   + MONTHLY_NUC_DECOM_FUND_RETURN(:)
            INCOME_VARIABLES(:,
     +                  monthly_nclr_decomm_fund_earnings) =
     +                                  MONTHLY_NUC_DECOM_FUND_RETURN(:)

            INCOME_VARIABLES(:,mnthly_investmt_erngs_rcvbl) =
     +            + INCOME_VARIABLES(:,Monthly Dividend 70 Earnings)
     +            + INCOME_VARIABLES(:,Monthly Regular Dividend Income)
...
C
C TAL TAXES ITES 81-95; TOTAL EXPENSE 150 OP INCOME 151 
C INCOME_VARIABLES(0:12,Monthly Federal ATL Income Tax Paid)
C
         UNALLOCATED_AMOUNT(Monthly Model Calc Property Taxes) =        
     +          ALLOCATE_ACCOUNT_BALANCE(ANNUAL_VARS(435), 
     +                       INCOME_VARIABLES(0,
     +                         Monthly Model Calc Property Taxes))
         INCOME_VARIABLES(:,Monthly Model Calc Property Taxes) =
     +       INCOME_VARIABLES(:,Monthly Model Calc Property Taxes)
     +       + PROPERTY_TAX_ADJUSTMENT(:)
C
...
C DEFERRED TAXES CR
C
         INCOME_VARIABLES(0,mthly_ltd_ps_income_tax_dfrls_cr) =
     +                                                  ANNUAL_VARS(681)
         INCOME_VARIABLES(1:,mthly_ltd_ps_income_tax_dfrls_cr) =
     +            INCOME_VARIABLES(0,
     +                       mthly_ltd_ps_income_tax_dfrls_cr)/12.
     ...
     +      + INCOME_VARIABLES(:,mthly_ltd_ps_income_tax_dfrls_cr)
     +      + INCOME_VARIABLES(:,Monthly NF Income Tax Deferrals Cr)
...
C
     ...
     +                 INCOME_VARIABLES(0,Monthly LTD Total Interest)
     +                 + LTD_BOOKED_ISSUED_INTEREST(0)
     +                 + ANNUAL_VARS(630) ! was 384 but only debit
            DO MO = 1, 12
...
C Federal Income Taxes Paid
     ...
     +                          Monthly Total Fed Income Tax Paid)))/9. 
     +                      - AVERAGE_VALUES(3)
     +                      - AVERAGE_VALUES(5)
     +                      - AVERAGE_VALUES(8)
         ELSEIF(KCPL()) THEN
...
C
     ...
     +                              + PS_ISSUE_YR_DIVIDEND_PAYMENT/2.
         ELSEIF(CLASS_TYPE == REGULATED_GROUP) THEN
            ...
            CASH_VARIABLES(7,Cash PS Issued) = PREF_STOCK_ISSD_loc
            CASH_VARIABLES(0,Cash PS Dividends) =
     +                                      PS_ISSUE_YR_DIVIDEND_PAYMENT
            CASH_VARIABLES(9,Cash PS Dividends) =
     +                                   PS_ISSUE_YR_DIVIDEND_PAYMENT/2.
...
C
     ...
     +                                 BOY_DEBIT_UNAMORT_INTEREST
               ...
               BALANCE_SHEET_VARIABLES(MO,
     +                              monthly_unamortized_issue_exp_bal) =
     +                                                 BOY_ISSUE_EXP_BAL
               BALANCE_SHEET_VARIABLES(MO,
...
C
     ...
     +                                    LIABS_NEC_BOY
      BALANCE_SHEET_VARIABLES(0,monthly_nucl_decommiss_liability) = 
     +                                  BOY_NUC_DECOM_FUND_LIABILITY
               BALANCE_SHEET_VARIABLES(0,
...
C 9/5/00 NEED TO ADD ACQUISTION OF NEW SUB.
     ...
     +               + INCOME_VARIABLES(MO,
     +                  mntly_dbt_file_nterest_exp_amrt_chg)
                  BALANCE_SHEET_VARIABLES(MO,
     ...
     +  - INCOME_VARIABLES(MO,monthly_common_stock_issue_amort)
     +               + INCOME_VARIABLES(MO,
     +     mnthly_chg_n_dbt_file_isue_exp_amrt)
                  BALANCE_SHEET_VARIABLES(MO,
     +                                  Monthly Subsidiary Investment) =
     +               BALANCE_SHEET_VARIABLES(MO-1,
     +                                    Monthly Subsidiary Investment)
...
C
     ...
     +                       - INCOME_VARIABLES(MO,
     +                            Monthly Deferred Revenue Amort)
     +                       + INCOME_VARIABLES(MO,
     +                             Monthly WVPA Accrued Member Revenues)
     +                       + CASH_VARIABLES(MO,
     +                              wvpa_cash_accd_mbr_revenue)
                  IF(SALT_RIVER_PROJECT()) THEN
      BALANCE_SHEET_VARIABLES(MO,monthly_nucl_decommiss_liability) =  
     ...
     +                        BALANCE_SHEET_VARIABLES(MO-1,
     +                              monthly_other_long_term_liabs)
     + - CASH_VARIABLES(MO,Cash Leased Nuclear Fuel)! - NUCLEAR_FUEL_LE
     + + CASH_VARIABLES(MO,Cash Lease Receipts)     ! + CAPITIALIZED_LE
     + - CASH_VARIABLES(MO,Cash Lease Expense)      ! - ATL_LEASE_PAYME
     + - CASH_VARIABLES(MO,Cash BTL Lease Cash)     ! - BTL_LEASE_PAYME
     ...
     +                        BALANCE_SHEET_VARIABLES(MO-1,
     +                              monthly_other_long_term_liabs)
     +                     - CASH_VARIABLES(MO,Cash Leased Nuclear Fuel)
     +                     + CASH_VARIABLES(MO,Cash Lease Receipts)     
     +                     - CASH_VARIABLES(MO,Cash Lease Expense)      
     +                     - CASH_VARIABLES(MO,Cash BTL Lease Cash)     
...
C
     ...
     +                               CLASS_TYPE == REGULATED_GROUP) THEN
                  ...
                  IF(VARIABLE == mnthly_investmt_erngs_rcvbl)
     +                                                             CYCLE
     ...
     +                                                             CYCLE
                  ...
                  IF(.NOT. EXCLUDE_SBU_PROPERTY_TAX) THEN
                     ...
                     IF(VARIABLE==Monthly Exp File Property Taxes) CYCLE
                     IF(VARIABLE ==
     +                    Monthly Model Calc Property Taxes) CYCLE
                  ENDIF
...
c
                  IF(VARIABLE ==
     +                 Cash Model Calculated Property Taxes) then
        CYCLE  ! 9/9/99 as fix for double paying
                       endif
                       
                  IF(VARIABLE == Cash Operating Revenue Tax) then
                    CYCLE  ! 9/9/99 as fix for double paying
                  endif
...
C
     ...
     +                                                SUB_LINK_2_CLASS))
                  ...
                  ENDIF
                  IF(VARIABLE ==
     +                  csh_ssd_st_txs_paid_by_pt) THEN
                     DO MO = 1, 12
                        IF(CLASS_IS_ACTIVE(MO)) THEN
                           CASH_LINKED_LEVEL(MO,
     +                       csh_ssd_st_txs_paid_by_pt,
...
C
     ...
     +                                   CLASS_LEVEL,
     +                                   ANNUAL_VARS)
        ns_class_parameters%INCOME_TAX_DEPRECIATION =  ANNUAL_VARS(130)
C
...
C ELIMINATE NA3 ODEC INCOME STATEMENT TRANSACTIONS
     ...
     +            + CASH_VARIABLES(:,Cash Exp File Property Taxes)
            DO VARIABLE = 1, CASH_VARS
               ...
               IF(VARIABLE == Cash Subsidiary Federal Taxes Paid ) CYCLE
               IF(VARIABLE == Cash Subsidiary State Taxes Paid ) CYCLE
               IF(VARIABLE==csh_ssd_st_txs_paid_by_pt)
     +                                                             CYCLE
               IF(VARIABLE == Cash Consolidate Taxes Paid) CYCLE
...
C
     ...
     +                  - INCOME_VARIABLES(MO,Monthly LTD Premium Amort)
                  ...
                  CYCLE
               ENDIF
               IF(VARIABLE == monthly_total_cptl_no_ct_ltd) THEN
...
C
     ...
     +                  - INCOME_VARIABLES(MO,
     +               mnthly_rtrmt_med_fund_earnings) ! 9/28/04 changes
                  RPT_BTL_TAXABLE_LTI_INCOME =
     +                     INCOME_VARIABLES(MO,
...
C
     ...
     +               INCOME_VARIABLES(MO,Total Base Revenues),
     +               INCOME_VARIABLES(MO,adjustment_clause),
     +               INCOME_VARIABLES(MO,secondary_sales),
...
C
     ...
     +    INCOME_VARIABLES(MO, 
     + monthly_expense_reserve_mgn_cap_purch), ! 215
     +    INCOME_VARIABLES(MO,
     +        monthly_income_rsv_mgn_cap_sls), ! 216
...
C BTL ITEMS ARE NOT OFFSET
     ...
     +       INCOME_VARIABLES(MO,Total Fuel Derivative Expenses), ! 90
C TAKE DEDUCTIONS
     +        ns_class_parameters%INCOME_TAX_DEPRECIATION/12.,! 91
     +        ANNUAL_VARS(228)/12., 
     +               ENVIROMENTAL_TAX,             ! 93
...
C
     ...
     +                - INCOME_VARIABLES(MO,
     +                         mnthly_rtrmt_med_fund_earnings) ! 9/28/04
     +                + INCOME_VARIABLES(MO,
...
C***********************************************************************
C
C ALLOCATED TO THE REVENUE TAX TO  MONTHS UNTIL SPECIFIC MONTHLY INFO 
! IS AVAILABLE
C
       use SpinDriftLib
...
C
C BALANCE NOLS GENERTED DURING THE YEAR WHICH WOULD OFFSET TAXABLE 
! INCOME
...
C***********************************************************************
C
C NOTE THAT THE VARIABLE POSITIONS ARE OFFSET BY 30 TO USE THE ANNUAL 
! LOCATION
C VALUES.
C
C
...
C
         BALANCE_SHEET_VARIABLES(:,
     +                          monthy_ltd_blnc_wo_ct_amt) =
     +           BALANCE_SHEET_VARIABLES(:,monthly_long_term_debt)
     +           - BALANCE_SHEET_VARIABLES(:,monthly_ltd_current_amount)
         BALANCE_SHEET_VARIABLES(:,
     +                           monthly_total_cptl_no_ct_ltd) =
     +           BALANCE_SHEET_VARIABLES(:,monthly_total_capital)
     +           - BALANCE_SHEET_VARIABLES(:,monthly_ltd_current_amount)
C
...
C
     ...
     +        + CASH_VARIABLES(R_MO,CashEarningsNonCorporate)
         IF(WVPA()) THEN
            SUM_CASH_PAYMENTS =
     +          SUM_CASH_PAYMENTS
     +        + CASH_VARIABLES(R_MO,WVPA_CMCOP) 
     +          + CASH_VARIABLES(R_MO,WVPA Cash NonMember Cost of Power)
     +    + CASH_VARIABLES(R_MO,WVPA Cash Member Cost of Services) ! 205
     +          + CASH_VARIABLES(R_MO,
     +                             WVPA_C_NMCOS) ! 206
     +          + CASH_VARIABLES(R_MO,
     +                              wvpa_cash_accd_mbr_revenue) ! 209
