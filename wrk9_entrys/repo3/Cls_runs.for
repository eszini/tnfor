C     Last change: MSG 8/9/2011 12:16:38 PM
...
C
      ...
      USE PROD_ARRAYS_DIMENSIONS
      use sizecom
      LOGICAL*1 FIRST_RECORD_OF_TABLE,INIT_CREATE_CLASS_LIST
      INTEGER*2 CREATE_CLASS_LIST,TABLE_NUMBER
      LOGICAL*1 R_SWITCH_TABLE_EXISTS,SWITCH_TABLE_EXISTS,END_OF_TABLE
      INTEGER*2 MAX_TABLE_NUMBER
      SAVE SWITCH_TABLE_EXISTS
      INTEGER*2 IREC,DELETE,LRECL/1024/,INUNIT,
     ...
     +            POST_COMMENT_VALUES(CLASS_RUN_SPECT_VALUES-13)
      ...
      CHARACTER*20 DEFERRED_FUEL_ACCOUNTING,
     ...
     +             PROPERTY_TAX_BASIS_VALUATION
      ...
      INTEGER*2 BC_TABLE_POSITION_FOR(:),
     +          PASS_TABLE_POINTER,INCREMENT_TABLE_NUMBER
      ALLOCATABLE :: BC_TABLE_POSITION_FOR
      INTEGER*2 MAX_OL_CLASS_NUMBER,MAX_BC_CLASS_NUMBER,R_CLASS_NUMBER,
     +          R_TABLE_POINTR(0:*),MAX_CLASS_IN_TABLE
      SAVE BC_TABLE_POSITION_FOR,MAX_BC_CLASS_NUMBER
      INTEGER*2 OL_TABLE_POSITION_FOR(:)
...
C***********************************************************************
C
C CONVERT THE FINANCIAL-PARAMETERS FILE
C***********************************************************************
...
C***********************************************************************
     ...
     +        STATUS="UNKNOWN",RECL=LRECL)
         ...
         CLASS_NAME = 'Parent'
         CLASS_NUM = 0
         FIRST_RECORD_OF_TABLE = INIT_CREATE_CLASS_LIST(INT2(-1))
...
C                                                                      C
C                  INIT_CLASS_RUN_SWITCH_FILE                          C
C        Copyright (c) 1995 M.S. Gerber & Associates, Inc.             C
C                      All Rights Reserved                             C
C                                                                      C
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C                                                                      C
C     Purpose:  This subroutine reads the asset class                  C
C               run switch file for each class by year.                C
C                                                                      C
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C
...
C
      ...
      INTEGER*2 CLASS_RUN_SPECT_VALUES
      PARAMETER (CLASS_RUN_SPECT_VALUES=32) ! CHANGE=LAST VAR NUM - 5
      CHARACTER*20 SWITCH_SETTINGS(CLASS_RUN_SPECT_VALUES),
     +             CLASS_NAME*32,
     +             CLASS_TYPE*1,TABLE_TYPE*1
      LOGICAL*1 SWITCH_FILE_TABLES_ACTIVE,R_CALCULATE_AMT,
     +          R_USE_BUDGET_EXPENSES
      INTEGER*2 YEAR,CLASS,MAX_CLASS_NUM,RUN_SWITCH_UNIT/441/
      INTEGER*2 MAX_SWITCH_FILE_CLASS_NUMBER,
     ...
     +          USE_RETAIN_POST_RETIREMENT_EARNINGS
      CHARACTER*1 REVENUE_TAX_BASIS,R_REVENUE_TAX_BASIS
      INTEGER (KIND=2), SAVE :: PARENT_TABLE_NUMBER,
     +         SUB_DEFAULT_TABLE_NUMBER,
     +         SBU_DEFAULT_TABLE_NUMBER,
     +         REG_DEFAULT_TABLE_NUMBER
      LOGICAL*1 USE_APPRAISED_PROP_VALUES,R_USE_APPRAISED_PROP_VALUES
...
C
            ...
            ENDIF
c           COVERAGE_RATIO = INDEX('123456789',SWITCH_SETTINGS(10)(1:1))
c            IF(COVERAGE_RATIO == 1 .AND. SWITCH_SETTINGS(10)(2:2)=='0')
c     +                                               COVERAGE_RATIO = 10
c            IF(COVERAGE_RATIO == 1 .AND. SWITCH_SETTINGS(10)(2:2)=='1')
c     +                                               COVERAGE_RATIO = 11
     ...
     +                                     SWITCH_SETTINGS(31)(1:1)=='Y'
         ...
         ELSEIF(YEAR ==1) THEN
            ...
            USE_RETAIN_POST_RETIREMENT_EARNINGS = .TRUE.
            PROPERTY_TAX_BASIS_VALUATION = 'A'
         ENDIF
...
C
      ...
      RETURN
C***********************************************************************
      ENTRY RETURN_RETAIN_POST_RETIREMENT_EARNINGS(
     +                            R_USE_RETAIN_POST_RETIREMENT_EARNINGS)
C***********************************************************************
C
         R_USE_RETAIN_POST_RETIREMENT_EARNINGS = .true.
c     +                               USE_RETAIN_POST_RETIREMENT_EARNINGS
      RETURN
      END
