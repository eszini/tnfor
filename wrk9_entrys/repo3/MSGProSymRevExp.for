...
C
      ...
      INTEGER*2, PARAMETER :: ProSymDataItems=20,
...
C PROCESS BASE FILE
         ...
         StartYr = MIN(StartYr,BASE_YEAR()+1)  ! provides for miss match of run years
         BaseStartYr = StartYr
         BaseEndYr = EndYr
...
C
     ...
     +         + ProSymRevAndExps(0,Yr,ASSET_CLASS,total_fixed_om_cost)
         R_VARIABLE_OM = R_VARIABLE_OM
     +       + ProSymRevAndExps(0,Yr,ASSET_CLASS,total_variable_om_cost)
...
C
     ...
     +             + ProSymRevAndExps(0,Yr,ASSET_CLASS,Revenue Capacity)
      RETURN
C***********************************************************************
      ENTRY RETURN_ProSymMonthlyRevsExpsBook(R_YEAR,
     +                                       R_ASSET_CLASS,
     +                                       INC_MONTH_VARS)
C***********************************************************************
C
...
C ADD REVENUES
C
         INC_MONTH_VARS(:,Monthly Secondary Sales) =
     +             INC_MONTH_VARS(:,Monthly Secondary Sales)
...
C ADD REVENUES
C
            CASH_MONTH_VARS(:,Cash Secondary Sales) =
     +             CASH_MONTH_VARS(:,Cash Secondary Sales)
     +             + ProSymRevAndExps(:,Yr,ASSET_CLASS,Revenue Energy)
     +             + ProSymRevAndExps(:,Yr,ASSET_CLASS,MCP Revenue)
     +             + ProSymRevAndExps(:,Yr,ASSET_CLASS,
     ...
     +             + ProSymRevAndExps(:,Yr,ASSET_CLASS,
     +                                             Revenue Pumping Cost)
            CASH_MONTH_VARS(:,Cash Other Revenue) =
     +             CASH_MONTH_VARS(:,Cash Other Revenue)
     +             + ProSymRevAndExps(:,Yr,ASSET_CLASS,
     +                                        Revenue Ancillary Service)
            CASH_MONTH_VARS(:,Cash Relationship Revenues) =
     +             CASH_MONTH_VARS(:,Monthly Relationship Revenues)
     +             + ProSymRevAndExps(:,Yr,ASSET_CLASS,
     +                                             Revenue Availability)
            CASH_MONTH_VARS(:,Cash Capacity Sales) =
     +             CASH_MONTH_VARS(:,Monthly Capacity Sales)
     +             + ProSymRevAndExps(:,Yr,ASSET_CLASS,Revenue Capacity)
C
C ADD EXPENSES
C
            CASH_MONTH_VARS(:,Cash Fossil Fuel) =
     +             CASH_MONTH_VARS(:,Cash Fossil Fuel)
     +             + ProSymRevAndExps(:,Yr,ASSET_CLASS,Total Fuel Cost)
     +             + ProSymRevAndExps(:,Yr,ASSET_CLASS,Total Start Cost)
            CASH_MONTH_VARS(:,Cash Fixed OandM) =
     +          CASH_MONTH_VARS(:,Cash Fixed OandM)
     +          + ProSymRevAndExps(:,Yr,ASSET_CLASS,total_fixed_om_cost)
            CASH_MONTH_VARS(:,Cash Variable OandM) =
     +       CASH_MONTH_VARS(:,Cash Variable OandM)
     +       + ProSymRevAndExps(:,Yr,ASSET_CLASS,total_variable_om_cost)
            CASH_MONTH_VARS(:,Cash Emission Credits) =
     +           CASH_MONTH_VARS(:,Cash Emission Credits)
     +           + ProSymRevAndExps(:,Yr,ASSET_CLASS,NOx Emissions Cost)
     +           + ProSymRevAndExps(:,Yr,ASSET_CLASS,SO2 Emissions Cost)
         ENDIF
