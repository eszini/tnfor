...
C LOAD INFORMATION TRANSACTIONS
     ...
     +         ! HOUR,SYSTEM; REAL before 20030429
     +         SYSTEM_HOURLY_LOADS(:,:)
      ALLOCATABLE ::
     +         SUM_USER_W_PRICES,
     +         MAX_USER_W_PRICE,
...
!
     ...
     +      HOURLY_MARGINAL_FUEL
      ...
      ! TMS 20041129 NUM_PRODUCTS increased by 2 to 15
      PARAMETER ( NUM_PRODUCTS=15)
...
C
     ...
     +       EXPECTED_SALE,PROB_2_SERVE_B4_SALES
C
...
!
     ...
     + ! ADDED FOR DISPATCHABLE HYDRO.
     +         TRANS_GROUP_SELLER_CAP,TRANS_GROUP_BUYER_CAP,
     +         TRANS_GROUP_CAP,
...
! A TO/FROM REPORT IS NOT AVAILABLE FOR MARKET
! PRICE METHOD (ONLY TRANSACT AGAINST MARKET)
...
!
      IF(USE_TRANSACT_LOADS .OR. USE_TF_FILE) THEN
         ! READS LDE FOR EACH TRANS LOAD GROUP.
         CALL READ_LOAD_DATA(HOURS_IN_MONTH,ISEAS)
      ENDIF
!
...
! READ MONTHLY MARKET PRICE FILE LDE<5 CHAR>.BXY
! WHERE XY = 01, 02, 03 FOR EACH YEAR.
...
!     ****************************************************************
      ...
      RETURN
!     ****************************************************************
      ENTRY GET_DAILY_OPTION_PAYOFF(R_STRIKE_PRICE,R_DAY,
     + R_DAILY_PAYOFF)
!     ****************************************************************
!
! DAILY PAYOFF SHOULD BE CALCULATED BEFORE HAND TO SAVE TIME,
! UNLESS WE GET INTO RAMPING, WHERE INTER-HOUR MW CHANGES ARE
! IMPORTANT.
!
         R_DAILY_PAYOFF = 0.
         START_HOUR = (R_DAY-1)*24 + 1
...
!
     ...
     +                                YEAR,UPPER_TRANS_GROUP,END_POINT)
...
!
               TG = GET_TRANS_GROUP_INDEX(I)
               DATA_BASE = GET_DATA_BASE_FOR_TRANS(TG,DAY_TYPE)
               IF(.NOT. TRANS_GROUP_GEN_ACTIVE(DATA_BASE)) THEN
...
!
                  ...
                  DATA_BASE = GET_DATA_BASE_FOR_TRANS(TG,DAY_TYPE)
                  IF(.NOT. TRANS_GROUP_GEN_ACTIVE(DATA_BASE))then
                    CYCLE ! DATA BASE NOT DEFINED IN THIS HOUR
                  endif
!
...
!     ****************************************************************
     ...
     +                                             SALES_ENERGY(R_MONTH)
      ...
      RETURN
!     ****************************************************************
      entry GetTransPowerSold(iTG,SellerTransPower) !positive for seller
!     ****************************************************************
...
C
!
! TO REDUCE ARRAY SIZES, REPORTING IS MONTHLY
...
! SOURCE CAN SUPPLY THAT RESOURCE MORE CHEAPLY, THEN IT WILL BUY FROM
! THAT ENTITY.
!
...
! EQUALS
! MARGINAL COST.  IF THE ALGORITHM CORRECTLY FINDS THE GREATEST TO
! SMALLEST
! NET MARGINS, THE MARKET PRICE SHOULD BE THE PRICE OF THE LAST
! TRANSACTION
...
!     4. MAKE SURE THAT THE SALE DOES NOT EXTEND BEYOND THE NEGATIVE
! VALUE
!
...
!
     ...
     +                                                          r8LoadB4
               ...
               ELSE
                  ...
                  M_HOURLY_LOAD_B4_SALES(I,HOUR_IN_DAY) =
     +                                              HOURLY_FORWARD_SALE
! CHANGED SIGN ON TRANS_ROR_CAPACITY 9/14/99. GAT.
...
!
            DO ! DO UNTIL TRANSACTIONS ARE NO LONGER ECONOMIC
               ! OR PHYICALLY POSSIBLE
!
!
...
!
!
! CALCULATE DELIVERED COST OF CURRENT TRANSACTION GROUP, I,
! TO OTHER GROUPS J
!
                  IF(HOURLY_TRANS_GROUP_GEN_ACTIVE(I)) THEN
                     SELLER_DB =
...
! 081307. TEMP !!!
     ...
     +                        HOURLY_TRANS_GROUP_LOAD_ACTIVE(I)) THEN
                        ! DOES THE OTHER SIDE OF THE DIAGONAL
!
...
!
     ...
     +                                                     BUYER_DB,L_M)
                  ...
                  ENDIF
!
! THE AMOUNT THAT THE BUYER IS ABLE TO PRODUCE AT THE SELLERS COST
! SHOULD
! BE LESS THAN OR EQUAL TO THE AMOUNT THAT THE BUYER IS ABLE PRODUCE
! AT ITS OWN COST.
! IE SELLERS_COST + TIE_COST... < BUYERS_COST AND BUYERS_COSTS
! ARE NON-DECREASING.
!
                  TEMP_BUYER_TRANSACTION =
...
!
     ...
     +                              SECOND_BEST_MARGIN)
               ...
               ELSE
!
                  EXIT ! NO TRANSACTIONS POSSIBLE AFTER CALCULATING
                       ! MARGINS. GO ON TO THE NEXT HOUR.
!
               ENDIF
!
               HOURLY_DUMP_USED = 0.
...
!
!
! ASSUME FUNCTIONS ARE APPROXIMATELY LINEAR AND SOLVE FOR THE
! INTERSECTION
! OF THE TWO LINES. THE TWO LINES HAVE COMMON MINIMUM AND MAXIMUM PRICES
! (I THINK).
! FURTHER ASSUME THAT THE BUYER IS A TRADITIONAL DOWNWARD SLOPING
! DEMAND CURVE,
...
! RECALCULATED PRICES ARE TO THE PRICES ESTIMATED BY THE INTERSECTED
! LINES.
!
!
!
...
!
     ...
     + TEMP_INTERP_TRANSACTION > 0.d0) THEN ! ADDED FOR SRP. 8/24/99. 
                  TEMP_TRANSACTION = TEMP_INTERP_TRANSACTION
                  TRANSACTION_COMPLETED = .TRUE.
                  CONVERGENCE_PATH = 3.
...
!
!
! DO UNTIL TRANSACTIONS ARE NO LONGER ECONOMIC OR PHYICALLY POSSIBLE
            ENDDO 
            CALL RW_PROCESS_MESSAGES()
!
! 1/3/3.
