...
C
     ...
     + STOP_AT_MATURITY,VECTOR_FOUND,FirstEnergy
...
C
         IF(R_CLASS <= MAX_ASSET_CLASS_NUM) THEN
            ...
            IF(ASSET_CLASS > 0 .OR. R_CLASS == 0) THEN
               YR = RUN_YEAR ! - 1
               DO MO = 0, 12
                  TAX_VARIABLES(MO,monthly_ltd_ps_tax_issue_expense) =
...
C
         IF(R_CLASS <= MAX_ASSET_CLASS_NUM) THEN
            ...
            IF(ASSET_CLASS >= -1) THEN
               YR = RUN_YEAR ! - 1
               INCOME_VARIABLES(:,BTL Monthly Total Other Income) =
     +               INCOME_VARIABLES(:,BTL Monthly Total Other Income)
     +                + OTHER_INCOME(:,YR,ASSET_CLASS)
               INCOME_VARIABLES(:,BTL Monthly Interest Income) =
     +                  INCOME_VARIABLES(:,BTL Monthly Interest Income)
     +                   + INTEREST_INCOME(:,YR,ASSET_CLASS)
               INCOME_VARIABLES(:,Monthly LTD Total Interest) =
     +                   INCOME_VARIABLES(:,Monthly LTD Total Interest)
     +                    + INTLTD(:,YR,ASSET_CLASS)
               INCOME_VARIABLES(:,Monthly LTD Booked Interest) =
                         ! used for elimination purposes
     +                   INCOME_VARIABLES(:,Monthly LTD Total Interest)
               INCOME_VARIABLES(:,Monthly Total PS Dividends) =
     +                   INCOME_VARIABLES(:,Monthly Total PS Dividends)
     +                    + INTPS(:,YR,ASSET_CLASS)
               INCOME_VARIABLES(:,Monthly Debt File Interest Amort) =
     +                    INCOME_VARIABLES(:,
     +                                Monthly Debt File Interest Amort)
     +                    + LTD_ISSUE_EXP_AMORT(:,YR,ASSET_CLASS)
     +                    - LTD_PREM_AMORT(:,YR,ASSET_CLASS)
               INCOME_VARIABLES(:,Monthly LTD Premium Amort) =
     +                    INCOME_VARIABLES(:,Monthly LTD Premium Amort)
     +                     + LTD_PREM_AMORT(:,YR,ASSET_CLASS)
               INCOME_VARIABLES(:,Monthly PS Amort Dividends) =
     +                   INCOME_VARIABLES(:,Monthly PS Amort Dividends)
     +                    + PS_ISSUE_EXP_AMORT(:,YR,ASSET_CLASS)
     +                    - PS_PREM_AMORT(:,YR,ASSET_CLASS)
               INCOME_VARIABLES(:,Monthly PS Premium Amort) =
     +                     INCOME_VARIABLES(:,Monthly PS Premium Amort)
     +                      + PS_PREM_AMORT(:,YR,ASSET_CLASS)
C
               INCOME_VARIABLES(:,Monthly Issue Expense Amorts) =
     +                 INCOME_VARIABLES(:,Monthly Issue Expense Amorts)
     +                  + LTD_ISSUE_EXP_AMORT(:,YR,ASSET_CLASS)
     +                  + PS_ISSUE_EXP_AMORT(:,YR,ASSET_CLASS)
     +                  + CS_ISSUE_EXP_AMORT(:,YR,ASSET_CLASS)
               INCOME_VARIABLES(:,monthly_common_stock_issue_amort)=
     +             INCOME_VARIABLES(:,monthly_common_stock_issue_amort)
     +              + CS_ISSUE_EXP_AMORT(:,YR,ASSET_CLASS)
     ...
     +              INCOME_VARIABLES(:,Monthly Regular Dividend Income)
     +               + INVESTMENT_INCOME(:,YR,ASSET_CLASS,Dividend)
               INCOME_VARIABLES(:,Monthly Interest on Notes Payable)=
     +            INCOME_VARIABLES(:,Monthly Interest on Notes Payable)
     +             + NOTES_PAYABLE(:,YR,ASSET_CLASS,Interest Payments)

              INCOME_VARIABLES(:,Monthly Debt File LInvestment Income)=
     +               INCOME_VARIABLES(:,
...
C
     ...
     +               + NOTES_RECEIVABLE(:,YR,ASSET_CLASS,
     +                                          Cash Interest Earnings)
               CASH_VARIABLES(:,Cash Interest on Notes Payable) =
     +                 CASH_VARIABLES(:,Cash Interest on Notes Payable)
     +                  + NOTES_PAYABLE(:,YR,ASSET_CLASS,
     +                                          Cash Interest Payments)
               CASH_VARIABLES(:,Cash Change Debt Investments) =
     +                  CASH_VARIABLES(:,Cash Change Debt Investments)
     +                  + INVESTMENT_INCOME(:,YR,ASSET_CLASS,Additions)
     +                 - INVESTMENT_INCOME(:,YR,ASSET_CLASS,Reductions)
               CASH_VARIABLES(:,Cash Net Investments) =
     +                      CASH_VARIABLES(:,Cash Net Investments)
     +                      + LOANS_TO_AFILLIATES(:,YR,ASSET_CLASS)
     +                     + INVESTMENT_IN_AFILLIATES(:,YR,ASSET_CLASS)
...
C
     ...
     +                                                    R_LTD_INT/12.
      ...
      RETURN
! *******************************************************************
      ENTRY MONTHLY_LTD_FULL_YEAR(RUN_YEAR,R_CLASS,R_LTD_BAL,
...
C
      ...
      RETURN
! *******************************************************************
      ENTRY MONTHLY_PS_FIRST_HALF(RUN_YEAR,R_CLASS,R_PS_DIV,
     + R_PS_RETIRE)
! *******************************************************************
C
         IF(R_CLASS <= MAX_ASSET_CLASS_NUM) THEN
...
C     COMPUTES A NEW AMORTIZATION SCHEDULE BASED UPON THE BALANCE IN
C     EFFECT ON THE REVERSE ANNIVERSARY OF THE MATURITY DATE (HERE
C     WE USE THE REVERSE ANNIVERSARY ON ADVICE FROM TIM PRYOR THAT:
C     SOME USERS MAY FEEL THAT IN CASES OF THE ISSUE DATE BEING IN THE
C     HISTORIC PAST, THEY ARE NOT BOUND TO ACCURATELY INPUT THE ISSUE
C     DATE; BUT IN FACT THEY DO DESCRIBE THE MATURITY DATE ACCURATELY)
C
C           TREAT THIS DEBT ISSUE AS A VARIABLE-RATE MORTGAGE,
C           WITH PRINCIPAL AND INTEREST VARYING OVER ITS LIFE;
C           AT MOST 4 CASES MUST BE ACTIVELY CONSIDERED:
C           00) ISSYR IS NOT < (BASE_YR+1) AND
C               YRMAT IS NOT > (BASE_YR-1+FINANCIAL_SIMULATION_YEARS)
C           01) ISSYR IS NOT < (BASE_YR+1) AND
C               YRMAT IS     > (BASE_YR-1+FINANCIAL_SIMULATION_YEARS)
C           10) ISSYR IS     < (BASE_YR+1) AND
C               YRMAT IS NOT > (BASE_YR-1+FINANCIAL_SIMULATION_YEARS)
C           11) ISSYR IS     < (BASE_YR+1) AND
C               YRMAT IS     > (BASE_YR-1+FINANCIAL_SIMULATION_YEARS)
C           [ CASES OF YRMAT < (BASE_YR+1)
C               OR ISSYR > (BASE_YR-1+FINANCIAL_SIMULATION_YEARS)
C                                                       ARE DEAD ISSUES,
C             OUTSIDE THE RANGE OF YEARS MODELED;
C             CASES OF ISSYR >= YRMAT ARE IGNORED AS SPURIOUS DATA ]
C           CASES 00 AND 01 CALL REMORT TO SCHEDULE THE AMORTIZATION
C           OF THE ORIGINAL, NOMINAL BALANCE (AMT);
C           CASES 10 AND 11 CALL REMORT IN THE FIRST MODEL YEAR (I=2)
C           TO SCHEDULE THE AMORTIZATION OF THE REDUCED, END-OF-BASE_YR
C           BALANCE (BYRBAL);
C           NOTE THAT BELOW, CASES 00 AND 01 ARE TREATED UNDER (2),
C           WHILE            CASES 10 AND 11 ARE TREATED UNDER (1);
C           THE DIFFERENCE IN TREATMENT OF YRMAT BETWEEN CASES 00 AND 10
C         ON THE ONE HAND, AND CASES 01 AND 11 ON THE OTHER, IS THAT THE
C           ARRAYS OF RESULTS ARE TRUNCATED WHEN YRMAT >
C                            BASE_YR-1+FINANCIAL_SIMULATION_YEARS;
C           REGARDLESS OF WHETHER YRMAT FALLS BEYOND THE MODEL PURVIEW,
C  THE INT, SINKING_FUND, AND BAL ARRAYS ARE OVER-WRITTEN WITHIN BOUNDS.
C
C           REAMORTIZE THE MORTGAGE WHEN THE CURRENT YEAR (YR)
C           IS NOT BEYOND THE MATURITY YEAR (YRMAT) AND EITHER
C           1)  THIS YR IS THE FIRST MODEL YEAR (WITH I=2)
C               AND THE ISSUE YEAR (ISSYR) PRECEDES YR, OR
C           2)  THIS YR IS THE ISSUE YEAR, OR
C           3)  THIS YR'S INTEREST RATE DIFFERS FROM THE PRIOR RATE
...
C             SINKING_FUND(I) = [ value retained from prior invocation ]
            ENDIF
         ENDIF
CNEXT COMPUTE THE ANNUAL 12/31/YR PAYMENT NEEDED TO AMORTIZE BAL(I) ...
         IF(NEWRAT /= 0.) THEN
            IN1231 = BAL(I) * NEWRAT
...
C        BAL(I) & SINKING_FUND(I) retain prior mid-year values; payment
! is due in 1 year
...
C        ACCOUNT FOR THE FRACTIONAL OVERFLOW INTO THE YEAR OF MATURITY
      ...
      ELSEIF (I == 2) THEN
C        THE MORTGAGE SECURITY EXISTED IN THE BASE_YEAR, AND EXPIRES 
! THIS YEAR
