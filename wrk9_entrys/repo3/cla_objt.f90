      ...
      ! 120218 ! 081818 ! 022510 ! 052609 1068 ! 011809 1041! 091107
      ! 1033! 100506 for KCPL ! 080906 ! /961/
      INTEGER*2 IREC,INUNIT,DELETE,LRECL/1159/
      INTEGER (kind=2) :: UNIT_NO,R_ACTIVE_CL_RECORDS
      INTEGER (kind=2) ::MONTHLY_CAPACITY_POINTER,I
      INTEGER (kind=2) ::MONTHLY_FUEL_INDEX
      ...
      CHARACTER (len=2) :: UNIT_TYPE
      CHARACTER (len=2) :: UNIT_TYPE_CATEGORY
...
! END 5/14/92 CHANGES
      ...
      REAL :: FRAOWN,FUELMX,PRFCST,SEFCST,EFOR_loc,FUELADJ,MAINRT(12), &
       VARCST, &
...
! ADDED 12/9/92 FOR SOUTHERN COMPANY &
       ...
       INTEGER*2 SEC_EMISS_PTR,EMISS_FUEL_ESCAL, &
...
! MULTI-CL FILE VARIABLES
      ...
      INTEGER*2 NUC_RECORD_POSITION(200,0:MAX_CL_FILES-1),ONLINE,OFLINE
...
! first read
                  ...
                  EXPENSE_ASSIGNMENT,TEMP_EXPENSE_COLLECTION,GENGRP, &
                     ...
                     SEFCST,SDESCR,FUELADJ,EFOR_loc, &
      ...
      call end_program("cla_objt:0009 - capacity_market_pointer " // &
       ...
       WRITE(11,REC=IREC) DELETE,UNIT_NAME,CL_LOAD_TYPE, &
        EXPENSE_ASSIGNMENT,EXPENSE_COLLECTION, GENGRP,FRAOWN, &
        ONLIMO,ONLIYR,OFLIMO,OFLIYR,FUELMX,PRFCST,PRESCR, SEFCST, &
        SDESCR,FUELADJ,EFOR_loc,MAINRT,VARCST,VRESCR, FIXED_COST, &
       FIXED_COST_ESCALATOR, DISPADJ,HRFACT,MW,PLANNING_FACTOR,COEFF, &
       AI_CAPACITY_RATE,AI_CAPACITY_ESCALATOR,  &
       AI_ENERGY_RATE,AI_ENERGY_ESCALATOR, AI_REMAINING_LIFE, &
       P_SO2,P_NOX_BK1,P_CO2,POOL_FRAC_OWN, P_OTHER2,P_OTHER3, &
       PHASE_I_STR,DISP_ADJ2_OUT, res_id_loc,P_FUEL_SUPPLY_ID, &
       P_NOX_BK2_OUT, SEC_EMISS_PTR,EMISS_FUEL_COST,EMISS_FUEL_ESCAL, &
       EMISS_FUEL_EMISS_PTR,EMISS_BLENDING_RATE, PRIM_FUEL_EMISS_PTR, &
       PRIM_FUEL_TYPE,FUEL_TYPES,TIE_GROUP, MONTHLY_CAPACITY_POINTER, &
       ANNUAL_CL_FIXED_COST, ANNUAL_CL_FIXED_COST_ESC, &
       MAINT_DAYS,DISPATCH_MULT, EXCESS_ENERGY_SALES, AI_TAX_LIFE, &
       AI_ADR_TAX_LIFE, ASSET_CLASS_NUM, ASSET_CLASS_VECTOR, &
       INTRA_COMPANY_CLASS_ID, INTRA_COMPANY_TRANSACTION, &
       SPECIAL_UNIT_ID, MINIMUM_CAPACITY_FACTOR, &
       MAXIMUM_CAPACITY_FACTOR, TRANSACTION_GROUP_ID, DAY_TYPE_ID, &
       UNIT_ACTIVE, BASECASE_PLANT_ID, BASECASE_UNIT_ID, &
       BASECASE_MARKET_AREA_ID, BASECASE_TRANS_AREA_ID, &
       BASECASE_PLANT_NERC_SUB_ID, BASECASE_PLANT_OWNER_ID, &
       UTILITY_OWNED, MONTHLY_FUEL_INDEX, START_UP_COSTS, &
       START_UP_LOGIC, RAMP_RATE, MIN_DOWN_TIME, REPORT_THIS_UNIT, &
       P_FUEL_DELIVERY, P_FUEL_DELIVERY_2, P_FUEL_DELIVERY_3, &
       MIN_UP_TIME, HESI_UNIT_ID_NUM, FOR_FREQUENCY, &
       FOR_DURATION, WINTER_TOTAL_CAPACITY, CONTRIBUTES_TO_SPIN, &
       H2_UNIT_ID_NUM, POWERDAT_PLANT_ID, APPLY_NOX_SEASON_DATE, &
       NOX_SEASON_DATE, RAMP_DOWN_RATE, NOX_CONTROL_PERCENT, &
       NOX_CONTROL_DATE, EMERGENCY_CAPACITY, EMERGENCY_HEATRATE, &
       MARKET_RESOURCE, PRIMARY_MOVER_STR, COUNTY, STATE_PROVINCE, &
       NOX_VOM, NOX_FOM, S_FUEL_DELIVERY, s_fuel_delivery_2, &
       s_fuel_delivery_3, CONSTANT_POLY, FIRST_POLY, SECOND_POLY, &
       THIRD_POLY, USE_POLY_HEAT_RATES, NEWGEN_UNIT_STATUS, &
       MONTHLY_MUST_RUN_VECTOR, EMISSION_MARKET_LINK, &
       WVPA_RATE_TRACKER, ALLOW_DECOMMIT, MIN_SPIN_CAP, MAX_SPIN_CAP, &
       WVPA_RES_TRACKER, WVPA_FUEL_TRACKER, MONTE_OR_FREQ, &
       WVPA_MEM_TRACKER, MONTHLY_DECOMMIT_VECTOR, TRANS_BUS_ID, &
       SOX_CONTROL_PERCENT, SOX_CONTROL_DATE, SOX_VOM, SOX_FOM, &
     LATITUDE, LONGITUDE, MW_INSIDE_FENCE, INTER_BLOCKS,  &   ! 152-154
 FIRST_YEAR_DECOMM_AVAIALABLE, DECOMMISSIONING_BASE_YR_COST,   & !  156
       DECOMMISSIONING_COST_ESCALATION, &
       ANNUAL_ENERGY_PLANNING_FACTOR, AGGREGATE_THIS_UNIT, & !  159
       NAME_PLATE_CAPACITY, FUEL_BLENDING_IS, FuelRatio, &
       BlendableFuelsPtr, & !  167
       FuelTransportationCost, BlendedEnthalpyUp, &
       BlendedEnthalpyLo, BlendedSO2Up, & !  178
       BettermentProjectID, DECOM_CONTINUING_COST, &
       DECOM_CONTINUING_COST_ESCALATION, EnrgPatternPointer, & !  181
       EMISSION_DATA_UNITS, & !  182
       EmissRedRate, & !  183-187
       EmissMaxRate, MaxEnergyLimit, &
       TECH_TYPE,LINKED_BETTERMENT_OPTION, &
       CO2_CONTROL_PERCENT,HG_CONTROL_PERCENT,OTHER3_CONTROL_PERCENT, &
       CO2_CONTROL_DATE, HG_CONTROL_DATE, OTHER3_CONTROL_DATE, &
     CO2_VOM, CO2_FOM, HG_VOM, HG_FOM, OTHER3_VOM, OTHER3_FOM, & !  209
     MARKET_FLOOR, MARKET_CEILING, START_UP_COSTS_ESCALATION,  & !  212
       CAPACITY_MARKET_TYPE, CAPACITY_MARKET_MONTH, &
       capacity_market_pointer_ocl, CAPACITY_MARKET_COST_ASSIGN, &
       CAPACITY_MARKET_EXP_COLLECT, CAPACITY_MARKET_COIN_ADJ_FACT, &
       PRIMARY_FUEL_CATEGORY, SECONDARY_FUEL_CATEGORY, &
       EMISSIONS_FUEL_CATEGORY, RPS_CONTRIBUTION_PERCENT, &
       RETIREMENT_CANDIDATE, RETROFIT_CANDIDATE, RETROFIT_PROJECT_ID, &
       CO2_BASIN_NAME, CO2_PIPELINE_DISTANCE, STATE_PROV_NAME, & !  228
       CO2_RETRO_HEAT_MULT, CO2_RETRO_CAP_MULT, THERMAL_GUID, &
       THERMAL_PARENT_ID, & !  232
       THERMAL_AGGREGATED_UNIT, FIRST_RETIREMENT_YEAR, &
       UNIT_TYPE, UNIT_TYPE_CATEGORY, RPS_PROGRAM_NUMBER
               ENDDO
               IF(IOS /= 0) EXIT !debugstop
            ENDDO
      ...
      IF(NUCLEAR_UNITS_ACTIVE > 0) THEN
         ...
         DO FILE_ID = 0, MAX_CL_FILES-1
          ...
          GENGRP,FRAOWN,ONLIMO,ONLIYR,OFLIMO,OFLIYR,FUELMX, &
           PRFCST,PRESCR,SEFCST,SDESCR,FUELADJ,EFOR_loc,MAINRT, &
...
! ***************************************************************
      ...
      DO
         DO
           ...
           PRESCR,SEFCST,SDESCR,FUELADJ,EFOR_loc,MAINRT,VARCST,VRESCR, &
               FIXED_COST,FIXED_COST_ESCALATOR, &
               DISPADJ,HRFACT,MW,PLANNING_FACTOR,COEFF, &
       ...
       ios_base=ios_base ! debugstop
            ...
            IF(IOS == 0) THEN
               ...
               READ(RECLN,*,ERR=200) DELETE,UNIT_NAME,CL_LOAD_TYPE, &
                  ...
                  PRESCR,SEFCST,SDESCR,FUELADJ,EFOR_loc,MAINRT,VARCST, &
...
! ***************************************************************
 ...
 1010 FORMAT('&',A)
      ...
      subroutine RETURN_MONTHLY_CL_REVENUES(R_CLASS,MONTH_VARS)
         use cla_decs
         use namescom
         implicit none
  ...
  IF(.NOT. ALLOCATED( &
    ns_cla_decs%MON_MDS_CL_CLASS_PURCHASES))THEN
    RETURN
 endif
    ...
    + ns_cla_decs%MON_MDS_CL_CLASS_FUEL_COST( &
           ...
           ns_cla_decs%ASSET_CLS_LOC,MO)
            ...
            ENDIF
         ENDIF
      end subroutine RETURN_MONTHLY_CL_REVENUES
! **************************************************************
...
!     PRODCOM.MON AND PROD2COM.MON FILES.
      ...
      integer :: min_tmp, nunits_tmp
      real (kind=4) :: MONTH_VARS(0:12,1:*)
      !real (kind=4) :: month_vars(:,:)
...
! FIX TO MAINTENANCE SCHEDULER. 3/8/1.
      ...
      REAL*4 :: TRANS_DISPATCH_EMIS_ADDER
...
! ADDL EXPANSION
      ...
      INTEGER*2 J,BASE_DATE,GET_YR,R_I,R_J,LAST_ACTIVE_UNIT, &
        DATE_CHECK, &
...
! ADDITONS TO CREATE CL FILE OF RESOURCES ADDTIONS 12/06/06 DR.G
      ...
      INTEGER (KIND=2) :: END_POINT_CHECK=-999, &
                          SEQU_NO
      integer :: bypass_invalid_delimiter
...
! END OF DATA DECLARATIONS
      ...
      ALLOCATE(CL_TG_CAP(0:6,0:MAX(1,UPPER_TRANS_GROUP), &
       STUDY_PERIOD,2))
...
!     TRANSACTION GROUPS
      ...
      DO FILE_ID = 0, MAX_CL_FILES-1
       ...
       FIRST_YEAR_DECOMM_AVAIALABLE(nunits), & 
       DECOMMISSIONING_BASE_YR_COST(nunits), &
       DECOM_CONTINUING_COST_ESCALATION(nunits), &
       ANNUAL_ENERGY_PLANNING_FACTOR(nunits), &
       AGGREGATE_THIS_UNIT(nunits), & 
       ...
       FuelRatio(4,nunits), &
       FuelRatio(5,nunits), &
       BlendableFuelsPtr(1,nunits), &
       BlendableFuelsPtr(2,nunits), &
       BlendableFuelsPtr(3,nunits), &
       BlendableFuelsPtr(4,nunits), &
       FuelTransportationCost(1,nunits), &
       FuelTransportationCost(2,nunits), & 
       ...
       START_UP_COSTS_ESCALATION(nunits), &
       CAPACITY_MARKET_TYPE(nunits), &
       CAPACITY_MARKET_MONTH(nunits), &
       ns_cla_decs%capacity_market_pointer_acl(nunits), &
       ...
       if(cla_bad(int( &
        ns_cla_decs%capacity_market_pointer_acl(max_cl_units)) &
        )) then
...
!      ! After first array read
       ...
       " array contains invalid value.")
         ...
         foshyd%CAP_FRAC_OWN(NUNITS) = 100.0

               FUEL_MIX_PTR(nUNITS) = 1.
               PFESCR(nUNITS) = 0.
               SBTUCT(nUNITS) = 0.
               SFESCR(nUNITS) = 0
               FUELADJ(nUNITS) = 0.
...
! THIS CREATES A MIDAS DATA READABLE CL FILE
        ...
        EXPENSE_COLLECTION(NUNITS), &
                  GENGRP(NUNITS), &
       foshyd%CAP_FRAC_OWN(NUNITS),ON_LINE_MONTH, &
                 ON_LINE_YEAR,OFF_LINE_MONTH(nunits), &
       OFF_LINE_YEAR(nunits), &
       FUEL_MIX_PTR(nunits), &
       PBTUCT(nunits), &
...
! End TODO: Check for duplicate name
       ...
       TRANSACTION_GROUP_ID(nunits))
            ...
            IF(INPUT_MW(2,nunits) > 1. &
       .AND. MW_INSIDE_FENCE > .1) THEN
...
! 12/05/01
       ...
       IF(trim(SPECIAL_UNIT_ID(nunits)) == &
         'BayShore') THEN
                  FIRST_ENERGY = .TRUE.
               ENDIF
            ENDIF
            ...
            NOX_SEASON_DATE(nunits) =  &
       NOX_SEASON_DATE(nunits) + 10000

            NOX_CONTROL_PERCENT(nunits) = &
                   MIN(100.,MAX(0.,1.- &
       NOX_CONTROL_PERCENT(nunits)/100.))
            NOX_CONTROL_DATE(nunits) = &
       NOX_CONTROL_DATE(nunits) + 10000

            SOX_CONTROL_PERCENT(nunits) = &
                   MIN(100.,MAX(0.,1. &
       -SOX_CONTROL_PERCENT(nunits)/100.))
            SOX_CONTROL_DATE(nunits) = &
       SOX_CONTROL_DATE(nunits) + 10000

            CO2_CONTROL_PERCENT(nunits) = &
                   MIN(100.,MAX(0.,1.-CO2_CONTROL_PERCENT( &
       nunits)/100.))
            HG_CONTROL_PERCENT(nunits) = &
                   MIN(100.,MAX(0.,1.-HG_CONTROL_PERCENT( &
       nunits)/100.))
            HG_CONTROL_DATE(nunits) = &
       HG_CONTROL_DATE(nunits) + 10000
            OTHER3_CONTROL_PERCENT(nunits) = &
                   MIN(100., &
       ...
       PRIM_FUEL_TYPE_STR(1:1)
            ...
            ELSE
               ...
               IF(CL_LOAD_TYPE(2:2)=='S') then
                ECONOMY_TRANS_TYPE(nunits)='S'
               endif
               IF(CL_LOAD_TYPE(2:2)=='B') then
                ECONOMY_TRANS_TYPE(nunits)='B'
               endif
...
! NEED TO SAVE PRICE INFORMATION
       ...
       SBTUCT(nunits)
            FUELADJ_SAVE(nunits) = &
       FUELADJ(nunits)
            VCPMWH_IN_SAVE(nunits) = &
       VCPMWH_IN(nunits)
            FIXED_COST_SAVE(nunits) = &
       FIXED_COST_IN(nunits)
            ANNUAL_CL_FIXED_COST_SAVE(nunits) = &
                 ANNUAL_CL_FIXED_COST(nunits)
            DISPADJ_SAVE(nunits) = &
       DISPADJ(nunits)
            DISPADJ2_SAVE(nunits) = &
       DISPADJ2(nunits)
            DISPATCH_MULT_SAVE(nunits) = &
       DISPATCH_MULT(nunits)
            EXCESS_ENERGY_SALES_SAVE(nunits) = &
                       EXCESS_ENERGY_SALES(nunits)
            CL_AI_CAPACITY_RATE_SAVE(nunits) = &
                        CL_AI_CAPACITY_RATE(nunits)
            CL_AI_ENERGY_RATE_SAVE(nunits) = &
       CL_AI_ENERGY_RATE(nunits)
            CL_AI_REMAINING_LIFE_SAVE(nunits) = &
                         AI_CL_REMAINING_LIFE(nunits)
            DISPADJ2_SAVE(nunits) = &
       DISPADJ2(nunits)
            EMISS_FUEL_COST_SAVE(nunits) = &
       EMISS_FUEL_COST(nunits)
      ...
      ECONOMY_TRANS_TYPE(nunits) = &
       ECONOMY_TRANS_TYPE(I)
                  CL_CAP_AREA_LINKED(nunits) = &
       ...
       START_UP_INDEX(nunits) = TOTAL_START_UP_UNITS
            ...
            IF(nunits > MAX_CL_UNITS) THEN
               ...
               CALL MG_LOCATE_WRITE(21,0, &
                                 'nunits are in the capacity-limited', &
                                                        ALL_VERSIONS,1)
...
! **************************************************************
         ...
         IF(R_MWH < .001) THEN
            ! RESET TO 0 ITER
            RETIREMENT_MEASURE = ITER_0_ECON_RETIRE_MARGIN(R_NUNITS)
...
! **************************************************************
         ...
         IF(.NOT. GRX_CONVERGED) THEN
            !RESET THE nunits RETIRED IN THE LOOP
...
&!  NO, FIXED BLEND, MODEL DETERMINED IF ONLY ONE FUEL ELSE USER OPTION
       ...
       "bad value: " // &
       trim(itos(int(ns_cla_decs%capacity_market_pointer_acl(nunits)))))
...
! 10/03/02. FOR REGIONAL CONSOLIDATION. NOTE REASSIGNMENT.
        ...
        GET_BELONGS_TO_GROUP(TRANSACTION_GROUP_ID(nunits))
            ...
            FIRST_RETIREMENT_YEAR(nunits) = &
                   100*(FIRST_RETIREMENT_YEAR(nunits) - 1900)
            CAP_MARKET_MONTH_NO(nunits) = &
                CAP_MARKET_MONTH_NO_INDEX(CAPACITY_MARKET_MONTH &
       (nunits))
! CONVERT TO PERCENT

            IF(INPUT_MW(2,nunits) > 1. .AND. &
       MW_INSIDE_FENCE > .1) THEN
               PERCENT_INSIDE_FENCE = MIN(1., &
                         MW_INSIDE_FENCE/INPUT_MW(2,nunits))
            ELSE
               PERCENT_INSIDE_FENCE = 0.
...
! ADDED SO THAT SP COMPATIBLE CL FILE OUTPUT
       ...
       TRANSACTION_GROUP_ID(nunits))
            ...
            IF(((CAPACITY_PLANNING_METHOD() == 'MX' .AND. &
             ...
             "ns_cla_decs%capacity_market_pointer_acl(" // &
         ...
         TRIM(CONVERT_2_STR(MONTHLY_MUST_RUN_VECTOR(nunits)))
          ...
          TRIM(CONVERT_2_STR(MONTHLY_DECOMMIT_VECTOR(nunits)))
           ...
           TRIM(CONVERT_2_STR(NAME_PLATE_CAPACITY(nunits)))    ! 160
               RECLN = TRIM(RECLN)//','// &
                 TRIM(CONVERT_2_STR(FUEL_BLENDING_IS(nunits)))
               RECLN = TRIM(RECLN)//','// &
      ...
      TRIM(CONVERT_2_STR( &
       ns_cla_decs%capacity_market_pointer_acl(nunits)))
               RECLN = TRIM(RECLN)//','// &
      TRIM(CONVERT_2_STR(CAPACITY_MARKET_COST_ASSIGN(nunits)))
               RECLN = TRIM(RECLN)//','// &
                  TRIM(CONVERT_2_STR( &
                        CAPACITY_MARKET_EXP_COLLECT(nunits)))
               RECLN = TRIM(RECLN)//','// &
...
! A+BX+CX^2
       ...
       COEFF(1,nunits)*.001
            ...
            OFF_LINE_YEAR(nunits) = UNIT_ON_LINE_YEAR + &
       OPERATION_LIFE
      ...
      ONLINE(nunits) = 100*(ON_LINE_YEAR-1900) + ON_LINE_MONTH
       ...
       HG_CONTROL_DATE(nunits) + 10000
            ...
            IF(NUMBER_OF_RESOURCE_ID(LOCAL_RESOURCE_ID) == 0) THEN
               ! FIRST OCCURRENCE
               RESOURCE_ID_TO_UNIT(LOCAL_RESOURCE_ID) = nunits
            ENDIF
            NUMBER_OF_RESOURCE_ID(LOCAL_RESOURCE_ID) = &
                           NUMBER_OF_RESOURCE_ID(LOCAL_RESOURCE_ID) + 1
...
! 2/13/96 CORRECTION FOR SHADOW nunits
...
! are compressed; nunits above needed only for properly dimensioning
! MAINT_INDEX_MONTHLY_MW; if subscript order on MAINT_INDEX_MONTHLY_MW
! were reversed, we could avoid passing nunits, making MAINT_SCHEDULER
...
! 070806. ADDED EXTRA CONDITION
       ...
       ! DO I REALLY NEED TO DO DETAILED REGARDLESS OF DETAILED SWITCH?
               ...
               ENDDO ! K COUNTER FOR ACTIVE nunits
...
! **************************************************************
         ...
         DO UNUM = 1, nunits
            ...
            IF(INTRA_COMPANY_CLASS_ID(UNUM) >= 0 .AND. &
               ...
               ns_cla_decs%ASSET_CLS_LOC = INTRA_COMPANY_CLASS_ID(UNUM)
    ...
    MON_MDS_INTRA_COMPANY_NF_BURN(ns_cla_decs%ASSET_CLS_LOC,R_ISEAS) + &
            ...
            ns_cla_decs%ASSET_CLS_LOC = ASSET_CLASS_NUM(UNUM)
...
! MARKET REVENUE PROXY FOR UNION EL nunits 8/7/97
...
!  CHECK nunits, KATHY TO PASS EXAMPLE
...
! **************************************************************
      ...
      RETURN
! **************************************************************
      ENTRY CLA_RETURN_UNITNM(R_NUNITS,R_NAME)
! **************************************************************
...
! DECLARATIONS FOR THE EXPANSION PLANT OPERATIONS FILE
      ...
      INTEGER*4 &
                     ...
                     POWERDAT_PLANT_ID
      REAL*4         CAP_FRAC_OWN_ord,S_FUELMX(:),S_PBTUCT(:), &
                     S_SBTUCT(:), &
                     S_FUELADJ_IN(:),S_EFOR(:), &
                     S_FUELADJ(:), &
                     S_MNRATE(:,:), &
      ...
      integer (kind=2), allocatable :: S_EMISSION_MARKET_LINK(:)
      INTEGER (kind=2) :: &
...
! READ THE CORRESPONDING EXPANSION PLANT RECORDS
         ...
         inquire(unit=13, NAME=file_name)
               call write_log_entry("cla_objt:0019", &
       "Reading " // trim(just_the_filename(file_name)) // &
       " (unit 13) ")
        ...
        TEMP_APPLY_NOX_SEASON_DATE, & 
            ...
            if(cla_ord_bad(int(capacity_market_pointer_ocl))) then
               call end_program("cla_objt:00013 - capacity_market" // &
       "_pointer contains invalid value after read.")
            endif
...
! **************************************************************
       ...
       int2(S_FIXED_COST_IN(S)), &
       int2(S_FIXED_COST_ESCALATOR(S)))
            S_ANNUAL_CL_FIXED_COST(S) = &
       get_escalated_value(int2(S_ANNUAL_CL_FIXED_COST(S)), &
       S_ANNUAL_CL_FIXED_COST_ESC(S))

       S_PBTUCT(S) = get_escalated_value(int2(S_PBTUCT(S)), &
       int2(S_PFESCR(S)))
       S_SBTUCT(S) = get_escalated_value(int2(S_SBTUCT(S)), &
       int2(S_SFESCR(S)))

       S_VCPMWH_IN(S) = get_escalated_value(int2(S_VCPMWH_IN(S)), &
       int2(S_OMESCR(S)))
...
! 01/11/02. FOR NEW TG RESERVE MARGIN REPORT.
            ...
            IF(TG < 1 .OR. TG > GET_NUMBER_OF_ACTIVE_GROUPS()) THEN
              WRITE(4,*) "BAD TRANSACTION GROUP ID IN THERMAL nunits", &
                                                        UNITNM(UNIT_NO)
            ENDIF
