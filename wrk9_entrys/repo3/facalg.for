...
!     ******************************************************************
      ...
      use end_routine, only: end_program
        ...
        write(PrtUni,ErrFmt)'error:  ',Prefix,Reason,Suffix
        if(PrtDetail>0) then
          ErrorsTrapped=1 ! until an error-coding system is devised
          ! and implemented
...
!       sorts iSup items into order based on values in key (1-based)
! array a;
...
! Addison-Wesley 1981
     ...
     +           (Decreasing.and.(a(p)>=a(q)))) then
      ...
      subroutine GetOneDigit(i,i0,i1) ! useful only during debugging
        integer*2 i,i0,i1,i2 ! i0,i1 are lower and upper bounds on i,
                             ! respectively
        character*1 GetCh,AnyChar

        do ! while(.true.)
          call Tone2(1200,600,33) ! prompt for input with a
                                  ! 2-tone sequence
...
!5 for additional option of keying in an integer number of calls to
! skip,
!after which the tone prompts for a 1-digit keystroke on [0,4] re above
     ...
     +        'Level of print detail=',PrtDetail
        ...
        if(ErrorsTrapped==0) then
             ! calls AllocFAInt & AllocAuxes
            call StorePlantData(
     +    UNIT,BLKNO,NBLOK2,NUM_ARB_CONSTRAINTS,NUM_CUMULANTS,
     +    LOWER_BOUND_CAP_FACTOR,
     +    UPPER_BOUND_CAP_FACTOR,MWBLOK,
     +    eavail_loc,MAINTENANCE_RATE,NUNITS)
        endif
...
!     end of routines known by MinimizeCost's caller
      ...
      ! returns the integral power of a real base
      real*4 function IntegralPower(x,n)
        real*4 x
        integer*2 n
...
!       parameter constants
     ...
     +    HalfMaxLn=43.695)
        ...
        if(t<-HalfMaxLn) then
           ! prevent divide overflows by restricting GaussianDen
...
! arguments; source is NBS Handbook of Math Functions, paragraph 26.2.16
      ...
      ! subroutine ends after all entry-points end
      recursive subroutine FacetAlgorithm 
     ...
     + ! 0 to inhibit extraction of abnormal modes from load curve
     +    LimAbnModes=0) 
        real*8 TailUQ
        real*4
     +    InfRHS,xLoadPoint,xLoadPtInf,MeanDxLoad,MeanLoad,SDLoad,
     +    ChgCapac,CapConv,
     + ! eq-load ordinates with 0 outages included
     +    EqLDur(0:LimEqPoints), 
...
!       variables used only by entry-points below, in order of use:
     ...
     +    Fmt8f='(1x,i3,i4,i2,4f7.4,f9.3,2f9.3,f9.1)')
        real*4 CostUQ,CapacUQ,RHS,SupRHS,SupCost,SysCap_loc,Capaci,
     +    BlkCoeff,SlackCoeff,
...
!    +    yAtxShifted,yExAtxShifted,
     ...
     +    Fmtf10='(1x,i3,i3,i2,f10.3,a,3f11.4)')
        real*4 Capac,efor_loc,EqAv,xShifted,yAtxShifted,yInterpolate,
...
!       nStgEqn=0 !! 1=> crash on call #3
     ...
     +       (    LOWER_BOUND_CAP_FACTOR(i)<eavail_loc(i)).and.
     ...
     +    +nCFLBVar  ! count of vars having a non-zero lower-bound on CF
        ...
        call initw1(jOfCallers,nVarUB,InvalidIndex) 
...
!         InputRate(j)=BlockHeatRate(iUnit,max(BlockPos(j),1))
          EquivAvail(j)=eavail_loc(iUnit) ! use availability to limit capacity factor:
          ...
          if(PrOutage==0.0) then ! all cumulants are zero
            ...
            ! the formula involving Sterling numbers:
              ...
              do k=2,nKappa
                Sterling(1,k)=1
                Sterling(k,k)=1
                NCMoment(k)=PrOutage*real(nIdent)
                km1=k-1
