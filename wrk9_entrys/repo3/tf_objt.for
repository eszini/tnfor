...
C
     ...
     +                  MONTH_VARS(MO,secondary_sales) + .000001 *
     +                   MONTHLY_AC_CONTRACT_REVENUE(ASSET_CLASS,RI,MO,
     +                                               secondary_sales,1)
...
!
!
      CURRENT_YEAR = get_BASE_YEAR() + YEAR
!
C
      BASE_ADJUSTMENT = -999.
...
C******************************************************************
     ...
     +      MONTHLY_TRANS_LOAD_HYDRO_MWH(TRANS_POSITION)
         ELSE
...
C******************************************************************
C
C     A ROUTINE TO PRODUCE CALENDAR CORRECT HOURLY MARGINAL COSTS
...
!
     ...
     +                     ACCESS="DIRECT",STATUS="UNKNOWN",RECL=LRECL)
      IREC = 0
!
c the makebin code for this file forces the year_ord variable to be 
! based on
c the base year_ord.  When doing overlays the overlay process does 
! not happen
c because of the year_ord check. The year_ord check isn't needed. 
! For this
...
C******************************************************************
     ...
     +               SCEN_MAKER_VARIABLE(SCENARIO_INDEX,R_MONTH,R_YEAR)
      ...
      RETURN
C******************************************************************
      ENTRY GET_SCENARIO_ENERGY(R_TEMP_YEAR,R_MONTH) 
C******************************************************************
         IF(.NOT. SAVE_SCENARIO_MAKER_STATUS) THEN
