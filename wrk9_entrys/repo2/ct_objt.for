...
C DECLARATION FOR CONTRACTS DATA
     ...
     +      CT_ANNUAL_FIXED_COST
      INTEGER*2 CNTR_ONLI_MO,CNTR_ONLI_YR,
     ...
     +      CT_ANNUAL_FIXED_COST_ESC 
      INTEGER*2 RESOURCE_ID,ENERGY_PATTERN_VECTOR
      CHARACTER*1 CNTRTYPE,CNTR_EXP_ASSIGN,
     +            CNTR_CAPACITY_SWITCH,CNTR_ENERGY_SWITCH
      CHARACTER*18 CNTR_EXP_COLLECT
      CHARACTER*20 INTRA_REVENUE_CLASS
      CHARACTER*20 CNTRNM
      CHARACTER*18 FILE_TYPE/'Purchase Contracts'/
      CHARACTER*2 CONTRACT_OL/'BC'/
...
C***********************************************************************
C
C CONVERT THE PURCHASE-CONTRACTS FILE
      ENTRY CT_MAKEBIN
C
...
C         RESOURCE_ID = 0
     ...
     +         CNTRNM,CNTRTYPE,CNTR_EXP_ASSIGN,
     +         CNTR_EXP_COLLECT,CNTR_OWN,CNTR_POOL,
     ...
     +         MIN_CONTRACT_FIXED_COST,CNTR_MIN_CAP_ESC,
     +         CONTRACT_FIXED_COST,CNTR_MAX_CAP_ESC,
     ...
     +         RESOURCE_ID,ENERGY_PATTERN_VECTOR,
...
C
      ...
      RETURN
C OVERLAY THE PURCHASE-CONTRACTS FILE
...
C
     ...
     +              CONTRACT_OL//"CNTCT.BIN",ACCESS="DIRECT",RECL=LRECL)
         ENDIF
         R_ACTIVE_CT_RECORDS = MAX(ACTIVE_CT_RECORDS,0)
         R_MAX_CT_ID_NUM = MAX(MAX_CT_ID_NUM,0)
      RETURN
!
 1000 FORMAT(A)
 1010 FORMAT('&',A)
...
C***********************************************************************
C
C          SUBROUTINE TO READ PURCHASE CONTRACTS BINARY DATA FILES
...
C
      ...
      SAVE
C
      INTEGER*2 I,IREC,DELETE,CONTRACTS_READ,YEAR_START,YEAR_END
      INTEGER IOS,R_CAP_TYPE
      INTEGER*2 R_YEAR
      REAL CT_PLANNING_CAPACITY,CT_DENOM_LOAD_REDUCTION
      INTEGER*2 CT_CAPACITY_PLANNING_ADDITIONS,R_I,VOID_I2
      LOGICAL*1 CT_OPTION_CHECK,RESOURCE_DOES_NOT_REDUCE_LOAD,
     +          SAVE_CT_LOAD_REDUCTION
      INTEGER*2 R_CONTRACT_NUM
C     CHARACTER*20 MONTH_NAME
C
      INTEGER*2 ADDED_CONTRACT_PROGRAMS(*),
     +          CONTRACT_OPTIONS_ADDED
C
C     VARIABLES FOR CAPACITY PLANNING
C
      INTEGER*2 YR
      INTEGER*2 PEAK_MONTH,CT_RECORDS,MAX_CT_ID_NUM
      REAL EN_MW,GET_CT_MW_FROM_POINTR
      INTEGER*2 ON_LINE_MONTH(:),ON_LINE_YEAR(:),
     +          OFF_LINE_MONTH(:),OFF_LINE_YEAR(:),
     +          CT_ID_TO_RECORD_POINTER(:)
      INTEGER*2 R_CONTRACT_START_YEAR,R_CONTRACT_PERIOD,ACCEPTANCE_NUM,
     +          R_CT_ID_NUM
     ...
     +               CT_ID_TO_RECORD_POINTER
      INTEGER*2 RESET_CONTRACT_ADDITIONS,
...
C
     ...
     +               CONTRACT_SECONDARY_SALES_REVS
      ...
      INTEGER*2 ASSET_CLASS_LIST(AVAIL_DATA_YEARS),CLASS_POINTER,
     +          ASSET_CLASS
      REAL*4 ASSET_ALLOCATION_LIST(AVAIL_DATA_YEARS),ASSET_ALLOCATOR
      LOGICAL*1 CALC_CONTRACT_CLASS_INFO,REALLY_KEPCO
      INTEGER*2 REV_TYPE,INCOME_STATEMENT_POSITION
      LOGICAL*1 WKP_ACTIVE,WEST_KOOTENAY_POWER
      REAL*4 CNTR_WEIGHTED_CAPACITY(*)
C
      REAL*4 R_FIXED_COST,
     +       R_VAR_COST,R_PURCHASE_COST,R_BTL_EXPENSES,
     +       R_ADJ_CLAUSE_COLLECTION,R_SECONDAY_SALES_REVENUE
      INTEGER*2 RETURN_CONTRACT_CLASS_INFO,R_CLASS_NUM
      LOGICAL*1 RETURN_CONTRACT_INTRA_INFO
...
C
     ...
     +          CNTRNM(I),CNTRTYPE(I),CNTR_EXP_ASSIGN(I),
     +          CNTR_EXP_COLLECT(I),CNTR_OWN(I),CNTR_POOL(I),
     +          ON_LINE_MONTH(I),ON_LINE_YEAR(I),
     +          OFF_LINE_MONTH(I),OFF_LINE_YEAR(I),
     +          CNTR_SO2(I),CT_FIRST_ENERGY_PRICE(I),CNTR_ENER_ESC(I),
     +          MIN_ENERGY(I),MAX_ENERGY(I),
     +          MIN_CONTRACT_FIXED_COST(I),CNTR_MIN_CAP_ESC(I),
     +          CONTRACT_FIXED_COST(I),CNTR_CAP_ESC(I),
     +          MIN_CAPACITY(I),MAX_CAPACITY(I),
     +          CNTR_CAPACITY_SWITCH(I),
     ...
     +          CNTR_ENERGY_SWITCH(I),
     +          CT_RESOURCE_ID(I),CT_ENERGY_PATTERN_VECTOR(I),
     +          CT_SECOND_ENERGY_PRICE(I),
     +          CT_SECOND_ENERGY_ESCALATOR(I),
     +          CT_ENERGY_COST_WEIGHTING_FACTOR(I),
     ...
     +          INTRA_ASSET_CLASS_ID(I),
     +          INTRA_ASSET_CLASS_ALLOCATION(I),
...
C
C THIS SECTION ADDS MW'S TO THE SYSTEM
C
               IF(EN_MW < 0.) THEN
     ...
     +                        CT_ANNUAL_LOAD_REDUCTION(YR) + EN_MW
            ...
            ENDDO
C
C END OF CAPACITY PLANNING SECTION
C
         ENDIF
C
C
...
C***********************************************************************
         ...
         ON_LINE_YEAR(I) = MIN(MAX(R_CONTRACT_START_YEAR,1901),2200)
...
C                    FOR KEPCO MAINTENANCE ENERGY IS IN WC MAINTENANCE
                  ELSE
