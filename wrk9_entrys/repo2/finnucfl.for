...
C  FILE NUCFUEL.FOR 2/16/84
C***********************************************************************
C
C          SUBROUTINE TO CALCULATE NUCLEAR FUEL VALUES
C          COPYRIGHT (C) 1983 M.S. GERBER & ASSOCIATES, INC.
C                     ALL RIGHTS RESERVED
C
C          MIDSA GOLD CAPABILITIES ADDED JUNE, 1991
...
C
      ...
      USE SIZECOM
C
      INTEGER*2 IREC,AFDC_CAP_VECTOR,DELETE,ACCTNO,INUNIT,LRECL/512/
      INTEGER IOS
      CHARACTER*5 BASE_FILE_NAME,OVERLAY_FAMILY_NAME,NUCFLFIL
     ...
     +          MAX_BC_NUC_FUEL_ID_NUM/0/
      INTEGER*2 R_UNIT_NUM
      INTEGER*2 BC_NUC_FUEL_CLASS_POINTER(:),
...
C     ADDED MIDAS GOLD 7/24/91
      INTEGER*2 CASH_VECT,PLANT_VECT
C DECLARATION FOR INPUT VARIABLES FOR NUCLEAR ASSETS
      REAL NFIP1,NFIPRB,NFIS1,AMORT1
      CHARACTER*30 FUEL_OWNERSHIP
      CHARACTER*21 FILE_TYPE/'Nuclear Fuel Accounts'/
      CHARACTER*2 NFUEL_OL/'BC'/,R_NFUEL_OL
C
C 7/15/96 VARIABLES ADDED FOR OE
C
      INTEGER*2 GENERATOR_ID
      REAL*4 REGION_BALANCE(4),REGION_AMORT_RATE(4),
     +       REGION_CAP_ALLOCATION(4)
      REAL*4 LEASOR_MARK_UP,ALLOCATION_TO_OWNER
      LOGICAL*1 R_NUC_FUEL_VERS_5,NUC_FUEL_VERS_5/.FALSE./
      LOGICAL*4 FILE_OPENED
      CHARACTER*20 FORM_TYPE
...
C***********************************************************************
C
C CONVERT THE NUCLEAR FUEL FILE
      ENTRY NF_MAKEBIN
...
C***********************************************************************
C
C OVERLAY THE NUCLEAR FUEL FILE
...
C***********************************************************************
C
C NOTE: 6/30/94 AFDCRT,AFDCBR,INTEREST_CAP_RATE, 
C CURRENT_INTEREST_CAP_RATE ARE ONLY PASSED TO AFUDC
C
      use SpinDriftLib
      use prod_arrays_dimensions
      ...
      SAVE
C
      LOGICAL*1 SAVE_BASE_CASE,VOID_LOGICAL,RETURN_ASSET_CLASS_LISTS
      INTEGER*2 FINANCIAL_SIMULATION_YEARS/0/,RUN_YEARS,EXTENSION_YEARS
      INTEGER*4 VALUES_TO_ZERO
C
      REAL ASSET_CLASS_LIST(:)
      REAL ASSET_ALLOCATION_LIST(:)
...
C NUCLEAR FUEL RATEBASE ADDERS
...
C
      INTEGER*2 NUM_OF_ASSET_CLASSES,MAX_ASSET_CLASS_NUM,
     +          ASSET_CLASS_POINTER(:)
      ALLOCATABLE :: ASSET_CLASS_POINTER
C
      INTEGER*2 R_ABMETH,R_ABYEAR
...
C
      ...
      INTEGER*2 LASTYR
C
      INTEGER*2 I,J,IREC,IEND,IOFFSET,DELETE,TACCTS,
...
C
     ...
     +               LEASOR_NF_BOOK_DEPRECIATION
C
      REAL CWIP(:),NFIS(:),NNFV(:),TAXEXP(:),NFCE(:),NFES(:),AFDC1(:),
...
C
      ...
      CHARACTER*30 DESC
C
      INTEGER*2 NUM_OF_NUC_UNITS,R_NUM_OF_NUC_UNITS
      INTEGER*2 ID_POINTER(:)
      REAL*4 GENERATION_BY_INSERVICE_YEAR(:,:),GET_VAR
      ALLOCATABLE :: ID_POINTER,GENERATION_BY_INSERVICE_YEAR
      LOGICAL*1 OFFSET_MAINTENANCE_VECTORS,OFFSET_MAINTENANCE_ACTIVE
      INTEGER*2 DAYS_IN_EACH_MONTH,NUC_UNITS_FOUND,YR,MO,
     +          DATE,GET_YR,RELOAD_YEAR
      INTEGER*2 R_ON_LINE,R_OFF_LINE
      INTEGER*2 R_UNIT_ID
      REAL*4 R_UNIT_CAPACITY,R_EFOR,R_MAINTENANCE_RATE(12),R_FRAC_OWNED
      REAL*4 MAINTENANCE_RATE(:,:,:),ENRG_BY_FISCAL_YEAR(:,:,:),
     +       HOURS_GENERATION,ANNUAL_EFOR,ANNUAL_CAPACITY
      ALLOCATABLE :: MAINTENANCE_RATE,ENRG_BY_FISCAL_YEAR
      REAL*4 MW_CAPACITY,MONTHLY_MW,MONTHLY_UNIT_CAPACITY(:,:),
     +       FRAC_OWNED
      LOGICAL*1 FUEL_PRICE_FROM_NF_FILE(:)
      ALLOCATABLE :: MONTHLY_UNIT_CAPACITY,FUEL_PRICE_FROM_NF_FILE
      INTEGER*2 R_MONTHLY_CAPACITY_POINTR
      CHARACTER*(*) R_UNIT_NAME
      LOGICAL*1 REMAINING_GENERATION_FOUND 
      INTEGER*2 NUC_FUEL_BURN_YEARS,REGIONS_IN_CORE
      REAL*4 ENRG_GENERATION
      INTEGER*2 POS
C
C 7/15/96 VARIABLES ADDED FOR OE
C
      INTEGER*2 GENERATOR_ID,BUNDLE_POINTER
      REAL*4 OPENING_REGION_BALANCE(4),REGION_AMORT_RATE(4),
     +       REGION_CAP_ALLOCATION(4),
     +       LEASOR_MARK_UP,
     +       ALLOCATION_TO_OWNER,
     +       R_ALLOCATION_TO_OWNER(*),
     +       OWNER_ALLOCATION(MAX_FINANCIAL_SIMULATION_YEARS)
      REAL*4 NUC_FUEL_COST(:,:,:),CURRENT_RATE,CAPITIALIZED_FUEL
      REAL*4 ANNUAL_LEASOR_MARK_UP(:,:),R_MARK_UP
      ALLOCATABLE :: NUC_FUEL_COST,ANNUAL_LEASOR_MARK_UP
      INTEGER*2 GEN_UNIT,MO_START
      LOGICAL*1 CALUCLATE_NUC_FUEL_PRICES,USE_NUC_FUEL_FILE_PRICES,
     +          NUC_FUEL_VERS_5,R_NUC_GEN_UNIT_FOUND
C
      CHARACTER*4 YEAR_STR,LOAD_NAME(4)*7
      LOGICAL*1 NUC_FUEL_REPORTING,NUCLEAR_FUEL_REPORT,
     +          REPORT_ALL_ACCOUNTS
      INTEGER*2 NUC_REPORTING_UNIT,NUC_FUEL_REGION_RPT
      INTEGER NUC_REPORTING_REC
      LOGICAL*1 REPORT_HEADER_OPEN/.FALSE./
      REAL*4 PRE_REGION_BURN(4),PRE_LOAD_BURN(4),PRE_LOAD_CLOSE(4),
     +       FUELING_START_MONTH,POST_REGION_AMORT_RATE(4),
     +       POST_REGION_BURN(4),POST_REGION_BALANCE(4)
      REAL*4 TOTAL_ENRG,AVERAGE_BURN_RATE,
     +       AFUDC_IN_CWIP(MAX_FINANCIAL_SIMULATION_YEARS)
      LOGICAL*1 HALF_YEAR_BOOK_DEP
      REAL*4 MMBTU_PRICE
      INTEGER*2 R_UNIT_POS,R_MO,R_YR
      CHARACTER*6 R_FUEL_PRICE_SOURCE
      REAL*4 R_OWNED_NF_SL_TAX_DEP
      REAL*4 ALLOCATION_VALUE(AVAIL_DATA_YEARS)
      REAL*4 CLASS_ALLOCATOR
      INTEGER*2 ALLOCATION_VECTOR
      CHARACTER*40 LAG_STR,CASH_STR
      CHARACTER*1 ACCOUNT_ACTIVE
      LOGICAL*1 LF95,LAHEY_LF95
      REAL (KIND=4) :: MONTHLY_UNIT_EFOR(12)
c      SAVE FUEL_OWNERSHIP
c      SAVE NF_SL_TAX_DEPRECIATION
c      SAVE DESC
c      SAVE NUM_OF_NUC_UNITS
c      SAVE ID_POINTER,GENERATION_BY_INSERVICE_YEAR
c      SAVE OFFSET_MAINTENANCE_ACTIVE
c      SAVE MAINTENANCE_RATE,ENRG_BY_FISCAL_YEAR
c      SAVE MONTHLY_UNIT_CAPACITY,FUEL_PRICE_FROM_NF_FILE
c      SAVE NUC_UNITS_FOUND
c      SAVE NUC_FUEL_BURN_YEARS
c      SAVE OPENING_REGION_BALANCE,REGION_AMORT_RATE,
c     +     REGION_CAP_ALLOCATION,
c     +     LEASOR_MARK_UP
c      SAVE NUC_FUEL_COST,ANNUAL_LEASOR_MARK_UP
c      SAVE CALUCLATE_NUC_FUEL_PRICES
c      SAVE NUC_REPORTING_UNIT,NUC_FUEL_REPORTING
c      SAVE NUC_REPORTING_REC
c      SAVE ASSET_CLASS_LIST,ASSET_ALLOCATION_LIST
c      SAVE ASSET_CLASS_POINTER,NUM_OF_ASSET_CLASSES,MAX_ASSET_CLASS_NUM
c      SAVE TNNFV,TDDB,TDBAMR,TRBDDB,TNFRB,TNFIP,
c     +     TRBFIP,TNFCE,TAFC1C,TNFIS,TAFC2C,TNFES,
c     +     TAFDC2,TAFDC1,TEXEXP,TTXEXP,TAFEXP,TAFCAJ,
c     +     TWODFT,TAFDCB,TAFDCD,TNFCE_LEASED,TPCAPINRST,
c     +     JDBAMR,JPCAPINRST,JTXEXP,
c     +     RBCWIP_AFDC_METH2,RB_AFDC1,
c     +     OWNED_NF_TAX_DEP,
c     +     OWNED_NF_SL_TAX_DEPRECIATION,
c     +     LEASOR_NF_TAX_DEP,
c     +     LEASOR_NF_DEFERRED_TAX_BASIS,
c     +     LEASOR_NF_SL_TAX_DEPRECIATION,
c     +     LEASOR_NF_BOOK_DEPRECIATION
c      SAVE CWIP,NFIS,NNFV,TAXEXP,NFCE,NFES,AFDC1,
c     +     AFDC1C,AFDC2,RBCWIP,DDB,RBDDB,AFCEXP,
c     +     AJAFDC,EXEXP,DBAMRT,NFRB,AFDC2C,WODFTX,
c     +     AFDC1B,AFDCDF,CAPINRST,PCAPINRST,
c     +     NF_TAX_DEP,
c     +     NF_DEFERRED_TAX_BASIS,
c     +     NF_BOOK_DEPRECIATION
C
...
C
     ...
     +                          ASSET_ALLOCATION_LIST(AVAIL_DATA_YEARS))
      CALL OPEN_NUCLEAR_FUEL_FILE(INT2(11))
...
C
     ...
     +                 AFUDC_IN_CWIP)
C
C CALCULATE BOOK ITEMS FOR NUCLEAR FUEL
C   INITIALIZE VALUES
C
            NFIS(1)   = NFIS1
            AFDC1C(1) = AFDC1(1)
...
C
            ...
            ENDDO
C
C OWNED OR LEASOR FUEL NEED TAX DEP AND BOOK DEP 
C
            NF_TAX_DEP = 0.
...
C
     ...
     +                        FINANCIAL_SIMULATION_YEARS)
            ENDIF
C
C Calculate NF monthly fuel prices in $/MWh 7/18/96
C
            IF(CALUCLATE_NUC_FUEL_PRICES .AND. GENERATOR_ID > 0) THEN
               CALL NUCLEAR_FUEL_MONTLY_PRICES(GENERATOR_ID)
            ENDIF
C
C     LIST INDIVIDUAL ACCOUNT INFORMATION TO DISK FILE
C
...
C
C     WRITE THE CUMULATED RESULTS TO A FILE
C
     ...
     +                            LEASOR_NF_SL_TAX_DEPRECIATION(I,CLASS)
      ...
      IF(SAVE_BASE_CASE) THEN
         CALL OPEN_NF_BASE_CASE_FILE(INT2(10))
...
C***********************************************************************
C
         NUM_OF_NUC_UNITS =   MAX(200,R_NUM_OF_NUC_UNITS)
...
C                     
     ...
     +                                      MONTHLY_UNIT_CAPACITY(MO,YR)
         ...
         RELOAD_YEAR = 1
C
C NOTE: 7/11/96 NEED A LAST LOAD VALUE THAT'S BETTER THAN
C               THE MIN() BELOW.
C
         FRAC_OWNED = R_FRAC_OWNED/100.
         DO YR = 1, MIN(AVAIL_DATA_YEARS,NUC_FUEL_BURN_YEARS+2)
...
C     AMOUNT OF TAX AMORTIZED IN A DEFERRED TAX ACCOUNT
      R_NRTXWO = R_TAXWO
C     TOTAL BOOK WO
...
C
C
C     THEN ABANDONMENT ITEMS ARE ZEROED
C
      TEMP1 = NFCE(LASTYR)
      TEMP2 = AFDC1(LASTYR)
      TEMP3 = AFDC1B(LASTYR)
...
C
      ...
      INTEGER*2 J,L,M,IVEC
      REAL TAXDPC,GPVF,DEPDB,DEPSL
      REAL NPVFT
      REAL*4 VECTOR_VALUES(AVAIL_DATA_YEARS)
...
C         
         ...
         IF(TAXLF <= 1.0) THEN
            ...
            ELSE
               ...
               DO J=1,MIN(FINANCIAL_SIMULATION_YEARS-1,AVAIL_DATA_YEARS)
