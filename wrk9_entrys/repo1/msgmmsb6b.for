...
C***********************************************************************
C
      INTEGER*2 R_VARIABLE_NUMBER
      REAL*4 VARIABLES_IN(0:R_VARIABLE_NUMBER),
     +       VARIABLES_OUT(0:R_VARIABLE_NUMBER)
      INTEGER*2 :: I
C
      ...
      RETURN
C***********************************************************************
      ENTRY INTRA_COMPANY_ADJUSTMENT(R_VARIABLE_NUMBER,
     +                            VARIABLES_IN,VARIABLES_OUT)
C***********************************************************************
         VARIABLES_OUT = VARIABLES_OUT - VARIABLES_IN
      ...
      RETURN
C***********************************************************************
      ENTRY SUB_CONSOLIDATING_ADJUSTMENTS(R_VARIABLE_NUMBER,
     +                                     VARIABLES_IN,VARIABLES_OUT)
C***********************************************************************
         VARIABLES_OUT = VARIABLES_OUT - VARIABLES_IN
      ...
      RETURN
C***********************************************************************
      ENTRY PASS_LEVEL_2_VARS(R_VARIABLE_NUMBER,
     +                                 VARIABLES_IN,VARIABLES_OUT)
C***********************************************************************
      ...
      RETURN
C***********************************************************************
      ENTRY PASS_PARENT_2_SBU_TOTALS(R_VARIABLE_NUMBER,
     +                                     VARIABLES_IN,VARIABLES_OUT)
C***********************************************************************
         VARIABLES_OUT = VARIABLES_IN
      ...
      END
C***********************************************************************
      FUNCTION LOAD_MASTER_CLASS_LIST(MAX_CLASS_NUM,TEMP_LIST,
     +                                                MASTER_CLASS_LIST)
C***********************************************************************
C
      INTEGER*2 LOAD_MASTER_CLASS_LIST,I
      INTEGER*2 MAX_CLASS_NUM,TEMP_LIST(MAX_CLASS_NUM),
...
C      
C CONSOLIDATING AND INTRA NA
C
C        VARIABLE(59) = NOT_AVAIL ! Average Equity
...
c         VARIABLE(364) = NOT_AVAIL ! PREFERRED_STOCK_IN_CAP
c         VARIABLE(365) = NOT_AVAIL ! LONG_TERM_DEBT_IN_CAP
c         VARIABLE(366) = NOT_AVAIL ! COMMON_EQUITY_BAL_IN_CAP
c         VARIABLE(367) = NOT_AVAIL ! SHORT_TERM_DEBT_IN_CAP
c         VARIABLE(368) = NOT_AVAIL ! FUNDS_4_CONSTRUCTION
...
C
      ENTRY SET_SUBSIDARY_VALUES_AND_NA(VARIABLE)
         IF(VARIABLE(53) == 0.) THEN
            ...
            VARIABLE(56) = NOT_AVAIL ! Dividends per Share ($)
         ENDIF
C         VARIABLE(53) = NOT_AVAIL ! Average Shares Outstanding
C         VARIABLE(299) = NOT_AVAIL ! Year-end Shares Outstanding
C         VARIABLE(54) = NOT_AVAIL ! Market to Book Ratio
C
      RETURN
      END
C***********************************************************************
...
C
      INTEGER*2 I
      REAL*4 VARIABLE(0:*),SBU_VARIABLES(0:*)
C
C INCOME STATEMENT VARIABLES
C
c         DO I = 0, 20
c            VARIABLE(I) = SBU_VARIABLES(I) + VARIABLE(I)
c         ENDDO
...
C
C
C VARIABLES 626-COMMON SHARES ISSUED THIS PERIOD; AND 627-STD INTEREST RATES DO NOT
C GET SUMMED
C
...
C        VARIABLE(240) = SBU_VARIABLES(240) + VARIABLE(240)  ! CAWTABA & CP&L EXPENSE
c        VARIABLE(247) = SBU_VARIABLES(247) + VARIABLE(247)  ! CAWTABA
c        VARIABLE(350) = SBU_VARIABLES(350) + VARIABLE(350)  ! BTL_AMORTIZATION_IN_EXPENSES
C
C CASH STUFF
C
...
c         VARIABLE(636) = SBU_VARIABLES(636) + VARIABLE(636)  ! FASB143_ARO_NET_ASSET_BAL
...
C         
         ...
         VARIABLE(719) = SBU_VARIABLES(719) + VARIABLE(719)  ! CASH_ADDS_2_GAS_INVENTORY
         VARIABLE(720) = SBU_VARIABLES(720) + VARIABLE(720)  ! EXPENSING_GAS_INVENTORY
C        VARIABLE(74) = SBU_VARIABLES(74) + VARIABLE(74)  ! CASH_BALANCE),
C        VARIABLE(75) = SBU_VARIABLES(75) + VARIABLE(75)  ! ASSET_NEC),
C        VARIABLE(76) = SBU_VARIABLES(76) + VARIABLE(76)  ! TOTAL_ASSETS),
C
C BALANCE SHEET-LIABILITIES
C         
c         VARIABLE(636) = SBU_VARIABLES(636) + VARIABLE(636)  ! FASB143_ARO_NET_ASSET_BAL
c         VARIABLE(637) = SBU_VARIABLES(637) + VARIABLE(637)  ! FASB143_ARO_LIABLILITY_BAL
...
C
      ...
      END
C***********************************************************************
      FUNCTION ASSET_CLASS_LINKAGES(ASSET_CLASS,R_ASSET_CLASS_LIST,
     +                              ASSET_CLASS_VECTOR,
...
C
      ...
      INTEGER*2 CLASS_POINTER,VECTOR_POINTER,ASSET_CLASS,LINKED_CLASS,
     +          ASSET_CLASS_VECTOR,I
      REAL*4 VECTOR_ASSET_CLASSES(AVAIL_DATA_YEARS),
     ...
     +       R_ASSET_ALLOCATION_LIST(*)
      ...
      LOGICAL*1 ASSET_CLASS_LINKAGES,RETURN_ASSET_CLASS_LISTS
C
         ASSET_CLASS_LIST = -99.
         ASSET_ALLOCATION_LIST = 0.
         CLASS_POINTER = 1
         IF(ASSET_CLASS < 0) THEN
            CALL GET_ASSET_VAR(ABS(ASSET_CLASS),DUMMY_TYPE,
     ...
     +                                       CLASS_POINTER >= 1000) EXIT ! AVAIL_DATA_YEARS) EXIT
            ENDDO
      ...
      RETURN
C***********************************************************************
      ENTRY RETURN_ASSET_CLASS_LISTS(ASSET_CLASS,
     +                               R_ASSET_CLASS_LIST,
     +                               ASSET_CLASS_VECTOR,
     +                               R_ASSET_ALLOCATION_LIST)
C***********************************************************************
...
C
      SAVE
      LOGICAL*1 CAL_AMT_SWITCH,R_CALCULATE_AMT
      LOGICAL*1 USE_BUDGET_EXPENSES,R_USE_BUDGET_EXPENSES
      LOGICAL*1 CALCULATE_ENVIRONMENTAL_TAX,R_CALC_ENVIRONMENTAL_TAX
      LOGICAL*1 CURRENT_COVERAGE_RATIO
      LOGICAL*1 RETAIN_POST_RETIREMENT_EARNINGS,
     +          R_USE_RETAIN_POST_RETIREMENT_EARNINGS
      INTEGER*2 R_COVERAGE_RATIO
C
      ...
      ENTRY RETAIN_POST_RETIREMENT_EARNINGS()
         CALL RETURN_RETAIN_POST_RETIREMENT_EARNINGS(
     +                            R_USE_RETAIN_POST_RETIREMENT_EARNINGS)
...
C
      SAVE
      REAL*4 REPORTING_VARIABLES(:),
     +       R_VARIABLE_VALUE(0:*)
      ALLOCATABLE :: REPORTING_VARIABLES
      INTEGER*2 I,NUM_ACTIVE_VARIABLES,R_VARIABLE_NUM
      REAL*4 PASS_THROUGH_VALUES(10)
      REAL*4 AI_LAGGED_GPV,
     +       AI_CLASS_CASH,
     +       AI_ANNUAL_CLASS_DEP,
     +       AI_AFUDC_ON_CASH,
     +       AI_CLASS_EXPENSE
      REAL*4 OPENING_INVESTMENT_BALANCE
      REAL*4 FA_AFUDC_RATE,
     +       NF_AFUDC_RATE,
     +       AI_AFUDC_RATE,
     +       AFUDC_BORROWED_RATE,
     +       AFUDC_ON_CASH
      REAL*4 TOTAL_CAPITAL,LTD_INTEREST_LESS_AFUDC
      INTEGER*2 COVERAGE_RATIO_TYPE
      CHARACTER*1 OPERATING_METHOD
      REAL*4 SRP_RATIO,LEASE_PAYMENTS_IN_TIER
      REAL*4 R_CLASS_MHW_SALES,R_ANNUAL_ECONOMY_SOLD
      REAL*4 NOT_AVAIL
      PARAMETER(NOT_AVAIL=-999999.)
C
C SPR VARIABLES
C
      LOGICAL*1 ECON_SWITCH,ECON_SALES
      REAL*4 SRP_FUNDS_FOR_DEBT_SERVICE,
     ...
     +       SRP_TOTAL_SALES,
     +       SRP_SALES_COST,
     +       SRP_OPERATING_MARGIN,
     +       SRP_AVERAGE_SALES_COST,
     +       SRP_AVERAGE_MARGIN
      REAL*8 GET_TOTAL_SALES_ENERGY
C
      REAL*4 INTEREST_COVERAGE_WITHOUT_AFDC,
     +       INTEREST_COVERAGE_WITH_AFDC,
     +       GOPINC,
     +       ITC_ITCAMT,
     +       TAXES_PAID,
     +       INTINC,
     ...
     +       SUM_OPERATIONS,
     +       INTEREST
      REAL*4 NON_CASH_EXPENSE_ADJUSTMENTS,
     +       TAX_AND_OTHER_ADJUSTMENTS,
     +       OTHER_CASH_SOURCES,
     +       CAPITAL_REQUIREMENTS,
     +       TOTAL_CAPITAL_SERVICE_PAYMENTS
C 
      REAL*4 TEMP_SRP_TOTAL_SALES,SRP_OP_FIN_TARGET_D,
     +       SRP_OP_FIN_MARGIN_D,SRP_OP_FIN_TARGET_D_MWH,
     +       SRP_OP_FIN_MARGIN_D_MWH,SRP_NET_FINANCING_COSTS
      INTEGER*2 R_RUN_YEAR
      REAL*4 AFDC1,INTEREST_PS_DIV
      REAL*4 TIMES_INT_B4_INC_TAXES_W_AFDC,
     ...
     +       TOTAL_INCOME_TAXES,
     +       EFFECTIVE_TAX_RATE
      REAL*4 ANNUALIZED_LTD_INTEREST,
     +       ANNUALIZED_PS_DIVIDENDS,
     +       ANNUALIZED_STD_INTEREST,
     +       EMBED_COST_CAPITAL_W_STD,
     +       EMBED_COST_CAPITAL_WO_STD,
     +       MARKET_VALUE,DIVIDENDS_PER_SHARE,
     +       SHARES_OUTSTANDING_YR_END
      REAL*4 GROSS_CASH_2_DEBT,
     +       GROSS_CASH_FLOW,
     +       TOTAL_DEBT,
...
C
C SAVE AREA 8/15/01 MSG
C
c      SAVE REPORTING_VARIABLES
c      SAVE NUM_ACTIVE_VARIABLES
c      SAVE PASS_THROUGH_VALUES
c      SAVE AI_LAGGED_GPV,
c     +     AI_CLASS_CASH,
c     +     AI_ANNUAL_CLASS_DEP,
c     +     AI_AFUDC_ON_CASH,
c     +     AI_CLASS_EXPENSE
c      SAVE OPENING_INVESTMENT_BALANCE
c      SAVE TOTAL_CAPITAL
C
         OPENING_INVESTMENT_BALANCE = 0.
         NUM_ACTIVE_VARIABLES = R_VARIABLE_NUM - 1
         IF(ALLOCATED(REPORTING_VARIABLES))
     +                                   DEALLOCATE(REPORTING_VARIABLES)
...
c         ENDDO
         ...
         CALL RETURN_AI_CLASS_VALUES(AI_LAGGED_GPV,
     +                               AI_CLASS_CASH,
     +                               AI_ANNUAL_CLASS_DEP,
     +                               AI_AFUDC_ON_CASH,
     +                               AI_CLASS_EXPENSE)
C
C ADDED TO PASS BASE AND ADJUSTMENT REVENUE INFORMATION TO COST OF SERVICE
C
         CALL STORE_REVENUE(R_VARIABLE_VALUE(0),R_VARIABLE_VALUE(1))
      RETURN
C***********************************************************************
      ENTRY RETURN_ASSET_CLASS_VALUES(R_VARIABLE_VALUE,
     +                                COVERAGE_RATIO_TYPE,
     +                                OPERATING_METHOD,
...
C***********************************************************************
C
C RESERVE MARGIN LEVEIZING COST VARIABLES
C
...
C        R_VARIABLE_VALUE(5)   = REPORTING_VARIABLES(68)  ! NPV(2)
         R_VARIABLE_VALUE(532) = REPORTING_VARIABLES(261) ! R_WORKING_CAP ! WCCUM(2)
C        R_VARIABLE_VALUE(20)  = REPORTING_VARIABLES(87)  ! LESS DEF TAXES CR
...
C    +                                                     COST_OF_CAPITAL_RISK_ADJUSTMENT)
...
C        R_VARIABLE_VALUE(97) = REPORTING_VARIABLES() ! AMT Credit Generated
C        R_VARIABLE_VALUE(98) = REPORTING_VARIABLES() ! AMT Credit Used
...
C        R_VARIABLE_VALUE(94) = REPORTING_VARIABLES() ! AMT Depreciation Preference Item
         R_VARIABLE_VALUE(95) = REPORTING_VARIABLES(139) ! AMT Taxable Income
C        R_VARIABLE_VALUE(96) = REPORTING_VARIABLES() ! AMT Available Credits
C        R_VARIABLE_VALUE(99) = REPORTING_VARIABLES() ! AMT Tax
C        R_VARIABLE_VALUE(100) = REPORTING_VARIABLES() ! AMT ITC Used 
C        R_VARIABLE_VALUE(101) = REPORTING_VARIABLES() ! AMT NOL
C        R_VARIABLE_VALUE(102) = REPORTING_VARIABLES() ! AMT NOL Used
         R_VARIABLE_VALUE(74) = REPORTING_VARIABLES(220) ! SL Tax Depreciation 
C
C END TAX
C
c
c added 1/24/97 Cash Flow Statement
c
...
C
     ...
     +                           REPORTING_VARIABLES(119)   ! Debt Retirements
c        R_VARIABLE_VALUE(117) = REPORTING_VARIABLES(301) ! TOTAL_CAPITAL_SERVICE
         R_VARIABLE_VALUE(114) = REPORTING_VARIABLES(39) ! + ! INTEREST_ON_LONG_TERM_DEBT
c    +                           REPORTING_VARIABLES(40)   ! STD_INTEREST
...
C        R_VARIABLE_VALUE() = REPORTING_VARIABLES() ! 
C
C EXTERNALITY VARIABLES
C
...
C
     ...
     +                   REPORTING_VARIABLES(41) ! TAFDCB
         IF(REPORTING_VARIABLES(47) /= 0.) THEN ! COMMON_EARNINGS
            R_VARIABLE_VALUE(87) = 100.*AFUDC_ON_CASH/
     +                                           REPORTING_VARIABLES(47) ! AFDC1_TO_COMMON_EARNINGS
         ELSE
            R_VARIABLE_VALUE(87) = 0.
         ENDIF
C
C NOTE TOTAL_CAPITAL IS USED INTEGER*2 THE SRP SECTION
C
...
C
C STANDARD CALCULATION OF INTEREST COVERAGE WITH AND WITHOUT AFUDC
C
...
C
     ...
     +                       REPORTING_VARIABLES(119)   ! Debt Retirements
         IF(LTD_STD_INTEREST /= 0.) THEN
            ...
            INTEREST_COVERAGE_WITH_AFDC = (RTEMP + AFUDC_ON_CASH)/
     +                                                  LTD_STD_INTEREST
         ENDIF
C
C COVERAGE RATIOS
...
C           LTD_STD_INTEREST = LTDINT(RUN_YEAR) + STDINT(2)
            IF(LTD_STD_INTEREST /= 0.) COVERAGE_DRIVER_RATIO = GOPINC/
     +                                                  LTD_STD_INTEREST
...
C           LTD_STD_INTEREST = LTDINT(RUN_YEAR) + STDINT(2)
     ...
     +                                - REPORTING_VARIABLES(41)  ! afudc borrowed
            IF(LTD_INTEREST_LESS_AFUDC /= 0.)
     +            COVERAGE_DRIVER_RATIO = (LTD_INTEREST_LESS_AFUDC
     +                                       + REPORTING_VARIABLES(45))/
...
C           LTD_STD_INTEREST = LTDINT(RUN_YEAR) + STDINT(2)
            IF(COVERAGE_RATIO_TYPE == 2) 
     +               LTD_STD_INTEREST = LTD_STD_INTEREST - AFUDC_ON_CASH
            IF(LTD_STD_INTEREST /= 0.) COVERAGE_DRIVER_RATIO = RTEMP/
     +                                                  LTD_STD_INTEREST
         ELSEIF(COVERAGE_RATIO_TYPE == 8 .OR. 
     +                                    COVERAGE_RATIO_TYPE == 9) THEN
            LEASE_PAYMENT_LIMIT = MAX(0.,LEASE_PAYMENTS_IN_TIER -
     +                                   .02*REPORTING_VARIABLES(79))/3. ! COMEQU(2)
            RTEMP = GOPINC+ITC_ITCAMT+TAXES_PAID+INTINC+OTHINC-
     +              BTL_TAXES + LEASE_PAYMENT_LIMIT
            INTEREST = LTD_STD_INTEREST - AFUDC_ON_CASH +
     +                                               LEASE_PAYMENT_LIMIT
...
C NOTE OTNINC IS NET OF BLT_EXPENSES
C              RTEMP = GOPINC + OTHINC + INTINC +
C    +                 TXPROP - STDINT(2) + 
C    +                 BKDPPT + BDPAFC + AMTEXP
     ...
     +                                      REPORTING_VARIABLES(217)   ! TXPROP +
               IF(SRP_TOTAL_DEBT_SERVICE /= 0.) 
     +              COVERAGE_DRIVER_RATIO = SRP_FUNDS_FOR_DEBT_SERVICE/
     +                                            SRP_TOTAL_DEBT_SERVICE
               IF(OPERATING_METHOD=='C') COVERAGE_DRIVER_RATIO=SRP_RATIO
C         ELSEIF(COVERAGE_RATIO_TYPE == 10) THEN
C            IF(INTEREST_ON_LONG_TERM_DEBT /= 0.)
C     +         COVERAGE_DRIVER_RATIO =
C     +                        (INTEREST_ON_LONG_TERM_DEBT + NET_INCOME)/
...
C
     ...
     +                                        REPORTING_VARIABLES(301))/ ! TOTAL_CAPITAL_SERVICE
     +                                     FUNDS_FOR_CONSTRUCTION
         ELSE
...
C        SUM_DEFERRED_ITEMS = ITC_ITCAMT + TXDEFP + TXNORM -
C    +                        NUCL_FUND_RETURN -
C    +                        CATAWBA_TOTAL_LEVEL_CAP_PAYMTS(RUN_YEAR)
C        SUM_AFUDC = AFDC1 + DEF_REVENUES
...
C                                                            KEPCO_WC_DEF_MAINT_ENRG_AMORT
...
C***********************************************************************
C
C CALCULATED SRP OUTPUT VARIALBES
C
      ENTRY SRP_REPORTING_VARIABLES(R_RUN_YEAR,
     +                              R_VARIABLE_VALUE,
     +                              R_CLASS_MHW_SALES,
     +                              R_ANNUAL_ECONOMY_SOLD)
C***********************************************************************
C
C
C SRP VARIABLES
...
C    +                            REPORTING_VARIABLES(48) -  ! CSDIV
     +                            REPORTING_VARIABLES(217)   ! TXPROP
...
C        R_VARIABLE_VALUE(475) = LTDRET(RUN_YEAR) + PSRED(RUN_YEAR)
     ...
     +                       R_ANNUAL_ECONOMY_SOLD + CAWCD_ENERGY)/1000.
         CALL RETURN_SRP_TOTAL_SALES(R_RUN_YEAR,TEMP_SRP_TOTAL_SALES)
         IF(TEMP_SRP_TOTAL_SALES > 0.) SRP_TOTAL_SALES = 
     +                                              TEMP_SRP_TOTAL_SALES
...
C    +                    REPORTING_VARIABLES(48) + ! CSDIV
     ...
     +                              SRP_OPERATING_MARGIN/SRP_TOTAL_SALES
            SRP_OP_FIN_MARGIN_D_MWH = 1000. *
     +                              SRP_OP_FIN_MARGIN_D/SRP_TOTAL_SALES
            SRP_OP_FIN_TARGET_D_MWH = 1000. *
     +                              SRP_OP_FIN_TARGET_D/SRP_TOTAL_SALES
         ELSE
...
cC 8/6/98 SRP NEW CALCULATIONS
...
C    ***** SEC RATIOS *******
C 
C TIMES INTEREST BEFORE INCOME TAXES SEC METHOD INCLUDING AFUDC
C TIMES INTEREST BEFORE INCOME TAXES SEC METHOD WITHOUT AFUDC
C
         IF(INTEREST /= 0.) THEN
            TIMES_INT_B4_INC_TAXES_W_AFDC = (NOPINC+TOTAL_INCOME_TAXES)/
     +                                                  INTEREST
            TIMES_INT_B4_INC_TAXES_WO_AFDC = (NOPINC-AFDC1+
     +                                      TOTAL_INCOME_TAXES)/INTEREST
         ELSE   
            TIMES_INT_B4_INC_TAXES_W_AFDC = NOT_AVAIL
            TIMES_INT_B4_INC_TAXES_WO_AFDC = NOT_AVAIL
         ENDIF
C
C TIMES INTEREST & PS DIV BEFORE INCOME TAXES SEC METHOD INCLUDING AFUDC
C TIMES INTEREST & PS DIV BEFORE INCOME TAXES SEC METHOD WITHOUT AFUDC
C
         ...
         IF(INTEREST_PS_DIV /= 0.) THEN
            ...
            TIMES_INT_PS_B4_INC_TAX_WO_AFDC = (NOPINC - AFDC1)/
     +                                                   INTEREST_PS_DIV
         ELSE   
...
C
         IF(REPORTING_VARIABLES(45)+TOTAL_INCOME_TAXES
     +                              -REPORTING_VARIABLES(37) /= 0.) THEN
            EFFECTIVE_TAX_RATE = 100.*TOTAL_INCOME_TAXES/
     +                (REPORTING_VARIABLES(45)+TOTAL_INCOME_TAXES
     +                                         -REPORTING_VARIABLES(37)) ! 45 is net income
         ELSE
...
C
     ...
     +                          + R_VARIABLE_VALUE(682)
     +                          + R_VARIABLE_VALUE(733)
...
C
     ...
     +                        + R_VARIABLE_VALUE(730) ! OCI BALANCE
         ...
         R_VARIABLE_VALUE(366) = CAPITAL_MULTIPILER *          
     +                       (COMMON_EQUITY_BAL + R_VARIABLE_VALUE(730))                         
         R_VARIABLE_VALUE(367) = SHORT_TERM_DEBT_BAL*CAPITAL_MULTIPILER
C
         FUNDS_FROM_OPERATIONS = R_VARIABLE_VALUE(632)
     +                           - R_VARIABLE_VALUE(387)
...
C
         ...
         ELSE
            IF(ABS(R_VARIABLE_VALUE(79)) >= 0.0001) THEN
               DIVIDEND_YIELD = 100.*R_VARIABLE_VALUE(48)/
     +                               R_VARIABLE_VALUE(79)
            ELSE
               DIVIDEND_YIELD = NOT_AVAIL
            ENDIF   
...
C
         IF(R_VARIABLE_VALUE(45)+TOTAL_INCOME_TAXES-AFDC1 /= 0.) THEN
            EFFECTIVE_TAX_RATE = 100.*TOTAL_INCOME_TAXES/
     +                   (R_VARIABLE_VALUE(45)+TOTAL_INCOME_TAXES-AFDC1) ! 45 is net income
         ELSE
            EFFECTIVE_TAX_RATE = NOT_AVAIL   
         ENDIF
C
C GROSS CASH FLOW TO TOTAL DEBT=MOODY'S
C
         GROSS_CASH_FLOW = R_VARIABLE_VALUE(632) ! 109 FUNDS_FROM_OPERATIONS
!     +                     + PS_DIVIDEND_CASH_PAYMENTS  388! PS DIVIDEND PAYMENTS
!     +                     + COMMON_STOCK_CASH_DIVIDENDS 398! COMMON_DIVIDENDS + 
         TOTAL_DEBT =  R_VARIABLE_VALUE(634) ! LONG_TERM_DEBT(2,R_CLASS_POS)
C     +                 + NOTES_PAYABLE_BALANCE
C     +                 + SHORT_TERM_DEBT(2,R_CLASS_POS)
         GROSS_CASH_2_DEBT =
...
C
     ...
     +                       R_VARIABLE_VALUE(119)   ! Debt Retirements
C
C COVERAGE RATIO RESULT FOR THE RATIO CHOOSEN
C
         COVERAGE_DRIVER_RATIO = NOT_AVAIL
         IF(COVERAGE_RATIO_TYPE == 1) THEN
C           LTD_STD_INTEREST = LTDINT(RUN_YEAR) + STDINT(2)
...
C           LTD_STD_INTEREST = LTDINT(RUN_YEAR) + STDINT(2)
     ...
     +                                                  LTD_STD_INTEREST
         ELSEIF(COVERAGE_RATIO_TYPE == 8 .OR. 
     +                                    COVERAGE_RATIO_TYPE == 9) THEN
            LEASE_PAYMENT_LIMIT = MAX(0.,R_VARIABLE_VALUE(778) -
     +                                   .02*R_VARIABLE_VALUE(79))/3. ! COMEQU(2)
            RTEMP = GOPINC+ITC_ITCAMT+TAXES_PAID+INTINC+OTHINC-
     +              BTL_TAXES + LEASE_PAYMENT_LIMIT
            INTEREST = LTD_STD_INTEREST - AFUDC_ON_CASH +
     +                                               LEASE_PAYMENT_LIMIT
...
C     +                                INTEREST_ON_LONG_TERM_DEBT
      ...
      END
C***********************************************************************
      SUBROUTINE LAG_CASH_USING_PATTERN_ALL_YRS(R_YRS,
     +                                          R_BOOK_VALUES,
     +                                          R_CASH_LAG_PATTERN,
     +                                          R_CASH_PAYMENTS,
     +                                          R_CASH_VALUES)
...
C
      INTEGER*2 R_YRS
      REAL*4 R_BOOK_VALUES(0:12,0:R_YRS),
     +       R_CASH_LAG_PATTERN(12),
     +       R_CASH_PAYMENTS(12),
     +       R_CASH_VALUES(0:12,0:R_YRS)
      INTEGER*2 MO,YR,LAG_MO
C
C
         DO YR = 1, R_YRS
            DO MO = 1, 12
C MOVE CASH ACCURRED LAST YEAR TO THIS YEAR
     ...
     +                 R_CASH_LAG_PATTERN(LAG_MO) * R_BOOK_VALUES(MO,YR)
               ...
               ENDDO
               DO LAG_MO = 14-MO, 12
                  R_CASH_PAYMENTS(LAG_MO+MO-13) =
     +                 R_CASH_LAG_PATTERN(LAG_MO) * R_BOOK_VALUES(MO,YR)
               ENDDO
               R_CASH_VALUES(0,YR) = R_CASH_VALUES(0,YR) +
     +                               R_CASH_VALUES(MO,YR)
            ENDDO
         ENDDO
      RETURN
...
C
      ...
      PARAMETER(NOT_AVAIL=-999999.)
C
C VARIABLE IN SEC RATIOS REPORT
C
...
C***********************************************************************
C
      REAL*4 FEDERAL_TAX_TABLE,TAXABLE_INCOME
C
...
C***********************************************************************
      REAL (KIND=4) :: R_VARIABLE(0:*),THE_RATIO_OF_A_TO_B,
     +                 TARGET_DSC,R_TARGET_DSC,
     +                 PrinIntrPayments,
     +                 MarginIntExpDep,TwoPercentTest
C
         TARGET_DSC = 1
         R_VARIABLE(761) = ! GRE_TOTAL_INTEREST_EXPENSE), ! 761
     +                     R_VARIABLE(760) ! GRE_FALKIRK_INTEREST),  ! 760
     +                     + R_VARIABLE(39) ! CASH INTEREST EXPENSE
C     +                     + R_VARIABLE(293) ! INTEREST EXPENSE
...
c         R_VARIABLE(769) = ! GRE_NET_MARGIN), ! 769
c     +                     TARGET_DSC * (R_VARIABLE(766)      ! GRE_MEMBER_REVENUE_REQUIREMENT), ! 773
c     +                                   + R_VARIABLE(768))   ! GRE_TOTAL_INTEREST_PAYMENTS), ! 768
c     +                     - R_VARIABLE(761)   ! GRE_TOTAL_INTEREST_EXPENSE), ! 761
c     +                     - R_VARIABLE(763)   ! GRE_TOTAL_DEPRECIATION), ! 763
         R_VARIABLE(769) = ! GRE_NET_MARGIN), ! 769
     +                     R_VARIABLE(0)  ! WAS R_VARIABLE(4)   ! TOTAL revenues
     +                     - R_VARIABLE(770)
     +                     + R_VARIABLE(771)  ! was a minus
     +                     + R_VARIABLE(772)
     ...
     +                     + R_VARIABLE(761)   ! GRE_TOTAL_INTEREST_EXPENSE), ! 761
     +                     + R_VARIABLE(763)   ! GRE_TOTAL_DEPRECIATION), ! 763
         R_VARIABLE(773) = THE_RATIO_OF_A_TO_B(MarginIntExpDep,
     +                                                 PrinIntrPayments)! GRE_DSC) ! 773
         R_VARIABLE(776) = R_VARIABLE(770)
     +                     - R_VARIABLE(771)
     +                     - R_VARIABLE(772)
         TwoPercentTest = THE_RATIO_OF_A_TO_B(R_VARIABLE(0),
     +                                                  R_VARIABLE(776))
...
C***********************************************************************
      ...
      REAL*4 NOT_AVAIL
      PARAMETER(NOT_AVAIL=-999999.)
      REAL*4 TOTAL_OPERATING_EXPENSES,BTL_TOTALS,TOTAL_TAXES_NON_INCOME,
     +       TOTAL_DEP_AMORT,EBIT,TOTAL_INTEREST,TOTAL_INCOME_TAXES,
     +       EBITDA,INCOME_AFTER_INTEREST,TOTAL_NON_REGULATED_REVENUES,
     +       TIER_INTEREST
      REAL*4 EBITDA_2_DEBT,THE_PERCENT_OF_A_TO_B,TOTAL_DEBT
C
     ...
     +                                  + R_VARIABLES(328) ! +Relationship Revenues",V,328
     +                                  + R_VARIABLES(239) ! +Regulatory Credit",V,239
     +                                  + R_VARIABLES(449) ! +Derivative Revenues
C
         TOTAL_OPERATING_EXPENSES = R_VARIABLES(13)    !Salaries & Wages,13
     +                              + R_VARIABLES(5)   !Fossil Fuel Expense,5
     +                              + R_VARIABLES(438) !Nuclear Fuel
     +                              + R_VARIABLES(233) !Decommissioning
     +                              + R_VARIABLES(232) !DOE Deposal
     +                              + R_VARIABLES(461) !Purchased Power Expense,461
     +                              + R_VARIABLES(393) !Deferred Fuel Expense,393
     +                              + R_VARIABLES(8)   !Fixed O&M Production Expense,8
     +                              + R_VARIABLES(363) !Variable O&M Production Expense,363
     +                              + R_VARIABLES(450) !Total Derivative Expense
     +                              + R_VARIABLES(421) !O&M Vacation,421
     +                              + R_VARIABLES(422) !O&M Pension,422
     +                              + R_VARIABLES(423) !O&M storm,423
     +                              + R_VARIABLES(9)   !Service Transaction Expense,9
     +                              + R_VARIABLES(440) !Other O&M,440
     +                              + R_VARIABLES(11)  !Gas Purchases,11
     ...
     +                              + R_VARIABLES(321) !Customer Accounts,321
     +                              + R_VARIABLES(322) !Customer Services,322
     +                              + R_VARIABLES(323) !Sales Expense,323
     +                              + R_VARIABLES(324) !A&G Operations,324
     +                              + R_VARIABLES(325) !A&G Maintenance,325
     +                              + R_VARIABLES(354) !Lease Expenses,354
     +                              + R_VARIABLES(13)  !DSM Program Expense
     +                              + R_VARIABLES(14)  !DSM Program Rebate
...
c         TOTAL_OPERATING_EXPENSES = R_VARIABLES(476) ! Total Cash Expenses
c     +                              + R_VARIABLES(233) !Decommissioning
C
...
C
         R_VARIABLES(29) = R_VARIABLES(681)
     +                     + R_VARIABLES(682)
     +                     + R_VARIABLES(733)
...
C     +                 + SHORT_TERM_DEBT(2,R_CLASS_POS)
         EBITDA_2_DEBT =
     +                 THE_PERCENT_OF_A_TO_B(EBITDA,TOTAL_DEBT) 
...
C TIER
     ...
     +                   + R_VARIABLES(40)  ! Interest on Short Term Debt,40
         IF(TIER_INTEREST /= 0.) THEN
            R_VARIABLES(494) = (R_VARIABLES(493) + TIER_INTEREST)/
     +                                     TIER_INTEREST
         ELSE
...
C
C Pre-Tax Interest Coverage  Var 782
C
...
C***********************************************************************
      ...
      INTEGER*2 I,CLASS_NUM
      REAL TOTAL_PRODUCTION_COSTS,TOTAL_POWER_SALES,TOTAL_POWER_REVENUES
      REAL TOTAL_ENERGY_GENERATION_COSTS,
     +     TOTAL_PRODUCTION_COSTS_NON_FUEL
...
C
      LOGICAL*1 RETURN_FE_PNL_EXPENSES,VOID_LOGICAL
      INTEGER*2 FT,CLASS
      REAL FUEL_COST(6),
     +     PURCHASE_POWER_EXPENSE(11),
     +     VARIABLE_EXPENSE(6),
     +     FIXED_EXPENSE(6),
     +     TOTAL_SALES_REVENUE(6),
     +     CL_ANN_TOTAL_CLASS_ENERGY(11),
     +     CL_ANN_TOTAL_CAPACITY(11),
     +     THE_RATIO_OF_A_TO_B,
     +     THE_PERCENT_OF_A_TO_B
      INTEGER*2 Coal,Gas,Oil,Nuclear,Water,Other,
     +          EastLake,BayShore,MarketPurchases,MarketSales
      PARAMETER (Coal=1,Gas=2,Oil=3,Nuclear=4,Water=5,Other=6,
     +           MarketSales=10,MarketPurchases=11)
...
C       4 == ECONOMY SALES WHICH INCREASE LOAD
     ...
     +     R_EL_MARKET_PURCHASES
      REAL UTILITY_SALES,
     +     UTILITY_SALES_QUANT,
     +     UTILITY_SALES_LOSSES,
     +     COMPETITIVE_SALES,
     +     COMPETITIVE_SALES_QUANT,
     +     COMPETITIVE_SALES_LOSSES,
     +     COMPETITIVE_SALES_CATEGY(1:9),
     +     COMPETITIVE_SALES_QUANT_CATEGY(1:9),
     +     COMPETITIVE_SALES_LOSSES_CATEGY(1:9),
     +     BAY_SHORE_FUEL_EXPENSE,
     +     ENRGY_BOUGHT,
     +     ENRGY_BOUGHT_COST
      INTEGER*2 RETURN_FE_PNL_REVENUES,VOID_INT2
      INTEGER*2 Annual Amount
      PARAMETER (Annual Amount = 0) 
C
C EXPENSE
C
         ...
         IF(CLASS >= 0) THEN
            VOID_LOGICAL = RETURN_FE_PNL_EXPENSES(CLASS,
     +                                        FUEL_COST,
     +                                        PURCHASE_POWER_EXPENSE,
     +                                        VARIABLE_EXPENSE,
     +                                        FIXED_EXPENSE,
     +                                        TOTAL_SALES_REVENUE,
     +                                        CL_ANN_TOTAL_CLASS_ENERGY,
     +                                        CL_ANN_TOTAL_CAPACITY,
     +                                        BAY_SHORE_FUEL_EXPENSE)
            CALL FE_ENRG_LIMITED_INFO(CLASS,
     +                           PURCHASE_POWER_EXPENSE(Water),
     +                           VARIABLE_EXPENSE(Water),
     +                           FIXED_EXPENSE(Water),
     +                           TOTAL_SALES_REVENUE(Water),
     ...
     +                           CL_ANN_TOTAL_CLASS_ENERGY(Water))
            IF(USE_BUDGET_VARIABLE_OM) THEN
               ...
               ENDIF
            ENDIF
            VOID_INT2 = RETURN_FE_PNL_REVENUES(CLASS,
     +                                  UTILITY_SALES,
     +                                  UTILITY_SALES_QUANT,
     +                                  UTILITY_SALES_LOSSES,
     +                                  COMPETITIVE_SALES_CATEGY,
     +                                  COMPETITIVE_SALES_QUANT_CATEGY,
     +                                  COMPETITIVE_SALES_LOSSES_CATEGY)
            CALL RETURN_FE_COMPETITIVE_SALE_INFO(CLASS,
     +                                  COMPETITIVE_SALES_CATEGY,
     +                                  COMPETITIVE_SALES_QUANT_CATEGY,
     +                                  COMPETITIVE_SALES_LOSSES_CATEGY)
            CLASS_NUM = CLASS-INT2(1)
            CALL GET_ANN_COST_ABOVE_RESOURCES(ENRGY_BOUGHT,
     +                                        ENRGY_BOUGHT_COST,
     +                                        Annual Amount,CLASS_NUM)
C
...
c            IF(VOID_LOGICAL) THEN
C FUEL CALCULATIONS
C  Coal
               R_VARIABLES(511) = R_VARIABLES(511)
     +                            + FUEL_COST(Coal)
     +                            - BAY_SHORE_FUEL_EXPENSE
C  Gas
               R_VARIABLES(512) = R_VARIABLES(512)
     +                            + FUEL_COST(Gas)
C  BayShore
               R_VARIABLES(513) = R_VARIABLES(513)
     +                            + BAY_SHORE_FUEL_EXPENSE
C  Water
c               R_VARIABLES(514) = R_VARIABLES(514)
c     +                            + FUEL_COST(Water)
C  Other
               R_VARIABLES(515) = R_VARIABLES(515)
     +                            + FUEL_COST(Other)
     +                            + FUEL_COST(Oil)
C PURCHASE INFORMATION
C VAR and FIXED O&M CALCULATIONS
C  Coal
               R_VARIABLES(517) = R_VARIABLES(517)
     +                            + VARIABLE_EXPENSE(Coal)
     +                            + VARIABLE_EXPENSE(BayShore)
     +                            + VARIABLE_EXPENSE(Other)
               R_VARIABLES(520) = R_VARIABLES(520)
     +                            + FIXED_EXPENSE(Coal)
     +                            + FIXED_EXPENSE(BayShore)
     +                            + FIXED_EXPENSE(Other)
C  Gas
               R_VARIABLES(518) = R_VARIABLES(518)
     +                            + VARIABLE_EXPENSE(Gas)
     +                            + VARIABLE_EXPENSE(Water)
               R_VARIABLES(521) = R_VARIABLES(521)
     +                            + FIXED_EXPENSE(Gas)
     +                            + FIXED_EXPENSE(Water)
C Fossil Non-fuel expense
...
C  Nuclear
               R_VARIABLES(519) = R_VARIABLES(519)
     +                            + VARIABLE_EXPENSE(Nuclear)
               R_VARIABLES(522) = R_VARIABLES(522)
     +                            + FIXED_EXPENSE(Nuclear)
C Nuclear Non-fuel expenses
...
C
     ...
     +                      + EXP_FUEL_EXPENSE
         DO I = 511, 515
            TOTAL_FOSSIL_COST = TOTAL_FOSSIL_COST
     +                          + R_VARIABLES(I)
         ENDDO
...
C
         ...
         DO I = 526, 528 ! 529 AND 530 FOR NEW REVENUE SOURCES
            TOTAL_POWER_REVENUES = TOTAL_POWER_REVENUES
     +                             + R_VARIABLES(I)
         ENDDO
         GROSS_MARGIN_DOLLARS = TOTAL_POWER_REVENUES
     +                          - TOTAL_PRODUCTION_COSTS
         GROSS_MARGIN_PERCENT = THE_PERCENT_OF_A_TO_B(
     +                                             GROSS_MARGIN_DOLLARS,
     +                                             TOTAL_POWER_REVENUES)
         
C
C GENERATION AND SALES
C
         R_VARIABLES(536) = 0.
         DO I = 531, 534 ! 535 ARE AVAIALBE FOR NEW GERNERATIN SOURCES 
            R_VARIABLES(536) = R_VARIABLES(536)
     +                         + R_VARIABLES(I)
         ENDDO
         TOTAL_GENERATION = R_VARIABLES(536)
     +                      + R_VARIABLES(537)  ! PURCHASES
         TOTAL_SALES = 0.
     ...
     +                    + R_VARIABLES(I)
         ENDDO
C
C CALCULATE RATIOS FOR SALES
C
         DO I = 0, 4  ! VARIABLE 543 TO 547 
            R_VARIABLES(543+I) = THE_RATIO_OF_A_TO_B(R_VARIABLES(526+I),
     +                                               R_VARIABLES(538+I))
         ENDDO
C AVERAGE REVENUE PER SALE         
         R_VARIABLES(548) = THE_RATIO_OF_A_TO_B(TOTAL_POWER_REVENUES,  
     +                                          TOTAL_SALES)
C PURCHASE POWER COST-AVERAGEs
         R_VARIABLES(549) = THE_RATIO_OF_A_TO_B(R_VARIABLES(516),
     +                                          R_VARIABLES(537))
         R_VARIABLES(619) = THE_RATIO_OF_A_TO_B(R_VARIABLES(616),
     +                                          R_VARIABLES(614))
         R_VARIABLES(620) = THE_RATIO_OF_A_TO_B(R_VARIABLES(617),
     +                                          R_VARIABLES(615))
         R_VARIABLES(623) = THE_RATIO_OF_A_TO_B(R_VARIABLES(622),
     +                                          R_VARIABLES(621))
         DO I = 1, 9
     ...
     +                                               R_VARIABLES(582+I))
         ...
         ENDDO
C AVERAGE TOTAL PRODUCTION COST
         R_VARIABLES(618) = THE_RATIO_OF_A_TO_B(R_VARIABLES(558),
     +                                          R_VARIABLES(554))
C TOTAL MWH SALES, LOSSES AND TOTAL OF BOTH
         R_VARIABLES(592) = R_VARIABLES(538)
     +                      + R_VARIABLES(539)
     +                      + R_VARIABLES(540)
     +                      + (R_VARIABLES(646)
     +                      + R_VARIABLES(648))*.001 ! 02/04/04.
         R_VARIABLES(595) = R_VARIABLES(592)
     +                      + R_VARIABLES(593)
     +                      + R_VARIABLES(594)
C
C ASSIGN CALCUALTED VALUES
C
         R_VARIABLES(560) = GROSS_MARGIN_PERCENT
         R_VARIABLES(559) = GROSS_MARGIN_DOLLARS
         R_VARIABLES(558) = TOTAL_PRODUCTION_COSTS
     +                      - R_VARIABLES(460)
...
C
      ...
      LOGICAL*1 TRANSFERS_ARE_ACTIVE,
     +          R_REV_TRANSFERS_ARE_ACTIVE,
     +          R_EXP_TRANSFERS_ARE_ACTIVE,
     +          ASSET_ANALYST_ONLY
      INTEGER*2 TRANSFR_COST_CLASS_VECTOR,
     +          TRANSFR_COST_ALLOC_VECTOR,
     +          TRANSFR_REV_CLASS_VECTOR,
     +          TRANSFR_REV_ALLOC_VECTOR
      REAL*4 TOTAL_AMEREN_MARKET_REVENUES, ! USED BY AMEREN 12/03/01 MSG
     +       TOTAL_AMEREN_FUEL_EXPENSE,
     +       TOTAL_AMEREN_VARIABLE_EXPENSE
      REAL*4 TRANSFR_MARKET_COSTS(0:12),
     +       TRANSFR_MARKET_COSTS_MWH(0:12),
     +       TRANSFR_MARKET_REVS(0:12),
     +       TRANSFR_MARKET_REVS_MWH(0:12),
     +       TRANSFR_MONTHLY_WHOLESALE_COST(0:12),
...
C
      ...
      REAL*4 ASSET_ALLOCATION_LIST(30)
C
      INTEGER*2 YR,MO,ASSET_CLASS,RESULT_PTR
      INTEGER*2 R_YEAR
      REAL*4 ASSET_ALLOCATOR,
     +       ALLOCATED_DOLLAR_AMOUNT(0:12),
     +       ALLOCATED_QUANT_AMOUNT(0:12),
     +       ALLOCATED_FUEL_DOLLAR_AMOUNT(0:12),
     +       ALLOCATED_VOM_DOLLAR_AMOUNT(0:12)
      INTEGER*2 NUMBER_OF_COST_CLASSES,MAX_COST_CLASS_NUMBER
      SAVE MAX_COST_CLASS_NUMBER
      REAL TRANSFR_COST_QUANTITIES,
     +     TRANSFR_FUEL_COST_AMOUNT,
     +     TRANSFR_VAR_COST_AMOUNT
      INTEGER*2 TRANSFR_COST_CLASS_POINTR
      ALLOCATABLE :: TRANSFR_COST_QUANTITIES(:,:),
     +               TRANSFR_COST_CLASS_POINTR(:),
     +               TRANSFR_FUEL_COST_AMOUNT(:,:),
     +               TRANSFR_VAR_COST_AMOUNT(:,:)
      SAVE TRANSFR_COST_QUANTITIES,
     +     TRANSFR_COST_CLASS_POINTR,
     +     TRANSFR_FUEL_COST_AMOUNT,
     +     TRANSFR_VAR_COST_AMOUNT
      INTEGER*2 NUMBER_OF_REV_CLASSES,MAX_REV_CLASS_NUMBER
      SAVE MAX_REV_CLASS_NUMBER
      INTEGER*2 TRANSFR_REV_CLASS_POINTR
      REAL TRANSFR_REV_AMOUNT,TRANSFR_REV_QUANTITIES
      ALLOCATABLE :: TRANSFR_REV_AMOUNT(:,:),
     +               TRANSFR_REV_QUANTITIES(:,:),
     +               TRANSFR_REV_CLASS_POINTR(:)
      SAVE TRANSFR_REV_AMOUNT,
     +     TRANSFR_REV_QUANTITIES,
     +     TRANSFR_REV_CLASS_POINTR
C
      INTEGER*2 ALLOCATION_VECTOR,CLASS_POINTER
      REAL*4 ALLOCATION_VALUE(0:12),
     ...
     +       VECTOR_MONTHLY_DATA(12,LAST_AVAILABLE_MONTHLY_YEAR)
      ...
      CHARACTER*1 DATA_TYPE,VECTOR_TYPE*20,DUMMY_TYPE
      LOGICAL*1 VOID_LOGICAL,RETURN_ASSET_CLASS_LISTS
C
      REAL R_CLASS_REVENUES,
     +     R_CLASS_REV_QUANITIES,
     +     R_CLASS_FUEL_EXPENSES,
     +     R_CLASS_VOM_EXPENSES,
...
C
     ...
     +                                  TRANSFR_MONTHLY_WHOLE_VOM_COST) ! R(0:12) USE THIS FOR AMEREN
         ...
         YR = R_YEAR
C
C GET CLASS USING A VECTOR
C GET ALLOCATION FACTORS
C SETUP ARRAYS TO LATER SEARCH FOR THE AMONTS TO TRANSFER
C         
C
C ALLOCATE TO THE COST VALUES TOTAL COMPANY AND ASSET CLASSES
C
     ...
     +                                ASSET_CLASS_LIST(CLASS_POINTER)+1)
     ...
     +                     ASSET_CLASS_LIST(CLASS_POINTER) == -99.) EXIT
         ...
         IF(ALLOCATED(TRANSFR_FUEL_COST_AMOUNT))
     +         DEALLOCATE(TRANSFR_FUEL_COST_AMOUNT,
     +                    TRANSFR_VAR_COST_AMOUNT,
     +                    TRANSFR_COST_QUANTITIES,
     +                    TRANSFR_COST_CLASS_POINTR)
...
C                     
     ...
     +                                   ALLOCATED_VOM_DOLLAR_AMOUNT(MO)
               TRANSFR_COST_QUANTITIES(MO,RESULT_PTR) =
     +                                        ALLOCATED_QUANT_AMOUNT(MO)
            ENDDO
...
C
C REVENUE ALLOCTION TO CLASSES
C
         VOID_LOGICAL = RETURN_ASSET_CLASS_LISTS(
     +                                        TRANSFR_REV_CLASS_VECTOR,
     +                                        ASSET_CLASS_LIST,
     ...
     +                     ASSET_CLASS_LIST(CLASS_POINTER) == -99.) EXIT
         ...
         IF(ALLOCATED(TRANSFR_REV_AMOUNT))
     +                              DEALLOCATE(TRANSFR_REV_AMOUNT,
     +                                         TRANSFR_REV_QUANTITIES,
     +                                         TRANSFR_REV_CLASS_POINTR)
...
C                     
     ...
     +                                  + ALLOCATED_QUANT_AMOUNT(MO)
            ...
            ENDIF
            DO MO = 0, 12
               TRANSFR_REV_AMOUNT(MO,RESULT_PTR) =
     +                                       ALLOCATED_DOLLAR_AMOUNT(MO)
               TRANSFR_REV_QUANTITIES(MO,RESULT_PTR) =
     +                                        ALLOCATED_QUANT_AMOUNT(MO)
            ENDDO
...
C
     ...
     +                       R_ASSET_CLASS <= MAX_REV_CLASS_NUMBER) THEN
               ...
               IF(RESULT_PTR > 0) THEN
                  ...
                  R_CLASS_REV_QUANITIES =
     +                              TRANSFR_REV_QUANTITIES(0,RESULT_PTR)
               ENDIF
            ENDIF
            IF(R_ASSET_CLASS > 0 .AND. 
     +                      R_ASSET_CLASS <= MAX_COST_CLASS_NUMBER) THEN
               RESULT_PTR = TRANSFR_COST_CLASS_POINTR(R_ASSET_CLASS)
               IF(RESULT_PTR > 0) THEN
                  R_EXP_TRANSFERS_ARE_ACTIVE = .TRUE.
                  R_CLASS_FUEL_EXPENSES = 
     +                           TRANSFR_FUEL_COST_AMOUNT(0,RESULT_PTR)
                  R_CLASS_VOM_EXPENSES = 
     +                             TRANSFR_VAR_COST_AMOUNT(0,RESULT_PTR)
                  R_CLASS_EXP_QUANITIES =
     +                             TRANSFR_COST_QUANTITIES(0,RESULT_PTR)
               ENDIF
            ENDIF
         ENDIF
      RETURN
...
C***********************************************************************
C
      REAL*4 THE_RATIO_OF_A_TO_B,THE_PERCENT_OF_A_TO_B
      REAL*4 A,B
      REAL*4 NOT_AVAIL
      PARAMETER(NOT_AVAIL=-999999.)
C
...
C           LTD_STD_INTEREST = LTDINT(RUN_YEAR) + STDINT(2)
     ...
     +                                               LEASE_PAYMENT_LIMIT
            ...
            IF(INTEREST /= 0.) COVERAGE_DRIVER_RATIO=RTEMP/INTEREST
         ELSEIF(COVERAGE_RATIO == 4 .OR.
     +                                    COVERAGE_RATIO == 5) THEN
            INTEREST = LTD_STD_INTEREST + LTD_PS_RETIREMENT
            RTEMP = GOPINC + ITC_ITCAMT + OTHINC + INTINC +  
     +              DEP_AMORTIZATION +
     +              R_VARIABLE_VALUE(29) +  ! TXDEFP
     +              R_VARIABLE_VALUE(28)    ! TXNORM
            IF(INTEREST /= 0.) COVERAGE_DRIVER_RATIO=RTEMP/INTEREST
         ELSEIF(COVERAGE_RATIO == 6 .OR. 
     +                                    COVERAGE_RATIO == 7) THEN
               SRP_TOTAL_DEBT_SERVICE = R_VARIABLE_VALUE(39) + ! LTDINT(RUN_YEAR) +
     +                                  R_VARIABLE_VALUE(119) + ! LTDRET(RUN_YEAR)
     +                                  R_VARIABLE_VALUE(154)   ! PSRED(RUN_YEAR)
C NOTE OTNINC IS NET OF BLT_EXPENSES
