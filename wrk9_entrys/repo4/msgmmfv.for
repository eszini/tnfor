...
C
      CHARACTER*1 R_DATA_TYPE,IOS_MESSAGE*64
      INTEGER*2 R_VECTOR,R_VECTOR_IN
      REAL*4 R_DATA(AVAIL_DATA_YEARS),R_ANNUAL_DATA(AVAIL_DATA_YEARS)
      INTEGER*2 I,IREC,DELETE,LRECL/1024/
      INTEGER*4 IOS
      INTEGER LAST_UNIT_NUMBER_OPENED/82/,UNIT_NUM
      CHARACTER*1 DATA_TYPE,DUMMY
      CHARACTER*5 BASE_FILE_NAME,OVERLAY_FAMILY_NAME,ASSET_VECTOR_FILE
      CHARACTER*128 FILE_NAME
      CHARACTER*64 DESCRIPTION
      CHARACTER*256 BASE_FILE_DIRECTORY,OUTPUT_DIRECTORY
      CHARACTER*256 DATA_DRIVE
      CHARACTER*4 COMMENT
      REAL*4 VALUES_30(AVAIL_DATA_YEARS)
      LOGICAL*4 FILE_EXISTS
C DECLARATION FOR DBREAD COMMON BLOCK
      CHARACTER*2048 RECLN
C DECLARATION FOR LOCALS
      CHARACTER*13 FILE_TYPE/'Asset Vectors'/
      CHARACTER*2 INTRSTOL/'BC'/,R_ASSET_VEC_OL
      INTEGER*2 VECTOR_NO,MAX_BASE_VECTOR_NUM/0/,
     +          MAX_OVERLAY_VECTOR_NUM/0/,
     +          TEMP_VECTOR_NO
      INTEGER*2 TEMP_POINTER(:),BASE_VECTOR_POINTER(:),
     +          OVLY_VECTOR_POINTER(:)
      ALLOCATABLE :: TEMP_POINTER,BASE_VECTOR_POINTER,
     +          OVLY_VECTOR_POINTER
      SAVE BASE_VECTOR_POINTER,
     +     OVLY_VECTOR_POINTER
      CHARACTER*20 VECTOR_TYPE,R_VECTOR_TYPE
      LOGICAL*1 GET_VECTOR_TYPE/.TRUE./,DUPLICATE_VECTOR_FOUND,
     +          R_MONTHLY_VALUES_EXIST,
     +          MONTHLY_VALUES_EXIST
      SAVE MONTHLY_VALUES_EXIST
C
C MONTHLY MIDAS GOLD ADDITIONS 2/13/98
C
      REAL*4 MIDAS_MONTHLY_VALUES(60)
      CHARACTER*1 MIDAS_MONTHLY_DATA_UNITS(5)
      CHARACTER*4 MIDAS_LAST_MONTH(5)
      REAL*4 R_MIDAS_MONTHLY_VALUES(60),
     +       BASE_YEAR_VALUE,
     +       BASE_YEAR_VECTOR_VALUE
      SAVE BASE_YEAR_VECTOR_VALUE
      CHARACTER*1 R_MIDAS_MONTHLY_DATA_UNITS(5)
      INTEGER*2 R_MIDAS_LAST_MONTH(5)
      LOGICAL*1 CPL_ACTIVE,CPL_IS_ACTIVE
      SAVE CPL_IS_ACTIVE
      REAL*4 R_BASE_YEAR_VECTOR_VALUE
      CHARACTER*1 VECTOR_ACTIVE
      LOGICAL ARRAY_AVAILABLE
      INTEGER*2 MAX_VECTOR_NUM,INUNIT
      INTEGER IOS_BASE
      LOGICAL*1 BANGOR
...
C**********************************************************************
C
C             CONVERSION ROUTINE FOR MIDAS 2.X ASSET PARAMETERS AND
C             INTEREST VECTOR FILES TO MIDAS GOLD ASSET RATE AND
C             ASSET VECTOR FILES.
C                       COPYRIGHT (C) 1991
...
C**********************************************************************
C
C CONVERT THE ASSET-VECTOR FILE
      ENTRY FV_MAKEBIN
C**********************************************************************
     ...
     +                          ' File position ',IREC
               ...
               ENDDO
               WRITE(11,REC=IREC) DELETE,VECTOR_NO,DATA_TYPE,
     +                            VECTOR_TYPE,
     +                            VECTOR_ACTIVE,
     +                            BASE_YEAR_VALUE,
     +                            VALUES_30,
     ...
     +         " CHECK WARNING MESSAGES"
      ...
      RETURN
C
C OVERLAY THE ASSET-VECTOR FILE
C
C**********************************************************************
...
C
            IF(VECTOR_NO > 0 .AND. VECTOR_NO <= 10000) THEN
               IF(.NOT. (DELETE >= 8 .OR. VECTOR_ACTIVE == 'N')) THEN
C
C CHECK FOR DUPLICATE VECTORS
C
                  IF(TEMP_POINTER(VECTOR_NO) /= 0) THEN
                     IF(.NOT. DUPLICATE_VECTOR_FOUND) THEN
                        WRITE(4,*) 'In overlay file ',
...
C
     ...
     +                    ' in the vector with the description ',
     +                    trim(DESCRIPTION)
     ...
     +                    ' is not in the range 1 to 10000.'
      ...
      IF(MAX_OVERLAY_VECTOR_NUM > 0) THEN
         IF(ALLOCATED(OVLY_VECTOR_POINTER))
...
C**********************************************************************
         OPEN(UNIT_NUM,FILE=trim(OUTPUT_DIRECTORY())//INTRSTOL//
     +         "ASTVEC.BIN",ACCESS="DIRECT",STATUS="UNKNOWN",RECL=LRECL)
...
C**********************************************************************
C
C             FUNCTION TO READ VARIABLE UNIT PARAMETERS FOR MIDAS     
C                   COPYRIGHT (C) 1986                             
C                        M.S. GERBER & ASSOCIATES, INC.
C                              ALL RIGHTS RESERVED
C
...
C
      ENTRY GET_ASSET_VAR_TYPE(R_VECTOR_IN,R_DATA_TYPE,R_DATA,
     +                                                    R_VECTOR_TYPE)
C**********************************************************************
C
     ...
     +               'Asset vector ',R_VECTOR,' was requested,'
            CALL MG_CLEAR_LINE_WRITE(20,0,79,trim(SCREEN_MESSAGES),
     +                                                   ALL_VERSIONS,1)
            CALL MG_CLEAR_LINE_WRITE(21,0,79,
     +                        'but it is not in the Asset Vector File.',
     +                                                   ALL_VERSIONS,1)
            call error('VECTOR ERROR')
            er_message='stop requested from msgmmfv SIID210'
            call end_program(er_message)
         ENDIF 
         IOS = 0
         INQUIRE(UNIT=LAST_UNIT_NUMBER_OPENED,OPENED=FILE_EXISTS,
     +           NAME=DATA_DRIVE)
         IF(.NOT. FILE_EXISTS) CALL OPEN_ASSET_VECTOR_FILE(82)        
         READ(LAST_UNIT_NUMBER_OPENED,REC=IREC,IOSTAT=IOS) DELETE,
     +                                         VECTOR_NO,
     ...
     +                               ' is NOT in the Asset Vector file.'
               ...
               RETURN
            ELSE
               CALL IOSTAT_MSG(IOS,IOS_MESSAGE)
...
c            CALL LOCATE(20,2)
c            WRITE(6,1030) ' Asset vector mismatch.  Vector ',R_VECTOR,
c     +                 'was requested.  Vector ',VECTOR_NO,
c     +                 ' was found.'
...
C
      ...
      ELSE
         R_DATA = 0.
!         WRITE(SCREEN_MESSAGES,'(A,I10,A)')
!     +                        'Asset vector ',R_VECTOR,' was requested.'
!         CALL MG_CLEAR_LINE_WRITE(20,0,79,trim(SCREEN_MESSAGES),
!     +                                                   ALL_VERSIONS,1)
!         CALL MG_CLEAR_LINE_WRITE(21,0,79,
!     +                         'Base Asset Vector File was named NONE.',
!     +                                                   ALL_VERSIONS,1)
!         CALL ERROR('Print Screen and e-mail to msg@msgerber.com')
!         STOP ' '
      ENDIF
      RETURN
C**********************************************************************
      ENTRY GET_MONTHLY_ANNUAL_VALUES(R_VECTOR_IN,R_DATA_TYPE,
     +                                R_VECTOR_TYPE,R_DATA,
     +                                R_MIDAS_MONTHLY_VALUES,
     +                                R_MIDAS_MONTHLY_DATA_UNITS,
     +                                R_MIDAS_LAST_MONTH)
...
C
      INCLUDE 'SPINLIB.MON'
      USE SIZECOM
      INTEGER*2 IREC,DELETE,YEAR,LRECL/100/
      INTEGER*4 IOS
      INTEGER LAST_UNIT_NUMBER_OPENED/10/,UNIT_NUM
      CHARACTER*5 BASE_FILE_NAME,OVERLAY_FAMILY_NAME,ASSET_RATE_FILE
      CHARACTER*256 FILE_NAME,FILE_NAME2
      CHARACTER*50 COMMENT
      CHARACTER*40 DESCRIPTION
      CHARACTER*256 BASE_FILE_DIRECTORY,OUTPUT_DIRECTORY
      CHARACTER*256 DATA_DRIVE
      REAL*4 VALUES_12(12),VALUES_6(6),VALUES(18)
      LOGICAL*4 FILE_EXISTS
C DECLARATION FOR DBREAD COMMON BLOCK
      CHARACTER*1024 RECLN
      LOGICAL*1 SET_VALUES_5_2_VALUES_12_1,
     +          SET_VALUES_6_2_VALUES_12_1
C DECLARATION FOR LOCALS
      CHARACTER*11 FILE_TYPE/'Asset Rates'/
      CHARACTER*2 PARMASOL/'BC'/,R_PARMASOL
      EQUIVALENCE (VALUES(1),VALUES_12(1)),
     +            (VALUES(13),VALUES_6(1))
      LOGICAL*1 LAHEY_LF95
      CHARACTER*30 SCREEN_OUTPUT
C**********************************************************************
C
...
C**********************************************************************
C
C CONVERT THE ASSET RATES FILE
      ENTRY AR_MAKEBIN
C
      BASE_FILE_NAME = ASSET_RATE_FILE()
      FILE_NAME = trim(BASE_FILE_DIRECTORY())//
     +                             "ARB"//trim(BASE_FILE_NAME)//".DAT"
      DATA_DRIVE = OUTPUT_DIRECTORY()
     ...
     +                      ACCESS="DIRECT",STATUS="UNKNOWN",RECL=LRECL)
         ...
         IF(IOS /= 0) THEN
            ...
            CALL MG_CLEAR_LINE_WRITE(20,0,79,trim(RECLN),
     +                                                   ALL_VERSIONS,1)
         ...
         DO
            ...
            IF(IOS /= 0) EXIT
            RECLN = trim(RECLN)//',,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,'
            READ(RECLN,*,IOSTAT=IOS) DELETE,YEAR,VALUES_12,
     +                                                  COMMENT,VALUES_6
            IF(IOS /= 0) THEN
...
c         ENDFILE(11)
      ...
      RETURN
C
C OVERLAY THE ASSET RATES FILE
      ENTRY AR_MAKEOVL(OVERLAY_FAMILY_NAME)
C     NOTE: THE VARIABLE VALUE WAS ADDED 5/25/91 FOR INTEREST
C           CAPITALIZATION FOR THE MIDAS GOLD VERSION
      IF(LAHEY_LF95()) THEN
         SCREEN_OUTPUT = trim(FILE_TYPE)//'-'//OVERLAY_FAMILY_NAME
         CALL MG_LOCATE_WRITE(16,30,SCREEN_OUTPUT,ALL_VERSIONS,0)
      ELSE
     ...
     +                                      STATUS="UNKNOWN",RECL=LRECL)
      OPEN(10,FILE=trim(DATA_DRIVE)//"ARO"//
     +                              trim(OVERLAY_FAMILY_NAME)//".DAT")
      READ(10,*) DELETE
      IREC = 0
      DO
...
C
      LOGICAL IS_OPEN
      CHARACTER*1 R_DATA_TYPE,IOS_MESSAGE*64
      INTEGER*2 R_VECTOR,R_VECTOR_IN,D_YR
      REAL*4 R_DATA(AVAIL_DATA_YEARS),ANNUAL_AVERAGE
      INTEGER*2 I,IREC,DELETE,LRECL/2048/
      INTEGER*4 IOS
      INTEGER LAST_UNIT_NUMBER_OPENED/827/,UNIT_NUM
      CHARACTER*1 DATA_TYPE,DUMMY
      CHARACTER*5 BASE_FILE_NAME,OVERLAY_FAMILY_NAME,
     +            CAPITAL_RATES_FILE
      CHARACTER*40 LHS_DISTRIBUTION
      CHARACTER*128 FILE_NAME
      CHARACTER*64 DESCRIPTION
      CHARACTER*256 BASE_FILE_DIRECTORY,OUTPUT_DIRECTORY
      CHARACTER*256 DATA_DRIVE,ERR_MESSAGE
      CHARACTER*4 COMMENT
      INTEGER*2 MONTH_ENDING,GET_MONTH_NUMBER
      REAL*4 VALUES_30(0:AVAIL_DATA_YEARS)
      LOGICAL*4 FILE_EXISTS
C DECLARATION FOR DBREAD COMMON BLOCK
      CHARACTER*2048 RECLN
C DECLARATION FOR LOCALS
      CHARACTER*13 FILE_TYPE/'Capital Rates'/
      CHARACTER*2 INTRSTOL/'BC'/,R_ASSET_VEC_OL
      INTEGER*2 VECTOR_NO,MAX_BASE_VECTOR_NUM/0/,
     +          MAX_OVERLAY_VECTOR_NUM/0/
      INTEGER*2 TEMP_POINTER(:),BASE_VECTOR_POINTER(:),
     ...
     +     OVLY_VECTOR_POINTER
      CHARACTER*20 VECTOR_TYPE,R_VECTOR_TYPE
      LOGICAL*1 DUPLICATE_VECTOR_FOUND,
     +          CAPITAL_RATES_FILE_ACTIVE
      SAVE CAPITAL_RATES_FILE_ACTIVE
C
C MONTHLY MIDAS GOLD ADDITIONS 2/13/98
C
      REAL*4 MIDAS_MONTHLY_VALUES(120),
     +       MONTHLY_VALUES_IN(12,AVAIL_DATA_YEARS),
     +       MONTHLY_INPUT_BY(12,10)
      EQUIVALENCE (MIDAS_MONTHLY_VALUES(1),MONTHLY_INPUT_BY(1,1))
      CHARACTER*1 MIDAS_MONTHLY_DATA_UNITS(10)
      CHARACTER*4 MIDAS_LAST_MONTH(10)
      INTEGER*2 R_YEARS_TO_RETURN
      REAL*4 R_MIDAS_MONTHLY_VALUES(0:12,0:R_YEARS_TO_RETURN),
     +       BASE_YEAR_VECTOR_VALUE
      SAVE BASE_YEAR_VECTOR_VALUE
      CHARACTER*1 R_MIDAS_MONTHLY_DATA_UNITS(10)
      INTEGER*2 R_MIDAS_LAST_MONTH(10)
      REAL*4 R_BASE_YEAR_VECTOR_VALUE
      CHARACTER*1 VECTOR_ACTIVE
      LOGICAL ARRAY_AVAILABLE
      INTEGER*2 MAX_VECTOR_NUM,INUNIT
      INTEGER IOS_BASE
      LOGICAL*1 LAHEY_LF95,LHS_ACTIVE
      CHARACTER*30 SCREEN_OUTPUT
      INTEGER*2 SCENARIO_INDEX,GET_SCENARIO_INDEX
      REAL*4 GET_SCENARIO_BY_INDEX
      INTEGER*2 YR,MO
      INTEGER*2 YRS_2_RETURN/1/
      LOGICAL*1 R_VECTOR_FOUND
      REAL*4 R_MIDAS_MONTHLY_ANNUAL_VALUES(0:12)
      REAL*4 TEMP_MONTHLY_VALUES(12)
C**********************************************************************
C
C     MONTHLY CAPITAL COST RATES FOR MIDAS GOLD
C                       COPYRIGHT (C) 2003
C                 M.S. GERBER & ASSOCIATES, INC.
C                       ALL RIGHTS RESERVED
C
...
C**********************************************************************
     ...
     +                             "MRB"//trim(BASE_FILE_NAME)//".DAT"
      ...
      IF(FILE_EXISTS) THEN
         ...
         DATA_DRIVE = OUTPUT_DIRECTORY()
         OPEN(10,FILE=FILE_NAME)
         OPEN(11,FILE=trim(DATA_DRIVE)//"BCCAPRTE.BIN",
     +                      ACCESS="DIRECT",STATUS="REPLACE",RECL=LRECL)
         ALLOCATE(TEMP_POINTER(0:10000))
         TEMP_POINTER = 0
         MIDAS_MONTHLY_DATA_UNITS = 'A'
...
C 
     ...
     +                      ACCESS="DIRECT",STATUS="UNKNOWN",RECL=LRECL)
         INUNIT= 11   
         OPEN(12,FILE=trim(DATA_DRIVE)//"OLCAPRTE.BIN",
     +                      ACCESS="DIRECT",STATUS="REPLACE",RECL=LRECL)
      ELSE
         OPEN(12,FILE=trim(DATA_DRIVE)//"OLCAPRTE.BIN",
     +                      ACCESS="DIRECT",STATUS="OLD",RECL=LRECL)
      ENDIF
C
      OPEN(10,FILE=trim(DATA_DRIVE)//"MRO"//
     +                              trim(OVERLAY_FAMILY_NAME)//".DAT")
      ALLOCATE(TEMP_POINTER(0:10000))
      TEMP_POINTER = 0
      READ(10,*) DELETE
...
C**********************************************************************
         OPEN(UNIT_NUM,FILE=trim(OUTPUT_DIRECTORY())//INTRSTOL//
     +         "CAPRTE.BIN",ACCESS="DIRECT",STATUS="UNKNOWN",RECL=LRECL)
         LAST_UNIT_NUMBER_OPENED = UNIT_NUM
      RETURN
C
C**********************************************************************
...
C**********************************************************************
C
C            FUNCTION TO READ CAPITAL RATES AND ADJUST THEM
C                FOR LHS DISTRIBUTIONSF
C                         MIDAS GOLD     
C                     COPYRIGHT (C) 2003                             
C                M.S. GERBER & ASSOCIATES, INC.
C                     ALL RIGHTS RESERVED
...
C
     ...
     +                                   DESCRIPTION
            IF(IOS /= 0) EXIT
C            IF(DELETE >= 8 .OR. VECTOR_ACTIVE == 'N') THEN
C               CYCLE
C            ENDIF
C
C PROCESS THE DATA 
...
C               DO MO = 1, 12 
                  IF(ANY(MONTHLY_INPUT_BY(1:12,YR)== -999999.,1)) THEN
                     IF(MONTHLY_INPUT_BY(1,YR) == -999999.)
     +                         MONTHLY_INPUT_BY(1,YR) = VALUES_30(YR)
                     DO MO = 2, 12
                        IF(MONTHLY_INPUT_BY(MO,YR) == -999999.) THEN
                           MONTHLY_INPUT_BY(MO,YR) =
     +                                         MONTHLY_INPUT_BY(MO-1,YR)
                        ENDIF
                     ENDDO
                  ENDIF
            ENDDO
            DO YR = 1, AVAIL_DATA_YEARS 
               ...
               ELSEIF(MIDAS_MONTHLY_DATA_UNITS(D_YR) == 'L') THEN
                  MONTHLY_VALUES_IN(:,YR) = MONTHLY_INPUT_BY(:,D_YR)
                  VALUES_30(YR) = SUM(MONTHLY_VALUES_IN(1:12,YR))/12. 
C                  MONTH_ENDING=GET_MONTH_NUMBER(MIDAS_LAST_MONTH(YR))
               ELSEIF(MIDAS_MONTHLY_DATA_UNITS(D_YR) == 'M') THEN 
                  MONTHLY_VALUES_IN(:,YR) = VALUES_30(YR)
     +                                     * MONTHLY_INPUT_BY(:,D_YR)
                  VALUES_30(YR) = SUM(MONTHLY_VALUES_IN(1:12,YR))/12. 
               ELSEIF(MIDAS_MONTHLY_DATA_UNITS(D_YR) == 'T' .AND.
     +                                                   YR > 1) THEN 
                  IF(ANNUAL_AVERAGE /= 0.) THEN
                     MONTHLY_VALUES_IN(:,YR) = VALUES_30(YR) * 
     +                                     MONTHLY_VALUES_IN(:,YR-1)/
     +                                         ANNUAL_AVERAGE                
                  ELSE
                     MONTHLY_VALUES_IN(:,YR) = VALUES_30(YR) 
                  ENDIF
            ...
            ENDDO
C            TEMP_MONTHLY_VALUES(1:12) = MONTHLY_VALUES_IN(1:12,10)
C            DO YR = 10, AVAIL_DATA_YEARS
C               MONTHLY_VALUES_IN(1:12,YR) = TEMP_MONTHLY_VALUES(1:12)
C            ENDDO
C
...
C
            IF(LHS_ACTIVE) THEN
               DO YR = 1, AVAIL_DATA_YEARS
                  ...
                  DO MO = 1, 12
                     IF(YR < 10) THEN
                        IF(MIDAS_MONTHLY_DATA_UNITS(YR) == 'L' .AND.
     +                                         MO <= MONTH_ENDING) CYCLE
                     ENDIF
...
C WRITE RECORD
C
            WRITE(31123,REC=IREC,IOSTAT=IOS) DELETE,
     +                                       VECTOR_NO,
     +                                       DATA_TYPE,
     +                                       VECTOR_TYPE,
     +                                       VECTOR_ACTIVE,
...
C**********************************************************************
C
C             FUNCTION TO READ CAPITAL RATES FOR MIDAS GOLD     
C                     COPYRIGHT (C) 2003                             
C                M.S. GERBER & ASSOCIATES, INC.
C                     ALL RIGHTS RESERVED
C
...
C
C
      ENTRY GET_CAP_RATES_MONTHLY_VALUES(R_VECTOR_FOUND,
     +                                   R_VECTOR_IN,
     +                                   R_MIDAS_MONTHLY_VALUES,
     +                                   R_YEARS_TO_RETURN)
      ...
      IF(ARRAY_AVAILABLE) THEN
         IF(R_VECTOR > MAX_VECTOR_NUM) THEN
            WRITE(4,'(A,I4,A)')
     +               ' Capital Rates vector ',R_VECTOR,' was requested,'
     +               //'but it is not in the Capital Rates File.'
            RETURN 
         ENDIF 
         READ(31123,REC=IREC,IOSTAT=IOS) DELETE,
     +                                   VECTOR_NO,
     +                                   DATA_TYPE,
     +                                   VECTOR_TYPE,
     +                                   VECTOR_ACTIVE,
     ...
     +                                                   ALL_VERSIONS,1)
         ...
         IF(VECTOR_NO /= R_VECTOR) THEN
            WRITE(4,'(A,I4,A,I4,A)')
     +              ' Capital Rates vector mismatch.  Vector ',R_VECTOR,
     +               'was requested.  Vector ',VECTOR_NO,' was found.'
            RETURN
         ENDIF
C
...
C
         ...
         DO YR = 1, YRS_2_RETURN
            ...
            R_MIDAS_MONTHLY_VALUES(1:12,YR) =
     +                                        MONTHLY_VALUES_IN(1:12,YR)
         ENDDO
      ELSE
         WRITE(4,'(A,I4,A)')
     +               ' Capital Rates vector ',R_VECTOR,' was requested.'
     +        //' Capital Rates file was named NONE.'
      ...
      RETURN
C**********************************************************************
      ENTRY GET_CAP_RATES_4_CURRENT_YEAR(R_VECTOR_FOUND,
     +                                   R_VECTOR_IN,
     +                                   R_MIDAS_MONTHLY_ANNUAL_VALUES,
     +                                   R_YEARS_TO_RETURN)
C*********************************************************************
C
      YRS_2_RETURN = MIN(R_YEARS_TO_RETURN,AVAIL_DATA_YEARS)
