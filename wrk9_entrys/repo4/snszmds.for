...
!
!  LDMGT_ACTIVE = .FALSE. : BASE CASE
!  LDMGT_ACTIVE = .TRUE.  : LOAD MANAGEMENT SCENARIO
!
      INCLUDE 'SpinLib.MON'
      use kepcocom
      use trans_slope
...
!      SAVE POS_OF_HOURLY_FILE
!      IF(YEAR == 1) POS_OF_HOURLY_FILE = -9999
!
...
!
      USE_EXTERNAL_HOURLY_LOADS = USE_EXT_LOAD_SOURCE()
!      USE_EXTERNAL_HOURLY_LOADS = .TRUE.
!      OPC_LOSS_MULT = 1.0387139 ! 1/(1-.037271)
...
!
               IF(.NOT.SYSTEM_BASED_FORECAST) THEN
                  IF(DAY < 3) THEN
                     SYS_HIST_PK =
     +                          SYSTEM_HISTORICAL_PEAKS(HOUR,DAY,MONTH)
                  ELSE
                     SYS_HIST_PK =
...
!
               IF(DSM_CHECK1.LT.0) THEN
                  ...
                  OFF_DAY = 2
                  CALL DSM_WARN(YEAR,
     +                 PK_DAY,OFF_DAY,MONTH,END_POINT,DSM_CHECK1)
               ENDIF
               IF(DSM_CHECK2.LT.0) THEN
                  PK_DAY = 3
                  OFF_DAY = 1
                  CALL DSM_WARN(YEAR,
     +                  PK_DAY,OFF_DAY,MONTH,END_POINT,DSM_CHECK2)
               ENDIF
!
!
!
...
!
     ...
     +                 PK_DAY,OFF_DAY,MONTH,END_POINT,DSM_CHECK1)
               ...
               IF(DSM_CHECK3.LT.0) THEN
                  ...
                  OFF_DAY = 1
                  CALL DSM_WARN(YEAR,
     +                 PK_DAY,OFF_DAY,MONTH,END_POINT,DSM_CHECK3)
               ENDIF
            ENDIF
!
...
!            HOURS_IN_SEASON = FLOAT(HOURS_IN_PERIOD(MONTH))
!            DAYS_PER_MONTH = INT(HOURS_IN_SEASON/24.)
!            CAL_DAYS_PER_MONTH = DAYS_PER_MONTH
            LEAP_SLOPE = 1.0
            LEAP_INTERCEPT = 0.0
            LEAP_ENERGY = 0.0
...
! 3/31/93. GAT. PER MARK'S SUGGESTION: NO LEAP YEAR STUFF.
               IF( REMAIN < .001 .AND. APPLY_LEAP_YEAR)
     +                                           LEAP_MONTH = .TRUE.
     ...
     +            HISTORICAL_DAY_COUNT(2,MONTH) - HOLIDAYS(MONTH)
               ...
               IF(CHANGE_IN_WEEK_DAYS < 0) THEN ! REDUCE WEEK DAYS
                  ...
                  DAY = 0
                  DOWHILE( DAY < ABS(CHANGE_IN_WEEK_DAYS))
                     I = I - 1
...
!
!
!                  CHRONOLOGICAL_HOUR(HOUR_IN_FCLOD) =  HOUR_IN_MONTH
...
!           MIDAS GOLD MAINTAINS 365 DAYS/YEAR.
!           HOWEVER, LDE*.BIN CONTAINS 366
            IF(MONTH == 2 .AND. .NOT.
...
!      IF(HOUR_MARGINAL_AFTER_ENRG)
!     +                         CALL SORT_HOURS_AND_DAYS_BY_LOADS(FRCLOD)
     ...
     +                       LDMGT_ACTIVE) CALL STORE_CLASS_PEAK_NET_DSM
      ...
      RETURN
! 1000 FORMAT(I2,I2,I2,9X,I1,4X,12I5)
!***********************************************************************
!
...
!
         ...
         ELSE
            REF_FILE_NAME = trim(BSYRLOAD())//".B"//
     +                              LOAD_FILE_CHAR_EXT(HOURLY_LOAD_IN())
         ENDIF
     ...
     +                                             trim(REF_FILE_NAME)
         ...
         IF(LDE_EXISTS) THEN
            OPEN(10,FILE=REF_FILE_NAME,ACCESS="TRANSPARENT",
     +                                                     STATUS="OLD")
            READ(10,REC=2) REC_LENGTH
            CLOSE(10)
            OPEN(10,FILE=REF_FILE_NAME,ACCESS="DIRECT",
...
!************************************
      ...
      DAY_ADJ = 0
      OFFSET = 4
      IF(MOD(YEAR,4) .NE. 0 .AND. MONTH .GT. 2) DAY_ADJ = 1
      IF(MOD(YEAR,4) .EQ. 0) OFFSET  = 3
      ITEMP = INT((YEAR-1980)/4)
      ITEMP = ITEMP+YEAR-1983+DAYS(MONTH)-DAY_ADJ+DAY+OFFSET
      DAYWEK = MOD(ITEMP,7) + 1
      RETURN
      END
!************************************
...
!         IF(MONTHLY_HOLIDAYS(11) >= 2 .AND.
!         IF(MONTH == 11 .AND. DAY == VETERANS) DAYWEK = 8
!         IF(MONTHLY_HOLIDAYS(11) >= 1 .AND.
...
!***********************************************************************
!
      SUBROUTINE DAYS_BY_DAY_TYPE(YEAR,MONTH,DAYS_PER_MONTH,HOLIDAYS,
     +                               WEEK_DAYS,PEAK_DAYS,WEEK_END_DAYS)
!
!***********************************************************************
      INTEGER (kind=2) ::  YEAR
      INTEGER (kind=2) ::  MONTH
      INTEGER (kind=2) ::  WEEK_DAYS,PEAK_DAYS,WEEK_END_DAYS,HOLIDAYS,
     +                                             DAYS_PER_MONTH
      PEAK_DAYS = 4
...
!
      SUBROUTINE WEEKEND_DAYS(YEAR,MONTH,DAYS_PER_MONTH,WEEK_END_DAYS)
         ...
         CALL DAYWEEK(MONTH,INT2(1),YEAR,DAYWEK)
         WEEK_END_DAYS = 8
         IF(MONTH .EQ. 2) THEN
