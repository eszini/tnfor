...
!     SUBROUTINE DR_BOOTH
!     ENTRY PUT_TRANS_C_HEAT
!      REAL ::         LEFT(MAX_CL_UNITS),RIGHT(MAX_CL_UNITS),
!      REAL ::         BLK1_HEAT(MAX_CL_UNITS),BLK2_HEAT(MAX_CL_UNITS)
...
!     ENTRY EMISSIONS_BY_ALL_FUELS
!     ENTRY CREATE_EMISSIONS_BY_FUEL_TYPE
...
!
     ...
     +          HAVE_HOURLY_MC_AFTER_EL,CAL_HOURLY_MC_AFTER_EL,
     +          APPLY_TRANS_REV_TO_WHOLESALE,
     +          YES_RUN_MULTIAREA_TRANSACT,RUN_TRANSACT_THIS_MONTH,
     +          TRANSACT_ACTIVE_THIS_MONTH,
     +          GET_TRANSACT_C_STATUS,TRANSACT_C_ACTIVE,
     ...
     +     SRP_SEGMENT_COST(MAX_DISPATCH_BLOCKS),
     +     AVE_RUNNING_RATE(LOAD_CURVE_POINTS),
     +     MONTHLY_ECONOMY_BOUGHT,MONTHLY_ECONOMY_COST,
     +     MONTHLY_ECONOMY_SOLD,MONTHLY_ECONOMY_REVENUE,
     +     WABASH_TRANS_BUY_ENERGY,WABASH_TRANS_BUY_RATE,
     +     WABASH_TRANS_SELL_ENERGY,WABASH_TRANS_SELL_RATE,
     +     CNTR_WEIGHTED_CAPACITY(MAX_CONTRACTS),
     +     REMAINING_ENERGY_FOR_BLOCK(MAX_DISPATCH_BLOCKS),
     +     LAST_B_FOR_BLOCK(MAX_DISPATCH_BLOCKS),
...
!      INTEGER (kind=2) ::   RPS_INDEX_TRANSLATE
...
!     REAL FUEL_MIX_P,FUEL_MIX_S    ! REMOVED 4/28/93
     ...
     +          LAST_POINT,EM,ISTART,AVAILABLE_CONTRACT_NO,
     +          D_RATE,OLD_LAST_POINT,PEAK_MONTH,CONTRACT,BCM_NO,
     +          PERIOD_COUNTER,BEFORE_UPDATE_MONTH,SUR_SALE_NO,S_I,
     +          SURPLUS_SALE(MAX_CONTRACTS),DISPATCH_BLOCK,
     +          FUEL_ID,
     +          R_I,
     +          R_BLOCK,
...
!
     ...
     +     LEFT(MAX_CL_UNITS),RIGHT(MAX_CL_UNITS),
     +     BLK1_HEAT(MAX_CL_UNITS),BLK2_HEAT(MAX_CL_UNITS),
     +     MAINTENANCE_RATE(MAX_CL_UNITS),TEMPEA(2,MAX_CL_UNITS),
     +     HEAT_RATE_FACTOR(MAX_CL_UNITS),
     +     EFFECTIVE_CAPACITY(2,MAX_CL_UNITS),P_NUCLEAR_FUEL_COST,
     +     VAR_COST,
     +     CL_DISPATCH_COST,MAXIMUM_CAPACITY(MAX_CONTRACTS),
     +     MAXIMUM_ENERGY(MAX_CONTRACTS),
     +     TEMP_CNTR_VAR_COST,TEMP_CNTR_FIXED_COST,CUM_CONTRACT_CAP,
     +     SEASONAL_CONTRACT_CAPACITY,MINIMUM_ENERGY(MAX_CONTRACTS),
     +     MINIMUM_CAPACITY(MAX_CONTRACTS),
     +     MAX_RATCHET_CAPACITY(MAX_CONTRACTS),
     +     REMAIN_ANN_ENRG(MAX_CONTRACTS),
     +     MIN_RATCHET_CAPACITY(MAX_CONTRACTS),GET_VAR,
     +     TOTAL_MIN_RATCHET_ENERGY,CNTR_ENRG,
     +     MAXIMUM_ANNUAL_CAPACITY(MAX_CONTRACTS),MIN_BC_FIXED_COST,
...
! 090706. TEST.
!
!         WRITE(7010,*) CLK(),',',END_POINT,',',YEAR,',',ISEAS,',',
!     +                                         NUNITS,',','TOP_DR_BOOTH'
!
!
...
! MAIN UNIT WITH FUEL UN-AVAILABLE BY PASS THE UNIT
                  CL_UNITS_ACTIVE = .NOT. (I == NBLOK2)
...
!        MIN_ENRG_FROM_ENRG_ONLY_ENRG = 0.
      ...
      P_PUR_ENRG = PUR_ENRG_FROM_TRANSACT ! MODIFIED 1/20/98. GAT. FOR TVA
      PVARCOST = 0.
      PFIXCOST = 0.
...
!              VOID_LOGICAL = DUKE_CATAWBA_MCGUIRE_RESULTS(I,UNIT_ENRG,
!    +                                               RCOST,
!    +                                               VAR_COST*UNIT_ENRG,
!    +                                               UNIT_FIXED_COST)
!           ENDIF
...
! 11/10/94. GAT. MOVED FOR TVA. ! 11/6/97. GAT. CREATED MONTHLY EMISSIONS FOR TVA.
     ...
     +                                   BTU_TAX_ACTIVE)
            ENDIF
! 6/14/93. GAT. REPORT FOR DEBUGGING.
            IF(FUEL_INVENTORY_ACTIVE .AND.
...
! TAKE CONTRACT ENERGY OUT HERE SO THAT THE BALANCE = 0.
               ...
               ENDIF
!              3/13/93. GAT. 2 TIER ENERGY CONTRACTS
...
!
!
! 10/13/98. GAT. ADDED NEW VARIABLES FOR MONTHLY MIDAS
!
...
!
     ...
     +                                       CT_LOSSES)
         ENDIF
!        9/5/95. GAT. MOVED ANNUAL TO PROCOST BELOW PRODUCTION COUNTER
...
!      integer (kind=2), intent(inout) :: R_UNIT
!      integer (kind=2), intent(in)    :: R_BLOCK_NO

         R_UNIT = UNIT(R_BLOCK_NO)
      RETURN
!      END SUBROUTINE GET_UNIT_FROM_BLOCK
...
!
     ...
     +     BLEND_HEAT,PRIM_HEAT

      REAL :: FUEL_MIX_RATE
 
!  Conv entry to subs - can be deleted
!      REAL :: SOX/-99./,CO2/-99./,OTH3/-99./,OTH2/-99./,
!     +     NOX1/-99./,NOX2/-99./
...
!     SAVE SOX_BY_FUEL_TYPE,NOX_BY_FUEL_TYPE,
!    +     CO2_BY_FUEL_TYPE,OTH2_BY_FUEL_TYPE,
!    +     OTH3_BY_FUEL_TYPE,
!    +     SOX_BY_BLOCK,
!    +     NOX_BY_BLOCK,
...
!  There are no calls to 'DO_EMISSIONS_BY_FUEL_TYPE' from any other source
...
!
         ...
         CALL SAVE_EMISSIONS_BY_BLOCK(UNIT_NO)
!         CALL CALC_EMISSION_REPORT_GROUPS(UNIT_NO,PRIM_HEAT,'P')
...
!            WRITE(4,*) 'INSIDE EMIS BY ALL FUELS'
!            WRITE(4,*) 'UNIT NO, FUEL MIX RATE',UNIT_NO,FUEL_MIX_RATE
...
! END DATA DECLARATIONS
     ...
     +                              I,',',LODDUR(I),',',LPROB(I,CURRENT)
         ENDDO
!        DO I = 1, NBLOK2
!           WRITE(LAST_ELDC_NO,2000) END_POINT,',',YEAR,',',ISEAS,',',
!    +                              I,',',REMAINING_ENERGY_FOR_BLOCK(I),
!    +                              ',',LAST_B_FOR_BLOCK(I)
!        ENDDO
      RETURN
...
!
!            R_MONTHLY_GROUP_REPORT(0,MGI,1) =
!     +            R_MONTHLY_GROUP_REPORT(0,MGI,1) +
...
! GROSS MARGIN PER JANET. 5/3/00. VARIABLE 17.
...
!
!               F_MONTHLY_GROUP_REPORT(ANNUAL,MGI,1) =
!     +            F_MONTHLY_GROUP_REPORT(R_PEAK_MONTH,MGI,1) ! CAPACITY: NOTE USE PEAK MONTH
!
!               F_MONTHLY_GROUP_REPORT(ANNUAL,MGI,16) =
!     +            F_MONTHLY_GROUP_REPORT(R_PEAK_MONTH,MGI,16) ! EQVIVALENT CAPACITY: NOTE USE PEAK MONTH
!
! 092206. TOOK UNITS CONVERSIOS OUT.
!
               F_MONTHLY_GROUP_REPORT(ANNUAL,MGI,2) =
     +               F_MONTHLY_GROUP_REPORT(ANNUAL,MGI,2) ! FIXED OM
!     +            F_MONTHLY_GROUP_REPORT(ANNUAL,MGI,2)/1000000. ! FIXED OM
               IF(F_MONTHLY_GROUP_REPORT(ANNUAL,MGI,3) > 0.001) THEN
                  F_MONTHLY_GROUP_REPORT(ANNUAL,MGI,4) =
     +               F_MONTHLY_GROUP_REPORT(ANNUAL,MGI,4)/ ! AVE HR
     +                       F_MONTHLY_GROUP_REPORT(ANNUAL,MGI,3)
               ELSE
...
!
               F_MONTHLY_GROUP_REPORT(ANNUAL,MGI,3) =
     +            F_MONTHLY_GROUP_REPORT(ANNUAL,MGI,3) ! ENERGY
!
               F_MONTHLY_GROUP_REPORT(ANNUAL,MGI,5) =
     +            F_MONTHLY_GROUP_REPORT(ANNUAL,MGI,5)/1000000. ! FUEL
               F_MONTHLY_GROUP_REPORT(ANNUAL,MGI,6) =
     +            F_MONTHLY_GROUP_REPORT(ANNUAL,MGI,6)/1000000. ! VARIABLE OM
!
               F_MONTHLY_GROUP_REPORT(ANNUAL,MGI,12) =
