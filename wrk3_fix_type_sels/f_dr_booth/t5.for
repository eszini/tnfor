...
!                       UNITCAP(GIBSON_BACKUP_POINTER) = UNITCAP(UNITNO)
                        UNIT_CAP_AFTER_LOSSES(GIBSON_BACKUP_POINTER) =
     +                                     UNIT_CAP_AFTER_LOSSES(UNITNO)
!                        ENRG = EAVAIL(GIBSON_BACKUP_POINTER) * BLK_MW *
!     +                      (1.-MAINTENANCE_RATE(GIBSON_BACKUP_POINTER))
                        IF(BLKNO(I) <= 1) THEN
C
...
!
!     DO DISPATCH_BLOCK = NBLOK,1,-1
!     DO DISPATCH_BLOCK = 1, NBLOK
      DO
         IF(ALLOCATE_FUEL_RIGHT_TO_LEFT) THEN
...
!                     BLK1_HEAT(I) = BLK1_HEAT(I) + 10000. * BLK1_LOSSES
!                     BLK2_HEAT(I) = BLK2_HEAT(I) + 10000. *
                     BLK1_HEAT(I) = BLK1_HEAT(I) + AHR * BLK1_LOSSES
                     BLK2_HEAT(I) = BLK2_HEAT(I) + AHR *
     +                                      (CL_LOSSES(I) - BLK1_LOSSES)
                  ELSE
!                     BLK2_HEAT(I) = BLK2_HEAT(I) + 10000. * CL_LOSSES(I)
                     BLK2_HEAT(I) = BLK2_HEAT(I) + AHR * CL_LOSSES(I)
                  ENDIF
!                  MMBTUS = MMBTUS + 10000.*CL_LOSSES(I)
...
C      INTEGER (kind=2) ::   ISTART,ISTOP,I
...
C
      CALL RETURN_ISTART_ISTOP(ISTART,ISTOP)
      BASE = LODDUR(1)
