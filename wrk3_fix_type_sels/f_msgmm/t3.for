...
!     ! leading space needed
      ...
      INTEGER (kind=2) ::  SAVE_CLASS_SALES_UNIT,
     +          CLASS_SALES_UNIT
...
!
      ...
      INTEGER (kind=2) ::  INTRA_ASSET_CLASS_ALLOCATION
...
!
     ...
     +          YES_RPS_CALC
      ...
      CHARACTER (len=32) ::  INTRA_COMPANY_TRANSACTION*1
...
!     ALLOCATABLE :: REVENUES,
     ...
     +          TEMP_L,GET_STATE_RPS_REQS
      ...
      INTEGER (kind=2) ::  CLASS_POINTER,SALES_UNIT/3458/,DELETE,
...
!
     ...
     +       R_COMPETITIVE_LOSS(9)
      REAL (kind=4) ::  R_FUEXP
      REAL (kind=4) ::  R_PREXP,R_OPEXP,R_MNEXP,R_OTHER1,R_OTHER2,
...
! ADDITIONAL SECTION
     ...
     +       BU_FUEL_REV_CASH_JAN
      INTEGER (kind=2) ::  SC_CALCULATION_START_YR,
     +          NC_CALCULATION_START_YR
      CHARACTER (len=1) ::  DEF_CLASS_TYPE
      REAL (kind=4) ::  NC_CALC_BASE_FUEL_FACTOR,NC_CALC_EMF_FACTOR,
...
!
      ...
      INTEGER (kind=2) ::  R_MASTER_CLASS_LIST(0:*)
      INTEGER (kind=2) ::  RC_REVENUE_FORECAST_HEADER
!
...
!
      REAL (kind=4) ::  R_CPL_FUEL_REVENUES,
...
!
      INCLUDE 'MTHNMCOM.MON'
      INTEGER (kind=2) ::  SALES_CLASSIFICATION,CPL_SALES_CLASS
      CHARACTER (len=*) :: R_CPL_JURISDICTION
!
...
! DETAILED REPORT OVERHEAD
     ...
     +          DYNAMIC_JURIS_ALLOC_ACTIVE,DYNAMIC_JURIS_ALLOC
      INTEGER (kind=2) ::  DYNAMIC_JURIS_RPT_HEADER
      INTEGER (kind=2) ::  DYNAMIC_JURIS_UNIT/0/
...
! DECLARATION FOR CLASS FORECASTS
      ...
      CHARACTER (len=32) ::  CLASS_NAME,ENERGY_UNITS*3,TEMP_CLASS_NAME
      CHARACTER (len=16) ::
     +    FILE_TYPE/'RPS State Demand'/ ! leading space needed
      INTEGER (kind=2) ::  SAVE_CLASS_SALES_UNIT,
     +          CLASS_SALES_UNIT
      INTEGER (kind=2) ::  BC_TABLE_NUMBER,OL_TABLE_NUMBER
      INTEGER (kind=2) ::  R_NUMBER_OF_TABLES
...
! DECLARATION FOR CLASS FORECASTS
      ...
      CHARACTER (len=32) ::  CLASS_NAME,ENERGY_UNITS*3,TEMP_CLASS_NAME
      CHARACTER (len=16) ::
     +    FILE_TYPE/'Thermal Retrofit     '/ ! leading space needed
      INTEGER (kind=2) ::  SAVE_CLASS_SALES_UNIT/0/,
     +          CLASS_SALES_UNIT
...
!
      ...
      INTEGER (kind=2) ::  IREC
      INTEGER (kind=2) ::  LRECL/328/ ! ADDED 20 VARIABLES ON 3/18/07.
!      INTEGER IOS
!
     ...
     +            TRAN_EXP_OWNER
      ...
      LOGICAL (kind=4) ::     TRAN_EXP_FILE_EXISTS/.FALSE./,
...
!     +            SET_HOUR_LONG_TRAN_EXP_PARAMS,
     ...
     +            R_CONGESTION_QUANT
      ...
      LOGICAL (kind=1) ::    TRANS_GROUP_ACTIVE_SWITCH,
...
!
     ...
     +          R_RPS_RESOURCE_COUNT
      REAL (kind=4) ::  RPS_SUPPLY_CURVES(:,:,:)
      REAL (kind=4) ::   ! ST, TECH, MAX RESOURCES
...
!
     ...
     +            R_NUMBER_OF_CO2_PARAM
      ...
      LOGICAL (kind=4) ::     FILE_EXISTS/.FALSE./
      LOGICAL (kind=4) ::     CO2_PARAM_FILE_EXISTS/.FALSE./,
